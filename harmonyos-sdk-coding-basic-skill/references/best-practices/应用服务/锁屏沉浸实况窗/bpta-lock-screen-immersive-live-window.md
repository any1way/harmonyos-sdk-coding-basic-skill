...
content_hash: sha256:83b1071e3eaeaee71f3e77fbc8bd8e7d01d1f658ad0b11f6e958c48549792de9
---
## 概述

在实际应用开发中，实时信息的高效呈现始终是提升用户体验的关键。实况窗作为高效的交互组件，有助于用户聚焦并迅速查看、处理任务，具备时段性、时效性、变化性的特征。锁屏沉浸实况窗能够详细展示应用的实时活动状态，将重要信息呈现在锁屏界面上，使用户一目了然，无需解锁屏幕进入应用即可获取最新的活动状态，尤其适合于实时性要求高，需要用户及时了解状态的场景，如动态显示网约车位置的出行打车场景、实时更新外卖进度的即时配送场景、以及在锁屏界面呈现电子登机牌的航班场景等。通过[Live View Kit（实况窗服务）](<../../../harmonyos-guides/应用服务/Live View Kit（实况窗服务）/live-view-kit-guide.md>)协助开发者快速集成实况窗功能，能轻松实现锁屏沉浸式的展示效果。

**图1** 用户获取实时信息界面

![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/7f/v3/e1QaIb8wTMO9v5Jys7jYjQ/zh-cn_image_0000002484414413.png?HW-CC-KV=V1&HW-CC-Date=20260612T020700Z&HW-CC-Expire=86400&HW-CC-Sign=8257CB9EAA17BB62FB815755A24AA4300CC2AE8731E8B75C9D756CE6C75D6405 "点击放大")
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/13/v3/9wE9_j5uTyagW94u72D8HA/zh-cn_image_0000002484415005.gif?HW-CC-KV=V1&HW-CC-Date=20260612T020700Z&HW-CC-Expire=86400&HW-CC-Sign=0C188ECF64D0F12BF84E902D71C96BA3C89BFCFABF5B55CA49B1ED9A7ED3A4DA "点击放大")

锁屏沉浸实况窗的创建依赖于实况窗功能，用户需要点击实况窗展开完整锁屏沉浸实况窗卡片。因此创建锁屏沉浸实况窗的应用需要申请实况窗权限和锁屏沉浸实况窗权限，详情请参考[Live View Kit（实况窗服务）开发准备](<../../../harmonyos-guides/应用服务/Live View Kit（实况窗服务）/开发准备/liveview-preparations.md>)。
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/3a/v3/nI-a0USETFeb1gD_mGfGXQ/zh-cn_image_0000002484291273.gif?HW-CC-KV=V1&HW-CC-Date=20260612T020700Z&HW-CC-Expire=86400&HW-CC-Sign=EA7C5CF2FE390756E0AA895FDAF003E9A0FB1F42DFCF6B75645CB5297304D83B "点击放大")
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/fe/v3/Xt92RmoNTSG0jyZzG5VHOg/zh-cn_image_0000002309310946.png?HW-CC-KV=V1&HW-CC-Date=20260612T020700Z&HW-CC-Expire=86400&HW-CC-Sign=DAEB78DAA8ABECFC8AF199DB4418BD7D42D621C8C03467B83E6E2A121BB1C89B "点击放大")

锁屏沉浸实况窗的创建和更新依赖于[Live View Kit（实况窗服务）](<../../../harmonyos-guides/应用服务/Live View Kit（实况窗服务）/live-view-kit-guide.md>)，具体流程如下：

1. 应用主进程创建实况窗：应用主进程通过调用[Live View Kit（实况窗服务）](<../../../harmonyos-guides/应用服务/Live View Kit（实况窗服务）/live-view-kit-guide.md>)创建实况窗。
2. 关联实况窗和锁屏沉浸实况窗：在创建实况窗时，需在参数中传入[LiveViewLockScreenExtensionAbility](<../../../harmonyos-references/Live View Kit（实况窗服务）/ArkTS API/LiveViewLockScreenExtensionAbility/liveview-lock-screen-ability.md>)名称，以便将实况窗与锁屏沉浸实况窗关联。此步骤需调用[Live View Kit（实况窗服务）](<../../../harmonyos-guides/应用服务/Live View Kit（实况窗服务）/live-view-kit-guide.md>)中的liveViewManager模块实现，具体实现方法请参阅[开发实况窗场景](<../../../harmonyos-guides/应用服务/Live View Kit（实况窗服务）/开发实况窗场景/liveview-scenes.md>)。
...
通过LiveView对象与[LiveViewLockScreenExtensionAbility](<../../../harmonyos-references/Live View Kit（实况窗服务）/ArkTS API/LiveViewLockScreenExtensionAbility/liveview-lock-screen-ability.md>)关联，并通过回调创建锁屏沉浸实况窗，以实现锁屏沉浸实况窗的展示。

### 开发步骤

在创建实况窗时，需在LiveView对象中指定[LiveViewLockScreenExtensionAbility](<../../../harmonyos-references/Live View Kit（实况窗服务）/ArkTS API/LiveViewLockScreenExtensionAbility/liveview-lock-screen-ability.md>)的名称，以便系统能够正确关联并渲染锁屏沉浸式实况窗。

1. 将[LiveViewLockScreenExtensionAbility](<../../../harmonyos-references/Live View Kit（实况窗服务）/ArkTS API/LiveViewLockScreenExtensionAbility/liveview-lock-screen-ability.md>)的名称写入创建实况窗时创建的liveView对象中。
...
2. 在应用module.json5中配置[LiveViewLockScreenExtensionAbility](<../../../harmonyos-references/Live View Kit（实况窗服务）/ArkTS API/LiveViewLockScreenExtensionAbility/liveview-lock-screen-ability.md>)的名称。
...
3. 在[LiveViewLockScreenExtensionAbility](<../../../harmonyos-references/Live View Kit（实况窗服务）/ArkTS API/LiveViewLockScreenExtensionAbility/liveview-lock-screen-ability.md>)的onSessionCreate()方法中完成锁屏沉浸实况窗页面的创建。
...
在通过[LiveViewLockScreenExtensionAbility](<../../../harmonyos-references/Live View Kit（实况窗服务）/ArkTS API/LiveViewLockScreenExtensionAbility/liveview-lock-screen-ability.md>)生命周期回调创建锁屏沉浸实况窗后，为了满足用户及时获取信息更新的需求，该实况窗需要实时更新。本章节将介绍如何实现锁屏沉浸实况窗的数据实时更新。

**图5** 锁屏沉浸实况窗实时更新

![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/97/v3/N9ka1hYOTEWsfnCTrkV_PQ/zh-cn_image_0000002484410785.gif?HW-CC-KV=V1&HW-CC-Date=20260612T020700Z&HW-CC-Expire=86400&HW-CC-Sign=85EA4A1592A90A646D82266FCF04F2E28F9248B73276C5255F4A18D6CB365CC3 "点击放大")
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/56/v3/8joE0zg1SAGgplAnPvzbRg/zh-cn_image_0000002343349997.png?HW-CC-KV=V1&HW-CC-Date=20260612T020700Z&HW-CC-Expire=86400&HW-CC-Sign=C760F99DC5676BA8DFC99DC0B8DAD002539CA9626E60FD7BCE76A96D524231F7 "点击放大")
...
2. 发布公共事件传递更新数据到锁屏沉浸实况窗。
   ...
   应用在主页面中通过[commonEventManager.publish()](<../../../harmonyos-references/基础功能/Basic Services Kit（基础服务）/ArkTS API/进程线程通信/@ohos.commonEventManager (公共事件模块)/js-apis-commoneventmanager.md#commoneventmanagerpublish>)接口发布公共事件'live\_view\_lock\_screen'，并在[CommonEventPublishData](<../../../harmonyos-references/基础功能/Basic Services Kit（基础服务）/ArkTS API/进程线程通信/commonEvent/CommonEventPublishData/js-apis-inner-commonevent-commoneventpublishdata.md>)的parameters属性中附带锁屏沉浸实况窗的创建和更新数据。
...
3. 订阅公共事件更新锁屏沉浸实况窗。
   ...
   锁屏沉浸实况窗进程在[LiveViewLockScreenExtensionAbility](<../../../harmonyos-references/Live View Kit（实况窗服务）/ArkTS API/LiveViewLockScreenExtensionAbility/liveview-lock-screen-ability.md>)中，使用[commonEventManager.createSubscriber()](<../../../harmonyos-references/基础功能/Basic Services Kit（基础服务）/ArkTS API/进程线程通信/@ohos.commonEventManager (公共事件模块)/js-apis-commoneventmanager.md#commoneventmanagercreatesubscriber>)接口创建主页面创建的公共事件'live\_view\_lock\_screen'的订阅者，通过[AppStorage（应用全局的UI状态存储）](<../../../harmonyos-guides/应用框架/ArkUI（方舟UI框架）/UI开发 (ArkTS声明式开发范式)/学习UI范式状态管理/状态管理（V1）/管理应用拥有的状态/AppStorage：应用全局的UI状态存储/arkts-appstorage.md>)将主页面传递的数据传入LockScreenPage.ets，以实现锁屏沉浸实况窗的创建和更新。
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/7/v3/LTUZRkwIQUWvAcJO75sJ7g/zh-cn_image_0000002451295374.png?HW-CC-KV=V1&HW-CC-Date=20260612T020700Z&HW-CC-Expire=86400&HW-CC-Sign=5E077CBDF3BC4D1ED1F74E27D13ABAFC0AAD2BA63E90276B2FFEFC89311F548F "点击放大")

为了适配不同尺寸的实况卡片和多样化的设备形态，沉浸式实况展示应采用自适应的多断点布局方案，以确保在各种产品上能够实现自适应布局。可以参考[断点](../../一次开发，多端部署/多设备界面开发/界面布局响应式变化/响应式布局/bpta-multi-device-responsive-layout.md#section1532120147301)。

锁屏沉浸实况窗在不同设备形态上的布局存在差异，主要体现在手机折叠态与展开态两种显示模式中。在手机展开状态下，锁屏沉浸实况窗的高宽比会减少，页面变得更宽。然而，无论是在哪种形态下，横断点都属于sm断点（[断点的定义](../../一次开发，多端部署/多设备界面开发/界面布局响应式变化/响应式布局/bpta-multi-device-responsive-layout.md#section186821126131515)），因此需要使用基于高宽比的纵断点进行区分，以实现多适配。

### 开发步骤

1. 在[LiveViewLockScreenExtensionAbility](<../../../harmonyos-references/Live View Kit（实况窗服务）/ArkTS API/LiveViewLockScreenExtensionAbility/liveview-lock-screen-ability.md>)中注册页面布局变化的监听器。
...
当应用接入锁屏沉浸实况窗后，因需实时更新锁屏沉浸实况窗页面，可能会导致功耗上升。其优化策略是采用智能后台更新机制，仅在应用进入后台时触发实况窗的刷新及数据同步。可使用[appManager.getRunningProcessInformation()](<../../../harmonyos-references/Ability Kit（程序框架服务）/ArkTS API/通用能力的接口(推荐)/@ohos.app.ability.appManager (应用管理)/js-apis-app-ability-appmanager.md#appmanagergetrunningprocessinformation>)接口来获取当前应用运行进程的相关信息，并通过返回的[ProcessInformation](<../../../harmonyos-references/Ability Kit（程序框架服务）/ArkTS API/接口依赖的元素及定义/application/ProcessInformation/js-apis-inner-application-processinformation.md>)模块中的state来判断当前进程的运行状态。
