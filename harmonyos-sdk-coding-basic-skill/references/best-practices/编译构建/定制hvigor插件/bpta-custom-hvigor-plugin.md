...
scraped_at: 2026-06-12T10:18:37+08:00
doc_updated_at: 2026-06-02
content_hash: sha256:ffa0490557de4de81dba50ae7bc6ad1899236dd4ae5b6c81d30a94c8150292f6
---
## 概述
...
* **[hvigor](../../../harmonyos-guides/构建应用/概述/ide-hvigor.md)**：基于TS实现的构建任务编排工具，主要提供任务管理机制，包括任务注册编排、工程模型管理、配置管理等关键能力。
* **[hvigor-ohos-plugin](../../../harmonyos-guides/构建应用/概述/构建系统生命周期/ide-hvigor-life-cycle.md#section199818016213)**：hvigor默认提供的构建插件，利用hvigor的任务编排机制实现应用/元服务构建任务流的执行，完成HAP/App的构建打包，应用于应用/元服务的构建。
* **[Task](../../../harmonyos-guides/构建应用/概述/构建系统生命周期/ide-hvigor-life-cycle.md#section194191858161220)**：即构建任务，作为hvigor构建过程中的基本工作单元，构建项目的具体工作由Task描述表达，比如源码编译任务，打包任务或签名任务等。每一种任务的执行逻辑由plugin插件提供。
* 编译产物：即工程/模块编译后的目标，是项目打包生成的用于依赖或运行的包文件，包括[HAP](../../../harmonyos-guides/基础入门/开发基础知识/应用程序包基础知识/应用程序包开发与使用/HAP/hap-package.md)（应用安装运行的基本单位）、[HAR](../../../harmonyos-guides/基础入门/开发基础知识/应用程序包基础知识/应用程序包开发与使用/HAR/har-package.md)（静态共享包）、[HSP](../../../harmonyos-guides/基础入门/开发基础知识/应用程序包基础知识/应用程序包开发与使用/HSP/in-app-hsp.md)（动态共享包）以及App（可上架的完整应用程序）等多种类型。

## 实现原理

定制hvigor插件，就是在编译构建的过程中插入开发者需要的自定义任务，将这些自定义任务抽象后封装成可复用的部分，通过输出plugin插件的目标形式，实现编译构建个性化逻辑的复用和共享分发。

![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/18/v3/oN4HkuVsSYeOLVgotbzQzQ/zh-cn_image_0000002229335981.png?HW-CC-KV=V1&HW-CC-Date=20260612T021834Z&HW-CC-Expire=86400&HW-CC-Sign=D6E675A3C7F4DEF08980A658352AB01B30D65AB8D418DF39CE5783053A9F6F2F "点击放大")
...
* [基于hvigorfile脚本开发](../../../harmonyos-guides/构建应用/扩展构建能力/开发Hvigor插件/ide-hvigor-plugin.md#section552855418188)
* [基于typescript项目开发](../../../harmonyos-guides/构建应用/扩展构建能力/开发Hvigor插件/ide-hvigor-plugin.md#section1825121193616)
...
下文的场景实例采用[基于typescript项目开发](../../../harmonyos-guides/构建应用/扩展构建能力/开发Hvigor插件/ide-hvigor-plugin.md#section1825121193616)的方法描述，开发者也可以将插件逻辑代码直接写于hvigorfile.ts中，切换为[基于hvigorfile脚本开发](../../../harmonyos-guides/构建应用/扩展构建能力/开发Hvigor插件/ide-hvigor-plugin.md#section552855418188)的实现方式。

定制hvigor插件涉及的相关能力，可查阅[扩展构建API](../../../harmonyos-guides/构建应用/扩展构建能力/扩展构建API/ide-hvigor-apis.md)。
...
1. [初始化typescript项目](../../../harmonyos-guides/构建应用/扩展构建能力/开发Hvigor插件/ide-hvigor-plugin.md#section1243715533156)：配置npm环境，安装typescript依赖，新建一个npm项目做为插件的工程。
2. [开发插件](../../../harmonyos-guides/构建应用/扩展构建能力/开发Hvigor插件/ide-hvigor-plugin.md#section627771916612)：对插件项目配置HarmonyOS镜像仓库，安装hvigor开发依赖，然后进行插件开发，以下是该场景的插件逻辑实现。
...
3. 共享插件：有两种方式可选。
   * [发布插件](../../../harmonyos-guides/构建应用/扩展构建能力/开发Hvigor插件/ide-hvigor-plugin.md#section855411147)：插件遵循npm发布规范，将其打包后发布到公共或自建的镜像仓库中。
   * 共享离线包：将插件工程压缩打包后分享出去。
4. [使用插件](../../../harmonyos-guides/构建应用/扩展构建能力/开发Hvigor插件/ide-hvigor-plugin.md#section60171414358)：在工程中的hvigor/hvigor-config.json5文件"dependencies"节点下添加插件依赖，例如"@bpta/custom\_plugin": "1.0.0"，然后执行DevEco Studio菜单File -> Sync and Refresh Project进行工程同步后，在模块中的hvigorfile.ts导入并使用插件方法。参考如下：
...
5. 执行Build -> Make Module编译，编译产物的文件名被修改为“name-version-author.har”的组成形式，同时生成路径从default目录改到了target目录下。结果如下图：
   ...
   ![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/81/v3/chX3qvDcRku7uA2-oGYp5A/zh-cn_image_0000002229335985.png?HW-CC-KV=V1&HW-CC-Date=20260612T021834Z&HW-CC-Expire=86400&HW-CC-Sign=8BE7AA7FF1AFE48D6BC1B35A9367E69230FBA016EACE7CD8EEF40D207FDE45DA "点击放大")
