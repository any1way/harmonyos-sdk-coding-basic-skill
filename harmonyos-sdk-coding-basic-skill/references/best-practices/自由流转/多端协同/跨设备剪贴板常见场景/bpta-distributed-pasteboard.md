...
scraped_at: 2026-06-12T10:12:46+08:00
doc_updated_at: 2026-05-18
content_hash: sha256:f0c8da1b8fce5859029290b9337d436bfbe72b223b0242d4c2f5e934bebf2a7f
---
## 概述
...
详细的使用限制可参考跨设备剪贴板[约束与限制](../跨设备剪贴板/bpta-distributed-pasteboard-cast.md#section42331744172619)。

**关键技术**

在开发跨设备剪贴板的功能时，系统将自动完成跨设备的数据传递，应用可根据实际需求，接入跨设备剪贴板，完成跨设备剪贴板的开发，具体运作机制可参考：跨设备剪贴板[运作机制](../跨设备剪贴板/bpta-distributed-pasteboard-cast.md#section111721843182510)。

## 开发流程

跨设备剪贴板的开发流程主要包括以下两个步骤：

1. 设备A复制数据，写入剪贴板服务：将需要存入系统剪贴板的数据转换为系统剪贴板可接受的通用格式，调用[SystemPasteboard.setData()](<../../../../harmonyos-references/基础功能/Basic Services Kit（基础服务）/ArkTS API/数据文件处理/@ohos.pasteboard (剪贴板)/js-apis-pasteboard.md#setdata9>)方法，将数据写入系统剪贴板。
2. 设备B粘贴数据，读取剪贴板内容：可以[使用粘贴控件](../../../../harmonyos-guides/系统/安全/程序访问控制/使用安全控件/使用粘贴控件/pastebutton.md)获得临时授权直接读取剪贴板数据。或申请系统剪贴板权限[ohos.permission.READ\_PASTEBOARD](../../../../harmonyos-guides/系统/安全/程序访问控制/应用权限管控/应用权限列表/受限开放权限/restricted-permissions.md#ohospermissionread_pasteboard)后，使用[@ohos.pasteboard (剪贴板)](<../../../../harmonyos-references/基础功能/Basic Services Kit（基础服务）/ArkTS API/数据文件处理/@ohos.pasteboard (剪贴板)/js-apis-pasteboard.md#systempasteboard>)接口读取剪贴板数据。具体开发步骤请参考[访问剪贴板内容](<../../../../harmonyos-guides/系统/基础功能/Basic Services Kit（基础服务）/剪贴板服务/申请访问剪贴板权限/get-pastedata-permission-guidelines.md#访问剪贴板内容>)。
...
[](https://contentcenter-videovali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_300_3/90/v3/o-xPXcJ3Ri-EKsB87PSRfg/zh-cn_media_0000002429043485.mp4?HW-CC-KV=V1&HW-CC-Date=20260612T021238Z&HW-CC-Expire=86400&HW-CC-Sign=7B88EDB820807C3B683EF8AE1C74A95F0C3E8A908123DEDADD7D79F93246D330)
...
使用createData()创建一个包含单URI类型的pasteData数据。使用[setData()](<../../../../harmonyos-references/基础功能/Basic Services Kit（基础服务）/ArkTS API/数据文件处理/@ohos.pasteboard (剪贴板)/js-apis-pasteboard.md#setdata9>)方法将pasteData数据写入系统剪贴板。
...
由于应用读取系统剪贴板的权限[ohos.permission.READ\_PASTEBOARD](../../../../harmonyos-guides/系统/安全/程序访问控制/应用权限管控/应用权限列表/受限开放权限/restricted-permissions.md#ohospermissionread_pasteboard)是受限访问的，因此需要先通过AGC平台申请此权限，才能获取系统剪贴板内的数据。具体申请权限的流程可参考[使用ACL的签名配置指导](../../../../harmonyos-guides/编写与调试应用/配置调试签名/ide-signing.md#section89479413571)。

首先，应用在启动时或从后台进入前台时，在[onPageShow()](../../../../harmonyos-references/ArkUI（方舟UI框架）/ArkTS组件/自定义组件/自定义组件的生命周期/ts-custom-component-lifecycle.md#onpageshow)生命周期中检查系统剪贴板中是否包含URI类型数据。URI类型数据存在时检查权限ohos.permission.READ\_PASTEBOARD是否已授权。确认权限授权后，使用[getData()](<../../../../harmonyos-references/基础功能/Basic Services Kit（基础服务）/ArkTS API/数据文件处理/@ohos.pasteboard (剪贴板)/js-apis-pasteboard.md#getdata9>)方法获取系统剪贴板内的数据，并使用[getPrimaryUri()](<../../../../harmonyos-references/基础功能/Basic Services Kit（基础服务）/ArkTS API/数据文件处理/@ohos.pasteboard (剪贴板)/js-apis-pasteboard.md#getprimaryuri7>)方法获取单URI类型的数据。校验数据格式正确后，拉起弹窗询问用户是否需要执行跳转。
...
随着富文本数据接入不同设备，为了使复制的数据能够被更多的设备B识别，设备A需要将同一份数据的多种样式存入剪贴板中。如设备A图文混排的富文本数据可以按[纯文本类型（'text/plain'）](<../../../../harmonyos-references/基础功能/Basic Services Kit（基础服务）/ArkTS API/数据文件处理/@ohos.pasteboard (剪贴板)/js-apis-pasteboard.md#常量>)、包含文字样式及图片的[HTML类型（'text/html'）](<../../../../harmonyos-references/基础功能/Basic Services Kit（基础服务）/ArkTS API/数据文件处理/@ohos.pasteboard (剪贴板)/js-apis-pasteboard.md#常量>)、[纯图片类型（'pixelMap'）](<../../../../harmonyos-references/基础功能/Basic Services Kit（基础服务）/ArkTS API/数据文件处理/@ohos.pasteboard (剪贴板)/js-apis-pasteboard.md#常量>)等多种类型存入剪贴板。设备B选择需要的类型读取对应类型的数据，然后实现粘贴操作。剪贴板支持的数据类型请参考[@ohos.pasteboard (剪贴板)](<../../../../harmonyos-references/基础功能/Basic Services Kit（基础服务）/ArkTS API/数据文件处理/@ohos.pasteboard (剪贴板)/js-apis-pasteboard.md#常量>)。

[](https://contentcenter-videovali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_300_3/39/v3/D_9KAXVEQ5WSFFDSdueO5Q/zh-cn_media_0000002429163385.mp4?HW-CC-KV=V1&HW-CC-Date=20260612T021238Z&HW-CC-Expire=86400&HW-CC-Sign=6C4C5EF49F7075A10644CA538E6B30A2C6618807B4F77089AEC19F3EC0BF7828)
...
创建[PasteData](<../../../../harmonyos-references/基础功能/Basic Services Kit（基础服务）/ArkTS API/数据文件处理/@ohos.pasteboard (剪贴板)/js-apis-pasteboard.md#pastedata>)类型对象pasteData，通过[PasteData.getRecord()](<../../../../harmonyos-references/基础功能/Basic Services Kit（基础服务）/ArkTS API/数据文件处理/@ohos.pasteboard (剪贴板)/js-apis-pasteboard.md#getrecord9>)方法获取数据条目[PasteDataRecord](<../../../../harmonyos-references/基础功能/Basic Services Kit（基础服务）/ArkTS API/数据文件处理/@ohos.pasteboard (剪贴板)/js-apis-pasteboard.md#pastedatarecord7>)。本示例中使用[RichEditor](../../../../harmonyos-references/ArkUI（方舟UI框架）/ArkTS组件/文本与输入/RichEditor/ts-basic-components-richeditor.md)组件实现富文本数据的初始化，开发者可以选择其他富文本实现方案读取粘贴富文本数据。使用初始化RichEditor组件[RichEditorStyledStringController](../../../../harmonyos-references/ArkUI（方舟UI框架）/ArkTS组件/文本与输入/RichEditor/ts-basic-components-richeditor.md#richeditorstyledstringcontroller12)控制器的[getStyledString()](../../../../harmonyos-references/ArkUI（方舟UI框架）/ArkTS组件/文本与输入/RichEditor/ts-basic-components-richeditor.md#getstyledstring12)方法获取富文本框内的数据。根据需要的数据类型，使用[PasteDataRecord.addEntry()](<../../../../harmonyos-references/基础功能/Basic Services Kit（基础服务）/ArkTS API/数据文件处理/@ohos.pasteboard (剪贴板)/js-apis-pasteboard.md#addentry14>)方法向条目中添加多种类型的数据。最后，调用[setData()](<../../../../harmonyos-references/基础功能/Basic Services Kit（基础服务）/ArkTS API/数据文件处理/@ohos.pasteboard (剪贴板)/js-apis-pasteboard.md#setdata9>)方法将包含多样式数据的对象pasteData存入系统剪贴板。需要注意，PasteDataRecord 仅支持单个PixelMap数据传输，如需传输多个PixelMap，需要向PasteData中存入多个PasteDataRecord分别保存PixelMap。
...
1. 数据写入系统剪贴板后，通过[粘贴控件](../../../../harmonyos-guides/系统/安全/程序访问控制/使用安全控件/使用粘贴控件/pastebutton.md)获得临时授权读取剪贴板数据。直接点击粘贴按钮，使用[getData()](<../../../../harmonyos-references/基础功能/Basic Services Kit（基础服务）/ArkTS API/数据文件处理/@ohos.pasteboard (剪贴板)/js-apis-pasteboard.md#getdata9>)方法，即可直接读取系统剪贴板的内容。
...
2. 对于存储在系统剪贴板中的多样式数据，使用[PasteDataRecord.getValidTypes()](<../../../../harmonyos-references/基础功能/Basic Services Kit（基础服务）/ArkTS API/数据文件处理/@ohos.pasteboard (剪贴板)/js-apis-pasteboard.md#getvalidtypes14>)方法获取剪贴板中数据类型与传入参数类型的交集的数据类型数组，交集结果表示请求的类型在剪贴板中存在。遍历数据类型数组，使用[PasteDataRecord.getData()](<../../../../harmonyos-references/基础功能/Basic Services Kit（基础服务）/ArkTS API/数据文件处理/@ohos.pasteboard (剪贴板)/js-apis-pasteboard.md#getdata14>)方法获取对应类型的数据，通过不同RichEditor组件的控制器，实现不同类型数据的粘贴。请注意若剪贴板中不存在请求的数据类型，则对应类型的粘贴操作无法执行，对应RichEditor组件中内容不做更新。
...
[](https://contentcenter-videovali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_300_3/3a/v3/ewCRmIbgQYCTXiuZPnKFWg/zh-cn_media_0000002395443772.mp4?HW-CC-KV=V1&HW-CC-Date=20260612T021238Z&HW-CC-Expire=86400&HW-CC-Sign=4710E42D5BD07B735469EBCB48B008D86675BE27855E5FF22EF33C89EA8F5467)
...
[使用粘贴控件](../../../../harmonyos-guides/系统/安全/程序访问控制/使用安全控件/使用粘贴控件/pastebutton.md)获取临时授权后，调用[getDataWithProgress()](<../../../../harmonyos-references/基础功能/Basic Services Kit（基础服务）/ArkTS API/数据文件处理/@ohos.pasteboard (剪贴板)/js-apis-pasteboard.md#getdatawithprogress15>)方法获取剪贴板的内容及进度。params参数用于在应用使用剪贴板提供的文件拷贝功能时提供必要的信息，包括目标路径、文件冲突处理选项、进度条类型等。[ProgressListener](<../../../../harmonyos-references/基础功能/Basic Services Kit（基础服务）/ArkTS API/数据文件处理/@ohos.pasteboard (剪贴板)/js-apis-pasteboard.md#progresslistener15>)是用于订阅进度数据变化的函数，可以当前获取数据的进度信息。进度数据状态变量更新后，刷新进度条Progress组件显示。
