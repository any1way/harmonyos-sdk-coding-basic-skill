...
scraped_at: 2026-06-12T10:13:09+08:00
doc_updated_at: 2026-05-18
content_hash: sha256:1dbd9e813ee4670b4423fa2b4637515ee2d103e563266025f4990ca99ecd70d6
---
## 概述

[Share Kit（分享服务）](<../../../../harmonyos-guides/应用服务/Share Kit（分享服务）/share-kit-guide.md>)提供隔空传送分享，支持用户通过“一抓一放”手势实现跨设备文件分享（图片、视频、文档等）以及跨设备链接分享。
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/1d/v3/5ME5UNpjRneJLcO17rh-9A/zh-cn_image_0000002464125198.jpg?HW-CC-KV=V1&HW-CC-Date=20260612T021306Z&HW-CC-Expire=86400&HW-CC-Sign=6A18536DFB707BA78874FEA5D155A0F6DD48030F72F73726C8BCCDD2319A2422 "点击放大")
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/a1/v3/ACvGMy7XQEm-QVTqMXfNog/zh-cn_image_0000002497324205.jpg?HW-CC-KV=V1&HW-CC-Date=20260612T021306Z&HW-CC-Expire=86400&HW-CC-Sign=BE249BEE95E087375A94A366B6EE6B28802A69053FF1F474BEEEBED0D9FB8D05 "点击放大")
...
2. **构建分享数据**：定义需要分享的数据[SharedData](<../../../../harmonyos-references/Share Kit（分享服务）/ArkTS API/systemShare（分享）/share-system-share.md#shareddata>)。
3. **分享数据**：在监听回调中调用[share()](<../../../../harmonyos-references/Share Kit（分享服务）/ArkTS API/harmonyShare（华为分享）/share-harmony-share.md#share>)方法来分享数据。
4. **文件接收策略与对端跳转处理**：
...
[harmonyShare（华为分享）](<../../../../harmonyos-references/Share Kit（分享服务）/ArkTS API/harmonyShare（华为分享）/share-harmony-share.md>)模块提供了隔空传送分享事件的监听方法[on('gesturesShare')](<../../../../harmonyos-references/Share Kit（分享服务）/ArkTS API/harmonyShare（华为分享）/share-harmony-share.md#ongesturesshare>)及取消监听的方法[off('gesturesShare')](<../../../../harmonyos-references/Share Kit（分享服务）/ArkTS API/harmonyShare（华为分享）/share-harmony-share.md#ongesturesshare>)。
...
该方法需传入capability参数，类型为[SendCapabilityRegistry](<../../../../harmonyos-references/Share Kit（分享服务）/ArkTS API/harmonyShare（华为分享）/share-harmony-share.md#sendcapabilityregistry>)，继承自[BaseCapabilityRegistry](<../../../../harmonyos-references/Share Kit（分享服务）/ArkTS API/harmonyShare（华为分享）/share-harmony-share.md#basecapabilityregistry>)。其包含windowId属性，需要传入当前应用的窗口ID。在PC/2in1或平板自由窗口模式中，系统会根据窗口ID判断当前应用是否获取了窗口焦点。只有在获取焦点后，当用户触发隔空传送事件时，才会触发应用注册的隔空传送方法。
...
将隔空传送监听方法[on('gesturesShare')](<../../../../harmonyos-references/Share Kit（分享服务）/ArkTS API/harmonyShare（华为分享）/share-harmony-share.md#ongesturesshare>)封装为immersiveListening()，取消监听的方法[off('gesturesShare')](<../../../../harmonyos-references/Share Kit（分享服务）/ArkTS API/harmonyShare（华为分享）/share-harmony-share.md#ongesturesshare>)封装为immersiveDisableListening()。在分享页面可通过初始化ShareModel类并调用immersiveListening()方法来启动监听，调用immersiveDisableListening()方法取消监听。
...
在分享数据时，分享发起方需要构建[SharedRecord](<../../../../harmonyos-references/Share Kit（分享服务）/ArkTS API/systemShare（分享）/share-system-share.md#sharedrecord>)对象。在文件分享场景中，发起方在构造此参数时，必须传入uri和utd这两个属性。

说明

* uri是指要分享的文件URI，而非文件路径，例如沙箱路径content.fileDir，应通过[fileUri.getUriFromPath](<../../../../harmonyos-references/Core File Kit（文件基础服务）/ArkTS API/@ohos.file.fileuri (文件URI)/js-apis-file-fileuri.md#fileurigeturifrompath>)获取其URI。
* utd则是当前文件的[标准化数据类型](../../../../harmonyos-guides/应用框架/ArkData（方舟数据管理）/标准化数据定义/UTD预置列表/uniform-data-type-list.md)，需要传入与分享的数据匹配的类型，以便系统匹配精确的目标应用，推荐使用[uniformTypeDescriptor.getUniformDataTypeByFilenameExtension](<../../../../harmonyos-references/ArkData（方舟数据管理）/ArkTS API/@ohos.data.uniformTypeDescriptor (标准化数据定义与描述)/js-apis-data-uniformtypedescriptor.md#uniformtypedescriptorgetuniformdatatypebyfilenameextension11>)方法，通过给定的文件后缀名查询标准化数据类型的ID。
...
分享接收端接收数据时遵循统一规则，详情请参考[目标设备接收分享数据一步直达体验](<../../../../harmonyos-guides/应用服务/Share Kit（分享服务）/Share Kit体验规范/目标设备接收分享数据一步直达体验/share-access-one-step.md>)。当传入的分享参数utd为uniformTypeDescriptor.UniformDataType.IMAGE且文件确认为图片文件时，接收端为phone、tablet设备时默认将其存储在图库中。当传入的utd为uniformTypeDescriptor.UniformDataType.FILE时，文件会默认存储到文件管理中。接收端为PC/2in1设备类型时则会将媒体以及非媒体文件存储至文件管理器。系统预置了常用类型，详情可参考[UTD预置列表](../../../../harmonyos-guides/应用框架/ArkData（方舟数据管理）/标准化数据定义/UTD预置列表/uniform-data-type-list.md)。
...
详情请参考碰一碰视频分享[典型场景](../碰一碰链接分享/bpta-application-knock-video-share.md#section95975396464)章节。
...
在隔空传送事件回调中调用this.immersiveLinkCallback()方法，实现分享数据构建，在链接分享场景中，发起方在构造[SharedRecord](<../../../../harmonyos-references/Share Kit（分享服务）/ArkTS API/systemShare（分享）/share-system-share.md#sharedrecord>)参数时，必须传入content和utd这两个属性，并通过sharableTarget.share()方法传输数据，完成隔空传送链接分享流程。
...
开发者可参考[使用App Linking实现应用间跳转](<../../../../harmonyos-guides/应用框架/Ability Kit（程序框架服务）/Stage模型开发指导/应用间跳转/拉起指定应用/使用App Linking实现应用间跳转/app-linking-startup.md>)进行配置和使用。例如，此处配置的App Linking的链接为：www.example.com，开发者需在entry模块的module.json5中进行如下配置：
...
分享服务支持链接分享和文件分享两种场景，但不支持两者混合分享。在混合分享时，数据可能会在分享过程中丢失或提示无效的数据类型。详情可参考[分享数据类型不支持](<../../../../harmonyos-guides/应用服务/Share Kit（分享服务）/Share Kit常见问题/分享数据类型不支持/share-faq-2.md>)。
