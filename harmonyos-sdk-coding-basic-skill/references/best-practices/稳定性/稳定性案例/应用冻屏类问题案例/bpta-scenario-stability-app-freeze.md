...
scraped_at: 2026-06-12T10:18:23+08:00
doc_updated_at: 2026-05-30
content_hash: sha256:e0b63845c413033a43dc036bdabf4d2f1d1d9a7e08d508f7531086d8a0c3dce5
---
## ThreadBlock类问题案例-未正确使用锁

### 概述

由于没有考虑到多线程操作时锁的时序问题，从而导致死锁，触发ThreadBlock检测，原理可参考[THREAD\_BLOCK\_6S 应用主线程卡死超时检测](<../../../../harmonyos-guides/系统/调测调优/Performance Analysis Kit（性能分析服务）/故障检测/AppFreeze（应用冻屏）检测/appfreeze-guidelines.md#thread_block_6s-应用主线程卡死超时>)。
...
请参考[应用冻屏问题定位步骤与思路](../../稳定性分析/应用冻屏类问题分析方法/应用冻屏问题排查方法/bpta-stability-app-freeze-way.md#section1950514261110)。
...
由于线程繁忙，导致用户在合一桌面切换主题时，页面高概率卡死，点击无响应，然后闪退到锁屏界面，严重影响用户体验，从而触发APP\_INPUT\_BLOCK检测，原理可参考[APP\_INPUT\_BLOCK 用户输入响应超时](<../../../../harmonyos-guides/系统/调测调优/Performance Analysis Kit（性能分析服务）/故障检测/AppFreeze（应用冻屏）检测/appfreeze-guidelines.md#app_input_block-用户输入响应超时>)。
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/fd/v3/5Qp9xJ0-QaCNbrlSVFszqQ/zh-cn_image_0000002370565628.png?HW-CC-KV=V1&HW-CC-Date=20260612T021821Z&HW-CC-Expire=86400&HW-CC-Sign=8E62A9232506A1FA74F402270667890B5ACD203D150EA533509AE0D4E12E8ACB)往前推6s左右，可以看到在14:40:53.498左右，有一个点击事件发给了scb。

![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/5f/v3/uCVNIeaOQR63FrdvSL8crA/zh-cn_image_0000002404125265.png?HW-CC-KV=V1&HW-CC-Date=20260612T021821Z&HW-CC-Expire=86400&HW-CC-Sign=D65BB863F9A7510F000ACF4DE31CEA35709C20C22E66EBD09D03F2C54C857DF9)这之间的6s存在大量的scb日志，判断是在进行更新渲染。

![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/cf/v3/AnZtwb9lSjOinVe7f0ktUA/zh-cn_image_0000002411349608.png?HW-CC-KV=V1&HW-CC-Date=20260612T021821Z&HW-CC-Expire=86400&HW-CC-Sign=01AC28135DC444D6AF26F01949C1828DE55C59F5597899FC232E65F4A0D911BB)

查看对应时间点的trace：

![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/f8/v3/gz2SkkL6SSudefUBJdAgJg/zh-cn_image_0000002404045457.png?HW-CC-KV=V1&HW-CC-Date=20260612T021821Z&HW-CC-Expire=86400&HW-CC-Sign=E5BE308DA6D6270924D3DADA2A2538800C5500103E18CC12619A00CA7333642C)发现scb主线程被占满，非常繁忙。耗时较长的任务是**CustomNodeUpdate SwiperPage**，后续需排查该组件里为何一直在刷新。
...
由于主线程中存在耗时操作，导致应用拉起、切前台等过程中卡死并闪退，触发LIFECYCLE\_TIMEOUT，原理可参考[AppFreeze（应用冻屏）检测](<../../../../harmonyos-guides/系统/调测调优/Performance Analysis Kit（性能分析服务）/故障检测/AppFreeze（应用冻屏）检测/appfreeze-guidelines.md>)。
...
4. let file = fileIo.openSync(fileuri, fileIo.OpenMode.READ_ONLY);
5. // ...
6. }
7. // ...
...
详见[应用冻屏类问题定位步骤与思路](../../稳定性分析/应用冻屏类问题分析方法/应用冻屏问题排查方法/bpta-stability-app-freeze-way.md#section1950514261110)。
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/75/v3/VapNE4uATsyIl42JfSx1uQ/zh-cn_image_0000002370565632.png?HW-CC-KV=V1&HW-CC-Date=20260612T021821Z&HW-CC-Expire=86400&HW-CC-Sign=56545DF30164037C2F393EBF13A43D5D72D7A467AE41DDDE191743C9F09A8769)查看具体代码：
...
5. let file = await fileIo.open(fileuri, fileIo.OpenMode.READ_ONLY); // 改为异步加载
6. // ...
7. }
8. // ...
9. }
