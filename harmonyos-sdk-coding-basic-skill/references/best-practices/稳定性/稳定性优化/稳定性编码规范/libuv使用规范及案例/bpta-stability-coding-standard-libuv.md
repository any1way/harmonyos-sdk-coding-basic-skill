...
scraped_at: 2026-06-12T10:18:02+08:00
doc_updated_at: 2026-03-12
content_hash: sha256:3705ae651d8dae211970f31d51a6b387b5bdfd31c32203559b4d0a51ed1aedf1
---
## 前言
...
本章节主要阐述libuv中Requests和Handles的使用规范，确保开发者正确使用libuv接口，避免应用出现稳定性问题。关于libuv原理，感兴趣的开发者可参考[libuv](../../../../../harmonyos-references/libuv/libuv.md)。

### libuv中Requests使用规范

在[libuv](../../../../../harmonyos-references/libuv/libuv.md)库中，Request表示一个短暂的请求，不会被事件循环长期持有。请求任务执行完毕后，该请求在事件循环上的使命结束。如果需要继续下一次请求，必须再次调用相关接口。在[libuv官方文档](https://docs.libuv.org/en/v1.x/)中，Request主要有如下几种类型：
...
在[libuv开发指南](../../../../../harmonyos-references/libuv/libuv.md)中，Handle表示一个持久的请求，可以被事件循环长期持有。如果开发者不调用uv\_close函数，创建的Handle会持久保存在事件循环上，影响事件循环的正常退出。在[官方文档](https://docs.libuv.org/en/v1.x/)中，Handle主要有如下几种类型：
...
对于uv\_loop\_t的使用，不会出现严重问题。开发者在使用时，需在事件循环退出时正确释放事件循环中的资源，避免因事件循环无法退出导致资源泄露。如果开发者创建的事件循环不清楚上层业务如何使用，可以在退出时调用uv\_walk遍历事件循环上的Handles，并在回调中调用uv\_close将Handles从事件循环中移除。然后执行uv\_run，确保异步任务执行完毕。这样可以保证事件循环的正常退出，前提是此过程中没有其他业务向事件循环中添加任务。具体可参考[libuv中的事件循环](../../../../../harmonyos-references/libuv/libuv.md#libuv中的事件循环)。

在使用libuv中的Handles时，确保在不使用时调用uv\_close，并且必须在事件循环所在的线程中调用，以避免多线程数据竞争问题。如果需要在多线程环境中操作Handles，可以参考[libuv开发指南](../../../../../harmonyos-references/libuv/libuv.md#libuv中的handles和requests)的“**libuv timer使用规范**“章节。
...
uv\_\_queue\_remove表示将某结点从事件循环队列上摘除。现在崩溃在这个函数上面，又是空指针解引用。这种情况下，开发者只需要参考[使用ASan检测内存错误](../../../稳定性检测/开发态稳定性检测/地址越界类问题检测/使用ASan检测内存错误/bpta-stability-asan-detection.md)这篇文档，开启ASan后复现，即可将具体崩溃栈获取到。通过解栈定位到具体代码行。在本例中，出错代码如下：
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/9/v3/s3VV_i1OR5WjFfXRvgbK0g/zh-cn_image_0000002229335745.png?HW-CC-KV=V1&HW-CC-Date=20260612T021800Z&HW-CC-Expire=86400&HW-CC-Sign=5E586AFB6DAB82DFF0209D89952F4D6DC2BE36BA6AA8F327BE9841BE05F79DE7)
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/43/v3/gB416qoHTfuhxprfdfSV0Q/zh-cn_image_0000002193850396.png?HW-CC-KV=V1&HW-CC-Date=20260612T021800Z&HW-CC-Expire=86400&HW-CC-Sign=9E811D8545C3A72BDC7EE2C3C7707C4869C16458DFA00975814A0B9CC3A80CA1)

第一次释放的调用栈如下：

![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/72/v3/1TkJdUeoQVmEXJ03RzCOxg/zh-cn_image_0000002194009964.png?HW-CC-KV=V1&HW-CC-Date=20260612T021800Z&HW-CC-Expire=86400&HW-CC-Sign=639F12EF5B8F060B87C7B1EE43EFFB9036E1B99538C2D956FCD4365E025BC2EF)

经过相关开发者的反编译，定位到现场，代码如下：

![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/e0/v3/eK1CprLDQ-WkfKPOLFG9yg/zh-cn_image_0000002194009972.png?HW-CC-KV=V1&HW-CC-Date=20260612T021800Z&HW-CC-Expire=86400&HW-CC-Sign=25E676C567AC909CFAE3CF5ECF4E70C9824B8F68ED9C4CAF515C62A70A4A3C2F)
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/4b/v3/vEoO9AY-TVulZqpjvgFxdw/zh-cn_image_0000002229450237.png?HW-CC-KV=V1&HW-CC-Date=20260612T021800Z&HW-CC-Expire=86400&HW-CC-Sign=6C57475BB5B5F1F2DFED8EC10D53A3FF50B81906D1ACC0D15921CB8D5628FA4A)
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/bc/v3/2dX_1zdMTjm1mHYDXW1HVQ/zh-cn_image_0000002194009968.png?HW-CC-KV=V1&HW-CC-Date=20260612T021800Z&HW-CC-Expire=86400&HW-CC-Sign=5BCB005D455FC11EFE675223FD04BC57CB950FB97F45A7006D3103A8FF9ED3B1)

x0包含after\_work\_cb的地址。根据该地址确定其所在的so文件地址范围，再用该地址减去so文件的起始地址，即可得到其偏移地址。通过反编译可以找到具体的代码行。

![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/ec/v3/cR4sJlihTf2pxA8si670dg/zh-cn_image_0000002193850392.png?HW-CC-KV=V1&HW-CC-Date=20260612T021800Z&HW-CC-Expire=86400&HW-CC-Sign=288D1A2E112003FD8F8261E29BF0E3CD8B31D99EFECC4BA171D078C9CCCC985A)
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/c9/v3/E4wOLsFXRgWklp5dOxNeDQ/zh-cn_image_0000002194009980.png?HW-CC-KV=V1&HW-CC-Date=20260612T021800Z&HW-CC-Expire=86400&HW-CC-Sign=E389DD42A8B30EE76C4828F8153218B5E05AB3F21541AA33E2196F5AFF97BF0A "点击放大")
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/e5/v3/CPvHCS7VTI6Fl6JgYpeAog/zh-cn_image_0000002194009984.png?HW-CC-KV=V1&HW-CC-Date=20260612T021800Z&HW-CC-Expire=86400&HW-CC-Sign=930C863D174E7CA1C50A1FB8D5A5D1CE78CB9A1D725F4BA44CBA34859AC7A66A)
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/82/v3/x7VrYMLuSnGwk-TZK6taBA/zh-cn_image_0000002194009976.png?HW-CC-KV=V1&HW-CC-Date=20260612T021800Z&HW-CC-Expire=86400&HW-CC-Sign=ED374443A4D4890C16BBB51A33E9AE919D1AEF7000938D283A96ADAA731D331C)
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/87/v3/GDu1VKX5QjaPwEB13HOaWg/zh-cn_image_0000002229450229.png?HW-CC-KV=V1&HW-CC-Date=20260612T021800Z&HW-CC-Expire=86400&HW-CC-Sign=8895D14F701695018E3938A0E989FC39D540CC3CE5B2BC56EED9448C1A2531C6)

该日志证明了同一个句柄在主线程的loop和taskpool的TaskWorker线程上进行了初始化。

接下来查看该SDK下的so代码，代码如下：

![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/8f/v3/WSp-UDgXR2uLKPO6-9LIGQ/zh-cn_image_0000002194009948.png?HW-CC-KV=V1&HW-CC-Date=20260612T021800Z&HW-CC-Expire=86400&HW-CC-Sign=BA1102B9D2278BD3846EF49481E61895E569E21AE61201CD47BE9BA00380C1DA)

![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/eb/v3/T4L6_AqiTNuF3YCWzAt-uw/zh-cn_image_0000002193850384.png?HW-CC-KV=V1&HW-CC-Date=20260612T021800Z&HW-CC-Expire=86400&HW-CC-Sign=695E481125FFBD72C330FAD3E1ECEE7552675BCC40AF8D56902EDA4956B391AD)
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/d2/v3/x7pLlm_-QKWR4oDlJuSKzg/zh-cn_image_0000002194009956.png?HW-CC-KV=V1&HW-CC-Date=20260612T021800Z&HW-CC-Expire=86400&HW-CC-Sign=88621BF3AFCA29D10B4673651F909CD8E39ED03D8CA2F8774336415C08663FCA)

问题分析：

排查应用方代码，代码如下：

![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/87/v3/5kE7lJJUSIqyZsItuQWogg/zh-cn_image_0000002229450221.png?HW-CC-KV=V1&HW-CC-Date=20260612T021800Z&HW-CC-Expire=86400&HW-CC-Sign=605FEF395E3D3784769A5E1B15C3BF1DE52183854E016FC3C4617DCEC3AD671C)
