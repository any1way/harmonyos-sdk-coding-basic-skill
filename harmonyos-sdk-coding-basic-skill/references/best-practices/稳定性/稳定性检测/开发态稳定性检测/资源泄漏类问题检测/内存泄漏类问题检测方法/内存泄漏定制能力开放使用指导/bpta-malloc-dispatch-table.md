...
scraped_at: 2026-06-12T10:17:23+08:00
doc_updated_at: 2026-03-19
content_hash: sha256:0a4df8a86b0634bd056f9d00d2a7bb476760003724fac78c445aa14b473a3bfb
---
## 概述
...
MallocDispatchTable简称内存分配表，提供对HarmonyOS [libc标准库](../../../../../../../harmonyos-references/libc标准库/musl.md)中的malloc、calloc、realloc、free等内存操作系列函数的Hook能力。此能力可用于跟踪应用的内存分配/释放信息，辅助内存泄漏问题快速定界定位。

注：内存基础知识参考文档：[内存基础知识](../../../../../../性能/性能分析/分析内存占用问题/内存基础知识/bpta-memory-basic-knowledge.md#section085818715389)。
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/6c/v3/P4fLIzkGT1i_8geNF7AjAQ/zh-cn_image_0000002501437914.png?HW-CC-KV=V1&HW-CC-Date=20260612T021722Z&HW-CC-Expire=86400&HW-CC-Sign=209BFEEA3D7A8679237100542B482E2971B72D041A0037F33F631282A7502AEB)
...
3. **创建一个[HiDebug\_MallocDispatch](<../../../../../../../harmonyos-references/调测调优/Performance Analysis Kit（性能分析服务）/C API/结构体/HiDebug_MallocDispatch/capi-hidebug-hidebug-mallocdispatch.md>)类型的分配表。**
   ...
   创建一个HiDebug\_MallocDispatch结构体current，通过[OH\_HiDebug\_GetDefaultMallocDispatchTable()](<../../../../../../../harmonyos-references/Performance Analysis Kit（性能分析服务）/C API/头文件/hidebug.h/capi-hidebug-h.md#oh_hidebug_getdefaultmallocdispatchtable>)分配其占用的堆内存。修改结构体中的函数指针，让其指向之前定义的MyMmap和MyMunmap函数。
4. **启用自定义 MallocDispatchTable。**
   ...
   将[libc标准库](../../../../../../../harmonyos-references/libc标准库/musl.md)中使用的MallocDispatchTable替换成自定义的分配表。
5. **调用libc标准库中的mmap函数。**
...
6. **停用自定义 MallocDispatchTable。**
   ...
   调用[OH\_HiDebug\_RestoreMallocDispatchTable()](<../../../../../../../harmonyos-references/Performance Analysis Kit（性能分析服务）/C API/头文件/hidebug.h/capi-hidebug-h.md#oh_hidebug_restoremallocdispatchtable>)接口可以恢复标准库默认的MallocDispatchTable。
...
1. 无法通过GWP-ASan功能进行内存越界检测。GWP-ASan 的工作原理详见文档：[GWP-ASan检测原理](../../../地址越界类问题检测/地址越界检测工具原理/bpta-stability-address-sanitizer-principle.md#section555616291854)。
2. 无法通过使用[native hook插件](../../../../../../../harmonyos-guides/系统/调测调优/调试命令/hiprofiler/hiprofiler.md#native-hook插件)对该应用程序进行函数调用栈捕获。

警告

1. 禁止在自定义方法中调用[libc标准库](../../../../../../../harmonyos-references/libc标准库/musl.md)内存操作函数（malloc/free/mmap/munmap），否则会导致死循环。
2. 禁止在自定义malloc方法中使用[hilog](../../../../../../../harmonyos-guides/系统/调测调优/调试命令/hilog/hilog.md)进行日志打印，否则会导致死锁问题。
