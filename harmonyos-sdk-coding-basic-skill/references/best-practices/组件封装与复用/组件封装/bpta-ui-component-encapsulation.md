...
scraped_at: 2026-06-11T16:54:21+08:00
doc_updated_at: 2026-05-18
content_hash: sha256:e8884da7de065f43bf10a27e56dee6bdcd389687fe04a6ac34622f7c9fbdeabd
---
## 概述
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/d8/v3/VPwtZkdJQ8OoNVZHW_8YSw/zh-cn_image_0000002362705992.png?HW-CC-KV=V1&HW-CC-Date=20260611T085420Z&HW-CC-Expire=86400&HW-CC-Sign=28289DE37EA7DC323ED9D0C4B0E2942456E2D8D35685A16C18BBC726AAAACBC2)

### 实现原理

组件的公共样式通过设置组件的属性实现，ArkUI可以使用[AttributeModifier属性修改器](<../../../harmonyos-guides/应用框架/ArkUI（方舟UI框架）/UI开发 (ArkTS声明式开发范式)/使用自定义能力/Modifier机制/属性修改器 (AttributeModifier)/arkts-user-defined-extension-attributemodifier.md>)，将属性封装在同一个AttributeModifier实现类中，在需要复用样式的组件中应用AttributeModifier实例对象。
...
2. 使用方创建AttributeModifier实例，将其作为参数传入到系统组件的attributeModifier()方法中，封装的样式会应用到该组件上。
   ...
   更多参考[AttributeModifier使用说明](<../../../harmonyos-guides/应用框架/ArkUI（方舟UI框架）/UI开发 (ArkTS声明式开发范式)/使用自定义能力/Modifier机制/属性修改器 (AttributeModifier)/arkts-user-defined-extension-attributemodifier.md#使用说明>)。
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/68/v3/dRsC391eT5CBGnSqjx0jdg/zh-cn_image_0000002396385773.png?HW-CC-KV=V1&HW-CC-Date=20260611T085420Z&HW-CC-Expire=86400&HW-CC-Sign=E55CC6EBC2BDDD831C5EDAEBE833BBF68E216A06377EDD6760848F9D41E42EB6 "点击放大")

### 实现原理

使用@Component[创建自定义组件](<../../../harmonyos-guides/应用框架/ArkUI（方舟UI框架）/UI开发 (ArkTS声明式开发范式)/学习UI范式基本语法/自定义组件/创建自定义组件/arkts-create-custom-components.md>)，将不变的部分实现在组件内部，将可能发生变化的部分，使用参数变量暴露出去。
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/7c/v3/VCOaCcSgT9O2lrB5IHGp7w/zh-cn_image_0000002362865888.png?HW-CC-KV=V1&HW-CC-Date=20260611T085420Z&HW-CC-Expire=86400&HW-CC-Sign=7F784DB01B4717DF522CC1C95540357977E8B50ED928BB1FD0416A505548CAA7 "点击放大")

### 实现原理

系统提供了[@Builder装饰器](<../../../harmonyos-guides/应用框架/ArkUI（方舟UI框架）/UI开发 (ArkTS声明式开发范式)/学习UI范式基本语法/组件扩展/@Builder装饰器：自定义构建函数/arkts-builder.md>)，其装饰的方法遵循自定义组件build()函数语法规则。将@Builder装饰的方法传入[wrapBuilder](<../../../harmonyos-guides/应用框架/ArkUI（方舟UI框架）/UI开发 (ArkTS声明式开发范式)/学习UI范式基本语法/组件扩展/wrapBuilder：封装全局@Builder/arkts-wrapbuilder.md>)后，会返回[WrappedBuilder](<../../../harmonyos-guides/应用框架/ArkUI（方舟UI框架）/UI开发 (ArkTS声明式开发范式)/学习UI范式基本语法/组件扩展/wrapBuilder：封装全局@Builder/arkts-wrapbuilder.md>)对象，该对象支持赋值和传递。
...
* **方法二：****使用@Watch**
  ...
  使用[@Watch](<../../../harmonyos-guides/应用框架/ArkUI（方舟UI框架）/UI开发 (ArkTS声明式开发范式)/学习UI范式状态管理/状态管理（V1）/管理组件拥有的状态/@Watch装饰器：状态变量更改通知/arkts-watch.md>)监听状态变量，当父组件修改此变量时，@Watch的回调方法将被执行，即实现了子组件中的方法调用。
...
* **方法三：使用事件通信**
  ...
  使用[Emitter](<../../../harmonyos-guides/系统/基础功能/Basic Services Kit（基础服务）/进程线程通信/使用Emitter进行线程间通信/itc-with-emitter.md>)通信机制，在子组件中添加事件监听，在父组件emit发布对应事件，监听的事件被执行，即实现了子组件中的方法调用。
...
使用[@BuilderParam参数](<../../../harmonyos-guides/应用框架/ArkUI（方舟UI框架）/UI开发 (ArkTS声明式开发范式)/学习UI范式基本语法/组件扩展/@BuilderParam装饰器：引用@Builder函数/arkts-builderparam.md#参数初始化组件>)或[@BuilderParam尾随闭包](<../../../harmonyos-guides/应用框架/ArkUI（方舟UI框架）/UI开发 (ArkTS声明式开发范式)/学习UI范式基本语法/组件扩展/@BuilderParam装饰器：引用@Builder函数/arkts-builderparam.md#尾随闭包初始化组件>)的方法，在封装的子组件中，将可变的UI内容做为变量开放出去，当父组件在应用子组件时实现具体的UI内容。
...
先将UI组件包装成全局@Builder，然后使用[wrapBuilder](<../../../harmonyos-guides/应用框架/ArkUI（方舟UI框架）/UI开发 (ArkTS声明式开发范式)/学习UI范式基本语法/组件扩展/wrapBuilder：封装全局@Builder/arkts-wrapbuilder.md>)依次封装每个@Builder，这样得到的WrappedBuilder对象/数组可以用于参数传递，可以使用ForEach对传递的数组进行渲染。

参考：[builder方法赋值给变量在ui语法中使用](<../../../harmonyos-guides/应用框架/ArkUI（方舟UI框架）/UI开发 (ArkTS声明式开发范式)/学习UI范式基本语法/组件扩展/wrapBuilder：封装全局@Builder/arkts-wrapbuilder.md#builder方法赋值给变量在ui语法中使用>)
