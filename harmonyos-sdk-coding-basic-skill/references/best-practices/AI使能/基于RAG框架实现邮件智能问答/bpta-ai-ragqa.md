...
scraped_at: 2026-06-01T16:59:22+08:00
doc_updated_at: 2026-05-18
content_hash: sha256:e32679cf119071ebcb85ed4acfb8082bc18935839987ffd7cbd3a905b3e52c4d
---
## 概述
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/60/v3/sD-gZqvtQaKHUigYT-0aDg/zh-cn_image_0000002368829345.png?HW-CC-KV=V1&HW-CC-Date=20260601T085922Z&HW-CC-Expire=86400&HW-CC-Sign=706063F778F304C5C2FF3D8B03BEE87E7C0BB98BC0DD1A92DEA79C933DA8EC74 "点击放大")
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/1b/v3/CIljyk5DTtaPO_gNGUARFw/zh-cn_image_0000002368709477.png?HW-CC-KV=V1&HW-CC-Date=20260601T085922Z&HW-CC-Expire=86400&HW-CC-Sign=31118A07114424490486EF33C267540C622B03705BF0797D866A43CE6AAE80EF "点击放大")
...
1、通过[relationalStore.getRdbStore()](<../../../harmonyos-references/ArkData（方舟数据管理）/ArkTS API/@ohos.data.relationalStore (关系型数据库)/Functions/arkts-apis-data-relationalstore-f.md#relationalstoregetrdbstore>)接口创建或打开已有关系型数据库。

2、通过[execute()](<../../../harmonyos-references/ArkData（方舟数据管理）/ArkTS API/@ohos.data.relationalStore (关系型数据库)/Interface (RdbStore)/arkts-apis-data-relationalstore-rdbstore.md#execute12>)接口执行建表操作，包含id、subject text、content text、image\_text text、attachment\_names text等字段。
...
依次读取原始数据文件，将读出的数据使用[execute()](<../../../harmonyos-references/ArkData（方舟数据管理）/ArkTS API/@ohos.data.relationalStore (关系型数据库)/Interface (RdbStore)/arkts-apis-data-relationalstore-rdbstore.md#execute12>)接口插入到新建的数据库中。
...
2、使用[requestInStream()](<../../../harmonyos-references/网络/Network Kit（网络服务）/ArkTS API/@ohos.net.http (数据请求)/js-apis-http.md#requestinstream10-1>)根据URL地址和相关配置项，发起HTTP网络请求并返回流式响应。
...
2、构建检索配置[retrieval.RetrievalConfig](<../../../harmonyos-references/Data Augmentation Kit（数据增强服务）/ArkTS API/retrieval（智慧化数据平台）/dataaugmentation-retrieval-api.md#retrievalconfig>)，配置向量数据库和倒排索引数据库的参数，并将这些配置绑定到相应的检索通道中，构建了一个用于多数据源检索的配置对象retrievalConfig。这个配置对象可以被后续的检索操作使用，以从指定的数据库中检索数据。
...
3、配置检索条件[retrieval.RetrievalCondition](<../../../harmonyos-references/Data Augmentation Kit（数据增强服务）/ArkTS API/retrieval（智慧化数据平台）/dataaugmentation-retrieval-api.md#retrievalcondition>)，从多个数据源（基于逆向索引和向量相似度）召回相关数据，并通过RRF方法对召回结果进行重排，最终返回5条最相关的记录，在信息检索系统中提升召回率和精确率。
...
4、配置[LLM会话](<../../../harmonyos-references/Data Augmentation Kit（数据增强服务）/ArkTS API/rag（检索增强生成）/dataaugmentation-rag-api.md#chatllm>)，在HTTP流式响应数据接收事件中进行数据解析，将问题的答案和是否结束封装在answer中传入回调。此步骤会因大模型的选型受到影响，开发中根据实际选择的大模型进行调整。
...
5、在完成检索条件、检索配置、LLM会话的配置后，即可配置[rag.Config](<../../../harmonyos-references/Data Augmentation Kit（数据增强服务）/ArkTS API/rag（检索增强生成）/dataaugmentation-rag-api.md#config>)。
...
1、使用[rag.createRagSession()](<../../../harmonyos-references/Data Augmentation Kit（数据增强服务）/ArkTS API/rag（检索增强生成）/dataaugmentation-rag-api.md#createragsession>)接口创建会话，需要传入应用上下文context和rag.Config。
...
2、使用[streamRun()](<../../../harmonyos-references/Data Augmentation Kit（数据增强服务）/ArkTS API/rag（检索增强生成）/dataaugmentation-rag-api.md#streamrun>)接口开始会话，需要传入提问的问题、提问选项和处理的动作，接收返回的字符串进行拼接形成答案。
