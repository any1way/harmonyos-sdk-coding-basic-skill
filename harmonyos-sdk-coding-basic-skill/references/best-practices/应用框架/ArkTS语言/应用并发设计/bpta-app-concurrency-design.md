...
scraped_at: 2026-06-11T16:56:57+08:00
doc_updated_at: 2026-03-12
content_hash: sha256:467eec4f030466de5ee5c689c8e7b163edf0ad480307013f4c289e3876a4b5f7
---
## 概述
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/6e/v3/zlWnJhk8RjC5Q8_e5xzYpQ/zh-cn_image_0000002194011076.png?HW-CC-KV=V1&HW-CC-Date=20260611T085655Z&HW-CC-Expire=86400&HW-CC-Sign=83347FF999907D69C409786B7510A4CC04876B84292111084A34ECEA6D23F054 "点击放大")
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/1e/v3/DGfm3_ypTl-kO5yUxh4PTA/zh-cn_image_0000002194011072.jpg?HW-CC-KV=V1&HW-CC-Date=20260611T085655Z&HW-CC-Expire=86400&HW-CC-Sign=A3DEE35350749902D6E217EDBA21DE0876FD90EC62670E0196E8FFFD73C93D93 "点击放大")

共享内存模型采用线程和锁的并发机制，不同线程共享内存并通过锁保护临界区。对于包含I/O操作或锁的业务，为防止阻塞，需开启多个线程执行不同业务。线程情况如下图所示：

![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/db/v3/RmEzDkx6QtqjbxLm2touFQ/zh-cn_image_0000002194011068.jpg?HW-CC-KV=V1&HW-CC-Date=20260611T085655Z&HW-CC-Expire=86400&HW-CC-Sign=14722D51E72168C90587F569652356CF50CAED1E80967364AEFAE20878D9C9CC "点击放大")

因此，应用经常存在几百个线程，增加调度开销和内存占用。

### ArkTS并发模型

![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/83/v3/b1ASP1bUTamMeKuBB_sV9g/zh-cn_image_0000002194011084.jpg?HW-CC-KV=V1&HW-CC-Date=20260611T085655Z&HW-CC-Expire=86400&HW-CC-Sign=A5A863A7979C9156B9531AD42493A96DA157D6899C764CC80F1818698C7DAF8E "点击放大")

ArkTS采用内存隔离的线程模型，不同线程间通过消息通信，线程内无锁化运行。业务内部的I/O操作由系统分发到后台的I/O任务池，不阻塞ArkTS上层逻辑，线程情况如下图所示。

![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/8f/v3/n0EegS0cTVaDUjikBUH3mA/zh-cn_image_0000002229336865.jpg?HW-CC-KV=V1&HW-CC-Date=20260611T085655Z&HW-CC-Expire=86400&HW-CC-Sign=FC5C42A2F5FD59101154D441575A79CA35E210D8B250CFB872302AAD348504BE "点击放大")

异步I/O不阻塞ArkTS线程，TaskPool及I/O线程池由系统管理，提升能效。

ArkTS语言支持了TaskPool和Worker的并发能力，接下来简单介绍TaskPool和Worker的功能。

TaskPool的运作机制可参考[TaskPool简介](../../../../harmonyos-guides/应用框架/ArkTS（方舟编程语言）/ArkTS并发/多线程并发/TaskPool简介/taskpool-introduction.md)，TaskPool提供任务分发入口，支持分发任务到不同优先级队列。TaskPool底层管理一定数量的工作线程，从队列获取任务执行。工作线程根据任务数量及线程数量自动扩缩容，保证执行效率。工作线程容量无法无限制扩充，上限数量为核心数减1。

Worker的运作机制可参考[Worker简介](../../../../harmonyos-guides/应用框架/ArkTS（方舟编程语言）/ArkTS并发/多线程并发/Worker简介/worker-introduction.md)，空任务的Worker线程的内存占用大约2MB左右，因此需要控制线程的数量，避免内存过大。
...
* **业务实现中的关键点**
  ...
  3. TaskPool中执行的任务不能有上下文依赖。
     ...
     TaskPool任务在子线程中执行，与宿主线程上下文环境不同，因此需确保任务的独立性，内部实现的依赖应通过参数传入或通过[ArkTS模块化](../../../../harmonyos-guides/应用框架/ArkTS（方舟编程语言）/ArkTS运行时/ArkTS模块化/arkts-runtime-module.md)导入的方式完成。
* **案例参考**
...
* **参考链接**
  ...
  [@ohos.worker (启动一个Worker)](<../../../../harmonyos-references/ArkTS（方舟编程语言）/ArkTS API/@ohos.worker (启动一个Worker)/js-apis-worker.md>)
* **与业界方案特殊差异说明**
...
* **不推荐应用实现方式**
  ...
  长时任务是指长时间不间断运行的独立任务，例如监听某个事件，发起执行后不会再接收发起方的输入。虽然也可以使用Worker（推荐用于常驻后台任务），但更推荐使用TaskPool，因为TaskPool更方便且资源消耗更低。[TaskPool和Worker的实现特点对比](<../../../../harmonyos-guides/应用框架/ArkTS（方舟编程语言）/ArkTS并发/多线程并发/TaskPool和Worker的对比 (TaskPool和Worker)/taskpool-vs-worker.md#实现特点对比>)。
...
* **场景描述**
  ...
  任务依赖机制用于管理并发任务的执行顺序。通过任务依赖，可以指定一个任务在另一个任务完成后才能执行，从而构建复杂的任务执行流程。任务依赖帮助开发者控制任务间的依赖关系，确保任务按预期顺序执行。在TaskPool中，任务依赖通过调用[addDependency()](<../../../../harmonyos-references/ArkTS（方舟编程语言）/ArkTS API/@ohos.taskpool (启动任务池)/js-apis-taskpool.md#adddependency11>)和[removeDependency()](<../../../../harmonyos-references/ArkTS（方舟编程语言）/ArkTS API/@ohos.taskpool (启动任务池)/js-apis-taskpool.md#removedependency11>)方法实现。
...
* **实现方案介绍**
  ...
  TaskPool提供[addDependency()](<../../../../harmonyos-references/ArkTS（方舟编程语言）/ArkTS API/@ohos.taskpool (启动任务池)/js-apis-taskpool.md#adddependency11>)和[removeDependency()](<../../../../harmonyos-references/ArkTS（方舟编程语言）/ArkTS API/@ohos.taskpool (启动任务池)/js-apis-taskpool.md#removedependency11>)两个接口，用于设置任务的依赖关系。默认情况下，任务不依赖其他任务。

  TaskPool维护任务依赖关系列表，调用[addDependency()](<../../../../harmonyos-references/ArkTS（方舟编程语言）/ArkTS API/@ohos.taskpool (启动任务池)/js-apis-taskpool.md#adddependency11>)/[removeDependency()](<../../../../harmonyos-references/ArkTS（方舟编程语言）/ArkTS API/@ohos.taskpool (启动任务池)/js-apis-taskpool.md#removedependency11>)更新列表。任务执行前查询列表，若任务依赖其他任务，则等待依赖任务完成；若任务被其他任务依赖，任务执行结束将依赖任务加入队列。
* **业务实现中的关键点**
...
* **实现方案介绍**
  ...
  [Priority](<../../../../harmonyos-references/ArkTS（方舟编程语言）/ArkTS API/@ohos.taskpool (启动任务池)/js-apis-taskpool.md#priority>)的IDLE优先级是用来标记需要在后台运行的耗时任务（例如数据同步、备份。），它的优先级别是最低的。这种优先级标记的任务只会在所有线程都空闲的情况下触发执行，并且只会占用一个线程来执行。
* **业务实现中的关键点**
...
* **参考链接**
  ...
  [@ohos.taskpool（启动任务池）](<../../../../harmonyos-references/ArkTS（方舟编程语言）/ArkTS API/@ohos.taskpool (启动任务池)/js-apis-taskpool.md>)

  [@ohos.events.emitter (Emitter)](<../../../../harmonyos-references/基础功能/Basic Services Kit（基础服务）/ArkTS API/进程线程通信/@ohos.events.emitter (Emitter)/js-apis-emitter.md>)
* **与业界方案特殊差异说明**
...
* **实现方案介绍（方案一）**
  ...
  ![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/e7/v3/cfWjqsHGRASFSx76yrdGIw/zh-cn_image_0000002194011080.jpg?HW-CC-KV=V1&HW-CC-Date=20260611T085655Z&HW-CC-Expire=86400&HW-CC-Sign=6598A8BC35370BF1D233BA47D5C9C736A001F5259FB76F5A605B6892FA954DDE "点击放大")
* **业务实现中的关键点**
  1. JS模块对象
...
* **业务实现中的关键点**
  ...
  Sendable类需要满足一定的约束，可参考[@Sendable装饰器](../../../../harmonyos-guides/应用框架/ArkTS（方舟编程语言）/ArkTS并发/并发线程间通信/线程间通信对象/Sendable对象/Sendable对象简介/arkts-sendable.md#sendable装饰器)。
* **案例参考**
