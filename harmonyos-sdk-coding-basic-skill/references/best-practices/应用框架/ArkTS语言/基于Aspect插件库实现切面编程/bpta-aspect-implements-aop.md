...
scraped_at: 2026-06-11T16:57:05+08:00
doc_updated_at: 2026-06-10
content_hash: sha256:35e2d13494ccef3a79bfa662ea49bb76e6ff9271d102a1340fd14f5f641920a4
---
## 概述
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/71/v3/CSfKWsF0SMOQVW0QWR7zGA/zh-cn_image_0000002583560435.png?HW-CC-KV=V1&HW-CC-Date=20260611T085703Z&HW-CC-Expire=86400&HW-CC-Sign=FDEBFCDD3E0C0A6C3DCEB917E0ADBC4B7D42FFC953A27940D66DA5002434F783 "点击放大")
...
5. │  ├──aop                             // 各类切面逻辑实现
6. │  │  ├──call
7. │  │  │  └──MyCallAspect.ets          // 方法调用点切面
8. │  │  ├──callback
9. │  │  │  └──MyCallbackAspect.ets      // Promise.then回调切面
10. │  │  ├──definition
11. │  │  │  └──MyDefinitionAspect.ets    // 方法定义点切面
12. │  │  └──eventcallback
13. │  │     └──MyEventCallbackAspect.ets // 事件回调切面
14. │  └──AspectRegistryUtil.ets          // 切面注册工具
15. ├──aspect.json                        // aspect配置文件
16. ├──...
17. └──README.md
```
...
4. 在目标模块的EntryAbility.ets中引入aspectRegister()，并在onCreate()中调用。
   ...
   2. aspectRegister(); // Step 4: call aspectRegister
   3. try {
   4. this.context.getApplicationContext().setColorMode(ConfigurationConstant.ColorMode.COLOR_MODE_NOT_SET);
   5. } catch (err) {
   6. Logger.error(TAG, `Failed to set colorMode. Cause: ${JSON.stringify(err)}`);
   7. }
   8. HMRouterMgr.init({ context: this.context });
   9. Logger.info(TAG, 'Ability onCreate');
   10. }
   ```

   [EntryAbility.ets](https://gitcode.com/HarmonyOS_Samples/aspect/blob/master/entry/src/main/ets/entryability/EntryAbility.ets#L27-L36)
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/f9/v3/uHaum5g6TmGakQAAwL6IwQ/zh-cn_image_0000002553040468.png?HW-CC-KV=V1&HW-CC-Date=20260611T085703Z&HW-CC-Expire=86400&HW-CC-Sign=63AF3EB49CB3F16800E645ABB3249D217BC01580C705EDEB4236862BFC2EC2B9 "点击放大")
...
3. 配置装饰器的入参，以便在目标API方法体首行插入切面逻辑。
   ...
   * scan：配置定义点的扫描范围，指定模块、路径、类和指定方法。
   ...
   ```
   1. @Before({
   2. insertType: InsertType.DEFINITION,
   3. scan: {
   4. module: 'entry',
   5. path: 'src/main/ets/pages/definition/DefinitionBefore',
   6. className: 'CompA',
   7. methodName: 'aboutToAppear'
   8. }
   9. })
   10. static addBefore(joinPoint: JoinPoint): void {
   11. let logList: string[] = AppStorage.get<Array<string>>('logList') ?? [];
   12. logList.push(ResourceUtil.getFormatStringWithoutContext($r('app.string.card_definition_before'),
   13. TimeUtil.getNowWithHMS()));
   14. logList.push(ResourceUtil.getFormatStringWithoutContext($r('app.string.card_location_params_list'),
   15. joinPoint?.className ?? '', joinPoint.moduleName ?? ''));
   16. MyDefinitionAspect.logListLink = AppStorage.link('logList');
   17. MyDefinitionAspect.logListLink.set(logList);
   18. }
   ```

   [MyDefinitionAspect.ets](https://gitcode.com/HarmonyOS_Samples/aspect/blob/master/MyAspect/src/main/ets/aop/definition/MyDefinitionAspect.ets#L23-L40)
...
17. });
18. this.logList.push(ResourceUtil.getFormatString(this.getUIContext(), $r('app.string.card_location_end'),
19. TimeUtil.getNowWithHMS()));
20. }
```

[CallBefore.ets](https://gitcode.com/HarmonyOS_Samples/aspect/blob/master/entry/src/main/ets/pages/call/CallBefore.ets#L82-L101)

**开发步骤**

![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/c4/v3/SQcdjXPJQ4Wp79j0YhsPZg/zh-cn_image_0000002583640531.png?HW-CC-KV=V1&HW-CC-Date=20260611T085703Z&HW-CC-Expire=86400&HW-CC-Sign=4DF75B8FF79650C2405AE2C52961F0355253BF766BEA3CF83814DD15C21DA2DA "点击放大")
...
3. 配置装饰器的入参，以便在目标API调用前插入切面逻辑。
   ...
   * api：配置目标API，包括模块、导入名称和方法名称。
   ...
   3. scan: {
   4. module: 'entry',
   5. path: 'src/main/ets/pages/call/CallBefore',
   6. },
   7. api: {
   ...
   11. }
   12. })
   13. static addBefore(joinPoint: JoinPoint): void {
   14. let logList: string[] = AppStorage.get<Array<string>>('logList') ?? [];
   15. logList.push(ResourceUtil.getFormatStringWithoutContext($r('app.string.card_location_before'),
   16. TimeUtil.getNowWithHMS()));
   17. logList.push(ResourceUtil.getFormatStringWithoutContext($r('app.string.card_location_params_list'),
   18. joinPoint?.className ?? '', joinPoint.moduleName ?? ''));
   19. MyCallAspect.logListLink = AppStorage.link('logList');
   20. MyCallAspect.logListLink.set(logList);
   21. }
   ```

   [MyCallAspect.ets](https://gitcode.com/HarmonyOS_Samples/aspect/blob/master/MyAspect/src/main/ets/aop/call/MyCallAspect.ets#L24-L44)
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/41/v3/0k-8pPi0SiO0KKog6Xx3YA/zh-cn_image_0000002552880850.png?HW-CC-KV=V1&HW-CC-Date=20260611T085703Z&HW-CC-Expire=86400&HW-CC-Sign=E2884FD3432F7A7D0921A8527B0C420631DBAC32FEB5DB5397740306716371EC "点击放大")
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/f3/v3/hk-cO5ddT-6JKd9-tMnuMg/zh-cn_image_0000002583560605.png?HW-CC-KV=V1&HW-CC-Date=20260611T085703Z&HW-CC-Expire=86400&HW-CC-Sign=0854FB2BD34219B1F7ADBF4A125621B1FEEE11F71A84296FA299812EF2268AC5 "点击放大")
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/bf/v3/8S3DiLwDQG6wGTKgPeneZQ/zh-cn_image_0000002552881028.png?HW-CC-KV=V1&HW-CC-Date=20260611T085703Z&HW-CC-Expire=86400&HW-CC-Sign=309908D1121A499799FBF0B234E644257C69DE87C538AC8DD21A553A973CEFF0 "点击放大")
