...
scraped_at: 2026-06-11T16:59:03+08:00
doc_updated_at: 2026-05-22
content_hash: sha256:253e58e797bb432b266fd4094605e758ae5cc09ca326b881d0556fa9d41b7fbe
---
## 概述
...
网络切换主要分为网络类型切换和无网络与有网络之间的切换。针对网络切换的场景，HarmonyOS提供了[网络连接管理](<../../../harmonyos-references/网络/Network Kit（网络服务）/ArkTS API/@ohos.net.connection (网络连接管理)/js-apis-net-connection.md>)能力，用于查询网络信息、监听网络连接的变化等。

在实现网络切换重连上，主要包含以下步骤。

* 使用网络连接管理的能力监听网络变化，并使用[AppStorage](<../../../harmonyos-guides/应用框架/ArkUI（方舟UI框架）/UI开发 (ArkTS声明式开发范式)/学习UI范式状态管理/状态管理（V1）/管理应用拥有的状态/AppStorage：应用全局的UI状态存储/arkts-appstorage.md>)存储应用全局网络状态。
* 使用[@StorageProp](<../../../harmonyos-guides/应用框架/ArkUI（方舟UI框架）/UI开发 (ArkTS声明式开发范式)/学习UI范式状态管理/状态管理（V1）/管理应用拥有的状态/AppStorage：应用全局的UI状态存储/arkts-appstorage.md#storageprop>)，将AppStorage存储的网络状态与对应的属性建立单向数据同步。
* 使用[@Watch](<../../../harmonyos-guides/应用框架/ArkUI（方舟UI框架）/UI开发 (ArkTS声明式开发范式)/学习UI范式状态管理/状态管理（V1）/管理组件拥有的状态/@Watch装饰器：状态变量更改通知/arkts-watch.md>)监听状态变量的变化，当状态变量变化后，对网络进行关闭或重连。
...
**图1** 无网络切换有网络后重连下载  
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/b2/v3/phdS-jbbQciXCYNg8VGXrA/zh-cn_image_0000002194011536.gif?HW-CC-KV=V1&HW-CC-Date=20260611T085858Z&HW-CC-Expire=86400&HW-CC-Sign=87E73368FB83CCA05F4A31C6DBAB6362BBAEE5AB60AFA0F26329C81058133F90 "点击放大")
...
在HarmonyOS中，应用切换到后台2秒后，应用的网络资源会被冻结，并且在12秒后进行释放。此时，再继续使用网络资源，就会出现网络不可用的情况。如果应用有后台使用网络资源的场景，可以使用[短时任务](<../../../harmonyos-guides/应用框架/Background Tasks Kit（后台任务开发服务）/短时任务(ArkTS)/transient-task.md>)或[长时任务](<../../../harmonyos-guides/应用框架/Background Tasks Kit（后台任务开发服务）/长时任务(ArkTS)/continuous-task.md>)。
...
* 结合[UIAbility组件生命周期](<../../../harmonyos-guides/应用框架/Ability Kit（程序框架服务）/Stage模型开发指导/Stage模型应用组件/UIAbility组件/UIAbility组件生命周期/uiability-lifecycle.md>)onForeground和onBackground，在前后台切换时，将应用前后台的状态存储在AppStorage中。
* 使用[@StorageProp](<../../../harmonyos-guides/应用框架/ArkUI（方舟UI框架）/UI开发 (ArkTS声明式开发范式)/学习UI范式状态管理/状态管理（V1）/管理应用拥有的状态/AppStorage：应用全局的UI状态存储/arkts-appstorage.md#storageprop>)将AppStorage存储的前后台状态与对应的属性建立单向数据同步。并使用[@Watch](<../../../harmonyos-guides/应用框架/ArkUI（方舟UI框架）/UI开发 (ArkTS声明式开发范式)/学习UI范式状态管理/状态管理（V1）/管理组件拥有的状态/@Watch装饰器：状态变量更改通知/arkts-watch.md>)监听状态变量的变化，当前后台变化后，关闭或重新连接。
...
**图2** 应用前后台切换后重连下载  
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/87/v3/pHarNyVOSBCXhKOcKZgLlw/zh-cn_image_0000002229337333.gif?HW-CC-KV=V1&HW-CC-Date=20260611T085858Z&HW-CC-Expire=86400&HW-CC-Sign=A091DDAC0D22FBEC90B3F3FDFD38A65B5A509762552A53181F3DEBB12FB3960B "点击放大")
