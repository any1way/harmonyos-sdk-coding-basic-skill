...
content_hash: sha256:5b096b0bca24ef303e96498077818a79f436b2a541cc4e58af30a9d801348375
---
## 概述
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/99/v3/O-faZH4yQY2_CfKDL2fhMA/zh-cn_image_0000002236694464.png?HW-CC-KV=V1&HW-CC-Date=20260611T085903Z&HW-CC-Expire=86400&HW-CC-Sign=5BAE53E730C703FDBF69C3ACE6D3F35CE0543C2420B53C4BE50114EAC23E813C "点击放大")

### 实现原理

SFFT基于[@ohos.taskpool](<../../../harmonyos-references/ArkTS（方舟编程语言）/ArkTS API/@ohos.taskpool (启动任务池)/js-apis-taskpool.md>)和[rcp](<../../../harmonyos-references/网络/Remote Communication Kit（远场通信服务）/ArkTS API/rcp（数据请求）/remote-communication-rcp.md>)实现了多线程下载，实现流程如下：

当使用多线程下载时：

1. 对于每一个下载任务，SFFT都会在正式下载前发送一个“试连”请求。请求使用RCP的[fetch()](<../../../harmonyos-references/网络/Remote Communication Kit（远场通信服务）/ArkTS API/rcp（数据请求）/remote-communication-rcp.md#fetch>)接口发送，使用的HTTP请求方法为“HEAD”，该请求仅用于获取文件资源的元数据而不会获取实际文件数据。
2. 当试连成功后，SFFT会从HEAD请求响应头中的“content-length”字段中获取文件大小，并根据开发者设置的下载策略将远端文件分成包含起始字节和结束字节的若干个数据块。
3. 对于每一个块，SFFT都会向TaskPool的[TaskGroup](<../../../harmonyos-references/ArkTS（方舟编程语言）/ArkTS API/@ohos.taskpool (启动任务池)/js-apis-taskpool.md#taskgroup10>)中添加一个请求任务，该请求任务请求头中的[transferRange](<../../../harmonyos-references/网络/Remote Communication Kit（远场通信服务）/ArkTS API/rcp（数据请求）/remote-communication-rcp.md#transferrange>)字段与文件块中的起始字节和结束字节相对应，在HTTP请求中，transferRange的值会被转为HTTP请求中的Range请求头字段。
4. SFFT使用TaskPool的[taskpool.execute()](<../../../harmonyos-references/ArkTS（方舟编程语言）/ArkTS API/@ohos.taskpool (启动任务池)/js-apis-taskpool.md#taskpoolexecute>)接口启用多个线程并发执行TaskGroup中的任务，获取服务端文件数据。
5. 通过指定RCP的[Request接口](<../../../harmonyos-references/网络/Remote Communication Kit（远场通信服务）/ArkTS API/rcp（数据请求）/remote-communication-rcp.md#request>)的[ResponseBodyDestination](<../../../harmonyos-references/网络/Remote Communication Kit（远场通信服务）/ArkTS API/rcp（数据请求）/remote-communication-rcp.md#responsebodydestination>)字段中数据流的写入位置和偏移量，将每个线程请求获取的传输数据并发写入到本地的同一文件中。

### 开发步骤

说明

SFFT三方库使用RCP发送HTTP请求，因此使用了以下权限。权限设置详情参考[开发准备](<../../../harmonyos-guides/系统/网络/Remote Communication Kit（远场通信服务）/开发准备/remote-communication-preparations.md>)。

* ohos.permission.INTERNET：允许应用访问互联网。
* ohos.permission.GET\_NETWORK\_INFO：允许应用获取数据网络信息。

以下代码默认应用已经安装并导入了SFFT三方库，详细的三方库安装教程可以参考["如何使用ohpm引入三四方库"](<../../../harmonyos-faqs/DevEco Studio/命令行工具/如何使用ohpm引入三四方库/faqs-command-line-tool-13.md>)。
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/13/v3/SK-Ik9xqTaKbCIdbuUWpZQ/zh-cn_image_0000002271813645.gif?HW-CC-KV=V1&HW-CC-Date=20260611T085903Z&HW-CC-Expire=86400&HW-CC-Sign=D36A9DB956367C0E28C90EEA41E744F3FC241C375F0023A3DE40D1469C523237 "点击放大")
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/f2/v3/ig7oETAwSh2X20UDCgeuLg/zh-cn_image_0000002271853533.png?HW-CC-KV=V1&HW-CC-Date=20260611T085903Z&HW-CC-Expire=86400&HW-CC-Sign=34F8F5E081F64737B7EA44E886A250303C69B05220E57B80306E5173E97FA281 "点击放大")

### 实现原理

SFFT基于RCP实现了分片上传，实现步骤如下：

1. 对于每一个上传任务，SFFT会先使用[fileIo.createStream](<../../../harmonyos-references/Core File Kit（文件基础服务）/ArkTS API/@ohos.file.fs (文件管理)/js-apis-file-fs.md#fileiocreatestream>)创建一个基于本地文件路径的文件流。
2. 根据分片策略，SFFT会将本地文件分成若干个小块（分片），并创建与分片数量对等的Promise请求，每个Promise将计算对应分片在文件中的字节偏移量，使用Stream对象的[read()](<../../../harmonyos-references/Core File Kit（文件基础服务）/ArkTS API/@ohos.file.fs (文件管理)/js-apis-file-fs.md#fileioread>)接口读取对应的分片数据。
3. SFFT控制Promise的并发度，使用RCP的[fetch](<../../../harmonyos-references/网络/Remote Communication Kit（远场通信服务）/ArkTS API/rcp（数据请求）/remote-communication-rcp.md#fetch>)接口并发发送上传请求。
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/90/v3/5XIg7LkwTsSapqR2ZNFAfw/zh-cn_image_0000002236854260.gif?HW-CC-KV=V1&HW-CC-Date=20260611T085903Z&HW-CC-Expire=86400&HW-CC-Sign=B5EC2413973B3176880546479495F280F3A78B16F94E982C439D65EE37440AE8 "点击放大")
...
在SFFT中，断点续下基于上述多线程下载原理，结合关系型数据库[@ohos.data.relationalStore (关系型数据库)](<../../../harmonyos-references/ArkData（方舟数据管理）/ArkTS API/@ohos.data.relationalStore (关系型数据库)/js-apis-data-relationalstore.md>)实现。

![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/17/v3/mxjsijMkTseqB-FdmvIlSw/zh-cn_image_0000002236694468.png?HW-CC-KV=V1&HW-CC-Date=20260611T085903Z&HW-CC-Expire=86400&HW-CC-Sign=7FF788ABBCD5768A87069B381EAD6F2ABDF7838E6B06B5652F330CA7D0653E43 "点击放大")
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/6c/v3/rPLf9C3jTtago25iv44UzQ/zh-cn_image_0000002271813649.png?HW-CC-KV=V1&HW-CC-Date=20260611T085903Z&HW-CC-Expire=86400&HW-CC-Sign=D28B9BF93D702F547B72401DA85C6E3FEA57C5BB70D8B4B859548F3A67489550 "点击放大")
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/5a/v3/iO25wSltSRSEoycCbFlneQ/zh-cn_image_0000002236694472.gif?HW-CC-KV=V1&HW-CC-Date=20260611T085903Z&HW-CC-Expire=86400&HW-CC-Sign=87B30203C44CD7A913EBF7BCCB6E42EE876BEA879E32897146340AA8A115AA33 "点击放大")
