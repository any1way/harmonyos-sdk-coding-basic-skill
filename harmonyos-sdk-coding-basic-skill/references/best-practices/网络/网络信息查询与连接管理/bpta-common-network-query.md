...
scraped_at: 2026-06-11T16:59:15+08:00
doc_updated_at: 2026-05-22
content_hash: sha256:f8f1efd782ab6988dc779f9cefab29562d79fae5c5de7099f9bf49cc42b576b6
---
## 概述

[Network Kit](<../../../harmonyos-references/Network Kit（网络服务）/network-api.md>)提供常用的网络信息查询与连接管理功能，包括获取网络类型、检查网络可用性、监听网络状态变化、查询Wi-Fi及蜂窝网络信息等。这些能力帮助开发者灵活应对复杂多变的网络环境，精准实现各类场景需求，显著提升用户的网络使用体验。
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/f8/v3/ft-WggeVQNeL9vNTJcNC-g/zh-cn_image_0000002408087960.gif?HW-CC-KV=V1&HW-CC-Date=20260611T085903Z&HW-CC-Expire=86400&HW-CC-Sign=71AF21F54D0B64B2A7D649965ECA4C9BA8C6504314D0D1940DDAE3F0B1937521 "点击放大")

### 实现方案

连接到指定Wi-Fi场景主要通过[@ohos.wifiManager (WLAN)](<../../../harmonyos-references/网络/Connectivity Kit（短距通信服务）/ArkTS API/@ohos.wifiManager (WLAN)/js-apis-wifimanager.md>)模块结合[@ohos.net.connection (网络连接管理)](<../../../harmonyos-references/网络/Network Kit（网络服务）/ArkTS API/@ohos.net.connection (网络连接管理)/js-apis-net-connection.md>)模块相关API来实现。通过@ohos.wifiManager模块检查Wi-Fi是否启用，获取系统扫描的Wi-Fi列表，选中指定Wi-Fi后发起连接请求；通过@ohos.net.connection模块检测网络连通性，判断是否需要进行登录认证（如 Portal 认证）才能正常访问网络。流程图如下：

![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/e0/v3/1oCf97qxTw-On3DXtToKQQ/zh-cn_image_0000002441647377.png?HW-CC-KV=V1&HW-CC-Date=20260611T085903Z&HW-CC-Expire=86400&HW-CC-Sign=F8070F36319663E128F1778DFDA9901D643FCAA413E21A04361E4C142C12F2F9 "点击放大")
...
2. 获取Wi-Fi信息，判断是否连接到指定Wi-Fi
   ...
   先使用[getDefaultNetSync()](<../../../harmonyos-references/网络/Network Kit（网络服务）/ArkTS API/@ohos.net.connection (网络连接管理)/js-apis-net-connection.md#connectiongetdefaultnetsync9>)接口判断默认网络是否连接，并通过[getNetCapabilitiesSync()](<../../../harmonyos-references/网络/Network Kit（网络服务）/ArkTS API/@ohos.net.connection (网络连接管理)/js-apis-net-connection.md#connectiongetnetcapabilitiessync10>)方法获取默认连接网络类型。若网络类型为Wi-Fi，则使用[getLinkedInfoSync()](<../../../harmonyos-references/网络/Connectivity Kit（短距通信服务）/ArkTS API/@ohos.wifiManager (WLAN)/js-apis-wifimanager.md#wifimanagergetlinkedinfosync18>)方法获取当前连接的Wi-Fi信息，该信息包含SSID等内容。将获取到的SSID与指定Wi-Fi的SSID进行比对，若一致则表示已连接到指定Wi-Fi。
   ...
   通过[getNetCapabilitiesSync()](<../../../harmonyos-references/网络/Network Kit（网络服务）/ArkTS API/@ohos.net.connection (网络连接管理)/js-apis-net-connection.md#connectiongetnetcapabilitiessync10>)方法获取网络能力信息对象，其networkCap属性包含网络的具体能力。若networkCap中包含connection.NetCap.NET\_CAPABILITY\_VALIDATED，则表示网络具备访问互联网的能力（即网络可用）；若networkCap中包含connection.NetCap.NET\_CAPABILITY\_PORTAL，则说明网络需要认证登录之后才能正常使用。
...
3. 获取系统扫描Wi-Fi列表
   ...
   在获取Wi-Fi列表之前，需要通过[isWifiActive()](<../../../harmonyos-references/网络/Connectivity Kit（短距通信服务）/ArkTS API/@ohos.wifiManager (WLAN)/js-apis-wifimanager.md#wifimanageriswifiactive>)方法判断Wi-Fi开关是否已打开。如果已打开，则通过调用[getScanInfoList()](<../../../harmonyos-references/网络/Connectivity Kit（短距通信服务）/ArkTS API/@ohos.wifiManager (WLAN)/js-apis-wifimanager.md#wifimanagergetscaninfolist10>)方法获取系统扫描附近的Wi-Fi网络，并返回一个包含所有扫描到的Wi-Fi信息的数组。数组中的每个元素包含了Wi-Fi的SSID、加密类型、信号强度等详细信息。
...
4. 连接到指定Wi-Fi
   ...
   首先，创建一个包含要连接的Wi-Fi的SSID、密码及安全类型（如WPA2\_PSK）等信息的Wi-Fi配置对象，使用[addCandidateConfig()](<../../../harmonyos-references/网络/Connectivity Kit（短距通信服务）/ArkTS API/@ohos.wifiManager (WLAN)/js-apis-wifimanager.md#wifimanageraddcandidateconfig>)方法，传入该配置对象以添加候选网络配置。然后，调用[connectToCandidateConfig()](<../../../harmonyos-references/网络/Connectivity Kit（短距通信服务）/ArkTS API/@ohos.wifiManager (WLAN)/js-apis-wifimanager.md#wifimanagerconnecttocandidateconfig>)方法发起连接请求。
...
网络状态感知的实现方案以实时监测网络状态变化并联动视频播放业务的调整为核心，主要依赖于[@ohos.net.connection（网络连接管理）](<../../../harmonyos-references/网络/Network Kit（网络服务）/ArkTS API/@ohos.net.connection (网络连接管理)/js-apis-net-connection.md>)模块和[netQuality（网络质量）](<../../../harmonyos-references/网络/Network Boost Kit（网络加速服务）/ArkTS API/netQuality（网络质量）/networkboost-netquality.md>)模块来实现。本章重点介绍网络视频播放时对网络状态变化的感知，视频播放的具体实现可参考[示例代码](bpta-common-network-query.md#section14109114818615)章节。为避免网络波动影响播放流畅性，建议开发者进行缓存处理。

网络状态的监听主要通过以下接口实现：

* [on('netCapabilitiesChange')](<../../../harmonyos-references/网络/Network Kit（网络服务）/ArkTS API/@ohos.net.connection (网络连接管理)/js-apis-net-connection.md#onnetcapabilitieschange>)：订阅网络能力变化事件，当网络类型切换（如从Wi-Fi切换到蜂窝网络）、网络状态从无到有等，该事件将被触发。
* [on('netLost')](<../../../harmonyos-references/网络/Network Kit（网络服务）/ArkTS API/@ohos.net.connection (网络连接管理)/js-apis-net-connection.md#onnetlost>)：订阅网络丢失事件，当网络严重中断或正常断开时触发该事件。
* [on('netUnavailable')](<../../../harmonyos-references/网络/Network Kit（网络服务）/ArkTS API/@ohos.net.connection (网络连接管理)/js-apis-net-connection.md#onnetunavailable>)：订阅网络不可用事件，当网络不可用时触发该事件。
* [on('netAvailable')](<../../../harmonyos-references/网络/Network Kit（网络服务）/ArkTS API/@ohos.net.connection (网络连接管理)/js-apis-net-connection.md#onnetavailable>)：订阅网络可用事件，当网络可用时触发该事件。
* [netQuality.on( 'netSceneChange')](<../../../harmonyos-references/网络/Network Boost Kit（网络加速服务）/ArkTS API/netQuality（网络质量）/networkboost-netquality.md#netqualityon-netscenechange>)：订阅网络场景信息，如从正常网络进入到弱网环境。
...
1. 订阅网络可用/不可用事件
   ...
   使用[on('netAvailable')](<../../../harmonyos-references/网络/Network Kit（网络服务）/ArkTS API/@ohos.net.connection (网络连接管理)/js-apis-net-connection.md#onnetavailable>)订阅网络可用事件通知，接收到网络可用通知时，检测当前网络是否具备访问Internet的能力。若网络能正常访问Internet且此前因网络问题导致播放失败，则重置播放器并继续播放视频。
   ...
   使用[on('netUnavailable')](<../../../harmonyos-references/网络/Network Kit（网络服务）/ArkTS API/@ohos.net.connection (网络连接管理)/js-apis-net-connection.md#onnetunavailable>)订阅网络不可用事件通知，接收到网络不可用事件时，使用Toast弹窗提示用户网络不可用。
...
2. 订阅网络能力变化事件
   ...
   通过[on('netCapabilitiesChange')](<../../../harmonyos-references/网络/Network Kit（网络服务）/ArkTS API/@ohos.net.connection (网络连接管理)/js-apis-net-connection.md#onnetcapabilitieschange>)方法可以订阅Wi-Fi和蜂窝网络切换的事件通知，当网络切换为蜂窝时暂停播放，否则继续播放视频。
   ...
   ![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/d5/v3/z1ONotYNRd2tVrHO6SB-kQ/zh-cn_image_0000002407928064.png?HW-CC-KV=V1&HW-CC-Date=20260611T085903Z&HW-CC-Expire=86400&HW-CC-Sign=719D2AF43DF960B062678040A91EE38249377920AB1627F29EDC5760EBC2AA25 "点击放大")
3. 订阅网络丢失事件

   通过[on('netLost')](<../../../harmonyos-references/网络/Network Kit（网络服务）/ArkTS API/@ohos.net.connection (网络连接管理)/js-apis-net-connection.md#onnetlost>)方法可以订阅网络丢失的事件通知，使用Toast提示用户网络已断开。
   ...
   ![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/73/v3/YN7ioBfLTJmW0xVoF3i9Bw/zh-cn_image_0000002441567241.png?HW-CC-KV=V1&HW-CC-Date=20260611T085903Z&HW-CC-Expire=86400&HW-CC-Sign=E70B73FBE740D90C8FE267555CCE731F510CCED134C1DC650BFB12F8FEDD0A0C "点击放大")
   ...
   ![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/27/v3/TdVkhrOfTAG6SJ_vUUMX5A/zh-cn_image_0000002408087964.png?HW-CC-KV=V1&HW-CC-Date=20260611T085903Z&HW-CC-Expire=86400&HW-CC-Sign=F95C50022BED1E24A863159964EB62D77769927218B4FF2447DAE1A0C5541EC7 "点击放大")
4. 订阅网络状态变化通知

   接下来需要调用[register()](<../../../harmonyos-references/网络/Network Kit（网络服务）/ArkTS API/@ohos.net.connection (网络连接管理)/js-apis-net-connection.md#register>)接口，用来订阅指定的网络状态变化通知，该接口需在on()方法调用之后使用。例如，若指定的网络可用，将触发on('netAvailable')、on('netCapabilitiesChange')回调；若超时时间内网络不可用，将触发on('netUnavailable')回调。若断网，将触发on('netLost')回调。
...
5. 订阅网络场景变化
   ...
   使用[netQuality.on('netSceneChange')](<../../../harmonyos-references/网络/Network Boost Kit（网络加速服务）/ArkTS API/netQuality（网络质量）/networkboost-netquality.md#netqualityon-netscenechange>)方法订阅网络场景变化通知，当网络为弱信号场景（weakSignal）或者拥塞场景（congestion）时，使用Toast弹窗提示用户当前网络不佳。建议开发者实现多种不同清晰度资源切换的功能，在此场景下，提示用户切换清晰度。
...
6. 取消订阅网络变化/取消订阅场景变化通知
   ...
   在退出页面时，通过调用[unregister()](<../../../harmonyos-references/网络/Network Kit（网络服务）/ArkTS API/@ohos.net.connection (网络连接管理)/js-apis-net-connection.md#unregister>)取消订阅网络状态变化通知，使用[netQuality.off('netSceneChange')](<../../../harmonyos-references/网络/Network Boost Kit（网络加速服务）/ArkTS API/netQuality（网络质量）/networkboost-netquality.md#netqualityoff-netscenechange>)取消订阅场景变化。
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/69/v3/oWy8ojxhRC2pHX1QP14XNA/zh-cn_image_0000002441647385.gif?HW-CC-KV=V1&HW-CC-Date=20260611T085903Z&HW-CC-Expire=86400&HW-CC-Sign=084C94264E3C962A71CE5EDDDE7592C0F80BC67CF1B71BFB0E8099F3AC6A91C8 "点击放大")

### 实现方案

使用[@ohos.geoLocationManager (位置服务)](<../../../harmonyos-references/Location Kit（位置服务）/ArkTS API/@ohos.geoLocationManager (位置服务)/js-apis-geolocationmanager.md>)模块的[getCurrentWifiBssidForLocating()](<../../../harmonyos-references/Location Kit（位置服务）/ArkTS API/@ohos.geoLocationManager (位置服务)/js-apis-geolocationmanager.md#geolocationmanagergetcurrentwifibssidforlocating14>)方法获取当前连接的Wi-Fi MAC地址（Bssid）。通过将获取的MAC地址与业务服务端保存的Wi-Fi MAC地址进行比对，判断打卡是否成功。关于业务服务端的实现逻辑需要开发者自己实现，本文不做介绍。

说明

需要注意的是，虽然@ohos.wifiManager的[getLinkedInfo()](<../../../harmonyos-references/网络/Connectivity Kit（短距通信服务）/ArkTS API/@ohos.wifiManager (WLAN)/js-apis-wifimanager.md#wifimanagergetlinkedinfo>)也能获取当前连接Wi-Fi的MAC地址（Bssid），但需要申请ohos.permission.GET\_WIFI\_PEERS\_MAC权限（仅系统应用可申请）才能返回真实地址，否则为随机地址，因此不推荐用于Wi-Fi打卡和其他需要真实MAC地址的场景。

### 开发步骤

1. 检查WLAN连接状态

   通过[wifiManager.isWifiActive()](<../../../harmonyos-references/网络/Connectivity Kit（短距通信服务）/ArkTS API/@ohos.wifiManager (WLAN)/js-apis-wifimanager.md#wifimanageriswifiactive>)和[wifiManager.isConnected()](<../../../harmonyos-references/网络/Connectivity Kit（短距通信服务）/ArkTS API/@ohos.wifiManager (WLAN)/js-apis-wifimanager.md#wifimanagerisconnected>)分别获取WLAN开关的状态和连接状态，确保已经连接了Wi-Fi网络。
...
3. 获取连接Wi-Fi的MAC地址
   ...
   使用[geoLocationManager.getCurrentWifiBssidForLocating()](<../../../harmonyos-references/Location Kit（位置服务）/ArkTS API/@ohos.geoLocationManager (位置服务)/js-apis-geolocationmanager.md#geolocationmanagergetcurrentwifibssidforlocating14>)方法获取Wi-Fi的MAC地址。
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/c4/v3/atpzsBZ4QQGTVw9k4NQaSg/zh-cn_image_0000002407928068.png?HW-CC-KV=V1&HW-CC-Date=20260611T085903Z&HW-CC-Expire=86400&HW-CC-Sign=DED8B2B59D31C423D11A7C90163C297E89FAE0011DBDC64D147D5C340CCE032E "点击放大")

### 实现方案

主要通过以下模块的相关API获取网络故障诊断场景所需的网络信息：

* [@ohos.net.connection (网络连接管理)](<../../../harmonyos-references/网络/Network Kit（网络服务）/ArkTS API/@ohos.net.connection (网络连接管理)/js-apis-net-connection.md>)：用于获取网络的基本信息，如网络类型、IP 地址、网关、DNS 服务器等。通过[getNetCapabilitiesSync()](<../../../harmonyos-references/网络/Network Kit（网络服务）/ArkTS API/@ohos.net.connection (网络连接管理)/js-apis-net-connection.md#connectiongetnetcapabilitiessync10>)方法获取网络能力信息，包括网络类型、网络是否可用等；[getConnectionPropertiesSync()](<../../../harmonyos-references/网络/Network Kit（网络服务）/ArkTS API/@ohos.net.connection (网络连接管理)/js-apis-net-connection.md#connectiongetconnectionpropertiessync10>)方法获取网络配置信息，包括 IP、网关、DNS等
* [@ohos.wifiManager (WLAN)](<../../../harmonyos-references/网络/Connectivity Kit（短距通信服务）/ArkTS API/@ohos.wifiManager (WLAN)/js-apis-wifimanager.md>)：获取Wi-Fi网络的信号强度、设备MAC地址等信息。使用[getLinkedInfoSync()](<../../../harmonyos-references/网络/Connectivity Kit（短距通信服务）/ArkTS API/@ohos.wifiManager (WLAN)/js-apis-wifimanager.md#wifimanagergetlinkedinfosync18>)方法获取当前连接Wi-Fi的信号强度（rssi属性）和MAC 地址（bssid属性）。
* [@ohos.telephony.radio (网络搜索)](<../../../harmonyos-references/Telephony Kit（蜂窝通信服务）/ArkTS API/@ohos.telephony.radio (网络搜索)/js-apis-radio.md>)：获取蜂窝网络的相关信息，如运营商名称、网络制式、信号强度、是否处于漫游状态以及主卡Radio是否打开等。例如，[getOperatorNameSync()](<../../../harmonyos-references/Telephony Kit（蜂窝通信服务）/ArkTS API/@ohos.telephony.radio (网络搜索)/js-apis-radio.md#radiogetoperatornamesync10>)方法获取运营商名称，[getSignalInformationSync()](<../../../harmonyos-references/Telephony Kit（蜂窝通信服务）/ArkTS API/@ohos.telephony.radio (网络搜索)/js-apis-radio.md#radiogetsignalinformationsync10>)方法获取信号强度信息和网络制式。
* [netQuality(网络质量)](<../../../harmonyos-references/网络/Network Boost Kit（网络加速服务）/ArkTS API/netQuality（网络质量）/networkboost-netquality.md>)：提供网络质量实时评估、网络场景识别以及弱信号预测等能力。使用[netQuality.on( 'netQosChange')](<../../../harmonyos-references/网络/Network Boost Kit（网络加速服务）/ArkTS API/netQuality（网络质量）/networkboost-netquality.md#netqualityon-netqoschange>)订阅网络质量信息，包含信号强度、下载速度、网络时延等。
...
以上检测项都未异常，则检测结果为网络正常。其中信号强度、时延和下载速度的获取，通过开启下载任务[request.agent.create()](<../../../harmonyos-references/基础功能/Basic Services Kit（基础服务）/ArkTS API/数据文件处理/@ohos.request (上传下载)/js-apis-request.md#requestagentcreate10-1>)，取10秒的平均值。

### 开发步骤

1. 获取网络连接信息

   该步骤整合了通过@ohos.net.connection模块可获取的各类网络基础连接信息，通过[getNetCapabilitiesSync()](<../../../harmonyos-references/网络/Network Kit（网络服务）/ArkTS API/@ohos.net.connection (网络连接管理)/js-apis-net-connection.md#connectiongetnetcapabilitiessync10>)方法获取网络类型和是否可用。
   ...
   通过[getConnectionPropertiesSync()](<../../../harmonyos-references/网络/Network Kit（网络服务）/ArkTS API/@ohos.net.connection (网络连接管理)/js-apis-net-connection.md#connectiongetconnectionpropertiessync10>)方法获取路由信息、IP信息、DNS服务信息，通过[getDefaultHttpProxy()](<../../../harmonyos-references/网络/Network Kit（网络服务）/ArkTS API/@ohos.net.connection (网络连接管理)/js-apis-net-connection.md#connectiongetdefaulthttpproxy10-1>)方法获取代理信息。
...
2. 获取Wi-Fi信息
   ...
   通过@ohos.wifiManager模块的[getLinkedInfoSync()](<../../../harmonyos-references/网络/Connectivity Kit（短距通信服务）/ArkTS API/@ohos.wifiManager (WLAN)/js-apis-wifimanager.md#wifimanagergetlinkedinfosync18>)方法获取信号强度和MAC地址信息，这些信息对诊断Wi-Fi连接不稳定、信号弱等问题至关重要。
...
3. 获取蜂窝网络信息
   ...
   * 使用[radio.getOperatorNameSync()](<../../../harmonyos-references/Telephony Kit（蜂窝通信服务）/ArkTS API/@ohos.telephony.radio (网络搜索)/js-apis-radio.md#radiogetoperatornamesync10>)方法获取运营商名称。
   * 使用[radio.isRadioOn()](<../../../harmonyos-references/Telephony Kit（蜂窝通信服务）/ArkTS API/@ohos.telephony.radio (网络搜索)/js-apis-radio.md#radioisradioon7-1>)方法检查蜂窝主卡的Radio是否开启。
   * 使用[radio.getNetworkState()](<../../../harmonyos-references/Telephony Kit（蜂窝通信服务）/ArkTS API/@ohos.telephony.radio (网络搜索)/js-apis-radio.md#radiogetnetworkstate-2>)方法判断蜂窝网络是否处于漫游状态。
   * 使用[radio.getSignalInformationSync()](<../../../harmonyos-references/Telephony Kit（蜂窝通信服务）/ArkTS API/@ohos.telephony.radio (网络搜索)/js-apis-radio.md#radiogetsignalinformationsync10>)方法获取蜂窝网络信号强度和网络制式。
