...
scraped_at: 2026-06-12T10:06:51+08:00
doc_updated_at: 2026-05-18
content_hash: sha256:b49682eac427c6f5d22898851fe5944b1bda3dd2ae417ed0529d88a4a443a52a
---
## 概述
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/a4/v3/FqSVj5jqRcqAOJEfWc_Zgg/zh-cn_image_0000002229451785.png?HW-CC-KV=V1&HW-CC-Date=20260612T020647Z&HW-CC-Expire=86400&HW-CC-Sign=8902311A0E0A7D4A4EB13743B0D90F5AC2DAD3A6E99AD033629B4A631C1EBBB3 "点击放大")

### 实现原理

**关键技术**

[Canvas](../../../../harmonyos-references/ArkUI（方舟UI框架）/ArkTS组件/画布绘制/Canvas/ts-components-canvas-canvas.md)提供画布组件，用于自定义绘制图形。使用[CanvasRenderingContext2D](../../../../harmonyos-references/ArkUI（方舟UI框架）/ArkTS组件/画布绘制/CanvasRenderingContext2D/ts-canvasrenderingcontext2d.md)对象在[Canvas](../../../../harmonyos-references/ArkUI（方舟UI框架）/ArkTS组件/画布绘制/Canvas/ts-components-canvas-canvas.md)组件上进行绘制，其中[fillText()](../../../../harmonyos-references/ArkUI（方舟UI框架）/ArkTS组件/画布绘制/CanvasRenderingContext2D/ts-canvasrenderingcontext2d.md#filltext)方法用于绘制文本，[drawImage()](../../../../harmonyos-references/ArkUI（方舟UI框架）/ArkTS组件/画布绘制/CanvasRenderingContext2D/ts-canvasrenderingcontext2d.md#drawimage)方法用于图像绘制。

**开发流程**

1. 创建[Canvas](../../../../harmonyos-references/ArkUI（方舟UI框架）/ArkTS组件/画布绘制/Canvas/ts-components-canvas-canvas.md)画布，在画布上绘制水印。
2. 使用[Stack](../../../../harmonyos-references/ArkUI（方舟UI框架）/ArkTS组件/行列与堆叠/Stack/ts-container-stack.md)组件或[浮层overlay](../../../../harmonyos-references/ArkUI（方舟UI框架）/ArkTS组件/通用属性/基础属性/浮层/ts-universal-attributes-overlay.md#overlay)属性，将画布与UI页面组件融合显示。

### 开发步骤

1. 封装水印组件
   1. 创建Canvas组件，监听[Canvas.onReady](../../../../harmonyos-references/ArkUI（方舟UI框架）/ArkTS组件/画布绘制/Canvas/ts-components-canvas-canvas.md#事件)事件，该事件回调在Canvas组件初始化完成时或大小变化时执行，在回调中进行水印绘制draw()方法的执行。并通过设置Canvas组件的[hitTestBehavior](../../../../harmonyos-references/ArkUI（方舟UI框架）/ArkTS组件/通用属性/交互属性/触摸交互控制/触摸测试控制/ts-universal-attributes-hit-test-behavior.md#hittestbehavior)属性，使水印组件不影响其他组件的触摸测试，让页面能正常交互。
   ...
   2. 实现绘制水印draw()方法。绘制的起点默认为坐标轴的原点（画布的左上角），通过坐标轴的平移及旋转，实现在画布的不同位置、不同角度绘制水印。如果水印有一定旋转角度，想保证第一个水印能完整显示，需要对绘制的起点做平移，平移距离通过旋转角度及水印宽高计算。
      * 旋转角度大于0，由下图可知，水印沿x轴方向平移距离positionX = tan(θ) \* 水印高度，即绘制起点为(positionX, 0)。
        ...
        ![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/3a/v3/uSeY9Q97ScOSRJWnetAtZg/zh-cn_image_0000002229337289.png?HW-CC-KV=V1&HW-CC-Date=20260612T020647Z&HW-CC-Expire=86400&HW-CC-Sign=AB786B0A753F2E081D59680568EB17130938F4DF052E6126D56CBB09990D3A16 "点击放大")
      * 旋转角度小于0，由下图可知，水印沿y轴方向平移距离positionY = tan(θ) \* 水印宽度，即绘制起点为(0, positionY)。

        ![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/78/v3/sKCDwZHDRHiy0FQh0lDrdg/zh-cn_image_0000002229337285.png?HW-CC-KV=V1&HW-CC-Date=20260612T020647Z&HW-CC-Expire=86400&HW-CC-Sign=9E8849AA567258A55DC068C2180E97FED5906F0D111360D435116BBA509E0E4F "点击放大")

      最终通过[CanvasRenderingContext2D.fillText()](../../../../harmonyos-references/ArkUI（方舟UI框架）/ArkTS组件/画布绘制/CanvasRenderingContext2D/ts-canvasrenderingcontext2d.md#filltext)方法进行水印文字的绘制。
...
2. 将水印组件与UI页面组件融合显示。
   ...
   方式二：设置UI组件的overlay属性，使水印组件作为UI组件的浮层显示。注意watermarkBuilder中嵌套了一层父元素Column，所以需要同时设置Column的[hitTestBehavior](../../../../harmonyos-references/ArkUI（方舟UI框架）/ArkTS组件/通用属性/交互属性/触摸交互控制/触摸测试控制/ts-universal-attributes-hit-test-behavior.md#hittestbehavior)属性，使浮层下方页面能正常交互。
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/69/v3/TSFrDtKeSW6hHOPVEYfaoQ/zh-cn_image_0000002229451781.png?HW-CC-KV=V1&HW-CC-Date=20260612T020647Z&HW-CC-Expire=86400&HW-CC-Sign=C47E18D9107997BF29621E24AF4C5EE0BC5574E8958620AE16D47511B0E33A2E "点击放大")

### 实现原理

**关键技术**

[OffscreenCanvas](../../../../harmonyos-references/ArkUI（方舟UI框架）/ArkTS组件/画布绘制/OffscreenCanvas/ts-components-offscreencanvas.md)提供离屏画布，与[Canvas](../../../../harmonyos-references/ArkUI（方舟UI框架）/ArkTS组件/画布绘制/Canvas/ts-components-canvas-canvas.md)使用场景区别在于是否需要将画布渲染在屏幕上。使用[OffscreenCanvasRenderingContext2D](../../../../harmonyos-references/ArkUI（方舟UI框架）/ArkTS组件/画布绘制/OffscreenCanvasRenderingContext2D/ts-offscreencanvasrenderingcontext2d.md)在[OffscreenCanvas](../../../../harmonyos-references/ArkUI（方舟UI框架）/ArkTS组件/画布绘制/OffscreenCanvas/ts-components-offscreencanvas.md)上进行离屏绘制，其中[fillText()](../../../../harmonyos-references/ArkUI（方舟UI框架）/ArkTS组件/画布绘制/OffscreenCanvasRenderingContext2D/ts-offscreencanvasrenderingcontext2d.md#filltext)方法用于绘制文本，[drawImage()](../../../../harmonyos-references/ArkUI（方舟UI框架）/ArkTS组件/画布绘制/OffscreenCanvasRenderingContext2D/ts-offscreencanvasrenderingcontext2d.md#drawimage)方法用于图像绘制。

**开发流程**

1. 解析图片得到pixelMap数据。
2. 创建与图片宽高一致的[OffscreenCanvas](../../../../harmonyos-references/ArkUI（方舟UI框架）/ArkTS组件/画布绘制/OffscreenCanvas/ts-components-offscreencanvas.md)离屏画布。
3. 将图片和水印依次绘制到离屏画布上。
4. 获取离屏画布的pixelMap数据。
5. 将pixelMap数据写入文件中。
...
1. 解析图片得到pixelMap数据。
   1. 使用[resourceManager.getMediaContent()](<../../../../harmonyos-references/Localization Kit（本地化开发服务）/ArkTS API/@ohos.resourceManager (资源管理)/js-apis-resource-manager.md#getmediacontent9-1>)方法获取图片内容，得到ArrayBuffer数据。使用[image.createImageSource(buf: ArrayBuffer)](<../../../../harmonyos-references/Image Kit（图片处理服务）/ArkTS API/@ohos.multimedia.image (图片处理)/Functions/arkts-apis-image-f.md#imagecreateimagesource9-2>)方法创建图片源实例。
   ...
   2. 使用[ImageSource.getImageInfo()](<../../../../harmonyos-references/Image Kit（图片处理服务）/ArkTS API/@ohos.multimedia.image (图片处理)/Interface (ImageSource)/arkts-apis-image-imagesource.md#getimageinfo-2>)方法获取图片宽、高信息，使用[ImageSource.createPixelMap()](<../../../../harmonyos-references/Image Kit（图片处理服务）/ArkTS API/@ohos.multimedia.image (图片处理)/Interface (ImageSource)/arkts-apis-image-imagesource.md#createpixelmap7>)方法创建PixelMap对象。
...
2. 通过[OffscreenCanvas](../../../../harmonyos-references/ArkUI（方舟UI框架）/ArkTS组件/画布绘制/OffscreenCanvas/ts-components-offscreencanvas.md)离屏画布绘制图片及水印，得到融合水印后的pixelMap数据。
   1. 创建与图片宽高一致的[OffscreenCanvas](../../../../harmonyos-references/ArkUI（方舟UI框架）/ArkTS组件/画布绘制/OffscreenCanvas/ts-components-offscreencanvas.md)离屏画布，这里注意单位保持一致。
   2. 使用[OffscreenCanvasRenderingContext2D.drawImage()](../../../../harmonyos-references/ArkUI（方舟UI框架）/ArkTS组件/画布绘制/OffscreenCanvasRenderingContext2D/ts-offscreencanvasrenderingcontext2d.md#drawimage)将图片绘制到离屏画布上。
   3. 使用[OffscreenCanvasRenderingContext2D.fillText()](../../../../harmonyos-references/ArkUI（方舟UI框架）/ArkTS组件/画布绘制/OffscreenCanvasRenderingContext2D/ts-offscreencanvasrenderingcontext2d.md#filltext)将水印绘制在离屏画布的指定位置。
   4. 使用[OffscreenCanvasRenderingContext2D.getPixelMap()](../../../../harmonyos-references/ArkUI（方舟UI框架）/ArkTS组件/画布绘制/OffscreenCanvasRenderingContext2D/ts-offscreencanvasrenderingcontext2d.md#getpixelmap)以当前离屏画布指定区域内的像素创建PixelMap对象。
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/5c/v3/QHSZpqAPSnyT_PSPlO693w/zh-cn_image_0000002229337293.png?HW-CC-KV=V1&HW-CC-Date=20260612T020647Z&HW-CC-Expire=86400&HW-CC-Sign=AB9F6C0D5492568CDB32DF724FDCD38F89D346D43E0BBBB54B57E42280A75C8A "点击放大")

### 实现原理

**关键技术**

[pdfService](<../../../../harmonyos-references/PDF Kit（PDF服务）/ArkTS API/pdfService（PDF服务）/pdf-arkts-pdfservice.md>)模块为应用提供统一管理PDF页面的页眉页脚、水印、背景、批注、书签的能力。[TextWatermarkInfo](<../../../../harmonyos-references/PDF Kit（PDF服务）/ArkTS API/pdfService（PDF服务）/pdf-arkts-pdfservice.md#textwatermarkinfo>)类和[ImageWatermarkInfo](<../../../../harmonyos-references/PDF Kit（PDF服务）/ArkTS API/pdfService（PDF服务）/pdf-arkts-pdfservice.md#imagewatermarkinfo>)分别提供创建文本水印和图片水印的能力。[PdfDocument](<../../../../harmonyos-references/PDF Kit（PDF服务）/ArkTS API/pdfService（PDF服务）/pdf-arkts-pdfservice.md#pdfdocument>)类提供与文档相关能力，其中[addWatermark()](<../../../../harmonyos-references/PDF Kit（PDF服务）/ArkTS API/pdfService（PDF服务）/pdf-arkts-pdfservice.md#addwatermark>)方法用于添加水印。

**开发流程**

1. 将应用侧PDF文件写入沙箱中。
2. 使用[pdfService](<../../../../harmonyos-references/PDF Kit（PDF服务）/ArkTS API/pdfService（PDF服务）/pdf-arkts-pdfservice.md>)模块相关API加载指定沙箱路径的PDF并添加水印。

### 开发步骤

1. 使用[getRawFileContentSync()](<../../../../harmonyos-references/Localization Kit（本地化开发服务）/ArkTS API/@ohos.resourceManager (资源管理)/js-apis-resource-manager.md#getrawfilecontentsync10>)方法获取resource/rawfile目录下的PDF文件内容，使用[writeSync()](<../../../../harmonyos-references/Core File Kit（文件基础服务）/ArkTS API/@ohos.file.fs (文件管理)/js-apis-file-fs.md#writesync10>)方法写入沙箱中。
...
2. 使用[PdfController](<../../../../harmonyos-references/PDF Kit（PDF服务）/ArkTS API/pdfViewManager（PDF预览）/pdf-arkts-pdfviewmanage.md#pdfcontroller>)控制器中的[loadDocument()](<../../../../harmonyos-references/PDF Kit（PDF服务）/ArkTS API/pdfService（PDF服务）/pdf-arkts-pdfservice.md#loaddocument>)方法通过沙箱路径加载文件，显示到PDF预览组件[PdfView](<../../../../harmonyos-references/PDF Kit（PDF服务）/ArkTS组件/PdfView（PDF预览组件）/pdf-arkts-pdfview-component.md>)中。
...
4. 通过PDF文档类PdfDocument创建文档对象，使用文档对象的loadDocument()方法加载文档，addWatermark()方法添加水印、[saveDocument()](<../../../../harmonyos-references/PDF Kit（PDF服务）/ArkTS API/pdfService（PDF服务）/pdf-arkts-pdfservice.md#savedocument>)方法将添加水印后的文档保存到沙箱中。
