...
scraped_at: 2026-06-12T10:15:33+08:00
doc_updated_at: 2026-03-12
content_hash: sha256:c5606a64477083fd80544a8fd26c45a01fb6cae1dc1d58d513f4f5531a32802f
---
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/97/v3/hRFv2tBQQn6u7pBgTiFwEQ/zh-cn_image_0000002427576872.png?HW-CC-KV=V1&HW-CC-Date=20260612T021532Z&HW-CC-Expire=86400&HW-CC-Sign=9BC01342C4C12C06E2C4CF7F94AA8225FD78110B098887FBF1F291CEA5B0BFF0 "点击放大")
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/11/v3/aL0kSRIbSEyWniow2zYaGw/zh-cn_image_0000002229451945.png?HW-CC-KV=V1&HW-CC-Date=20260612T021532Z&HW-CC-Expire=86400&HW-CC-Sign=2E767AA30C7B94E2B9155C16DD09F56C190E50DAEE7F65250F40B50A68F84A8D "点击放大")
...
具体的页面和自定义组件生命周期可以参考[自定义组件生命周期](<../../../../../harmonyos-guides/应用框架/ArkUI（方舟UI框架）/UI开发 (ArkTS声明式开发范式)/学习UI范式基本语法/自定义组件/自定义组件生命周期/arkts-page-custom-components-lifecycle.md>)。
...
详细的组件嵌套性能数据分析可参考[《精简节点数》](../../../../布局与弹窗/布局优化指导/bpta-improve-layout-performance.md#section9293918175210)。
...
4. 在自定义组件中要实现插槽功能，需要使用@Builder和@BuilderParam实现。具体实现可参考：[@BuilderParam装饰器：引用@Builder函数](<../../../../../harmonyos-guides/应用框架/ArkUI（方舟UI框架）/UI开发 (ArkTS声明式开发范式)/学习UI范式基本语法/组件扩展/@BuilderParam装饰器：引用@Builder函数/arkts-builderparam.md>)。
...
具体可以参考[优先使用@Builder方法代替自定义组件](../UI组件性能优化/bpta-ui-component-performance-optimization.md#section18773182614502)。
...
**图1** 自定义组件树变化示意图  
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/26/v3/G_dVV8wsSyC8MkyMNcZGRA/zh-cn_image_0000002229337457.png?HW-CC-KV=V1&HW-CC-Date=20260612T021532Z&HW-CC-Expire=86400&HW-CC-Sign=B02D1FB328F95EB6C5D21C8000FD26F30A08951881E0BA44F1E78E51C52F53CF "点击放大")
...
ArkUI提供了[动态属性设置](../../../../../harmonyos-references/ArkUI（方舟UI框架）/ArkTS组件/通用属性/动态属性与自定义/动态属性设置/ts-universal-attributes-attribute-modifier.md)的接口，支持使用自定义AttributeModifier构建组件并配置属性。当需要给自定义组件设置较多属性时，如果将所有的属性设置都内移，会出现传递参数过多的问题，同时也会创建更多状态变量，增加参数的传递耗时。虽然减少了节点数量，但是性能没有得到有效提升。推荐使用自定义AttributeModifier的方式来动态设置自定义组件的属性，减少节点数量的同时，也避免了参数过多导致耗时的问题，具体优化示例如下所示。
...
具体可以参考[合理使用布局组件](../../../../布局与弹窗/布局优化指导/bpta-improve-layout-performance.md#section12745188175420)
...
在实现文本浮层、按压遮罩或颜色叠加等场景时，通常会采用Stack布局嵌套组件的方式。实际上有些场景直接使用组件属性或借助系统API的能力就能实现，例如使用[overlay](../../../../../harmonyos-references/ArkUI（方舟UI框架）/ArkTS组件/通用属性/基础属性/浮层/ts-universal-attributes-overlay.md#overlay)属性可以实现浮层场景，使用[ColorMetrics](<../../../../../harmonyos-references/ArkUI（方舟UI框架）/ArkTS API/UI界面/arkui/Graphics/js-apis-arkui-graphics.md#colormetrics12>)可以实现颜色叠加效果。直接使用组件属性的方式可以减少Stack布局嵌套组件的使用，从而减少嵌套组件带来的节点数。以文本浮层场景为例，如下图所示，使用overlay属性实现文本浮层比Stack组件嵌套方式少了一层Stack节点。开发这一类场景时，推荐优先使用组件属性代替嵌套组件。

**图2** 使用组件属性代替嵌套组件示意图  
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/64/v3/n4WIQg96TFGRnMidTWGHYw/zh-cn_image_0000002194011652.png?HW-CC-KV=V1&HW-CC-Date=20260612T021532Z&HW-CC-Expire=86400&HW-CC-Sign=5AB3E58616336C560E0EAB6BB375764CB9289C621ACCD669CFA85C7343C36894 "点击放大")

**使用overlay属性实现浮层**

使用overlay属性可以直接给组件添加浮层，实现堆叠的效果，常见的场景有文本浮层、按压遮罩等。相较于Stack布局嵌套组件的方式，使用overlay属性减少了Stack组件节点的创建。以增加文本浮层为例，具体优化示例如下所示。更多使用overlay属性实现浮层场景的示例可参考[浮层-示例](../../../../../harmonyos-references/ArkUI（方舟UI框架）/ArkTS组件/通用属性/基础属性/浮层/ts-universal-attributes-overlay.md#示例)。
...
系统通过[ColorMetrics](<../../../../../harmonyos-references/ArkUI（方舟UI框架）/ArkTS API/UI界面/arkui/Graphics/js-apis-arkui-graphics.md#colormetrics12>)接口提供了颜色计算能力，可以用于颜色叠加显示的场景。同样，与Stack布局嵌套组件的方式相比，直接使用ColorMetrics能力可以减少Stack层的布局节点，具体优化示例如下所示。
...
关于实践数据对比部分，具体可以参考[布局优化指导](../../../../布局与弹窗/布局优化指导/bpta-improve-layout-performance.md)和[UI组件性能优化](../UI组件性能优化/bpta-ui-component-performance-optimization.md)的数据对比部分。
