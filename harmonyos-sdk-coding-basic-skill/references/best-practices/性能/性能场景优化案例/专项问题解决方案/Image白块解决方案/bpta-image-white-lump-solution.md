...
scraped_at: 2026-06-12T10:16:10+08:00
doc_updated_at: 2026-05-18
content_hash: sha256:e4dc20952dacd1955ad7b574add1fdf2c3083e807b42d04e4b186fd679cd7b35
---
## 概述
...
* 预下载阶段：在组件创建前（如父页面初始化时），将网络图片通过[@ohos.file.fs (文件管理)](<../../../../../harmonyos-references/Core File Kit（文件基础服务）/ArkTS API/@ohos.file.fs (文件管理)/js-apis-file-fs.md>)进行提前缓存。
* 缓存复用阶段：当Image组件加载时，首先检查应用沙箱是否存在缓存。若存在，则直接读取缓存数据；若不存在，再发起网络请求。非首次请求时，该机制可避免重复下载，从而缩短白块持续时间。

**图1** Image加载网络图片两种方式对比  
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/c8/v3/qx_t0rgxRiWg3HwMEiw2_g/zh-cn_image_0000002429762245.png?HW-CC-KV=V1&HW-CC-Date=20260612T021605Z&HW-CC-Expire=86400&HW-CC-Sign=32CCBEB296AD21577C6FD89282C5077C37AD31A3F0D738D2B0116CA44BD91523 "点击放大")

说明

1. 开发者在使用Image加载较大的网络图片时，网络下载推荐使用[HTTP](<../../../../../harmonyos-guides/系统/网络/Network Kit（网络服务）/访问网络/使用HTTP访问网络/http-request.md>)工具提前预下载。
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/68/v3/uzyPM8UIQ0O3rzd5bmZUvA/zh-cn_image_0000002429764493.png?HW-CC-KV=V1&HW-CC-Date=20260612T021605Z&HW-CC-Expire=86400&HW-CC-Sign=480311FB9788CBDE12C500603394377349DB42AD406A9E426B5902AACD1A9066 "点击放大")
...
1. 在父组件里aboutToAppear()中提前发起网络请求，当父页面点击按钮跳转子页面PageOne，此时触发pixMap请求读取应用沙箱中已缓存解码的网络图片并存储在localStorage中。非首次点击时，不再重复调用getPixMap()，避免每次点击都从沙箱里读取文件。
   ...
   23. try {
   24. let file: fileIo.File = fileIo.openSync(fileUrl, fileIo.OpenMode.READ_WRITE); // Open the file in a synchronous manner
...
2. 在NetRequest.ets中定义网络请求httpRequest()，通过fileIo.access()检查文件是否存在，当文件存在时不再重复请求，并写入沙箱中。
   ...
   13. fileIo.access(fileUrl, fileIo.AccessModeType.READ).then((res) => { // Check whether files exist
   ...
   38. let outputStream: fileIo.Stream = fileIo.createStreamSync(fileUrl, 'w+');
...
下面使用trace对优化前后性能进行对比分析。开发者可以通过使用[DevEco Profiler](../../../性能分析/性能分析简介/bpta-optimization-overview.md#section2012922312284)工具来抓捕trace图并识别分析应用中的性能问题。
...
**图3** 直接使用Image加载网络数据  
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/41/v3/Qh4zxG9BQ_WARt8SGfOaKQ/zh-cn_image_0000002229337465.png?HW-CC-KV=V1&HW-CC-Date=20260612T021605Z&HW-CC-Expire=86400&HW-CC-Sign=494D329C3688C3E2F25797BBCE836E51A877622DC8226B3FBF25CD09486C0EEC "点击放大")
...
**图4** 使用预下载的方式  
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/68/v3/RW6eXcViRkSZKWwX40US2A/zh-cn_image_0000002229451957.png?HW-CC-KV=V1&HW-CC-Date=20260612T021605Z&HW-CC-Expire=86400&HW-CC-Sign=F7599764D30FFA203B079A3C100182947D77BD99BD189CEE62CA4CE924845E7F "点击放大")
