...
scraped_at: 2026-06-11T16:59:43+08:00
doc_updated_at: 2026-03-12
content_hash: sha256:fcb71ef01d2445b24f7aace7784d91fcd728a47339de37479c7769a2894c1f5a
---
## 概述

在移动设备普及的今天，相机已成为人们生活中不可或缺的工具。在移动端开发中，自定义相机具有极高的实用价值，开发者能够根据不同的应用场景和用户需求，定制独特的拍摄功能与交互体验。

如果开发者仅需调用系统相机拍摄照片或录制视频，可直接使用[CameraPicker](<../../../../harmonyos-references/Camera Kit（相机服务）/ArkTS API/@ohos.multimedia.cameraPicker (相机选择器)/js-apis-camerapicker.md>)。但构建高度自定义的相机应用或实现对相机数据流进行实时分析等复杂功能时，则需要使用[Camera Kit](<../../../../harmonyos-references/Camera Kit（相机服务）/camera-api.md>)（相机服务）访问和操作相机硬件来实现。
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/d9/v3/WGsH-TOVSW6ppahXrowtDQ/zh-cn_image_0000002400337125.gif?HW-CC-KV=V1&HW-CC-Date=20260611T085919Z&HW-CC-Expire=86400&HW-CC-Sign=8594F2EC4735E3B60A62A3A9052C26D110C5EA97A30F22400BE2C258A259547B "点击放大")
...
* [XComponent](../../../../harmonyos-references/ArkUI（方舟UI框架）/ArkTS组件/渲染绘制/XComponent/ts-basic-components-xcomponent.md)：用于满足开发者较为复杂的自定义渲染需求的渲染组件。为相机提供Surface，相机将预览流数据写入Surface，[XComponent](../../../../harmonyos-references/ArkUI（方舟UI框架）/ArkTS组件/渲染绘制/XComponent/ts-basic-components-xcomponent.md)从Surface读取数据并显示。
* [Camera Kit](<../../../../harmonyos-references/Camera Kit（相机服务）/camera-api.md>)：用于相机设备管理，相机输出流管理以及相机会话管理。相机会话用于配置输入流和输出流，以及设置闪光灯、焦距等参数。
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/57/v3/-ojS52BvQMew6UMJ90vbYg/zh-cn_image_0000002366737342.png?HW-CC-KV=V1&HW-CC-Date=20260611T085919Z&HW-CC-Expire=86400&HW-CC-Sign=2CE520DCD25AA15740F60DC5E762D6D86EE0773BBC5FF808673BDDCC3138869D "点击放大")
...
1. 在使用相机相关功能前，需要申请ohos.permission.CAMERA相机权限。申请权限分为以下两步。
   ...
   使用[AtManager.requestPermissionsFromUser()](<../../../../harmonyos-references/Ability Kit（程序框架服务）/ArkTS API/通用能力的接口(推荐)/@ohos.abilityAccessCtrl (程序访问控制管理)/js-apis-abilityaccessctrl.md#requestpermissionsfromuser9>)方法拉起弹窗请求用户授权，若用户拒绝则使用[requestPermissionOnSetting()](<../../../../harmonyos-references/Ability Kit（程序框架服务）/ArkTS API/通用能力的接口(推荐)/@ohos.abilityAccessCtrl (程序访问控制管理)/js-apis-abilityaccessctrl.md#requestpermissiononsetting12>)方法拉起权限设置弹窗，二次向用户申请授权。具体授权逻辑可根据业务自行调整，可参考[应用权限申请](../../../应用安全/应用权限申请/bpta-permission-application.md)。
...
2. 获取相机设备，创建并启动相机输入流。
   ...
   使用[camera.getCameraManager()](<../../../../harmonyos-references/Camera Kit（相机服务）/ArkTS API/@ohos.multimedia.camera (相机管理)/Functions/arkts-apis-camera-f.md#cameragetcameramanager>)方法获取cameraManager实例。
   ...
   使用[camera.getSupportedCameras()](<../../../../harmonyos-references/Camera Kit（相机服务）/ArkTS API/@ohos.multimedia.camera (相机管理)/Interface (CameraManager)/arkts-apis-camera-cameramanager.md#getsupportedcameras>)方法获取相机设备列表。通过[camera.CameraPosition](<../../../../harmonyos-references/Camera Kit（相机服务）/ArkTS API/@ohos.multimedia.camera (相机管理)/Enums/arkts-apis-camera-e.md#cameraposition>)类型获取对应的相机设备。CAMERA\_POSITION\_BACK表示后置镜头，CAMERA\_POSITION\_FRONT表示前置镜头。
   ...
   使用[camera.createCameraInput()](<../../../../harmonyos-references/Camera Kit（相机服务）/ArkTS API/@ohos.multimedia.camera (相机管理)/Interface (CameraManager)/arkts-apis-camera-cameramanager.md#createcamerainput>)方法创建该相机设备的输入流并打开相机。
...
3. 创建Surface。
   ...
   使用[XComponent](../../../../harmonyos-references/ArkUI（方舟UI框架）/ArkTS组件/渲染绘制/XComponent/ts-basic-components-xcomponent.md)组件渲染预览画面。指定其type属性为SURFACE。

   使用[getXComponentSurfaceId()](../../../../harmonyos-references/ArkUI（方舟UI框架）/ArkTS组件/渲染绘制/XComponent/ts-basic-components-xcomponent.md#getxcomponentsurfaceid9)方法获取surfaceId。

   使用[setXComponentSurfaceRect()](../../../../harmonyos-references/ArkUI（方舟UI框架）/ArkTS组件/渲染绘制/XComponent/ts-basic-components-xcomponent.md#setxcomponentsurfacerect12)方法设置surface的宽高属性为预览画面显示区域的宽高。

   使用[setXComponentSurfaceRotation()](../../../../harmonyos-references/ArkUI（方舟UI框架）/ArkTS组件/渲染绘制/XComponent/ts-basic-components-xcomponent.md#setxcomponentsurfacerotation12)方法锁定surface在屏幕旋转时的方向。
...
4. 创建预览输出流。
   1. 选择对应的[camera.SceneMode](<../../../../harmonyos-references/Camera Kit（相机服务）/ArkTS API/@ohos.multimedia.camera (相机管理)/Enums/arkts-apis-camera-e.md#scenemode11>)。拍照模式下的预览选择SceneMode.NORMAL\_PHOTO，录像模式下的预览选择SceneMode.NORMAL\_VIDEO。
   2. 根据[camera.SceneMode](<../../../../harmonyos-references/Camera Kit（相机服务）/ArkTS API/@ohos.multimedia.camera (相机管理)/Enums/arkts-apis-camera-e.md#scenemode11>)获取相机设备的输出能力。
   3. 在输出能力[CameraOutputCapability](<../../../../harmonyos-references/Camera Kit（相机服务）/ArkTS API/@ohos.multimedia.camera (相机管理)/Interfaces (其他)/arkts-apis-camera-i.md#cameraoutputcapability>)类的previewProfiles属性中查找所需规格的预览输出能力previewProfile。在profile的选择上需要注意以下两点：
   ...
   4. 使用[CameraManager.createPreviewOutput()](<../../../../harmonyos-references/Camera Kit（相机服务）/ArkTS API/@ohos.multimedia.camera (相机管理)/Interface (CameraManager)/arkts-apis-camera-cameramanager.md#createpreviewoutput>)方法创建预览输出流。
   ...
   以直板机后置相机为例，在设备自然方向下，相机的后置镜头安装角度为90度（不同设备的相机安装角度可通过[CameraDevice.cameraOrientation](<../../../../harmonyos-references/Camera Kit（相机服务）/ArkTS API/@ohos.multimedia.camera (相机管理)/Interfaces (其他)/arkts-apis-camera-i.md#cameradevice>)获取），屏幕旋转角度为0度。所以输出能力profile中的宽高与预览显示区域或surface的宽高比例是倒置的。例如在显示区域宽高为1080\*1920，所需查找profile的宽高为1920\*1080。横屏显示方向下，profile宽高与预览显示区域或surface的宽高比例保持一致。
...
5. 配置并启动相机会话。
   ...
   使用[CameraManager.createSession()](<../../../../harmonyos-references/Camera Kit（相机服务）/ArkTS API/@ohos.multimedia.camera (相机管理)/Interface (CameraManager)/arkts-apis-camera-cameramanager.md#createsession11>)方法创建相机会话Session。

   使用[Session.beginConfig()](<../../../../harmonyos-references/Camera Kit（相机服务）/ArkTS API/@ohos.multimedia.camera (相机管理)/Interface (Session)/arkts-apis-camera-session.md#beginconfig11>)方法开始相机会话配置。

   使用[Session.addInput()](<../../../../harmonyos-references/Camera Kit（相机服务）/ArkTS API/@ohos.multimedia.camera (相机管理)/Interface (Session)/arkts-apis-camera-session.md#addinput11>)方法和[Session.addOutput()](<../../../../harmonyos-references/Camera Kit（相机服务）/ArkTS API/@ohos.multimedia.camera (相机管理)/Interface (Session)/arkts-apis-camera-session.md#addoutput11>)方法分别将相机的输入流和预览输出流配置到相机会话中。

   使用[Session.commitConfig()](<../../../../harmonyos-references/Camera Kit（相机服务）/ArkTS API/@ohos.multimedia.camera (相机管理)/Interface (Session)/arkts-apis-camera-session.md#commitconfig11-1>)方法提交相机会话配置信息。

   使用[Session.start()](<../../../../harmonyos-references/Camera Kit（相机服务）/ArkTS API/@ohos.multimedia.camera (相机管理)/Interface (Session)/arkts-apis-camera-session.md#start11-1>)方法启动相机会话。
...
6. 释放资源，注意释放的顺序。
   ...
   使用[CameraOutput.release()](<../../../../harmonyos-references/Camera Kit（相机服务）/ArkTS API/@ohos.multimedia.camera (相机管理)/Interface (CameraOutput)/arkts-apis-camera-cameraoutput.md#release-1>)方法释放预览输出流。
   ...
   使用[CameraInput.close()](<../../../../harmonyos-references/Camera Kit（相机服务）/ArkTS API/@ohos.multimedia.camera (相机管理)/Interface (CameraInput)/arkts-apis-camera-camerainput.md#close-1>)方法关闭相机，使用[Session.release()](<../../../../harmonyos-references/Camera Kit（相机服务）/ArkTS API/@ohos.multimedia.camera (相机管理)/Interface (Session)/arkts-apis-camera-session.md#release11-1>)方法释放相机会话资源。
...
7. 监听预览流状态。
   ...
   注册[frameStart](<../../../../harmonyos-references/Camera Kit（相机服务）/ArkTS API/@ohos.multimedia.camera (相机管理)/Interface (PreviewOutput)/arkts-apis-camera-previewoutput.md#onframestart>)预览帧启动和[frameEnd](<../../../../harmonyos-references/Camera Kit（相机服务）/ArkTS API/@ohos.multimedia.camera (相机管理)/Interface (PreviewOutput)/arkts-apis-camera-previewoutput.md#onframeend>)预览帧结束的事件，在事件回调中做对应的业务处理。
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/84/v3/GOpKcAfCQFaQiiLSOuR-XQ/zh-cn_image_0000002400217229.gif?HW-CC-KV=V1&HW-CC-Date=20260611T085919Z&HW-CC-Expire=86400&HW-CC-Sign=498D94ADCC2A38B0F9A319073FE0E437763289555C31E0F0F65065CC85E9283D "点击放大")

预览页面中用isFront属性标识前置还是后置镜头，根据isFront获取[camera.CameraPosition](<../../../../harmonyos-references/Camera Kit（相机服务）/ArkTS API/@ohos.multimedia.camera (相机管理)/Enums/arkts-apis-camera-e.md#cameraposition>)的值。关于折叠屏CameraPosition的选择可参考[相机硬件差异](../../../一次开发，多端部署/多设备功能开发/相机硬件差异/bpta-multi-device-camera.md#section13854163154917)。
...
实现前后置切换转场动效可参考[相机基础动效](<../../../../harmonyos-guides/媒体/Camera Kit（相机服务）/开发相机应用基础能力(ArkTS)/相机基础动效(ArkTS)/camera-animation.md>)。

### 设置相机焦距

![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/40/v3/UYjEQ2-1RUGlkUeo1wl3NA/zh-cn_image_0000002366577450.gif?HW-CC-KV=V1&HW-CC-Date=20260611T085919Z&HW-CC-Expire=86400&HW-CC-Sign=5BD8B4485631B187D7493C8387202319FE6450F1A6D6153CFC118DA42A3EFC2D "点击放大")

使用[getZoomRatioRange()](<../../../../harmonyos-references/Camera Kit（相机服务）/ArkTS API/@ohos.multimedia.camera (相机管理)/Interface (ZoomQuery)/arkts-apis-camera-zoomquery.md#getzoomratiorange11>)方法获取当前相机设备支持设置的焦距范围，根据业务需求在页面上生成相应焦距的按钮。
...
在点击焦距按钮事件的回调函数中使用[setSmoothZoom()](<../../../../harmonyos-references/Camera Kit（相机服务）/ArkTS API/@ohos.multimedia.camera (相机管理)/Interface (Zoom)/arkts-apis-camera-zoom.md#setsmoothzoom11>)方法平滑变焦到按钮对应的焦距。
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/b8/v3/IUR4sTF7SbqOmtt9m1y8UQ/zh-cn_image_0000002400337137.gif?HW-CC-KV=V1&HW-CC-Date=20260611T085919Z&HW-CC-Expire=86400&HW-CC-Sign=5B31F48D09073797119BFCEAAA9AFDE8B2A5F4F27D07E3ECE7E2FBB7D19FEC30 "点击放大")

使用[setFlashMode()](<../../../../harmonyos-references/Camera Kit（相机服务）/ArkTS API/@ohos.multimedia.camera (相机管理)/Interface (Flash)/arkts-apis-camera-flash.md#setflashmode11>)方法设置闪光灯模式，在设置前需使用[isFlashModeSupported()](<../../../../harmonyos-references/Camera Kit（相机服务）/ArkTS API/@ohos.multimedia.camera (相机管理)/Interface (FlashQuery)/arkts-apis-camera-flashquery.md#isflashmodesupported11>)方法检测设备是否支持设置所选闪光灯模式。
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/d7/v3/cMluPuxOQ6SUPg7IuxEYvw/zh-cn_image_0000002366737362.gif?HW-CC-KV=V1&HW-CC-Date=20260611T085919Z&HW-CC-Expire=86400&HW-CC-Sign=D946ED6CF1E0177EE771E7C0BD8979FCE220757958BDE06A91497648FDB3A4AC "点击放大")

设置焦点[camera.Point](<../../../../harmonyos-references/Camera Kit（相机服务）/ArkTS API/@ohos.multimedia.camera (相机管理)/Interfaces (其他)/arkts-apis-camera-i.md#point>)的坐标是以充电口在右侧时横向设备方向为基准，该坐标系左上角为{ 0，0 }，右下角为{ 1，1 }。
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/6f/v3/v1vqD60RShae_N1kYonBNg/zh-cn_image_0000002400217253.png?HW-CC-KV=V1&HW-CC-Date=20260611T085919Z&HW-CC-Expire=86400&HW-CC-Sign=CD8063F31B9DEE9B6FA76ED57D83E548CAFE9527BDDA3CFFD9736C443A46E174 "点击放大")
...
在相机会话启动后，使用[setFocusMode()](<../../../../harmonyos-references/Camera Kit（相机服务）/ArkTS API/@ohos.multimedia.camera (相机管理)/Interface (Focus)/arkts-apis-camera-focus.md#setfocusmode11>)方法设置对焦模式为FOCUS\_MODE\_CONTINUOUS\_AUTO，当点击预览画面对焦时，设置对焦模式为FOCUS\_MODE\_AUTO，以支持对焦点设置。在设置前需检测相机是否支持该对焦模式。手动对焦结束后将对焦模式切换为FOCUS\_MODE\_CONTINUOUS\_AUTO，以获得更好的对焦体验。
...
使用[setFoucusPoint()](<../../../../harmonyos-references/Camera Kit（相机服务）/ArkTS API/@ohos.multimedia.camera (相机管理)/Interface (Focus)/arkts-apis-camera-focus.md#setfocuspoint11>)方法实现点击对焦。
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/71/v3/Ld39w0I_Tdiw57qQQbOsrA/zh-cn_image_0000002384579202.gif?HW-CC-KV=V1&HW-CC-Date=20260611T085919Z&HW-CC-Expire=86400&HW-CC-Sign=D88C1753A7B12F4F1549C5FB47003ABD8511C26DDD106565211F6E847D514E4E "点击放大")

在相机会话启动后，使用[setExposureMode()](<../../../../harmonyos-references/Camera Kit（相机服务）/ArkTS API/@ohos.multimedia.camera (相机管理)/Interface (AutoExposure)/arkts-apis-camera-autoexposure.md#setexposuremode11>)方法设置曝光模式为EXPOSURE\_MODE\_CONTINUOUS\_AUTO，当点击预览画面时，设置曝光模式为EXPOSURE\_MODE\_AUTO，以支持曝光区域中心点设置。在设置前需检测相机是否支持该曝光模式。手动设置结束后将曝光模式切换为EXPOSURE\_MODE\_CONTINUOUS\_AUTO，以获得更好的曝光体验。
...
点击屏幕预览区域，使用[setMeteringPoint()](<../../../../harmonyos-references/Camera Kit（相机服务）/ArkTS API/@ohos.multimedia.camera (相机管理)/Interface (AutoExposure)/arkts-apis-camera-autoexposure.md#setmeteringpoint11>)方法设置曝光区域中心点。屏幕布局坐标和相机坐标之间的转换逻辑详见[实现点击对焦](bpta-custom-camera-preview.md#section2356188242)小节。
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/6/v3/z_0XQ3W-QXSFCWvaRynAsA/zh-cn_image_0000002366577462.gif?HW-CC-KV=V1&HW-CC-Date=20260611T085919Z&HW-CC-Expire=86400&HW-CC-Sign=3904EA53735F882BDB491E2CD931378811609215DFF03591C9DC2F462ECCFF35 "点击放大")

使用[PreviewOutput.getSupportedFrameRates()](<../../../../harmonyos-references/Camera Kit（相机服务）/ArkTS API/@ohos.multimedia.camera (相机管理)/Interface (PreviewOutput)/arkts-apis-camera-previewoutput.md#getsupportedframerates12>)方法获取预览流支持的帧率范围。
...
在帧率切换按钮点击事件的回调函数中，使用[PreviewOutput.setFrameRate()](<../../../../harmonyos-references/Camera Kit（相机服务）/ArkTS API/@ohos.multimedia.camera (相机管理)/Interface (PreviewOutput)/arkts-apis-camera-previewoutput.md#setframerate12>)方法对预览帧率进行动态调整。
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/f/v3/KcOqeYzKSuSk9JXHhPykvQ/zh-cn_image_0000002400337145.gif?HW-CC-KV=V1&HW-CC-Date=20260611T085919Z&HW-CC-Expire=86400&HW-CC-Sign=621B7CA20E0434CB18C1A7EBC956D5039EBCDBDABA9077E4D8E06605A831ED25 "点击放大")

使用[PinchGesture()](../../../../harmonyos-references/ArkUI（方舟UI框架）/ArkTS组件/手势处理/基础手势/PinchGesture/ts-basic-gestures-pinchgesture.md)接口给预览区域元素绑定捏合事件。
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/97/v3/PN9_GsLvSdWGISDpAXXW7g/zh-cn_image_0000002366737386.png?HW-CC-KV=V1&HW-CC-Date=20260611T085919Z&HW-CC-Expire=86400&HW-CC-Sign=CB59AA7A4A2AEE6567CD522C473B8BA2AA694EC6AF4AC64DF0EF5B29374B4578 "点击放大")

获取预览区域的宽高，通过行数和列数计算出每条网格线的起始坐标，在[Canvas](../../../../harmonyos-references/ArkUI（方舟UI框架）/ArkTS组件/画布绘制/Canvas/ts-components-canvas-canvas.md)上进行绘制。注意设置[hitTestBehavior](../../../../harmonyos-references/ArkUI（方舟UI框架）/ArkTS组件/通用属性/交互属性/触摸交互控制/触摸测试控制/ts-universal-attributes-hit-test-behavior.md#hittestbehavior)属性为HitTestMode.Transparent，不影响下方预览区域的正常交互。
...
用[Stack](../../../../harmonyos-references/ArkUI（方舟UI框架）/ArkTS组件/行列与堆叠/Stack/ts-container-stack.md)组件将网格线组件堆叠在预览区域上层。
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/40/v3/6PWmvBFzQPm-Dx43kNlFpQ/zh-cn_image_0000002400217277.png?HW-CC-KV=V1&HW-CC-Date=20260611T085919Z&HW-CC-Expire=86400&HW-CC-Sign=0D68C5225E4D6DDB5B2B98533ED9113E6B73F397C5F0DA6A36BF313B9543AFDA "点击放大")

水平仪的实现需要用到重力加速度传感器。通过[sensor](<../../../../harmonyos-references/Sensor Service Kit（传感器服务）/ArkTS API/@ohos.sensor (传感器)/js-apis-sensor.md>)模块获取重力加速度在x, y, z轴方向上的分量。以充电口在下的竖屏方向为基准，x, y, z轴的方向如下。
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/e/v3/5o6OByyyTH-BiJ_TviXxUg/zh-cn_image_0000002366577482.png?HW-CC-KV=V1&HW-CC-Date=20260611T085919Z&HW-CC-Expire=86400&HW-CC-Sign=E8D0BE564739839314AAB26A89DCC15776E55FBB486485592DBB894156F08699 "点击放大")
...
使用[sensor.on()](<../../../../harmonyos-references/Sensor Service Kit（传感器服务）/ArkTS API/@ohos.sensor (传感器)/js-apis-sensor.md#sensoron>)方法订阅重力加速度传感器数据。
...
计算指示线的旋转角度，以及设备是否水平。设置[hitTestBehavior](../../../../harmonyos-references/ArkUI（方舟UI框架）/ArkTS组件/通用属性/交互属性/触摸交互控制/触摸测试控制/ts-universal-attributes-hit-test-behavior.md#hittestbehavior)属性为HitTestMode.Transparent，不影响下层预览区域的正常交互。
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/6/v3/v-sOP17uSxeU0Lk4u7V3lg/zh-cn_image_0000002400337165.gif?HW-CC-KV=V1&HW-CC-Date=20260611T085919Z&HW-CC-Expire=86400&HW-CC-Sign=6E594DC6B6CBFE085531475BEFE310DE60E79CC1DE653118D2E2FC64F074C1BD "点击放大")
...
预览页面初始化时，启动定时器。使用[UIObserver](<../../../../harmonyos-references/ArkUI（方舟UI框架）/ArkTS API/UI界面/@ohos.arkui.UIContext (UIContext)/Class (UIContext)/arkts-apis-uicontext-uicontext.md#getuiobserver11>)监听[willClick()](<../../../../harmonyos-references/ArkUI（方舟UI框架）/ArkTS API/UI界面/@ohos.arkui.UIContext (UIContext)/Class (UIObserver)/arkts-apis-uicontext-uiobserver.md#onwillclick12>)事件，30s内如果有操作，则重置定时器，直到30s内无任何操作，设置控制遮罩显隐的状态变量isSleeping为true，并释放相机资源。
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/25/v3/xWjU19blSXOBOAQlH05i_g/zh-cn_image_0000002366737398.gif?HW-CC-KV=V1&HW-CC-Date=20260611T085919Z&HW-CC-Expire=86400&HW-CC-Sign=F290708DE2A7AB9FA5A16777A87E577B416D7DD52367BC73C75D54E2C64317D0 "点击放大")

使用[ApplicationContext.on('applicationStateChange')](<../../../../harmonyos-references/Ability Kit（程序框架服务）/ArkTS API/接口依赖的元素及定义/application/ApplicationContext (应用上下文)/js-apis-inner-application-applicationcontext.md#applicationcontextonapplicationstatechange10>)方法注册对当前应用前后台状态变化的监听。在切换至后台触发的[onApplicationBackground()](<../../../../harmonyos-references/Ability Kit（程序框架服务）/ArkTS API/Stage模型能力的接口/@ohos.app.ability.ApplicationStateChangeCallback (应用进程状态变化监听器)/js-apis-app-abil_14078489.md#applicationstatechangecallbackonapplicationbackground>)回调函数中释放相机相关资源。在切换至前台触发的[onApplicationForeground()](<../../../../harmonyos-references/Ability Kit（程序框架服务）/ArkTS API/Stage模型能力的接口/@ohos.app.ability.ApplicationStateChangeCallback (应用进程状态变化监听器)/js-apis-app-abil_14078489.md#applicationstatechangecallbackonapplicationforeground>)回调函数中重新启动相机及预览。
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/27/v3/8hgrPHMlSbu4wjwxtFr-Qw/zh-cn_image_0000002523637923.png?HW-CC-KV=V1&HW-CC-Date=20260611T085919Z&HW-CC-Expire=86400&HW-CC-Sign=7C5ADC5DC16D24082E33F208CBF44E361467203B8AE06B86A793481C6F88D24E "点击放大")

相机的[元数据](<../../../../harmonyos-guides/媒体/Camera Kit（相机服务）/开发相机应用基础能力(ArkTS)/元数据(ArkTS)/camera-metadata.md>)输出流携带了人脸检测信息，应用可配置元数据输出流并读取检测信息绘制检测框。相较于[基于Core Vision Kit的人脸检测](<../../../../harmonyos-guides/AI/Core Vision Kit（基础视觉服务）/人脸检测/core-vision-face-detector.md>)能力，元数据输出流在相机预览时返回数据更快，性能更好，具体对比如下：
...
3. 注册[on('metadataObjectsAvailable')](<../../../../harmonyos-references/Camera Kit（相机服务）/ArkTS API/@ohos.multimedia.camera (相机管理)/Interface (MetadataOutput)/arkts-apis-camera-metadataoutput.md#onmetadataobjectsavailable>)回调，监听元数据流中的人脸信息。
...
6. 使用[Line](../../../../harmonyos-references/ArkUI（方舟UI框架）/ArkTS组件/图形绘制/Line/ts-drawing-components-line.md)组件，将人脸检测框绘制到预览画面中。
...
在开发相机应用时，如果预览流仅用于展示，通常使用[XComponent](../../../../harmonyos-references/ArkUI（方舟UI框架）/ArkTS组件/渲染绘制/XComponent/ts-basic-components-xcomponent.md)组件实现。若需要获取每帧的图像做二次处理（例如二维码识别或人脸识别等场景），则需要通过[ImageReceiver](<../../../../harmonyos-references/Image Kit（图片处理服务）/ArkTS API/@ohos.multimedia.image (图片处理)/Interface (ImageReceiver)/arkts-apis-image-imagereceiver.md>)监听预览流每帧数据，并创建第二路预览流，也称为双路预览。

### 实现原理

**关键技术**

* [ImageReceiver](<../../../../harmonyos-references/Image Kit（图片处理服务）/ArkTS API/@ohos.multimedia.image (图片处理)/Interface (ImageReceiver)/arkts-apis-image-imagereceiver.md>)用于创建Surface接收每帧的图像数据。
* 通过[ImageReceiver](<../../../../harmonyos-references/Image Kit（图片处理服务）/ArkTS API/@ohos.multimedia.image (图片处理)/Interface (ImageReceiver)/arkts-apis-image-imagereceiver.md>)中的imageArrival事件监听预览流每帧数据，解析图像内容。

* 判断图像宽度width与stride是否一致，不一致则进行裁剪（stride指图像的一行数据在内存中实际占用的字节数，为了内存对齐和提高读取效率的要求，通常大于图像的宽度）。可参考[相机预览花屏解决方案](../相机预览花屏解决方案/bpta-deal-stride-solution.md)。
* 屏幕处于不同的显示方向时，原始图像数据需旋转不同的角度，以确保图像在合适的方向显示。需考虑屏幕旋转角度、相机镜头安装角度，框架具体的实现机制可参考[屏幕旋转角度](<../../../../harmonyos-guides/媒体/Camera Kit（相机服务）/开发相机应用基础能力(ArkTS)/相机旋转/相机旋转角度的术语/camera-rotation-term.md#屏幕旋转角度>)。在实际开发中，推荐通过[PreviewOutput.getPreviewRotation()](<../../../../harmonyos-references/Camera Kit（相机服务）/ArkTS API/@ohos.multimedia.camera (相机管理)/Interface (PreviewOutput)/arkts-apis-camera-previewoutput.md#getpreviewrotation12>)方法直接获取旋转角度。
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/3e/v3/99YWta9FQOuoAoCFODif6A/zh-cn_image_0000002400217289.png?HW-CC-KV=V1&HW-CC-Date=20260611T085919Z&HW-CC-Expire=86400&HW-CC-Sign=85CD2BC4C1AA78BE2ABE12B0A7550F313D7C6968A68EB2E7A13A8A65327C45E3 "点击放大")

### 开发步骤

1. 创建[ImageReceiver](<../../../../harmonyos-references/Image Kit（图片处理服务）/ArkTS API/@ohos.multimedia.image (图片处理)/Interface (ImageReceiver)/arkts-apis-image-imagereceiver.md>)组件并获取surfaceId。
...
3. 使用[ImageReceiver.on()](<../../../../harmonyos-references/Image Kit（图片处理服务）/ArkTS API/@ohos.multimedia.image (图片处理)/Interface (ImageReceiver)/arkts-apis-image-imagereceiver.md#on9>)方法，注册imageArrival事件接收图像数据。使用[ImageReceiver.read()](<../../../../harmonyos-references/Image Kit（图片处理服务）/ArkTS API/@ohos.multimedia.image (图片处理)/Interface (ImageReceiver)/arkts-apis-image-imagereceiver.md#readnextimage9-1>)方法和[image.getComponent()](<../../../../harmonyos-references/Image Kit（图片处理服务）/ArkTS API/@ohos.multimedia.image (图片处理)/Interface (Image)/arkts-apis-image-image.md#getcomponent9-1>)方法解析获取图像的Buffer。
...
4. 根据stride和图像宽高对Buffer数据进行裁剪。使用[image.createPixelMap()](<../../../../harmonyos-references/Image Kit（图片处理服务）/ArkTS API/@ohos.multimedia.image (图片处理)/Functions/arkts-apis-image-f.md#imagecreatepixelmap8>)方法创建pixelMap数据。

   说明

   注意在使用createPixelMap()方法处理Buffer数据时，传入的Buffer数据的像素格式[(srcPixelFormat: PixelMapFormat)](<../../../../harmonyos-references/Image Kit（图片处理服务）/ArkTS API/@ohos.multimedia.image (图片处理)/Enums/arkts-apis-image-e.md#pixelmapformat7>)要与获取预览输出流能力Profile中的[(format: CameraFormat)](<../../../../harmonyos-references/Camera Kit（相机服务）/ArkTS API/@ohos.multimedia.camera (相机管理)/Enums/arkts-apis-camera-e.md#cameraformat>)输出格式保持一致，防止出现图像数据显示异常。format格式之间的映射关系可参考[双路预览](<../../../../harmonyos-guides/媒体/Camera Kit（相机服务）/开发相机应用基础能力(ArkTS)/双路预览(ArkTS)/camera-dual-channel-preview.md#用于处理图像的第一路预览流>)。
...
5. 使用[PreviewOutput.getPreviewRotation()](<../../../../harmonyos-references/Camera Kit（相机服务）/ArkTS API/@ohos.multimedia.camera (相机管理)/Interface (PreviewOutput)/arkts-apis-camera-previewoutput.md#getpreviewrotation12>)获取图像旋转角度，使用[PixelMap.rotate()](<../../../../harmonyos-references/Image Kit（图片处理服务）/ArkTS API/@ohos.multimedia.image (图片处理)/Interface (PixelMap)/arkts-apis-image-pixelmap.md#rotate9-1>)方法对图像数据进行旋转。在使用前置镜头时，存在水平镜像和垂直镜像的差异，为了统一翻转逻辑，在屏幕旋转角度为90度或270度时，需额外旋转180度将图像转正，使用[PixelMap.flip()](<../../../../harmonyos-references/Image Kit（图片处理服务）/ArkTS API/@ohos.multimedia.image (图片处理)/Interface (PixelMap)/arkts-apis-image-pixelmap.md#flip9-1>)方法将图像数据进行水平翻转，以达到镜像效果。参考[应用自绘制预览角度处理](<../../../../harmonyos-guides/媒体/Camera Kit（相机服务）/开发相机应用基础能力(ArkTS)/相机旋转/相机旋转角度的术语/camera-rotation-term.md#应用自绘制预览角度处理>)。
...
6. 对[ImageReceiver](<../../../../harmonyos-references/Image Kit（图片处理服务）/ArkTS API/@ohos.multimedia.image (图片处理)/Interface (ImageReceiver)/arkts-apis-image-imagereceiver.md>)组件获取到的图像数据处理后，需要将对应的图像Buffer释放，以确保Surface的BufferQueue正常轮转，防止出现缓冲区溢出等问题。如果对Buffer进行异步操作，则需要在异步操作结束后，确保当前Buffer没有使用的情况下再释放该资源。
