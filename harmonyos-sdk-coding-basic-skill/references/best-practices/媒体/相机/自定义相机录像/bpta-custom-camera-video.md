...
scraped_at: 2026-06-11T16:59:49+08:00
doc_updated_at: 2026-03-19
content_hash: sha256:1ad1b600d2a51e70745bb4b2bb351161cb48a05fe4f6739977e01c57bb7b5dcb
---
## 概述
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/72/v3/YF6jt0HURu6m37GqMkg_gQ/zh-cn_image_0000002401601769.gif?HW-CC-KV=V1&HW-CC-Date=20260611T085944Z&HW-CC-Expire=86400&HW-CC-Sign=92646B465D73B55AD488A3C6FFFADE2F1D4A5AD4DADA3B0C287228AB55522C7A "点击放大")

### 开发步骤

![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/44/v3/6BHIZTmwRT2l6dr5e7WJpQ/zh-cn_image_0000002367922004.png?HW-CC-KV=V1&HW-CC-Date=20260611T085944Z&HW-CC-Expire=86400&HW-CC-Sign=92C93A898DAD658A5143A9CDD5EA4143B8A4503FB484BB45B58A0C92290619C8 "点击放大")

![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/2b/v3/PjiRW0K_QLGD6MlU8ZYP7A/zh-cn_image_0000002383780282.png?HW-CC-KV=V1&HW-CC-Date=20260611T085944Z&HW-CC-Expire=86400&HW-CC-Sign=03DD770FD00D050DE3D8219F7292CDAC9061065722DD17E25FC0FA151C2D420B "点击放大")
...
3. 获取Surface
   ...
   系统提供的media接口可以创建一个录像AVRecorder实例，通过该实例的[getInputSurface()](<../../../../harmonyos-references/Media Kit（媒体服务）/ArkTS API/@ohos.multimedia.media (媒体服务)/Interface (AVRecorder)/arkts-apis-media-avrecorder.md#getinputsurface9-1>)方法获取SurfaceId，用于后续录像输出流的关联，处理录像输出流的数据。
...
4. 创建录像输出流
   ...
   通过[CameraOutputCapability](<../../../../harmonyos-references/Camera Kit（相机服务）/ArkTS API/@ohos.multimedia.camera (相机管理)/Interfaces (其他)/arkts-apis-camera-i.md#cameraoutputcapability>)模块中的videoProfiles属性，可获取当前设备支持的录像输出流配置，根据设备能力和目标配置，选择合适的视频配置。在当前示例中，演示了根据不同相机位置、图片质量去设置不同的分辨率和帧率，最后通过[createVideoOutput()](<../../../../harmonyos-references/Camera Kit（相机服务）/ArkTS API/@ohos.multimedia.camera (相机管理)/Interface (CameraManager)/arkts-apis-camera-cameramanager.md#createvideooutput>)方法创建录像输出流。
...
5. 启动录像
   ...
   先通过videoOutput的[start()](<../../../../harmonyos-references/Camera Kit（相机服务）/ArkTS API/@ohos.multimedia.camera (相机管理)/Interface (VideoOutput)/arkts-apis-camera-videooutput.md#start-1>)方法启动录像输出流，再通过avRecorder的[start()](<../../../../harmonyos-references/Media Kit（媒体服务）/ArkTS API/@ohos.multimedia.media (媒体服务)/Interface (AVRecorder)/arkts-apis-media-avrecorder.md#start9>)方法开始录像。如需实现前置摄像头录像功能，先通过[isMirrorSupported()](<../../../../harmonyos-references/Camera Kit（相机服务）/ArkTS API/@ohos.multimedia.camera (相机管理)/Interface (VideoOutput)/arkts-apis-camera-videooutput.md#ismirrorsupported15>)方法判断设备是否支持镜像功能；如果支持且当前为前置摄像头，则调用[enableMirror()](<../../../../harmonyos-references/Camera Kit（相机服务）/ArkTS API/@ohos.multimedia.camera (相机管理)/Interface (VideoOutput)/arkts-apis-camera-videooutput.md#enablemirror15>)方法开启镜像效果。
...
6. 暂停录像
   ...
   先通过avRecorder的[pause()](<../../../../harmonyos-references/Media Kit（媒体服务）/ArkTS API/@ohos.multimedia.media (媒体服务)/Interface (AVRecorder)/arkts-apis-media-avrecorder.md#pause9-1>)方法暂停录像，再通过videoOutput的[stop()](<../../../../harmonyos-references/Camera Kit（相机服务）/ArkTS API/@ohos.multimedia.camera (相机管理)/Interface (VideoOutput)/arkts-apis-camera-videooutput.md#stop-1>)方法停止录像输出流。
...
7. 恢复录像
   ...
   先通过videoOutput的[start()](<../../../../harmonyos-references/Camera Kit（相机服务）/ArkTS API/@ohos.multimedia.camera (相机管理)/Interface (VideoOutput)/arkts-apis-camera-videooutput.md#start-1>)方法启动录像输出流，再通过avRecorder的[resume()](<../../../../harmonyos-references/Media Kit（媒体服务）/ArkTS API/@ohos.multimedia.media (媒体服务)/Interface (AVRecorder)/arkts-apis-media-avrecorder.md#resume9-1>)方法恢复录像。
...
8. 停止录像
   ...
   先通过avRecorder的[stop()](<../../../../harmonyos-references/Media Kit（媒体服务）/ArkTS API/@ohos.multimedia.media (媒体服务)/Interface (AVRecorder)/arkts-apis-media-avrecorder.md#stop9-1>)方法停止录像，再通过videoOutput的[stop()](<../../../../harmonyos-references/Camera Kit（相机服务）/ArkTS API/@ohos.multimedia.camera (相机管理)/Interface (VideoOutput)/arkts-apis-camera-videooutput.md#stop-1>)方法停止录像输出流。
...
9. 释放资源
   ...
   先通过avRecorder的[release()](<../../../../harmonyos-references/Media Kit（媒体服务）/ArkTS API/@ohos.multimedia.media (媒体服务)/Interface (AVRecorder)/arkts-apis-media-avrecorder.md#release9-1>)方法释放录像资源，再通过videoOutput的[release()](<../../../../harmonyos-references/Camera Kit（相机服务）/ArkTS API/@ohos.multimedia.camera (相机管理)/Interface (CameraOutput)/arkts-apis-camera-cameraoutput.md#release-1>)方法释放输出流。
...
通过session中[setVideoStabilizationMode()](<../../../../harmonyos-references/Camera Kit（相机服务）/ArkTS API/@ohos.multimedia.camera (相机管理)/Interface (Stabilization)/arkts-apis-camera-stabilization.md#setvideostabilizationmode11>)方法可以设置视频防抖模式，详细请参见[Stabilization](<../../../../harmonyos-references/Camera Kit（相机服务）/ArkTS API/@ohos.multimedia.camera (相机管理)/Interface (Stabilization)/arkts-apis-camera-stabilization.md#setvideostabilizationmode11>)。
...
录像的旋转角度与重力方向（即设备旋转角度）相关。调用[VideoOutput](<../../../../harmonyos-references/Camera Kit（相机服务）/ArkTS API/@ohos.multimedia.camera (相机管理)/Interface (VideoOutput)/arkts-apis-camera-videooutput.md>)类中的[getVideoRotation()](<../../../../harmonyos-references/Camera Kit（相机服务）/ArkTS API/@ohos.multimedia.camera (相机管理)/Interface (VideoOutput)/arkts-apis-camera-videooutput.md#getvideorotation12>)可以获取到录像的旋转角度。详细请参见[适配相机旋转角度(ArkTS)](<../../../../harmonyos-guides/媒体/Camera Kit（相机服务）/开发相机应用基础能力(ArkTS)/相机旋转/适配相机旋转角度(ArkTS)/camera-rotation-angle-adaptation.md#录像>) 。
...
HDR Vivid是UWA认证的动态HDR视频标准，能够拍摄出层次更丰富、光影细节更鲜明的画面，显著提升画面质感。应用仅需接入媒体领域提供的API，即可集成HarmonyOS的HDR Vivid视频采集、转码和解码显示功能。与普通录像相比，HDR录像需要开启视频防抖，随后查询设备支持的色彩空间列表，最终通过调用[setColorSpace()](<../../../../harmonyos-references/Camera Kit（相机服务）/ArkTS API/@ohos.multimedia.camera (相机管理)/Interface (ColorManagement)/arkts-apis-camera-colormanagement.md#setcolorspace12>)方法完成色彩空间设置。普通录像无需执行这些步骤。详细请参见[HDR Vivid相机录像(ArkTS)](<../../../../harmonyos-guides/媒体/Camera Kit（相机服务）/开发相机应用基础能力(ArkTS)/HDR Vivid相机录像(ArkTS)/camera-hdr-recording.md>)。
