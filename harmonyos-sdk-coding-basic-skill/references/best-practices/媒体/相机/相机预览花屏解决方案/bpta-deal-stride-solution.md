...
content_hash: sha256:03dd72e4f3cc33d7bbadddd478980d5202c99e6a61b8b865ed06ce619fa94c7c
---
## 概述

开发者在[使用相机服务](<../../../../harmonyos-guides/媒体/Camera Kit（相机服务）/camera-kit.md>)时，如果仅用于预览流展示，通常使用[XComponent](../../../../harmonyos-references/ArkUI（方舟UI框架）/ArkTS组件/渲染绘制/XComponent/ts-basic-components-xcomponent.md)组件实现，如果需要获取每帧图像做二次处理(例如获取每帧图像完成二维码识别或人脸识别场景)，可以通过[ImageReceiver](<../../../../harmonyos-references/Image Kit（图片处理服务）/ArkTS API/@ohos.multimedia.image (图片处理)/Interface (ImageReceiver)/arkts-apis-image-imagereceiver.md>)中imageArrival事件监听预览流每帧数据，解析图像内容。在解析图像内容时，如果未考虑stride，直接通过使用width\*height读取图像内容去解析图像，会导致相机预览异常，从而出现相机预览花屏的现象。
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/1f/v3/usz_17gjR0WDUTU4MSWEKA/zh-cn_image_0000002194011680.png?HW-CC-KV=V1&HW-CC-Date=20260611T085917Z&HW-CC-Expire=86400&HW-CC-Sign=DC335D82601023C55C1643537C9BA7509F1A6862C8F6108EAEFD26A67994BD10 "点击放大")
...
通过ImageReceiver中imageArrival事件监听获取底层返回的图像数据，详细请参见[双路预览(ArkTS)](<../../../../harmonyos-guides/媒体/Camera Kit（相机服务）/开发相机应用基础能力(ArkTS)/双路预览(ArkTS)/camera-dual-channel-preview.md#开发步骤>)。
