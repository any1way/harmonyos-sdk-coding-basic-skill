...
content_hash: sha256:552173e78ac5419ea85aa76f91d4f8ccfa0c007f676295a72a05e11b1dc49117
---
## 概述

在个人相册制作、电商产品展示、理财销售回放等多个场景中，都需要将图片合成视频。开发者通过调用Image Kit、视频编码、媒体数据封装提供的接口，可以实现图片合成视频的功能。

* [Image Kit](<../../../../harmonyos-guides/媒体/Image Kit（图片处理服务）/Image Kit简介/image-overview.md>)提供图片的解码、编码、编辑、元数据处理等功能。
* [视频编码](<../../../../harmonyos-guides/媒体/AVCodec Kit（音视频编解码服务）/音视频编解码/视频编码/video-encoding.md>)可将未压缩的视频数据压缩为视频码流，如H.264、H.265。
* [媒体数据封装](<../../../../harmonyos-guides/媒体/AVCodec Kit（音视频编解码服务）/媒体数据封装与解析/媒体数据封装/audio-video-muxer.md>)可完成媒体文件的封装，将编码后的音视频数据，按一定的格式写入媒体文件中。
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/65/v3/bB4dazS6SZKG_QA7mZadsQ/zh-cn_image_0000002529433631.gif?HW-CC-KV=V1&HW-CC-Date=20260611T090225Z&HW-CC-Expire=86400&HW-CC-Sign=4F9EA9DE2E19A92FA275E91EB0DB99C372B4A91F68431D668884BBDB4DE46B50 "点击放大")

### 实现原理

[Image Kit](<../../../../harmonyos-guides/媒体/Image Kit（图片处理服务）/Image Kit简介/image-overview.md>)中的[PixelMap](<../../../../harmonyos-references/Image Kit（图片处理服务）/ArkTS API/@ohos.multimedia.image (图片处理)/Interface (PixelMap)/arkts-apis-image-pixelmap.md>)是用于读取或写入图像数据以及获取图像信息的图像像素类。在图片合成视频的过程中，首先将图片解码为PixelMap，然后使用[Buffer模式](<../../../../harmonyos-guides/媒体/AVCodec Kit（音视频编解码服务）/音视频编解码/视频编码/video-encoding.md#buffer模式>)编码，将PixelMap中保存的图像数据复制到编码器的输入buffer中，未压缩的YUV输出成已压缩的视频码流H.264，编码完成后封装成视频文件。

![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/1c/v3/cJ-lOejuSGuAJBziu6uHwQ/zh-cn_image_0000002497593680.png?HW-CC-KV=V1&HW-CC-Date=20260611T090225Z&HW-CC-Expire=86400&HW-CC-Sign=DE0CF77F7D80699629D1D3E22BE4A13E9DDCA52516A76BFF13A972C74A5DB667)

如果需要使用[Surface模式](<../../../../harmonyos-guides/媒体/AVCodec Kit（音视频编解码服务）/音视频编解码/视频编码/video-encoding.md#surface模式>)编码，将图片解码成PixelMap后，需要先从编码器的NativeWindow申请buffer，然后将PixelMap中保存的图像数据复制到申请的buffer中并提交buffer，编码完成后再封装为视频文件，具体实现可以参考[NativeWindow开发指导 (C/C++)](<../../../../harmonyos-guides/图形/ArkGraphics 2D（方舟2D图形服务）/图形缓冲区/NativeWindow开发指导 (CC++)/native-window-guidelines.md>)。

### 开发步骤

1. 使用[PhotoViewPicker](<../../../../harmonyos-references/Media Library Kit（媒体文件管理服务）/ArkTS API/@ohos.file.photoAccessHelper (相册管理模块)/Class (PhotoViewPicker)/arkts-apis-photoaccess_90850778.md>)从图库中选择图片，数量不少于两张。当选择数量不足两张时，将弹出提示框。
...
2. 遍历处理从图库选择的图片。
   ...
   （2）配置图片解码参数，使用[imageSource.createPixelMap()](<../../../../harmonyos-references/Image Kit（图片处理服务）/ArkTS API/@ohos.multimedia.image (图片处理)/Interface (ImageSource)/arkts-apis-image-imagesource.md#createpixelmap7>)获取解码后的PixelMap。
...
5. 所有图片都处理完成后，使用[Video](../../../../harmonyos-references/ArkUI（方舟UI框架）/ArkTS组件/图片与视频/Video/ts-media-components-video.md)组件播放合成后的视频。
...
6. 使用[SaveButton](../../../../harmonyos-references/ArkUI（方舟UI框架）/ArkTS组件/SaveButton/ts-security-components-savebutton.md)安全控件，将视频保存到图库。
   ...
   （2）调用[MediaAssetChangeRequest.createImageAssetRequest()](<../../../../harmonyos-references/Media Library Kit（媒体文件管理服务）/ArkTS API/@ohos.file.photoAccessHelper (相册管理模块)/Class (MediaAssetChangeRequest)/kts-apis-photo_01462016.md#createimageassetrequest11>)和[PhotoAccessHelper.applyChanges()](<../../../../harmonyos-references/Media Library Kit（媒体文件管理服务）/ArkTS API/@ohos.file.photoAccessHelper (相册管理模块)/Interface (PhotoAccessHelper)/arkts-apis-photo_93807367.md#applychanges11>)接口，将视频保存到图库。
...
将图片数据复制到编码器的输入buffer时，应依据编码器的跨距值进行复制，跨距值可通过接口[OH\_VideoEncoder\_GetInputDescription()](<../../../../harmonyos-references/AVCodec Kit（音视频编解码服务）/C API/头文件/native_avcodec_videoencoder.h/capi-native-avcodec-videoencoder-h.md#oh_videoencoder_getinputdescription>)获取。
