...
scraped_at: 2026-06-11T17:02:39+08:00
doc_updated_at: 2026-05-18
content_hash: sha256:11973175319ae487b2fba5ac723559d9cf3c1eee149923bcbcbef8ed698680a8
---
## 概述
...
| --- | --- | --- | --- |
|  | 实现方案 | 优点 | 缺点 |
| 硬件耳返 | 基于[AudioLoopback](<../../../../harmonyos-references/Audio Kit（音频服务）/ArkTS API/@ohos.multimedia.audio (音频管理)/Interface (AudioLoopback)/arkts-apis-audio-audioloopback.md>)实现音频耳返 | 1. 物理耳返，延时低。 2. 在API21，提供混响和均衡器等音效。 | 1. 当前仅支持有线耳机，不支持蓝牙耳机。 2. 在耳返的场景下，仅支持录制麦克风。 |
| 软件耳返 | 基于[OHAudio](<../../../../harmonyos-references/Audio Kit（音频服务）/C API/模块/OHAudio/capi-ohaudio.md>)实现音频耳返 | 1. 可以支持有线耳机和蓝牙耳机。 2. 可以支持耳返（使用者自己的声音或其它需要的信息等）和背景音乐同时录制写入到文件中。 | 1. 暂无硬件耳返的接口，软件耳返延时比硬件耳返高。 2. 无音效相关的接口，音效等算法需要自行实现。 |
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/6b/v3/dkqDtfJPTgiVZjooSk_8iA/zh-cn_image_0000002544829543.gif?HW-CC-KV=V1&HW-CC-Date=20260611T090230Z&HW-CC-Expire=86400&HW-CC-Sign=9C339CC32A36EBE7899E1BDFB686DBAC94D9761FA4BBEBCE9086978C8E2E0C66 "点击放大")

### 实现原理

AudioLoopback是HarmonyOS提供的音频返听接口，用于实现低时延耳返功能，支持自动创建低时延渲染器与采集器，采集的音频可直接通过内部路由返回到渲染器，实时传输到耳机。

AudioLoopback的状态变化如下图所示，在创建AudioLoopback实例后，调用对应的方法可以进入指定的状态实现对应行为。同时需要注意的是，在确定的状态执行不合适的方法，可能导致AudioLoopback发生错误，建议开发者在调用状态转换的方法前进行状态检查，避免程序运行产生预期以外的结果，详细开发指导请参考：[实现音频低时延耳返](<../../../../harmonyos-guides/媒体/Audio Kit（音频服务）/音频录制/实现音频低时延耳返/audio-ear-monitor-loopback.md>)。

![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/59/v3/hPXvoS9xQjS0SAP1iFHb-A/zh-cn_image_0000002513149636.png?HW-CC-KV=V1&HW-CC-Date=20260611T090230Z&HW-CC-Expire=86400&HW-CC-Sign=1706990FED9595D942CB6752A63B091D8EE511F54CDDFF9D3C40EAF646BF5E66 "点击放大")

### 开发步骤

使用AudioLoopback控制耳返的开启和关闭，结合[AVRecorder](<../../../../harmonyos-references/Media Kit（媒体服务）/ArkTS API/@ohos.multimedia.media (媒体服务)/Interface (AVRecorder)/arkts-apis-media-avrecorder.md>)实现音频的录制，将录制的音频保存在应用沙箱目录，并通过[@ohos.file.fs (文件管理)](<../../../../harmonyos-references/Core File Kit（文件基础服务）/ArkTS API/@ohos.file.fs (文件管理)/js-apis-file-fs.md>)打开录制的音频文件，再通过[AVPlayer](<../../../../harmonyos-references/Media Kit（媒体服务）/ArkTS API/@ohos.multimedia.media (媒体服务)/Interface (AVPlayer)/arkts-apis-media-avplayer.md>)实现已录制音频的播放控制，详细流程如下图所示：

![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/c1/v3/bGSHGZcCTu6oMwF9Vl6NrA/zh-cn_image_0000002513309570.png?HW-CC-KV=V1&HW-CC-Date=20260611T090230Z&HW-CC-Expire=86400&HW-CC-Sign=D441BEB4135E40D00F4287F2005423039E194D9F85262D32E8F8CADA942C78D5 "点击放大")

具体开发步骤如下：

1. 创建AudioLoopback。
   * 通过[isAudioLoopbackSupported()](<../../../../harmonyos-references/Audio Kit（音频服务）/ArkTS API/@ohos.multimedia.audio (音频管理)/Interface (AudioStreamManager)/arkts-apis-audio-audiostreammanager.md#isaudioloopbacksupported20>)查询当前系统是否支持音频返听模式。若支持，则调用[audio.createAudioLoopback()](<../../../../harmonyos-references/Audio Kit（音频服务）/ArkTS API/@ohos.multimedia.audio (音频管理)/Functions/arkts-apis-audio-f.md#audiocreateaudioloopback20>)接口创建音频返听器。
   ...
   * 通过AudioLoopback的[getStatus()](<../../../../harmonyos-references/Audio Kit（音频服务）/ArkTS API/@ohos.multimedia.audio (音频管理)/Interface (AudioLoopback)/arkts-apis-audio-audioloopback.md#getstatus20>)方获取音频返听状态，当返听状态为AVAILABLE\_IDLE时，表示返听可用，此时可调用[enable()](<../../../../harmonyos-references/Audio Kit（音频服务）/ArkTS API/@ohos.multimedia.audio (音频管理)/Interface (AudioLoopback)/arkts-apis-audio-audioloopback.md#enable20>)方法传入参数true，开启音频返听。
   ...
   * 当返听状态为AVAILABLE\_RUNNING时，表示返听在运行中，调用[enable()](<../../../../harmonyos-references/Audio Kit（音频服务）/ArkTS API/@ohos.multimedia.audio (音频管理)/Interface (AudioLoopback)/arkts-apis-audio-audioloopback.md#enable20>)方法传入参数false，禁用音频返听。
   ...
   * 调用[setVolume()](<../../../../harmonyos-references/Audio Kit（音频服务）/ArkTS API/@ohos.multimedia.audio (音频管理)/Interface (AudioLoopback)/arkts-apis-audio-audioloopback.md#setvolume20>)方法实现音频返听音量的调节。
...
2. 创建AVRecorder。
   * 调用[media.createAVRecorder()](<../../../../harmonyos-references/Media Kit（媒体服务）/ArkTS API/@ohos.multimedia.media (媒体服务)/Functions/arkts-apis-media-f.md#mediacreateavrecorder9>)方法，用于创建AVRecorder实例。
   ...
   * 调用[prepare()](<../../../../harmonyos-references/Media Kit（媒体服务）/ArkTS API/@ohos.multimedia.media (媒体服务)/Interface (AVRecorder)/arkts-apis-media-avrecorder.md#prepare9>)方法设置音频录制的音频编码比特率、采集声道数、编码格式、采样率和容器格式等信息。
     ...
     14. try {
     15. let audioFile: fileIo.File = fileIo.openSync(filePath, fileIo.OpenMode.READ_WRITE | fileIo.OpenMode.CREATE);
   ...
   * 调用[start()](<../../../../harmonyos-references/Media Kit（媒体服务）/ArkTS API/@ohos.multimedia.media (媒体服务)/Interface (AVRecorder)/arkts-apis-media-avrecorder.md#start9>)方法开始音频录制。
   ...
   * 调用[pause()](<../../../../harmonyos-references/Media Kit（媒体服务）/ArkTS API/@ohos.multimedia.media (媒体服务)/Interface (AVRecorder)/arkts-apis-media-avrecorder.md#pause9>)方法暂停音频录制。
   ...
   * 调用[resume()](<../../../../harmonyos-references/Media Kit（媒体服务）/ArkTS API/@ohos.multimedia.media (媒体服务)/Interface (AVRecorder)/arkts-apis-media-avrecorder.md#resume9>)方法恢复音频录制。
   ...
   * 调用[stop()](<../../../../harmonyos-references/Media Kit（媒体服务）/ArkTS API/@ohos.multimedia.media (媒体服务)/Interface (AVRecorder)/arkts-apis-media-avrecorder.md#stop9>)方法停止音频录制。
   ...
   * 调用[release()](<../../../../harmonyos-references/Media Kit（媒体服务）/ArkTS API/@ohos.multimedia.media (媒体服务)/Interface (AVRecorder)/arkts-apis-media-avrecorder.md#release9>)方法释放音频录制资源。
...
3. 创建AVPlayer。
   * 调用[media.createAVPlayer()](<../../../../harmonyos-references/Media Kit（媒体服务）/ArkTS API/@ohos.multimedia.media (媒体服务)/Functions/arkts-apis-media-f.md#mediacreateavplayer9-1>)方法创建AVPlayer实例。
   ...
   * 调用[play()](<../../../../harmonyos-references/Media Kit（媒体服务）/ArkTS API/@ohos.multimedia.media (媒体服务)/Interface (AVPlayer)/arkts-apis-media-avplayer.md#play9-1>)方法，用于播放录制的音频资源。
   ...
   * 调用[pause()](<../../../../harmonyos-references/Media Kit（媒体服务）/ArkTS API/@ohos.multimedia.media (媒体服务)/Interface (AVPlayer)/arkts-apis-media-avplayer.md#pause9-1>)方法，暂停播放音频。
   ...
   * 调用[release()](<../../../../harmonyos-references/Media Kit（媒体服务）/ArkTS API/@ohos.multimedia.media (媒体服务)/Interface (AVPlayer)/arkts-apis-media-avplayer.md#release9-1>)方法，释放播放资源。
...
4. 在页面内点击录制按钮后，请求用户授权麦克风权限，通过AudioLoopback开启耳返，同时通过AVRecorder进行耳返音频的录制。
   * 开启耳返并录制音频。
     ...
     [@ohos.abilityAccessCtrl (程序访问控制管理)](<../../../../harmonyos-references/Ability Kit（程序框架服务）/ArkTS API/通用能力的接口(推荐)/@ohos.abilityAccessCtrl (程序访问控制管理)/js-apis-abilityaccessctrl.md>)提供了应用程序的权限校验和管理能力，通过[requestPermissionsFromUser()](<../../../../harmonyos-references/Ability Kit（程序框架服务）/ArkTS API/通用能力的接口(推荐)/@ohos.abilityAccessCtrl (程序访问控制管理)/js-apis-abilityaccessctrl.md#requestpermissionsfromuser9>)接口拉起弹框，向用户请求授权麦克风权限。
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/a0/v3/csN1q1aGQFKd6bvishy_2Q/zh-cn_image_0000002544789547.gif?HW-CC-KV=V1&HW-CC-Date=20260611T090230Z&HW-CC-Expire=86400&HW-CC-Sign=A003339F127AD66CCE3E7F4DA15BE5887C67BA6C8561FD958E32F47EF04B16E5 "点击放大")
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/0b/v3/qE6rdwUFRFOKxMr-TyX_VA/zh-cn_image_0000002544829551.png?HW-CC-KV=V1&HW-CC-Date=20260611T090230Z&HW-CC-Expire=86400&HW-CC-Sign=B174C29C80EE403D978103CE00E19A688F464B4D98F6AD0EBFB9B552255E2E30 "点击放大")

### 开发步骤

1. 录制音频数据。
   * 调用[OH\_AudioStreamBuilder\_Create()](<../../../../harmonyos-references/Audio Kit（音频服务）/C API/头文件/native_audiostreambuilder.h/capi-native-audiostreambuilder-h.md#oh_audiostreambuilder_create>)，并指定[OH\_AudioStream\_Type](<../../../../harmonyos-references/Audio Kit（音频服务）/C API/头文件/native_audiostream_base.h/capi-native-audiostream-base-h.md#oh_audiostream_type>)音频流类型为AUDIOSTREAM\_TYPE\_CAPTURER，创建输入类型的音频流构造器实例，并设置输入音频流的采样率、声道数、低时延模式、采样格式、编码类型及工作场景等属性。
   ...
   * 调用[OH\_AudioCapturer\_Start()](<../../../../harmonyos-references/Audio Kit（音频服务）/C API/头文件/native_audiocapturer.h/capi-native-audiocapturer-h.md#oh_audiocapturer_start>)启动音频采集器，获取音频数据。
   ...
   * 调用[OH\_AudioCapturer\_Pause()](<../../../../harmonyos-references/Audio Kit（音频服务）/C API/头文件/native_audiocapturer.h/capi-native-audiocapturer-h.md#oh_audiocapturer_pause>)暂停音频流数据输入。
   ...
   * 调用[OH\_AudioCapturer\_Stop()](<../../../../harmonyos-references/Audio Kit（音频服务）/C API/头文件/native_audiocapturer.h/capi-native-audiocapturer-h.md#oh_audiocapturer_stop>)停止音频采集器，停止音频流数据输入。
   ...
   * 调用[OH\_AudioCapturer\_Release()](<../../../../harmonyos-references/Audio Kit（音频服务）/C API/头文件/native_audiocapturer.h/capi-native-audiocapturer-h.md#oh_audiocapturer_release>)释放输入音频流，并通过[OH\_AudioStreamBuilder\_Destroy()](<../../../../harmonyos-references/Audio Kit（音频服务）/C API/头文件/native_audiostreambuilder.h/capi-native-audiostreambuilder-h.md#oh_audiostreambuilder_destroy>)销毁输入类型的音频流构造器。
...
2. 播放音频数据。
   ...
   * 调用[OH\_AudioRenderer\_Start()](<../../../../harmonyos-references/Audio Kit（音频服务）/C API/头文件/native_audiorenderer.h/capi-native-audiorenderer-h.md#oh_audiorenderer_start>)输出音频数据。
   ...
   * 调用[OH\_AudioRenderer\_Pause()](<../../../../harmonyos-references/Audio Kit（音频服务）/C API/头文件/native_audiorenderer.h/capi-native-audiorenderer-h.md#oh_audiorenderer_pause>)暂停输出音频数据。
   ...
   * 调用[OH\_AudioRenderer\_SetVolume()](<../../../../harmonyos-references/Audio Kit（音频服务）/C API/头文件/native_audiorenderer.h/capi-native-audiorenderer-h.md#oh_audiorenderer_setvolume>)设置音频流音量值。
   ...
   * 调用[OH\_AudioRenderer\_Stop()](<../../../../harmonyos-references/Audio Kit（音频服务）/C API/头文件/native_audiorenderer.h/capi-native-audiorenderer-h.md#oh_audiorenderer_stop>)停止输出音频流。
   ...
   * 调用[OH\_AudioRenderer\_Release()](<../../../../harmonyos-references/Audio Kit（音频服务）/C API/头文件/native_audiorenderer.h/capi-native-audiorenderer-h.md#oh_audiorenderer_release>)释放输出音频流资源，并通过OH\_AudioStreamBuilder\_Destroy()销毁输出类型的音频流构造器。
