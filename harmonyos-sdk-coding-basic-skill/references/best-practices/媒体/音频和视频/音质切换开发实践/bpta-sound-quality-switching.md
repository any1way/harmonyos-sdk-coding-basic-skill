...
content_hash: sha256:6c2226590e68889964cfdfa27e4f5e08413a4beefd328d4d982bcf47cd13b093
---
## 概述
...
音质可以在未压缩、无损压缩和有损压缩之间切换，根据上述播放能力支持范围可以看出，音质切换的本质为[AudioRenderer](<../../../../harmonyos-references/Audio Kit（音频服务）/ArkTS API/@ohos.multimedia.audio (音频管理)/Interface (AudioRenderer)/arkts-apis-audio-audiorenderer.md>)与[AVPlayer](<../../../../harmonyos-references/Media Kit（媒体服务）/ArkTS API/@ohos.multimedia.media (媒体服务)/Interface (AVPlayer)/arkts-apis-media-avplayer.md>)的播放切换，以及AVPlayer与AVPlayer的播放切换（即使用AVPlayer在不同音频格式间的播放切换）。
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/87/v3/Wcwj-bGCSy2jdYavfAj7pw/zh-cn_image_0000002490682592.gif?HW-CC-KV=V1&HW-CC-Date=20260611T090215Z&HW-CC-Expire=86400&HW-CC-Sign=4F367A7A6CFD66ED0A9F7CF77D044581CF8D63AFEE836D63147E0AD27D51FE04 "点击放大")
...
2. 使用AVPlayer播放MP3文件，即无损未压缩的PCM音质切换到MP3音质（标清）时，通过AVPlayer的[seek()](<../../../../harmonyos-references/Media Kit（媒体服务）/ArkTS API/@ohos.multimedia.media (媒体服务)/Interface (AVPlayer)/arkts-apis-media-avplayer.md#seek9>)接口可直接跳转到对应时间戳位置播放。
...
2. 在AVPlayerController类中，定义如下方法：
   * reset()：在其中调用AVPlayer实例的[reset()](<../../../../harmonyos-references/Media Kit（媒体服务）/ArkTS API/@ohos.multimedia.media (媒体服务)/Interface (AVPlayer)/arkts-apis-media-avplayer.md#reset9>)方法，重置播放。
   * pause()：在其中调用AVPlayer实例的[pause()](<../../../../harmonyos-references/Media Kit（媒体服务）/ArkTS API/@ohos.multimedia.media (媒体服务)/Interface (AVPlayer)/arkts-apis-media-avplayer.md#pause9-1>)方法，暂停播放。
   * seek()：在其中调用AVPlayer实例的[seek()](<../../../../harmonyos-references/Media Kit（媒体服务）/ArkTS API/@ohos.multimedia.media (媒体服务)/Interface (AVPlayer)/arkts-apis-media-avplayer.md#seek9>)，跳转播放。
...
3. 在AudioRendererController类中，定义如下方法：
   ...
   * start()：在其中调用AudioRenderer的[start()](<../../../../harmonyos-references/Audio Kit（音频服务）/ArkTS API/@ohos.multimedia.audio (音频管理)/Interface (AudioRenderer)/arkts-apis-audio-audiorenderer.md#start8>)方法，启动音频渲染器。
   * pause()：在其中调用AudioRenderer的[pause()](<../../../../harmonyos-references/Audio Kit（音频服务）/ArkTS API/@ohos.multimedia.audio (音频管理)/Interface (AudioRenderer)/arkts-apis-audio-audiorenderer.md#pause8>)方法，暂停音频渲染。
...
4. 在PlayerController类中，定义如下方法：
   * changeType()：传入[RawFileDescriptor](<../../../../harmonyos-references/Localization Kit（本地化开发服务）/ArkTS API/@ohos.resourceManager (资源管理)/js-apis-resource-manager.md#rawfiledescriptor9>)音频资源描述符，并通过audioType的值，判断使用AudioRenderer或AVPlayer播放音频。
...
5. 在页面中自定义changeType()事件，通过[getRawfd()](<../../../../harmonyos-references/Localization Kit（本地化开发服务）/ArkTS API/@ohos.resourceManager (资源管理)/js-apis-resource-manager.md#getrawfd9-1>)获取resources/rawfile目录下的音频资源，将获取到的数据传入PlayerController的changeType()方法，调用playerController的seek()方法传入当前Slider进度的值，用于跳转播放。
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/33/v3/jH67GA0gTrKHXFYckTwqtA/zh-cn_image_0000002522762335.gif?HW-CC-KV=V1&HW-CC-Date=20260611T090215Z&HW-CC-Expire=86400&HW-CC-Sign=904FF19F3F666B73487D0895D71A48F7B8EABB23499B37C0A9850E2C1B39CE9E "点击放大")
