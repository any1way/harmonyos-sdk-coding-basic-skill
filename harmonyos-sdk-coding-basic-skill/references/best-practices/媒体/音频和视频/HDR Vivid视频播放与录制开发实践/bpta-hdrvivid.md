...
scraped_at: 2026-06-11T17:02:06+08:00
doc_updated_at: 2026-03-19
content_hash: sha256:40e9547972356d78a95f5ffc5b62a298318a786df4b91feed115e5fa9aa5ac80
---
## 概述
...
[AVPlayer](<../../../../harmonyos-references/Media Kit（媒体服务）/ArkTS API/@ohos.multimedia.media (媒体服务)/Interface (AVPlayer)/arkts-apis-media-avplayer.md>)提供功能完善的一体化播放能力，应用只需提供流媒体来源，无需数据解析和解码，即可实现播放效果。

![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/7d/v3/ZmsuwzZ7SviT-O-m-TPCyw/zh-cn_image_0000002541928599.png?HW-CC-KV=V1&HW-CC-Date=20260611T090205Z&HW-CC-Expire=86400&HW-CC-Sign=FE96C2C21E5D4A929D5DA5536B63D69CAA14D64E6FB492D16BBA203A47AEFBA9 "点击放大")

### 开发步骤

具体开发步骤可参考最佳实践《[基于AVPlayer基础播控实践](../基于AVPlayer播放视频系列开发实践/基于AVPlayer基础播控实践/bpta-avplayer-basic-control.md)》。

## 基于AVCodec解码播放HDR Vivid视频

### 实现原理

AVCodec模块中[视频解码](<../../../../harmonyos-guides/媒体/AVCodec Kit（音视频编解码服务）/音视频编解码/视频解码/video-decoding.md>)的Native API接口，可以完成视频解码功能，即将媒体数据在系统侧解码成YUV文件并送显至应用上。

![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/c5/v3/5vkJSf_0QOuqQrUJWslg7Q/zh-cn_image_0000002510248622.png?HW-CC-KV=V1&HW-CC-Date=20260611T090205Z&HW-CC-Expire=86400&HW-CC-Sign=ED46D880147AA3EC289E0490221ABA7091CCFC109330899ED45BAE19725DB1FD "点击放大")

### 开发步骤

使用系统解码器AVCodec开发HDR Vivid视频播放功能，主要开发步骤为（详细开发步骤可参考[HDR Vivid视频播放](<../../../../harmonyos-guides/媒体/AVCodec Kit（音视频编解码服务）/音视频编解码/HDR Vivid能力/HDR Vivid视频播放/hdr-vivid-video-player.md>)）：

1. 调用[OH\_VideoDecoder\_CreateByMime()](<../../../../harmonyos-references/AVCodec Kit（音视频编解码服务）/C API/头文件/native_avcodec_videodecoder.h/capi-native-avcodec-videodecoder-h.md#oh_videodecoder_createbymime>)通过HEVC格式创建解码器实例对象。如果需要对HDR Vivid视频进行解码，需要配置MimeType为H265 (即OH\_AVCODEC\_MIMETYPE\_VIDEO\_HEVC)。
...
2. 调用[OH\_VideoDecoder\_RegisterCallback()](<../../../../harmonyos-references/AVCodec Kit（音视频编解码服务）/C API/头文件/native_avcodec_videodecoder.h/capi-native-avcodec-videodecoder-h.md#oh_videodecoder_registercallback>)设置回调函数。
...
3. 调用[OH\_VideoDecoder\_Configure()](<../../../../harmonyos-references/AVCodec Kit（音视频编解码服务）/C API/头文件/native_avcodec_videodecoder.h/capi-native-avcodec-videodecoder-h.md#oh_videodecoder_configure>)配置解码器。
...
4. 从[XComponent](../../../../harmonyos-references/ArkUI（方舟UI框架）/ArkTS组件/渲染绘制/XComponent/ts-basic-components-xcomponent.md)组件获取window参数，设置Surface，并调用[OH\_VideoDecoder\_Prepare()](<../../../../harmonyos-references/AVCodec Kit（音视频编解码服务）/C API/头文件/native_avcodec_videodecoder.h/capi-native-avcodec-videodecoder-h.md#oh_videodecoder_prepare>)使解码器就绪。
...
5. 调用[OH\_VideoDecoder\_Start()](<../../../../harmonyos-references/AVCodec Kit（音视频编解码服务）/C API/头文件/native_avcodec_videodecoder.h/capi-native-avcodec-videodecoder-h.md#oh_videodecoder_start>)启动解码器。
...
6. 调用[OH\_VideoDecoder\_PushInputBuffer()](<../../../../harmonyos-references/AVCodec Kit（音视频编解码服务）/C API/头文件/native_avcodec_videodecoder.h/capi-native-avcodec-videodecoder-h.md#oh_videodecoder_pushinputbuffer>)写入解码流。
...
7. 调用[OH\_VideoDecoder\_RenderOutputBuffer()](<../../../../harmonyos-references/AVCodec Kit（音视频编解码服务）/C API/头文件/native_avcodec_videodecoder.h/capi-native-avcodec-videodecoder-h.md#oh_videodecoder_renderoutputbuffer>)渲染并释放解码帧。
...
8. 最后调用[OH\_VideoDecoder\_Destroy()](<../../../../harmonyos-references/AVCodec Kit（音视频编解码服务）/C API/头文件/native_avcodec_videodecoder.h/capi-native-avcodec-videodecoder-h.md#oh_videodecoder_destroy>)销毁解码器实例，释放资源。
...
应用通过调用[AVRecorder](<../../../../harmonyos-guides/媒体/Media Kit（媒体服务）/Media Kit简介/media-kit-intro.md#avrecorder>)实现视频录制时，先通过Camera接口调用相机服务，通过视频HDI捕获图像数据送显至应用，同时送至AVRecorder的录制服务，录制服务将图像数据编码后封装至文件中，实现视频录制功能。流程图如下：

![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/91/v3/Wxwh0t8eQrSdaTGWL3qXfQ/zh-cn_image_0000002510408618.png?HW-CC-KV=V1&HW-CC-Date=20260611T090205Z&HW-CC-Expire=86400&HW-CC-Sign=2CA2C6EF85733B2BD8AF070FCDA853AC2EE232F028E20F1194E1050D59521558 "点击放大")

说明

AVRecorder不支持设置AVMetadata音视频元数据的HDR类型。

使用[Interface (CameraManager)](<../../../../harmonyos-references/Camera Kit（相机服务）/ArkTS API/@ohos.multimedia.camera (相机管理)/Interface (CameraManager)/arkts-apis-camera-cameramanager.md>)+[Interface (AVRecorder)](<../../../../harmonyos-references/Media Kit（媒体服务）/ArkTS API/@ohos.multimedia.media (媒体服务)/Interface (AVRecorder)/arkts-apis-media-avrecorder.md>)录制HDR Vivid视频，与录制普通视频的区别主要在于：
...
针对原理中描述的四点不同，开发视频录制功能时，可参考以下步骤（详细开发步骤可参考[HDR Vivid相机录像(ArkTS)](<../../../../harmonyos-guides/媒体/Camera Kit（相机服务）/开发相机应用基础能力(ArkTS)/HDR Vivid相机录像(ArkTS)/camera-hdr-recording.md>)）：

1. 调用[media.createAVRecorder()](<../../../../harmonyos-references/Media Kit（媒体服务）/ArkTS API/@ohos.multimedia.media (媒体服务)/Functions/arkts-apis-media-f.md#mediacreateavrecorder9>)创建AVRecorder实例。
...
2. 配置预览流与录像输出流的分辨率为16:9，[AVRecorderProfile](<../../../../harmonyos-references/Media Kit（媒体服务）/ArkTS API/@ohos.multimedia.media (媒体服务)/Interfaces (其他)/arkts-apis-media-i.md#avrecorderprofile9>)参数中的变量isHdr为True，videoCodec为VIDEO\_HEVC格式，以录制HDR Vivid视频。
...
3. 调用[createVideoOutput()](<../../../../harmonyos-references/Camera Kit（相机服务）/ArkTS API/@ohos.multimedia.camera (相机管理)/Interface (CameraManager)/arkts-apis-camera-cameramanager.md#createvideooutput>)创建VideoOutput实例，选择yuv 10bit profile。
...
4. 创建并配置普通录像模式（[Interface (VideoSession)](<../../../../harmonyos-references/Camera Kit（相机服务）/ArkTS API/@ohos.multimedia.camera (相机管理)/Interface (VideoSession)/arkts-apis-camera-videosession.md>)）的相机会话。
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/8b/v3/tgzogI4eQRW6PwuPyd4m-w/zh-cn_image_0000002541848601.png?HW-CC-KV=V1&HW-CC-Date=20260611T090205Z&HW-CC-Expire=86400&HW-CC-Sign=1282FB86FB6112462C5471046AD2A7C629F54DB043B06C79892785D242CAF847 "点击放大")

使用[Interface (CameraManager)](<../../../../harmonyos-references/Camera Kit（相机服务）/ArkTS API/@ohos.multimedia.camera (相机管理)/Interface (CameraManager)/arkts-apis-camera-cameramanager.md>)+AVCodec录制HDR Vivid视频，与录制普通视频的区别主要在于：
...
针对以上四点不同，使用Camera+AVCodec开发HDR Vivid视频录制功能时，可参考以下步骤（详细开发步骤可参考[HDR Vivid视频录制](<../../../../harmonyos-guides/媒体/AVCodec Kit（音视频编解码服务）/音视频编解码/HDR Vivid能力/HDR Vivid视频录制/hdr-vivid-video-recorder.md>)）：

1. 调用[OH\_VideoEncoder\_CreateByMime()](<../../../../harmonyos-references/AVCodec Kit（音视频编解码服务）/C API/头文件/native_avcodec_videoencoder.h/capi-native-avcodec-videoencoder-h.md#oh_videoencoder_createbymime>)根据MIME类型创建HEVC格式视频编码器并初始化。
...
2. 配置HDR Vivid相关参数，包括可选配置视频帧宽度、视频帧高度、视频颜色格式，以及必须配置为HEVC\_PROFILE\_MAIN\_10的[OH\_HEVCProfile()](<../../../../harmonyos-references/AVCodec Kit（音视频编解码服务）/C API/头文件/native_avcodec_base.h/capi-native-avcodec-base-h.md#oh_hevcprofile>)，表示HEVC编码档次为10bit主档次。
...
3. 使用[OH\_VideoEncoder\_Configure()](<../../../../harmonyos-references/AVCodec Kit（音视频编解码服务）/C API/头文件/native_avcodec_videoencoder.h/capi-native-avcodec-videoencoder-h.md#oh_videoencoder_configure>)配置编码器。
...
4. ArkTS侧调用[createVideoOutput()](<../../../../harmonyos-references/Camera Kit（相机服务）/ArkTS API/@ohos.multimedia.camera (相机管理)/Interface (CameraManager)/arkts-apis-camera-cameramanager.md#createvideooutput>)创建VideoOutput实例，选择yuv 10bit profile。
...
7. 调用[OH\_AVMuxer\_Create()](<../../../../harmonyos-references/AVCodec Kit（音视频编解码服务）/C API/头文件/native_avmuxer.h/capi-native-avmuxer-h.md#oh_avmuxer_create>)创建AVMuxer封装器实例对象，设置封装格式及封装路径，配置HDR Vivid相关参数。
...
8. 调用[OH\_VideoEncoder\_Start()](<../../../../harmonyos-references/AVCodec Kit（音视频编解码服务）/C API/头文件/native_avcodec_videoencoder.h/capi-native-avcodec-videoencoder-h.md#oh_videoencoder_start>)启动编码器。
...
方案1：可以参考文档使用[hdrBrightness()](../../../../harmonyos-references/ArkUI（方舟UI框架）/ArkTS组件/渲染绘制/XComponent/ts-basic-components-xcomponent.md#hdrbrightness20)实现用于调整组件播放HDR视频的亮度。

方案2：参考《[基于AVPlayer播放长视频实践](../基于AVPlayer播放视频系列开发实践/基于AVPlayer播放长视频实践/bpta-avplayer-long-video.md)》中[亮度控制](../基于AVPlayer播放视频系列开发实践/基于AVPlayer播放长视频实践/bpta-avplayer-long-video.md#section512331617222)章节。
