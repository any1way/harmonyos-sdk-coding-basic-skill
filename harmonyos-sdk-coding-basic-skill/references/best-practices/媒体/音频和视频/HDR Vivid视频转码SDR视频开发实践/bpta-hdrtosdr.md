...
scraped_at: 2026-06-11T17:02:09+08:00
doc_updated_at: 2026-03-19
content_hash: sha256:06f6ac2bd65e1492c0a4d86014aaf14bc4bad852f4929a829a01686800a33ad6
---
## 概述
...
本文主要面向所有开发者。在开始之前，建议已了解视频解码的[Surface模式](<../../../../harmonyos-guides/媒体/AVCodec Kit（音视频编解码服务）/音视频编解码/视频解码/video-decoding.md#surface模式>)、[视频色彩空间转换](<../../../../harmonyos-guides/媒体/Media Kit（媒体服务）/媒体开发指导(CC++)/视频处理/视频色彩空间转换/video-csc.md>)。
...
使用[AVTranscoder](<../../../../harmonyos-guides/媒体/Media Kit（媒体服务）/Media Kit简介/media-kit-intro.md#avtranscoder>)可以实现视频转码功能，从API 20开始支持视频转码的C/C++开发，转码功能可在手机、平板、PC/2in1设备上作为系统提供的基础能力使用。可以通过调用[canIUse()](<../../../../harmonyos-references/ArkTS API/SysCap (系统能力)/js-apis-syscap.md#caniuse>)接口来判断当前设备是否支持AVTranscoder，当canIUse("SystemCapability.Multimedia.Media.AVTranscoder")的返回值为true时，表示可以使用转码能力。转码步骤如下：初始化与准备阶段，调用[OH\_AVTranscoder\_Create()](<../../../../harmonyos-references/Media Kit（媒体服务）/C API/头文件/avtranscoder.h/capi-avtranscoder-h.md#oh_avtranscoder_create>)创建`OH\_AVTranscoder` 对象；启动与运行阶段，调用OH\_AVTranscoder\_Start()启动转码任务，此时可调用[OH\_AVTranscoder\_Pause()](<../../../../harmonyos-references/Media Kit（媒体服务）/C API/头文件/avtranscoder.h/capi-avtranscoder-h.md#oh_avtranscoder_pause>)暂停任务。在暂停状态下，可调用[OH\_AVTranscoder\_Resume()](<../../../../harmonyos-references/Media Kit（媒体服务）/C API/头文件/avtranscoder.h/capi-avtranscoder-h.md#oh_avtranscoder_resume>)恢复任务；任务进行时，若想取消该任务，可调用[OH\_AVTranscoder\_Cancel()](<../../../../harmonyos-references/Media Kit（媒体服务）/C API/头文件/avtranscoder.h/capi-avtranscoder-h.md#oh_avtranscoder_cancel>)终止转码任务。

![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/a8/v3/HB7XoRKvQwm7ZH6kLruVBg/zh-cn_image_0000002541928605.png?HW-CC-KV=V1&HW-CC-Date=20260611T090208Z&HW-CC-Expire=86400&HW-CC-Sign=A9DBF7BD00211345A83234146EAB0CA6A5C0A64AAE082BDDD4C17BFB3814E13E "点击放大")

### 开发步骤

具体开发步骤，可参考[使用AVTranscoder实现视频转码(C/C++)](<../../../../harmonyos-guides/媒体/Media Kit（媒体服务）/媒体开发指导(CC++)/视频转码/使用AVTranscoder实现视频转码(CC++)/using-ndk-avtranscoder-for-transcodering.md>)。

关键点：调用[OH\_AVTranscoderConfig\_SetDstVideoType()](<../../../../harmonyos-references/Media Kit（媒体服务）/C API/头文件/avtranscoder.h/capi-avtranscoder-h.md#oh_avtranscoderconfig_setdstvideotype>)设置输出视频的编码格式为“video/avc”。
...
4. 在[OH\_AVTranscoder\_SetStateCallback()](<../../../../harmonyos-references/Media Kit（媒体服务）/C API/头文件/avtranscoder.h/capi-avtranscoder-h.md#oh_avtranscoder_setstatecallback>)中设置AvTranscoder状态改变监听。
...
使用AVCodec原生转码能力，主要的开发步骤为（详细开发步骤可参考[视频解码支持HDRVivid2SDR](<../../../../harmonyos-guides/媒体/AVCodec Kit（音视频编解码服务）/音视频编解码/HDR Vivid能力/视频解码支持HDRVivid2SDR/hdrvivid2sdr.md>)）：
...
3. 调用[OH\_VideoDecoder\_Configure()](<../../../../harmonyos-references/AVCodec Kit（音视频编解码服务）/C API/头文件/native_avcodec_videodecoder.h/capi-native-avcodec-videodecoder-h.md#oh_videodecoder_configure>)配置解码器。必选配置项有：视频帧宽度、视频帧高度、视频像素格式、指定输出为SDR。
...
4. 目前仅Surface模式支持该能力，后续步骤具体可参考：视频解码的[Surface模式](<../../../../harmonyos-guides/媒体/AVCodec Kit（音视频编解码服务）/音视频编解码/视频解码/video-decoding.md#surface模式>)。

## 基于VideoProcessing模块实现HDR Vivid视频到SDR视频转码

### 实现原理

开发者可以调用[VideoProcessing](<../../../../harmonyos-references/Media Kit（媒体服务）/C API/模块/VideoProcessing/capi-videoprocessing.md>)模块提供的C API接口，实现HDR2SDR的色彩空间转换。支持的转码范围如下：
...
HarmonyOS提供了Native侧的[VideoProcessing](<../../../../harmonyos-references/Media Kit（媒体服务）/C API/模块/VideoProcessing/capi-videoprocessing.md>)模块，可以将HDR Vivid视频转码成SDR视频，主要的开发步骤为（详细开发步骤可参考[视频色彩空间转换](<../../../../harmonyos-guides/媒体/Media Kit（媒体服务）/媒体开发指导(CC++)/视频处理/视频色彩空间转换/video-csc.md>)）：

1. 调用[OH\_VideoProcessing\_Create()](<../../../../harmonyos-references/Media Kit（媒体服务）/C API/头文件/video_processing.h/capi-video-processing-h.md#oh_videoprocessing_create>)创建视频处理实例。
   ...
   调用[OH\_VideoProcessing\_GetSurface()](<../../../../harmonyos-references/Media Kit（媒体服务）/C API/头文件/video_processing.h/capi-video-processing-h.md#oh_videoprocessing_getsurface>)在视频处理启动之前创建输入surface。
...
2. 调用[OH\_VideoProcessing\_SetSurface()](<../../../../harmonyos-references/Media Kit（媒体服务）/C API/头文件/video_processing.h/capi-video-processing-h.md#oh_videoprocessing_setsurface>)设置surface。
...
3. 调用[OH\_VideoProcessing\_RegisterCallback()](<../../../../harmonyos-references/Media Kit（媒体服务）/C API/头文件/video_processing.h/capi-video-processing-h.md#oh_videoprocessing_registercallback>)等函数创建并绑定回调函数。
...
4. 调用[OH\_VideoProcessing\_Start()](<../../../../harmonyos-references/Media Kit（媒体服务）/C API/头文件/video_processing.h/capi-video-processing-h.md#oh_videoprocessing_start>)启动色彩空间转换处理。
...
5. 调用[OH\_VideoProcessing\_Stop()](<../../../../harmonyos-references/Media Kit（媒体服务）/C API/头文件/video_processing.h/capi-video-processing-h.md#oh_videoprocessing_stop>)停止色彩空间转换处理。
...
6. 调用[OH\_VideoProcessing\_Destroy()](<../../../../harmonyos-references/Media Kit（媒体服务）/C API/头文件/video_processing.h/capi-video-processing-h.md#oh_videoprocessing_destroy>)及[OH\_VideoProcessing\_DeinitializeEnvironment()](<../../../../harmonyos-references/Media Kit（媒体服务）/C API/头文件/video_processing.h/capi-video-processing-h.md#oh_videoprocessing_deinitializeenvironment>)释放处理实例和资源。
...
开发者可以调用[VideoProcessing](<../../../../harmonyos-references/Media Kit（媒体服务）/C API/模块/VideoProcessing/capi-videoprocessing.md>)模块提供的C API接口，实现HDR2HDR的色彩空间转换。支持的转码范围如下：
...
具体开发步骤，可参考[视频色彩空间转换](<../../../../harmonyos-guides/媒体/Media Kit（媒体服务）/媒体开发指导(CC++)/视频处理/视频色彩空间转换/video-csc.md>)。
...
5. 设置Surface。
   ...
   [VideoProcessing.cpp](https://gitcode.com/harmonyos_samples/hdr2sdr/blob/master/entry/src/main/cpp/capbilities/VideoProcessing.cpp#L48-L59)
6. 调用[OH\_VideoProcessing\_Start()](<../../../../harmonyos-references/Media Kit（媒体服务）/C API/头文件/video_processing.h/capi-video-processing-h.md#oh_videoprocessing_start>)启动色彩空间转换处理。
   ...
   [VideoProcessing.cpp](https://gitcode.com/harmonyos_samples/hdr2sdr/blob/master/entry/src/main/cpp/capbilities/VideoProcessing.cpp#L92-L95)
7. 调用[OH\_VideoProcessing\_Stop()](<../../../../harmonyos-references/Media Kit（媒体服务）/C API/头文件/video_processing.h/capi-video-processing-h.md#oh_videoprocessing_stop>)停止色彩空间转换处理。
   ...
   调用[OH\_VideoProcessing\_Destroy()](<../../../../harmonyos-references/Media Kit（媒体服务）/C API/头文件/video_processing.h/capi-video-processing-h.md#oh_videoprocessing_destroy>)销毁视频处理实例。
