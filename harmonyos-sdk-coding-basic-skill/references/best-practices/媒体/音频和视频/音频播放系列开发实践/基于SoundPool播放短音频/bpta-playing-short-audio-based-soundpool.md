...
scraped_at: 2026-06-11T17:01:15+08:00
doc_updated_at: 2026-03-12
content_hash: sha256:00496555ef77c398e0148e07ad7cf61c857674f6f60eb79292c695676619349d
---
## 概述
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/1f/v3/JqHYEBpiQ2Cdrguv80HrPg/zh-cn_image_0000002555337527.png?HW-CC-KV=V1&HW-CC-Date=20260611T090113Z&HW-CC-Expire=86400&HW-CC-Sign=E46AF8B0667ADB8B26EE91CEFD447FB82D25C3280D16628A4ED866C36E5812F1 "点击放大")
...
使用SoundPool播放短音频，且[StreamUsage](<../../../../../harmonyos-references/Audio Kit（音频服务）/ArkTS API/@ohos.multimedia.audio (音频管理)/Enums/arkts-apis-audio-e.md#streamusage>)指定为Music、Movie、AudioBook等类型时，其申请焦点时默认为并发模式，不会影响其他音频，若开发过程中涉及焦点管理的问题，请参考[音频焦点管理解决方案](../../音频焦点管理解决方案/bpta-audio-focus-management.md#section8811136185118)。
...
2. 设置[on('loadComplete')](<../../../../../harmonyos-references/Media Kit（媒体服务）/ArkTS API/multimedia/SoundPool (音频池)/js-apis-inner-multimedia-soundpool.md#onloadcomplete>)回调，用于监听“资源加载完成”的状态。开发者应在监听到“资源加载完成”的状态后，方可执行后续的播放操作，否则系统会抛出异常。
...
3. 设置[on('playFinished')](<../../../../../harmonyos-references/Media Kit（媒体服务）/ArkTS API/multimedia/SoundPool (音频池)/js-apis-inner-multimedia-soundpool.md#onplayfinished>)或者[on('playFinishedWithStreamId')](<../../../../../harmonyos-references/Media Kit（媒体服务）/ArkTS API/multimedia/SoundPool (音频池)/js-apis-inner-multimedia-soundpool.md#onplayfinishedwithstreamid18>)回调，用于监听“播放完成”，以便于在播放后处理对应的业务。
...
4. 设置[on('error')](<../../../../../harmonyos-references/Media Kit（媒体服务）/ArkTS API/multimedia/SoundPool (音频池)/js-apis-inner-multimedia-soundpool.md#onerror>)回调，进行错误类型的监听，以便遇到播放问题后，进行快速定位。
...
5. 调用[load()](<../../../../../harmonyos-references/Media Kit（媒体服务）/ArkTS API/multimedia/SoundPool (音频池)/js-apis-inner-multimedia-soundpool.md#load>)方法，加载音频资源。
...
6. 调用[play()](<../../../../../harmonyos-references/Media Kit（媒体服务）/ArkTS API/multimedia/SoundPool (音频池)/js-apis-inner-multimedia-soundpool.md#play>)方法，播放音频资源。
...
7. 开发者可以通过配置播放参数PlayParameters实现不同的播放效果，也可以通过单独调用[setLoop](<../../../../../harmonyos-references/Media Kit（媒体服务）/ArkTS API/multimedia/SoundPool (音频池)/js-apis-inner-multimedia-soundpool.md#setloop>)、[setPriority](<../../../../../harmonyos-references/Media Kit（媒体服务）/ArkTS API/multimedia/SoundPool (音频池)/js-apis-inner-multimedia-soundpool.md#setpriority>)、[setvolume](<../../../../../harmonyos-references/Media Kit（媒体服务）/ArkTS API/multimedia/SoundPool (音频池)/js-apis-inner-multimedia-soundpool.md#setvolume>)、[setRate](<../../../../../harmonyos-references/Media Kit（媒体服务）/ArkTS API/multimedia/SoundPool (音频池)/js-apis-inner-multimedia-soundpool.md#setrate-1>)等函数来实现不同的播放效果。下面以设置短音频的循环模式为例，其他设置方法的调用方式相同。
