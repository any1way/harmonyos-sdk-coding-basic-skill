...
scraped_at: 2026-06-11T16:55:22+08:00
doc_updated_at: 2026-05-18
content_hash: sha256:4af0d42d537f33830cc3aa5e8083f22b35398bebec5ec427f09a073ddc491aa8
---
## 概述
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/24/v3/ksKgk9ocQguBN0K-0h4Z8Q/zh-cn_image_0000002387038021.png?HW-CC-KV=V1&HW-CC-Date=20260611T085511Z&HW-CC-Expire=86400&HW-CC-Sign=FF04CD9811F0662764303E80F7110D18EFCB27E475F92B8A85CC574FD886EFAA "点击放大")

为了实现上图中分组参差不齐的效果，首先需要创建多个SectionOptions，并重写其中的[onGetItemMainSizeByIndex()](../../../harmonyos-references/ArkUI（方舟UI框架）/ArkTS组件/滚动与滑动/WaterFlow/ts-container-waterflow.md#getitemmainsizebyindex12)方法的返回值。完整代码如下：
...
WaterFlow中FlowItem的高度通过SectionOptions的[onGetItemMainSizeByIndex()](../../../harmonyos-references/ArkUI（方舟UI框架）/ArkTS组件/滚动与滑动/WaterFlow/ts-container-waterflow.md#getitemmainsizebyindex12)方法返回值来控制，其中参数index表示FlowItem的索引位置。
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/a4/v3/l93TnfrDSGuON9WBYsZJSQ/zh-cn_image_0000002386957729.gif?HW-CC-KV=V1&HW-CC-Date=20260611T085511Z&HW-CC-Expire=86400&HW-CC-Sign=DA740CADC8D30EC0E359299B04A93F5727CDE233C0D4A3477434DD2CF8D135CC "点击放大")
...
在应用开发中，开发者需要在列表模式与瀑布流模式间进行切换，或适应屏幕宽度的变化。为解决此类需求，通常通过动态调整瀑布流的列数来实现。建议采用瀑布流的移动窗口布局模式，以实现更快速的列数转换。参考：[动态切换列数](<../../../harmonyos-guides/应用框架/ArkUI（方舟UI框架）/UI开发 (ArkTS声明式开发范式)/列表与网格/创建瀑布流（WaterFlow）/arkts-layout-development-create-waterflow.md#动态切换列数>)。

## 瀑布流滑动

### 滑动性能优化

瀑布流上下滑动时，因其具有无限加载数据的特性，能够展示大量数据。不同大小的子元素将会带来测量和绘制的性能消耗过大。例如，在实际开发中，当瀑布流中的数据量过大时，可能会遇到渲染速度慢、滑动丢帧、内存占用高等问题，这些问题直接影响用户操作体验。为了解决这些开发中可能遇到的性能问题，建议参考[瀑布流加载丢帧优化](../../性能/性能场景优化案例/界面渲染性能优化/瀑布流加载丢帧优化/bpta-waterflow-performance-optimization.md#section088318458314)。

### 停止滑动时自动播放

在某些开发场景中，开发者可能想要在瀑布流停止滑动时播放其中的视频，效果图如下：

![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/2e/v3/A-x8Rq5ATA61zhpxY_NB2Q/zh-cn_image_0000002353157698.gif?HW-CC-KV=V1&HW-CC-Date=20260611T085511Z&HW-CC-Expire=86400&HW-CC-Sign=A67DF097B2E5AE5EEFA66E92100640CBAF5A87F388171A8E7B8D890A680FEDFB "点击放大")

若要实现上述效果，可利用组件的[onVisibleAreaChange()](../../../harmonyos-references/ArkUI（方舟UI框架）/ArkTS组件/通用事件/组件变化事件/组件可见区域变化事件/ts-universal-component-visible-area-change-event.md#onvisibleareachange)方法监听组件显示状态，以控制视频播放或暂停。完整代码如下：
...
示例代码构建的数据屏幕中仅有一个视频。若屏幕中包含多个视频，滑动停止时需要播放首个完全显示的视频，则需借助WaterFlow的[onScrollIndex()](../../../harmonyos-references/ArkUI（方舟UI框架）/ArkTS组件/滚动与滑动/WaterFlow/ts-container-waterflow.md#onscrollindex11)方法，实现相关逻辑。
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/8d/v3/BECPMcooRmmof5Jy57uRdQ/zh-cn_image_0000002353317494.gif?HW-CC-KV=V1&HW-CC-Date=20260611T085511Z&HW-CC-Expire=86400&HW-CC-Sign=9A929B7227038BD034E635A9263F1488512F81B3081BF3E5BB91EFD8202FF326 "点击放大")

为了实现上述效果，开发者可通过Refresh组件实现瀑布流下拉刷新。通过Refresh组件进行页面下拉操作，并绑定显示刷新Loading动效的容器组件，以实现下拉刷新效果。随后，在[onRefreshing()](../../../harmonyos-references/ArkUI（方舟UI框架）/ArkTS组件/滚动与滑动/Refresh/ts-container-refresh.md#onrefreshing)事件中更新数据。完整代码如下：
...
WaterFlow实现加载更多功能，通常采用onReachEnd回调（[onReachEnd](../../../harmonyos-references/ArkUI（方舟UI框架）/ArkTS组件/滚动与滑动/WaterFlow/ts-container-waterflow.md#onreachend)）或onScrollIndex回调（[onScrollIndex](../../../harmonyos-references/ArkUI（方舟UI框架）/ArkTS组件/滚动与滑动/WaterFlow/ts-container-waterflow.md#onscrollindex11)）来实现。

1. 在onReachEnd加载数据时，应用会在数据源中的所有数据渲染完成后加载新数据，这会导致明显的加载动画出现，用户需等待新数据加载完毕后才能继续浏览瀑布流。
2. 相比onReachEnd方式，onScrollIndex回调可以在用户滑动到接近底部时预加载数据，实现无缝浏览体验。例如，当前可视区最后一个组件的索引加上20等于数据源总量时，才开始加载数据，这能避免每次索引变化时均加载数据。建议在[onScrollIndex](../../../harmonyos-references/ArkUI（方舟UI框架）/ArkTS组件/滚动与滑动/WaterFlow/ts-container-waterflow.md#onscrollindex11)回调中执行此操作，使瀑布流在未触底前即开始加载数据，进而提高用户体验，确保用户几乎不察觉数据加载过程。
...
在使用[WaterFlowSections](../../../harmonyos-references/ArkUI（方舟UI框架）/ArkTS组件/滚动与滑动/WaterFlow/ts-container-waterflow.md#waterflowsections12)对WaterFlow中的子元素进行分组的场景下，若需删除WaterFlow中的数据，不仅需从数据源中删除数据，还需更新对应section的itemsCount数量，并执行sections.[update()](../../../harmonyos-references/ArkUI（方舟UI框架）/ArkTS组件/滚动与滑动/WaterFlow/ts-container-waterflow.md#update12)操作。完整代码如下：
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/2/v3/7-79iOVISOG7PREet3pgdQ/zh-cn_image_0000002387038025.gif?HW-CC-KV=V1&HW-CC-Date=20260611T085511Z&HW-CC-Expire=86400&HW-CC-Sign=06ACC6212B1D2EE7A6D59B1C059F823B62883055B84EA6FEDBD7B9895B24E109 "点击放大")

为了实现上图的效果，可通过配置FlowItem的[transition()](<../../../harmonyos-references/ArkUI（方舟UI框架）/ArkTS组件/动画/组件内转场 (transition)/ts-transition-animation-component.md>)属性并添加转场参数，使组件在插入和删除时显示过渡动画。同时，在删除时添加[animateTo](<../../../harmonyos-references/ArkUI（方舟UI框架）/ArkTS组件/动画/显式动画 (animateTo)/ts-explicit-animation.md>)动画效果即可。完整代码如下：
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/2d/v3/AMRZ1W_ySDuprVqs2a8efQ/zh-cn_image_0000002386957733.gif?HW-CC-KV=V1&HW-CC-Date=20260611T085511Z&HW-CC-Expire=86400&HW-CC-Sign=A0E26A5FF531E44BC33DF2A3ECD4902CD66DF8D443A8D8A3C66E65558E9E0DBC "点击放大")

该效果可通过WaterFlow组件的[fadingEdge](../../../harmonyos-references/ArkUI（方舟UI框架）/ArkTS组件/滚动与滑动/滚动组件通用接口/ts-container-scrollable-common.md#fadingedge14)实现，并通过fadingEdgeLength参数设置边缘渐隐长度。具体代码参考：[设置边缘渐隐效果](../../../harmonyos-references/ArkUI（方舟UI框架）/ArkTS组件/滚动与滑动/WaterFlow/ts-container-waterflow.md#示例5设置边缘渐隐效果)。
...
2. 实现下拉刷新/上拉加载更多
   ...
   通过Refresh组件和[onScrollIndex()](../../../harmonyos-references/ArkUI（方舟UI框架）/ArkTS组件/滚动与滑动/WaterFlow/ts-container-waterflow.md#onscrollindex11)方法实现下拉刷新与上拉加载更多功能。具体实现参考[瀑布流数据更新](bpta-waterflow-operations.md#section193081340162215)。
3. 实现长按删除FlowItem
...
4. 实现瀑布流吸顶效果
   ...
   通过WaterFlow的[onWillScroll()](../../../harmonyos-references/ArkUI（方舟UI框架）/ArkTS组件/滚动与滑动/滚动组件通用接口/ts-container-scrollable-common.md#onwillscroll12)方法和position属性设置最大滑动偏移量，以实现滑动吸顶效果。具体参考[瀑布流排版](bpta-waterflow-operations.md#section154611537122218)。
5. 实现FlowItem自动播放效果

   通过[onVisibleAreaChange()](../../../harmonyos-references/ArkUI（方舟UI框架）/ArkTS组件/通用事件/组件变化事件/组件可见区域变化事件/ts-universal-component-visible-area-change-event.md#onvisibleareachange)方法检测组件显示状态，控制FlowItem中视频的播放或暂停。具体参考[瀑布流滑动](bpta-waterflow-operations.md#section119347413579)。

### **实现效果**

最终效果如下图：

![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/77/v3/zc95bzzzRkmeNJJ2KfBo4Q/zh-cn_image_0000002353157702.gif?HW-CC-KV=V1&HW-CC-Date=20260611T085511Z&HW-CC-Expire=86400&HW-CC-Sign=B1F087B951768176F0FEDCCDCB531ECE1B7989A6A48A017F2941050E81B37FB3 "点击放大")

## 常见问题

### **如何将多个FlowItem强制显示到左上角位置，如下图所示**：

![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/cd/v3/9NI31CJSTtKAwJRgtL6rvw/zh-cn_image_0000002353317498.png?HW-CC-KV=V1&HW-CC-Date=20260611T085511Z&HW-CC-Expire=86400&HW-CC-Sign=4B13CD6361730D5E739AC8075095E32DCCB4750D4696215EF0825E1A30BB101F "点击放大")
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/35/v3/eVXcVj9gRK-8zFTfN8dfxA/zh-cn_image_0000002387038029.png?HW-CC-KV=V1&HW-CC-Date=20260611T085511Z&HW-CC-Expire=86400&HW-CC-Sign=E210243BA5F737DA7414D0B10DE86758E6AB2D0105FCFD7D30389917E7849E86 "点击放大")
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/4c/v3/wnIDU1niRli-aSdGGUcacg/zh-cn_image_0000002386957737.gif?HW-CC-KV=V1&HW-CC-Date=20260611T085511Z&HW-CC-Expire=86400&HW-CC-Sign=8DFE754D652E84D246A49E6011DE7C0FB9050819603867341C91148B09592F12 "点击放大")
