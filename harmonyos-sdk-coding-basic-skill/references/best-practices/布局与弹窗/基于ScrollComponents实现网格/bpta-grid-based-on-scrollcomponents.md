...
scraped_at: 2026-06-11T16:55:23+08:00
doc_updated_at: 2026-05-18
content_hash: sha256:5e8b2be786554004d6dca172bf53dd9900cb3103f26390a3805d1e1545bf3f70
---
## 概述
...
* 默认支持懒加载，开发者不用使用[LazyForEach](../../../harmonyos-references/ArkUI（方舟UI框架）/ArkTS组件/状态管理与渲染控制/LazyForEach/ts-rendering-control-lazyforeach.md)和定义IDataSource数据源，减少一定的代码量
...
* 不支持[设置子组件所占行列数](<../../../harmonyos-guides/应用框架/ArkUI（方舟UI框架）/UI开发 (ArkTS声明式开发范式)/列表与网格/创建网格 (GridGridItem)/arkts-layout-development-create-grid.md#设置子组件所占行列数>)
* 不支持捏合手势识别
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/ee/v3/ZZhrlAMTQ2SCosncQQlU2g/zh-cn_image_0000002390335989.jpg?HW-CC-KV=V1&HW-CC-Date=20260611T085521Z&HW-CC-Expire=86400&HW-CC-Sign=328D48366CC492A8E978154CD9930F39653F67ACD5C87C3152C47419C4B48CC1 "点击放大")
...
4. 注册子节点模板
   ...
   2. 单元格内子组件可拆分组合
      ...
      **图2** 可拆分组件复用创建流程图  
      ![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/1a/v3/yPP-aY6aSEyARzFYaEEa9g/zh-cn_image_0000002356815874.jpg?HW-CC-KV=V1&HW-CC-Date=20260611T085521Z&HW-CC-Expire=86400&HW-CC-Sign=4EAF507FDD9CD6196F0E8487D4FAAADEEBE1AC1374E3C49BF77B9713FD2BD7E4 "点击放大")

      开发者可参考图3日志打印"generateItem reuse "表示复用，检验是否复用成功。

      **图3** 日志效果图  
      ![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/ad/v3/T1zpanQ3Qci1iLGhk62HKw/zh-cn_image_0000002390415913.png?HW-CC-KV=V1&HW-CC-Date=20260611T085521Z&HW-CC-Expire=86400&HW-CC-Sign=8220F31E00C34979AFC47FC934E3B3232319981E632AE32AD6F17A9255FEAA60 "点击放大")
      ...
      [CardGridComponent.ets](https://gitcode.com/harmonyos_samples/GridScrollComponent/blob/master/entry/src/main/ets/view/CardGridComponent.ets#L25-L213)
   3. 单元格结构类型不同
...
3. 跨页面共享单例中的RecycledPool
   ...
   [PhotoGridComponent.ets](https://gitcode.com/harmonyos_samples/GridScrollComponent/blob/master/entry/src/main/ets/view/PhotoGridComponent.ets#L25-L139)
...
[CardGridComponent.ets](https://gitcode.com/harmonyos_samples/GridScrollComponent/blob/master/entry/src/main/ets/view/CardGridComponent.ets#L36-L123)
...
**图4** @Reusable测试结果  
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/9a/v3/3hSrsdquTV2E8ERWGjhW_w/zh-cn_image_0000002356655986.png?HW-CC-KV=V1&HW-CC-Date=20260611T085521Z&HW-CC-Expire=86400&HW-CC-Sign=D1B240B4C5369C96B9D81FCCBCFBCBF262C39D8BCDD82043AA52CE5CB15E86D9 "点击放大")

ScrollComponents：网络请求期间主线程空闲较少，请求结束后首屏组件绘帧耗时较短

**图5** ScrollComponents测试结果  
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/ca/v3/Z-vNaXKCQxKPYg6MrLl6fA/zh-cn_image_0000002390336069.png?HW-CC-KV=V1&HW-CC-Date=20260611T085521Z&HW-CC-Expire=86400&HW-CC-Sign=3D8DE9994DF6A45785E9D50E4DAA4D070F6B1DF65ADF4DE094325F739134033B "点击放大")
...
下拉刷新是提升用户体验的关键功能，既要保证数据无缝加载，又要维持流畅的交互效果。下拉刷新场景下使用懒加载刷新避免媒体资源加载造成UI渲染阻塞。效果参考：[Refresh示例6实现下拉刷新上拉加载更多](../../../harmonyos-references/ArkUI（方舟UI框架）/ArkTS组件/滚动与滑动/Refresh/ts-container-refresh.md#示例6实现下拉刷新上拉加载更多)。
...
当开发网格页面涉及大量数据，需要进行分页或分批次进行请求时，结合ScrollComponents提供的懒加载能力实现加载更多的效果。效果参考：[Refresh示例6实现下拉刷新上拉加载更多](../../../harmonyos-references/ArkUI（方舟UI框架）/ArkTS组件/滚动与滑动/Refresh/ts-container-refresh.md#示例6实现下拉刷新上拉加载更多)。
...
通过设置行列数量与尺寸占比可以确定网格布局的整体排列方式。效果参考：[Grid设置行列数量与占比](<../../../harmonyos-guides/应用框架/ArkUI（方舟UI框架）/UI开发 (ArkTS声明式开发范式)/列表与网格/创建网格 (GridGridItem)/arkts-layout-development-create-grid.md#设置行列数量与占比>)。
...
[PhotoGridComponent.ets](https://gitcode.com/harmonyos_samples/GridScrollComponent/blob/master/entry/src/main/ets/view/PhotoGridComponent.ets#L35-L81)

### 设置主轴方向

通过layoutDirection设置网格布局的主轴方向，决定子组件的排列方式。效果参考：[Grid设置主轴方向](<../../../harmonyos-guides/应用框架/ArkUI（方舟UI框架）/UI开发 (ArkTS声明式开发范式)/列表与网格/创建网格 (GridGridItem)/arkts-layout-development-create-grid.md#设置主轴方向>)。
...
[PhotoGridComponent.ets](https://gitcode.com/harmonyos_samples/GridScrollComponent/blob/master/entry/src/main/ets/view/PhotoGridComponent.ets#L36-L80)

## 网格布局中显示数据的场景

### 场景描述

可以通过在单元格组件中添加文本等组件在网格中显示数据。效果参考：[Grid在网格布局中显示数据](<../../../harmonyos-guides/应用框架/ArkUI（方舟UI框架）/UI开发 (ArkTS声明式开发范式)/列表与网格/创建网格 (GridGridItem)/arkts-layout-development-create-grid.md#在网格布局中显示数据>)。
...
在两个网格单元之间的网格横向间距称为行间距，网格纵向间距称为列间距。通过rowsGap和columnsGap可以设置网格布局的行列间距。效果参考：[Grid设置行列间距](<../../../harmonyos-guides/应用框架/ArkUI（方舟UI框架）/UI开发 (ArkTS声明式开发范式)/列表与网格/创建网格 (GridGridItem)/arkts-layout-development-create-grid.md#设置行列间距>)。
...
[CardGridComponent.ets](https://gitcode.com/harmonyos_samples/GridScrollComponent/blob/master/entry/src/main/ets/view/CardGridComponent.ets#L37-L122)

## 网格构建可滚动的布局场景

### 场景描述

通过columnsTemplate和rowsTemplate可以让网格拥有滚动能力。效果参考：[Grid构建可滚动的网格布局](<../../../harmonyos-guides/应用框架/ArkUI（方舟UI框架）/UI开发 (ArkTS声明式开发范式)/列表与网格/创建网格 (GridGridItem)/arkts-layout-development-create-grid.md#构建可滚动的网格布局>)。
...
[CardGridComponent.ets](https://gitcode.com/harmonyos_samples/GridScrollComponent/blob/master/entry/src/main/ets/view/CardGridComponent.ets#L38-L121)

## 网格控制滚动位置场景

### 场景描述

通过对设置[Scroller](../../../harmonyos-references/ArkUI（方舟UI框架）/ArkTS组件/滚动与滑动/Scroll/ts-container-scroll.md#scroller)对象，可以进行滚动控制。效果参考：[Grid控制滚动位置](<../../../harmonyos-guides/应用框架/ArkUI（方舟UI框架）/UI开发 (ArkTS声明式开发范式)/列表与网格/创建网格 (GridGridItem)/arkts-layout-development-create-grid.md#控制滚动位置>)。
...
1. 绑定Scroller对象
   ...
   [CardGridComponent.ets](https://gitcode.com/harmonyos_samples/GridScrollComponent/blob/master/entry/src/main/ets/view/CardGridComponent.ets#L26-L201)
...
通过nestedScroll和onScrollFrameBegin设置嵌套滚动效果。效果参考：[Grid示例4grid嵌套滚动](../../../harmonyos-references/ArkUI（方舟UI框架）/ArkTS组件/滚动与滑动/Grid/ts-container-grid.md#示例4grid嵌套滚动)。
...
通过对拖拽事件处理和数据交换逻辑可以实现单元格拖拽效果。效果参考：[Grid示例5grid拖拽场景](../../../harmonyos-references/ArkUI（方舟UI框架）/ArkTS组件/滚动与滑动/Grid/ts-container-grid.md#示例5grid拖拽场景)。

### 开发步骤

1. 设置属性editMode(true)设置网格是否进入编辑模式，进入编辑模式可以拖拽网格组件内部单元格。
2. 在[onItemDragStart](../../../harmonyos-references/ArkUI（方舟UI框架）/ArkTS组件/滚动与滑动/Grid/ts-container-grid.md#onitemdragstart8)回调中设置拖拽过程中显示的图片。
3. 在[onItemDrop](../../../harmonyos-references/ArkUI（方舟UI框架）/ArkTS组件/滚动与滑动/Grid/ts-container-grid.md#onitemdrop8)中获取拖拽起始位置，和拖拽插入位置，并在[onItemDrop](../../../harmonyos-references/ArkUI（方舟UI框架）/ArkTS组件/滚动与滑动/Grid/ts-container-grid.md#onitemdrop8)中完成转移数组位置逻辑。
...
通过layoutDirection、maxCount、minCount、cellLength实现自适应效果。rowsTemplate、columnsTemplate都不设置layoutDirection、maxCount、minCount、cellLength才生效。效果参考：[Grid示例6自适应grid](../../../harmonyos-references/ArkUI（方舟UI框架）/ArkTS组件/滚动与滑动/Grid/ts-container-grid.md#示例6自适应grid)。
...
通过属性columnsTemplate中auto-fill、auto-fit和auto-stretch实现列数自适应效果。效果参考：[Grid示例8设置自适应列数](../../../harmonyos-references/ArkUI（方舟UI框架）/ArkTS组件/滚动与滑动/Grid/ts-container-grid.md#示例8设置自适应列数)。
...
可以通过alignItems设置当前行最高单元格高度为其他单元格的高度。效果参考：[Grid示例9以当前行最高的griditem的高度为其他griditem的高度](../../../harmonyos-references/ArkUI（方舟UI框架）/ArkTS组件/滚动与滑动/Grid/ts-container-grid.md#示例9以当前行最高的griditem的高度为其他griditem的高度)。
...
通过[fadingEdge](../../../harmonyos-references/ArkUI（方舟UI框架）/ArkTS组件/滚动与滑动/滚动组件通用接口/ts-container-scrollable-common.md#fadingedge14)属性来设置边缘渐隐效果。效果参考：[Grid示例10设置边缘渐隐](../../../harmonyos-references/ArkUI（方舟UI框架）/ArkTS组件/滚动与滑动/Grid/ts-container-grid.md#示例10设置边缘渐隐)。
...
可以使用gridViewManager.setItemViewStyle()接口设置单元格样式。效果参考：[GridItem示例2设置griditem样式](../../../harmonyos-references/ArkUI（方舟UI框架）/ArkTS组件/滚动与滑动/GridItem/ts-container-griditem.md#示例2设置griditem样式)。
...
[CardGridComponent.ets](https://gitcode.com/harmonyos_samples/GridScrollComponent/blob/master/entry/src/main/ets/view/CardGridComponent.ets#L39-L120)
...
2. 响应删除事件
   ...
   [CardGridComponent.ets](https://gitcode.com/harmonyos_samples/GridScrollComponent/blob/master/entry/src/main/ets/view/CardGridComponent.ets#L40-L132)
