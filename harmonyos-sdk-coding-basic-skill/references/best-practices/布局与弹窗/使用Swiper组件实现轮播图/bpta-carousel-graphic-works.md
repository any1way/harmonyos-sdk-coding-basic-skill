...
scraped_at: 2026-06-11T16:54:48+08:00
doc_updated_at: 2026-03-12
content_hash: sha256:300b2c1cbad22ed47dfd8ba14e4bd8409ffc11f8bbc79483317fbb535a056d50
---
## 概述
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/19/v3/Vl6N7ZG2REqr0sXrcJDftw/zh-cn_image_0000002396390305.gif?HW-CC-KV=V1&HW-CC-Date=20260611T085442Z&HW-CC-Expire=86400&HW-CC-Sign=3FE2EDF2C80C4055D50B545919E087DC40CF6CACCD2D976BBF24F8110D5B74C2 "点击放大")

### 实现原理

图文作品轮播可以通过Swiper组件及其指示器的联动效果来实现。由于Swiper组件的指示器不可自定义，因此需要分开实现。

图片区域需要使用[Swiper](../../../harmonyos-references/ArkUI（方舟UI框架）/ArkTS组件/滚动与滑动/Swiper/ts-container-swiper.md)组件来实现。将图片合集的数据传入Swiper组件后，需要对Swiper组件设置一些属性，来完成图片自动轮播效果。

1. 通过设置[loop](../../../harmonyos-references/ArkUI（方舟UI框架）/ArkTS组件/滚动与滑动/Swiper/ts-container-swiper.md#loop)属性控制是否循环播放，该属性默认值为true。当loop为true时，在显示第一页或最后一页时，可以继续往前切换到前一页或者往后切换到后一页。如果loop为false，则在第一页或最后一页时，无法继续向前或者向后切换页面。
2. 通过设置[autoPlay](../../../harmonyos-references/ArkUI（方舟UI框架）/ArkTS组件/滚动与滑动/Swiper/ts-container-swiper.md#autoplay)属性，控制是否自动轮播子组件。该属性默认值为false。autoPlay为true时，会自动切换播放子组件。
3. 通过设置[interval](../../../harmonyos-references/ArkUI（方舟UI框架）/ArkTS组件/滚动与滑动/Swiper/ts-container-swiper.md#interval)属性，控制子组件与子组件之间的播放间隔。interval属性默认值为3000，单位毫秒。
4. 通过设置[indicator](../../../harmonyos-references/ArkUI（方舟UI框架）/ArkTS组件/滚动与滑动/Swiper/ts-container-swiper.md#indicator)属性为false，来关闭Swiper组件自带的导航点指示器样式。

底部导航点（进度条）有三种样式：未完成状态的样式、已完成状态的样式和正在进行进度增长的样式。

1. 进度条布局：开发者可以使用[层叠布局 (Stack)](<../../../harmonyos-guides/应用框架/ArkUI（方舟UI框架）/UI开发 (ArkTS声明式开发范式)/组件布局/构建布局/层叠布局 (Stack)/arkts-layout-development-stack-layout.md>)，配合Row容器来实现进度条的布局。
2. 图文播放时间与进度条匹配：要实现进度条缓慢增长至完成状态且用时与图片播放时间相匹配的效果，可以给Row容器组件添加[属性动画 (animation)](<../../../harmonyos-references/ArkUI（方舟UI框架）/ArkTS组件/动画/属性动画 (animation)/ts-animatorproperty.md>)，设置duration（动画持续时间）与图片播放时间匹配即可。
...
1. 为Swiper组件设置[loop](../../../harmonyos-references/ArkUI（方舟UI框架）/ArkTS组件/滚动与滑动/Swiper/ts-container-swiper.md#loop)、[autoPlay](../../../harmonyos-references/ArkUI（方舟UI框架）/ArkTS组件/滚动与滑动/Swiper/ts-container-swiper.md#autoplay)、[interval](../../../harmonyos-references/ArkUI（方舟UI框架）/ArkTS组件/滚动与滑动/Swiper/ts-container-swiper.md#interval)和[indicator](../../../harmonyos-references/ArkUI（方舟UI框架）/ArkTS组件/滚动与滑动/Swiper/ts-container-swiper.md#indicator)属性。在手指未滑动的情况下，图片每3秒会进行一次切换，并且自动进行轮播。
   ...
   ![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/8e/v3/yJ3RCIrPTlqMNrIqfh8BcQ/zh-cn_image_0000002362872296.gif?HW-CC-KV=V1&HW-CC-Date=20260611T085442Z&HW-CC-Expire=86400&HW-CC-Sign=29BD11D72621DDB1640D8C2CC1ACCA6E5D4E23D9D7F6D085B9A8A73735514A4B)
2. 创建进度条自定义组件progressComponent。代码中，this.progressData为图片集合的数组，this.currentIndex为当前播放的图片在图片集合数组中的索引，index为进度条对应的图片在图片集合数组中的索引。当this.currentIndex > index时，表示图片集合数组中索引0-index的进度条都是已完成状态。
   ...
   ![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/67/v3/WgX_CrpFTSWENbPqGIg-vw/zh-cn_image_0000002396272325.gif?HW-CC-KV=V1&HW-CC-Date=20260611T085442Z&HW-CC-Expire=86400&HW-CC-Sign=EDD80B95D3EFFEC416ECCCBA44AF6F0C23C8AB79287A3D4160A782461DDC3B4D)
3. 滑动切换图片后，关闭自动轮播与循环轮播。此时，开发者需要给Swiper组件添加[onGestureSwipe](../../../harmonyos-references/ArkUI（方舟UI框架）/ArkTS组件/滚动与滑动/Swiper/ts-container-swiper.md#ongestureswipe10)事件，来判断页面是否跟手滑动。其中slide为布尔值，用来判断页面是否跟手滑动。默认值为false，当页面跟手滑动时，slide的值为true。当进行滑动切换时，autoPlay、loop属性的取值为false，即关闭自动轮播与循环播放功能。若想实现滑动图片后仍自动循环轮播，直接去掉slide相关代码片段即可。
   ...
   ![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/57/v3/avdE4xBDTCacruigizQ3KA/zh-cn_image_0000002362712392.gif?HW-CC-KV=V1&HW-CC-Date=20260611T085442Z&HW-CC-Expire=86400&HW-CC-Sign=3D68CE71672C87A59E8497242619D7BCB80BEE7D381B3108BD7F8FC7F536FB2D "点击放大")
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/e/v3/HgwBJde1QJqt93NgzrErgQ/zh-cn_image_0000002362716188.gif?HW-CC-KV=V1&HW-CC-Date=20260611T085442Z&HW-CC-Expire=86400&HW-CC-Sign=8FE14E28B3E942B63EBDC05AE5A5B9CE410CF9A01C492677CBC6B7582A4680FD "点击放大")

### 实现原理

使用[层叠布局 (Stack)](<../../../harmonyos-guides/应用框架/ArkUI（方舟UI框架）/UI开发 (ArkTS声明式开发范式)/组件布局/构建布局/层叠布局 (Stack)/arkts-layout-development-stack-layout.md>)可以实现图片的叠加，结合滑动手势来实现图片的切换，并通过[animateTo](<../../../harmonyos-references/ArkUI（方舟UI框架）/ArkTS API/UI界面/@ohos.arkui.UIContext (UIContext)/Class (UIContext)/arkts-apis-uicontext-uicontext.md#animateto>)添加相应的切换动画。通过给图片添加以下属性来实现。

* [zIndex](../../../harmonyos-references/ArkUI（方舟UI框架）/ArkTS组件/通用属性/基础属性/Z序控制/ts-universal-attributes-z-order.md#zindex)属性来设置组件的堆叠顺序，Index值越大，显示层级越高，即zIndex值大的组件会覆盖在zIndex值小的组件上方。
* [visibility](../../../harmonyos-references/ArkUI（方舟UI框架）/ArkTS组件/通用属性/基础属性/显隐控制/ts-universal-attributes-visibility.md#visibility)属性来控制显示或隐藏，值为Visibility.Hidden时，表示隐藏，但参与布局进行占位；值为Visibility.Visible时，表示显示；值为Visibility.None时，表示隐藏，但不参与布局，不进行占位。
* [scale](../../../harmonyos-references/ArkUI（方舟UI框架）/ArkTS组件/通用属性/视效与模糊/图形变换/ts-universal-attributes-transformation.md#scale)属性来控制是否缩放，可以分别设置X轴、Y轴、Z轴的缩放比例，默认值为1。
