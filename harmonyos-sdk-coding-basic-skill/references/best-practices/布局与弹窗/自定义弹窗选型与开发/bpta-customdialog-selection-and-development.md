...
scraped_at: 2026-06-11T16:55:06+08:00
doc_updated_at: 2026-05-18
content_hash: sha256:af262bec553ac592b0d3bf1a0078737f6e56df3281c2010839d0a134e7da00c7
---
## 概述
...
* **主要推荐的弹窗**
  1. 基于UIContext实现的自定义弹窗：UIContext弹窗是一种基于上下文（Context）的弹窗管理机制，通过ComponentContent封装内容可以与UI界面解耦，调用更加灵活，可以满足开发者的封装诉求。具有较高的灵活性，弹窗样式完全自定义。当关联的UIContext销毁时（如页面关闭），弹窗会自动关闭，无需手动管理。其次，弹窗的层级由UIContext管理，与页面路由解耦，适合复杂场景。当前实现弹窗的主要API有[UIContext.openBindSheet()](<../../../harmonyos-references/ArkUI（方舟UI框架）/ArkTS API/UI界面/@ohos.arkui.UIContext (UIContext)/Class (UIContext)/arkts-apis-uicontext-uicontext.md#openbindsheet12>)、[UIContext.getPromptAction().openCustomDialog()](<../../../harmonyos-references/ArkUI（方舟UI框架）/ArkTS API/UI界面/@ohos.arkui.UIContext (UIContext)/Class (PromptAction)/arkts-apis-uicontext-promptaction.md#opencustomdialog12>)、[UIContext.getOverlayManager()](<../../../harmonyos-references/ArkUI（方舟UI框架）/ArkTS API/UI界面/@ohos.arkui.UIContext (UIContext)/Class (UIContext)/arkts-apis-uicontext-uicontext.md#getoverlaymanager12>)等。
  2. 基于Navigation.Dialog实现的自定义弹窗：NavDestination.Dialog（即[页面显示类型](<../../../harmonyos-guides/应用框架/ArkUI（方舟UI框架）/UI开发 (ArkTS声明式开发范式)/设置组件导航和页面路由/组件导航(Navigation) (推荐)/Navigation子页面/arkts-navigation-navdestination.md#页面显示类型>)中的弹窗类型）是基于Navigation组件实现的弹窗效果，它本质上属于路由页面，存在于路由栈中，可用于实现模态、半模态等形式，适用于透明页面、切换页面弹窗不消失等场景。
...
* **不推荐使用的弹窗**
  1. [基础自定义弹出框 (CustomDialog)](<../../../harmonyos-guides/应用框架/ArkUI（方舟UI框架）/UI开发 (ArkTS声明式开发范式)/使用弹窗/弹出框 (Dialog)/基础自定义弹出框 (CustomDialog)/arkts-common-components-custom-dialog.md>)：[CustomDialogController](<../../../harmonyos-references/ArkUI（方舟UI框架）/ArkTS组件/弹窗/自定义弹窗 (CustomDialog)/ts-methods-custom-dialog-box.md#customdialogcontroller>) 在使用上存在较多限制，不支持动态创建和动态刷新，只能在@Component修饰的自定义组件内部使用。这导致弹窗的创建与管理必须依赖具体组件，增加了代码复杂度和维护成本。此外，当一个页面需要展示多个自定义弹窗时，需为每个弹窗单独声明对应的 CustomDialogController，进一步造成 UI 层代码冗余，难以实现弹窗与页面逻辑的解耦。
  2. [@ohos.promptAction (弹窗)](<../../../harmonyos-references/ArkUI（方舟UI框架）/ArkTS API/UI界面/@ohos.promptAction (弹窗)/js-apis-promptaction.md>)：@ohos.promptAction是一种全局方法，在没有UIContext上下文的场景中执行时会有问题，或者因为开发者不能指定UIContext可能导致弹窗显示到非开发者预期的窗口里，所以不推荐使用。此外，其采用系统默认弹窗样式，无法进行深度样式自定义，适用于希望保持与系统风格一致的简单提示类弹窗，适用范围较为有限。
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/dc/v3/Ln34W2rQRQSWATHHaVrsdw/zh-cn_image_0000002311582804.gif?HW-CC-KV=V1&HW-CC-Date=20260611T085451Z&HW-CC-Expire=86400&HW-CC-Sign=4B0C5DE6A3415514E996CD9DC4B93E53BE58CA255A80B8DCC9A0F7FB6AB8D619 "点击放大")

**实现方案**

由于[showToast](<../../../harmonyos-references/ArkUI（方舟UI框架）/ArkTS API/UI界面/@ohos.arkui.UIContext (UIContext)/Class (PromptAction)/arkts-apis-uicontext-promptaction.md#showtoast>)仅支持弹出文本类型，无法实现图文混合形式的提示弹窗，针对此类场景可以使用[UIContext.getPromptAction().openCustomDialog()](<../../../harmonyos-references/ArkUI（方舟UI框架）/ArkTS API/UI界面/@ohos.arkui.UIContext (UIContext)/Class (PromptAction)/arkts-apis-uicontext-promptaction.md#opencustomdialog12>)实现。
...
通过[UIContext.getPromptAction().openCustomDialog()](<../../../harmonyos-references/ArkUI（方舟UI框架）/ArkTS API/UI界面/@ohos.arkui.UIContext (UIContext)/Class (PromptAction)/arkts-apis-uicontext-promptaction.md#opencustomdialog12>)打开弹窗，使用[BaseDialogOptions](<../../../harmonyos-references/ArkUI（方舟UI框架）/ArkTS API/UI界面/@ohos.promptAction (弹窗)/js-apis-promptaction.md#basedialogoptions11>)配置弹窗样式。
...
需要注意的是，dialog类型弹窗在弹出时会抢占焦点，此处如果存在正在输入的文本框，会导致键盘收起。在需要保持原有界面可用性的场景下，用户希望弹窗不主动获取焦点，避免打断用户的当前操作，可以通过设置弹窗的[promptAction.BaseDialogOptions](<../../../harmonyos-references/ArkUI（方舟UI框架）/ArkTS API/UI界面/@ohos.promptAction (弹窗)/js-apis-promptaction.md#basedialogoptions11>)的[focusable](../../../harmonyos-references/ArkUI（方舟UI框架）/ArkTS组件/通用属性/交互属性/焦点控制/ts-universal-attributes-focus.md#focusable)属性为false，即不允许弹窗获取焦点。
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/8c/v3/7ll9JlLrQaGvIBQspe5crw/zh-cn_image_0000002345701585.gif?HW-CC-KV=V1&HW-CC-Date=20260611T085451Z&HW-CC-Expire=86400&HW-CC-Sign=7034C64F8B3E71F17A10EEB75EA0BCF91DE7C5B08DA8F3647C14F543A94BF895 "点击放大")
...
可以使用[UIContext.getPromptAction().openCustomDialog()](<../../../harmonyos-references/ArkUI（方舟UI框架）/ArkTS API/UI界面/@ohos.arkui.UIContext (UIContext)/Class (PromptAction)/arkts-apis-uicontext-promptaction.md#opencustomdialog12>)实现，其支持通过levelMode和levelUniqueId配置页面级弹窗。
...
* **切换****页面弹窗不消失**
  ...
  通过自定义弹窗选项[BaseDialogOptions](<../../../harmonyos-references/ArkUI（方舟UI框架）/ArkTS API/UI界面/@ohos.promptAction (弹窗)/js-apis-promptaction.md#basedialogoptions11>)中的levelMode和levelUniqueId设置弹出框在指定页面内弹出，此参数接收页面内的节点id，设置后，弹出框显示时会自动查询此id对应的节点所在的Navigation页面，并将其挂载在此页面下。如下代码示例所示，Button节点为指定页面的节点，设置自定义id后，通过[getFrameNodeById()](<../../../harmonyos-references/ArkUI（方舟UI框架）/ArkTS API/UI界面/@ohos.arkui.UIContext (UIContext)/Class (UIContext)/arkts-apis-uicontext-uicontext.md#getframenodebyid12>)方法获取该节点，再通过[getUniqueId](../../../harmonyos-references/ArkUI（方舟UI框架）/ArkTS组件/自定义组件/自定义组件内置方法/ts-custom-component-api.md#getuniqueid12)获取节点的内部id，并将其作为levelUniqueId的值传入。
...
* **侧滑拦截**
  ...
  通过UIContext.getPromptAction弹窗的onWillDismiss回调函数实现侧滑拦截。当用户执行点击遮障层关闭、侧滑、三键back、键盘ESC关闭交互操作时，注册该回调函数后，弹窗不会立刻关闭。在回调函数中通过[DismissReason](../../../harmonyos-references/ArkUI（方舟UI框架）/ArkTS组件/通用属性/弹窗控制/Popup控制/ts-universal-attributes-popup.md#dismissreason12枚举说明)枚举确定关闭原因，从而选择是否关闭弹窗。
...
**使用[NavDestinationMode.DIALOG弹窗](<../../../harmonyos-guides/应用框架/ArkUI（方舟UI框架）/UI开发 (ArkTS声明式开发范式)/设置组件导航和页面路由/组件导航(Navigation) (推荐)/Navigation子页面/arkts-navigation-navdestination.md#页面显示类型>)实现**
...
* **侧滑拦截**
  ...
  通过NavDestination的回调函数[onBackPressed()](../../../harmonyos-references/ArkUI（方舟UI框架）/ArkTS组件/导航与切换/NavDestination/ts-basic-components-navdestination.md#onbackpressed10)实现侧滑拦截。当点击物理返回按钮或使用手势滑动时，触发该回调。返回值为true时，表示重写返回键逻辑，即可实现侧滑拦截。
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/af/v3/CW2WQ6HUS6S3rvqQVtOCFQ/zh-cn_image_0000002311742592.gif?HW-CC-KV=V1&HW-CC-Date=20260611T085451Z&HW-CC-Expire=86400&HW-CC-Sign=DA62648A94CDA3427FC1066C217F87C0F7811A0626E92CBB1F0887EE7B7ED3EC "点击放大")

**实现方案**

* **在页面中更新弹窗内容**：

  更新弹窗中自定义组件的内容可以通过ComponentContent提供的[update()](<../../../harmonyos-references/ArkUI（方舟UI框架）/ArkTS API/UI界面/arkui/ComponentContent/js-apis-arkui-componentcontent.md#update>)方法来实现，即contentNode.update()更新弹窗的数据并同步到UI进行展示。
...
* **自定义弹窗显示和退出动画**
  ...
  可以使用[BaseDialogOptions](<../../../harmonyos-references/ArkUI（方舟UI框架）/ArkTS API/UI界面/@ohos.promptAction (弹窗)/js-apis-promptaction.md#basedialogoptions11>)的transition参数设置弹窗显示和退出的过渡效果，以此实现以渐隐渐显的方式呈现自定义弹窗的显示和退出动画。
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/b2/v3/ijnUcZN1TQy4W5t6zshqAw/zh-cn_image_0000002345581817.gif?HW-CC-KV=V1&HW-CC-Date=20260611T085451Z&HW-CC-Expire=86400&HW-CC-Sign=9AE3F8175D85B6452FF84C502DE9FED1221DE06C3AC1F3038F42241AAE3AF63B "点击放大")

**实现方案**

使用[openBindsheet](<../../../harmonyos-references/ArkUI（方舟UI框架）/ArkTS API/UI界面/@ohos.arkui.UIContext (UIContext)/Class (UIContext)/arkts-apis-uicontext-uicontext.md#openbindsheet12>)，通过ComponentContent封装半模态页面中显示的组件内容，[SheetOptions](../../../harmonyos-references/ArkUI（方舟UI框架）/ArkTS组件/通用属性/模态转场设置/半模态转场/ts-universal-attributes-sheet-transition.md#sheetoptions)设置半模态页面样式。
...
* **可变高度操作弹窗**
  ...
  可以通过[SheetOptions](../../../harmonyos-references/ArkUI（方舟UI框架）/ArkTS组件/通用属性/模态转场设置/半模态转场/ts-universal-attributes-sheet-transition.md#sheetoptions)的detents参数设置为可变高度，跟手拖动。
  ...
  ![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/26/v3/7F9JZvWtQhKeUfOSyY2ftg/zh-cn_image_0000002311582836.gif?HW-CC-KV=V1&HW-CC-Date=20260611T085451Z&HW-CC-Expire=86400&HW-CC-Sign=8B99535804E644BA9FA2449214CD6419B49A857949D588373C59B5BE90C90DEA "点击放大")

### 实现评论回复弹窗

评论回复模块在图文和视频应用中被广泛使用，包含编辑区域、好友列表、常用表情列表和表情面板（见下图），它允许用户进行输入文字、表情、@好友、选择图片等操作。具体实现方案请参见：[评论回复弹窗开发实践](../评论回复弹窗/bpta-comment-reply-pop-up-window.md)。
