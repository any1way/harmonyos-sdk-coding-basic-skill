...
scraped_at: 2026-06-01T16:59:31+08:00
doc_updated_at: 2026-03-12
content_hash: sha256:5de3fcc8d237f6e632588e680d847b60c9e916ef32673397d414aeffb0b230df
---
## 概述
...
1. 选择加解密算法并确定密钥
   * 根据具体的安全需求，选择合适的加解密算法。可以是[对称密钥加解密算法](<../../../harmonyos-guides/系统/安全/Crypto Architecture Kit（加解密算法框架服务）/加解密/加解密算法规格/对称密钥加解密算法规格/crypto-sym-encrypt-decrypt-spec.md>)（例如AES）、[非对称密钥加解密算法](<../../../harmonyos-guides/系统/安全/Crypto Architecture Kit（加解密算法框架服务）/加解密/加解密算法规格/非对称密钥加解密算法规格/crypto-asym-encrypt-decrypt-spec.md>)（例如RSA）等。
...
3. HarmonyOS平台在初始化加解密引擎时，需要传入固定的参数，参数格式可参考[加解密算法规格](<../../../harmonyos-guides/系统/安全/Crypto Architecture Kit（加解密算法框架服务）/加解密/加解密算法规格/crypto-encrypt-decrypt-spec.md>)。以AES为例，详细分析密钥长度使用问题和偏移量参数的设置问题，具体可参考[AES加解密](bpta-cross-platform-compatibility.md#section848458172418)。

### SM2加解密

[SM2](<../../../harmonyos-guides/系统/安全/Crypto Architecture Kit（加解密算法框架服务）/密钥生成和转换/密钥生成和转换规格/非对称密钥生成和转换规格/crypto-asym-key-generation-conversion-spec.md#sm2>)，是一种基于椭圆曲线的公钥密码算法。
...
* SM2密钥格式转换
  ...
  将对应的16进制参数放入对应的位置，可通过[使用密钥参数生成](<../../../harmonyos-guides/系统/安全/Crypto Architecture Kit（加解密算法框架服务）/密钥生成和转换/密钥生成和转换规格/非对称密钥生成和转换规格/crypto-asym-key-generation-conversion-spec.md#使用密钥参数生成-3>)SM2公钥。
  ...
  同理，可通过[使用密钥参数生成](<../../../harmonyos-guides/系统/安全/Crypto Architecture Kit（加解密算法框架服务）/密钥生成和转换/密钥生成和转换规格/非对称密钥生成和转换规格/crypto-asym-key-generation-conversion-spec.md#使用密钥参数生成-3>)SM2私钥。
...
* SM2密文格式转换
  ...
  HarmonyOS平台支持的SM2密文格式为国密标准的ASN.1格式，参数组合顺序为C1C3C2。具体参数含义请参考[转换SM2密文格式](<../../../harmonyos-guides/系统/安全/Crypto Architecture Kit（加解密算法框架服务）/加解密/加解密算法规格/非对称密钥加解密算法规格/crypto-asym-encrypt-decrypt-spec.md#转换sm2密文格式>)。

  对于其他平台加密的C1C2C3顺序的密文，在HarmonyOS平台解密时，需先提取所需要的参数，并构造[SM2CipherTextSpec](<../../../harmonyos-references/安全/Crypto Architecture Kit（加解密算法框架服务）/ArkTS API/@ohos.security.cryptoFramework (加解密算法库框架)/js-apis-cryptoframework.md#sm2ciphertextspec12>)对象，接着调用[genCipherTextBySpec()](<../../../harmonyos-references/安全/Crypto Architecture Kit（加解密算法框架服务）/ArkTS API/@ohos.security.cryptoFramework (加解密算法库框架)/js-apis-cryptoframework.md#genciphertextbyspec12>)方法序列化生成ASN.1格式的SM2密文。具体可参考[使用SM2密文格式转换(ArkTS)](<../../../harmonyos-guides/系统/安全/Crypto Architecture Kit（加解密算法框架服务）/加解密/加解密开发指导/使用SM2密文格式转换(ArkTS)/crypto-sm2-ciphertext-conversion.md>)。
...
[AES](<../../../harmonyos-guides/系统/安全/Crypto Architecture Kit（加解密算法框架服务）/密钥生成和转换/密钥生成和转换规格/对称密钥生成和转换规格/crypto-sym-key-generation-conversion-spec.md#aes>)（Advanced Encryption Standard），最常见的对称加密算法。

下面介绍并解决在HarmonyOS平台与其他平台交互使用AES加解密开发中遇到的问题：密钥长度和指定分组模式后的偏移量参数设置。

* AES密钥长度使用问题

  在[AES](<../../../harmonyos-guides/系统/安全/Crypto Architecture Kit（加解密算法框架服务）/密钥生成和转换/密钥生成和转换规格/对称密钥生成和转换规格/crypto-sym-key-generation-conversion-spec.md#aes>)加解密算法中，密钥长度是固定的。
  ...
  以指南[使用AES对称密钥（GCM模式）加解密(ArkTS)](<../../../harmonyos-guides/系统/安全/Crypto Architecture Kit（加解密算法框架服务）/加解密/加解密开发指导/使用AES对称密钥（GCM模式）加解密(ArkTS)/crypto-aes-sym-encrypt-decrypt-gcm.md>)为例，使用'AES128|GCM|PKCS7'初始化加解密算法实例。加解密时对应的密钥应为128位，即16个字符。因此，在生成密钥时，传入的字符为newUint8Array([83,217,231,76,28,113,23,219,250,71,209,210,205,97,32,159])。
...
* AES指定分组模式后偏移量参数设置问题
  ...
  由于AES为分组加密算法，分组长度为128位。在实际应用中，最后一组明文可能不足128位（16字节），此时可以通过不同的[填充模式](<../../../harmonyos-guides/系统/安全/Crypto Architecture Kit（加解密算法框架服务）/加解密/加解密算法规格/对称密钥加解密算法规格/crypto-sym-encrypt-decrypt-spec.md#填充模式>)进行数据填充。

  以[GCM模式](<../../../harmonyos-guides/系统/安全/Crypto Architecture Kit（加解密算法框架服务）/加解密/加解密开发指导/使用AES对称密钥（GCM模式）加解密(ArkTS)/crypto-aes-sym-encrypt-decrypt-gcm.md>)和[CBC模式](<../../../harmonyos-guides/系统/安全/Crypto Architecture Kit（加解密算法框架服务）/加解密/加解密开发指导/使用AES对称密钥（CBC模式）加解密(ArkTS)/crypto-aes-sym-encrypt-decrypt-cbc.md>)为例，指定字符串参数分别为'AES128|GCM|PKCS7'和'AES128|CBC|PKCS7'，偏移量分别为16字节和12字节。
