...
scraped_at: 2026-06-12T10:20:42+08:00
doc_updated_at: 2026-06-05
content_hash: sha256:4a65b1831d4eccf797630682e1460ef9d307f45b425e6fb99e04608e250cdee1
---
1. 配置ohpmignore文件：如果部分工程源文件无需构建到HAR包中，可在module目录下新建.ohpmignore文件，用于配置打包时要忽略的文件。将无需打包的文件或文件夹名称写入.ohpmignore文件中，DevEco Studio构建时将过滤掉这些文件目录。
   ...
   ![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/44/v3/r9owBOjXTaWjMbvQ3gV5jA/zh-cn_image_0000002194318100.png?HW-CC-KV=V1&HW-CC-Date=20260612T022041Z&HW-CC-Expire=86400&HW-CC-Sign=CC8A9B84C957644405E1171AE22179CEFCE24707C8F593CA5E89004C923AA122)

   更改 .ohpmignore 配置后，需清空相应模块的 build 文件夹，或在DevEco Studio中选择 Build -> clean project，然后再进行打包。
2. 构建闭源HAR：DevEco Studio支持闭源HAR构建，通过对代码进行编译混淆，生成闭源HAR。在不共享源码的情况下，通过闭源HAR对外提供组件、资源等，可以实现多个模块或多个工程共享组件、资源等。具体实现方式可以参考：[以release模式构建](../../../../harmonyos-guides/构建应用/配置构建流程/构建HAR/ide-hvigor-build-har.md#section19788284410)。
3. 可以通过开启混淆能力来保护代码资产。代码混淆的使用约束包括：仅支持Stage工程；在编译模式为release模式时生效；模块及其依赖的HAR和HSP均未关闭混淆。
   ...
   * "files"：配置混淆规则文件路径。
   ...
   具体内容可参考链接：[混淆加固](../../../../harmonyos-guides/构建应用/混淆加固/ide-build-obfuscation.md)。
