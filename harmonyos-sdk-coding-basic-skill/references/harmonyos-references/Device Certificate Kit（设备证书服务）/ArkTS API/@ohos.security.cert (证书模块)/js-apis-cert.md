...
scraped_at: 2026-06-11T16:12:13+08:00
doc_updated_at: 2026-06-05
content_hash: sha256:1d2c95c3db6fddef5f2698537c28f64c5a117c3d1c68574ff8184ca292bab02f
---
证书算法库框架提供证书相关接口。其中，依赖加解密算法库框架的基础算法能力的部分，详细接口说明可参考[@ohos.security.cryptoFramework (加解密算法库框架)](<../../../Crypto Architecture Kit（加解密算法框架服务）/ArkTS API/@ohos.security.cryptoFramework (加解密算法库框架)/js-apis-cryptoframework.md>)。
...
| 名称 | 类型 | 只读 | 可选 | 说明 |
| --- | --- | --- | --- | --- |
| type | [GeneralNameType](js-apis-cert.md#generalnametype12) | 否 | 否 | 指定具体的证书主体类型。 |
...
| allowDownloadIntermediateCa23+ | boolean | 否 | 是 | 表示是否允许尝试从网络下载缺失的中间CA证书。  true表示允许；false表示不允许。默认值为false。  下载地址将从证书AIA扩展中获取，仅支持http，如需使用网络下载，需申请ohos.permission.INTERNET权限。配置方式请参见[声明权限](../../../../harmonyos-guides/系统/安全/程序访问控制/应用权限管控/申请应用权限/声明权限/declare-permissions.md)。  **元服务API：** 从API version 23开始，该接口支持在元服务中使用。 |
...
以下错误码的详细介绍请参见[证书错误码](../../错误码/证书错误码/errorcode-cert.md)。
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| key | [cryptoFramework.PubKey](<../../../Crypto Architecture Kit（加解密算法框架服务）/ArkTS API/@ohos.security.cryptoFramework (加解密算法库框架)/js-apis-cryptoframework.md#pubkey>) | 是 | 用于验签的公钥对象。 |
...
| 类型 | 说明 |
| --- | --- |
| [cryptoFramework.PubKey](<../../../Crypto Architecture Kit（加解密算法框架服务）/ArkTS API/@ohos.security.cryptoFramework (加解密算法库框架)/js-apis-cryptoframework.md#pubkey>) | X509证书公钥对象：仅用于X509Cert的verify接口。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| key | [cryptoFramework.PubKey](<../../../Crypto Architecture Kit（加解密算法框架服务）/ArkTS API/@ohos.security.cryptoFramework (加解密算法库框架)/js-apis-cryptoframework.md#pubkey>) | 是 | 表示用于验签的公钥对象。 |
...
34. async function crlGetExtensionsObject() {
...
由于端侧系统时间不可信，证书链校验不包含对证书有效时间的校验。如果需要检查证书的时间有效性，可使用X509证书的[checkValidityWithDate](js-apis-cert.md#checkvaliditywithdate)方法进行检查。详见[证书规格](<../../../../harmonyos-guides/系统/安全/Device Certificate Kit（设备证书服务）/证书算法库框架/证书算法库框架概述/certificate-framework-overview.md#证书规格>)。
...
76. } catch (error) {
77. console.error(`decryptEnvelopedData failed: errCode: ${error.code}, errMsg: ${error.message}`);
78. }
79. }
```
