...
scraped_at: 2026-06-11T16:14:22+08:00
doc_updated_at: 2026-04-30
content_hash: sha256:87eace950c7f80002ad2fa5a9f8a3a1f1cf6d9b4c7d8029adb654f7929fb24e5
---
abilityConnectionManager模块提供了应用协同接口管理能力。设备组网成功（需登录同账号、双端打开蓝牙）后，系统应用和三方应用可以跨设备拉起同应用的一个[UIAbility](<../../../Ability Kit（程序框架服务）/ArkTS API/Stage模型能力的接口/@ohos.app.ability.UIAbility (带界面的应用组件)/js-apis-app-ability-uiability.md>)，拉起并连接成功后可实现跨设备数据传输（文本信息）。
...
| context | [Context](<../../../Ability Kit（程序框架服务）/ArkTS API/接口依赖的元素及定义/application/Context (Stage模型的上下文基类)/js-apis-inner-application-context.md>) | 是 | 表示应用上下文。 |
| peerInfo | [PeerInfo](js-apis-distributed-_62841863.md#peerinfo) | 是 | 对端的协同信息。 |
| connectOptions | [ConnectOptions](js-apis-distributed-_62841863.md#connectoptions) | 是 | 应用设置的连接选项。 |
...
| 类型 | 说明 |
| --- | --- |
| [PeerInfo](js-apis-distributed-_62841863.md#peerinfo) | undefined | 若存在对应PeerInfo，则返回接收端的协作应用信息。若sessionId未找到，则查询失败，返回undefined。 |
...
| 类型 | 说明 |
| --- | --- |
| Promise<ConnectResult> | 以Promise形式返回[连接结果](js-apis-distributed-_62841863.md#connectresult)。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| type | string | 是 | 事件回调类型，支持的事件为'connect'，完成[abilityConnectionManager.connect()](js-apis-distributed-_62841863.md#abilityconnectionmanagerconnect)调用，触发该事件。 |
| sessionId | number | 是 | 创建的协同会话ID。 |
| callback | Callback<[EventCallbackInfo](js-apis-distributed-_62841863.md#eventcallbackinfo)> | 是 | 注册的回调函数。 |
...
| sessionId | number | 是 | 创建的协同会话ID。 |
| callback | Callback<[EventCallbackInfo](js-apis-distributed-_62841863.md#eventcallbackinfo)> | 否 | 注册的回调函数。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| type | string | 是 | 事件回调类型，支持的事件为'disconnect'，完成[abilityConnectionManager.disconnect()](js-apis-distributed-_62841863.md#abilityconnectionmanagerdisconnect)调用，触发该事件。 |
| sessionId | number | 是 | 创建的协同会话ID。 |
| callback | Callback<[EventCallbackInfo](js-apis-distributed-_62841863.md#eventcallbackinfo)> | 是 | 注册的回调函数。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| type | string | 是 | 事件回调类型，支持的事件为'receiveMessage'，完成[abilityConnectionManager.sendMessage()](js-apis-distributed-_62841863.md#abilityconnectionmanagersendmessage)调用，触发该事件。 |
| sessionId | number | 是 | 创建的协同会话ID。 |
| callback | Callback<[EventCallbackInfo](js-apis-distributed-_62841863.md#eventcallbackinfo)> | 是 | 注册的回调函数。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| type | string | 是 | 事件回调类型，支持的事件为'receiveData'，完成[abilityConnectionManager.sendData()](js-apis-distributed-_62841863.md#abilityconnectionmanagersenddata)调用，触发该事件。 |
| sessionId | number | 是 | 创建的协同会话ID。 |
| callback | Callback<[EventCallbackInfo](js-apis-distributed-_62841863.md#eventcallbackinfo)> | 是 | 注册的回调函数。 |
...
应用连接成功后，设备A或设备B可向对端设备发送[ArrayBuffer](../../../../harmonyos-guides/应用框架/ArkTS（方舟编程语言）/ArkTS并发/并发线程间通信/线程间通信对象/ArrayBuffer对象/arraybuffer-object.md)字节流。
...
| --- | --- | --- | --- |
| sessionId | number | 是 | 协同会话ID。 |
| data | [ArrayBuffer](../../../../harmonyos-guides/应用框架/ArkTS（方舟编程语言）/ArkTS并发/并发线程间通信/线程间通信对象/ArrayBuffer对象/arraybuffer-object.md) | 是 | 字节流信息。 |
...
| startOptions | [StartOptionParams](js-apis-distributed-_62841863.md#startoptionparams) | 否 | 是 | 配置应用启动选项。 |
...
| errorCode | [ConnectErrorCode](js-apis-distributed-_62841863.md#connecterrorcode) | 否 | 是 | 表示连接错误码。 |
...
| reason | [DisconnectReason](js-apis-distributed-_62841863.md#disconnectreason) | 否 | 是 | 表示断连原因。 |
...
| 名称 | 类型 | 只读 | 可选 | 说明 |
| --- | --- | --- | --- | --- |
| eventType | [CollaborateEventType](js-apis-distributed-_62841863.md#collaborateeventtype) | 否 | 否 | 表示协同事件的类型。 |
