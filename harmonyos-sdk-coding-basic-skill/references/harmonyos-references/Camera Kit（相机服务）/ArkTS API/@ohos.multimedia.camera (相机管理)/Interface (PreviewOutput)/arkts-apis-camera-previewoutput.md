...
scraped_at: 2026-06-01T16:20:22+08:00
doc_updated_at: 2026-05-26
content_hash: sha256:cc50eec40c7d95695c6dc43282eeb9bf9d895eed624efc975efc2b97a8b98ca2
---
预览输出类。继承[CameraOutput](<../Interface (CameraOutput)/arkts-apis-camera-cameraoutput.md>)。
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| type | string | 是 | 监听事件，固定为'error'，previewOutput创建成功可监听。预览接口使用错误时触发该事件，比如调用[Session.start](<../Interface (Session)/arkts-apis-camera-session.md#start11-1>)，[CameraOutput.release](<../Interface (CameraOutput)/arkts-apis-camera-cameraoutput.md#release-1>)等接口发生错误时返回对应错误信息。 |
...
| 类型 | 说明 |
| --- | --- |
| Array<[FrameRateRange](<../Interfaces (其他)/arkts-apis-camera-i.md#frameraterange>)> | 支持的帧率范围列表。若接口调用失败，返回undefined。 |
...
仅在[PhotoSession](<../Interface (PhotoSession)/arkts-apis-camera-photosession.md>)或[VideoSession](<../Interface (VideoSession)/arkts-apis-camera-videosession.md>)模式下支持。
...
| 类型 | 说明 |
| --- | --- |
| [FrameRateRange](<../Interfaces (其他)/arkts-apis-camera-i.md#frameraterange>) | 帧率范围 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| displayRotation | number | 否 | 显示设备的屏幕旋转角度，通过[display.getDefaultDisplaySync](<../../../../ArkUI（方舟UI框架）/ArkTS API/屏幕管理/@ohos.display (屏幕属性)/js-apis-display.md#displaygetdefaultdisplaysync9>)获得。  从API version 23开始，入参displayRotation为可选参数，当不传入参数时，由系统获取displayRotation进行预览旋转角度计算。 |

**返回值：**

| 类型 | 说明 |
| --- | --- |
| [ImageRotation](../Enums/arkts-apis-camera-e.md#imagerotation) | 返回预览旋转角度。若接口调用失败，返回undefined。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| previewRotation | [ImageRotation](../Enums/arkts-apis-camera-e.md#imagerotation) | 是 | 预览旋转角度 |
| isDisplayLocked | boolean | 否 | Surface在屏幕旋转时是否锁定方向，未设置时默认取值为false，即不锁定方向。true表示锁定方向，false表示不锁定方向。详情请参考[SurfaceRotationOptions](../../../../ArkUI（方舟UI框架）/ArkTS组件/渲染绘制/XComponent/ts-basic-components-xcomponent.md#surfacerotationoptions12对象说明) |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| callback | AsyncCallback<void> | 是 | 回调函数。当开始输出预览流成功，err为undefined，否则为错误对象。错误码类型[CameraErrorCode](../Enums/arkts-apis-camera-e.md#cameraerrorcode)。 |
...
该接口只能在使用[Session.commitConfig](<../Interface (Session)/arkts-apis-camera-session.md#commitconfig11>)接口之前调用，否则会影响预览流出流格式。
...
配置延迟预览的Surface，可以在[commitConfig](<../Interface (Session)/arkts-apis-camera-session.md#commitconfig11-1>)配流和[start](<../Interface (Session)/arkts-apis-camera-session.md#start11-1>)启流之后运行。
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| surfaceId | string | 是 | 从[XComponent](../../../../ArkUI（方舟UI框架）/ArkTS组件/渲染绘制/XComponent/ts-basic-components-xcomponent.md)组件获取的surfaceId。 |
