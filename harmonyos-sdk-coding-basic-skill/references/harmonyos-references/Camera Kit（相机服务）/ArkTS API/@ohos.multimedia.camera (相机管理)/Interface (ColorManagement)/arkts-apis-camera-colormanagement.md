...
content_hash: sha256:c0e56a37b2f940efd9a857365879a22d2d490dccbdb06594ccdbbc70b76aee0d
---
ColorManagement继承自[ColorManagementQuery](<../Interface (ColorManagementQuery)/arkts-apis-camera-colormanagementquery.md>)。
...
使用该接口前，必须先通过[getSupportedColorSpaces](<../Interface (ColorManagementQuery)/arkts-apis-camera-colormanagementquery.md#getsupportedcolorspaces12>)获取当前设备所支持的ColorSpaces。该接口建议在[addOutput](<../Interface (Session)/arkts-apis-camera-session.md#addoutput11>)之后、[commitConfig](<../Interface (Session)/arkts-apis-camera-session.md#commitconfig11-1>)之前调用，如果在[commitConfig](<../Interface (Session)/arkts-apis-camera-session.md#commitconfig11-1>)之后调用该接口，会导致相机会话配置耗时增加。

P3广色域与HDR高动态范围成像：

应用可以下发不同的色彩空间（ColorSpace）参数来支持P3广色域以及HDR的功能。若应用不主动设置色彩空间，拍照、录像模式均默认为SDR拍摄。

应用针对不同模式使能HDR效果、设置的色彩空间以及设置相机输出流[Profile](<../Interfaces (其他)/arkts-apis-camera-i.md#profile>)中的[CameraFormat](../Enums/arkts-apis-camera-e.md#cameraformat)一一对应关系可参考下表。例如，在录像模式下若需要选择HDR拍摄，相机预览输出流和录像输出流[Profile](<../Interfaces (其他)/arkts-apis-camera-i.md#profile>)中的[CameraFormat](../Enums/arkts-apis-camera-e.md#cameraformat)可选择CAMERA\_FORMAT\_YCRCB\_P010，色彩空间ColorSpace可选择设置BT2020\_HLG\_LIMIT。
...
从API version 23开始，可以通过接口[getSupportedFullOutputCapability](<../Interface (CameraManager)/arkts-apis-camera-cameramanager.md#getsupportedfulloutputcapability23>)查询是否支持拍照模式下的预览P010格式。
...
* 若应用主动设置色彩空间，在拍照模式下，预览输出格式与色彩空间必须按照下列表格中的对应关系配置，若不满足则会在[setColorSpace](arkts-apis-camera-colormanagement.md#setcolorspace12)或[commitConfig](<../Interface (Session)/arkts-apis-camera-session.md#commitconfig11-1>)时返回错误码。
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| colorSpace | [colorSpaceManager.ColorSpace](<../../../../ArkGraphics 2D（方舟2D图形服务）/ArkTS API/@ohos.graphics.colorSpaceManager (色彩管理)/js-apis-colorspacemanager.md#colorspace>) | 是 | 色彩空间，通过[getSupportedColorSpaces](<../Interface (ColorManagementQuery)/arkts-apis-camera-colormanagementquery.md#getsupportedcolorspaces12>)接口获取。 |
...
| 类型 | 说明 |
| --- | --- |
| [colorSpaceManager.ColorSpace](<../../../../ArkGraphics 2D（方舟2D图形服务）/ArkTS API/@ohos.graphics.colorSpaceManager (色彩管理)/js-apis-colorspacemanager.md#colorspace>) | 当前设置的色彩空间。 |
