...
scraped_at: 2026-06-11T16:19:18+08:00
doc_updated_at: 2026-05-19
content_hash: sha256:efcaf090ff5303645747ac546c47fe97c9aa8ec836ccf318fa19dec80270c395
---
说明
...
* [StatusBarItem](../statusBarManager（状态栏管理服务）/statusbar-extension-manager.md#statusbaritem)中配置的hoverTips字符串长度为0或者大于128。
* 调用[updateStatusBarHoverTips](../statusBarManager（状态栏管理服务）/statusbar-extension-manager.md#statusbarmanagerupdatestatusbarhovertips)方法时传入的字符串长度为0或者大于128。
...
* 调用[updateStatusBarMenuItem](../statusBarManager（状态栏管理服务）/statusbar-extension-manager.md#statusbarmanagerupdatestatusbarmenuitem)时，若传入的item的menuCode未在状态栏的一级菜单项中预先定义，则会导致更新失败。
* 调用[updateStatusBarSubMenuItem](../statusBarManager（状态栏管理服务）/statusbar-extension-manager.md#statusbarmanagerupdatestatusbarsubmenuitem)时，若传入的item的menuCode未在状态栏的二级菜单项中预先定义，则会导致更新失败。
...
当前传入的某个一级菜单项既不包含[menuAction](../statusBarManager（状态栏管理服务）/statusbar-extension-manager.md#statusbarmenuitem)，也不包含[subMenu](../statusBarManager（状态栏管理服务）/statusbar-extension-manager.md#statusbarmenuitem)。
...
* 调用[addCustomCategory](../quickBarManager（快捷栏管理服务）/desktop-quickbar-extension-manager.md#quickbarmanageraddcustomcategory)方法时传入的字符串长度大于512或者内容为空。
* 调用[updateCustomCategory](../quickBarManager（快捷栏管理服务）/desktop-quickbar-extension-manager.md#quickbarmanagerupdatecustomcategory)方法时传入的字符串长度大于512或者内容为空。
* 调用[addQuickTask](../quickBarManager（快捷栏管理服务）/desktop-quickbar-extension-manager.md#quickbarmanageraddquicktask)方法时传入的字符串长度大于512或者内容为空。
* 调用[updateQuickTask](../quickBarManager（快捷栏管理服务）/desktop-quickbar-extension-manager.md#quickbarmanagerupdatequicktask)方法时传入的字符串长度大于512或者内容为空。
...
* 调用[addQuickTask](../quickBarManager（快捷栏管理服务）/desktop-quickbar-extension-manager.md#quickbarmanageraddquicktask)方法时传入的parameters参数的数组大小超过64。
* 调用[updateQuickTask](../quickBarManager（快捷栏管理服务）/desktop-quickbar-extension-manager.md#quickbarmanagerupdatequicktask)方法时传入的parameters参数的数组大小超过64。
