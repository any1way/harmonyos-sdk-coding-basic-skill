...
scraped_at: 2026-06-11T15:40:48+08:00
doc_updated_at: 2026-06-05
content_hash: sha256:a6ef61f205956f753a8e9dda0ebf57bc7b252e1f663e5afedb95b210d0a6bce3
---
...
任务池API返回错误码。如需了解各错误码的详细信息，请参阅文档[语言基础类库错误码](../../错误码/语言基础类库错误码/errorcode-utils.md)。

请查阅[TaskPool注意事项](../../../../harmonyos-guides/应用框架/ArkTS（方舟编程语言）/ArkTS并发/多线程并发/TaskPool简介/taskpool-introduction.md#taskpool注意事项)，了解使用TaskPool时的相关注意点。
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| func | Function | 是 | 执行的逻辑需要传入一个函数，该函数必须使用[@Concurrent装饰器](../../../../harmonyos-guides/应用框架/ArkTS（方舟编程语言）/ArkTS并发/多线程并发/TaskPool简介/taskpool-introduction.md#concurrent装饰器)装饰。支持的函数返回值类型请参考[序列化支持类型](js-apis-taskpool.md#序列化支持类型)。 |
...
以下错误码的详细介绍请参见[通用错误码](../../../通用错误码/errorcode-universal.md)和[语言基础类库错误码](../../错误码/语言基础类库错误码/errorcode-utils.md)。
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| func | (...args: A) => R | Promise<R> | 是 | 执行的逻辑需要传入函数，必须使用[@Concurrent装饰器](../../../../harmonyos-guides/应用框架/ArkTS（方舟编程语言）/ArkTS并发/多线程并发/TaskPool简介/taskpool-introduction.md#concurrent装饰器)装饰，支持的函数返回值类型请参考[序列化支持类型](js-apis-taskpool.md#序列化支持类型)。 |
...
20. console.info("Succeeded in executing task, res is:" + res);
21. });
22. taskpool.execute(taskGroup2).then((res: Array<Object>) => {
23. console.info("Succeeded in executing task, res is:" + res);
24. });
```
...
以下错误码的详细介绍，请参见[通用错误码](../../../通用错误码/errorcode-universal.md)和[语言基础类库错误码](../../错误码/语言基础类库错误码/errorcode-utils.md)。
...
31. console.info(`Succeeded in executing task. result: ` + res);
...
19. console.info(`Succeeded in executing task. res is: ` + res);
20. });
21. taskpool.execute(taskGroup2).then((res: Array<Object>) => {
22. console.info(`Succeeded in executing task. res is: ` + res);
23. }).catch((err: BusinessError) => {
24. console.error(`Failed to execute task. Code: ${err.code}, message: ${err.message}`);
25. });
26. setTimeout(() => {
27. try {
...
29. console.error(`Failed to execute task. Code: ${err.code}, message: ${err.message}`);
30. });
31. setTimeout(() => {
...
| 类型 | 说明 |
| --- | --- |
| boolean | 如果被检查函数标注了[@Concurrent装饰器](../../../../harmonyos-guides/应用框架/ArkTS（方舟编程语言）/ArkTS并发/多线程并发/TaskPool简介/taskpool-introduction.md#concurrent装饰器)，则返回true，否则返回false。 |
...
表示所创建任务（Task）执行时的优先级。工作线程优先级跟随任务优先级更新，对应关系参考[QoS等级定义](<../../../../harmonyos-guides/系统/基础功能/Kernel Enhance Kit（内核增强能力）/QoS 开发指导/qos-guidelines.md#qos等级定义>)。
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| func | Function | 是 | 执行的逻辑需要传入函数，必须使用[@Concurrent装饰器](../../../../harmonyos-guides/应用框架/ArkTS（方舟编程语言）/ArkTS并发/多线程并发/TaskPool简介/taskpool-introduction.md#concurrent装饰器)装饰，支持的函数返回值类型请参考[序列化支持类型](js-apis-taskpool.md#序列化支持类型)。 |
...
需搭配[@Sendable装饰器](../../../../harmonyos-guides/应用框架/ArkTS（方舟编程语言）/ArkTS并发/并发线程间通信/线程间通信对象/Sendable对象/Sendable对象简介/arkts-sendable.md#sendable装饰器)使用，否则会抛异常。建议开发者使用该装饰器以避免异常。
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| cloneList | Object[] | ArrayBuffer[] | 是 | - 传入数组的类型必须为[Sendable支持的数据类型](../../../../harmonyos-guides/应用框架/ArkTS（方舟编程语言）/ArkTS并发/并发线程间通信/线程间通信对象/Sendable对象/Sendable对象简介/arkts-sendable.md#sendable支持的数据类型)或ArrayBuffer。  - 所有传入cloneList的对象持有的[Sendable class](../../../../harmonyos-guides/应用框架/ArkTS（方舟编程语言）/ArkTS并发/并发线程间通信/线程间通信对象/Sendable对象/Sendable对象简介/arkts-sendable.md#sendable-class)实例或ArrayBuffer类型对象，在线程间传输的行为都会变成拷贝传递，即修改传输后的对象不会对原有对象产生任何影响。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| func | Function | 是 | 需要传入使用[@Concurrent装饰器](../../../../harmonyos-guides/应用框架/ArkTS（方舟编程语言）/ArkTS并发/多线程并发/TaskPool简介/taskpool-introduction.md#concurrent装饰器)装饰的函数。支持的返回值类型请参考[序列化支持类型](js-apis-taskpool.md#序列化支持类型)。 |
...
序列化支持类型参考[线程间通信对象概述](../../../../harmonyos-guides/应用框架/ArkTS（方舟编程语言）/ArkTS并发/并发线程间通信/线程间通信对象/线程间通信对象概述/serializable-overview.md)里的介绍。
...
14. console.info("Succeeded in executing task, result: " + result);
...
21. console.info("Succeeded in executing task, result: " + result);
22. }).catch((e: BusinessError) => {
...
27. console.info("Succeeded in executing task, result: " + res);
