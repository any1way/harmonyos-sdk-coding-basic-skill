...
scraped_at: 2026-06-01T15:36:23+08:00
doc_updated_at: 2026-04-20
content_hash: sha256:6f0074683814d9ef28517800064c6888413e4784ac0526afb9cfc29bf542ef37
---
...
| callback | Callback<[emitter.EventData](<../../../基础功能/Basic Services Kit（基础服务）/ArkTS API/进程线程通信/@ohos.events.emitter (Emitter)/js-apis-emitter.md#eventdata>)> | 是 | 回调函数，返回事件传输的数据。 |
...
| --- | --- | --- | --- |
| event | string | 是 | 事件回调类型，支持的事件包括：'close' | 'drain' |'error' | 'finish' 。  - 'close'：完成[end()](js-apis-stream.md#end)调用，结束写入操作，触发该事件。  - 'drain'：在可写流缓冲区中数据清空时触发该事件。  - 'error'：在可写流发生异常时触发该事件。  - 'finish'：在数据缓冲区全部写入到目标后触发该事件。 |
| callback | Callback<[emitter.EventData](<../../../基础功能/Basic Services Kit（基础服务）/ArkTS API/进程线程通信/@ohos.events.emitter (Emitter)/js-apis-emitter.md#eventdata>)> | 否 | 回调函数。 |
...
| callback | Callback<[emitter.EventData](<../../../基础功能/Basic Services Kit（基础服务）/ArkTS API/进程线程通信/@ohos.events.emitter (Emitter)/js-apis-emitter.md#eventdata>)> | 是 | 回调函数，返回事件数据。 |
