...
scraped_at: 2026-06-01T16:24:09+08:00
doc_updated_at: 2026-04-20
content_hash: sha256:1172ea21202cc9facc864fd03e3488d4c7dde4b1624baead8ec606b0bc41c707
---
说明

从API version 6开始支持，从API version 9开始废弃，建议使用[AVPlayer](<../Interface (AVPlayer)/arkts-apis-media-avplayer.md>)替代。

音频播放管理类，用于管理和播放音频媒体。在调用AudioPlayer的方法前，需要先通过[createAudioPlayer()](../Functions/arkts-apis-media-f.md#mediacreateaudioplayerdeprecated)构建一个AudioPlayer实例。
...
| audioInterruptMode9+ | [audio.InterruptMode](<../../../../Audio Kit（音频服务）/ArkTS API/@ohos.multimedia.audio (音频管理)/Enums/arkts-apis-audio-e.md#interruptmode9>) | 否 | 是 | 音频焦点模型。 |
| currentTime | number | 是 | 否 | 音频的当前播放位置，单位为毫秒（ms）。 |
| duration | number | 是 | 否 | 音频时长，单位为毫秒（ms）。 |
| state | [AudioState](../Types/arkts-apis-media-t.md#audiostatedeprecated) | 是 | 否 | 可以查询音频播放的状态，该状态不可作为调用play/pause/stop等状态切换的触发条件。 |
...
从API version 6开始支持，从API version 9开始废弃，建议使用[AVPlayer.play](<../Interface (AVPlayer)/arkts-apis-media-avplayer.md#play9>)替代。
...
从API version 6开始支持，从API version 9开始废弃，建议使用[AVPlayer.pause](<../Interface (AVPlayer)/arkts-apis-media-avplayer.md#pause9>)替代。
...
从API version 6开始支持，从API version 9开始废弃，建议使用[AVPlayer.stop](<../Interface (AVPlayer)/arkts-apis-media-avplayer.md#stop9>)替代。
...
从API version 7开始支持，从API version 9开始废弃，建议使用[AVPlayer.reset](<../Interface (AVPlayer)/arkts-apis-media-avplayer.md#reset9>)替代。
...
从API version 6开始支持，从API version 9开始废弃，建议使用[AVPlayer.seek](<../Interface (AVPlayer)/arkts-apis-media-avplayer.md#seek9>)替代。
...
从API version 6开始支持，从API version 9开始废弃，建议使用[AVPlayer.setVolume](<../Interface (AVPlayer)/arkts-apis-media-avplayer.md#setvolume9>)替代。
...
从API version 6开始支持，从API version 9开始废弃，建议使用[AVPlayer.release](<../Interface (AVPlayer)/arkts-apis-media-avplayer.md#release9>)替代。
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| callback | AsyncCallback<Array<[MediaDescription](<../Interfaces (其他)/arkts-apis-media-i.md#mediadescription8>)>> | 是 | 回调函数。获取音频轨道信息成功时，err为undefined，data为获取到的MediaDescription数组，否则为错误对象。 |
...
| 类型 | 说明 |
| --- | --- |
| Promise<Array<[MediaDescription](<../Interfaces (其他)/arkts-apis-media-i.md#mediadescription8>)>> | 音频轨道信息MediaDescription数组Promise返回值。 |
...
| callback | function | 是 | 音频缓存事件回调方法。  [BufferingInfoType](../Enums/arkts-apis-media-e.md#bufferinginfotype8)value值固定为0。 |
...
从API version 6开始支持，从API version 9开始废弃，建议使用[AVPlayer.on('stateChange')](<../Interface (AVPlayer)/arkts-apis-media-avplayer.md#onstatechange9>)替代。
...
从API version 6开始支持，从API version 9开始废弃，建议使用[AVPlayer.on('timeUpdate')](<../Interface (AVPlayer)/arkts-apis-media-avplayer.md#ontimeupdate9>)替代。
...
从API version 6开始支持，从API version 9开始废弃，建议使用[AVPlayer.on('error')](<../Interface (AVPlayer)/arkts-apis-media-avplayer.md#onerror9>)替代。
