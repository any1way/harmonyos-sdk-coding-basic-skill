...
scraped_at: 2026-06-01T16:24:14+08:00
doc_updated_at: 2026-04-24
content_hash: sha256:008d2c5c065e6860f59a6f32f9dba5875fac26cbb48ef0e054e2484533d92943
---
音频池提供了短音频的加载、播放、音量设置、循环设置、停止播放、资源卸载等功能。

SoundPool需要和@ohos.multimedia.media配合使用，需要先通过[media.createSoundPool](<../../@ohos.multimedia.media (媒体服务)/Functions/arkts-apis-media-f.md#mediacreatesoundpool10>)完成音频池实例的创建。
...
| rate | number | 否 | 是 | 设置音频播放的倍速，具体倍速范围参照[AudioRendererRate](<../../../../Audio Kit（音频服务）/ArkTS API/@ohos.multimedia.audio (音频管理)/Enums/arkts-apis-audio-e.md#audiorendererrate8>)。默认值：0。 |
...
| 名称 | 类型 | 只读 | 可选 | 说明 |
| --- | --- | --- | --- | --- |
| errorCode | T | 否 | 否 | 错误码。errorCode的类型T为[BusinessError](<../../../../基础功能/Basic Services Kit（基础服务）/ArkTS API/其他/@ohos.base (公共回调信息)/js-apis-base.md#businesserror>)类型。 |
...
音频池提供了系统声音的加载、播放、音量设置、循环设置、停止播放和资源卸载等功能，在调用SoundPool的接口前，需要先通过[media.createSoundPool](<../../@ohos.multimedia.media (媒体服务)/Functions/arkts-apis-media-f.md#mediacreatesoundpool10>)创建实例。
...
* SoundPool目前不支持后台播放、设置音频打断等音频焦点策略和跳过音频头尾的静音帧。SoundPool低时延播放可参考[使用SoundPool播放短音频(ArkTS)](<../../../../../harmonyos-guides/媒体/Media Kit（媒体服务）/媒体开发指导(ArkTS)/播放/使用SoundPool播放短音频(ArkTS)/using-soundpool-for-playback.md>)。
...
| --- | --- | --- | --- |
| streamID | number | 是 | 音频流ID，通过play方法获取。 |
| rate | [audio.AudioRendererRate](<../../../../Audio Kit（音频服务）/ArkTS API/@ohos.multimedia.audio (音频管理)/Enums/arkts-apis-audio-e.md#audiorendererrate8>) | 是 | 音频rate相关参数。 |
...
设置同一ID音频在播放时的打断模式。创建soundPool之后，该接口仅在首次调用soundPool的Play函数之前设置有效，期间可多次设置，否则将默认使用[SAME\_SOUND\_INTERRUPT](<../../@ohos.multimedia.media (媒体服务)/Enums/arkts-apis-media-e.md#soundinterruptmode23>)，即对同一ID的音频，如果前者尚未播放完成，后者在播放前会先打断前者的播放。
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| interruptMode | [media.SoundInterruptMode](<../../@ohos.multimedia.media (媒体服务)/Enums/arkts-apis-media-e.md#soundinterruptmode23>) | 是 | 同一ID音频在播放时的打断模式，通过media.SoundInterruptMode枚举获取。 |
