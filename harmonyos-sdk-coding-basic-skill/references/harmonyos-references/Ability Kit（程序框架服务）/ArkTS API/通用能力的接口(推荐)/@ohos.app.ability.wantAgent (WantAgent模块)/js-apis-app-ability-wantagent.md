...
scraped_at: 2026-06-01T15:30:46+08:00
doc_updated_at: 2026-04-28
content_hash: sha256:06e98088adc87d5e3d9b935ad552b5e63ff387d075918e20c8c0587e4e8e33e7
---
WantAgent模块封装了[Want](<../@ohos.app.ability.Want (Want)/js-apis-app-ability-want.md>)对象，允许应用程序在未来的某个时间点触发WantAgent实例执行指定操作（如启动Ability、发送公共事件等）。

该模块提供了创建WantAgent实例、获取WantAgent实例所属应用的包名、获取WantAgent实例所属应用的UID、主动触发WantAgent实例、判断两个WantAgent实例是否相等等功能。WantAgent的一个典型应用场景是通知处理。例如，当用户点击通知时，会触发WantAgent的[trigger](js-apis-app-ability-wantagent.md#wantagenttrigger)接口，并拉起目标应用。具体使用请参考[通知模块](<../../../../../harmonyos-guides/应用服务/Notification Kit（用户通知服务）/发布通知/为通知添加行为意图/notification-with-wantagent.md>)。
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| info | [WantAgentInfo](../../接口依赖的元素及定义/wantAgent/WantAgentInfo/js-apis-inner-wantagent-wantagentinfo.md) | 是 | 表示创建WantAgent所需的配置信息，包括目标UIAbility、操作类型、请求码等。三方应用在WantAgentInfo中只能设置本应用的UIAbility。 |
...
这里所执行的操作类型，是在创建WantAgent实例时通过[WantAgentInfo](../../接口依赖的元素及定义/wantAgent/WantAgentInfo/js-apis-inner-wantagent-wantagentinfo.md)参数中的actionType属性指定的。
...
| --- | --- | --- | --- |
| agent | WantAgent | 是 | WantAgent对象。 |
| triggerInfo | [TriggerInfo](../../接口依赖的元素及定义/wantAgent/TriggerInfo/js-apis-inner-wantagent-triggerinfo.md) | 是 | 表示触发WantAgent实例时携带的信息，如自定义的extraInfos。 |
...
当两个WantAgent实例由当前用户下的同一应用使用相同的[WantAgentInfo](../../接口依赖的元素及定义/wantAgent/WantAgentInfo/js-apis-inner-wantagent-wantagentinfo.md)信息创建，并且实例未被[cancel](js-apis-app-ability-wantagent.md#wantagentcancel)取消，这两个实例相等。在通知（携带WantAgent实例）场景，通知更新时会比较2个通知中的WantAgent实例，不相等时会把旧通知的WantAgent实例删除。
...
| want | [Want](<../@ohos.app.ability.Want (Want)/js-apis-app-ability-want.md>) | 否 | 否 | 触发wantAgent时实际使用的want信息。 |
...
| 类型 | 说明 |
| --- | --- |
| [\_TriggerInfo](../../接口依赖的元素及定义/wantAgent/TriggerInfo/js-apis-inner-wantagent-triggerinfo.md) | TriggerInfo对象。 |
...
| 类型 | 说明 |
| --- | --- |
| [\_WantAgentInfo](../../接口依赖的元素及定义/wantAgent/WantAgentInfo/js-apis-inner-wantagent-wantagentinfo.md) | WantAgentInfo对象。 |
