...
scraped_at: 2026-06-01T15:29:59+08:00
doc_updated_at: 2026-05-14
content_hash: sha256:889dabe4cd322b13e93349cf682510dc2e3b0202017278d9cd994c87dffbb684
---
sendableContextManager模块提供Context与[SendableContext](../../接口依赖的元素及定义/application/SendableContext/js-apis-inner-application-sendablecontext.md)相互转换的能力。
...
例如，从主线程向子线程（如TaskPool或Worker工作线程）传递Sendable数据（符合[Sendable协议](../../../../../harmonyos-guides/应用框架/ArkTS（方舟编程语言）/ArkTS并发/并发线程间通信/线程间通信对象/Sendable对象/Sendable对象简介/arkts-sendable.md#sendable协议)的数据）时，需要通过Context与SendableContext之间的相互转换来实现。过程如下：

* 主线程向子线程传递Sendable数据时，需要将Context转换为SendableContext。
* 子线程使用Sendable数据时，需要将SendableContext转换为Context。

这里的Context与[createModuleContext](<../@ohos.app.ability.application (应用工具类)/js-apis-app-ability-application.md#applicationcreatemodulecontext>)方法创建的Context不同，具体差异如下：

* 与SendableContext相互转换的Context：ArkTS并发实例持有的应用侧Context是不同的实例，底层对应同一个Context对象。当一个实例中Context属性和方法被修改时，相关实例中的Context属性和方法将会同步修改。其中，Context实例中的eventHub属性比较特殊，不同实例中的eventHub是独立的对象，不支持跨ArkTS实例使用。如果需要使用[EventHub](../../接口依赖的元素及定义/application/EventHub/js-apis-inner-application-eventhub.md)跨实例传递数据，可以通过[setEventHubMultithreadingEnabled](js-apis-app-ability-sendablecontextmanager.md#sendablecontextmanagerseteventhubmultithreadingenabled20)启用跨线程数据传递功能。
* 通过[createModuleContext](<../@ohos.app.ability.application (应用工具类)/js-apis-app-ability-application.md#applicationcreatemodulecontext>)创建的Context：ArkTS并发实例持有的应用侧Context是不同的实例，底层对应不同的Context对象。

## 约束限制

PhonePC/2in1TabletTVWearable

“Context转换为SendableContext”和“SendableContext转换为Context”两个环节中的Context类型必须保持一致。例如，主线程使用[convertFromContext](js-apis-app-ability-sendablecontextmanager.md#sendablecontextmanagerconvertfromcontext)将[UIAbilityContext](../../接口依赖的元素及定义/application/UIAbilityContext/js-apis-inner-application-uiabilitycontext.md)转换为SendableContext，子线程收到该SendableContext之后，需要通过[convertToUIAbilityContext](js-apis-app-ability-sendablecontextmanager.md#sendablecontextmanagerconverttouiabilitycontext)将SendableContext转换为[UIAbilityContext](../../接口依赖的元素及定义/application/UIAbilityContext/js-apis-inner-application-uiabilitycontext.md)。

目前支持转换的Context包括[Context](<../../接口依赖的元素及定义/application/Context (Stage模型的上下文基类)/js-apis-inner-application-context.md>)、[ApplicationContext](<../../接口依赖的元素及定义/application/ApplicationContext (应用上下文)/js-apis-inner-application-applicationcontext.md>)、[AbilityStageContext](../../接口依赖的元素及定义/application/AbilityStageContext/js-apis-inner-application-abilitystagecontext.md)、[UIAbilityContext](../../接口依赖的元素及定义/application/UIAbilityContext/js-apis-inner-application-uiabilitycontext.md)。
...
Sendable上下文，符合[Sendable协议](../../../../../harmonyos-guides/应用框架/ArkTS（方舟编程语言）/ArkTS并发/并发线程间通信/线程间通信对象/Sendable对象/Sendable对象简介/arkts-sendable.md#sendable协议)，继承自[lang.ISendable](<../../../../ArkTS（方舟编程语言）/ArkTS API/@arkts.lang (ArkTS语言基础能力)/js-apis-arkts-lang.md#langisendable>)。
...
| 类型 | 说明 |
| --- | --- |
| [\_SendableContext](../../接口依赖的元素及定义/application/SendableContext/js-apis-inner-application-sendablecontext.md) | 表示Sendable上下文，可以与Context对象相互转换，用于ArkTS并发实例间（包括主线程、TaskPool&Worker工作线程）的数据传递。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| context | [common.Context](<../../接口依赖的元素及定义/application/Context (Stage模型的上下文基类)/js-apis-inner-application-context.md>) | 是 | Context对象。支持Context基类，[ApplicationContext](<../../接口依赖的元素及定义/application/ApplicationContext (应用上下文)/js-apis-inner-application-applicationcontext.md>)、[AbilityStageContext](../../接口依赖的元素及定义/application/AbilityStageContext/js-apis-inner-application-abilitystagecontext.md)和[UIAbilityContext](../../接口依赖的元素及定义/application/UIAbilityContext/js-apis-inner-application-uiabilitycontext.md)子类。 |

**返回值：**

| 类型 | 说明 |
| --- | --- |
| SendableContext | [SendableContext](../../接口依赖的元素及定义/application/SendableContext/js-apis-inner-application-sendablecontext.md)对象。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| sendableContext | [SendableContext](../../接口依赖的元素及定义/application/SendableContext/js-apis-inner-application-sendablecontext.md) | 是 | SendableContext对象。 |

**返回值：**

| 类型 | 说明 |
| --- | --- |
| common.Context | [Context](<../../接口依赖的元素及定义/application/Context (Stage模型的上下文基类)/js-apis-inner-application-context.md>)对象。 |
...
| 类型 | 说明 |
| --- | --- |
| common.ApplicationContext | [ApplicationContext](<../../接口依赖的元素及定义/application/ApplicationContext (应用上下文)/js-apis-inner-application-applicationcontext.md>)对象。 |
...
| 类型 | 说明 |
| --- | --- |
| common.AbilityStageContext | [AbilityStageContext](../../接口依赖的元素及定义/application/AbilityStageContext/js-apis-inner-application-abilitystagecontext.md)对象。 |
...
```
1. import { AbilityStage, sendableContextManager } from '@kit.AbilityKit';
2. import { hilog } from '@kit.PerformanceAnalysisKit';
3. import { worker } from '@kit.ArkTS';
...
16. export default class MyAbilityStage extends AbilityStage {
17. worker: worker.ThreadWorker = new worker.ThreadWorker('entry/ets/workers/Worker.ets');
...
| 类型 | 说明 |
| --- | --- |
| common.UIAbilityContext | [UIAbilityContext](../../接口依赖的元素及定义/application/UIAbilityContext/js-apis-inner-application-uiabilitycontext.md)对象。 |
...
设置[Context](<../../接口依赖的元素及定义/application/Context (Stage模型的上下文基类)/js-apis-inner-application-context.md>)中的[EventHub](../../接口依赖的元素及定义/application/EventHub/js-apis-inner-application-eventhub.md)是否启用跨线程通信能力。
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| context | [common.Context](<../../接口依赖的元素及定义/application/Context (Stage模型的上下文基类)/js-apis-inner-application-context.md>) | 是 | Context对象。其中，EventHub支持传递的序列化数据类型参见[序列化支持类型](<../../../../ArkTS（方舟编程语言）/ArkTS API/@ohos.taskpool (启动任务池)/js-apis-taskpool.md#序列化支持类型>)，数据大小不超过16MB。 |
...
主线程启用[Context](<../../接口依赖的元素及定义/application/Context (Stage模型的上下文基类)/js-apis-inner-application-context.md>)中[EventHub](../../接口依赖的元素及定义/application/EventHub/js-apis-inner-application-eventhub.md)的跨线程通信能力，并将Context转换为[SendableContext](../../接口依赖的元素及定义/application/SendableContext/js-apis-inner-application-sendablecontext.md)后发送到[Worker](<../../../../ArkTS（方舟编程语言）/ArkTS API/@ohos.worker (启动一个Worker)/js-apis-worker.md>)线程。
...
[Worker](<../../../../ArkTS（方舟编程语言）/ArkTS API/@ohos.worker (启动一个Worker)/js-apis-worker.md>)线程接收到[SendableContext](../../接口依赖的元素及定义/application/SendableContext/js-apis-inner-application-sendablecontext.md)后，将其转换为[Context](<../../接口依赖的元素及定义/application/Context (Stage模型的上下文基类)/js-apis-inner-application-context.md>)。然后，在Worker线程内，启用Context中[EventHub](../../接口依赖的元素及定义/application/EventHub/js-apis-inner-application-eventhub.md)的跨线程通信能力，并通过该功能向主线程发送消息。
