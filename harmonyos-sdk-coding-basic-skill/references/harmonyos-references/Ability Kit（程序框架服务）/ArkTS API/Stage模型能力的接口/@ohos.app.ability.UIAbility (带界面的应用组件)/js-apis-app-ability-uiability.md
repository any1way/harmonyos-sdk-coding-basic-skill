...
scraped_at: 2026-06-11T15:34:38+08:00
doc_updated_at: 2026-04-20
content_hash: sha256:fc791bb4958e11aa30ba4d5e378d881749efcc41ce29699cf1b5af6c311a4c02
---
UIAbility是包含UI界面的应用组件，继承自[Ability](<../@ohos.app.ability.Ability (Ability基类)/js-apis-app-ability-ability.md>)，提供UIAbility组件创建、销毁、前后台切换等[生命周期](js-apis-app-ability-uiability.md#uiability生命周期状态)回调，同时也具备[后台通信能力](js-apis-app-ability-uiability.md#后台通信能力)。
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/3b/v3/3ubPDBdzS_u6x5iVJhYlxg/zh-cn_image_0000002622859299.png?HW-CC-KV=V1&HW-CC-Date=20260611T073436Z&HW-CC-Expire=86400&HW-CC-Sign=3C41A6FCBA127DEF7433AD62183F4A1476A8F5B1672FA8D19D42D1647B8016C9)
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/2b/v3/SX1YHHhVQ5yLQOQ7E3YbYw/zh-cn_image_0000002622857307.png?HW-CC-KV=V1&HW-CC-Date=20260611T073436Z&HW-CC-Expire=86400&HW-CC-Sign=1C570BA2A432D42C21B57F230AC80D3A5ACD3B9CA028B41F73A412ED3D4E966B)

* Caller UIAbility调用[startAbilityByCall()](../../接口依赖的元素及定义/application/UIAbilityContext/js-apis-inner-application-uiabilitycontext.md#startabilitybycall)接口获取[Caller](js-apis-app-ability-uiability.md#caller)对象，并使用Caller对象的[call](js-apis-app-ability-uiability.md#call)方法向Callee UIAbility发送数据。
...
| 名称 | 类型 | 只读 | 可选 | 说明 |
| --- | --- | --- | --- | --- |
| context | [UIAbilityContext](../../接口依赖的元素及定义/application/UIAbilityContext/js-apis-inner-application-uiabilitycontext.md) | 否 | 否 | UIAbility的上下文。  **元服务API**：从API version 11开始，该接口支持在元服务中使用。 |
| launchWant | [Want](<../../通用能力的接口(推荐)/@ohos.app.ability.Want (Want)/js-apis-app-ability-want.md>) | 否 | 否 | UIAbility[冷启动](<../../../../../harmonyos-guides/应用框架/Ability Kit（程序框架服务）/Stage模型开发指导/Stage模型应用组件/UIAbility组件/启动应用内的UIAbility组件/uiability-intra-device-interaction.md#目标uiability冷启动>)时接收到的Want参数，取值为[onCreate](js-apis-app-ability-uiability.md#oncreate)接收到的Want参数。  **元服务API**：从API version 11开始，该接口支持在元服务中使用。 |
| lastRequestWant | [Want](<../../通用能力的接口(推荐)/@ohos.app.ability.Want (Want)/js-apis-app-ability-want.md>) | 否 | 否 | 最近一次拉起UIAbility请求的Want参数。  - 首次拉起UIAbility时，取值为[onCreate](js-apis-app-ability-uiability.md#oncreate)接收到的Want参数。  - 重复拉起UIAbility时，取值为[onNewWant](js-apis-app-ability-uiability.md#onnewwant)最近一次接收到的Want参数。  **元服务API**：从API version 11开始，该接口支持在元服务中使用。 |
...
| specifiedId23+ | string | 否 | 是 | 仅当UIAbility启动模式为[specified](<../../../../../harmonyos-guides/应用框架/Ability Kit（程序框架服务）/Stage模型开发指导/Stage模型应用组件/UIAbility组件/UIAbility组件启动模式/uiability-launch-type.md#specified启动模式>)时存在，取值为开发者自定义的UIAbility标识。 |
...
当UIAbility实例创建完成时，系统会触发该回调，开发者可在该回调中执行初始化逻辑（如定义变量、加载资源等）。该回调仅会在UIAbility[冷启动](<../../../../../harmonyos-guides/应用框架/Ability Kit（程序框架服务）/Stage模型开发指导/Stage模型应用组件/UIAbility组件/启动应用内的UIAbility组件/uiability-intra-device-interaction.md#目标uiability冷启动>)时触发。
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| want | [Want](<../../通用能力的接口(推荐)/@ohos.app.ability.Want (Want)/js-apis-app-ability-want.md>) | 是 | 调用方拉起该UIAbility时传递的数据。 |
| launchParam | [AbilityConstant.LaunchParam](<../@ohos.app.ability.AbilityConstant (Ability相关常量)/js-apis-app-ability-abilityconstant.md#launchparam>) | 是 | 应用启动参数，包含应用启动原因、应用上次退出原因等。 |
...
当[WindowStage](<../../../../ArkUI（方舟UI框架）/ArkTS API/窗口管理/@ohos.window (窗口)/Interface (WindowStage)/arkts-apis-window-windowstage.md>)实例创建完成后，系统会触发该回调。开发者可以在该回调中通过WindowStage加载页面。
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| windowStage | [window.WindowStage](<../../../../ArkUI（方舟UI框架）/ArkTS API/窗口管理/@ohos.window (窗口)/Interface (WindowStage)/arkts-apis-window-windowstage.md>) | 是 | WindowStage实例对象。 |
...
在应用迁移启动时，无论是[冷启动](<../../../../../harmonyos-guides/应用框架/Ability Kit（程序框架服务）/Stage模型开发指导/Stage模型应用组件/UIAbility组件/启动应用内的UIAbility组件/uiability-intra-device-interaction.md#目标uiability冷启动>)还是[热启动](<../../../../../harmonyos-guides/应用框架/Ability Kit（程序框架服务）/Stage模型开发指导/Stage模型应用组件/UIAbility组件/启动应用内的UIAbility组件/uiability-intra-device-interaction.md#目标uiability热启动>)，都会在执行完[onCreate()](js-apis-app-ability-uiability.md#oncreate)/[onNewWant()](js-apis-app-ability-uiability.md#onnewwant)后，触发onWindowStageRestore()生命周期函数，不执行onWindowStageCreate()生命周期函数。
...
当UIAbility被销毁（例如使用[terminateSelf](../../接口依赖的元素及定义/application/UIAbilityContext/js-apis-inner-application-uiabilitycontext.md#terminateself)接口停止UIAbility）时，系统触发该回调。开发者可以在该生命周期中执行资源清理、数据保存等相关操作。
...
| 类型 | 说明 |
| --- | --- |
| [AbilityConstant.OnContinueResult](<../@ohos.app.ability.AbilityConstant (Ability相关常量)/js-apis-app-ability-abilityconstant.md#oncontinueresult>) | Promise<[AbilityConstant.OnContinueResult](<../@ohos.app.ability.AbilityConstant (Ability相关常量)/js-apis-app-ability-abilityconstant.md#oncontinueresult>)> | 表示是否同意迁移的结果：  - AGREE：表示同意。  - REJECT：表示拒绝，如应用在onContinue中异常可以返回REJECT。  - MISMATCH：表示版本不匹配，接续源端应用可以在onContinue中获取到迁移对端应用的版本号，进行协商后，如果版本不匹配导致无法迁移，可以返回该结果。  该回调与onWindowStageRestore成对出现。在接续场景下，源端的UIAbility触发onContinue保存自定义数据，在目标端UIAbility触发onWindowStageRestore恢复自定义数据。 |
...
当已经启动的UIAbility实例再次被拉起时，系统会触发该回调。若在特定场景下（参见[Scenarios](<../@ohos.app.ability.contextConstant (Context相关常量)/js-apis-app-ability-contextconstant.md#scenarios20>)），不需要触发该生命周期回调，可以使用[setOnNewWantSkipScenarios](../../接口依赖的元素及定义/application/UIAbilityContext/js-apis-inner-application-uiabilitycontext.md#setonnewwantskipscenarios20)接口设置。
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| want | [Want](<../../通用能力的接口(推荐)/@ohos.app.ability.Want (Want)/js-apis-app-ability-want.md>) | 是 | 调用方再次拉起该UIAbility时传递的数据。 |
| launchParam | [AbilityConstant.LaunchParam](<../@ohos.app.ability.AbilityConstant (Ability相关常量)/js-apis-app-ability-abilityconstant.md#launchparam>) | 是 | UIAbility启动参数，包含启动原因等。 |
...
该接口需要与[appRecovery](<../../通用能力的接口(推荐)/@ohos.app.ability.appRecovery (应用故障恢复)/js-apis-app-ability-apprecovery.md>)配合使用。如果应用已使能故障恢复功能（即[enableAppRecovery](<../../通用能力的接口(推荐)/@ohos.app.ability.appRecovery (应用故障恢复)/js-apis-app-ability-apprecovery.md#apprecoveryenableapprecovery>)接口中的saveOccasion参数设置为SAVE\_WHEN\_ERROR），当应用出现故障时，系统将触发该回调来保存UIAbility的数据。
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| reason | [AbilityConstant.StateType](<../@ohos.app.ability.AbilityConstant (Ability相关常量)/js-apis-app-ability-abilityconstant.md#statetype>) | 是 | 触发应用保存状态的原因，当前仅支持APP\_RECOVERY（即应用故障恢复场景）。 |
...
| 类型 | 说明 |
| --- | --- |
| [AbilityConstant.OnSaveResult](<../@ohos.app.ability.AbilityConstant (Ability相关常量)/js-apis-app-ability-abilityconstant.md#onsaveresult>) | 返回一个数据保存策略的对象（如全部拒绝、全部允许、只允许故障恢复场景等）。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| stateType | [AbilityConstant.StateType](<../@ohos.app.ability.AbilityConstant (Ability相关常量)/js-apis-app-ability-abilityconstant.md#statetype>) | 是 | 触发应用保存状态的原因，当前仅支持APP\_RECOVERY（即应用故障恢复场景）。 |
| wantParam | Record<string, Object> | 是 | 用户自定义的应用状态数据，应用再启动时被保存在[onCreate](js-apis-app-ability-uiability.md#oncreate)的Want.parameters中。 |
...
| 类型 | 说明 |
| --- | --- |
| Promise<[AbilityConstant.OnSaveResult](<../@ohos.app.ability.AbilityConstant (Ability相关常量)/js-apis-app-ability-abilityconstant.md#onsaveresult>)> | Promise对象。返回一个数据保存策略的对象（如全部拒绝、全部允许、只允许故障恢复场景等）。 |
...
在UIAbility即将关闭前（例如用户通过点击应用窗口右上角的关闭按钮、或者通过Dock栏/托盘右键退出应用时），系统会触发该回调，用于在UIAbility正式关闭前执行其他操作。开发者可以在该回调中返回true阻拦此次关闭，然后在合适时机主动调用[terminateSelf](../../接口依赖的元素及定义/application/UIAbilityContext/js-apis-inner-application-uiabilitycontext.md#terminateself)接口关闭。例如，询问用户是否确认关闭UIAbility，再主动销毁UIAbility。

说明

* 从API version 15开始，当[UIAbility.onPrepareToTerminateAsync](js-apis-app-ability-uiability.md#onpreparetoterminateasync15)实现时，本回调函数将不执行。当[AbilityStage.onPrepareTerminationAsync](<../@ohos.app.ability.AbilityStage (AbilityStage组件管理器)/js-apis-app-ability-abilitystage.md#onprepareterminationasync15>)或[AbilityStage.onPrepareTermination](<../@ohos.app.ability.AbilityStage (AbilityStage组件管理器)/js-apis-app-ability-abilitystage.md#onpreparetermination15>)实现时，在dock栏或系统托盘处右键点击关闭，本回调函数将不执行。
* 如果应用本身或者所使用的三方框架注册了[window.WindowStage.on('windowStageClose')](<../../../../ArkUI（方舟UI框架）/ArkTS API/窗口管理/@ohos.window (窗口)/Interface (WindowStage)/arkts-apis-window-windowstage.md#onwindowstageclose14>)监听，本回调函数将不执行。
...
开发者可以在该回调中返回true阻拦此次关闭，然后在合适时机主动调用[terminateSelf](../../接口依赖的元素及定义/application/UIAbilityContext/js-apis-inner-application-uiabilitycontext.md#terminateself)接口关闭。例如，询问用户是否确认关闭UIAbility，再主动销毁UIAbility。

说明

* 当[AbilityStage.onPrepareTerminationAsync](<../@ohos.app.ability.AbilityStage (AbilityStage组件管理器)/js-apis-app-ability-abilitystage.md#onprepareterminationasync15>)或[AbilityStage.onPrepareTermination](<../@ohos.app.ability.AbilityStage (AbilityStage组件管理器)/js-apis-app-ability-abilitystage.md#onpreparetermination15>)实现时，在dock栏或系统托盘处右键点击关闭，本回调函数将不执行。
* 如果应用本身或者所使用的三方框架注册了[window.WindowStage.on('windowStageClose')](<../../../../ArkUI（方舟UI框架）/ArkTS API/窗口管理/@ohos.window (窗口)/Interface (WindowStage)/arkts-apis-window-windowstage.md#onwindowstageclose14>)监听，本回调函数将不执行。
...
* 该生命周期回调不支持[specified启动模式](<../../../../../harmonyos-guides/应用框架/Ability Kit（程序框架服务）/Stage模型开发指导/Stage模型应用组件/UIAbility组件/UIAbility组件启动模式/uiability-launch-type.md#specified启动模式>)。
* 通过[startAbility](../../接口依赖的元素及定义/application/UIAbilityContext/js-apis-inner-application-uiabilitycontext.md#startability)等方法拉起协同方应用时，需要在Want对象中设置协同标记[Flags](<../../通用能力的接口(推荐)/@ohos.app.ability.wantConstant (Want常量)/js-apis-app-ability-wantconstant.md#flags>)为FLAG\_ABILITY\_ON\_COLLABORATE。
* [冷启动](<../../../../../harmonyos-guides/应用框架/Ability Kit（程序框架服务）/Stage模型开发指导/Stage模型应用组件/UIAbility组件/启动应用内的UIAbility组件/uiability-intra-device-interaction.md#目标uiability冷启动>)时，该回调在[onForeground](js-apis-app-ability-uiability.md#onforeground)前或[onBackground](js-apis-app-ability-uiability.md#onbackground)后调用；[热启动](<../../../../../harmonyos-guides/应用框架/Ability Kit（程序框架服务）/Stage模型开发指导/Stage模型应用组件/UIAbility组件/启动应用内的UIAbility组件/uiability-intra-device-interaction.md#目标uiability热启动>)时，该回调在[onNewWant](js-apis-app-ability-uiability.md#onnewwant)前调用。
...
| 类型 | 说明 |
| --- | --- |
| [AbilityConstant.CollaborateResult](<../@ohos.app.ability.AbilityConstant (Ability相关常量)/js-apis-app-ability-abilityconstant.md#collaborateresult18>) | 协同方是否接受协同的结果。 |
...
调用方Caller UIAbility通过[startAbilityByCall](../../接口依赖的元素及定义/application/UIAbilityContext/js-apis-inner-application-uiabilitycontext.md#startabilitybycall)接口拉起目标Callee UIAbility，目标UIAbility启动成功后，返回一个Caller对象给调用方进行通信。
...
| data | [rpc.Parcelable](<../../../../IPC Kit（进程间通信服务）/ArkTS API/@ohos.rpc (RPC通信)/js-apis-rpc.md#parcelable9>) | 是 | 由Caller向Callee发送的消息内容，消息内容是序列化的数据。 |
...
| 类型 | 说明 |
| --- | --- |
| Promise<[rpc.MessageSequence](<../../../../IPC Kit（进程间通信服务）/ArkTS API/@ohos.rpc (RPC通信)/js-apis-rpc.md#messagesequence9>)> | Promise对象，返回Callee UIAbility的应答数据。 |
...
| --- | --- | --- | --- |
| method | string | 是 | 由Caller和Callee双方约定好的方法名，Callee方通过该字段区分消息类型。 |
| callback | [CalleeCallback](js-apis-app-ability-uiability.md#calleecallback) | 是 | 一个[rpc.MessageSequence](<../../../../IPC Kit（进程间通信服务）/ArkTS API/@ohos.rpc (RPC通信)/js-apis-rpc.md#messagesequence9>)类型入参的js通知同步回调函数, 回调函数至少要返回一个空的[rpc.Parcelable](<../../../../IPC Kit（进程间通信服务）/ArkTS API/@ohos.rpc (RPC通信)/js-apis-rpc.md#parcelable9>)数据对象, 其他视为函数执行错误。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| indata | [rpc.MessageSequence](<../../../../IPC Kit（进程间通信服务）/ArkTS API/@ohos.rpc (RPC通信)/js-apis-rpc.md#messagesequence9>) | 是 | 发送需传递的数据。 |

**返回值：**

| 类型 | 说明 |
| --- | --- |
| [rpc.Parcelable](<../../../../IPC Kit（进程间通信服务）/ArkTS API/@ohos.rpc (RPC通信)/js-apis-rpc.md#parcelable9>) | 返回的数据对象。 |
