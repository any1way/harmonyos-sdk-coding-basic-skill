...
scraped_at: 2026-06-01T15:29:49+08:00
doc_updated_at: 2026-04-20
content_hash: sha256:e9abb35a27db65a0d5e7fe1db84bdf35127826f050e9a0a8bd8290921ae43b17
---
本模块提供意图执行基类，开发者通过本模块对接端侧[意图框架](<../../../../../harmonyos-guides/应用框架/Ability Kit（程序框架服务）/Stage模型开发指导/意图框架开发指导/意图框架概述/insight-intent-overview.md>)，[通过配置文件开发意图](<../../../../../harmonyos-guides/应用框架/Ability Kit（程序框架服务）/Stage模型开发指导/意图框架开发指导/开发意图/使用配置文件开发意图/insight-intent-config-development.md>)实现意图的业务逻辑。

除了可以通过配置文件开发意图，还可以通过装饰器开发意图。对于API version 20及以后的版本，推荐使用[通过装饰器开发意图](<../../../../../harmonyos-guides/应用框架/Ability Kit（程序框架服务）/Stage模型开发指导/意图框架开发指导/开发意图/使用装饰器开发意图/insight-intent-decorator-development.md>)。
...
| 名称 | 类型 | 只读 | 可选 | 说明 |
| --- | --- | --- | --- | --- |
| context | [InsightIntentContext](<../@ohos.app.ability.InsightIntentContext (意图执行上下文)/js-apis-app-ability-insightintentcontext.md>) | 否 | 否 | 意图执行上下文。 |
...
当意图执行依赖[UIAbility](<../@ohos.app.ability.UIAbility (带界面的应用组件)/js-apis-app-ability-uiability.md>)组件前台启动时，会在UIAbility组件生命周期执行中触发本意图执行接口。支持同步返回和使用Promise异步返回。

* 若UIAbility组件冷启动，意图执行时UIAbility组件生命周期触发顺序：[onCreate](<../@ohos.app.ability.UIAbility (带界面的应用组件)/js-apis-app-ability-uiability.md#oncreate>)、[onWindowStageCreate](<../@ohos.app.ability.UIAbility (带界面的应用组件)/js-apis-app-ability-uiability.md#onwindowstagecreate>)、onExecuteInUIAbilityForegroundMode、[onForeground](<../@ohos.app.ability.UIAbility (带界面的应用组件)/js-apis-app-ability-uiability.md#onforeground>)。
* 若UIAbility组件热启动，且启动时UIAbility组件处于后台，意图执行时UIAbility组件生命周期触发顺序：[onNewWant](<../@ohos.app.ability.UIAbility (带界面的应用组件)/js-apis-app-ability-uiability.md#onnewwant>)、onExecuteInUIAbilityForegroundMode、[onForeground](<../@ohos.app.ability.UIAbility (带界面的应用组件)/js-apis-app-ability-uiability.md#onforeground>)。
...
| pageLoader | [window.WindowStage](<../../../../ArkUI（方舟UI框架）/ArkTS API/窗口管理/@ohos.window (窗口)/Interface (WindowStage)/arkts-apis-window-windowstage.md>) | 是 | 表示windowStage实例对象，和[onWindowStageCreate](<../@ohos.app.ability.UIAbility (带界面的应用组件)/js-apis-app-ability-uiability.md#onwindowstagecreate>)接口的windowStage实例是同一个，可用于加载意图执行的页面。 |

**返回值：**

| 类型 | 说明 |
| --- | --- |
| [insightIntent.ExecuteResult](<../@ohos.app.ability.insightIntent (意图框架基础定义)/js-apis-app-ability-insightintent.md#executeresult>) | Promise<[insightIntent.ExecuteResult](<../@ohos.app.ability.insightIntent (意图框架基础定义)/js-apis-app-ability-insightintent.md#executeresult>)> | 返回意图执行结果或返回带有意图执行结果的Promise对象，表示本次意图执行返回给系统入口的数据。 |
...
当意图执行依赖[UIAbility](<../@ohos.app.ability.UIAbility (带界面的应用组件)/js-apis-app-ability-uiability.md>)组件后台启动时，会在UIAbility组件生命周期执行中触发本意图执行接口。支持同步返回和使用Promise异步返回。

* 若UIAbility组件冷启动，意图执行时UIAbility组件生命周期触发顺序：[onCreate](<../@ohos.app.ability.UIAbility (带界面的应用组件)/js-apis-app-ability-uiability.md#oncreate>)、onExecuteInUIAbilityBackgroundMode、[onBackground](<../@ohos.app.ability.UIAbility (带界面的应用组件)/js-apis-app-ability-uiability.md#onbackground>)。
...
当意图执行依赖[UIExtensionAbility](<../@ohos.app.ability.UIExtensionAbility (带界面的ExtensionAbility组件)/js-apis-app-abili_64670486.md>)启动时，会在UIExtensionAbility组件生命周期执行中触发本意图执行接口。支持同步返回和使用Promise异步返回。

* 意图执行时UIExtensionAbility生命周期触发顺序：[onCreate](<../@ohos.app.ability.UIExtensionAbility (带界面的ExtensionAbility组件)/js-apis-app-abili_64670486.md#oncreate>)、[onSessionCreate](<../@ohos.app.ability.UIExtensionAbility (带界面的ExtensionAbility组件)/js-apis-app-abili_64670486.md#onsessioncreate>)、onExecuteInUIExtensionAbility、[onForeground](<../@ohos.app.ability.UIExtensionAbility (带界面的ExtensionAbility组件)/js-apis-app-abili_64670486.md#onforeground>)。
...
| name | string | 是 | 意图名称。 |
| param | Record<string, Object> | 是 | 意图参数，表示本次意图执行由系统入口传递给应用的数据。 |
| pageLoader | [UIExtensionContentSession](<../@ohos.app.ability.UIExtensionContentSession (带界面扩展能力的界面操作类)/js-apis-app-ability_18888586.md>) | 是 | 表示UIExtensionContentSession实例对象，和[onSessionCreate](<../@ohos.app.ability.UIExtensionAbility (带界面的ExtensionAbility组件)/js-apis-app-abili_64670486.md#onsessioncreate>)接口的UIExtensionContentSession实例是同一个，可用于加载意图执行的页面。 |
