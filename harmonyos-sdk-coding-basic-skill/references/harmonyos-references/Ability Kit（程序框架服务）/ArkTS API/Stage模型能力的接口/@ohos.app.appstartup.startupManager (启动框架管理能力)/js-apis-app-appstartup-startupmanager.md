...
scraped_at: 2026-06-01T15:30:04+08:00
doc_updated_at: 2026-04-17
content_hash: sha256:3d32cbacdf5b6805819b3b82f9bce4ca4dc6c54cb712da099f5e7ece74fd62c9
---
本模块提供[应用启动框架](<../../../../../harmonyos-guides/应用框架/Ability Kit（程序框架服务）/Stage模型开发指导/Stage模型应用组件/应用启动框架AppStartup/app-startup.md>)管理启动任务的能力，只能在主线程调用。
...
本接口只支持[应用级so](<../../../../../harmonyos-guides/应用框架/Ability Kit（程序框架服务）/Ability Kit术语/ability-terminology.md#应用级so>)文件加载，不支持[系统级so](<../../../../../harmonyos-guides/应用框架/Ability Kit（程序框架服务）/Ability Kit术语/ability-terminology.md#系统级so>)文件加载。
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| startupTasks | Array<string> | 是 | 表示准备执行的启动任务[StartupTask](<../@ohos.app.appstartup.StartupTask (启动框架任务)/js-apis-app-appstartup-startuptask.md>)的名称或预加载so名称的数组。 |
| config | [StartupConfig](<../@ohos.app.appstartup.StartupConfig (启动框架配置信息)/js-apis-app-appstartup-startupconfig.md>) | 否 | 表示启动任务配置信息，包含启动框架超时时间与启动任务监听器配置。 |
...
执行启动框架启动任务或加载so文件。支持指定[AbilityStageContext](../../接口依赖的元素及定义/application/AbilityStageContext/js-apis-inner-application-abilitystagecontext.md)用于启动任务的加载。使用Promise异步回调。
...
| --- | --- | --- | --- |
| startupTasks | Array<string> | 是 | 表示准备执行的启动任务[StartupTask](<../@ohos.app.appstartup.StartupTask (启动框架任务)/js-apis-app-appstartup-startuptask.md>)的名称或预加载so名称的数组。 |
| context | [common.AbilityStageContext](../../接口依赖的元素及定义/application/AbilityStageContext/js-apis-inner-application-abilitystagecontext.md) | 是 | 表示执行启动任务[StartupTask](<../@ohos.app.appstartup.StartupTask (启动框架任务)/js-apis-app-appstartup-startuptask.md>)的AbilityStage上下文，作为入参传给启动任务的[init](<../@ohos.app.appstartup.StartupTask (启动框架任务)/js-apis-app-appstartup-startuptask.md#init>)。 |
| config | [StartupConfig](<../@ohos.app.appstartup.StartupConfig (启动框架配置信息)/js-apis-app-appstartup-startupconfig.md>) | 是 | 表示启动任务配置信息，包含启动框架超时时间与启动任务监听器配置。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| startupTask | string | 是 | 启动任务[StartupTask](<../@ohos.app.appstartup.StartupTask (启动框架任务)/js-apis-app-appstartup-startuptask.md>)的名称或预加载so名称。 |

**返回值：**

| 类型 | 说明 |
| --- | --- |
| Object | 输入为启动任务名时，返回指定的启动任务[init](<../@ohos.app.appstartup.StartupTask (启动框架任务)/js-apis-app-appstartup-startuptask.md#init>)返回的执行结果。  输入为so文件名时，返回undefined。 |
