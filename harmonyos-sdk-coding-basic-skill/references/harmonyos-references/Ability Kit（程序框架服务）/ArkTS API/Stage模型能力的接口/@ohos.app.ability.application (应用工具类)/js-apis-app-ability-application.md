...
scraped_at: 2026-06-01T15:29:26+08:00
doc_updated_at: 2026-05-07
content_hash: sha256:837b77eb9887bee9f23f785673344a14a5d862473883801238d0e0305feec19d
---
开发者可以通过该模块管理和获取应用的上下文[Context](<../../../../../harmonyos-guides/应用框架/Ability Kit（程序框架服务）/Stage模型开发指导/Stage模型应用组件/应用上下文Context/application-context-stage.md>)，以及控制应用进程的状态。
...
| TYPE\_CREATE\_ABILITY\_STAGE | 2 | 进程最终预加载到[AbilityStage](<../@ohos.app.ability.AbilityStage (AbilityStage组件管理器)/js-apis-app-ability-abilitystage.md>)创建完成阶段。 |
| TYPE\_CREATE\_WINDOW\_STAGE | 3 | 进程最终预加载到[WindowStage](<../../../../ArkUI（方舟UI框架）/ArkTS API/窗口管理/@ohos.window (窗口)/Interface (WindowStage)/arkts-apis-window-windowstage.md>)创建完成阶段。 |
| TYPE\_CREATE\_BACKGROUND\_ABILITY 23+ | 4 | 进程最终预加载到[onBackground](<../@ohos.app.ability.UIAbility (带界面的应用组件)/js-apis-app-ability-uiability.md#onbackground>)执行完成阶段。 |
...
创建指定模块的上下文。创建出的模块上下文中[resourceManager.Configuration](<../../../../Localization Kit（本地化开发服务）/ArkTS API/@ohos.resourceManager (资源管理)/js-apis-resource-manager.md#configuration>)资源继承自入参上下文，便于开发者获取[跨HAP/HSP包应用资源](../../../../../harmonyos-guides/基础入门/资源分类与访问/resource-categories-and-access.md#访问跨haphsp包资源)。使用Promise异步回调。
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| context | [Context](<../../接口依赖的元素及定义/application/Context (Stage模型的上下文基类)/js-apis-inner-application-context.md>) | 是 | 表示应用上下文。 |
...
| 类型 | 说明 |
| --- | --- |
| Promise<[Context](<../../接口依赖的元素及定义/application/Context (Stage模型的上下文基类)/js-apis-inner-application-context.md>)> | Promise对象。返回创建的Context。 |

**错误码：**

以下错误码详细介绍请参考[通用错误码说明文档](../../../../通用错误码/errorcode-universal.md)。
...
| 类型 | 说明 |
| --- | --- |
| [ApplicationContext](<../../接口依赖的元素及定义/application/ApplicationContext (应用上下文)/js-apis-inner-application-applicationcontext.md>) | 应用上下文。 |
...
开发者可以调用该接口将当前进程放入[备选主控进程](<../../../../../harmonyos-guides/应用框架/Ability Kit（程序框架服务）/Ability Kit术语/ability-terminology.md#candidatemasterprocess备选主控进程>)链表。使用Promise异步回调。

当[主控进程](<../../../../../harmonyos-guides/应用框架/Ability Kit（程序框架服务）/Ability Kit术语/ability-terminology.md#masterprocess主控进程>)销毁后，再次启动配置了isolationProcess为true的UIAbility/UIExtensionAbility组件时，系统会根据是否存在备选主控进程执行相应操作。

* 如果存在备选主控进程，系统会将备选主控进程链表首节点的进程设置为主控进程，触发[onNewProcessRequest](<../@ohos.app.ability.AbilityStage (AbilityStage组件管理器)/js-apis-app-ability-abilitystage.md#onnewprocessrequest11>)回调。
...
如果当前进程已经是[主控进程](<../../../../../harmonyos-guides/应用框架/Ability Kit（程序框架服务）/Ability Kit术语/ability-terminology.md#masterprocess主控进程>)，调用该接口无效并且不会抛出错误码。
...
以下错误码详细介绍请参考[通用错误码说明文档](../../../../通用错误码/errorcode-universal.md)和[元能力子系统错误码](../../../错误码/元能力子系统错误码/errorcode-ability.md)。
...
放弃当前进程的[主控进程](<../../../../../harmonyos-guides/应用框架/Ability Kit（程序框架服务）/Ability Kit术语/ability-terminology.md#masterprocess主控进程>)身份。使用Promise异步回调。
...
* 只有在进程首次执行[AbilityStage.onCreate](<../@ohos.app.ability.AbilityStage (AbilityStage组件管理器)/js-apis-app-ability-abilitystage.md#oncreate>)完成之前调用该接口，才可以返回真实的预加载类型。
