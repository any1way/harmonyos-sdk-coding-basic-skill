...
scraped_at: 2026-06-01T15:29:46+08:00
doc_updated_at: 2026-03-09
content_hash: sha256:ac6bb443bf6aa9449723169ce2fd39b5f294555e25e9bbac2a6f2bac6d542693
---
InsightIntentDecorator模块提供了几类意图装饰器，用于装饰类或方法。开发者可以[使用装饰器开发意图](<../../../../../harmonyos-guides/应用框架/Ability Kit（程序框架服务）/Stage模型开发指导/意图框架开发指导/开发意图/使用装饰器开发意图/insight-intent-decorator-development.md>), 将应用的功能定义为意图，并集成到智能问答、智能搜索、智能推荐等AI入口。
...
* [@InsightIntentEntry](js-apis-app-ability-insightintentdecorator.md#insightintententry)装饰器：使用该装饰器装饰一个继承自[InsightIntentEntryExecutor](<../@ohos.app.ability.InsightIntentEntryExecutor (@InsightIntentEntry的意图执行基类)/js-apis_94625279.md>)的类，实现意图操作并配置意图依赖的Ability组件，便于AI入口拉起依赖的Ability组件时，执行对应的意图操作。该装饰器支持的参数参见[EntryIntentDecoratorInfo](js-apis-app-ability-insightintentdecorator.md#entryintentdecoratorinfo)。
* [@InsightIntentForm](js-apis-app-ability-insightintentdecorator.md#insightintentform)装饰器：使用该装饰器装饰[FormExtensionAbility](<../../../../Form Kit（卡片开发服务）/ArkTS API/@ohos.app.form.FormExtensionAbility (FormExtensionAbility)/js-apis-app-form-formextensionability.md>)并配置FormExtensionAbility绑定的卡片名称，便于AI入口通过意图添加卡片。该装饰器支持的参数参见[FormIntentDecoratorInfo](js-apis-app-ability-insightintentdecorator.md#formintentdecoratorinfo)。
* [@InsightIntentEntity](js-apis-app-ability-insightintentdecorator.md#insightintententity)装饰器：使用该装饰器装饰一个继承自[IntentEntity](<../@ohos.app.ability.insightIntent (意图框架基础定义)/js-apis-app-ability-insightintent.md#intententity20>)的类，可将该类定义为意图实体，用于传递意图调用时所需的参数。该装饰器支持的参数参见[IntentEntityDecoratorInfo](js-apis-app-ability-insightintentdecorator.md#intententitydecoratorinfo)。
...
uri链接格式需要符合[应用链接说明](<../../../../../harmonyos-guides/应用框架/Ability Kit（程序框架服务）/Stage模型开发指导/应用间跳转/拉起指定应用/应用链接说明/app-uri-config.md>)中的要求。
...
| schema | string | 否 | 是 | 表示接入的标准意图的名称。开发者[接入标准意图](<../../../../../harmonyos-guides/应用框架/Ability Kit（程序框架服务）/Stage模型开发指导/意图框架开发指导/开发意图/意图开发概述/insight-intent-definition.md#接入标准意图>)时，需要配置该字段，[创建自定义意图](<../../../../../harmonyos-guides/应用框架/Ability Kit（程序框架服务）/Stage模型开发指导/意图框架开发指导/开发意图/意图开发概述/insight-intent-definition.md#创建自定义意图>)时，无需配置该字段。标准意图列表参见[附录：标准意图接入规范](<../../../../../harmonyos-guides/应用框架/Ability Kit（程序框架服务）/Stage模型开发指导/意图框架开发指导/开发意图/附录：标准意图接入规范/insight-intent-access-specifications.md>)。 |
| icon | ResourceStr | 否 | 是 | 表示意图图标，用于在AI入口显示。  - 当取值为字符串类型时，表示图标读取网络资源。  - 当取值为[Resource](<../../../../Localization Kit（本地化开发服务）/ArkTS API/@ohos.resourceManager (资源管理)/js-apis-resource-manager.md>)时，表示图标读取本地资源。 |
...
| WANT | 'want' | 表示意图参数类别为'want'。意图参数将通过[Want](<../../通用能力的接口(推荐)/@ohos.app.ability.Want (Want)/js-apis-app-ability-want.md>)的parameters字段传给应用。 |
...
PageIntentDecoratorInfo继承自[IntentDecoratorInfo](js-apis-app-ability-insightintentdecorator.md#intentdecoratorinfo)，用于描述[@InsightIntentPage](js-apis-app-ability-insightintentdecorator.md#insightintentpage)装饰器支持的参数，例如目标页面的[NavDestination](../../../../ArkUI（方舟UI框架）/ArkTS组件/导航与切换/Navigation/ts-basic-components-navigation.md#navdestination10)名称。
...
| navigationId | string | 否 | 是 | 表示与意图绑定的[Navigation组件](../../../../ArkUI（方舟UI框架）/ArkTS组件/导航与切换/Navigation/ts-basic-components-navigation.md#属性)的id属性。 |
| navDestinationName | string | 否 | 是 | 表示与意图绑定[NavDestination组件](../../../../ArkUI（方舟UI框架）/ArkTS组件/导航与切换/Navigation/ts-basic-components-navigation.md#navdestination10)的名称。 |
...
使用该装饰器装饰一个继承自[InsightIntentEntryExecutor](<../@ohos.app.ability.InsightIntentEntryExecutor (@InsightIntentEntry的意图执行基类)/js-apis_94625279.md>)的类，实现意图操作并配置意图依赖的Ability组件，便于AI入口拉起依赖的Ability组件时，执行对应的意图操作。该装饰器支持的参数参见[EntryIntentDecoratorInfo](js-apis-app-ability-insightintentdecorator.md#entryintentdecoratorinfo)。
...
| executeMode | [insightIntent.ExecuteMode](<../@ohos.app.ability.insightIntent (意图框架基础定义)/js-apis-app-ability-insightintent.md#executemode>)[] | 否 | 否 | 表示意图调用执行模式。即拉起绑定的Ability时支持的执行模式。 |
...
卡片名称定义的要求参见[卡片配置](<../../../../../harmonyos-guides/应用框架/Form Kit（卡片开发服务）/ArkTS卡片开发（推荐）/配置ArkTS卡片的配置文件/arkts-ui-widget-configuration.md#卡片配置>)。
...
使用该装饰器装饰一个继承自[IntentEntity](<../@ohos.app.ability.insightIntent (意图框架基础定义)/js-apis-app-ability-insightintent.md#intententity20>)的类，可将该类定义为意图实体，用于传递意图调用时所需的参数。该装饰器支持的参数参见[IntentEntityDecoratorInfo](js-apis-app-ability-insightintentdecorator.md#intententitydecoratorinfo)。
