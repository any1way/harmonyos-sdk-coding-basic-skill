...
scraped_at: 2026-06-01T15:31:08+08:00
doc_updated_at: 2026-04-30
content_hash: sha256:5c094ebcc5775910e379f4082221b551aadefc479b82bc9bbd623955bcb0eeca
---
ApplicationContext作为应用上下文，继承自[Context](<../Context (Stage模型的上下文基类)/js-apis-inner-application-context.md>)，提供了应用生命周期监听、进程管理、应用环境设置等应用级别的管控能力。
...
| callback | [AbilityLifecycleCallback](<../../../Stage模型能力的接口/@ohos.app.ability.abilityLifecycleCallback (UIAbility生命周期回调监听器)/js-apis-app-abi_71548541.md>) | 是 | UIAbility生命周期变化时触发的回调方法。 |
...
* 使用[onConfigurationUpdate](<../../../Stage模型能力的接口/@ohos.app.ability.Ability (Ability基类)/js-apis-app-ability-ability.md#abilityonconfigurationupdate>)也可以实现对系统环境变量的监听。相较于Ability的[onConfigurationUpdate](<../../../Stage模型能力的接口/@ohos.app.ability.Ability (Ability基类)/js-apis-app-ability-ability.md#abilityonconfigurationupdate>)接口，当前接口的使用场景更加灵活，不仅可以在应用组件中使用，还可以在页面中使用，但是支持订阅的环境变量与Ability的[onConfigurationUpdate](<../../../Stage模型能力的接口/@ohos.app.ability.Ability (Ability基类)/js-apis-app-ability-ability.md#abilityonconfigurationupdate>)接口存在差异，如不支持订阅direction、screenDensity、displayId，详见[Configuration](<../../../通用能力的接口(推荐)/@ohos.app.ability.Configuration (环境变量)/js-apis-app-ability-configuration.md#configuration>)中各个环境变量的说明。
* 当前接口在实际触发时存在一定限制。例如如果开发者通过[setLanguage](js-apis-inner-application-applicationcontext.md#applicationcontextsetlanguage11)接口设置应用的语言，即便系统语言发生变化，系统也不再触发当前接口的[callback](<../../../Stage模型能力的接口/@ohos.app.ability.EnvironmentCallback (系统环境变化监听器)/js-apis-app-ability-environmentcallback.md>)回调。详见[使用场景](<../../../../../../harmonyos-guides/应用框架/Ability Kit（程序框架服务）/Stage模型开发指导/Stage模型应用组件/获取设置环境变量/subscribe-system-environment-variable-changes.md#使用场景>)。
...
| callback | [EnvironmentCallback](<../../../Stage模型能力的接口/@ohos.app.ability.EnvironmentCallback (系统环境变化监听器)/js-apis-app-ability-environmentcallback.md>) | 是 | 系统环境变化时触发的回调方法。 |
...
| callback | [ApplicationStateChangeCallback](<../../../Stage模型能力的接口/@ohos.app.ability.ApplicationStateChangeCallback (应用进程状态变化监听器)/js-apis-app-abil_14078489.md>) | 是 | 当前应用进程状态切换时触发的回调方法。 |
...
| --- | --- | --- | --- |
| type | string | 是 | 此类型表示当前应用进程状态变化，固定为'applicationStateChange'。 |
| callback | [ApplicationStateChangeCallback](<../../../Stage模型能力的接口/@ohos.app.ability.ApplicationStateChangeCallback (应用进程状态变化监听器)/js-apis-app-abil_14078489.md>) | 否 | 回调函数。取值可以为使用[ApplicationContext.on('applicationStateChange')](js-apis-inner-application-applicationcontext.md#applicationcontextonapplicationstatechange10)方法定义的callback回调，也可以为空。  - 如果传入已定义的回调，则取消该监听。  - 如果未传入参数，则取消所有已注册的该类型事件的监听。 |
...
注册监听系统环境[Configuration](<../../../通用能力的接口(推荐)/@ohos.app.ability.Configuration (环境变量)/js-apis-app-ability-configuration.md#configuration>)的变化。使用callback异步回调。仅支持主线程调用。
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| callback | [systemConfiguration.UpdatedCallback](<../../../Stage模型能力的接口/@ohos.app.ability.systemConfiguration (系统环境模块)/js-apis-app-ability-systemconfiguration.md#updatedcallback>) | 是 | 系统环境变化时触发的回调方法。 |
...
取消监听系统环境[Configuration](<../../../通用能力的接口(推荐)/@ohos.app.ability.Configuration (环境变量)/js-apis-app-ability-configuration.md#configuration>)的变化。仅支持主线程调用。
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| callback | [systemConfiguration.UpdatedCallback](<../../../Stage模型能力的接口/@ohos.app.ability.systemConfiguration (系统环境模块)/js-apis-app-ability-systemconfiguration.md#updatedcallback>) | 否 | 回调函数。取值可以为使用[ApplicationContext.onSystemConfigurationUpdated](js-apis-inner-application-applicationcontext.md#applicationcontextonsystemconfigurationupdated24)方法注册的callback回调，也可以为空。  - 如果传入已定义的回调，则取消该监听。  - 如果未传入参数，则取消所有已注册的监听。 |
...
| 类型 | 说明 |
| --- | --- |
| Promise<Array<[ProcessInformation](../ProcessInformation/js-apis-inner-application-processinformation.md)>> | Promise对象，返回接口运行结果及有关运行进程的信息，可进行错误处理或其他自定义处理。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| callback | AsyncCallback<Array<[ProcessInformation](../ProcessInformation/js-apis-inner-application-processinformation.md)>> | 是 | 回调函数，返回有关运行进程的信息。 |
...
该接口用于应用异常场景中强制退出应用。如需正常退出应用，可以使用[terminateSelf()](../UIAbilityContext/js-apis-inner-application-uiabilitycontext.md#terminateself-1)接口。
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| colorMode | [ConfigurationConstant.ColorMode](<../../../通用能力的接口(推荐)/@ohos.app.ability.ConfigurationConstant (环境变量相关的常量定义)/js-apis-app-ability-configurationconstant.md#colormode>) | 是 | 深浅色模式，包括：深色模式、浅色模式、未设置颜色模式（默认）。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| language | string | 是 | 设置语言，当前支持的语言列表可以通过[getSystemLanguages()](<../../../../../Localization Kit（本地化开发服务）/ArkTS API/@ohos.i18n (国际化-I18n)/js-apis-i18n.md#getsystemlanguages9>)获取。 |
...
应用文件路径详见[应用文件目录信息](<../../../../../../harmonyos-guides/应用框架/Core File Kit（文件基础服务）/应用文件/应用沙箱目录/app-sandbox-directory.md#应用文件目录与应用文件路径>)。图中仅标识了el1~el2目录下的应用文件路径，其他文件加密类型目录下的应用文件路径可以参考el1。
...
在元服务调用本接口成功后的3秒内，再次调用本接口、[restartSelfAtomicService()](<../../../通用能力的接口(推荐)/@ohos.app.ability.abilityManager (Ability信息管理)/js-apis-app-ability-abilitymanager.md#abilitymanagerrestartselfatomicservice20>)或[UIAbilityContext.restartApp()](../UIAbilityContext/js-apis-inner-application-uiabilitycontext.md#restartapp22)接口中的任一接口，系统将返回错误码16000064。

在应用调用本接口成功后的3秒内，若再次调用本接口或[UIAbilityContext.restartApp()](../UIAbilityContext/js-apis-inner-application-uiabilitycontext.md#restartapp22)接口中的任一接口，系统将返回错误码16000064。
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| want | [Want](<../../../通用能力的接口(推荐)/@ohos.app.ability.Want (Want)/js-apis-app-ability-want.md>) | 是 | Want类型参数，传入需要启动的UIAbility信息，校验abilityName，不校验bundleName。 |
...
以上错误码详细介绍请参考[元能力子系统错误码](../../../../错误码/元能力子系统错误码/errorcode-ability.md)。
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| font | string | 是 | 设置字体类型，字体可以通过[UIContext.registerFont](<../../../../../ArkUI（方舟UI框架）/ArkTS API/UI界面/@ohos.arkui.UIContext (UIContext)/Class (Font)/arkts-apis-uicontext-font.md#registerfont>)方法进行注册使用。 |
...
* 为了确保该接口在进程退出前生效，调用时机应尽量提前。建议在[AbilityStage](<../../../Stage模型能力的接口/@ohos.app.ability.AbilityStage (AbilityStage组件管理器)/js-apis-app-ability-abilitystage.md>)的onCreate()中调用该接口。
...
以下错误码详细介绍请参考[通用错误码](../../../../../通用错误码/errorcode-universal.md)、[元能力子系统错误码](../../../../错误码/元能力子系统错误码/errorcode-ability.md)。
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| fontSizeScale | number | 是 | 表示字体缩放比例，取值为非负数。当应用字体[跟随系统](../../../../../../harmonyos-guides/基础入门/开发基础知识/应用配置文件（Stage模型）/app.json5配置文件/app-configuration-file.md#configuration标签)且该字段取值超过[fontSizeMaxScale](../../../../../../harmonyos-guides/基础入门/开发基础知识/应用配置文件（Stage模型）/app.json5配置文件/app-configuration-file.md#configuration标签)取值时，实际生效值为[fontSizeMaxScale](../../../../../../harmonyos-guides/基础入门/开发基础知识/应用配置文件（Stage模型）/app.json5配置文件/app-configuration-file.md#configuration标签)取值。 |
...
| 类型 | 说明 |
| --- | --- |
| Promise<Array<[window.WindowStage](<../../../../../ArkUI（方舟UI框架）/ArkTS API/窗口管理/@ohos.window (窗口)/Interface (WindowStage)/arkts-apis-window-windowstage.md>)>> | Promise对象，返回应用当前进程内的所有WindowStage对象。 |
