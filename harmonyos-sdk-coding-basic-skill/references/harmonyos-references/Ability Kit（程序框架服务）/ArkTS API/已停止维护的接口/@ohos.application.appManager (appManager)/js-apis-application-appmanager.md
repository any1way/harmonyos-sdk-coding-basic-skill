...
scraped_at: 2026-06-01T15:31:48+08:00
doc_updated_at: 2026-04-13
content_hash: sha256:36916d6b0e1cc1994ad371573072c0c55511996a8991768b73640328e0c2810e
---
appManager模块提供应用管理的能力，包括查询当前系统是否处于稳定性测试场景、查询当前设备是否为RAM（Random Access Memory，随机存取存储器）受限设备、获取当前应用程序可以使用的最大内存值、获取有关运行进程的信息等。

说明

本模块首批接口从API version 8 开始支持，从API version 9废弃，替换模块为[@ohos.app.ability.appManager](<../../通用能力的接口(推荐)/@ohos.app.ability.appManager (应用管理)/js-apis-app-ability-appmanager.md>)。后续版本的新增接口，采用上角标单独标记接口的起始版本。
...
从 API Version 9 开始废弃，建议使用[appManager.getRunningProcessInformation](<../../通用能力的接口(推荐)/@ohos.app.ability.appManager (应用管理)/js-apis-app-ability-appmanager.md#appmanagergetrunningprocessinformation>)替代。
...
| 类型 | 说明 |
| --- | --- |
| Promise<Array<[ProcessRunningInfo](../../接口依赖的元素及定义/application/ProcessRunningInfo/js-apis-inner-application-processrunninginfo.md)>> | Promise对象，返回有关运行进程的信息。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| callback | AsyncCallback<Array<[ProcessRunningInfo](../../接口依赖的元素及定义/application/ProcessRunningInfo/js-apis-inner-application-processrunninginfo.md)>> | 是 | 回调函数，返回有关运行进程的信息。 |
