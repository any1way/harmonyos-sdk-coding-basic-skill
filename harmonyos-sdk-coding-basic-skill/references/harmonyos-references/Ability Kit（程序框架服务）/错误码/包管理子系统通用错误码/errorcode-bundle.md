...
scraped_at: 2026-06-01T15:32:44+08:00
doc_updated_at: 2026-05-28
content_hash: sha256:f2ebd50528640c0a7b260b30bf808019816a9e606d8352fa676cb65240851054
---
说明
...
2. 可以使用[查询应用信息命令（dump）](../../../../harmonyos-guides/系统/调测调优/调试命令/bm工具/bm-tool.md#查询应用信息命令dump)查看应用是否安装。查看输出的打印信息，应用未安装时，该命令执行会报错。
...
2. 可以使用[查询应用信息命令（dump）](../../../../harmonyos-guides/系统/调测调优/调试命令/bm工具/bm-tool.md#查询应用信息命令dump)查看对应的模块是否安装。查看输出的打印信息中hapModuleNames字段对应的列表是否存在该moduleName，不存在则说明应用未安装该模块。
...
3. 调用[bundleManager.getProfileByAbility](<../../ArkTS API/通用能力的接口(推荐)/@ohos.bundle.bundleManager (应用程序包管理模块)/js-apis-bundlemanager.md#bundlemanagergetprofilebyability>)、[bundleManager.getProfileByExtensionAbility](<../../ArkTS API/通用能力的接口(推荐)/@ohos.bundle.bundleManager (应用程序包管理模块)/js-apis-bundlemanager.md#bundlemanagergetprofilebyextensionability>) 等通过abilityName、moduleName组合查询的接口时，对应的应用没有安装moduleName对应的模块，对应模块下的ability也不存在。

**处理步骤**

1. 检查abilityName拼写是否正确。
2. 可以使用[查询应用信息命令（dump）](../../../../harmonyos-guides/系统/调测调优/调试命令/bm工具/bm-tool.md#查询应用信息命令dump)查看对应的应用是否存在这个abilityName。查看输出的打印信息中hapModuleInfos字段对应的abilityInfos下是否包含name等于该abilityName，不包含则说明该abilityName不存在。
3. 可以使用[查询应用信息命令（dump）](../../../../harmonyos-guides/系统/调测调优/调试命令/bm工具/bm-tool.md#查询应用信息命令dump)查看输出的打印信息中hapModuleNames字段对应的列表是否存在对应的moduleName，不存在则说明应用未安装该模块，对应模块下的ability也不存在。
...
调用bundleManager模块中的[getBundleNameByUid](<../../ArkTS API/通用能力的接口(推荐)/@ohos.bundle.bundleManager (应用程序包管理模块)/js-apis-bundlemanager.md#bundlemanagergetbundlenamebyuid14>)时，指定的uid无效。
...
检查系统中是否存在对应的应用uid值。可以使用[查询应用信息命令（dump）](../../../../harmonyos-guides/系统/调测调优/调试命令/bm工具/bm-tool.md#查询应用信息命令dump)查看已安装应用的uid。执行命令行后会输出对应已安装应用的uid，如果输出多个uid，选择大于0的。如果该命令无打印值输出，表示应用未安装。
...
确认指定的ability是否被禁用，可以使用[bm工具](../../../../harmonyos-guides/系统/调测调优/调试命令/bm工具/bm-tool.md)查询对应的应用信息。
...
调用bundleManager模块中的[canOpenLink](<../../ArkTS API/通用能力的接口(推荐)/@ohos.bundle.bundleManager (应用程序包管理模块)/js-apis-bundlemanager.md#bundlemanagercanopenlink12>)时，指定的link无效。
...
调用bundleManager模块中的[canOpenLink](<../../ArkTS API/通用能力的接口(推荐)/@ohos.bundle.bundleManager (应用程序包管理模块)/js-apis-bundlemanager.md#bundlemanagercanopenlink12>)时，指定link的scheme未在querySchemes字段下配置。
...
检查是否在querySchemes字段下配置了相应的URL scheme，可以参考[使用canOpenLink判断应用是否可访问](<../../../../harmonyos-guides/应用框架/Ability Kit（程序框架服务）/Stage模型开发指导/应用间跳转/拉起指定应用/（可选）使用canOpenLink判断应用是否可访问/canopenlink.md>)。
...
调用[bundleManager.getLaunchWant](<../../ArkTS API/通用能力的接口(推荐)/@ohos.bundle.bundleManager (应用程序包管理模块)/js-apis-bundlemanager.md#bundlemanagergetlaunchwant13>)接口时，应用的启动组件Want信息不存在。
