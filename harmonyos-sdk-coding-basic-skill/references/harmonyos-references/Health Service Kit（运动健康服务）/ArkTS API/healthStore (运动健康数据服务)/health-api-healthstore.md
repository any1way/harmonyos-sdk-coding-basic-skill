...
scraped_at: 2026-06-11T16:44:41+08:00
doc_updated_at: 2026-06-05
content_hash: sha256:420c941c513e8ee849543a6028906375783a37dfbaa31182d6f196dbdd207fe4
---
本模块提供运动健康数据服务。
...
| scopes | string[] | 否 | 是 | 非数据类型权限，使用scope授权，请参见[OAuth权限](<../../../../harmonyos-guides/应用服务/Health Service Kit（运动健康服务）/开发接入/PhoneTablet应用开发/手动数据同步/health-cloudsync.md#oauth权限>)，若未填写，默认为空。  **起始版本：** 5.1.0(18)  **元服务API：** 从版本5.1.0(18)开始，该接口支持在元服务中使用。 |
...
| 名称 | **类型** | 只读 | 可选 | **说明** |
| --- | --- | --- | --- | --- |
| readDataTypes | [DataType](health-api-healthstore.md#datatype)[] | 否 | 否 | 授权成功的读数据类型，其对应权限在[应用申请权限](<../../../../harmonyos-guides/应用服务/Health Service Kit（运动健康服务）/开发接入/开发准备/申请运动健康服务/health-apply.md>)和用户授权权限的交集中。  **元服务API：** 从版本5.0.0(12)开始，该接口支持在元服务中使用。 |
| writeDataTypes | [DataType](health-api-healthstore.md#datatype)[] | 否 | 否 | 授权成功的写数据类型，其对应权限在[应用申请权限](<../../../../harmonyos-guides/应用服务/Health Service Kit（运动健康服务）/开发接入/开发准备/申请运动健康服务/health-apply.md>)和用户授权权限的交集中。  **元服务API：** 从版本5.0.0(12)开始，该接口支持在元服务中使用。 |
| scopes | string[] | 否 | 是 | 非数据类型权限，使用scope授权，请参见[OAuth权限](<../../../../harmonyos-guides/应用服务/Health Service Kit（运动健康服务）/开发接入/PhoneTablet应用开发/手动数据同步/health-cloudsync.md#oauth权限>)，若未填写，默认为空。  **起始版本：** 5.1.0(18)  **元服务API：** 从版本5.1.0(18)开始，该接口支持在元服务中使用。 |
...
| 名称 | **类型** | 只读 | 可选 | **说明** |
| --- | --- | --- | --- | --- |
| summaries | Pick<K, keyof K> | 否 | 否 | 统计数据，健康记录关联的统计数据类型参考[healthSequenceHelper](<../运动健康数据类型常量及模型定义/healthSequenceHelper (健康记录类型常量)/health-api-healthsequencehelper.md>)定义的模型。 |
| details | Pick<DK, keyof DK> | 否 | 是 | 详情数据，健康记录关联的详情数据类型参考[healthSequenceHelper](<../运动健康数据类型常量及模型定义/healthSequenceHelper (健康记录类型常量)/health-api-healthsequencehelper.md>)定义的模型，若未填写，默认为空。 |
...
| 名称 | **类型** | 只读 | 可选 | **说明** |
| --- | --- | --- | --- | --- |
| fields | Pick<K, keyof K> | 否 | 否 | 数据的字段，数据类型对应的字段参考[samplePointHelper](<../运动健康数据类型常量及模型定义/samplePointHelper (采样数据类型常量)/health-api-samplepointhelper.md>)定义的模型。 |
...
| fields | Partial<Pick<FK, keyof FK>> | 否 | 是 | 要读取的字段列表，若samplePointDataType参数为数组，fields字段不能设置，数据类型对应的字段参考[samplePointHelper](<../运动健康数据类型常量及模型定义/samplePointHelper (采样数据类型常量)/health-api-samplepointhelper.md>)定义的模型。若为空，则用于读取所有字段。 |
...
| withPartialDetails | (keyof DK)[] | 否 | 是 | 读取部分详情数据类型（若需要读取部分详情，withDetails参数不能填写），锻炼记录与健康记录关联的详情数据类型分别参考[exerciseSequenceHelper](<../运动健康数据类型常量及模型定义/exerciseSequenceHelper (锻炼记录类型常量)/health-api-exercisedequencehelper.md>)与[healthSequenceHelper](<../运动健康数据类型常量及模型定义/healthSequenceHelper (健康记录类型常量)/health-api-healthsequencehelper.md>)定义的模型。 |
...
| **参数名** | **类型** | 必填 | **说明** |
| --- | --- | --- | --- |
| context | [common.Context](<../../../Ability Kit（程序框架服务）/ArkTS API/接口依赖的元素及定义/application/Context (Stage模型的上下文基类)/js-apis-inner-application-context.md#context>) | 是 | 上下文，目前只支持[UIAbility](<../../../Ability Kit（程序框架服务）/ArkTS API/Stage模型能力的接口/@ohos.app.ability.UIAbility (带界面的应用组件)/js-apis-app-ability-uiability.md>)或[UIExtensionAbility](<../../../Ability Kit（程序框架服务）/ArkTS API/Stage模型能力的接口/@ohos.app.ability.UIExtensionAbility (带界面的ExtensionAbility组件)/js-apis-app-abili_64670486.md>)的上下文环境。 |
...
| 错误码ID | 错误信息 |
| --- | --- |
| [401](<../../ArkTS API错误码/errorcode-healthservice.md#section401-参数不合法>) | Parameter error. Possible causes: 1. Mandatory parameters are left unspecified;2. Incorrect parameter types. |
...
| [401](<../../ArkTS API错误码/errorcode-healthservice.md#section401-参数不合法>) | Parameter error. Possible causes: 1. Mandatory parameters are left unspecified;2. Incorrect parameter types; 3.Parameter verification failed. |
| [1002700001](<../../ArkTS API错误码/errorcode-healthservice.md#section1002700001-系统内部错误>) | System internal error. |
...
| [1002702001](<../../ArkTS API错误码/errorcode-healthservice.md#section1002702001-账号未登录>) | Account error. The user has not logged in with HUAWEI ID. |
| [1002702002](<../../ArkTS API错误码/errorcode-healthservice.md#section1002702002-账号异常>) | Account error. Failed to obtain account information with HUAWEI ID. |
| [1002703001](<../../ArkTS API错误码/errorcode-healthservice.md#section1002703001-用户隐私未同意>) | User privacy is not agreed. |
...
16. hilog.info(0x0000, 'testTag', `Succeeded in reading data,
17. the bodyTemperature is ${samplePoint.fields.bodyTemperature}.`);
18. });
19. } catch (err) {
20. hilog.error(0x0000, 'testTag', `Failed to read data. Code: ${err.code}, message: ${err.message}`);
21. }
```
...
8. const sequenceReadRequest:
9. healthStore.ExerciseSequenceReadRequest<healthStore.exerciseSequenceHelper.running.DetailFields> = {
10. startTime: startTime,
11. endTime: endTime,
12. exerciseType: healthStore.exerciseSequenceHelper.running.EXERCISE_TYPE,
13. count: 1,
14. sortOrder: 1,
15. readOptions: {
16. withPartialDetails: ['exerciseHeartRate', 'altitude']
17. }
18. };
19. try {
20. const runningSequences =
21. await healthStore.readData<healthStore.exerciseSequenceHelper.running.Model>(sequenceReadRequest);
22. hilog.info(0x0000, 'testTag', 'Succeeded in reading data.');
23. runningSequences.forEach((runningSequence) => {
24. hilog.info(0x0000, 'testTag', `the start time is ${runningSequence.startTime}.`);
25. hilog.info(0x0000, 'testTag', `the end time is ${runningSequence.endTime}.`);
26. Object.keys(runningSequence.summaries).forEach((key) => {
27. Object.keys(runningSequence.summaries[key]).forEach((fieldName) => {
28. hilog.info(0x0000, 'testTag',
29. `the summaries of ${key} field ${fieldName} is ${runningSequence.summaries[key][fieldName]}.`);
30. });
31. });
32. });
33. } catch (err) {
...
4. // 删除跑步
5. try {
6. const sequenceReadRequest:
7. healthStore.ExerciseSequenceReadRequest<healthStore.exerciseSequenceHelper.running.DetailFields> = {
8. startTime: 1698040800000,
9. endTime: 1698042600000,
10. exerciseType: healthStore.exerciseSequenceHelper.running.EXERCISE_TYPE
11. };
12. const runningSequences = await
13. healthStore.readData<healthStore.exerciseSequenceHelper.running.Model>(sequenceReadRequest);
14. for (let index = 0; index < runningSequences.length; index++) {
15. const runningSequence = runningSequences[index];
16. await healthStore.deleteData(runningSequence);
17. }
18. hilog.info(0x0000, 'testTag', 'Succeeded in deleting data.');
19. } catch (err) {
20. hilog.error(0x0000, 'testTag', `Failed to delete data. Code: ${err.code}, message: ${err.message}`);
21. }
```
...
22. try {
23. const aggregateResults =
24. await healthStore.aggregateData<healthStore.samplePointHelper.dailyActivities.AggregateResult>(aggregateRequest);

26. hilog.info(0x0000, 'testTag', 'Succeeded in reading data.');
27. aggregateResults.forEach((aggregateResult) => {
28. hilog.info(0x0000, 'testTag', `the start time is ${aggregateResult.startTime}.`);
29. hilog.info(0x0000, 'testTag', `the end time is ${aggregateResult.endTime}.`);
30. Object.keys(aggregateResult.fields).forEach((fieldName) => {
31. hilog.info(0x0000, 'testTag', `the sum of ${fieldName} is ${aggregateResult.fields[fieldName].sum}.`);
32. });
33. });
34. } catch (err) {
35. hilog.error(0x0000, 'testTag', `Failed to read data. Code: ${err.code}, message: ${err.message}`);
36. }
```
...
| **参数名** | **类型** | 必填 | **说明** |
| --- | --- | --- | --- |
| context | [common.UIAbilityContext](<../../../Ability Kit（程序框架服务）/ArkTS API/接口依赖的元素及定义/application/UIAbilityContext/js-apis-inner-application-uiabilitycontext.md#uiabilitycontext-1>) | 是 | UIAbility上下文。 |
...
以下错误码的详细介绍请参见[运动健康服务ArkTS API错误码](<../../ArkTS API错误码/errorcode-healthservice.md>)，其他错误码请参见[华为账号服务ArkTS错误码](<../../../Account Kit（华为账号服务）/ArkTS错误码/account-api-error-code.md>)。
...
12. let authorizationResponse =
13. await healthStore.requestAuthorizations(this.getUIContext().getHostContext() as common.UIAbilityContext,
14. authorizationParameter);

16. hilog.info(0x0000, 'testTag', 'Succeeded in requesting authorization.');
17. authorizationResponse.writeDataTypes.forEach(dataType => {
18. hilog.info(0x0000, 'testTag', `grantedWriteDataType is : ${dataType.name}`);
19. });
20. authorizationResponse.readDataTypes.forEach(dataType => {
21. hilog.info(0x0000, 'testTag', `grantedReadDataTypes is : ${dataType.name}`);
22. });
23. } catch (err) {
24. hilog.error(0x0000, 'testTag', `Failed to request authorization. Code: ${err.code}, message: ${err.message}`);
25. }
```
...
| --- | --- |
| [201](<../../ArkTS API错误码/errorcode-healthservice.md#section201-鉴权失败>) | Permission verification failed. |
| [801](<../../ArkTS API错误码/errorcode-healthservice.md#section801-该设备不支持此api>) | Capability not supported. Failed to call the API due to limited device capabilities. |
| [1002700001](<../../ArkTS API错误码/errorcode-healthservice.md#section1002700001-系统内部错误>) | System internal error. |
| [1002700002](<../../ArkTS API错误码/errorcode-healthservice.md#section1002700002-数据库异常>) | Database processing error. |
| [1002701001](<../../ArkTS API错误码/errorcode-healthservice.md#section1002701001-网络错误>) | Network error. The network is unavailable. |
| [1002702001](<../../ArkTS API错误码/errorcode-healthservice.md#section1002702001-账号未登录>) | Account error. The user has not logged in with HUAWEI ID. |
