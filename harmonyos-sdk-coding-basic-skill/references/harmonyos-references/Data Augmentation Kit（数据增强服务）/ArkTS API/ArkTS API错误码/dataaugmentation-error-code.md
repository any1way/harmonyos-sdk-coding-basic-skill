...
scraped_at: 2026-06-01T15:58:07+08:00
doc_updated_at: 2026-04-20
content_hash: sha256:dd4cf8e92b2e800d40926af21ef240200dde30a38d9cddc5e105b488d20d9433
---
说明
...
1. 请开发者重新调用[streamRun](../rag（检索增强生成）/dataaugmentation-rag-api.md#streamrun)。
2. 多次重试均失败则需请开发者优化[streamChat](../rag（检索增强生成）/dataaugmentation-rag-api.md#streamchat)中使用的大语言模型及相应实现。
...
开发者在[streamChat](../rag（检索增强生成）/dataaugmentation-rag-api.md#streamchat)中请求大语言模型时失败。
...
开发者在[streamChat](../rag（检索增强生成）/dataaugmentation-rag-api.md#streamchat)中请求大语言模型时大语言模型繁忙。
...
开发者在[streamChat](../rag（检索增强生成）/dataaugmentation-rag-api.md#streamchat)中的某一次大语言模型请求时，累计转发给rag的响应数据长度超出范围8192字节。
...
[RagSession](../rag（检索增强生成）/dataaugmentation-rag-api.md#ragsession)未关闭，重复创建[RagSession](../rag（检索增强生成）/dataaugmentation-rag-api.md#ragsession)。

**处理步骤**

如果[Config](../rag（检索增强生成）/dataaugmentation-rag-api.md#config)没有变化，可以使用之前的[RagSession](../rag（检索增强生成）/dataaugmentation-rag-api.md#ragsession)继续对话，否则，请先使用[close](../rag（检索增强生成）/dataaugmentation-rag-api.md#close)接口关闭旧的[RagSession](../rag（检索增强生成）/dataaugmentation-rag-api.md#ragsession)，再使用新的[Config](../rag（检索增强生成）/dataaugmentation-rag-api.md#config)调用[createRagSession](../rag（检索增强生成）/dataaugmentation-rag-api.md#createragsession)接口创建新的[RagSession](../rag（检索增强生成）/dataaugmentation-rag-api.md#ragsession)进行对话。
...
前一次[streamRun](../rag（检索增强生成）/dataaugmentation-rag-api.md#streamrun)回答期间重新调用了[streamRun](../rag（检索增强生成）/dataaugmentation-rag-api.md#streamrun)开启新一轮问答。

**处理步骤**

如果开发者希望保留前一次回答结果，请等待前一次[streamRun](../rag（检索增强生成）/dataaugmentation-rag-api.md#streamrun)结束，再重新调用[streamRun](../rag（检索增强生成）/dataaugmentation-rag-api.md#streamrun)进行新一轮问答。
...
调用[RagSession](../rag（检索增强生成）/dataaugmentation-rag-api.md#ragsession)的接口时，[RagSession](../rag（检索增强生成）/dataaugmentation-rag-api.md#ragsession)已关闭。

**处理步骤**

调用[RagSession](../rag（检索增强生成）/dataaugmentation-rag-api.md#ragsession)的接口前请确认存在打开的[RagSession](../rag（检索增强生成）/dataaugmentation-rag-api.md#ragsession)。
...
用户已取消[streamRun](../rag（检索增强生成）/dataaugmentation-rag-api.md#streamrun)。

**可能原因**

调用[streamRun](../rag（检索增强生成）/dataaugmentation-rag-api.md#streamrun)接口过程中，用户主动取消了本次[streamRun](../rag（检索增强生成）/dataaugmentation-rag-api.md#streamrun)。

**处理步骤**

请用户按需取消[streamRun](../rag（检索增强生成）/dataaugmentation-rag-api.md#streamrun)。
...
请开发者尝试重试[streamRun](../rag（检索增强生成）/dataaugmentation-rag-api.md#streamrun)。
...
开发者调用[createRagSession](../rag（检索增强生成）/dataaugmentation-rag-api.md#createragsession)时传入的[Config](../rag（检索增强生成）/dataaugmentation-rag-api.md#config)不满足约束条件。

**处理步骤**

请开发者查阅API参考中对各类型的描述，结合实际业务需要配置正确[Config](../rag（检索增强生成）/dataaugmentation-rag-api.md#config)。
...
1. 使用rag能力前请确认已配置[知识加工](<../../../../harmonyos-guides/应用框架/Data Augmentation Kit（数据增强服务）/知识加工/data-augmentation-knowledge-processing.md>)schema。
...
1. 确保主键数量不超过算法允许的限制。
2. 修正[Config](../rag（检索增强生成）/dataaugmentation-rag-api.md#config)后重试。
...
1. 参照[知识源配置](../knowledgeProcessor（知识加工）/dataaugmentation-knowledgeprocessor-api.md#knowledgesourceconfig)类型，正确配置rdbSource字段。
...
1. 参考开发步骤中的[知识schema配置示例](<../../../../harmonyos-guides/应用框架/Data Augmentation Kit（数据增强服务）/知识加工/data-augmentation-knowledge-processing.md#开发步骤>)，将knowledge\_schema.json放到指定的entry/src/main/resources/rawfile/arkdata/knowledge目录下。
...
1. 参#fig照[知识schema规格及约束限制](<../../../../harmonyos-guides/应用框架/Data Augmentation Kit（数据增强服务）/知识加工/data-augmentation-knowledge-processing.md#约束限制>)修改其中内容。
