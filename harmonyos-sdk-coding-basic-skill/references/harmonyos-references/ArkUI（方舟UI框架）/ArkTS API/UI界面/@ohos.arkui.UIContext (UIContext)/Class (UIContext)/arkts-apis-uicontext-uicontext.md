...
scraped_at: 2026-06-11T15:42:10+08:00
doc_updated_at: 2026-05-19
content_hash: sha256:e9373466d3b67ec194c3c2accf88df072b637fcde013b4b823d5c7906531c95f
---
UIContext实例对象。
...
* 以下API需要通过对应的UIContext实例调用。获取UIContext分为三种方式，第一种是使用ohos.window中的[getUIContext()](<../../../窗口管理/@ohos.window (窗口)/Interface (Window)/arkts-apis-window-window.md#getuicontext10>)方法获取UIContext实例，第二种是通过自定义组件内置方法[getUIContext()](../../../../ArkTS组件/自定义组件/自定义组件内置方法/ts-custom-component-api.md#getuicontext)获取UIContext实例，第三种是通过UIContext类的静态方法如[getCallingScopeUIContext](arkts-apis-uicontext-uicontext.md#getcallingscopeuicontext22)获取UIContext实例。本文中UIContext对象以uiContext表示。
...
获取当前[调用作用域](<../../../../../../harmonyos-guides/应用框架/ArkUI（方舟UI框架）/UI开发 (ArkTS声明式开发范式)/UI系统场景化能力/使用UI上下文接口操作界面（UIContext）/arkts-global-interface.md#基本概念>)的UIContext，调用作用域不明确时返回undefined。
...
| 类型 | 说明 |
| --- | --- |
| UIContext | undefined | 当前[调用作用域](<../../../../../../harmonyos-guides/应用框架/ArkUI（方舟UI框架）/UI开发 (ArkTS声明式开发范式)/UI系统场景化能力/使用UI上下文接口操作界面（UIContext）/arkts-global-interface.md#基本概念>)的UIContext，调用作用域不明确时返回undefined。 |
...
| 类型 | 说明 |
| --- | --- |
| [ResolvedUIContext](<../Class (ResolvedUIContext)/arkts-apis-uicontext-resolveduicontext.md>) | 返回带有解析策略的UIContext实例对象。 |
...
判断UIContext对象对应的UI实例是否有效。使用[getUIContext](<../../../窗口管理/@ohos.window (窗口)/Interface (Window)/arkts-apis-window-window.md#getuicontext10>)方法获取UIContext对象。后端UI实例存在时，该UI实例有效。通过new UIContext()创建的UIContext对象无对应的UI实例；多次[loadContent](<../../../窗口管理/@ohos.window (窗口)/Interface (Window)/arkts-apis-window-window.md#loadcontent9>)后，旧的UI实例会失效。多窗口应用场景，当窗口关闭后，该窗口的UI实例失效。总而言之，当UIContext对象没有对应的后端UI实例时，该对象是无效的。
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/f3/v3/WFvxAhaGTICRbjzJUavp4g/zh-cn_image_0000002592219876.gif?HW-CC-KV=V1&HW-CC-Date=20260611T074206Z&HW-CC-Expire=86400&HW-CC-Sign=2F15D5776CF4E27A71C33180747EB951AC023E29A8D744E8F6952BC7FF246B02)
...
| 类型 | 说明 |
| --- | --- |
| [Font](<../Class (Font)/arkts-apis-uicontext-font.md>) | 返回Font实例对象。 |

**示例：**

完整示例请参考[Font](<../Class (Font)/arkts-apis-uicontext-font.md>)中的示例。
...
| 类型 | 说明 |
| --- | --- |
| [ComponentUtils](<../Class (ComponentUtils)/arkts-apis-uicontext-componentutils.md>) | 返回ComponentUtils实例对象。 |

**示例：**

完整示例请参考[示例1（获取ComponentUtils对象）](<../../@ohos.arkui.componentUtils (componentUtils)/js-apis-arkui-componentutils.md#示例1获取componentutils对象>)。
...
| 类型 | 说明 |
| --- | --- |
| [UIInspector](<../Class (UIInspector)/arkts-apis-uicontext-uiinspector.md>) | 返回UIInspector实例对象。 |

**示例：**

完整示例请参考[UIInspector](<../Class (UIInspector)/arkts-apis-uicontext-uiinspector.md>)中的示例。
...
| 类型 | 说明 |
| --- | --- |
| [UIObserver](<../Class (UIObserver)/arkts-apis-uicontext-uiobserver.md>) | 返回UIObserver实例对象。 |
...
| 类型 | 说明 |
| --- | --- |
| [MediaQuery](<../Class (MediaQuery)/arkts-apis-uicontext-mediaquery.md>) | 返回MediaQuery实例对象。 |
...
| 类型 | 说明 |
| --- | --- |
| [Router](<../Class (Router)/arkts-apis-uicontext-router.md>) | 返回Router实例对象。 |

**示例：**

完整示例请参考[pushUrl](<../Class (Router)/arkts-apis-uicontext-router.md#pushurl>)。
...
| 类型 | 说明 |
| --- | --- |
| [PromptAction](<../Class (PromptAction)/arkts-apis-uicontext-promptaction.md>) | 返回PromptAction实例对象。 |

**示例：**

完整示例请参考[PromptAction](<../Class (PromptAction)/arkts-apis-uicontext-promptaction.md>)中的示例。
...
| 类型 | 说明 |
| --- | --- |
| [OverlayManager](<../Class (OverlayManager)/arkts-apis-uicontext-overlaymanager.md>) | 返回OverlayManager实例对象。 |

**示例：**

完整示例请参考[OverlayManager](<../Class (OverlayManager)/arkts-apis-uicontext-overlaymanager.md>)中的示例。
...
设置[OverlayManager](<../Class (OverlayManager)/arkts-apis-uicontext-overlaymanager.md>)参数。用于在使用OverlayManager能力之前先初始化overlayManager的参数，包括是否需要渲染overlay根节点等属性。该方法需要在执行getOverlayManager方法之前执行生效，且该方法只生效一次。
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| options | [OverlayManagerOptions](<../Interfaces (其他)/arkts-apis-uicontext-i.md#overlaymanageroptions15>) | 是 | OverlayManager参数。 |
...
用于获取当前[OverlayManagerOptions](<../Interfaces (其他)/arkts-apis-uicontext-i.md#overlaymanageroptions15>)参数。
...
| 类型 | 说明 |
| --- | --- |
| [OverlayManagerOptions](<../Interfaces (其他)/arkts-apis-uicontext-i.md#overlaymanageroptions15>) | 返回当前OverlayManagerOptions。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| param | [AnimateParam](<../../../../ArkTS组件/动画/显式动画 (animateTo)/ts-explicit-animation.md#animateparam对象说明>) | 是 | 设置动画效果相关参数。 |
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/0b/v3/0ZiIl9AJTjCSMFMEIqLfug/zh-cn_image_0000002592379810.gif?HW-CC-KV=V1&HW-CC-Date=20260611T074206Z&HW-CC-Expire=86400&HW-CC-Sign=347610FDE836E0443FF0919BFFCCA9AD0F275C07E3EA68B0F174CE4BA8B2EFE7)
...
* 如果在[aboutToAppear](../../../../ArkTS组件/自定义组件/自定义组件的生命周期/ts-custom-component-lifecycle.md#abouttoappear)中调用动画，自定义组件内的build还未执行，内部组件还未创建，动画时机过早，动画属性没有初值无法对组件产生动画。
* 执行[aboutToDisappear](../../../../ArkTS组件/自定义组件/自定义组件的生命周期/ts-custom-component-lifecycle.md#abouttodisappear)时，组件即将销毁，不能在aboutToDisappear里面做动画。
* 在组件出现和消失时，可以通过[组件内转场](<../../../../ArkTS组件/动画/组件内转场 (transition)/ts-transition-animation-component.md>)添加动画效果。
* 组件内转场不支持的属性，可以参考[显式动画](<../../../../ArkTS组件/动画/显式动画 (animateTo)/ts-explicit-animation.md>)中的[示例2](<../../../../ArkTS组件/动画/显式动画 (animateTo)/ts-explicit-animation.md#示例2动画执行结束后组件消失>)，使用animateTo实现动画执行结束后组件消失的效果。
* 某些场景下，在[状态管理V2](<../../../../../../harmonyos-guides/应用框架/ArkUI（方舟UI框架）/UI开发 (ArkTS声明式开发范式)/学习UI范式状态管理/状态管理概述/arkts-state-management-overview.md#状态管理v2>)中使用animateTo动画，会产生异常效果，具体可参考：[在状态管理V2中使用animateTo动画效果异常](<../../../../../../harmonyos-guides/应用框架/ArkUI（方舟UI框架）/UI开发 (ArkTS声明式开发范式)/学习UI范式状态管理/状态管理（V2）/管理组件拥有的状态/@Local装饰器：组件内部状态/arkts-new-local.md#在状态管理v2中使用animateto动画效果异常>)。
* UIAbility从前台切换至后台时会立即结束仍在步进中的有限循环动画，从而触发动画播放完成回调[onFinish](<../../../../ArkTS组件/动画/显式动画 (animateTo)/ts-explicit-animation.md#animateparam对象说明>)。
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| value | [AnimateParam](<../../../../ArkTS组件/动画/显式动画 (animateTo)/ts-explicit-animation.md#animateparam对象说明>) | 是 | 设置动画效果相关参数。 |
...
| 类型 | 说明 |
| --- | --- |
| [LocalStorage](../../../../ArkTS组件/状态管理与渲染控制/应用级变量的状态管理/ts-state-management.md#localstorage9) | undefined | 返回LocalStorage实例。共享的LocalStorage实例不存在时返回undefined。 |
...
| 类型 | 说明 |
| --- | --- |
| [Context](../Types/arkts-apis-uicontext-t.md#context12) | undefined | 返回当前组件所在Ability的Context，Context的具体类型为当前Ability关联的Context对象。例如：在UIAbility窗口中的页面调用该接口，返回类型为[UIAbilityContext](<../../../../../Ability Kit（程序框架服务）/ArkTS API/接口依赖的元素及定义/application/UIAbilityContext/js-apis-inner-application-uiabilitycontext.md#uiabilitycontext-1>)。在ExtensionAbility窗口中的页面调用该接口，返回类型为[ExtensionContext](<../../../../../Ability Kit（程序框架服务）/ArkTS API/接口依赖的元素及定义/application/ExtensionContext/js-apis-inner-application-extensioncontext.md>)。ability上下文不存在时返回undefined。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| id | string | 是 | 节点对应的[组件标识](../../../../ArkTS组件/通用属性/基础属性/组件标识/ts-universal-attributes-component-id.md)。 |

**返回值：**

| 类型 | 说明 |
| --- | --- |
| [FrameNode](../../arkui/FrameNode/js-apis-arkui-framenode.md) | null | 返回的组件树的实体节点或者空节点。 |
...
完整示例请参考[获取根节点示例](../../arkui/FrameNode/js-apis-arkui-framenode.md#获取根节点示例)。
...
2. 当uniqueId对应的是自定义组件时：
   * 若其有渲染内容，且没有被[@Reusable装饰器](<../../../../../../harmonyos-guides/应用框架/ArkUI（方舟UI框架）/UI开发 (ArkTS声明式开发范式)/学习UI范式基本语法/自定义组件/自定义组件复用/@Reusable装饰器：V1组件复用/arkts-reusable.md>)修饰时，返回该自定义组件的根节点，类型为\_\_Common\_\_。
   * 若其无渲染内容，或者被[@Reusable装饰器](<../../../../../../harmonyos-guides/应用框架/ArkUI（方舟UI框架）/UI开发 (ArkTS声明式开发范式)/学习UI范式基本语法/自定义组件/自定义组件复用/@Reusable装饰器：V1组件复用/arkts-reusable.md>)修饰时，在该自定义组件的子组件创建完成前调用此接口，将返回null；在该自定义组件的子组件创建完成后调用，返回其第一个子组件的FrameNode。
...
4. 模态弹窗并不在任何Page节点中。当uniqueId对应的节点在模态弹窗中，例如[CustomDialog](<../../../../ArkTS组件/弹窗/自定义弹窗 (CustomDialog)/ts-methods-custom-dialog-box.md>)、[bindSheet](../../../../ArkTS组件/通用属性/模态转场设置/半模态转场/ts-universal-attributes-sheet-transition.md#bindsheet)和[bindContentCover](../../../../ArkTS组件/通用属性/模态转场设置/全屏模态转场/ts-universal-attributes-modal-transition.md#bindcontentcover)构建的模态页面中，routerPageInfo属性为undefined。
...
| 类型 | 说明 |
| --- | --- |
| [PageInfo](<../Interfaces (其他)/arkts-apis-uicontext-i.md#pageinfo12>) | 返回节点对应的Router和NavDestination信息。 |
...
| 类型 | 说明 |
| --- | --- |
| observer.[NavigationInfo](<../../@ohos.arkui.observer (无感监听)/js-apis-arkui-observer.md#navigationinfo12>) | undefined | 返回节点对应的Navigation信息。 |
...
不支持在输入法类型窗口中使用子窗（[showInSubWindow](<../../../../ArkTS组件/弹窗/警告弹窗 (AlertDialog)/ts-methods-alert-dialog-box.md#alertdialogparam对象说明>) 为true）的showAlertDialog，详情见输入法框架的约束与限制说明[createPanel](<../../../../../IME Kit（输入法开发服务）/ArkTS API/@ohos.inputMethodEngine (输入法服务)/js-apis-inputmethodengine.md#createpanel10-1>)。
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| options | [AlertDialogParamWithConfirm](<../../../../ArkTS组件/弹窗/警告弹窗 (AlertDialog)/ts-methods-alert-dialog-box.md#alertdialogparamwithconfirm对象说明>) | [AlertDialogParamWithButtons](<../../../../ArkTS组件/弹窗/警告弹窗 (AlertDialog)/ts-methods-alert-dialog-box.md#alertdialogparamwithbuttons对象说明>) | [AlertDialogParamWithOptions](<../../../../ArkTS组件/弹窗/警告弹窗 (AlertDialog)/ts-methods-alert-dialog-box.md#alertdialogparamwithoptions10对象说明>) | 是 | 定义并显示AlertDialog组件。 |
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/a6/v3/77C5NukYSzeWyfW1k6GzVw/zh-cn_image_0000002622859319.gif?HW-CC-KV=V1&HW-CC-Date=20260611T074206Z&HW-CC-Expire=86400&HW-CC-Sign=E0607274A367CA5675AA05C83A5CD075A4A113317AB34FC57288DB5DDBF6A227)
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| value | [ActionSheetOptions](<../../../../ArkTS组件/弹窗/列表选择弹窗 (ActionSheet)/ts-methods-action-sheet.md#actionsheetoptions对象说明>) | 是 | 配置列表弹窗的参数。 |
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/a7/v3/Yvoa2kcQSuaeqHULqexttA/zh-cn_image_0000002622699439.gif?HW-CC-KV=V1&HW-CC-Date=20260611T074206Z&HW-CC-Expire=86400&HW-CC-Sign=96627E5CCD1E469F97FCAD447072579424B6415D6900CEC569C1BFF5D9B650BF)
...
不支持在输入法类型窗口中使用子窗（showInSubwindow为true）的showDatePickerDialog，详情见输入法框架的约束与限制说明[createPanel](<../../../../../IME Kit（输入法开发服务）/ArkTS API/@ohos.inputMethodEngine (输入法服务)/js-apis-inputmethodengine.md#createpanel10-1>)。
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| options | [DatePickerDialogOptions](<../../../../ArkTS组件/弹窗/日期滑动选择器弹窗 (DatePickerDialog)/ts-methods-datepicker-dialog.md#datepickerdialogoptions对象说明>) | 是 | 配置日期滑动选择器弹窗的参数。 |
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/48/v3/UR46hCwYSHm2GodtRTYXxQ/zh-cn_image_0000002592219878.gif?HW-CC-KV=V1&HW-CC-Date=20260611T074206Z&HW-CC-Expire=86400&HW-CC-Sign=D4334574AB0EF19F7BA5082BE3D42AA75EAE433D37395EF2D10823BA6021174C)
...
不支持在输入法类型窗口中使用子窗（showInSubwindow为true）的showTimePickerDialog，详情见输入法框架的约束与限制说明[createPanel](<../../../../../IME Kit（输入法开发服务）/ArkTS API/@ohos.inputMethodEngine (输入法服务)/js-apis-inputmethodengine.md#createpanel10-1>)。
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| options | [TimePickerDialogOptions](<../../../../ArkTS组件/弹窗/时间滑动选择器弹窗 (TimePickerDialog)/ts-methods-timepicker-dialog.md#timepickerdialogoptions对象说明>) | 是 | 配置时间滑动选择器弹窗的参数。 |
...
不支持在输入法类型窗口中使用子窗（showInSubwindow为true）的showTextPickerDialog，详情见输入法框架的约束与限制说明[createPanel](<../../../../../IME Kit（输入法开发服务）/ArkTS API/@ohos.inputMethodEngine (输入法服务)/js-apis-inputmethodengine.md#createpanel10-1>)。
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| options | [TextPickerDialogOptions](<../../../../ArkTS组件/弹窗/文本滑动选择器弹窗 (TextPickerDialog)/ts-methods-textpicker-dialog.md#textpickerdialogoptions对象说明>) | 是 | 配置文本滑动选择器弹窗的参数。 |
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/8f/v3/sDhHCACwQoyd06XcV0jrwg/zh-cn_image_0000002592379812.gif?HW-CC-KV=V1&HW-CC-Date=20260611T074206Z&HW-CC-Expire=86400&HW-CC-Sign=33EF3F966609073FC2F347A5FC796FC5094132664561E7F4B1F48A56040FFFFC)
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| style | [TextPickerDialogOptions](<../../../../ArkTS组件/弹窗/文本滑动选择器弹窗 (TextPickerDialog)/ts-methods-textpicker-dialog.md#textpickerdialogoptions对象说明>)| [TextPickerDialogOptionsExt](<../../../../ArkTS组件/弹窗/文本滑动选择器弹窗 (TextPickerDialog)/ts-methods-textpicker-dialog.md#textpickerdialogoptionsext20对象说明>) | 是 | 配置文本滑动选择器弹窗的参数。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| options | [AnimatorOptions](<../../@ohos.animator (动画)/js-apis-animator.md#animatoroptions>) | 是 | 定义动画选项。 |

**返回值：**

| 类型 | 说明 |
| --- | --- |
| [AnimatorResult](<../../@ohos.animator (动画)/js-apis-animator.md#animatorresult>) | Animator结果接口。 |
...
创建animator动画结果对象（AnimatorResult）。与[createAnimator](arkts-apis-uicontext-uicontext.md#createanimator)相比，新增对[SimpleAnimatorOptions](<../../@ohos.animator (动画)/js-apis-animator.md#simpleanimatoroptions18>)类型入参的支持。
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| options | [AnimatorOptions](<../../@ohos.animator (动画)/js-apis-animator.md#animatoroptions>) | [SimpleAnimatorOptions](<../../@ohos.animator (动画)/js-apis-animator.md#simpleanimatoroptions18>) | 是 | 定义动画选项。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| value | [KeyboardAvoidMode](../Enums/arkts-apis-uicontext-e.md#keyboardavoidmode11) | 是 | 键盘弹出时的页面避让模式。  默认值：KeyboardAvoidMode.OFFSET |
...
setKeyboardAvoidMode针对页面生效，对于弹窗类组件不生效，比如Dialog、Popup、Menu、BindSheet、BindContentCover、Toast、OverlayManager。弹窗类组件的避让模式可以参考[CustomDialogControllerOptions对象说明](<../../../../ArkTS组件/弹窗/自定义弹窗 (CustomDialog)/ts-methods-custom-dialog-box.md#customdialogcontrolleroptions对象说明>)。

**示例：**

完整示例请参考[示例4（设置键盘避让模式为压缩）](../../../../ArkTS组件/通用属性/布局与边框/安全区域/ts-universal-attributes-expand-safe-area.md#示例4设置键盘避让模式为压缩)、[示例5（设置键盘避让模式为上抬）](../../../../ArkTS组件/通用属性/布局与边框/安全区域/ts-universal-attributes-expand-safe-area.md#示例5设置键盘避让模式为上抬)以及[示例6（切换避让模式）](../../../../ArkTS组件/通用属性/布局与边框/安全区域/ts-universal-attributes-expand-safe-area.md#示例6切换避让模式)。
...
| 类型 | 说明 |
| --- | --- |
| [KeyboardAvoidMode](../Enums/arkts-apis-uicontext-e.md#keyboardavoidmode11) | 返回当前的页面避让模式。 |
...
| 类型 | 说明 |
| --- | --- |
| Nullable<[AtomicServiceBar](<../Interface (AtomicServiceBar)/arkts-apis-uicontext-atomicservicebar.md>)> | 如果是元服务则返回AtomicServerBar类型，否则返回undefined。 |
...
| 类型 | 说明 |
| --- | --- |
| [DragController](<../Class (DragController)/arkts-apis-uicontext-dragcontroller.md>) | 获取DragController对象。 |

**示例：**

完整示例请参考[DragController](<../Class (DragController)/arkts-apis-uicontext-dragcontroller.md>)中的示例。
...
产生关键帧动画。该接口的使用说明请参考[keyframeAnimateTo](<../../../../ArkTS组件/动画/关键帧动画 (keyframeAnimateTo)/ts-keyframeanimateto.md>)。
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| param | [KeyframeAnimateParam](<../../../../ArkTS组件/动画/关键帧动画 (keyframeAnimateTo)/ts-keyframeanimateto.md#keyframeanimateparam对象说明>) | 是 | 关键帧动画的整体动画参数。 |
| keyframes | Array<[KeyframeState](<../../../../ArkTS组件/动画/关键帧动画 (keyframeAnimateTo)/ts-keyframeanimateto.md#keyframestate对象说明>)> | 是 | 所有的关键帧状态的列表。 |
...
获取[FocusController](<../Class (FocusController)/arkts-apis-uicontext-focuscontroller.md>)对象，可通过该对象控制焦点。
...
| 类型 | 说明 |
| --- | --- |
| [FocusController](<../Class (FocusController)/arkts-apis-uicontext-focuscontroller.md>) | 获取FocusController对象。 |

**示例：**

完整示例请参考[FocusController](<../Class (FocusController)/arkts-apis-uicontext-focuscontroller.md>)中的示例。
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| id | string | 是 | 指定的[组件标识](../../../../ArkTS组件/通用属性/基础属性/组件标识/ts-universal-attributes-component-id.md)id。 |
...
| 类型 | 说明 |
| --- | --- |
| string | 获取指定的组件及其子组件的属性的JSON字符串。组件中每个字段的含义请参考[getInspectorInfo](../../arkui/FrameNode/js-apis-arkui-framenode.md#getinspectorinfo12)的返回值说明。 |
...
获取[CursorController](<../Class (CursorController)/arkts-apis-uicontext-cursorcontroller.md>)对象，可通过该对象控制光标。
...
| 类型 | 说明 |
| --- | --- |
| [CursorController](<../Class (CursorController)/arkts-apis-uicontext-cursorcontroller.md>) | 获取CursorController对象。 |

**示例：**

完整示例请参考[CursorController](<../Class (CursorController)/arkts-apis-uicontext-cursorcontroller.md>)中的示例。
...
获取[ContextMenuController](<../Class (ContextMenuController)/arkts-apis-uicontext-contextmenucontroller.md>)对象，可通过该对象控制菜单。
...
| 类型 | 说明 |
| --- | --- |
| [ContextMenuController](<../Class (ContextMenuController)/arkts-apis-uicontext-contextmenucontroller.md>) | 获取ContextMenuController对象。 |
...
| 类型 | 说明 |
| --- | --- |
| [MeasureUtils](<../Class (MeasureUtils)/arkts-apis-uicontext-measureutils.md>) | 提供文本宽度、高度等相关计算。 |

**示例：**

完整示例请参考[MeasureUtils](<../Class (MeasureUtils)/arkts-apis-uicontext-measureutils.md>)中的示例。
...
典型使用场景（如长截图）及最佳实践请参考[使用组件截图](<../../../../../../harmonyos-guides/应用框架/ArkUI（方舟UI框架）/UI开发 (ArkTS声明式开发范式)/UI系统场景化能力/使用组件截图（ComponentSnapshot）/arkts-uicontext-component-snapshot.md>)。
...
| 类型 | 说明 |
| --- | --- |
| [ComponentSnapshot](<../Class (ComponentSnapshot)/arkts-apis-uicontext-componentsnapshot.md>) | 获取ComponentSnapshot对象。 |

**示例：**

完整示例请参考[ComponentSnapshot](<../Class (ComponentSnapshot)/arkts-apis-uicontext-componentsnapshot.md>)中的示例。
...
像素密度：当前窗口生效的像素密度值，即虚拟屏幕的密度[VirtualScreenConfig](<../../../屏幕管理/@ohos.display (屏幕属性)/js-apis-display.md#virtualscreenconfig16>).density。

说明

1. getUIContext需在windowStage.[loadContent](<../../../窗口管理/@ohos.window (窗口)/Interface (WindowStage)/arkts-apis-window-windowstage.md#loadcontent9>)之后调用，确保UIContext初始化完成后调用此接口，否则无法返回准确结果。
2. UI实例未创建时，[像素单位](../../../../ArkTS组件/公共定义/像素单位/ts-pixel-units.md)中的vp2px接口使用默认屏幕的虚拟像素比进行转换。在该场景下，开发者使用UIContext接口替换时，可参考[像素单位转换接口替换为UIContext接口](<../../../../../../harmonyos-guides/应用框架/ArkUI（方舟UI框架）/UI开发 (ArkTS声明式开发范式)/UI系统场景化能力/使用UI上下文接口操作界面（UIContext）/arkts-global-interface.md#像素单位转换接口替换为uicontext接口>)。
...
1. getUIContext需在windowStage.[loadContent](<../../../窗口管理/@ohos.window (窗口)/Interface (WindowStage)/arkts-apis-window-windowstage.md#loadcontent9>)之后调用，确保UIContext初始化完成后调用此接口，否则无法返回准确结果。
2. UI实例未创建时，[像素单位](../../../../ArkTS组件/公共定义/像素单位/ts-pixel-units.md)中的px2vp接口使用默认屏幕的虚拟像素比进行转换。在该场景下，开发者使用UIContext接口替换时，可参考[像素单位转换接口替换为UIContext接口](<../../../../../../harmonyos-guides/应用框架/ArkUI（方舟UI框架）/UI开发 (ArkTS声明式开发范式)/UI系统场景化能力/使用UI上下文接口操作界面（UIContext）/arkts-global-interface.md#像素单位转换接口替换为uicontext接口>)。
...
字体缩放比例：系统设置的字体缩放系数，对应 [Configuration.fontScale](../../../../ArkTS组件/公共定义/基础类型定义/ts-types.md#configuration)。

说明

getUIContext需在windowStage.[loadContent](<../../../窗口管理/@ohos.window (窗口)/Interface (WindowStage)/arkts-apis-window-windowstage.md#loadcontent9>)之后调用，确保UIContext初始化完成后调用此接口，否则无法返回准确结果。
...
转换公式为：px值 = lpx值 × 实际屏幕宽度与逻辑宽度（通过[designWidth](../../../../../../harmonyos-guides/基础入门/开发基础知识/应用配置文件（Stage模型）/module.json5配置文件/module-configuration-file.md#pages标签)配置）的比值。
...
转换公式为：lpx值 = px值 ÷ 实际屏幕宽度与逻辑宽度（通过[designWidth](../../../../../../harmonyos-guides/基础入门/开发基础知识/应用配置文件（Stage模型）/module.json5配置文件/module-configuration-file.md#pages标签)配置）的比值。
...
若UIContext位于主应用程序进程中的[UIExtensionAbility](<../../../../../Ability Kit（程序框架服务）/ArkTS API/Stage模型能力的接口/@ohos.app.ability.UIExtensionAbility (带界面的ExtensionAbility组件)/js-apis-app-abili_64670486.md>)内，则返回主应用程序的顶层窗口ID。
...
获取当前实例所在窗口的宽度断点枚举值。具体枚举值根据窗口宽度vp值确定，详见 [WidthBreakpoint](../../../../ArkTS组件/公共定义/枚举说明/ts-appendix-enums.md#widthbreakpoint13)。
...
| 类型 | 说明 |
| --- | --- |
| [WidthBreakpoint](../../../../ArkTS组件/公共定义/枚举说明/ts-appendix-enums.md#widthbreakpoint13) | 当前实例所在窗口的宽度断点枚举值。若窗口宽度为 0vp，则返回WIDTH\_XS。 |
...
获取当前实例所在窗口的高度断点。具体枚举值根据窗口高宽比确定，详见 [HeightBreakpoint](../../../../ArkTS组件/公共定义/枚举说明/ts-appendix-enums.md#heightbreakpoint13)。
...
| 类型 | 说明 |
| --- | --- |
| [HeightBreakpoint](../../../../ArkTS组件/公共定义/枚举说明/ts-appendix-enums.md#heightbreakpoint13) | 当前实例所在窗口的宽高比对应的高度断点枚举值。若窗口高宽比为0，则返回HEIGHT\_SM。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| frameCallback | [FrameCallback](<../Class (FrameCallback)/arkts-apis-uicontext-framecallback.md>) | 是 | 下一帧需要执行的回调。 |
...
以下错误码的详细介绍请参见[通用错误码](../../../../../通用错误码/errorcode-universal.md)和[半模态错误码](../../../../错误码/UI界面/半模态错误码/errorcode-bindsheet.md)错误码。
...
| --- | --- | --- | --- |
| bindSheetContent | [ComponentContent<T>](../../arkui/ComponentContent/js-apis-arkui-componentcontent.md) | 是 | 半模态页面中显示的组件内容。 |
| sheetOptions | [SheetOptions](../../../../ArkTS组件/通用属性/模态转场设置/半模态转场/ts-universal-attributes-sheet-transition.md#sheetoptions) | 是 | 半模态页面样式。  **说明：**  不支持更新SheetOptions.uiContext、SheetOptions.mode、回调函数。 |
...
参考[configuration标签](../../../../../../harmonyos-guides/基础入门/开发基础知识/应用配置文件（Stage模型）/app.json5配置文件/app-configuration-file.md#configuration标签)，配置fontSizeScale的值为“followSystem”。
...
参考[configuration标签](../../../../../../harmonyos-guides/基础入门/开发基础知识/应用配置文件（Stage模型）/app.json5配置文件/app-configuration-file.md#configuration标签)，配置fontSizeMaxScale的值为“1.75”。
...
绑定Tabs组件和可滚动容器组件（支持[List](../../../../ArkTS组件/滚动与滑动/List/ts-container-list.md)、[Scroll](../../../../ArkTS组件/滚动与滑动/Scroll/ts-container-scroll.md)、[Grid](../../../../ArkTS组件/滚动与滑动/Grid/ts-container-grid.md)、[WaterFlow](../../../../ArkTS组件/滚动与滑动/WaterFlow/ts-container-waterflow.md)），当滑动可滚动容器组件时，会触发所有与其绑定的Tabs组件的TabBar的显示和隐藏动效，上滑隐藏，下滑显示。一个TabsController可与多个Scroller绑定，一个Scroller也可与多个TabsController绑定。
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| tabsController | [TabsController](../../../../ArkTS组件/导航与切换/Tabs/ts-container-tabs.md#tabscontroller) | 是 | Tabs组件的控制器。 |
| scroller | [Scroller](../../../../ArkTS组件/滚动与滑动/Scroll/ts-container-scroll.md#scroller) | 是 | 可滚动容器组件的控制器。 |
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/c6/v3/ia6gQ6fMSmO8DONS2mFIGg/zh-cn_image_0000002622859321.gif?HW-CC-KV=V1&HW-CC-Date=20260611T074206Z&HW-CC-Expire=86400&HW-CC-Sign=DEC6A31507F6B9BE3D9AA49604F40EAE7410DA0F56F11BC00F138EEB0A51C350)
...
绑定Tabs组件和嵌套的可滚动容器组件（支持[List](../../../../ArkTS组件/滚动与滑动/List/ts-container-list.md)、[Scroll](../../../../ArkTS组件/滚动与滑动/Scroll/ts-container-scroll.md)、[Grid](../../../../ArkTS组件/滚动与滑动/Grid/ts-container-grid.md)、[WaterFlow](../../../../ArkTS组件/滚动与滑动/WaterFlow/ts-container-waterflow.md)），当滑动父组件或子组件时，会触发所有与其绑定的Tabs组件的TabBar的显示和隐藏动效，上滑隐藏，下滑显示。一个TabsController可与多个嵌套的Scroller绑定，嵌套的Scroller也可与多个TabsController绑定。
...
| --- | --- | --- | --- |
| tabsController | [TabsController](../../../../ArkTS组件/导航与切换/Tabs/ts-container-tabs.md#tabscontroller) | 是 | Tabs组件的控制器。 |
| parentScroller | [Scroller](../../../../ArkTS组件/滚动与滑动/Scroll/ts-container-scroll.md#scroller) | 是 | 可滚动容器组件的控制器。 |
| childScroller | [Scroller](../../../../ArkTS组件/滚动与滑动/Scroll/ts-container-scroll.md#scroller) | 是 | 可滚动容器组件的控制器。其对应组件为parentScroller对应组件的子组件，且组件间存在嵌套滚动关系。 |
...
获取[TextMenuController](<../Class (TextMenuController)/arkts-apis-uicontext-textmenucontroller.md>)对象，可通过该对象控制文本选择菜单。
...
| 类型 | 说明 |
| --- | --- |
| [TextMenuController](<../Class (TextMenuController)/arkts-apis-uicontext-textmenucontroller.md>) | 获取TextMenuController对象。 |

**示例：**

参考[TextMenuController](<../Class (TextMenuController)/arkts-apis-uicontext-textmenucontroller.md>)接口示例。
...
返回的UI上下文只可用于创建[自定义节点](<../../../../../../harmonyos-guides/应用框架/ArkUI（方舟UI框架）/UI开发 (ArkTS声明式开发范式)/使用自定义能力/自定义节点/自定义节点概述/arkts-user-defined-node.md>)，不能执行其他UI操作。
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| context | common.[UIAbilityContext](<../../../../../Ability Kit（程序框架服务）/ArkTS API/接口依赖的元素及定义/application/UIAbilityContext/js-apis-inner-application-uiabilitycontext.md>) | common.[ExtensionContext](<../../../../../Ability Kit（程序框架服务）/ArkTS API/接口依赖的元素及定义/application/ExtensionContext/js-apis-inner-application-extensioncontext.md>) | 是 | [UIAbility](<../../../../../Ability Kit（程序框架服务）/ArkTS API/Stage模型能力的接口/@ohos.app.ability.UIAbility (带界面的应用组件)/js-apis-app-ability-uiability.md>)或[ExtensionAbility](<../../../../../Ability Kit（程序框架服务）/ArkTS API/Stage模型能力的接口/@ohos.app.ability.ExtensionAbility (扩展能力基类)/js-apis-app-ability-extensionability.md>)所对应的上下文环境。 |
...
| event | [KeyEvent](../../../../ArkTS组件/通用事件/基础输入事件/按键事件/ts-universal-events-key.md#keyevent对象说明) | 是 | KeyEvent对象。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| mode | [PixelRoundMode](../../../../ArkTS组件/公共定义/枚举说明/ts-appendix-enums.md#pixelroundmode18) | 是 | 像素取整模式。  默认值：PixelRoundMode.PIXEL\_ROUND\_ON\_LAYOUT\_FINISH |
...
| 类型 | 说明 |
| --- | --- |
| [PixelRoundMode](../../../../ArkTS组件/公共定义/枚举说明/ts-appendix-enums.md#pixelroundmode18) | 当前页面的像素取整模式。 |
...
以下错误码的详细介绍请参见[UI上下文错误码](../../../../错误码/UI界面/UI上下文错误码/errorcode-uicontext.md)。
...
setImageCacheCount方法需要在@Entry标记的页面，[onPageShow](../../../../ArkTS组件/自定义组件/自定义组件的生命周期/ts-custom-component-lifecycle.md#onpageshow)或[aboutToAppear](../../../../ArkTS组件/自定义组件/自定义组件的生命周期/ts-custom-component-lifecycle.md#abouttoappear)里面设置才生效。
...
setImageRawDataCacheSize方法需要在@Entry标记的页面，[onPageShow](../../../../ArkTS组件/自定义组件/自定义组件的生命周期/ts-custom-component-lifecycle.md#onpageshow)或[aboutToAppear](../../../../ArkTS组件/自定义组件/自定义组件的生命周期/ts-custom-component-lifecycle.md#abouttoappear)里面设置才生效。
...
获取[Magnifier](<../Class (Magnifier)/arkts-apis-uicontext-magnifier.md>)对象，可控制放大镜显示和隐藏。
...
| 类型 | 说明 |
| --- | --- |
| [Magnifier](<../Class (Magnifier)/arkts-apis-uicontext-magnifier.md>) | Magnifier对象，可用于控制放大镜的显示和隐藏。 |

**示例：**

参考[Magnifier](<../Class (Magnifier)/arkts-apis-uicontext-magnifier.md>)的[bind](<../Class (Magnifier)/arkts-apis-uicontext-magnifier.md#bind>)接口示例。
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| feature | [CustomKeyboardContinueFeature](../Enums/arkts-apis-uicontext-e.md#customkeyboardcontinuefeature23) | 是 | 自定义键盘之间切换时是否接续。  默认值：CustomKeyboardContinueFeature.DISABLED，即不接续。 |
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/f3/v3/APb85PtdQ9SSPftO-qgwqg/zh-cn_image_0000002622699441.gif?HW-CC-KV=V1&HW-CC-Date=20260611T074206Z&HW-CC-Expire=86400&HW-CC-Sign=1B68FD06C7E457DE5ADDA7AE02A55F710D800DDCB88C52D6F96E35F57AE17005)
...
| 类型 | 说明 |
| --- | --- |
| [FrameNode](../../arkui/FrameNode/js-apis-arkui-framenode.md#framenode-1) | null | 返回页面的根节点的实体节点或者空节点。  若无有效的FrameNode则返回null。  若窗口内无加载完成的页面则返回null。 |

**错误码：**

以下错误码详细介绍请参考[UI上下文错误码](../../../../错误码/UI界面/UI上下文错误码/errorcode-uicontext.md)。
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/7a/v3/OxzVgO1uQNuoQ9cCuJ2Zjw/zh-cn_image_0000002592219880.jpg?HW-CC-KV=V1&HW-CC-Date=20260611T074206Z&HW-CC-Expire=86400&HW-CC-Sign=8254DCCD150BD64662E1D0022825C752B8FE7ABAB4AF38EC0F356DD8CDE1C315)
