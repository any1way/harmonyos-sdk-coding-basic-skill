...
scraped_at: 2026-06-11T15:41:37+08:00
doc_updated_at: 2026-06-05
content_hash: sha256:fb85e5a9d66e40fb8c55b9fee7b8c45a78ca553eb086fb68e3285f3d370e1c8c
---
...
4. @Component
5. struct Index {
6. options: AnimationOptions = { duration: 2000, iterations: 1 };
7. // $r('app.media.gif')需要替换为开发者所需的图像资源文件。
8. @State drawable: DrawableDescriptor = new AnimatedDrawableDescriptor($r('app.media.gif'), this.options);
9. @State result: string = '';

11. aboutToAppear() {
12. // 在页面显示前提前加载资源到内存中，加快Image组件渲染速度。
13. // 使用loadSync同步加载：
14. // let loadResult: DrawableDescriptorLoadedResult = this.drawable.loadSync()
15. // 使用load异步加载：
16. this.drawable.load().then((loadResult: DrawableDescriptorLoadedResult) => {
17. this.result = `width: ${loadResult.imageWidth}, height: ${loadResult.imageHeight}`
18. console.info(`load result = ${JSON.stringify(loadResult)}`)
19. }).catch(() => {
20. console.error("load failed")
21. })
22. }

24. build() {
25. Column() {
26. Image(this.drawable)
27. .width(100)
28. .height(100)
29. Text(this.result)
30. }
...
父类对象提供可重写的方法，包含：获取[PixelMap](<../../../../Image Kit（图片处理服务）/ArkTS API/@ohos.multimedia.image (图片处理)/Interface (PixelMap)/arkts-apis-image-pixelmap.md>)实例，图片资源加载能力。
...
| 类型 | 说明 |
| --- | --- |
| [image.PixelMap](<../../../../Image Kit（图片处理服务）/ArkTS API/@ohos.multimedia.image (图片处理)/Interface (PixelMap)/arkts-apis-image-pixelmap.md>) | PixelMap |
...
以下错误码的详细介绍请参见[DrawableDescriptor错误码](../../../错误码/UI界面/DrawableDescriptor错误码/errorcode-drawable-descriptor.md)。
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| src | [image.PixelMap](<../../../../Image Kit（图片处理服务）/ArkTS API/@ohos.multimedia.image (图片处理)/Interface (PixelMap)/arkts-apis-image-pixelmap.md>) | 否 | PixelMap类型参数，存储 PixelMap 图片数据。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| mode | [drawing.BlendMode](<../../../../ArkGraphics 2D（方舟2D图形服务）/ArkTS API/@ohos.graphics.drawing (绘制模块)/Enums/arkts-apis-graphics-drawing-e.md#blendmode>) | 是 | 混合模式。 |
...
| stopMode24+ | [AnimationStopMode](js-apis-arkui-drawabledescriptor.md#animationstopmode24) | 否 | 是 | 设置动图的停止模式。  默认值：AnimationStopMode.FIRST\_FRAME，表示动图停止时回到首帧。  **元服务API：** 从API version 24开始，该接口支持在元服务中使用。  **模型约束：** 此接口仅可在Stage模型下使用。 |
...
使用[Image](../../../ArkTS组件/图片与视频/Image/ts-basic-components-image.md)组件播放PixelMap数组或动图资源时传入AnimatedDrawableDescriptor对象，该对象继承自[DrawableDescriptor](js-apis-arkui-drawabledescriptor.md#drawabledescriptor)。
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| pixelMaps | Array<[image.PixelMap](<../../../../Image Kit（图片处理服务）/ArkTS API/@ohos.multimedia.image (图片处理)/Interface (PixelMap)/arkts-apis-image-pixelmap.md>)> | 是 | PixelMap 数组类型参数，存储 PixelMap 图片数据。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| src | [ResourceStr](../../../ArkTS组件/公共定义/基础类型定义/ts-types.md#resourcestr) | Array<[image.PixelMap](<../../../../Image Kit（图片处理服务）/ArkTS API/@ohos.multimedia.image (图片处理)/Interface (PixelMap)/arkts-apis-image-pixelmap.md>)> | 是 | 动图资源地址或者[PixelMap](<../../../../Image Kit（图片处理服务）/ArkTS API/@ohos.multimedia.image (图片处理)/Interface (PixelMap)/arkts-apis-image-pixelmap.md>)对象构成的数组。  ResourceStr当前支持的范围：应用资源Resource，沙箱路径（file://<bundleName>/<sandboxPath>），BASE64字符串。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| id | string | 否 | 组件的id。  当[Image](../../../ArkTS组件/图片与视频/Image/ts-basic-components-image.md)组件与AnimatedDrawableDescriptor确保1比1持有（仅传入一个[Image](../../../ArkTS组件/图片与视频/Image/ts-basic-components-image.md)组件）时，id非必填；  若同一AnimatedDrawableDescriptor需绑定多个[Image](../../../ArkTS组件/图片与视频/Image/ts-basic-components-image.md)组件，则必须设置唯一id以准确获取对应组件的动画控制器（唯一性由开发者保证）。  此规则基于动画系统设计原则：动画数据可多组件共享，但各组件动画独立运行，AnimationController与组件严格1比1持有关系（一个组件一个AnimationController对象）。  另外，[AnimatedDrawableDescriptor](js-apis-arkui-drawabledescriptor.md#animateddrawabledescriptor12)支持不可见时自动暂停播放功能，详见[onVisibleAreaChange](../../../ArkTS组件/通用事件/组件变化事件/组件可见区域变化事件/ts-universal-component-visible-area-change-event.md#onvisibleareachange)。 |
...
[Image](../../../ArkTS组件/图片与视频/Image/ts-basic-components-image.md)组件与AnimatedDrawableDescriptor保持1比1持有关系，示例代码如下。
...
[Image](../../../ArkTS组件/图片与视频/Image/ts-basic-components-image.md)组件与AnimatedDrawableDescriptor保持1比N持有关系，示例代码如下。
...
| 类型 | 说明 |
| --- | --- |
| [AnimationStatus](../../../ArkTS组件/公共定义/枚举说明/ts-appendix-enums.md#animationstatus) | 动图的播放状态。包含4种状态：初始态、播放态、暂停态、停止态。 |
