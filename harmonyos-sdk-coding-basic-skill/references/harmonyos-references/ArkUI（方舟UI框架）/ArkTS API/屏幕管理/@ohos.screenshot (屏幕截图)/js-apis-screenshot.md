...
content_hash: sha256:0835000f1e0df9acc6375770680581ecd064c8523a49c04da66a8cc49a82d32c
---
本模块提供屏幕截图的能力。
...
| 名称 | 类型 | 只读 | 可选 | 说明 |
| --- | --- | --- | --- | --- |
| displayId | number | 否 | 是 | 表示截取图像的显示设备[Display](<../@ohos.display (屏幕属性)/js-apis-display.md#display>)的ID号，默认为0，该参数应为大于或等于0的整数，非整数会报参数错误。  **元服务API：** 从API version 14开始，该接口支持在元服务中使用。 |
| blackWindowIds21+ | Array<number> | 否 | 是 | 表示截取图像时不显示的窗口ID列表，默认为空。窗口ID应为大于0的整数，目前仅[闪控球窗口](<../../窗口管理/@ohos.window.floatingBall (闪控球窗口)/js-apis-floatingball.md>)生效，窗口ID为非闪控球窗口、非整数、小于等于0、或者不存在的窗口ID时报参数错误，错误码为401。推荐使用[getFloatingBallWindowInfo()](<../../窗口管理/@ohos.window.floatingBall (闪控球窗口)/js-apis-floatingball.md#getfloatingballwindowinfo>)方法获取闪控球窗口ID属性。  **元服务API：** 从API version 21开始，该接口支持在元服务中使用。 |
...
| pixelMap | [image.PixelMap](<../../../../Image Kit（图片处理服务）/ArkTS API/@ohos.multimedia.image (图片处理)/Interface (PixelMap)/arkts-apis-image-pixelmap.md>) | 否 | 否 | 表示截取的图像PixelMap对象。 |
...
| 类型 | 说明 |
| --- | --- |
| Promise<[image.PixelMap](<../../../../Image Kit（图片处理服务）/ArkTS API/@ohos.multimedia.image (图片处理)/Interface (PixelMap)/arkts-apis-image-pixelmap.md>)> | Promise对象。返回一个PixelMap对象。 |
