...
scraped_at: 2026-06-11T15:47:47+08:00
doc_updated_at: 2026-05-26
content_hash: sha256:566ad95ac2a9312a7c66a1977a777bb6a1f688d43c19254c57b80d76246385d3
---
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| value | [AnimationStatus](../../公共定义/枚举说明/ts-appendix-enums.md#animationstatus) | 是 | 默认为初始状态，用于控制播放状态。  默认值：AnimationStatus.Initial |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| value | [FillMode](../../公共定义/枚举说明/ts-appendix-enums.md#fillmode) | 是 | 当前播放方向下，动画开始前和结束后的状态。  默认值：FillMode.Forwards |
...
设置组件是否通过系统[onVisibleAreaChange](../../通用事件/组件变化事件/组件可见区域变化事件/ts-universal-component-visible-area-change-event.md#onvisibleareachange)的可见性判定，控制组件的暂停和播放。
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| monitorInvisibleArea | boolean | 是 | 当设置为true时，组件将基于系统的[onVisibleAreaChange](../../通用事件/组件变化事件/组件可见区域变化事件/ts-universal-component-visible-area-change-event.md#onvisibleareachange)可见性判定，控制组件的暂停和播放。  当组件的运行状态为[AnimationStatus](../../公共定义/枚举说明/ts-appendix-enums.md#animationstatus)的Running时，若判定组件不可见，则自动执行暂停操作；若判定为可见，则自动恢复播放。  当设置为false时，组件的暂停和播放不受到onVisibleAreaChange影响。  默认值：false  **说明：**  当该属性由true动态修改为false时，组件将依据当前的[AnimationStatus](../../公共定义/枚举说明/ts-appendix-enums.md#animationstatus)状态进行处理。  例如，若当前状态为Running且因[onVisibleAreaChange](../../通用事件/组件变化事件/组件可见区域变化事件/ts-universal-component-visible-area-change-event.md#onvisibleareachange)的不可见回调暂停，则在属性由true改为false后，组件会从上次暂停的位置重新开始播放。  由该属性导致的不可见暂停和可见暂停操作不会改变用户设置的[state](ts-basic-components-imageanimator.md#state)值。 |
...
| 名称 | 类型 | 只读 | 可选 | 说明 |
| --- | --- | --- | --- | --- |
| src | string | [Resource](../../公共定义/基础类型定义/ts-types.md#resource)9+ | [PixelMap](../图像类型定义/ts-image-common.md#pixelmap)12+ | 否 | 否 | 图片路径，图片格式为jpg、jpeg、svg、png、bmp、webp、ico和heif，从API version9开始支持[Resource](../../公共定义/基础类型定义/ts-types.md#resource)类型的路径，从API version 12开始支持[PixelMap](../图像类型定义/ts-image-common.md#pixelmap)类型。  **string格式说明：**  - 支持加载本地图片路径和网络图片地址。使用相对路径引用本地图片时，不支持跨包或跨模块调用。resources目录下的文件不支持通过相对路径访问，需使用[Resource](../../公共定义/基础类型定义/ts-types.md#resource)类型（如$r或$rawfile）来引用，引用方式请参考[加载图片资源](<../../../../../harmonyos-guides/应用框架/ArkUI（方舟UI框架）/UI开发 (ArkTS声明式开发范式)/媒体展示/显示图片 (Image)/arkts-graphics-display.md#加载图片资源>)。  - 支持http和https网络图片地址，使用网络图片时需要申请权限ohos.permission.INTERNET。  - 支持file://路径前缀的字符串，应用沙箱URI为file://\<bundleName>/\<sandboxPath>。沙箱路径需要使用[fileUri.getUriFromPath(path)](<../../../../Core File Kit（文件基础服务）/ArkTS API/@ohos.file.fileuri (文件URI)/js-apis-file-fileuri.md#fileurigeturifrompath>)方法将路径转换为应用沙箱URI，然后传入显示。同时需要保证目录包路径下的文件有可读权限。  - 支持Base64字符串。  **卡片能力：** 从API version 10开始，该接口支持在ArkTS卡片中使用。 |
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/8c/v3/TWYu6C4zSWGnR6HVTO1RFg/zh-cn_image_0000002622859755.gif?HW-CC-KV=V1&HW-CC-Date=20260611T074744Z&HW-CC-Expire=86400&HW-CC-Sign=1B41027ACAC285715C3AE9C5CDDE8CA06933C9DC4505EDC10F660E9E928CD175)
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/7d/v3/4L6grt5HR9uakgOj-NzWPg/zh-cn_image_0000002622699875.gif?HW-CC-KV=V1&HW-CC-Date=20260611T074744Z&HW-CC-Expire=86400&HW-CC-Sign=67F1646FC126DA838FDBE0E16EBC40D08F34FC0A3ECD19B810CB2C05E23D6A5B)
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/88/v3/asRCM2ZKT12UEoygFdYDCQ/zh-cn_image_0000002592220314.gif?HW-CC-KV=V1&HW-CC-Date=20260611T074744Z&HW-CC-Expire=86400&HW-CC-Sign=DF7CB1BDA25AD670B1646FE47709A9F77D07C3DF691AB5B83085305214E40BFA)
