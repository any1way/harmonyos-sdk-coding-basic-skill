...
scraped_at: 2026-06-11T15:49:45+08:00
doc_updated_at: 2026-06-05
content_hash: sha256:30e0ec9f82acc1db7ff684342e2d3bb8f48c0ffaba52e90236f298b395b7aa2b
---
...
aboutToAppear函数在创建自定义组件的新实例后，在其build()函数执行前调用。允许在aboutToAppear函数中改变[状态变量](<../../../../../harmonyos-guides/应用框架/ArkUI（方舟UI框架）/UI开发 (ArkTS声明式开发范式)/学习UI范式状态管理/状态管理术语/arkts-state-management-glossary.md#state-variables状态变量>)，更改将在后续执行build()函数中生效。实现[自定义布局](../自定义组件的自定义布局/ts-custom-component-layout.md)的自定义组件的aboutToAppear生命周期在布局过程中触发。具体使用说明，详见[自定义组件生命周期指南](<../../../../../harmonyos-guides/应用框架/ArkUI（方舟UI框架）/UI开发 (ArkTS声明式开发范式)/学习UI范式基本语法/自定义组件/自定义组件生命周期/arkts-page-custom-components-lifecycle.md>)。

说明

* 在该回调函数内，建议仅执行当前节点组件的初始化逻辑，避免高耗时操作阻塞主线程。对于高耗时操作，推荐采用缓存或异步方案替代。最佳实践请参考[UI组件性能优化-避免在自定义组件的生命周期内执行高耗时操作](../../../../../best-practices/性能/性能场景优化案例/界面渲染性能优化/UI组件性能优化/bpta-ui-component-performance-optimization.md#section18755173594714)。
* 在需要频繁创建和销毁组件的场景中，将会频繁调用该回调函数。最佳实践请参考[主线程耗时操作优化指导-组件生命周期回调](../../../../../best-practices/性能/性能场景优化案例/界面渲染性能优化/主线程耗时操作优化/bpta-time-optimization-of-the-main-thread.md#section418843713435)。
...
onDidBuild函数在自定义组件的build()函数执行后调用，开发者可以在这个阶段实现埋点数据上报等不影响实际UI的功能。具体使用说明，详见[自定义组件生命周期指南](<../../../../../harmonyos-guides/应用框架/ArkUI（方舟UI框架）/UI开发 (ArkTS声明式开发范式)/学习UI范式基本语法/自定义组件/自定义组件生命周期/arkts-page-custom-components-lifecycle.md>)。
...
aboutToDisappear函数在自定义组件析构销毁时执行。不允许在aboutToDisappear函数中改变状态变量，特别是@Link变量的修改可能会导致应用程序行为不稳定。具体使用说明，详见[自定义组件生命周期指南](<../../../../../harmonyos-guides/应用框架/ArkUI（方舟UI框架）/UI开发 (ArkTS声明式开发范式)/学习UI范式基本语法/自定义组件/自定义组件生命周期/arkts-page-custom-components-lifecycle.md>)。不建议在aboutToDisappear函数调用后再触发例如[自定义弹窗的创建](<../../弹窗/自定义弹窗 (CustomDialog)/ts-methods-custom-dialog-box.md#open>)等逻辑，这可能会因为组件树信息丢失导致应用行为异常，例如[@Consume](<../../../../../harmonyos-guides/应用框架/ArkUI（方舟UI框架）/UI开发 (ArkTS声明式开发范式)/学习UI范式状态管理/状态管理（V1）/管理组件拥有的状态/@Provide装饰器和@Consume装饰器：与后代组件双向同步/arkts-provide-and-consume.md>)找不到对应的[@Provide](<../../../../../harmonyos-guides/应用框架/ArkUI（方舟UI框架）/UI开发 (ArkTS声明式开发范式)/学习UI范式状态管理/状态管理（V1）/管理组件拥有的状态/@Provide装饰器和@Consume装饰器：与后代组件双向同步/arkts-provide-and-consume.md>)、弹窗内白屏不显示组件等。
...
router路由页面（即[@Entry](<../../../../../harmonyos-guides/应用框架/ArkUI（方舟UI框架）/UI开发 (ArkTS声明式开发范式)/学习UI范式基本语法/自定义组件/创建自定义组件/arkts-create-custom-components.md#entry>)装饰的自定义组件）每次显示时触发一次，包括路由跳转、应用进入前台等场景。
...
router路由页面（即[@Entry](<../../../../../harmonyos-guides/应用框架/ArkUI（方舟UI框架）/UI开发 (ArkTS声明式开发范式)/学习UI范式基本语法/自定义组件/创建自定义组件/arkts-create-custom-components.md#entry>)装饰的自定义组件）每次隐藏时触发一次，包括路由跳转、应用进入后台等场景。

说明

在该回调函数内，建议避免执行高耗时操作阻塞主线程造成卡顿。对于高耗时操作例如相机资源释放，推荐使用异步方案替代。最佳实践请参考[优化应用时延问题-延迟执行资源释放操作](../../../../../best-practices/性能/性能场景优化案例/应用启动与响应优化/应用时延优化/bpta-application-latency-optimization-cases.md#section8783201923819)。
...
在router路由页面（即[@Entry](<../../../../../harmonyos-guides/应用框架/ArkUI（方舟UI框架）/UI开发 (ArkTS声明式开发范式)/学习UI范式基本语法/自定义组件/创建自定义组件/arkts-create-custom-components.md#entry>)装饰的自定义组件）生效，当用户点击返回按钮时触发。返回true表示页面自己处理返回逻辑，不进行页面路由；返回false表示使用默认的路由返回逻辑，不设置返回值按照false处理。
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/1/v3/1U7xlr8lRMaqIhahv6VgWw/zh-cn_image_0000002592380492.gif?HW-CC-KV=V1&HW-CC-Date=20260611T074838Z&HW-CC-Expire=86400&HW-CC-Sign=23BA86E15DD3614314925D174FF42E2F6A6A048D0545BF9F8BB9E5872D7BBCD4)
...
该回调仅生效于由[@Entry](<../../../../../harmonyos-guides/应用框架/ArkUI（方舟UI框架）/UI开发 (ArkTS声明式开发范式)/学习UI范式基本语法/自定义组件/创建自定义组件/arkts-create-custom-components.md#entry>)装饰的、作为[router](<../../../ArkTS API/UI界面/@ohos.router (页面路由)(不推荐)/js-apis-router.md>)路由页面存在的自定义组件。当之前存在于路由栈中的页面，通过单实例模式[RouterMode](<../../../ArkTS API/UI界面/@ohos.router (页面路由)(不推荐)/js-apis-router.md#routermode9>)移动到栈顶时触发该回调。
...
* [避免对@Link/@ObjectLink/@Prop等自动更新的状态变量，在aboutToReuse()中重复赋值](../../../../../best-practices/组件封装与复用/组件复用/bpta-component-reuse.md#section7441712174414)。
* 在滑动场景中，使用组件复用通常需要用该回调函数去更新组件的状态变量，因此在该回调函数中应避免耗时操作，否则会导致丢帧卡顿。最佳实践请参考[主线程耗时操作优化指导-组件复用回调](../../../../../best-practices/性能/性能场景优化案例/界面渲染性能优化/主线程耗时操作优化/bpta-time-optimization-of-the-main-thread.md#section20815336174316)。
...
详细内容请参考[@ReusableV2](<../../../../../harmonyos-guides/应用框架/ArkUI（方舟UI框架）/UI开发 (ArkTS声明式开发范式)/学习UI范式基本语法/自定义组件/自定义组件复用/@ReusableV2装饰器：V2组件复用/arkts-new-reusablev2.md>)。
...
| 类型 | 说明 |
| --- | --- |
| [Theme](<../../../ArkTS API/UI界面/@ohos.arkui.theme(主题换肤)/js-apis-arkui-theme.md#theme>) | 自定义组件当前生效的Theme对象。 |
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/b5/v3/_K7SDZxxTo2NvTv-w2-CgA/zh-cn_image_0000002622860003.png?HW-CC-KV=V1&HW-CC-Date=20260611T074838Z&HW-CC-Expire=86400&HW-CC-Sign=EC8A358D2B1EA396757E8FD35EB44BCF18BC050E0C89CA57860CB2AF57EE90A3)
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/e/v3/8yGunyv8QdObJMRfnBxomw/zh-cn_image_0000002622700121.png?HW-CC-KV=V1&HW-CC-Date=20260611T074838Z&HW-CC-Expire=86400&HW-CC-Sign=59B595E02D0CC7073A7C257712299ADA3F572E201F737CAED5B32D33011D5514)
