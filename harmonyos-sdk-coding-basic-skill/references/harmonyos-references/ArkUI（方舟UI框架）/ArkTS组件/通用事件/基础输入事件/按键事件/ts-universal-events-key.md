...
scraped_at: 2026-06-11T15:43:43+08:00
doc_updated_at: 2026-04-20
content_hash: sha256:4c28640d0ce823a8aed016b6f998c4d4ed50ccb3cde141e288f02545c4e6e2db
---
按键事件是指组件与物理键盘、遥控器等按键设备交互时触发的事件，适用于所有可获焦组件，例如Button。对于默认不可获焦的组件，如Text，Image等，可以将[focusable](../../../通用属性/交互属性/焦点控制/ts-universal-attributes-focus.md#focusable)属性设置为true后使用按键事件。

按键事件触发的流程和具体时机参考[按键事件数据流](<../../../../../../harmonyos-guides/应用框架/ArkUI（方舟UI框架）/UI开发 (ArkTS声明式开发范式)/添加交互响应/输入设备与事件/支持键盘输入事件/arkts-interaction-development-guide-keyboard.md#按键事件数据流>)。
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| event | [Callback](../../../公共定义/基础类型定义/ts-types.md#callback12)<[KeyEvent](ts-universal-events-key.md#keyevent对象说明), boolean> | 是 | 按键事件的回调。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| event | [Callback](../../../公共定义/基础类型定义/ts-types.md#callback12)<[KeyEvent](ts-universal-events-key.md#keyevent对象说明), boolean> | 是 | 处理按键事件的回调。 |
...
该回调的返回值为true时，视作该按键事件已被消费，不会[冒泡](<../../../../../../harmonyos-guides/应用框架/ArkUI（方舟UI框架）/UI开发 (ArkTS声明式开发范式)/添加交互响应/交互基础机制说明/arkts-interaction-basic-principles.md#事件冒泡>)给父组件处理。
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| event | [Callback](../../../公共定义/基础类型定义/ts-types.md#callback12)<[KeyEvent](ts-universal-events-key.md#keyevent对象说明), boolean> | 是 | 处理按键事件分发的回调。 |
...
| 名称 | 类型 | 只读 | 可选 | 说明 |
| --- | --- | --- | --- | --- |
| type | [KeyType](../../../公共定义/枚举说明/ts-appendix-enums.md#keytype) | 否 | 否 | 按键的类型。  **元服务API：** 从API version 11开始，该接口支持在元服务中使用。 |
| keyCode | number | 否 | 否 | 按键的键值。按键设备提供的键值请参考[KeyCode](<../../../../../基础功能/Input Kit（多模输入服务）/ArkTS API/@ohos.multimodalInput.keyCode (键值)/js-apis-keycode.md#keycode>)。  **元服务API：** 从API version 11开始，该接口支持在元服务中使用。 |
| keyText | string | 否 | 否 | 按键的名称。  **元服务API：** 从API version 11开始，该接口支持在元服务中使用。 |
| keySource | [KeySource](../../../公共定义/枚举说明/ts-appendix-enums.md#keysource) | 否 | 否 | 触发当前按键的输入设备类型。  **元服务API：** 从API version 11开始，该接口支持在元服务中使用。 |
...
| stopPropagation | () => void | 否 | 否 | 阻塞[事件冒泡](<../../../../../../harmonyos-guides/应用框架/ArkUI（方舟UI框架）/UI开发 (ArkTS声明式开发范式)/添加交互响应/交互基础机制说明/arkts-interaction-basic-principles.md#事件冒泡>)传递。  **元服务API：** 从API version 11开始，该接口支持在元服务中使用。 |
...
| 类型 | 说明 |
| --- | --- |
| [IntentionCode](<../../../../../基础功能/Input Kit（多模输入服务）/ArkTS API/@ohos.multimodalInput.intentionCode (意图事件)/js-apis-intentioncode.md#intentioncode>) | 按键对应的意图。 |
...
该示例通过按钮设置了按键事件。按钮获焦时，按下按键可触发onKeyEvent回调。按键事件触发的流程和具体时机参考[按键事件数据流](<../../../../../../harmonyos-guides/应用框架/ArkUI（方舟UI框架）/UI开发 (ArkTS声明式开发范式)/添加交互响应/输入设备与事件/支持键盘输入事件/arkts-interaction-development-guide-keyboard.md#按键事件数据流>)。
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/0c/v3/zLvkKCk2T5qLL_S4ELKqXA/zh-cn_image_0000002622699489.gif?HW-CC-KV=V1&HW-CC-Date=20260611T074341Z&HW-CC-Expire=86400&HW-CC-Sign=E9F97E5AF9683D693A207C1C29AA4CAC084A6D74BC3E97E3A5DFDD461185EB6D)
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/a3/v3/9YlYL8zPSQGrxeJOyChWaA/zh-cn_image_0000002592219928.gif?HW-CC-KV=V1&HW-CC-Date=20260611T074341Z&HW-CC-Expire=86400&HW-CC-Sign=9D085AC2E4FB326A77F70FF781B68E236A283BEDF9AF87705D419A37FD45FC8D)
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/47/v3/IzythWiWT26vHO-RJBS9aw/zh-cn_image_0000002592379862.gif?HW-CC-KV=V1&HW-CC-Date=20260611T074341Z&HW-CC-Expire=86400&HW-CC-Sign=9F895461D453797A87233B516E1AAFB717FCC57055ACB0259A776370271F7091)
