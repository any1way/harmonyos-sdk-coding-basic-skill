...
scraped_at: 2026-06-11T15:44:05+08:00
doc_updated_at: 2026-04-20
content_hash: sha256:2f42654d71fcb829ed541d8ba939509a154c777fab99a6c249d93db03553e1de
---
...
组件可见区域变化时触发该回调。开发指导及常见问题请参考[感知组件可见性](<../../../../../../harmonyos-guides/应用框架/ArkUI（方舟UI框架）/UI开发 (ArkTS声明式开发范式)/UI系统场景化能力/感知组件可见性/arkts-manage-components-visibility.md>)指南。
...
* 不支持兄弟组件对自身节点的遮挡计算，不支持所有祖先的兄弟节点对自身节点的遮挡计算，不支持窗口遮挡计算，不支持组件旋转计算，如[Stack](../../../行列与堆叠/Stack/ts-container-stack.md)、[Z序控制](../../../通用属性/基础属性/Z序控制/ts-universal-attributes-z-order.md)、[rotate](../../../通用属性/视效与模糊/图形变换/ts-universal-attributes-transformation.md#rotate)等。
* 不支持非挂树节点的可见面积变化计算。例如，预加载的节点、通过[overlay](../../../通用属性/基础属性/浮层/ts-universal-attributes-overlay.md#overlay)能力挂载的自定义节点。
* 不支持[scale](../../../通用属性/视效与模糊/图形变换/ts-universal-attributes-transformation.md#scale)属性，如果想要支持[scale](../../../通用属性/视效与模糊/图形变换/ts-universal-attributes-transformation.md#scale)，则需使用[onVisibleAreaChange22+](ts-universal-component-visible-area-change-event.md#onvisibleareachange22)，将measureFromViewport设置为true。
...
组件可见区域变化时触发该回调。可以通过measureFromViewport设置可见区域计算模式。开发指导及常见问题请参考[感知组件可见性](<../../../../../../harmonyos-guides/应用框架/ArkUI（方舟UI框架）/UI开发 (ArkTS声明式开发范式)/UI系统场景化能力/感知组件可见性/arkts-manage-components-visibility.md>)指南。
...
| event | [VisibleAreaChangeCallback](ts-universal-component-visible-area-change-event.md#visibleareachangecallback12) | 是 | 组件可见区域变化事件的回调。 |
| measureFromViewport | boolean | 是 | 设置可见区域计算模式。  当measureFromViewport设置为true时，系统在计算该组件的可见区域时，会考虑父组件的[clip](../../../通用属性/视效与模糊/形状裁剪/ts-universal-attributes-sharp-clipping.md#clip12) 属性设置。如果父组件的[clip](../../../通用属性/视效与模糊/形状裁剪/ts-universal-attributes-sharp-clipping.md#clip12)为false，则认为其内的子组件可以超出其区域进行显示，因此超出父组件的区域也将被视为可见区域纳入计算；如果父组件的[clip](../../../通用属性/视效与模糊/形状裁剪/ts-universal-attributes-sharp-clipping.md#clip12)设置为true，则组件超出父组件的区域会被裁剪，无法显示，因此会被视为不可见区域进行计算。而当measureFromViewport设置为false时，则不考虑[clip](../../../通用属性/视效与模糊/形状裁剪/ts-universal-attributes-sharp-clipping.md#clip12)的影响，直接将组件超出父组件的部分视为不可见区域。  measureFromViewport设置为true时，祖先节点设置[scale](../../../通用属性/视效与模糊/图形变换/ts-universal-attributes-transformation.md#scale)属性，组件可见比例会被正确计算。 |
...
* 不支持[scale](../../../通用属性/视效与模糊/图形变换/ts-universal-attributes-transformation.md#scale)属性，如果想要支持[scale](../../../通用属性/视效与模糊/图形变换/ts-universal-attributes-transformation.md#scale)，则需将[VisibleAreaEventOptions](ts-universal-component-visible-area-change-event.md#visibleareaeventoptions12)的measureFromViewport设置为true。
...
| measureFromViewport22+ | boolean | 否 | 是 | 设置可见区域计算模式。  当measureFromViewport设置为true时，系统在计算该组件的可见区域时，会考虑父组件的[clip](../../../通用属性/视效与模糊/形状裁剪/ts-universal-attributes-sharp-clipping.md#clip12) 属性设置。如果父组件的[clip](../../../通用属性/视效与模糊/形状裁剪/ts-universal-attributes-sharp-clipping.md#clip12)为false，则认为其内的子组件可以超出其区域进行显示，因此超出父组件的区域也将被视为可见区域纳入计算；如果父组件的[clip](../../../通用属性/视效与模糊/形状裁剪/ts-universal-attributes-sharp-clipping.md#clip12)设置为true，则组件超出父组件的区域会被裁剪，无法显示，因此会被视为不可见区域进行计算。而当measureFromViewport设置为false时，则不考虑[clip](../../../通用属性/视效与模糊/形状裁剪/ts-universal-attributes-sharp-clipping.md#clip12)的影响，直接将组件超出父组件的部分视为不可见区域。  默认值：false  measureFromViewport设置为true时，祖先节点设置[scale](../../../通用属性/视效与模糊/图形变换/ts-universal-attributes-transformation.md#scale)属性，组件可见比例会被正确计算。  **元服务API：** 从API version 22开始，该接口支持在元服务中使用。 |
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/af/v3/h3Sq3B5rSO2fks3MTI2ugw/zh-cn_image_0000002622699501.gif?HW-CC-KV=V1&HW-CC-Date=20260611T074404Z&HW-CC-Expire=86400&HW-CC-Sign=D584DCCDC5F8CCA41494682DF4CC1927640E7813674A0A48C32EAE1D7D2CD241)
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/98/v3/7TBygqPkT--UxASEe3lBXw/zh-cn_image_0000002592219940.jpg?HW-CC-KV=V1&HW-CC-Date=20260611T074404Z&HW-CC-Expire=86400&HW-CC-Sign=83E6C37406269A21A236FE576AEE7FC11C96733D00D026DB4380E8D0371B7C56)
