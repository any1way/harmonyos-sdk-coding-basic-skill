...
scraped_at: 2026-06-11T15:50:14+08:00
doc_updated_at: 2026-04-08
content_hash: sha256:b6c2be36f970b6624749b882972106c9d17d62c5b3f37f017be63417d7b0a45e
---
Popup是用于显示特定样式气泡。
...
* 建议结合[Popup控制](../../通用属性/弹窗控制/Popup控制/ts-universal-attributes-popup.md)中的自定义气泡功能。
...
| showClose | boolean | [Resource](../../公共定义/基础类型定义/ts-types.md#resource) | 否 | 是 | 设置popup关闭按钮。  true：显示关闭按钮；false：不显示关闭按钮。  Resource：显示对应的图标。  默认值：true  **元服务API：** 从API version 12开始，该接口支持在元服务中使用。 |
...
| direction12+ | [Direction](../../公共定义/枚举说明/ts-appendix-enums.md#direction) | 否 | 是 | 布局方向。  默认值：Direction.Auto  **元服务API：** 从API version 12开始，该接口支持在元服务中使用。 |
| maxWidth18+ | [Dimension](../../公共定义/基础类型定义/ts-types.md#dimension10) | 否 | 是 | 设置popup的最大宽度，通过此接口popup可以自定义宽度显示。  **说明：**  1. 在使用引用资源类型时，规定其参数类型要与属性方法本身类型一致。  2. maxWidth是数字类型，支持float和integer，例如$r('app.float.maxWidth')、$r('app.integer.maxWidth')。  3. 当类型为Resource时，如果未设置单位，默认单位为px。  默认值：400vp  **元服务API：** 从API version 18开始，该接口支持在元服务中使用。 |
...
| 名称 | 类型 | 只读 | 可选 | 说明 |
| --- | --- | --- | --- | --- |
| text | [ResourceStr](../../公共定义/基础类型定义/ts-types.md#resourcestr) | 否 | 否 | 设置文本内容。 |
| fontSize | number | string | [Resource](../../公共定义/基础类型定义/ts-types.md#resource) | 否 | 是 | 设置文本字体大小。  默认值：$r('sys.float.ohos\_id\_text\_size\_body2')  string类型可选值：可以转化为数字的字符串（如'10'）或带长度单位的字符串（如'10px'），不支持设置百分比字符串。  number：取值范围(0,+∞)。 |
| fontColor | [ResourceColor](../../公共定义/基础类型定义/ts-types.md#resourcecolor) | 否 | 是 | 设置文本字体颜色。  默认值：$r('sys.color.ohos\_id\_color\_text\_secondary') |
| fontWeight | number | [FontWeight](../../公共定义/枚举说明/ts-appendix-enums.md#fontweight) | string | 否 | 是 | 设置文本字体粗细。  number类型取值[100,900]，取值间隔为100，默认为400，取值越大，字体越粗。  string类型仅支持number类型取值的字符串形式，例如“400”，以及“bold”、“bolder”、“lighter”、“regular” 、“medium”分别对应FontWeight中相应的枚举值。  默认值：FontWeight.Regular |
...
| 名称 | 类型 | 只读 | 可选 | 说明 |
| --- | --- | --- | --- | --- |
| text | [ResourceStr](../../公共定义/基础类型定义/ts-types.md#resourcestr) | 否 | 否 | 设置按钮内容。 |
| action | () => void | 否 | 是 | 设置按钮click回调。  默认不执行任何操作。 |
| fontSize | number | string | [Resource](../../公共定义/基础类型定义/ts-types.md#resource) | 否 | 是 | 设置按钮文本字体大小。  默认值：$r('sys.float.ohos\_id\_text\_size\_button2')  string类型可选值：可以转化为数字的字符串（如'10'）或带长度单位的字符串（如'10px'），不支持设置百分比字符串。  设置值为异常值时取默认值。 |
| fontColor | [ResourceColor](../../公共定义/基础类型定义/ts-types.md#resourcecolor) | 否 | 是 | 设置按钮文本字体颜色。  默认值：$r('sys.color.ohos\_id\_color\_text\_primary\_activated') |
...
| 名称 | 类型 | 只读 | 可选 | 说明 |
| --- | --- | --- | --- | --- |
| image | [ResourceStr](../../公共定义/基础类型定义/ts-types.md#resourcestr) | 否 | 否 | 设置图标内容。 |
| width | [Dimension](../../公共定义/基础类型定义/ts-types.md#dimension10) | 否 | 是 | 设置图标宽度。  默认值：32VP |
| height | [Dimension](../../公共定义/基础类型定义/ts-types.md#dimension10) | 否 | 是 | 设置图标高度。  默认值：32VP |
| fillColor | [ResourceColor](../../公共定义/基础类型定义/ts-types.md#resourcecolor) | 否 | 是 | 设置图标填充颜色。仅针对svg图源生效。  默认不改变图标颜色。 |
| borderRadius | [Length](../../公共定义/基础类型定义/ts-types.md#length) | [BorderRadiuses](../../公共定义/基础类型定义/ts-types.md#borderradiuses9) | 否 | 是 | 设置图标圆角。  默认值：$r('sys.float.ohos\_id\_corner\_radius\_default\_s') |
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/c6/v3/otKGlSOwS06rdks8B03Mnw/zh-cn_image_0000002592380522.png?HW-CC-KV=V1&HW-CC-Date=20260611T075013Z&HW-CC-Expire=86400&HW-CC-Sign=A7B3DE4AF8B66CFF26148C8E6712B8D6D6B5CB7668ED91438ECDD34D728788FD)
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/6d/v3/bGNg266nS2idniURF8UgZw/zh-cn_image_0000002622860033.png?HW-CC-KV=V1&HW-CC-Date=20260611T075013Z&HW-CC-Expire=86400&HW-CC-Sign=61E420EC6D4BB876A3CAD5539C3FC0C532C6B785E5C3BD900FC562EB7DA2EF4F)
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/b4/v3/VE68KFNJQDOP0Fx87pLm9A/zh-cn_image_0000002622700151.png?HW-CC-KV=V1&HW-CC-Date=20260611T075013Z&HW-CC-Expire=86400&HW-CC-Sign=EE6058FB09C442990F949CE32933C529C616455894CA9A002B4AEBB3A452F9D1)
