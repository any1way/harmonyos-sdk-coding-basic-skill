...
content_hash: sha256:3703e9cce6e2781696141b466768c1af59cf460cf45ab17cd782eaa88fae71b7
---
该组件用于实现支持懒加载的网格布局，其父组件仅限于[WaterFlow](../WaterFlow/ts-container-waterflow.md)或[FlowItem](../FlowItem/ts-container-flowitem.md)，并支持使用自定义组件、[NodeContainer](../../自定义占位组件/NodeContainer/ts-basic-components-nodecontainer.md)组件封装后，在WaterFlow或FlowItem组件下应用。

该组件仅在WaterFlow组件的单列模式或分段布局中的单列分段，并且布局方向[FlexDirection](../../公共定义/枚举说明/ts-appendix-enums.md#flexdirection)设置为FlexDirection.Column的情况下支持懒加载。在WaterFlow的多列模式或布局方向为横向（FlexDirection.Row或FlexDirection.RowReverse）的情况下使用该组件，则不支持懒加载。此外，在布局方向为FlexDirection.ColumnReverse的WaterFlow组件下使用该组件会导致显示异常。当懒加载功能生效时，该组件仅加载WaterFlow显示区域内的子组件，并在帧间空闲时隙预加载显示区域上方和下方各半屏的内容。
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| value | [LengthMetrics](<../../../ArkTS API/UI界面/arkui/Graphics/js-apis-arkui-graphics.md#lengthmetrics12>) | 是 | 列与列的间距。  默认值：LengthMetrics.vp(0) |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| value | [LengthMetrics](<../../../ArkTS API/UI界面/arkui/Graphics/js-apis-arkui-graphics.md#lengthmetrics12>) | 是 | 行与行的间距。  默认值：LengthMetrics.vp(0) |
...
仅支持[通用事件](../../通用事件/ts-component-general-events.md)。
...
MyDataSource实现了[LazyForEach](../../状态管理与渲染控制/LazyForEach/ts-rendering-control-lazyforeach.md)数据源接口[IDataSource](../../状态管理与渲染控制/LazyForEach/ts-rendering-control-lazyforeach.md#idatasource)，用于通过LazyForEach给LazyVGridLayout提供子组件。
...
10. Column() {
11. WaterFlow() {
12. // 第一个LazyVGridLayout：单列布局
13. LazyVGridLayout() {
14. LazyForEach(this.arr1, (item:number)=>{
15. Text('item' + item.toString())
16. .height(64)
17. .width('100%')
18. .borderRadius(5)
...
21. })
22. }
23. .columnsTemplate('1fr') // 单列布局
24. .rowsGap(LengthMetrics.vp(10)) // 行间距10vp

26. // 第二个LazyVGridLayout：双列布局
27. LazyVGridLayout() {
28. LazyForEach(this.arr2, (item:number)=>{
29. Text('item' + item.toString())
30. .height(128)
31. .width('100%')
32. .borderRadius(5)
...
35. })
36. }
37. .columnsTemplate('1fr 1fr') // 双列布局，两列等宽
38. .rowsGap(LengthMetrics.vp(10)) // 行间距10vp
39. .columnsGap(LengthMetrics.vp(10)) // 列间距10vp
40. }.padding(10)
41. .rowsGap(10)
42. }
43. .width('100%').height('100%')
44. .backgroundColor('#DCDCDC')
45. }

47. // 初始化数据源
48. aboutToAppear(): void {
49. for (let i = 0; i < 6; i++) {
50. this.arr1.pushData(i);
51. }
52. for (let i = 0; i < 100; i++) {
53. this.arr2.pushData(i);
54. }
55. }
56. }
```
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/26/v3/Qxpyl_j-T56ZdRoTXCd2xg/zh-cn_image_0000002622699679.gif?HW-CC-KV=V1&HW-CC-Date=20260611T074624Z&HW-CC-Expire=86400&HW-CC-Sign=DC3E4A6C2299033AC3812E7D5FBEB590430BE9E79616A2D61B787E2F52575093)
