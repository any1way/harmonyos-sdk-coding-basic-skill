...
scraped_at: 2026-06-11T15:46:24+08:00
doc_updated_at: 2026-06-09
content_hash: sha256:9c7a0e06f6872143f52628a75df12ffe73f7fdf02e663fe80746433857a59fbb
---
...
仅支持[FlowItem](../FlowItem/ts-container-flowitem.md)子组件和自定义组件。自定义组件在WaterFlow下使用时，建议使用FlowItem作为自定义组件的顶层组件，不建议给自定义组件设置属性和事件方法。
...
在涉及大量子组件的情况下，建议采用懒加载、缓存数据、组件复用、固定宽高以及布局优化等方法，以提升性能和减少内存占用。最佳实践请参考[优化瀑布流加载慢丢帧问题](../../../../../best-practices/性能/性能场景优化案例/界面渲染性能优化/瀑布流加载丢帧优化/bpta-waterflow-performance-optimization.md)。
...
| 名称 | 类型 | 只读 | 可选 | 说明 |
| --- | --- | --- | --- | --- |
| footer | [CustomBuilder](../../公共定义/基础类型定义/ts-types.md#custombuilder8) | 否 | 是 | 设置WaterFlow尾部组件，用于在瀑布流末尾显示自定义内容（如加载提示、底部标识等）。不设置时不显示尾部组件。  **说明：**  使用方法参见[示例1](ts-container-waterflow.md#示例1使用基本瀑布流)。  **元服务API：** 从API version 11开始，该接口支持在元服务中使用。 |
| footerContent18+ | [ComponentContent](<../../../ArkTS API/UI界面/arkui/ComponentContent/js-apis-arkui-componentcontent.md>) | 否 | 是 | 设置WaterFlow尾部组件。  该参数的优先级高于参数footer，即同时设置footer和footerContent时，以footerContent设置的组件为准。  **元服务API：** 从API version 18开始，该接口支持在元服务中使用。 |
| scroller | [Scroller](../Scroll/ts-container-scroll.md#scroller) | 否 | 是 | 可滚动组件的控制器，与可滚动组件绑定。  **说明：**  不允许和其他滚动类组件，如：[ArcList](../ArcList/ts-container-arclist.md)、[List](../List/ts-container-list.md)、[Grid](../Grid/ts-container-grid.md)、[Scroll](../Scroll/ts-container-scroll.md)和[WaterFlow](ts-container-waterflow.md)绑定同一个滚动控制对象。  **元服务API：** 从API version 11开始，该接口支持在元服务中使用。 |
...
| columnsGap | [Dimension](../../公共定义/基础类型定义/ts-types.md#dimension10) | 否 | 是 | 该分组的列间距，不设置该参数时默认使用瀑布流的[columnsGap](ts-container-waterflow.md#columnsgap)，设置非法值时使用0vp。 |
| rowsGap | [Dimension](../../公共定义/基础类型定义/ts-types.md#dimension10) | 否 | 是 | 该分组的行间距，不设置该参数时默认使用瀑布流的[rowsGap](ts-container-waterflow.md#rowsgap)，设置非法值时使用0vp。 |
| margin | [Margin](../../公共定义/基础类型定义/ts-types.md#margin) | [Dimension](../../公共定义/基础类型定义/ts-types.md#dimension10) | 否 | 是 | 该分组的外边距参数为Length类型时，四个方向外边距同时生效。  默认值：0  单位：vp  margin设置百分比时，上下左右外边距均以瀑布流的width作为基础值。 |
...
| SLIDING\_WINDOW | 1 | 移动窗口式的布局模式。只考虑视窗内的布局信息，对视窗上方的FlowItem没有依赖关系，因此向后跳转或切换列数时只需要布局视窗内的FlowItem。建议优先采用该模式，尤其在应用需要支持屏幕旋转或动态切换列数的场景下。  **说明：**  1. 无动画跳转到较远的位置时，会以目标位置为基准，向前或向后布局FlowItem。这之后如果滑回跳转前的位置，内容的布局效果可能和之前不一致。 这个效果会导致跳转后回滑到顶部时，顶部节点可能不对齐。所以该布局模式下会在滑动到顶部后自动调整布局，保证顶部对齐。在有多个分组的情况下，会在滑动结束时调整在视窗内的分组。  2. [scroller](ts-container-waterflow.md#waterflowoptions对象说明)的[currentOffset](../Scroll/ts-container-scroll.md#currentoffset)或[offset](../Scroll/ts-container-scroll.md#offset23)接口返回的总偏移量在触发跳转或数据更新后不准确，在回滑到顶部时会重新校准，从API version 23开始，新增offset接口。  3. 如果在同一帧内调用跳转（如无动画的[scrollToIndex](../Scroll/ts-container-scroll.md#scrolltoindex)、[scrollEdge](../Scroll/ts-container-scroll.md#scrolledge)）和输入偏移量（如滑动手势或滚动动画），两者都会生效。  4. 调用无动画的[scrollToIndex](../Scroll/ts-container-scroll.md#scrolltoindex)进行跳转，如果跳转到较远位置（超过视窗内的FlowItem数量的位置）时，移动窗口模式对总偏移量进行估算。  5. 仅在API version 18及以上版本中支持滚动条[scrollBar](../滚动组件通用接口/ts-container-scrollable-common.md#scrollbar11)显示。低于此版本时，设置滚动条将不显示。 |
...
WaterFlow组件使用通用属性[clip12+](../../通用属性/视效与模糊/形状裁剪/ts-universal-attributes-sharp-clipping.md#clip12)和通用属性[clip18+](../../通用属性/视效与模糊/形状裁剪/ts-universal-attributes-sharp-clipping.md#clip18)时默认值都为true。

WaterFlow组件内容裁剪模式[ContentClipMode14+枚举说明](../滚动组件通用接口/ts-container-scrollable-common.md#contentclipmode14枚举说明)为ContentClipMode.CONTENT\_ONLY，padding区域会被裁剪不显示。
...
当value设置为ItemFillPolicy类型时，将根据WaterFlow组件宽度对应[断点类型](<../../../../../harmonyos-guides/应用框架/ArkUI（方舟UI框架）/UI开发 (ArkTS声明式开发范式)/组件布局/构建布局/栅格布局 (GridRowGridCol)/arkts-layout-development-grid-layout.md#栅格容器断点>)确定列数。
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| value | string | [ItemFillPolicy](../../公共定义/基础类型定义/ts-types.md#itemfillpolicy22) | 是 | 当前瀑布流组件布局列的数量。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| value | [Length](../../公共定义/基础类型定义/ts-types.md#length) | 是 | 列与列的间距。  默认值：0  取值范围：[0, +∞)，小于0时按0处理。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| value | [Length](../../公共定义/基础类型定义/ts-types.md#length) | 是 | 行与行的间距。  默认值：0  取值范围：[0, +∞)，小于0时按0处理。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| value | [FlexDirection](../../公共定义/枚举说明/ts-appendix-enums.md#flexdirection) | 是 | 布局的主轴方向。  默认值：FlexDirection.Column |
...
设置前后两个方向的嵌套滚动模式，实现与父组件的滚动联动。使用方法参考[嵌套滚动实现方式二](../Scroll/ts-container-scroll.md#示例3嵌套滚动实现方式二)。
...
配合[clip](../../通用属性/视效与模糊/形状裁剪/ts-universal-attributes-sharp-clipping.md#clip12)或[clipContent](../滚动组件通用接口/ts-container-scrollable-common.md#clipcontent14)属性可以显示出预加载节点。
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| enable | boolean | 是 | 是否同步加载WaterFlow区域内所有子组件。  true表示同步加载，false表示异步加载。  默认值：true。  **说明：**  设置为false时，在首次显示、不带动画[scrollToIndex](../Scroll/ts-container-scroll.md#scrolltoindex)跳转场景，若当帧布局耗时超过50ms，会将WaterFlow区域内尚未布局的子组件延后到下一帧进行布局。 |
...
frameNode中[getEvent('WaterFlow')](<../../../ArkTS API/UI界面/arkui/FrameNode/js-apis-arkui-framenode.md#geteventwaterflow19>)方法的返回值，可用于给WaterFlow节点设置滚动事件。

UIWaterFlowEvent继承于[UIScrollableCommonEvent](../滚动组件通用接口/ts-container-scrollable-common.md#uiscrollablecommonevent19)。
...
WaterFlowDataSource实现了LazyForEach数据源接口[IDataSource](../../状态管理与渲染控制/LazyForEach/ts-rendering-control-lazyforeach.md#idatasource)，用于通过LazyForEach给WaterFlow提供子组件。
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/de/v3/KxZGNFgBSYKn84a6Ckum0g/zh-cn_image_0000002622699673.gif?HW-CC-KV=V1&HW-CC-Date=20260611T074621Z&HW-CC-Expire=86400&HW-CC-Sign=67D010590A7E04ACE35F4AC98F9D16526B349418B07587645881E6AD84FD76FA)
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/54/v3/6LzkLFWkROq8_kgFsATALA/zh-cn_image_0000002592220112.png?HW-CC-KV=V1&HW-CC-Date=20260611T074621Z&HW-CC-Expire=86400&HW-CC-Sign=D9C8910950706647670A7FA001CE81BEED93B06BA84F75A6B4F822A5CCFBE476)

### 示例3（使用分组）

该示例展示了分组的初始化以及splice、push、update、values、length等接口的不同效果。

如果配合状态管理V2使用，详情见：[WaterFlow与makeObserved](<../../../../../harmonyos-guides/应用框架/ArkUI（方舟UI框架）/UI开发 (ArkTS声明式开发范式)/学习UI范式状态管理/状态管理V1-V2迁移指导/状态管理V1向V2迁移场景/内置对象的迁移/arkts-v1-v2-migration-inner-object.md#滚动组件>)。
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/94/v3/kfG6HOfCQTu_KdGDCt0CrQ/zh-cn_image_0000002592380046.png?HW-CC-KV=V1&HW-CC-Date=20260611T074621Z&HW-CC-Expire=86400&HW-CC-Sign=EB43D4F75E882F87E727A459495208C05E15953B187331E8F23AD6903488C42E)

### 示例4（双指缩放改变列数）

该示例通过[priorityGesture](../../手势处理/绑定手势/绑定手势事件/ts-gesture-settings.md#prioritygesture)和[PinchGesture](../../手势处理/基础手势/PinchGesture/ts-basic-gestures-pinchgesture.md)实现了双指缩放改变列数效果。
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/88/v3/EHlg9hc-QyWNVjrZehFOwA/zh-cn_image_0000002622859555.gif?HW-CC-KV=V1&HW-CC-Date=20260611T074621Z&HW-CC-Expire=86400&HW-CC-Sign=180F9E3CE2F68026EFEC666BE238CE2B3BA48AD499CF2DB739B91FC0225F665C)

### 示例5（设置边缘渐隐效果）

该示例通过[fadingEdge](../滚动组件通用接口/ts-container-scrollable-common.md#fadingedge14)实现了WaterFlow组件开启边缘渐隐效果，并通过fadingEdgeLength参数设置边缘渐隐长度。
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/3a/v3/EzTFsrh1Q5W761YoDM3yPw/zh-cn_image_0000002622699675.gif?HW-CC-KV=V1&HW-CC-Date=20260611T074621Z&HW-CC-Expire=86400&HW-CC-Sign=75187CD4C37B3209EDA9BCCD6A91913DCB7D8354A7DEFF7414D9540688697F5F)

### 示例6（单边边缘效果）

该示例通过[edgeEffect](../滚动组件通用接口/ts-container-scrollable-common.md#edgeeffect11)接口，实现了WaterFlow组件设置单边边缘效果。
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/7a/v3/JLtpMZy7Tq-wbxp8s1e48w/zh-cn_image_0000002592220114.gif?HW-CC-KV=V1&HW-CC-Date=20260611T074621Z&HW-CC-Expire=86400&HW-CC-Sign=45807CF9F127F75DBB7905DCEC8A6311C74E4EB75129A4EF60A9797E953C2B41)

### 示例7（WaterFlow组件设置和改变尾部组件）

从API version 18开始，该示例通过[WaterFlowOptions对象说明](ts-container-waterflow.md#waterflowoptions对象说明)的footerContent接口，实现了WaterFlow组件设置尾部组件。通过[ComponentContent](<../../../ArkTS API/UI界面/arkui/ComponentContent/js-apis-arkui-componentcontent.md>)的update函数更新尾部组件。
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/2e/v3/A3nkL2AqT52N6TbgWNSMIA/zh-cn_image_0000002592380048.gif?HW-CC-KV=V1&HW-CC-Date=20260611T074621Z&HW-CC-Expire=86400&HW-CC-Sign=752CAD333CA856BDA33A5EC71844304869922ADABB6FF35BEB82392B24BE5BE6)

### 示例8（WaterFlow组件实现下拉刷新）

该示例通过[Refresh](../Refresh/ts-container-refresh.md)组件和WaterFlow组件，实现了下拉刷新瀑布流组件数据源。
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/af/v3/OK0xb0G9Rae4Nw-YEk1Fjg/zh-cn_image_0000002622859557.gif?HW-CC-KV=V1&HW-CC-Date=20260611T074621Z&HW-CC-Expire=86400&HW-CC-Sign=B561A7700663320E691BC2E7534FB6E0C8A8A5303124DB711732EDEC6CAD7C79)
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/12/v3/CxvGXZ1iQouCT0J5SlsaYQ/zh-cn_image_0000002622699677.png?HW-CC-KV=V1&HW-CC-Date=20260611T074621Z&HW-CC-Expire=86400&HW-CC-Sign=65C023ED6852BEEC8943CEFF4B117FBDAC60E559DC4C0B83CCD14BD9B4F1DE59)

WaterFlow宽度属于md断点区间时显示3列。

![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/cc/v3/B9n0oVX_QdaASKJ16oVAHw/zh-cn_image_0000002592220116.png?HW-CC-KV=V1&HW-CC-Date=20260611T074621Z&HW-CC-Expire=86400&HW-CC-Sign=4BD746895EF07839C427AE5606159BFAEC1AFEC2E19FFD4244F63804F2D4D2D4)

WaterFlow宽度属于lg及更大的断点区间时显示5列。

![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/47/v3/gAqWspVMQyq_btNocCWX8A/zh-cn_image_0000002592380050.png?HW-CC-KV=V1&HW-CC-Date=20260611T074621Z&HW-CC-Expire=86400&HW-CC-Sign=F4E642F7991145D6DD1DAADFD027CC0AC060BD41ECD24767402B038EF48F6202)
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/ed/v3/ttMcZ5YETTO4V71ZWc3zyg/zh-cn_image_0000002622859559.gif?HW-CC-KV=V1&HW-CC-Date=20260611T074621Z&HW-CC-Expire=86400&HW-CC-Sign=B8DA4A00B41EADF7382AD927C33E750C0D5FB16BEC8C4AD559754B2C94409F72)

### 示例11（设置滚动事件）

该示例通过FrameNode中的[getEvent('WaterFlow')](<../../../ArkTS API/UI界面/arkui/FrameNode/js-apis-arkui-framenode.md#geteventwaterflow19>)获取[UIWaterFlowEvent](ts-container-waterflow.md#uiwaterflowevent19)，并为WaterFlow设置滚动事件回调，用于事件监听方因无法直接修改页面代码而无法使用声明式接口设置回调的场景。
