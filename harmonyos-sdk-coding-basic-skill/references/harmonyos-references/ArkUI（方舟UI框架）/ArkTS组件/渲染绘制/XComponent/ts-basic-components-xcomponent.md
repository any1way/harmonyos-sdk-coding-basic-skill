...
scraped_at: 2026-06-11T15:48:46+08:00
doc_updated_at: 2026-06-03
content_hash: sha256:f2feba794d1be66d98882c04977bed1b719265c9bbd13ac14e37e3f7fc40aa4b
---
提供用于图形绘制和媒体数据写入的Surface，XComponent负责将其嵌入到视图中，支持应用自定义Surface位置和大小。具体指南请参考[自定义渲染 (XComponent)文档](<../../../../../harmonyos-guides/应用框架/ArkUI（方舟UI框架）/UI开发 (ArkTS声明式开发范式)/添加组件/自定义渲染 (XComponent)/napi-xcomponent-guidelines.md>)。
...
| type | [XComponentType](../../公共定义/枚举说明/ts-appendix-enums.md#xcomponenttype10) | 是 | 用于指定XComponent组件类型。 |
...
| 名称 | 类型 | 只读 | 可选 | 说明 |
| --- | --- | --- | --- | --- |
| type | [XComponentType](../../公共定义/枚举说明/ts-appendix-enums.md#xcomponenttype10) | 否 | 否 | 用于指定XComponent组件类型。 |
...
| imageAIOptions | [ImageAIOptions](../../图片与视频/图像类型定义/ts-image-common.md#imageaioptions12) | 否 | 是 | 给组件设置一个AI分析选项，通过此项可配置分析类型或绑定一个分析控制器。 |

## NativeXComponentParameters19+

PhonePC/2in1TabletTVWearable

定义XComponent的具体配置参数。通过这种构造参数创建的XComponent，可以将其对应的[FrameNode](<../../../ArkTS API/UI界面/arkui/FrameNode/js-apis-arkui-framenode.md>)对象传递至Native侧，使用NDK接口进行Surface生命周期的相关设置和[添加事件监听](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/ndk-add-component-events)。
...
对于TEXTURE和SURFACE类型的XComponent组件，当不设置[renderFit](../../通用属性/视效与模糊/组件内容填充方式/ts-universal-attributes-renderfit.md#renderfit)属性时，取默认值为RenderFit.RESIZE\_FILL。

对于SURFACE类型的XComponent组件，背景色设置为不透明的纯黑色，在API version 18之前，其[renderFit](../../通用属性/视效与模糊/组件内容填充方式/ts-universal-attributes-renderfit.md#renderfit18)通用属性仅支持设置为RenderFit.RESIZE\_FILL；在API version 18及之后，支持所有的RenderFit枚举值。

对于使用[ArkUI NDK接口](<../../../../../harmonyos-guides/应用框架/ArkUI（方舟UI框架）/UI开发 (基于NDK构建UI)/接入ArkTS页面/ndk-access-the-arkts-page.md>)创建的XComponent组件，不支持使用属性获取函数[getAttribute](<../../../C API/结构体/ArkUI_NativeNodeAPI_1/capi-arkui-nativemodule-arkui-nativenodeapi-1.md#getattribute>)获取其renderFit属性值。
...
不支持[ArkUI NDK接口](<../../../../../harmonyos-guides/应用框架/ArkUI（方舟UI框架）/UI开发 (基于NDK构建UI)/基于NDK构建UI概述/ndk-build-ui-overview.md>)创建的XComponent组件。
...
* 仅XComponent构造参数中的type为[XComponentType](../../公共定义/枚举说明/ts-appendix-enums.md#xcomponenttype10).SURFACE时该接口生效，否则该接口不生效。
* 如果将参数type设置为[HdrType](ts-basic-components-xcomponent.md#hdrtype24枚举说明).AIHDR，调用该接口前需先检查[Display](<../../../ArkTS API/屏幕管理/@ohos.display (屏幕属性)/js-apis-display.md#display>)的hdrFormats性是否包含[HDRFormat](<../../../../ArkGraphics 2D（方舟2D图形服务）/ArkTS API/@ohos.graphics.hdrCapability (HDR能力)/js-apis-hdrcapability.md#hdrformat>).VIDEO\_AIHDR。仅当包含HDRFormat.VIDEO\_AIHDR时，当前设备才支持AI HDR类型，参数设置才会生效；否则按默认值HdrType.DEFAULT处理。
* 不支持[ArkUI NDK接口](<../../../../../harmonyos-guides/应用框架/ArkUI（方舟UI框架）/UI开发 (基于NDK构建UI)/基于NDK构建UI概述/ndk-build-ui-overview.md>)创建的XComponent组件。
...
| type | [HdrType](ts-basic-components-xcomponent.md#hdrtype24枚举说明) | 否 | 播放HDR视频时的HDR类型。  默认值: HdrType.DEFAULT |
...
从API version 12开始，type为SURFACE或TEXTURE时，支持[通用事件](../../通用事件/ts-component-general-events.md)。

说明

当配置libraryname参数时，[点击事件](../../通用事件/交互响应事件/点击事件/ts-universal-events-click.md)、[触摸事件](../../通用事件/基础输入事件/触摸事件/ts-universal-events-touch.md)、[挂载卸载事件](../../通用事件/组件变化事件/挂载卸载事件/ts-universal-events-show-hide.md)、[按键事件](../../通用事件/基础输入事件/按键事件/ts-universal-events-key.md)、[焦点事件](../../通用事件/交互响应事件/焦点事件/ts-universal-focus-event.md)、[鼠标事件](../../通用事件/基础输入事件/鼠标事件/ts-universal-mouse-key.md)仅响应C-API侧事件接口。
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| event | [VoidCallback](../../公共定义/基础类型定义/ts-types.md#voidcallback12) | 是 | XComponent销毁后回调事件。 |
...
该方法优先级高于[border](../../通用属性/布局与边框/边框设置/ts-universal-attributes-border.md#border)、[padding](../../通用属性/布局与边框/尺寸设置/ts-universal-attributes-size.md#padding)等可以改变内容偏移和大小的属性。
...
当XComponent持有的Surface销毁后进行该回调，仅XComponent类型为SURFACE("surface")或TEXTURE时有效，具体可以参考指南[创建XComponent和管理Surface生命周期](<../../../../../harmonyos-guides/应用框架/ArkUI（方舟UI框架）/UI开发 (ArkTS声明式开发范式)/添加组件/自定义渲染 (XComponent)/napi-xcomponent-guidelines.md#创建xcomponent和管理surface生命周期>)章节。
...
配置AI分析并启动AI分析功能，使用前需先启用图像AI分析能力[enableAnalyzer](ts-basic-components-xcomponent.md#enableanalyzer12)。使用Promise异步回调。
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| config | [ImageAnalyzerConfig](../../图片与视频/图像类型定义/ts-image-common.md#imageanalyzerconfig12) | 是 | 执行AI分析所需要的入参，用于配置AI分析功能。 |
...
返回可用于向XComponent上绘制内容的画布对象。具体绘制方法请参考[Canvas](<../../../../ArkGraphics 2D（方舟2D图形服务）/ArkTS API/@ohos.graphics.drawing (绘制模块)/Class (Canvas)/arkts-apis-graphics-drawing-canvas.md>)。
...
| 类型 | 说明 |
| --- | --- |
| [DrawingCanvas](../../画布绘制/DrawingRenderingContext/ts-drawingrenderingcontext.md#drawingcanvas12对象说明) | null | 可用于向XComponent区域绘制的画布对象或者空对象null。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| canvas | [DrawingCanvas](../../画布绘制/DrawingRenderingContext/ts-drawingrenderingcontext.md#drawingcanvas12对象说明) | 是 | 之前调用lockCanvas方法返回的画布对象。 |
...
surfaceWidth和surfaceHeight属性在未调用[setXComponentSurfaceRect](ts-basic-components-xcomponent.md#setxcomponentsurfacerect12)也未设置[border](../../通用属性/布局与边框/边框设置/ts-universal-attributes-border.md#border)和[padding](../../通用属性/布局与边框/尺寸设置/ts-universal-attributes-size.md#padding)等属性时，其取值大小为XComponent组件的大小。
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/38/v3/JuWBkpEARG6MuJYG28ByBw/zh-cn_image_0000002592380420.gif?HW-CC-KV=V1&HW-CC-Date=20260611T074844Z&HW-CC-Expire=86400&HW-CC-Sign=8F74D9E10B55935743676F0CBCD7163EBABAA18D2AEB49E3325F767CC5FB1B85)
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/6e/v3/xL-lvdT9RaSHws9FvK2ybA/zh-cn_image_0000002622859931.png?HW-CC-KV=V1&HW-CC-Date=20260611T074844Z&HW-CC-Expire=86400&HW-CC-Sign=3BD4D8F3407348BABA37BF1CA4E84642867B313FF842C056C87AD61479051926)
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/34/v3/5yEXnEUFRbS5EUwcmn5lbQ/zh-cn_image_0000002622700049.jpeg?HW-CC-KV=V1&HW-CC-Date=20260611T074844Z&HW-CC-Expire=86400&HW-CC-Sign=0B746181F74F4784822B13AFEC341790594BBD3253A2A3C7F49FF776C299B631)
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/0/v3/HYBoPih4QrSpi9NfTYprGw/zh-cn_image_0000002592220490.jpeg?HW-CC-KV=V1&HW-CC-Date=20260611T074844Z&HW-CC-Expire=86400&HW-CC-Sign=0049055813FE0FE3C024741CB5B28EA03321043285318DCB15F2C0606374EB56)
