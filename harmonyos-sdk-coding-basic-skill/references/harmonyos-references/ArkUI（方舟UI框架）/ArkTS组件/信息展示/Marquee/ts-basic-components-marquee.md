...
scraped_at: 2026-06-11T15:48:02+08:00
doc_updated_at: 2026-05-18
content_hash: sha256:d6f677a242d0d7ff2a96d0cb676610e458012f51edef08c00bd05bafa1183c6f
---
...
为了不影响滚动帧率，建议在滚动类组件中Marquee的个数不超过4个，或者使用[Text](../../文本与输入/Text/ts-basic-components-text.md)组件的[TextOverflow.MARQUEE](../../公共定义/枚举说明/ts-appendix-enums.md#textoverflow)替代。

对于Marquee组件动态帧率的场景，可以使用[MarqueeDynamicSyncScene](<../../../ArkTS API/UI界面/@ohos.arkui.UIContext (UIContext)/Class (MarqueeDynamicSyncScene)/arkts-apis-uicontext-marqueedynamicsyncscene.md>)接口实现。

在文本宽度小于跑马灯组件宽度时，使用[属性动画](<../../动画/属性动画 (animation)/ts-animatorproperty.md>)实现滚动。
...
| step8+ | number | 否 | 是 | 滚动动画文本滚动步长。当step大于Marquee的文本宽度时，取默认值。  默认值：6  单位：[vp](../../公共定义/像素单位/ts-pixel-units.md#基本像素单位)  **卡片能力：** 从API version 9开始，该接口支持在ArkTS卡片中使用。  **元服务API：** 从API version 11开始，该接口支持在元服务中使用。 |
...
| spacing23+ | [LengthMetrics](<../../../ArkTS API/UI界面/arkui/Graphics/js-apis-arkui-graphics.md#lengthmetrics12>) | 否 | 是 | 两轮跑马灯之间的间距。如果LengthMetrics的unit值是PERCENT，当前设置不生效，按默认值处理。  默认值：跑马灯组件宽度。  **卡片能力：** 从API version 23开始，该接口支持在ArkTS卡片中使用。  **元服务API：** 从API version 23开始，该接口支持在元服务中使用。  **模型约束：** 此接口仅可在Stage模型下使用。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| value | [ResourceColor](../../公共定义/基础类型定义/ts-types.md#resourcecolor) | 是 | 字体颜色。  Wearable设备上默认值为：'#c5ffffff'，显示为淡蓝色，其他设备默认值为：'e6182431'，显示为黑色。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| value | number | [FontWeight](../../公共定义/枚举说明/ts-appendix-enums.md#fontweight) | string | 是 | 文本的字体粗细，number类型取值[100, 900]，取值间隔为100，默认为400，取值越大，字体越粗。string类型仅支持number类型取值的字符串形式，例如"400"，以及"bold"、"bolder"、"lighter"、"regular"、"medium"，分别对应FontWeight中相应的枚举值。  默认值：FontWeight.Normal |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| value | string | [Resource](../../公共定义/基础类型定义/ts-types.md#resource) | 是 | 字体列表。默认字体'HarmonyOS Sans'。  应用当前支持'HarmonyOS Sans'字体和注册自定义字体[loadFontSync](<../../../../ArkGraphics 2D（方舟2D图形服务）/ArkTS API/@ohos.graphics.text (文本模块)/js-apis-graphics-text.md#loadfontsync>)。  卡片当前仅支持'HarmonyOS Sans'字体。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| value | [MarqueeUpdateStrategy](../../公共定义/枚举说明/ts-appendix-enums.md#marqueeupdatestrategy12) | 是 | 跑马灯组件属性更新后，跑马灯的滚动策略。  默认值: MarqueeUpdateStrategy.DEFAULT |
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/d3/v3/gG9HJpKaQfK2ZazF5U-l-g/zh-cn_image_0000002622859823.gif?HW-CC-KV=V1&HW-CC-Date=20260611T074801Z&HW-CC-Expire=86400&HW-CC-Sign=C33F9A1E4937AE39D75CC28BC8E36073212CA690BCCD8A35845CAA7A420AEE63)
