...
scraped_at: 2026-06-11T15:49:25+08:00
doc_updated_at: 2026-04-30
content_hash: sha256:b31c997dff9e6e84eab8f29065ffdf4e6c5013e2fc422c84cc33c2ed9b357447
---
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| value | [Dimension](../公共定义/基础类型定义/ts-types.md#dimension10) | 是 | 安全控件上图标的尺寸。  默认值：16vp。  不支持设置百分比字符串。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| value | [Position](../公共定义/基础类型定义/ts-types.md#position) | 是 | 安全控件的左上角相对于父容器左上角的偏移位置。  **异常情况说明**：  1.当入参为异常值（如入参不符合Position定义等）、入参为Position类型但x和y均为异常值（如undefined或其他与格式要求不符的字符串等）时，该属性不生效；  2.当入参的Position中，x和y有且仅有一个异常值时，值异常的属性会被置为0。如输入{x: 0, y: 'a'}，最终效果按{x: 0, y: 0}显示。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| value | [Position](../公共定义/基础类型定义/ts-types.md#position) | 是 | 安全控件在位置定位时的锚点，以控件左上角作为基准点进行偏移。通常配合position和offset属性使用，单独使用时，效果类似offset。  无默认值，设置异常值时该属性不生效。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| value | [Position](../公共定义/基础类型定义/ts-types.md#position) | [Edges12+](../公共定义/基础类型定义/ts-types.md#edges12) | [LocalizedEdges12+](../公共定义/基础类型定义/ts-types.md#localizededges12) | 是 | 安全控件相对于自身布局位置的坐标偏移。设置此属性不会影响父容器的布局，仅在绘制过程中调整位置。  无默认值，设置异常值时该属性不生效。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| value | [Dimension](../公共定义/基础类型定义/ts-types.md#dimension10) | 是 | 安全控件上文字的尺寸。  默认值：16fp。  不支持设置百分比字符串。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| value | [FontStyle](../公共定义/枚举说明/ts-appendix-enums.md#fontstyle) | 是 | 安全控件上文字的样式。  默认值：FontStyle.Normal。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| value | number | [FontWeight](../公共定义/枚举说明/ts-appendix-enums.md#fontweight) | string | [Resource](../公共定义/基础类型定义/ts-types.md#resource)20+ | 是 | 安全控件上文字粗细。  number类型取值[100, 900]，取值间隔为100，取值越大，字体越粗。  string类型支持使用数字字符串（如'400'），以及FontWeight中的枚举值对应的字符串（如'bold'、'bolder'、'lighter'、'regular'、'medium'）。  从API version 20开始，支持Resource类型。Resource类型仅支持'integer'和'string'，当类型为'integer'时，取值参考前述number类型；当类型为'string'时，取值参考前述string类型。  如果控件未设置fontWeight，文字粗细将默认设置为FontWeight.Medium；如果value入参为非法值，文字粗细将被设置为FontWeight.Normal。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| value | string | [Resource](../公共定义/基础类型定义/ts-types.md#resource) | 是 | 安全控件上文字的字体。  默认字体：'HarmonyOS Sans'。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| value | [ResourceColor](../公共定义/基础类型定义/ts-types.md#resourcecolor) | 是 | 安全控件上文字的颜色。  默认值：$r('sys.color.font\_on\_primary')。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| value | [ResourceColor](../公共定义/基础类型定义/ts-types.md#resourcecolor) | 是 | 安全控件上图标的颜色。  默认值：$r('sys.color.icon\_on\_primary')。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| value | [ResourceColor](../公共定义/基础类型定义/ts-types.md#resourcecolor) | 是 | 安全控件的背景颜色。安全控件按钮背景色高八位的α值低于0x1a（例如0x1800ff00）时，安全控件按钮背景色高八位的α值会被系统强制调整为0xff。  默认值：$r('sys.color.icon\_emphasize')。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| value | [BorderStyle](../公共定义/枚举说明/ts-appendix-enums.md#borderstyle) | 是 | 安全控件的边框的样式。  默认不设置边框样式。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| value | [Dimension](../公共定义/基础类型定义/ts-types.md#dimension10) | 是 | 安全控件的边框宽度。  默认不设置边框宽度。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| value | [ResourceColor](../公共定义/基础类型定义/ts-types.md#resourcecolor) | 是 | 安全控件的边框颜色。  默认不设置边框颜色。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| value | [Dimension](../公共定义/基础类型定义/ts-types.md#dimension10) | 是 | 安全控件的边框圆角半径。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| radius | [Dimension](../公共定义/基础类型定义/ts-types.md#dimension10) | [BorderRadiuses](../公共定义/基础类型定义/ts-types.md#borderradiuses9) | 是 | 安全控件的边框圆角半径。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| value | [Padding](../公共定义/基础类型定义/ts-types.md#padding) | [Dimension](../公共定义/基础类型定义/ts-types.md#dimension10) | 是 | 安全控件的内边距。  默认值：上下8vp，左右16vp。  **说明**：本参数不支持设置百分比字符串数据类型。若设置百分比字符串，则对应内边距显示为0。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| alignType | [Alignment](../公共定义/枚举说明/ts-appendix-enums.md#alignment) | 是 | 安全控件图标文本的对齐方式。图标文本作为整体在控件背托范围内进行对齐，UX显示受[padding](ts-securitycomponent-attributes.md#padding)影响，在padding生效的基础上进行指定方式对齐。  默认值：Alignment.Center。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| value | [Dimension](../公共定义/基础类型定义/ts-types.md#dimension10) | 是 | 安全控件中图标和文字的间距。  默认值：4vp。  **说明**：本参数不支持设置百分比字符串数据类型，若设置百分比字符串，则图标和文字的间距显示为0；从API 14开始，若设置值为负值，则使用默认值。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| value | [Length](../公共定义/基础类型定义/ts-types.md#length) | 是 | 安全控件自身的宽度，缺省时将根据元素内容自适配宽度。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| value | [Length](../公共定义/基础类型定义/ts-types.md#length) | 是 | 安全控件自身的高度，缺省时将根据元素内容自适配高度。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| value | [SizeOptions](../公共定义/基础类型定义/ts-types.md#sizeoptions) | 是 | 宽高尺寸，缺省时将根据元素内容自适配高宽尺寸。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| value | [ConstraintSizeOptions](../公共定义/基础类型定义/ts-types.md#constraintsizeoptions) | 是 | 约束尺寸，组件布局时，进行尺寸范围限制。未显式指定单位时，单位为vp。constraintSize的优先级高于Width和Height。取值结果参考[constraintSize取值对width/height影响](../通用属性/布局与边框/尺寸设置/ts-universal-attributes-size.md#constraintsize)。  默认值：  {  minWidth: 0,  maxWidth: Infinity,  minHeight: 0,  maxHeight: Infinity  }。 |
...
指定设置在相对容器中子组件的对齐规则，仅当父容器为[RelativeContainer](../行列与堆叠/RelativeContainer/ts-container-relativecontainer.md)时生效。
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| alignRule | [AlignRuleOption](../通用属性/布局与边框/位置设置/ts-universal-attributes-location.md#alignruleoption9对象说明) | 是 | 指定设置在相对容器中子组件的对齐规则。 |
...
指定设置在相对容器中子组件的对齐规则，仅当父容器为[RelativeContainer](../行列与堆叠/RelativeContainer/ts-container-relativecontainer.md)时生效。该方法水平方向上以start和end分别替代上述[alignRules](ts-securitycomponent-attributes.md#alignrules15)的left和right，以便在RTL模式下能镜像显示，建议使用该方法指定设置在相对容器中子组件的对齐规则。
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| alignRule | [LocalizedAlignRuleOptions](../通用属性/布局与边框/位置设置/ts-universal-attributes-location.md#localizedalignruleoptions12对象说明) | 是 | 指定设置在相对容器中子组件的对齐规则。 |
...
指定以该组件为链头所构成的链的参数，仅当父容器为[RelativeContainer](../行列与堆叠/RelativeContainer/ts-container-relativecontainer.md)时生效。
...
| style | [ChainStyle](../通用属性/布局与边框/位置设置/ts-universal-attributes-location.md#chainstyle12) | 是 | 链的样式。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| scale | number | [Resource](../公共定义/基础类型定义/ts-types.md#resource) | 是 | 文本最小的字体缩小倍数。  取值范围：[0, 1]。  **说明：**  设置的值小于0时，按值为0处理，即缩小不受限制；设置的值大于1，按值为1处理，即缩小不生效。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| scale | number | [Resource](../公共定义/基础类型定义/ts-types.md#resource) | 是 | 文本最大的字体放大倍数。  取值范围：[1, +∞)。  **说明：**  设置的值小于1时，按值为1处理，异常值默认不生效。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| minSize | number | string | [Resource](../公共定义/基础类型定义/ts-types.md#resource) | 是 | 文本最小显示字号。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| maxSize | number | string | [Resource](../公共定义/基础类型定义/ts-types.md#resource) | 是 | 文本最大显示字号。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| line | number | [Resource](../公共定义/基础类型定义/ts-types.md#resource)20+ | 是 | 文本的最大行数。  number类型入参的取值范围：[1, +∞)。从API version 20开始，支持Resource类型。Resource类型仅支持'integer'，取值范围为[1, +∞)。  **说明：**  设置的值小于1时，按默认值1000000处理。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| policy | [TextHeightAdaptivePolicy](../公共定义/枚举说明/ts-appendix-enums.md#textheightadaptivepolicy10) | 是 | 文本自适应高度的方式。  默认值：TextHeightAdaptivePolicy.MAX\_LINES\_FIRST。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| style | [FocusBoxStyle](../通用属性/交互属性/焦点控制/ts-universal-attributes-focus.md#focusboxstyle12对象说明) | 是 | 设置安全控件系统焦点框样式。 |
...
为避免控件样式不合法导致授权失败，请开发者先了解安全控件样式的[约束与限制](../../../../harmonyos-guides/系统/安全/程序访问控制/使用安全控件/安全控件概述/security-component-overview.md#约束与限制)。
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/51/v3/0MRiGDITTnKgbGSvqdKXPg/zh-cn_image_0000002592380476.png?HW-CC-KV=V1&HW-CC-Date=20260611T074923Z&HW-CC-Expire=86400&HW-CC-Sign=5B3E818933CA35F6B4466E3988971956C0B818167277C92440C59CCB2D90FF23)
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/dd/v3/7M1FoZjtSGqSjcYF5_sDQw/zh-cn_image_0000002622859987.png?HW-CC-KV=V1&HW-CC-Date=20260611T074923Z&HW-CC-Expire=86400&HW-CC-Sign=A5EAA149DC61C059F8B8890790854EF25909EC3CC6D7DD08D19B6D015FCCFB2B)
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/73/v3/l_uVb6UzThqytuObFdiWWA/zh-cn_image_0000002622700105.jpg?HW-CC-KV=V1&HW-CC-Date=20260611T074923Z&HW-CC-Expire=86400&HW-CC-Sign=0E16DADD73B13B02B4BD8D30B2C63FD80F754959742295BEEC2B96DAEDE404F0)
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/82/v3/4Uh3DLm6S1auUoBXft2Uew/zh-cn_image_0000002592220546.gif?HW-CC-KV=V1&HW-CC-Date=20260611T074923Z&HW-CC-Expire=86400&HW-CC-Sign=04DB27F4925F7A81921CDC05AD1CDC549D4568B03E0543F11DD3AA86972FA5A9)
