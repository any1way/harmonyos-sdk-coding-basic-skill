...
scraped_at: 2026-06-11T15:46:50+08:00
doc_updated_at: 2026-04-20
content_hash: sha256:891fa2ef0f37db729bcd1e2785672b87f685f90c617b10af77b2e09649b6b254
---
...
| isOn8+ | boolean | 否 | 是 | 开关是否打开。  true：打开；false：关闭。  默认值：false  该属性支持[$$](<../../../../../harmonyos-guides/应用框架/ArkUI（方舟UI框架）/UI开发 (ArkTS声明式开发范式)/学习UI范式状态管理/语法糖/$$语法：系统组件双向同步/arkts-two-way-sync.md>)双向绑定变量。  该属性支持[!!](<../../../../../harmonyos-guides/应用框架/ArkUI（方舟UI框架）/UI开发 (ArkTS声明式开发范式)/学习UI范式状态管理/语法糖/!!语法：双向绑定/arkts-new-binding.md#系统组件参数双向绑定>)双向绑定变量。  **卡片能力：** 从API version 9开始，该接口支持在ArkTS卡片中使用。  **元服务API：** 从API version 11开始，该接口支持在元服务中使用。 |
...
Toggle的样式继承对应组件样式的默认值，且不支持设置。例如，如果ToggleType为Button，则该组件样式继承[ButtonType](../Button/ts-basic-components-button.md#buttontype枚举说明)的默认值。由于Button.type从API version 18开始，默认类型从胶囊型变更为圆角矩形，胶囊型按钮不支持设置[borderRadius](../../通用属性/布局与边框/边框设置/ts-universal-attributes-border.md#borderradius)，此时使用Toggle组件设置borderRadius也不生效。
...
| 名称 | 值 | 说明 |
| --- | --- | --- |
| Checkbox | 0 | 提供单选框样式。  **说明：**  API version 11开始，Checkbox默认样式由圆角方形变为圆形。  [通用属性margin](../../通用属性/布局与边框/尺寸设置/ts-universal-attributes-size.md#margin)的默认值为：  {  top: '14px',  right: '14px',  bottom: '14px',  left: '14px'  }。  默认尺寸为：  {width:'20vp', height:'20vp'}。 |
| Switch | 1 | 提供开关样式。  **说明：**  [通用属性margin](../../通用属性/布局与边框/尺寸设置/ts-universal-attributes-size.md#margin)默认值为：  {  top: '6px',  right: '14px',  bottom: '6px',  left: '14px'  }。  默认尺寸为：  {width:'36vp', height:'20vp'}。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| color | [ResourceColor](../../公共定义/基础类型定义/ts-types.md#resourcecolor) | 是 | Switch类型的圆形滑块颜色。  默认值：$r('sys.color.ohos\_id\_color\_foreground\_contrary') |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| modifier | [ContentModifier](../../通用属性/动态属性与自定义/自定义内容/ts-universal-attributes-content-modifier.md#contentmodifiert)[<ToggleConfiguration>](ts-basic-components-toggle.md#toggleconfiguration12对象说明) | 是 | 在Toggle组件上，定制内容区的方法。  modifier：内容修改器，开发者需要自定义class实现ContentModifier接口。 |
...
| 名称 | 类型 | 只读 | 可选 | 说明 |
| --- | --- | --- | --- | --- |
| pointRadius | number | [Resource](../../公共定义/基础类型定义/ts-types.md#resource) | 否 | 是 | 设置Switch类型的圆形滑块半径，单位为vp。  **说明：**  不支持百分比，设定值小于0时按照默认算法设置，设定值大于等于0时按照设定值设置。  未设定此属性时，圆形滑块半径根据默认算法设置。  默认算法：（组件高度（单位：vp） / 2） - （2vp \* 组件高度（单位：vp） / 20vp）。 |
| unselectedColor | [ResourceColor](../../公共定义/基础类型定义/ts-types.md#resourcecolor) | 否 | 是 | 设置Switch类型关闭状态的背景颜色。  默认值：深色和浅色模式下均为0x337F7F7F。从API version 20开始，如果开启了[优化深浅色模式切换开销](<../../../../../harmonyos-guides/应用框架/ArkUI（方舟UI框架）/UI开发 (ArkTS声明式开发范式)/主题设置/应用深浅色适配/ui-dark-light-color-adaptation.md#优化深浅色模式切换开销>)能力，浅色模式下默认值为0x19000000，表现效果为10%透明度的黑色；深色模式下默认值为0x19FFFFFF，表现效果为10%透明度的白色。 |
| pointColor | [ResourceColor](../../公共定义/基础类型定义/ts-types.md#resourcecolor) | 否 | 是 | 设置Switch类型的圆形滑块颜色。  默认值：$r('sys.color.ohos\_id\_color\_foreground\_contrary') |
| trackBorderRadius | number | [Resource](../../公共定义/基础类型定义/ts-types.md#resource) | 否 | 是 | 设置Switch类型的滑轨的圆角，单位为vp。  **说明：**  不支持百分比，设定值小于0时按照默认算法设置，设定值大于组件高度一半时按照组件高度一半设置，其他场合按照设定值设置。  未设定此属性时，滑轨圆角根据默认算法设置。  默认算法：组件高度（单位：vp） / 2。 |
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/da/v3/SVdL3Nr7T9uyrxBlT01tPg/zh-cn_image_0000002592220162.gif?HW-CC-KV=V1&HW-CC-Date=20260611T074649Z&HW-CC-Expire=86400&HW-CC-Sign=F3FFBD150C58C46DC6D47188B8D4415C8BA9E28B1E45894811BE385928305E8A)
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/4a/v3/iD-Y6N9LQ0OwnZ5FIcMbRw/zh-cn_image_0000002592380096.gif?HW-CC-KV=V1&HW-CC-Date=20260611T074649Z&HW-CC-Expire=86400&HW-CC-Sign=5497396FADB977AA5D1549E065B94080FB4DB09DE2AAA5256874A77E495C8D93)
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/7d/v3/wLQPsbJ0TnOVP3Oq4_H0FA/zh-cn_image_0000002622859605.gif?HW-CC-KV=V1&HW-CC-Date=20260611T074649Z&HW-CC-Expire=86400&HW-CC-Sign=0650B01C9A1D4821DC481C899350FFEBB49E444D39F864E69B8CE63192E8CC9B)
