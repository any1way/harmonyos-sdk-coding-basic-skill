...
scraped_at: 2026-06-11T15:46:55+08:00
doc_updated_at: 2026-03-09
content_hash: sha256:031f7a1c5205f210c935cf98e823558ac8ca16b3e43147d3fcf115cdededfac0
---
...
* UIPickerComponent容器的[height](../../通用属性/布局与边框/尺寸设置/ts-universal-attributes-size.md#height)建议设置为200vp。当设置的高度大于等于该建议值时，可完全显示7个选项；小于该建议值时，显示范围会从上下边缘向中间裁剪，可显示的选项数量也会相应减少，始终保持选中项垂直居中。
* 当UIPickerComponent容器未设置[width](../../通用属性/布局与边框/尺寸设置/ts-universal-attributes-size.md#width)时，取当前视图中可见子组件的最大宽度作为容器宽度。建议为UIPickerComponent容器设置宽度，或为每个子组件设置相同宽度，以避免滑动过程中容器宽度动态发生变化，影响显示效果。
* UIPickerComponent容器的子组件的对齐方式固定为居中对齐，不支持通过[align](../../通用属性/布局与边框/位置设置/ts-universal-attributes-location.md#align)属性改变子组件的对齐方式。
...
* 支持多个子组件。
* 支持子组件类型：[Text](../../文本与输入/Text/ts-basic-components-text.md)、[Image](../../图片与视频/Image/ts-basic-components-image.md)、[Row](../../行列与堆叠/Row/ts-container-row.md)和[SymbolGlyph](../../文本与输入/SymbolGlyph/ts-basic-components-symbolglyph.md)。
* 支持渲染控制类型：[if/else](<../../../../../harmonyos-guides/应用框架/ArkUI（方舟UI框架）/UI开发 (ArkTS声明式开发范式)/学习UI范式渲染控制/ifelse：条件渲染/arkts-rendering-control-ifelse.md>)和[ForEach](<../../../../../harmonyos-guides/应用框架/ArkUI（方舟UI框架）/UI开发 (ArkTS声明式开发范式)/学习UI范式渲染控制/ForEach：循环渲染/arkts-rendering-control-foreach.md>)。
...
* 子组件为Text、Image、SymbolGlyph时，[height](../../通用属性/布局与边框/尺寸设置/ts-universal-attributes-size.md#height)属性不生效，固定为40vp。
* 子组件为Row容器时，Row容器的[height](../../通用属性/布局与边框/尺寸设置/ts-universal-attributes-size.md#height)属性不生效，固定为40vp，Row容器内的子组件[height](../../通用属性/布局与边框/尺寸设置/ts-universal-attributes-size.md#height)属性能正常生效，最终显示效果由Row容器决定。
* 图文组合类型选项需要使用Row容器包含图片和文本组件。使用图文组合类型选项时，建议将图片的[height](../../通用属性/布局与边框/尺寸设置/ts-universal-attributes-size.md#height)设置为40vp及以下，避免图片较大时被裁剪。
* UIPickerComponent容器内所有文本组件（包括Row容器内的文本组件）的fontSize属性默认为20fp。用户设置将覆盖默认值，设置异常值时以文本组件[fontSize](../../文本与输入/Text/ts-basic-components-text.md#fontsize)处理的结果为准。建议统一设置或不设置fontSize以保证良好的显示效果。
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| isLoop | [Optional](../../通用属性/动态属性与自定义/自定义属性设置/ts-universal-attributes-custom-property.md#optionalt)<boolean> | 是 | 是否可循环滚动。  - true：可循环滚动。  - false：不可循环滚动。  默认值：true  当isLoop的值为undefined时，使用默认值。  如果子组件的个数小于8个，无论isLoop设置为true还是false，都不会循环滚动。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| enable | [Optional](../../通用属性/动态属性与自定义/自定义属性设置/ts-universal-attributes-custom-property.md#optionalt)<boolean> | 是 | 设置是否开启触控反馈。  - true：开启触控反馈。  - false：不开启触控反馈。  默认值：true  当enable的值为undefined时，使用默认值。  开启后，是否存在触控反馈取决于系统硬件支持情况。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| callback | [Optional](../../通用属性/动态属性与自定义/自定义属性设置/ts-universal-attributes-custom-property.md#optionalt)<[OnUIPickerComponentCallback](ts-container-ui-picker-component.md#onuipickercomponentcallback)> | 是 | 当选中项发生变化时触发的回调函数。  当callback的值为undefined时，不使用回调函数。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| callback | [Optional](../../通用属性/动态属性与自定义/自定义属性设置/ts-universal-attributes-custom-property.md#optionalt)<[OnUIPickerComponentCallback](ts-container-ui-picker-component.md#onuipickercomponentcallback)> | 是 | 当选择器滑动停止时触发的回调函数。  当callback的值为undefined时，不使用回调函数。 |
...
| strokeWidth | [LengthMetrics](<../../../ArkTS API/UI界面/arkui/Graphics/js-apis-arkui-graphics.md#lengthmetrics12>) | 否 | 是 | 分割线的线宽。  默认值：2.0px  单位：与LengthMetrics一致。  取值范围：[0, 选中项高度的一半（即20vp）]。strokeWidth小于0或大于选中项高度的一半时使用默认值。不支持“百分比”类型。  **说明：**  1. 当type为PickerIndicatorType.DIVIDER时生效。  2. 通过LengthMetrics.resource方式设置时，使用非长度属性的值会按照0vp处理。 |
| dividerColor | [ResourceColor](../../公共定义/基础类型定义/ts-types.md#resourcecolor) | 否 | 是 | 分割线的颜色。  默认值：'sys.color.comp\_divider'  **说明：**  当type为PickerIndicatorType.DIVIDER时生效。 |
| startMargin | [LengthMetrics](<../../../ArkTS API/UI界面/arkui/Graphics/js-apis-arkui-graphics.md#lengthmetrics12>) | 否 | 是 | 分割线与UIPickerComponent容器侧边起始端的距离。  默认值：0  单位：与LengthMetrics一致。  取值范围：startMargin与endMargin之和不得超过UIPickerComponent容器的宽度。设置小于0或startMargin与endMargin之和超过UIPickerComponent容器的宽度时，使用默认值。不支持“百分比”类型。  **说明：**  当type为PickerIndicatorType.DIVIDER时生效。 |
| endMargin | [LengthMetrics](<../../../ArkTS API/UI界面/arkui/Graphics/js-apis-arkui-graphics.md#lengthmetrics12>) | 否 | 是 | 分割线与UIPickerComponent容器侧边结束端的距离。  默认值：0  单位：与LengthMetrics一致。  取值范围：startMargin与endMargin之和不得超过UIPickerComponent容器的宽度。设置小于0或startMargin与endMargin之和超过UIPickerComponent容器的宽度时，使用默认值。不支持“百分比”类型。  **说明：**  当type为PickerIndicatorType.DIVIDER时生效。 |
| backgroundColor | [ResourceColor](../../公共定义/基础类型定义/ts-types.md#resourcecolor) | 否 | 是 | 选中项背景的颜色。  默认值：'sys.color.comp\_background\_tertiary'  **说明：**  当type为PickerIndicatorType.BACKGROUND时生效。 |
| borderRadius | [LengthMetrics](<../../../ArkTS API/UI界面/arkui/Graphics/js-apis-arkui-graphics.md#lengthmetrics12>) | [BorderRadiuses](../../公共定义/基础类型定义/ts-types.md#borderradiuses9) | [LocalizedBorderRadiuses](../../公共定义/基础类型定义/ts-types.md#localizedborderradiuses12) | 否 | 是 | 选中项背景的边框圆角半径。  默认值：{ value:12, unit:LengthUnit.vp }，即四个圆角半径均为12vp。  取值范围：取选中项的宽和高之中较小的边长为x，最大不超过x的一半。当取值小于0时，使用默认值；当取值大于最大值时，使用最大值。  **说明：**  1. 当type为PickerIndicatorType.BACKGROUND时生效。  2. [LengthMetrics](<../../../ArkTS API/UI界面/arkui/Graphics/js-apis-arkui-graphics.md#lengthmetrics12>)：统一设置四个圆角半径的大小和单位。  3. [BorderRadiuses](../../公共定义/基础类型定义/ts-types.md#borderradiuses9)：单独设置四个圆角半径的大小（单位为vp）。  4. [LocalizedBorderRadiuses](../../公共定义/基础类型定义/ts-types.md#localizedborderradiuses12)：单独设置四个圆角半径的大小和单位。 |
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/7e/v3/A7r0XTRBQTCc0-sn6Uef4w/zh-cn_image_0000002592380102.gif?HW-CC-KV=V1&HW-CC-Date=20260611T074653Z&HW-CC-Expire=86400&HW-CC-Sign=4E9A30E24FEA9F4ADD508A448B91220C667F56E5F2D11CE28CE3B645A62AF252)
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/f2/v3/VtMc7A_WQuem0kqU9VWQ-g/zh-cn_image_0000002622859611.gif?HW-CC-KV=V1&HW-CC-Date=20260611T074653Z&HW-CC-Expire=86400&HW-CC-Sign=4E2031F73BBBC4BF578D16662CFD13C9060CDF3AA57B9BC8EA3228CFC95B8002)
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/9d/v3/xEgn3Zc2RbKSUhfOmeD2hA/zh-cn_image_0000002622699731.gif?HW-CC-KV=V1&HW-CC-Date=20260611T074653Z&HW-CC-Expire=86400&HW-CC-Sign=4CD9645D0DD1725006950EC6C797B79CCFA771214E70D82E0D7441B169243239)
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/ae/v3/jisLfxRIR3eEgWtxbIUzUA/zh-cn_image_0000002592220170.gif?HW-CC-KV=V1&HW-CC-Date=20260611T074653Z&HW-CC-Expire=86400&HW-CC-Sign=B2CCA5CA5FDEBD9035C2F7EDAC423B01C9AF7AD22F23F90F11B34C3FCCDD6BCE)
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/b0/v3/pCdpNP7xTJmlqp-6d9mc1w/zh-cn_image_0000002592380104.gif?HW-CC-KV=V1&HW-CC-Date=20260611T074653Z&HW-CC-Expire=86400&HW-CC-Sign=820D313E6EE0EEC1951E6A69D779AF07062E1AD27A1CD6150C7559E9085A577F)
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/b7/v3/8kqA3k_pQ3qlExIVeugPLA/zh-cn_image_0000002622859613.gif?HW-CC-KV=V1&HW-CC-Date=20260611T074653Z&HW-CC-Expire=86400&HW-CC-Sign=E1D61F7FD8627A883E097F77C5E4FDF6065FBC3D6DC08E9F3F8B9C6C1598D445)
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/f/v3/rGVtAnJtTm-8gc_srK_1Qw/zh-cn_image_0000002622699733.gif?HW-CC-KV=V1&HW-CC-Date=20260611T074653Z&HW-CC-Expire=86400&HW-CC-Sign=E30CB266FEEC01B4452B034C5DFD98C7E46E25A4C32EC9D125428478C5BBB1F5)
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/7e/v3/HJW9U8ZaRyuv5qt4vOddDw/zh-cn_image_0000002592220172.gif?HW-CC-KV=V1&HW-CC-Date=20260611T074653Z&HW-CC-Expire=86400&HW-CC-Sign=5BCADB4E6B1A27413857220912DA11CA851A563A499F39FBBF5FECFA6FCE346C)
