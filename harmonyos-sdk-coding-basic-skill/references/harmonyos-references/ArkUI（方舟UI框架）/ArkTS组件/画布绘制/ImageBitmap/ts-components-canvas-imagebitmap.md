...
scraped_at: 2026-06-11T15:48:24+08:00
doc_updated_at: 2026-05-26
content_hash: sha256:b35bee99512a4ee53d4d2278b7e00dacbbbe261b6f810dd06cf065ef4e4b54e6
---
ImageBitmap对象可以存储canvas渲染的像素数据。从API version 11开始，当应用创建[Worker线程](../../../../../harmonyos-guides/应用框架/ArkTS（方舟编程语言）/ArkTS并发/多线程并发/Worker简介/worker-introduction.md)，支持使用postMessage将ImageBitmap实例传到Worker中进行绘制，并使用onmessage接收Worker线程发送的绘制结果进行显示。

说明

* 从 API version 8 开始支持。后续版本如有新增内容，则采用上角标单独标记该内容的起始版本。
* ImageBitmap对象仅支持加载静态图片，如需播放动图，建议使用[Image](../../图片与视频/Image/ts-basic-components-image.md)组件。
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| src | string | 是 | 图片的数据源支持本地图片。  1、string格式用于加载本地图片，例如ImageBitmap("common/images/example.jpg")，type为"entry"和"feature"类型的Module，其图片加载路径的起点为当前Module的ets文件夹，type为"har"和"shared"类型的Module，其图片加载路径的起点为当前构建的"entry"或"feature"类型Module的ets文件夹。  type为"har"和"shared"类型的Module中推荐使用[ImageSource](<../../../../../harmonyos-guides/媒体/Image Kit（图片处理服务）/图片开发指导(ArkTS)/图片解码/使用ImageSource完成图片解码/image-decoding.md>)图片解码方式将资源图片解码为统一的PixelMap加载使用。  2、支持本地图片类型：bmp、jpg、png、svg和webp类型。  **说明：**  - ArkTS卡片上不支持http://等网络相关路径前缀、datashare://路径前缀以及file://data/storage路径前缀的字符串。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| data | [PixelMap](<../../../../Image Kit（图片处理服务）/ArkTS API/@ohos.multimedia.image (图片处理)/Interface (PixelMap)/arkts-apis-image-pixelmap.md>) | 是 | 图片的数据源支持PixelMap对象。 |
...
| --- | --- | --- | --- |
| src | string | 是 | 图片的数据源支持本地图片。  1、string格式用于加载本地图片，例如ImageBitmap("common/images/example.jpg")，type为"entry"和"feature"类型的Module，其图片加载路径的起点为当前Module的ets文件夹，type为"har"和"shared"类型的Module，其图片加载路径的起点为当前构建的"entry"或"feature"类型Module的ets文件夹。  type为"har"和"shared"类型的Module中推荐使用[ImageSource](<../../../../../harmonyos-guides/媒体/Image Kit（图片处理服务）/图片开发指导(ArkTS)/图片解码/使用ImageSource完成图片解码/image-decoding.md>)图片解码方式将资源图片解码为统一的PixelMap加载使用。  2、支持本地图片类型：bmp、jpg、png、svg和webp类型。  **说明：**  - ArkTS卡片上不支持http://等网络相关路径前缀、datashare://路径前缀以及file://data/storage路径前缀的字符串。 |
| unit | [LengthMetricsUnit](<../../../ArkTS API/UI界面/arkui/Graphics/js-apis-arkui-graphics.md#lengthmetricsunit12>) | 是 | 用来配置ImageBitmap对象的单位模式，配置后无法动态更改，配置方法同[CanvasRenderingContext2D](../CanvasRenderingContext2D/ts-canvasrenderingcontext2d.md)。  异常值undefined、NaN和Infinity按默认值处理。 |
...
| --- | --- | --- | --- |
| data | [PixelMap](<../../../../Image Kit（图片处理服务）/ArkTS API/@ohos.multimedia.image (图片处理)/Interface (PixelMap)/arkts-apis-image-pixelmap.md>) | 是 | 图片的数据源支持PixelMap对象。 |
| unit | [LengthMetricsUnit](<../../../ArkTS API/UI界面/arkui/Graphics/js-apis-arkui-graphics.md#lengthmetricsunit12>) | 是 | 用来配置ImageBitmap对象的单位模式，配置后无法动态更改，配置方法同[CanvasRenderingContext2D](../CanvasRenderingContext2D/ts-canvasrenderingcontext2d.md)。 |
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/2a/v3/Qi1KOUMNTYCO60VHnRgZtQ/zh-cn_image_0000002622699999.png?HW-CC-KV=V1&HW-CC-Date=20260611T074823Z&HW-CC-Expire=86400&HW-CC-Sign=F584852B31161101D9DCFC132ACF7C3004BE08442A9DF4B7B3A0E7B0F55E9373)
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/aa/v3/RIqGXUEVSlOPdkTwzyEKqw/zh-cn_image_0000002592220438.png?HW-CC-KV=V1&HW-CC-Date=20260611T074823Z&HW-CC-Expire=86400&HW-CC-Sign=12D6E95AFF21661439234F9A022168D9555B76E8776D895327C57F4A1D297895)
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/37/v3/_bv7CWZWQROnO1YnDtTpBA/zh-cn_image_0000002622699999.png?HW-CC-KV=V1&HW-CC-Date=20260611T074823Z&HW-CC-Expire=86400&HW-CC-Sign=30C00AC0F2D1719DCF8EFFFBF5FC1873C2EF9A250D645268D28AE1807F5038E0)
