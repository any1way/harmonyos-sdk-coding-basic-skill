...
scraped_at: 2026-06-11T15:45:45+08:00
doc_updated_at: 2026-04-28
content_hash: sha256:8718d9c607716ebe54e6b6d83f57911c27febf90a25f411810dcddb4c278503f
---
Flex是以弹性方式布局子组件的容器组件，能够高效地排列、对齐子元素并分配剩余空间。

具体指南请参考[弹性布局](<../../../../../harmonyos-guides/应用框架/ArkUI（方舟UI框架）/UI开发 (ArkTS声明式开发范式)/组件布局/构建布局/弹性布局 (Flex)/arkts-layout-development-flex-layout.md>)。

说明

* 该组件从API version 7开始支持。后续版本如有新增内容将采用上角标单独标记该内容的起始版本。
* Flex组件在渲染时存在二次布局过程，因此在对性能有严格要求的场景下建议使用[Column](../Column/ts-container-column.md)、[Row](../Row/ts-container-row.md)代替。最佳实践请参考[布局优化指导-合理使用布局组件](../../../../../best-practices/布局与弹窗/布局优化指导/bpta-improve-layout-performance.md#section12745188175420)。
* Flex组件主轴不设置长度时默认撑满父容器，如果包含设置[position](../../通用属性/布局与边框/位置设置/ts-universal-attributes-location.md#position)的子组件，此时Flex组件不会撑满父容器。[Column](../Column/ts-container-column.md)、[Row](../Row/ts-container-row.md)组件主轴不设置长度时默认跟随子节点大小。
* Flex、Column、Row组件在没有子节点且不设置宽高时，默认宽高为-1。
* 主轴长度可设置为auto使Flex自适应子组件布局，自适应时，Flex长度受[constraintSize](../../通用属性/布局与边框/尺寸设置/ts-universal-attributes-size.md#constraintsize)属性以及父容器传递的最大最小长度限制，且constraintSize属性优先级更高。
...
| 名称 | 类型 | 只读 | 可选 | 说明 |
| --- | --- | --- | --- | --- |
| direction | [FlexDirection](../../公共定义/枚举说明/ts-appendix-enums.md#flexdirection) | 否 | 是 | 子组件在Flex容器上排列的方向，即主轴的方向。  默认值：FlexDirection.Row  异常值按默认值处理。  **卡片能力：** 从API version 9开始，该接口支持在ArkTS卡片中使用。  **元服务API：** 从API version 11开始，该接口支持在元服务中使用。 |
| wrap | [FlexWrap](../../公共定义/枚举说明/ts-appendix-enums.md#flexwrap) | 否 | 是 | Flex容器是单行/列还是多行/列排列。  默认值：FlexWrap.NoWrap  异常值按默认值处理。  **说明：**  在多行布局时，通过交叉轴方向，确认新行堆叠方向。  **卡片能力：** 从API version 9开始，该接口支持在ArkTS卡片中使用。  **元服务API：** 从API version 11开始，该接口支持在元服务中使用。 |
| justifyContent | [FlexAlign](../../公共定义/枚举说明/ts-appendix-enums.md#flexalign) | 否 | 是 | 所有子组件在Flex容器主轴上的对齐格式。  默认值：FlexAlign.Start  异常值按默认值处理。  **卡片能力：** 从API version 9开始，该接口支持在ArkTS卡片中使用。  **元服务API：** 从API version 11开始，该接口支持在元服务中使用。 |
| alignItems | [ItemAlign](../../公共定义/枚举说明/ts-appendix-enums.md#itemalign) | 否 | 是 | 所有子组件在Flex容器交叉轴上的对齐格式。  默认值：ItemAlign.Start  异常值按默认值处理。  **卡片能力：** 从API version 9开始，该接口支持在ArkTS卡片中使用。  **元服务API：** 从API version 11开始，该接口支持在元服务中使用。 |
| alignContent | [FlexAlign](../../公共定义/枚举说明/ts-appendix-enums.md#flexalign) | 否 | 是 | 当交叉轴存在额外空间时，多行内容之间的对齐方式。仅在wrap为Wrap或WrapReverse下生效。  默认值：FlexAlign.Start  异常值按默认值处理。  **卡片能力：** 从API version 9开始，该接口支持在ArkTS卡片中使用。  **元服务API：** 从API version 11开始，该接口支持在元服务中使用。 |
...
| 名称 | 类型 | 只读 | 可选 | 说明 |
| --- | --- | --- | --- | --- |
| main | [LengthMetrics](<../../../ArkTS API/UI界面/arkui/Graphics/js-apis-arkui-graphics.md#lengthmetrics12>) | 否 | 是 | Flex容器主轴上的space。  默认值：LengthMetrics.px(0) |
| cross | [LengthMetrics](<../../../ArkTS API/UI界面/arkui/Graphics/js-apis-arkui-graphics.md#lengthmetrics12>) | 否 | 是 | Flex容器交叉轴上的space。  默认值：LengthMetrics.px(0) |
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/dc/v3/rD-0YgpcQLOnrGTVIyQOPw/zh-cn_image_0000002592379988.png?HW-CC-KV=V1&HW-CC-Date=20260611T074544Z&HW-CC-Expire=86400&HW-CC-Sign=A44F98ADED342AE3A480C85BBB2F9CC41BD0F1CA3AF7FC8E07521B1FB1E01E6B)
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/a5/v3/60nGEez6Q6uIOHmjk5Qmfg/zh-cn_image_0000002622859497.png?HW-CC-KV=V1&HW-CC-Date=20260611T074544Z&HW-CC-Expire=86400&HW-CC-Sign=25D94056267495191904C93E637F2D94D173CFFC254A0A9D32FE0E046BBCBD96)
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/d4/v3/We8HSrFpTfykYQXFo1R5sQ/zh-cn_image_0000002622699617.png?HW-CC-KV=V1&HW-CC-Date=20260611T074544Z&HW-CC-Expire=86400&HW-CC-Sign=975CD34AE69804843409EBFD2F9573E24B31F163F558E4C228BC4E4405761BB2)
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/f1/v3/PQQMa1heT--hsfKjKF-6tQ/zh-cn_image_0000002592220056.png?HW-CC-KV=V1&HW-CC-Date=20260611T074544Z&HW-CC-Expire=86400&HW-CC-Sign=8C77178464A0CF55FE5931EC5406C52B3D23DF6E22B4396A44C3D727F2252859)
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/98/v3/BPjvT4d0S4CVu0Jn0cel2w/zh-cn_image_0000002592379990.png?HW-CC-KV=V1&HW-CC-Date=20260611T074544Z&HW-CC-Expire=86400&HW-CC-Sign=09E1E9AD73C2304D034B47C3F314C8838370D9B035D655639FF4A7E916642315)
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/f3/v3/ax9QU5QfTxm2CT337rFlmA/zh-cn_image_0000002622859499.png?HW-CC-KV=V1&HW-CC-Date=20260611T074544Z&HW-CC-Expire=86400&HW-CC-Sign=7D194F7E3BA64B79856636588BE359BDF29F692C8A3C71741202FEA85C875DC4)
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/a4/v3/f6USd9Z1SNmpRvxK8iBnOg/zh-cn_image_0000002622699619.png?HW-CC-KV=V1&HW-CC-Date=20260611T074544Z&HW-CC-Expire=86400&HW-CC-Sign=16D8A484DABD09815D1030126992D40ED2224986831D0F5D09F02CFC209C639C)
