...
content_hash: sha256:4462f98b3201546378c1d9927107b1ceae591a2b078fb05e97ed1dae3f0e8f7e
---
作为[Text](../Text/ts-basic-components-text.md)、[ContainerSpan](../ContainerSpan/ts-basic-components-containerspan.md)组件的子组件，用于显示行内文本的组件。
...
不支持[通用属性](../../通用属性/ts-component-general-attributes.md)。若需设置通用属性，应使用[Text](../Text/ts-basic-components-text.md)进行设置，或改用[属性字符串](../属性字符串/ts-universal-styled-string.md)中的[CustomSpan](../属性字符串/ts-universal-styled-string.md#customspan)自行绘制。

[通用事件](../../通用事件/ts-component-general-events.md)只支持点击事件[onClick](../../通用事件/交互响应事件/点击事件/ts-universal-events-click.md#onclick)和悬浮事件[onHover](../../通用事件/交互响应事件/悬浮事件/ts-universal-events-hover.md#onhover)。
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| value | string | [Resource](../../公共定义/基础类型定义/ts-types.md#resource) | 是 | 文本内容。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| value | number | [ResourceStr](../../公共定义/基础类型定义/ts-types.md#resourcestr) | 是 | 文本字符间距。  单位：[fp](../../公共定义/像素单位/ts-pixel-units.md#基本像素单位)  从API version 20开始，支持[Resource](../../公共定义/基础类型定义/ts-types.md#resource)类型。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| value | number | string | [Resource](../../公共定义/基础类型定义/ts-types.md#resource) | 是 | 字体大小。fontSize为number类型时，使用fp单位。字体默认大小16fp。string类型支持number类型取值的字符串形式，可以附带单位，例如"10"、"10fp"，不支持设置百分比字符串。  Wearable设备上默认值为：15fp |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| weight | number | [FontWeight](../../公共定义/枚举说明/ts-appendix-enums.md#fontweight) | [ResourceStr](../../公共定义/基础类型定义/ts-types.md#resourcestr) | 是 | 文本的字体粗细，number类型取值[100, 900]，取值间隔为100，默认为400，取值越大，字体越粗。string类型仅支持number类型取值的字符串形式，例如"400"，以及"bold"、"bolder"、"lighter"、"regular"、"medium"，分别对应FontWeight中相应的枚举值。设置过大可能会在不同字体下有截断。  默认值：FontWeight.Normal |
| fontWeightConfigs | [FontWeightConfigs](../文本组件公共接口/ts-text-common.md#fontweightconfigs24对象说明) | 否 | 字体粗细配置。默认值继承[FontWeightConfigs](../文本组件公共接口/ts-text-common.md#fontweightconfigs24对象说明)。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| value | string | [Resource](../../公共定义/基础类型定义/ts-types.md#resource) | 是 | 字体列表。  默认字体'HarmonyOS Sans'。  使用多个字体时，请用逗号','分隔，字体的优先级按顺序生效。例如：'Arial,HarmonyOS Sans'。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| value | [Length](../../公共定义/基础类型定义/ts-types.md#length) | 是 | 文本行高。  number类型时单位为fp。设置string类型时，支持number类型取值的字符串形式，可以附带单位，例如"10"、"10fp"。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| value | [Font](../../公共定义/基础类型定义/ts-types.md#font) | 是 | 文本样式，包括字体大小、字体粗细、字体族和字体风格。 |
| fontConfigs | [FontConfigs](../文本组件公共接口/ts-text-common.md#fontconfigs24对象说明) | 否 | 字体配置。默认值继承[FontConfigs](../文本组件公共接口/ts-text-common.md#fontconfigs24对象说明)。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| value | [ShadowOptions](../../通用属性/视效与模糊/图像效果/ts-universal-attributes-image-effect.md#shadowoptions对象说明) | Array<[ShadowOptions](../../通用属性/视效与模糊/图像效果/ts-universal-attributes-image-effect.md#shadowoptions对象说明)> | 是 | 文字阴影效果。 |

## 事件

PhonePC/2in1TabletTVWearable

通用事件支持[点击事件onClick](../../通用事件/交互响应事件/点击事件/ts-universal-events-click.md#onclick)、[悬浮事件onHover](../../通用事件/交互响应事件/悬浮事件/ts-universal-events-hover.md#onhover)。
...
设置背景样式。作为[ContainerSpan](../ContainerSpan/ts-basic-components-containerspan.md)的子组件时可以继承它的此属性值，优先使用其自身的此属性。
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| value | [LengthMetrics](<../../../ArkTS API/UI界面/arkui/Graphics/js-apis-arkui-graphics.md#lengthmetrics12>) | 是 | 设置Span基线的偏移量，设置该值为百分比时，按默认值显示。  正数内容向上偏移，负数向下偏移。  默认值：0  在ImageSpan中，设置为非0时，[verticalAlign](../ImageSpan/ts-basic-components-imagespan.md#verticalalign)将固定为ImageSpanAlignment.BASELINE对齐；设置为0时，要使基线对齐策略生效，需同时设置[verticalAlign](../ImageSpan/ts-basic-components-imagespan.md#verticalalign)为ImageSpanAlignment.BASELINE。 |
...
| 名称 | 类型 | 只读 | 可选 | 说明 |
| --- | --- | --- | --- | --- |
| color | [ResourceColor](../../公共定义/基础类型定义/ts-types.md#resourcecolor) | 否 | 是 | 文本背景色。 |
| radius | [Dimension](../../公共定义/基础类型定义/ts-types.md#dimension10) | [BorderRadiuses](../../公共定义/基础类型定义/ts-types.md#borderradiuses9) | 否 | 是 | 文本背景圆角。 |
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/b9/v3/inOvk_0CTgqz2o_YCImJAQ/zh-cn_image_0000002622859719.png?HW-CC-KV=V1&HW-CC-Date=20260611T074727Z&HW-CC-Expire=86400&HW-CC-Sign=2FF8E0185DA651A3C47D9D4206B7229756D4C4620E011C7655615172745A4508)
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/99/v3/r1Vl5DyxRaywn7QZBdda9Q/zh-cn_image_0000002622699839.png?HW-CC-KV=V1&HW-CC-Date=20260611T074727Z&HW-CC-Expire=86400&HW-CC-Sign=282063E661F8ACBD5576F8F24A18650A6C866C65BFEEE67224AC7A6245207F58)
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/9b/v3/q-hHW3YLTZuKhsNcBtKswQ/zh-cn_image_0000002592220278.png?HW-CC-KV=V1&HW-CC-Date=20260611T074727Z&HW-CC-Expire=86400&HW-CC-Sign=2040D5D9410C4AA1635E185867A1B3A4D451FB4AD329EB79D16E476F1B2122DE)
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/1b/v3/2TYIQZHAQaqoSyz6cm2m_Q/zh-cn_image_0000002592380212.png?HW-CC-KV=V1&HW-CC-Date=20260611T074727Z&HW-CC-Expire=86400&HW-CC-Sign=6B16AA901B6B293A4E63198509503D7DB227B2A121C0471126DFA499672129EA)
