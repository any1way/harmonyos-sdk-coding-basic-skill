...
content_hash: sha256:27ad5cca948e50eaba60a81b1cc8186916aa15b57217b0b0d5640fbd0118bf9d
---
[Text](../Text/ts-basic-components-text.md)、[ContainerSpan](../ContainerSpan/ts-basic-components-containerspan.md)组件的子组件，用于显示行内图片。
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| value | [ResourceStr](../../公共定义/基础类型定义/ts-types.md#resourcestr) | [PixelMap](<../../../../Image Kit（图片处理服务）/ArkTS API/@ohos.multimedia.image (图片处理)/Interface (PixelMap)/arkts-apis-image-pixelmap.md>) | 是 | 图片的数据源，支持本地图片和网络图片。  当使用相对路径引用图片资源时，例如ImageSpan("common/test.jpg")，不支持跨包/跨模块调用该ImageSpan组件，建议使用$r方式来管理需全局使用的图片资源。  - 支持的图片格式包括png、jpg、bmp、svg、gif和heif。  - 支持Base64字符串。格式data:image/[png|jpeg|bmp|webp|heif];base64,[base64 data]，其中[base64 data]为Base64字符串数据。  - 支持file://data/storage路径前缀的字符串，用于读取本应用安装目录下file文件夹下的图片资源。需要保证目录包路径下的文件有可读权限。 |

## 属性

PhonePC/2in1TabletTVWearable

属性继承自[BaseSpan](../Span/ts-basic-components-span.md#basespan)，通用属性方法支持[尺寸设置](../../通用属性/布局与边框/尺寸设置/ts-universal-attributes-size.md)、[背景设置](../../通用属性/基础属性/背景设置/ts-universal-attributes-background.md)、[边框设置](../../通用属性/布局与边框/边框设置/ts-universal-attributes-border.md)。
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| value | [ImageSpanAlignment](../../公共定义/枚举说明/ts-appendix-enums.md#imagespanalignment10) | 是 | 图片基于行高的对齐方式。  默认值：ImageSpanAlignment.BOTTOM |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| value | [ImageFit](../../公共定义/枚举说明/ts-appendix-enums.md#imagefit) | 是 | 图片的缩放类型。  默认值：ImageFit.Cover |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| value | [PixelMap](<../../../../Image Kit（图片处理服务）/ArkTS API/@ohos.multimedia.image (图片处理)/Interface (PixelMap)/arkts-apis-image-pixelmap.md>) | 是 | 设置图片加载过程中显示的占位图，支持[PixelMap](<../../../../Image Kit（图片处理服务）/ArkTS API/@ohos.multimedia.image (图片处理)/Interface (PixelMap)/arkts-apis-image-pixelmap.md>)类型。  默认值：null |
...
开启或关闭[SVG标签解析能力增强功能](../../图片与视频/SVG标签解析能力增强/ts-image-svg2-capabilities.md)，开启后相关SVG图片显示效果会有变化。
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| enable | Optional<boolean> | 是 | 控制是否开启[SVG标签解析能力增强功能](../../图片与视频/SVG标签解析能力增强/ts-image-svg2-capabilities.md)。  true：支持SVG解析新能力；false：保持原有SVG解析能力。  默认值：false |

## 事件

PhonePC/2in1TabletTVWearable

通用事件仅支持[点击控制事件](../../已停止维护的组件与接口/点击控制/ts-universal-attributes-click.md)。还支持以下事件：
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| callback | [ImageErrorCallback](../../图片与视频/Image/ts-basic-components-image.md#imageerrorcallback9) | 是 | 图片加载异常时触发的回调。 |
...
| 名称 | 类型 | 只读 | 可选 | 说明 |
| --- | --- | --- | --- | --- |
| width | number | 否 | 否 | 图片的宽。  单位：[px](../../公共定义/像素单位/ts-pixel-units.md#基本像素单位) |
| height | number | 否 | 否 | 图片的高。  单位：[px](../../公共定义/像素单位/ts-pixel-units.md#基本像素单位) |
| componentWidth | number | 否 | 否 | 组件的宽。  单位：[px](../../公共定义/像素单位/ts-pixel-units.md#基本像素单位) |
| componentHeight | number | 否 | 否 | 组件的高。  单位：[px](../../公共定义/像素单位/ts-pixel-units.md#基本像素单位) |
| loadingStatus | number | 否 | 否 | 图片加载成功的状态值。  **说明：**  返回的状态值为0时，表示图片数据加载成功。返回的状态值为1时，表示图片解码成功。 |
| contentWidth | number | 否 | 否 | 图片实际绘制的宽度。  单位：[px](../../公共定义/像素单位/ts-pixel-units.md#基本像素单位)  **说明：**  仅在loadingStatus返回1时有效。 |
| contentHeight | number | 否 | 否 | 图片实际绘制的高度。  单位：[px](../../公共定义/像素单位/ts-pixel-units.md#基本像素单位)  **说明：**  仅在loadingStatus返回1时有效。 |
| contentOffsetX | number | 否 | 否 | 实际绘制内容相对于组件自身的x轴偏移。  单位：[px](../../公共定义/像素单位/ts-pixel-units.md#基本像素单位)  **说明：**  仅在loadingStatus返回1时有效。 |
| contentOffsetY | number | 否 | 否 | 实际绘制内容相对于组件自身的y轴偏移。  单位：[px](../../公共定义/像素单位/ts-pixel-units.md#基本像素单位)  **说明：**  仅在loadingStatus返回1时有效。 |
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/7c/v3/XhvNRocOQeCBd-T32u1qDQ/zh-cn_image_0000002592220280.png?HW-CC-KV=V1&HW-CC-Date=20260611T074728Z&HW-CC-Expire=86400&HW-CC-Sign=EA71C580C1D4149D07ECFAB2401FCAD5AE6F3554D13362609708F219D94B0E3D)

### 示例2（设置背景样式）

从API version 11开始，该示例通过[textBackgroundStyle](../Span/ts-basic-components-span.md#textbackgroundstyle11)属性展示了文本设置背景样式的效果。
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/36/v3/nlMgTFFJRvqNdHQ2Nssk2w/zh-cn_image_0000002592380214.png?HW-CC-KV=V1&HW-CC-Date=20260611T074728Z&HW-CC-Expire=86400&HW-CC-Sign=3BC6C8D8706E5851DFAA062B6D2BE117F3DDDF869F1779ECBCDE4D1B7EFDF107)
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/d8/v3/mAKFXHWtTJuKsbdHa-N5mw/zh-cn_image_0000002622859723.png?HW-CC-KV=V1&HW-CC-Date=20260611T074728Z&HW-CC-Expire=86400&HW-CC-Sign=D316D4141A0D4D58F66C2BF201B9BD2EC62E38A0FDE3F0F4393E5FB3B584977A)
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/c0/v3/M6RqT1P5QriAqoMluEEU7w/zh-cn_image_0000002622699843.gif?HW-CC-KV=V1&HW-CC-Date=20260611T074728Z&HW-CC-Expire=86400&HW-CC-Sign=6A02338A66122B017C887566AD1BF5FCBD22879983B5ED804436229718F41FBB)

### 示例6（使用supportSvg2属性时，SVG图片的显示效果）

从API version 22开始，该示例通过设置[supportSvg2](ts-basic-components-imagespan.md#supportsvg222)属性，使[SVG标签解析能力增强功能](../../图片与视频/SVG标签解析能力增强/ts-image-svg2-capabilities.md)的[SVG易用性提升](../../图片与视频/SVG标签解析能力增强/ts-image-svg2-capabilities.md#svg易用性提升)能力生效。
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/9a/v3/1rSMRM9HQbeyzbd3VG2VCg/zh-cn_image_0000002592220282.png?HW-CC-KV=V1&HW-CC-Date=20260611T074728Z&HW-CC-Expire=86400&HW-CC-Sign=CE6AC52F5A42B976F8BF5AC289DC82452496D7D5FB80B082F093BB6DD215DF52)
