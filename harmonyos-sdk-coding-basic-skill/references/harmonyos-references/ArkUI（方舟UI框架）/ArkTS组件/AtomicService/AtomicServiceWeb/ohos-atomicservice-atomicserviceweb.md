...
scraped_at: 2026-06-11T15:49:37+08:00
doc_updated_at: 2026-06-09
content_hash: sha256:cd3ddc9502432da0d68a6b4a1d325811d7097fb59b17b4e41acb9be3620222c6
---
...
| 名称 | 类型 | 必填 | 装饰器类型 | 说明 |
| --- | --- | --- | --- | --- |
| src | [ResourceStr](../../公共定义/基础类型定义/ts-types.md#resourcestr) | 是 | - | 网页资源地址，访问网络资源需要在AGC配置业务域名，访问本地资源仅支持包内文件（$rawfile）。不支持通过状态变量（例如@State）动态更新地址。加载的网页中支持通过JS SDK提供的接口调用系统能力，具体以JS SDK为准。 |
...
| navPathStack | [NavPathStack](../../导航与切换/Navigation/ts-basic-components-navigation.md#navpathstack10) | 否 | - | 路由栈信息。当使用NavDestination作为页面的根容器时，需传入NavDestination容器对应的NavPathStack处理页面路由。 |
| mixedMode | [MixedMode](<../../../../ArkWeb（方舟Web）/ArkTS 组件/Web/Enums/arkts-basic-components-web-e.md#mixedmode>) | 否 | @Prop | 设置是否允许加载超文本传输协议（HTTP）和超文本传输安全协议（HTTPS）混合内容，默认不允许加载HTTP和HTTPS混合内容。 |
| darkMode | [WebDarkMode](<../../../../ArkWeb（方舟Web）/ArkTS 组件/Web/Enums/arkts-basic-components-web-e.md#webdarkmode9>) | 否 | @Prop | 设置Web深色模式，默认关闭。 |
| forceDarkAccess | boolean | 否 | @Prop | 设置网页是否开启强制深色模式。true表示设置网页开启强制深色模式，false表示设置网页不开启强制深色模式。默认值：false。该属性仅在darkMode开启深色模式时生效。 |
| nestedScroll15+ | [NestedScrollOptions](../../滚动与滑动/滚动组件通用接口/ts-container-scrollable-common.md#nestedscrolloptions10对象说明) | [NestedScrollOptionsExt](<../../../../ArkWeb（方舟Web）/ArkTS 组件/Web/Interfaces（其他）/arkts-basic-components-web-i.md#nestedscrolloptionsext14>) | 否 | @Prop | 设置嵌套滚动选项。  **元服务API：** 从API version 15开始，该接口支持在元服务中使用。 |
...
| onProgressChange | Callback<[OnProgressChangeEvent](<../../../../ArkWeb（方舟Web）/ArkTS 组件/Web/Interfaces（其他）/arkts-basic-components-web-i.md#onprogresschangeevent12>)> | 否 | - | 网页加载进度变化时触发该回调。 |
...
| 类型 | 说明 |
| --- | --- |
| string | 默认用户代理。默认User-Agent定义与使用场景请参考[User-Agent开发指导](../../../../../harmonyos-guides/应用框架/ArkWeb（方舟Web）/设置基本属性和事件/User-Agent开发指导/web-default-useragent.md)。 |

**错误码：**

以下错误码的详细介绍请参见[错误码](../../../../ArkWeb（方舟Web）/错误码/Webview错误码/errorcode-webview.md)。
...
| 类型 | 说明 |
| --- | --- |
| string | 用户自定义代理信息。默认User-Agent定义与使用场景请参考[User-Agent开发指导](../../../../../harmonyos-guides/应用框架/ArkWeb（方舟Web）/设置基本属性和事件/User-Agent开发指导/web-default-useragent.md)。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| url | string | [Resource](../../公共定义/基础类型定义/ts-types.md#resource) | 是 | 需要加载的 URL。 |
...
| 名称 | 类型 | 只读 | 可选 | 说明 |
| --- | --- | --- | --- | --- |
| request | [WebResourceRequest](<../../../../ArkWeb（方舟Web）/ArkTS 组件/Web/Class (WebResourceRequest)/arkts-basic-components-web-webresourcerequest.md>) | 否 | 否 | 网页请求的封装信息。 |
| error | [WebResourceError](<../../../../ArkWeb（方舟Web）/ArkTS 组件/Web/Class (WebResourceError)/arkts-basic-components-web-webresourceerror.md>) | 否 | 否 | 网页加载资源错误的封装信息 。 |
...
| --- | --- | --- | --- | --- |
| request | [WebResourceRequest](<../../../../ArkWeb（方舟Web）/ArkTS 组件/Web/Class (WebResourceRequest)/arkts-basic-components-web-webresourcerequest.md>) | 否 | 否 | 网页请求的封装信息。 |
| response | [WebResourceResponse](<../../../../ArkWeb（方舟Web）/ArkTS 组件/Web/Class (WebResourceResponse)/arkts-basic-components-web-webresourceresponse.md>) | 否 | 否 | 资源响应的封装信息。 |
...
| 名称 | 类型 | 只读 | 可选 | 说明 |
| --- | --- | --- | --- | --- |
| data | [WebResourceRequest](<../../../../ArkWeb（方舟Web）/ArkTS 组件/Web/Class (WebResourceRequest)/arkts-basic-components-web-webresourcerequest.md>) | 否 | 否 | 网页请求的封装信息。 |
...
36. console.info(`AtomicServiceWebLog forward`);
37. })
38. Button('backward').onClick(() => {
39. this.controller.backward();
40. console.info(`AtomicServiceWebLog backward`);
41. })
42. Button('refresh').onClick(() => {
43. this.controller.refresh();
44. console.info(`AtomicServiceWebLog refresh`);
45. })
46. Button('loadUrl').onClick(() => {
47. this.controller.loadUrl('https://www.baidu.com/');
48. console.info(`AtomicServiceWebLog loadUrl`);
49. })
50. Button('深色模式').onClick(() => {
51. this.forceDarkAccess = !this.forceDarkAccess;
52. })
53. Button('mixedMode').onClick(() => {
54. this.mixedMode = this.mixedMode == MixedMode.None ? MixedMode.All : MixedMode.None;
55. })
56. Button('点击').onClick(() => {
57. console.info(`AtomicServiceWebLog getUserAgent = ${this.controller.getUserAgent()}`);
58. console.info(`AtomicServiceWebLog getCustomUserAgent = ${this.controller.getCustomUserAgent()}`);
59. this.controller.setCustomUserAgent('test' + this.num++);

61. console.info(`AtomicServiceWebLog getUserAgent after set = ${this.controller.getUserAgent()}`);
62. console.info(`AtomicServiceWebLog getCustomUserAgent after set = ${this.controller.getCustomUserAgent()}`);
63. })
64. AtomicServiceWeb({
65. src: 'https://www.example.com',
66. mixedMode: this.mixedMode,
67. darkMode: this.darkMode,
68. forceDarkAccess: this.forceDarkAccess,
69. controller: this.controller,
70. onControllerAttached: () => {
71. console.info("AtomicServiceWebLog onControllerAttached call back success");
72. },
73. onLoadIntercept: (event: OnLoadInterceptEvent) => {
74. console.info("AtomicServiceWebLog onLoadIntercept call back success " + JSON.stringify({
75. getRequestUrl: event.data.getRequestUrl(),
76. getRequestMethod: event.data.getRequestMethod(),
77. getRequestHeader: event.data.getRequestHeader(),
78. isRequestGesture: event.data.isRequestGesture(),
79. isMainFrame: event.data.isMainFrame(),
80. isRedirect: event.data.isRedirect(),
81. }))
82. return false;
83. },
84. onProgressChange: (event: OnProgressChangeEvent) => {
85. console.info("AtomicServiceWebLog onProgressChange call back success " + JSON.stringify(event));
86. },
87. onMessage: (event: OnMessageEvent) => {
88. console.info("onMessage call back success " + JSON.stringify(event));
89. },
90. onPageBegin: (event: OnPageBeginEvent) => {
91. console.info("onPageBegin call back success " + JSON.stringify(event));
92. },
93. onPageEnd: (event: OnPageEndEvent) => {
94. console.info("onPageEnd call back success " + JSON.stringify(event));
95. },
96. onHttpErrorReceive: (event: OnHttpErrorReceiveEvent) => {
97. console.info("onHttpErrorReceive call back success " + JSON.stringify(event));
98. },
99. onErrorReceive: (event: OnErrorReceiveEvent) => {
100. console.info("onErrorReceive call back success " + JSON.stringify(event));
101. }
102. })
103. }
104. }
