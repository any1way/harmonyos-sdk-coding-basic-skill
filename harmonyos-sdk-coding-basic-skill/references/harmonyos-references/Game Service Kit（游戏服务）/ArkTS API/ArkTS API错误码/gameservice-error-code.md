...
scraped_at: 2026-06-01T16:34:06+08:00
doc_updated_at: 2026-05-26
content_hash: sha256:c5565e2853f5d65975b379303da51041854ffd1a67e1c7c41022d9c2f80ae35c
---
说明

以下仅介绍Game Service Kit特有错误码，通用错误码请参考[通用错误码](../../../通用错误码/errorcode-universal.md)。
...
2. 请检查[init](<../gamePerformance (游戏场景感知)/gameservice-gameperformance.md#gameperformanceinit>)接口传参是否正确。
...
* 调用[getDeviceInfoByScope](<../gamePerformance (游戏场景感知)/gameservice-gameperformance.md#gameperformancegetdeviceinfobyscope>)接口查询温度信息时，设备刚刚启动。

**处理步骤**

* 请先调用[init](<../gamePerformance (游戏场景感知)/gameservice-gameperformance.md#gameperformanceinit>)接口，并确保调用成功。
* 调用[getDeviceInfoByScope](<../gamePerformance (游戏场景感知)/gameservice-gameperformance.md#gameperformancegetdeviceinfobyscope>)接口查询温度信息时，请确保设备已启动至少一分钟。
...
* [unionLogin](../gamePlayer（基础游戏服务）/gameservice-gameplayer.md#gameplayerunionlogin)登录接口的入参accountIcon总大小超过35KB。
* 接口中的入参context不符合要求。
...
2. 排查各接口中的入参context是否符合要求：
   ...
   * UI组件外无法直接获取context。
     ...
     建议先在EntryAbility中获取context并保存至AppStorage，再使用AppStorage获取context，详情请参见[AppStorage：应用全局的UI状态存储](<../../../../harmonyos-guides/应用框架/ArkUI（方舟UI框架）/UI开发 (ArkTS声明式开发范式)/学习UI范式状态管理/状态管理（V1）/管理应用拥有的状态/AppStorage：应用全局的UI状态存储/arkts-appstorage.md>)。
...
1. 请检查配置项是否配置正确，详情请参见[开发准备](<../../../../harmonyos-guides/应用服务/Game Service Kit（游戏服务）/基础游戏服务（必选）/游戏登录/网络游戏登录/使用华为账号登录（必选）/gameservice-gameplayer-huawei.md#开发准备>)。
...
调用[unionLogin](../gamePlayer（基础游戏服务）/gameservice-gameplayer.md#gameplayerunionlogin)接口再次引导用户进行实名认证。
...
调用[bindPlayer](../gamePlayer（基础游戏服务）/gameservice-gameplayer.md#gameplayerbindplayer)或[unbindPlayer](../gamePlayer（基础游戏服务）/gameservice-gameplayer.md#gameplayerunbindplayer)接口时，teamPlayerId传错。

**处理步骤**

请检查teamPlayerId是否正确，并重新调用[bindPlayer](../gamePlayer（基础游戏服务）/gameservice-gameplayer.md#gameplayerbindplayer)或[unbindPlayer](../gamePlayer（基础游戏服务）/gameservice-gameplayer.md#gameplayerunbindplayer)接口传递正确teamPlayerId。
...
重新调用[init](../gamePlayer（基础游戏服务）/gameservice-gameplayer.md#gameplayerinit)接口。
...
调用[bindPlayer](../gamePlayer（基础游戏服务）/gameservice-gameplayer.md#gameplayerbindplayer)接口时，游戏官方账号与华为teamPlayerId已绑定。
...
调用[unbindPlayer](../gamePlayer（基础游戏服务）/gameservice-gameplayer.md#gameplayerunbindplayer)接口时，游戏官方账号与华为teamPlayerId未绑定。
...
1. 请确认当前游戏调用[unionLogin](../gamePlayer（基础游戏服务）/gameservice-gameplayer.md#gameplayerunionlogin)接口是否报错。
...
请重新调用[unionLogin](../gamePlayer（基础游戏服务）/gameservice-gameplayer.md#gameplayerunionlogin)接口，获取玩家信息。
...
如需继续登录，请重新调用[unionLogin](../gamePlayer（基础游戏服务）/gameservice-gameplayer.md#gameplayerunionlogin)接口。
...
检查[配置签名证书指纹](<../../../../harmonyos-guides/应用服务/Game Service Kit（游戏服务）/基础游戏服务（必选）/游戏登录/网络游戏登录/使用华为账号登录（必选）/gameservice-gameplayer-huawei.md#配置签名证书指纹>)、[配置APP ID和Client ID](<../../../../harmonyos-guides/应用服务/Game Service Kit（游戏服务）/基础游戏服务（必选）/游戏登录/网络游戏登录/使用华为账号登录（必选）/gameservice-gameplayer-huawei.md#配置app-id和client-id>)等信息是否已配置或正确，并根据情况补充配置或进行修改。
...
请登录[AppGallery Connect](https://developer.huawei.com/consumer/cn/service/josp/agc/index.html)网站，在“我的应用 > 运营 > 商品管理 > 商品列表” 查看对应商品是否存在或必填信息是否完整及已经提交审核并审核通过。如未审核通过，可使用沙盒账号来测试（参见[沙盒测试](<../../../../harmonyos-guides/应用服务/IAP Kit（应用内支付服务）/沙盒测试/iap-sandbox.md>)）。
...
可通过[queryPurchases](<../../../IAP Kit（应用内支付服务）/ArkTS API/IAP/iap-iap.md#iapquerypurchases>)接口确认用户是否购买了该商品。

* 若商品为消耗型商品或非续期订阅商品，检查商品是否发货，确认发货成功之后调用[finishPurchase](<../../../IAP Kit（应用内支付服务）/ArkTS API/IAP/iap-iap.md#iapfinishpurchase>)接口完成购买，下次可正常购买。
...
可通过[queryPurchases](<../../../IAP Kit（应用内支付服务）/ArkTS API/IAP/iap-iap.md#iapquerypurchases>)接口查询是否有该商品的确认发货记录。
...
1. 首次使用设备进行登录时，请确认网络连接正常。
2. 请检查[create](../gameNearbyTransfer(游戏近场快传)/gameservice-nearbytransfer.md#gamenearbytransfercreate)接口传参是否正确。
3. 未开通近场快传开放能力，请参见[申请近场快传开放能力](<../../../../harmonyos-guides/应用服务/Game Service Kit（游戏服务）/游戏近场快传（可选）/开发准备/gameservice-nearbytransfer-config-agc.md#申请近场快传开放能力>)申请并开启近场快传开放能力。
...
请先调用[create](../gameNearbyTransfer(游戏近场快传)/gameservice-nearbytransfer.md#gamenearbytransfercreate)接口初始化，并确保调用成功。
