...
content_hash: sha256:797d2f5e24c304c052dfcf4d3b5dfbc9d1058578cbbe4e7e50a1a0ba3427835e
---
...
| userProfile | image.[PixelMap](<../../../Image Kit（图片处理服务）/ArkTS API/@ohos.multimedia.image (图片处理)/Interface (PixelMap)/arkts-apis-image-pixelmap.md>) | 否 | 否 | 用户头像图片，需导入图像处理模块，详情见[PixelMap](<../../../Image Kit（图片处理服务）/ArkTS API/@ohos.multimedia.image (图片处理)/Interface (PixelMap)/arkts-apis-image-pixelmap.md>)。  支持传入的最大图片大小为221x221像素，推荐传入的图片大小为112x112像素。通过 [PixelMap](<../../../Image Kit（图片处理服务）/ArkTS API/@ohos.multimedia.image (图片处理)/Interface (PixelMap)/arkts-apis-image-pixelmap.md>)的[getPixelBytesNumber](<../../../Image Kit（图片处理服务）/ArkTS API/@ohos.multimedia.image (图片处理)/Interface (PixelMap)/arkts-apis-image-pixelmap.md#getpixelbytesnumber7>)接口获取到的图片大小要小于196608bit。 |
...
| AUDIO\_EVENT\_MIC\_DISABLE | 5 | 静音且不允许操作麦克风。  **起始版本**: 6.1.1(24)  **模型约束**: 该接口可在FA与Stage模型下使用。 |
