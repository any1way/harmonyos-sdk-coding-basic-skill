...
scraped_at: 2026-06-11T16:50:06+08:00
doc_updated_at: 2026-06-03
content_hash: sha256:04d872d6693443db7a17c197aab2ea9a4d74fa466c333c4803e9786079c5f412
---
说明
...
| 参数 | 取值描述 | 样例 |
| --- | --- | --- |
| Authorization | 鉴权方式：  **JWT方式**  详情参见[基于服务账号生成鉴权令牌](<../../../../harmonyos-guides/应用服务/Push Kit（推送服务）/端云调试/基于服务账号生成鉴权令牌/push-jwt-token.md>)。  **说明：**  · 调用服务通知API接口必须使用**PS256**算法。  · 建议JWT令牌过期时间设置为3600秒，有效期内可以复用。  Bearer后面拼接空格，再拼接获取的鉴权信息。 | Bearer eyJr\*\*\*\*\*OiIx---\*\*\*\*.eyJh\*\*\*\*\*iJodHR--\*\*\*.QRod\*\*\*\*\*4Gp---\*\*\*\* |
...
| 参数 | 是否必选 | 参数类型 | 描述 |
| --- | --- | --- | --- |
| actionType | 是 | Integer | 消息点击后的行为。  0：默认打开元服务首页，支持通过指定data参数跳转至该元服务内指定页面，详情请参见[调用方UIAbility指定启动页面](<../../../../harmonyos-guides/应用框架/Ability Kit（程序框架服务）/Stage模型开发指导/Stage模型应用组件/UIAbility组件/启动应用内的UIAbility组件/uiability-intra-device-interaction.md#调用方uiability指定启动页面>)。 |
...
请按照响应消息中的提示，使用从端侧上报的openID，或请求华为账号服务器[获取用户信息](<../../../Account Kit（华为账号服务）/REST API/获取用户信息/概述/account-api-get-user-info-overview.md>)中的openID，重新下发消息。
...
请按照响应消息中的提示，检查并修改[模板参数](push-api-service-noti.md#request-body)，详情请参见[选用订阅模板](https://developer.huawei.com/consumer/cn/doc/atomic-guides/push-as-service-noti#section880418143379)。
