...
scraped_at: 2026-06-11T16:49:54+08:00
doc_updated_at: 2026-06-09
content_hash: sha256:f9a5d4934644cb10538befdd60ac9ac8a9a3ec226ae9561f59a5b4a8f15157a1
---
说明
...
1. 请进行重试操作。
2. [pushService.on('tokenUpdate')](../pushService（推送服务基础能力）/push-pushservice.md#pushserviceontokenupdate)接口返回1000900001，优先排查是否重复注册。
...
执行ExtensionAbility的[onReceiveMessage](../RemoteNotificationExtensionAbility（通知扩展Ability）/push-remote-notification-extension-ability.md#onreceivemessage)()回调时出现执行异常。

**处理步骤**

请检查ExtensionAbility的[onReceiveMessage](../RemoteNotificationExtensionAbility（通知扩展Ability）/push-remote-notification-extension-ability.md#onreceivemessage)()回调中的代码执行逻辑。
...
执行ExtensionAbility的[onReceiveMessage](../RemoteNotificationExtensionAbility（通知扩展Ability）/push-remote-notification-extension-ability.md#onreceivemessage)()回调超时。
...
重复调用pushService.[receiveMessage](../pushService（推送服务基础能力）/push-pushservice.md#pushservicereceivemessage)()注册相同类型的场景化消息。

**处理步骤**

请删除多余的pushService.[receiveMessage](../pushService（推送服务基础能力）/push-pushservice.md#pushservicereceivemessage)()调用，仅在同一个回调中处理接收到的场景化消息。
...
* 应用配置错误：
  ...
  + 在[AppGallery Connect](https://developer.huawei.com/consumer/cn/service/josp/agc/index.html)的“项目设置 > 开放能力管理”中未启用“推送服务”。请参见[开通推送服务](<../../../../harmonyos-guides/应用服务/Push Kit（推送服务）/开发准备/开通推送服务/push-config-setting.md#操作步骤>)步骤5、步骤6完成对应阶段的签名。
  + 在[AppGallery Connect](https://developer.huawei.com/consumer/cn/service/josp/agc/index.html)的“项目设置 > 开放能力管理”中已启用“推送服务”，但未重新申请Profile文件。请参见[开通推送服务](<../../../../harmonyos-guides/应用服务/Push Kit（推送服务）/开发准备/开通推送服务/push-config-setting.md#操作步骤>)步骤5、步骤6完成对应阶段的签名。
* 网络不可用。
* 华为服务端异常。
* 不支持云真机调试。
...
请在[AppGallery Connect](https://developer.huawei.com/consumer/cn/service/josp/agc/index.html)网站，打开项目，在左侧导航栏选择“增长 > 推送服务”开通推送服务，详情参考指导[操作步骤](<../../../../harmonyos-guides/应用服务/Push Kit（推送服务）/开发准备/开通推送服务/push-config-setting.md#操作步骤>)第3点。
...
请检查设备所在地与AGC上设置的[数据处理位置](<../../../../harmonyos-guides/应用服务/Push Kit（推送服务）/开发准备/开通推送服务/push-config-setting.md#可选设置数据处理位置>)是否匹配。
...
请调用pushService.[unbindAppProfileId](../pushService（推送服务基础能力）/push-pushservice.md#pushserviceunbindappprofileid-1)()解绑若干账号后重试，推荐的绑定和解绑时机请参见[开发通知消息账号校验](<../../../../harmonyos-guides/应用服务/Push Kit（推送服务）/推送场景化消息/推送通知消息/发送通知消息/push-send-alert.md#开发通知消息账号校验>)。
...
请先[申请推送服务token](<../../../../harmonyos-guides/应用服务/Push Kit（推送服务）/开发准备/获取Push Token/push-get-token.md>)后再请求该接口。
...
1. 应用或元服务未在AGC上注册。
2. 订阅类型[type](../serviceNotification（服务通知）/push-servicenotification.md#servicenotificationrequestsubscribenotification-1)与应用类型不匹配。

**处理步骤**

1. 检查应用或元服务是否在AGC上完成注册，详情请参见[应用开发准备](../../../../harmonyos-guides/应用开发准备/应用开发准备/application-dev-overview.md)。
2. 检查订阅类型[type](../serviceNotification（服务通知）/push-servicenotification.md#servicenotificationrequestsubscribenotification-1)与应用类型是否匹配，仅元服务支持通过华为账号订阅。
...
排查是否调用[pushService.on('distributedMessageReceive')](../pushService（推送服务基础能力）/push-pushservice.md#pushserviceondistributedmessagereceive)接口重复注册。
