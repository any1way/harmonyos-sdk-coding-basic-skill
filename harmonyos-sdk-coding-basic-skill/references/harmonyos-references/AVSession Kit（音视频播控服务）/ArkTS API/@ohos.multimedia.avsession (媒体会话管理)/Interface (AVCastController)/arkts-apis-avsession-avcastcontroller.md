...
scraped_at: 2026-06-01T16:19:00+08:00
doc_updated_at: 2026-05-26
content_hash: sha256:37641157d0ad6a8a811a7f6dae300032f2b1d5c2e59aa2ec1718e5395020bb7b
---
在投播建立后，调用[avSession.getAVCastController](<../Interface (AVSession)/arkts-apis-avsession-avsession.md#getavcastcontroller10>)后，返回会话控制器实例。控制器可查看会话ID，并可完成对会话发送命令及事件，获取会话元数据，播放状态信息等操作。
...
```
1. avCastController.getAVPlaybackState((err: BusinessError, state: avSession.AVPlaybackState) => {
2. if (err) {
3. console.error(`Failed to get AV playback state: ${err.message}`);
4. return;
5. }
...
4. console.error(`Failed to get AV playback state: ${err.message}`);
5. });
```
...
| 类型 | 说明 |
| --- | --- |
| Promise<Array<[DecoderType](../Enums/arkts-apis-avsession-e.md#decodertype19)>> | Promise对象。返回远端设备所支持的解码能力列表。 |
...
7. console.error(`Failed to get supported decoders: ${err.message}`);
8. });
```
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| decoderType | [DecoderType](../Enums/arkts-apis-avsession-e.md#decodertype19) | 是 | 设备所支持的解码格式。  设备所支持的解码格式包括：  'OH\_AVCODEC\_MIMETYPE\_VIDEO\_AVC'：VIDEO AVC，  'OH\_AVCODEC\_MIMETYPE\_VIDEO\_HEVC'：VIDEO HEVC，  'OH\_AVCODEC\_MIMETYPE\_AUDIO\_VIVID'：AUDIO AV3A。 |

**返回值：**

| 类型 | 说明 |
| --- | --- |
| Promise<[ResolutionLevel](../Enums/arkts-apis-avsession-e.md#resolutionlevel19)> | Promise对象。返回远端设备推荐的分辨率。 |
...
5. console.error(`Failed to get recommended resolution level: ${err.message}`);
6. });
```
...
| 类型 | 说明 |
| --- | --- |
| Promise<Array<[hdrCapability.HDRFormat](<../../../../ArkGraphics 2D（方舟2D图形服务）/ArkTS API/@ohos.graphics.hdrCapability (HDR能力)/js-apis-hdrcapability.md#hdrformat>)>> | Promise对象。返回远端设备所支持的HDR能力。 |
...
```
1. import  { type hdrCapability } from '@kit.ArkGraphics2D';
...
9. console.error(`Failed to get supported HDR capabilities: ${err.message}`);
10. });
```
...
5. }
6. }).catch((err: BusinessError) => {
7. console.error(`Failed to get supported play speeds: ${err.message}`);
8. });
```
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| command | [AVCastControlCommand](<../Interfaces (其他)/arkts-apis-avsession-i.md#avcastcontrolcommand10>) | 是 | 会话的相关命令和命令相关参数。 |
...
4. }).catch((err: BusinessError) => {
5. console.error(`Failed to send control command: ${err.message}`);
6. });
```
...
```
1. let avCommand: avSession.AVCastControlCommand = {command:'play'};
2. avCastController.sendControlCommand(avCommand, (err: BusinessError) => {
3. if (err) {
4. console.error(`Failed to send control command: ${err.message}`);
5. return;
6. }
...
```
1. avCastController.sendCustomData({customData : "This is custom data"}).then(() => {
2. console.info('Succeeded in sending custom data.');
3. }).catch((err: BusinessError) => {
4. console.error(`Failed to send custom data: ${err.message}`);
5. });
```
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| item | [AVQueueItem](<../Interfaces (其他)/arkts-apis-avsession-i.md#avqueueitem10>) | 是 | 播放列表中单项的相关属性。 |
| callback | AsyncCallback<void> | 是 | 回调函数。当命令发送成功，err为undefined，否则返回错误对象。 |
...
19. avCastController.prepare(playItem, (err: BusinessError) => {
20. if (err) {
21. console.error(`Failed to prepare: ${err.message}`);
22. return;
23. }
24. console.info('Succeeded in preparing.');
25. });
```
...
22. console.error(`Failed to prepare: ${err.message}`);
23. });
```
...
19. // 启动播放。
20. avCastController.start(playItem, (err: BusinessError) => {
21. if (err) {
22. console.error(`Failed to start: ${err.message}`);
23. return;
24. }
25. console.info('Succeeded in starting.');
26. });
```
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| callback | AsyncCallback<[AVQueueItem](<../Interfaces (其他)/arkts-apis-avsession-i.md#avqueueitem10>)> | 是 | 回调函数。当命令发送成功，err为undefined，否则返回错误对象。 |
...
```
1. avCastController.getCurrentItem((err: BusinessError, value: avSession.AVQueueItem) => {
2. if (err) {
3. console.error(`Failed to get current item: ${err.message}`);
4. return;
5. }
6. console.info('Succeeded in getting current item.');
7. });
```
...
| 类型 | 说明 |
| --- | --- |
| Promise<[AVQueueItem](<../Interfaces (其他)/arkts-apis-avsession-i.md#avqueueitem10>)> | Promise对象，返回当前的播放资源，否则返回错误对象。 |
...
3. }).catch((err: BusinessError) => {
4. console.error(`Failed to get current item: ${err.message}`);
5. });
```
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| callback | AsyncCallback<Array<[AVCastControlCommandType](../Types/arkts-apis-avsession-t.md#avcastcontrolcommandtype10)>> | 是 | 回调函数。返回当前支持的命令。 |
...
```
1. avCastController.getValidCommands((err: BusinessError, state: avSession.AVCastControlCommandType[]) => {
2. if (err) {
3. console.error(`Failed to get valid commands: ${err.message}`);
4. return;
5. }
6. console.info('Succeeded in getting valid commands.');
7. });
```
...
| 类型 | 说明 |
| --- | --- |
| Promise<Array<[AVCastControlCommandType](../Types/arkts-apis-avsession-t.md#avcastcontrolcommandtype10)>> | Promise对象，返回当前支持的命令。 |
...
3. }).catch((err: BusinessError) => {
4. console.error(`Failed to get valid commands: ${err.message}`);
5. });
```
...
7. avCastController.processMediaKeyResponse(assetId, licenseResponseData).then(() => {
8. console.info('Succeeded in processing media key response.');
9. }).catch((err: BusinessError) => {
10. console.error(`Failed to process media key response: ${err.message}`);
11. });
12. }
```
...
```
1. avCastController.release((err: BusinessError) => {
2. if (err) {
3. console.error(`Failed to release: ${err.message}`);
4. return;
5. }
6. console.info('Succeeded in releasing.');
7. });
```
...
3. }).catch((err: BusinessError) => {
4. console.error(`Failed to release: ${err.message}`);
5. });
```
...
| callback | (state: [AVPlaybackState](<../Interfaces (其他)/arkts-apis-avsession-i.md#avplaybackstate10>)) => void | 是 | 回调函数，参数state是变化后的播放状态。 |
...
| callback | (state: [AVPlaybackState](<../Interfaces (其他)/arkts-apis-avsession-i.md#avplaybackstate10>)) => void | 否 | 回调函数，参数state是变化后的播放状态。  该参数为可选参数，若不填写该参数，则认为取消所有相关会话的事件监听。 |
...
| callback | Callback<[AVQueueItem](<../Interfaces (其他)/arkts-apis-avsession-i.md#avqueueitem10>)> | 是 | 回调函数，参数AVQueueItem是当前正在播放的媒体内容。 |
...
| callback | Callback<[AVQueueItem](<../Interfaces (其他)/arkts-apis-avsession-i.md#avqueueitem10>)> | 是 | 回调函数，参数AVQueueItem是当前正在播放的媒体内容。当监听事件注册成功，err为undefined，否则返回错误对象。 |
...
| callback | Callback<[AVQueueItem](<../Interfaces (其他)/arkts-apis-avsession-i.md#avqueueitem10>)> | 否 | 回调函数，参数AVQueueItem是当前正在播放的媒体内容。当监听事件取消成功，err为undefined，否则返回错误对象。该参数为可选参数，若不填写该参数，则认为取消所有相关会话的事件监听。 |
...
| callback | Callback<Array<[AVCastControlCommandType](../Types/arkts-apis-avsession-t.md#avcastcontrolcommandtype10)>> | 是 | 回调函数。参数commands是有效命令的集合。 |
...
| callback | Callback<Array<[AVCastControlCommandType](../Types/arkts-apis-avsession-t.md#avcastcontrolcommandtype10)>> | 否 | 回调函数。参数commands是有效命令的集合。  该参数为可选参数，若不填写该参数，则认为取消所有相关会话的事件监听。 |
...
以下错误码的详细介绍请参见[通用错误码说明文档](../../../../通用错误码/errorcode-universal.md)、[Media错误码](<../../../../Media Kit（媒体服务）/错误码/Media错误码/errorcode-media.md>)以及[媒体会话管理错误码](../../../错误码/媒体会话管理错误码/errorcode-avsession.md)。
...
| callback | [KeyRequestCallback](../Types/arkts-apis-avsession-t.md#keyrequestcallback12) | 是 | 回调函数，媒体资源及许可证请求数据。 |
...
| callback | [KeyRequestCallback](../Types/arkts-apis-avsession-t.md#keyrequestcallback12) | 否 | 回调函数。当监听事件取消成功，err为undefined，否则返回错误对象。该参数为可选参数，若不填写该参数，则认为取消所有相关会话的事件监听。 |
...
3. avCastController.on('castControlGenericError', (error: BusinessError) => {
4. console.info(`castControlGenericError happened, error code: ${error.code}, error message : ${error.message}.`)
5. })
```
...
3. avCastController.on('castControlIoError', (error: BusinessError) => {
4. console.info(`castControlIoError happened, error code: ${error.code}, error message : ${error.message}.`)
5. })
```
...
3. avCastController.on('castControlParsingError', (error: BusinessError) => {
4. console.info(`castControlParsingError happened, error code: ${error.code}, error message : ${error.message}.`)
5. })
```
...
3. avCastController.on('castControlDecodingError', (error: BusinessError) => {
4. console.info(`castControlDecodingError happened, error code: ${error.code}, error message : ${error.message}.`)
5. })
```
...
3. avCastController.on('castControlAudioRendererError', (error: BusinessError) => {
4. console.info(`castControlAudioRendererError happened, error code: ${error.code}, error message : ${error.message}.`)
5. })
```
...
3. avCastController.on('castControlDrmError', (error: BusinessError) => {
4. console.info(`castControlDrmError happened, error code: ${error.code}, error message : ${error.message}.`)
5. })
```
