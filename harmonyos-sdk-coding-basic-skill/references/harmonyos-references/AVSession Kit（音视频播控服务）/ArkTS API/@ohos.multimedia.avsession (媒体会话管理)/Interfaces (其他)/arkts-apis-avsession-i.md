...
scraped_at: 2026-06-01T16:19:07+08:00
doc_updated_at: 2026-05-07
content_hash: sha256:8e393c78dd799c2114cebbf4c8ffbeffbb860a43db779ce459c45ad91f8e5975
---
说明
...
| 名称 | 类型 | 只读 | 可选 | 说明 |
| --- | --- | --- | --- | --- |
| command | [AVCastControlCommandType](../Types/arkts-apis-avsession-t.md#avcastcontrolcommandtype10) | 否 | 否 | 命令。每种命令对应的参数不同，具体的对应关系可查阅[AVCastControlCommandType](../Types/arkts-apis-avsession-t.md#avcastcontrolcommandtype10)。 |
| parameter | [media.PlaybackSpeed](<../../../../Media Kit（媒体服务）/ArkTS API/@ohos.multimedia.media (媒体服务)/Enums/arkts-apis-media-e.md#playbackspeed8>) | number | string | [LoopMode](../Enums/arkts-apis-avsession-e.md#loopmode10) | 否 | 是 | 命令对应的参数。 |
...
| state | [CastDisplayState](../Enums/arkts-apis-avsession-e.md#castdisplaystate12) | 否 | 否 | 投播显示设备状态。 |
...
| 名称 | 类型 | 只读 | 可选 | 说明 |
| --- | --- | --- | --- | --- |
| assetId | string | 否 | 否 | 媒体ID。媒体信息的唯一标识，由应用自定义。  - 该属性发生变化则其他元数据属性都将被刷新。  - 若该属性维持不变，且不设置相应的媒体元数据信息，那么将不会更新对应的媒体元数据信息。  - 当该属性设为空值时，调用[setAVMetadata](<../Interface (AVSession)/arkts-apis-avsession-avsession.md#setavmetadata10>)方法将失败，返回错误码6600101。  **元服务API：** 从API version 12开始，该接口支持在元服务中使用。 |
...
| avQueueImage11+ | [image.PixelMap](<../../../../Image Kit（图片处理服务）/ArkTS API/@ohos.multimedia.image (图片处理)/Interface (PixelMap)/arkts-apis-image-pixelmap.md>) | string | 否 | 是 | 歌单（歌曲列表）封面图。  图片的像素数据或者图片路径地址（本地路径或网络路径）。应用通过setAVMetadata设置图片数据。  - 设置的数据类型为PixelMap时，通过getAVMetadata获取的将为PixelMap。  - 设置为url图片路径，获取的为url图片路径。 |
...
| mediaImage | [image.PixelMap](<../../../../Image Kit（图片处理服务）/ArkTS API/@ohos.multimedia.image (图片处理)/Interface (PixelMap)/arkts-apis-image-pixelmap.md>) | string | 否 | 是 | 图片的像素数据或者图片路径地址(本地路径或网络路径)。应用通过setAVMetadata设置图片数据。  - 设置的数据类型为PixelMap时，通过getAVMetadata获取的将为PixelMap。  - 设置为url图片路径，获取的为url图片路径。  **元服务API：** 从API version 12开始，该接口支持在元服务中使用。 |
| bundleIcon18+ | [image.PixelMap](<../../../../Image Kit（图片处理服务）/ArkTS API/@ohos.multimedia.image (图片处理)/Interface (PixelMap)/arkts-apis-image-pixelmap.md>) | 是 | 是 | 应用图标图片的像素数据。只读类型，不从应用侧设置。 |
...
| filter11+ | number | 否 | 是 | 当前session支持的协议，默认为TYPE\_CAST\_PLUS\_STREAM。具体取值参考[ProtocolType](../Enums/arkts-apis-avsession-e.md#protocoltype11)。  **元服务API：** 从API version 12开始，该接口支持在元服务中使用。 |
| drmSchemes12+ | Array<string> | 否 | 是 | 当前session支持的DRM方案，取值为DRM方案uuid。 |
| skipIntervals11+ | [SkipIntervals](../Enums/arkts-apis-avsession-e.md#skipintervals11) | 否 | 是 | 快进快退支持的时间间隔。默认为SECONDS\_15，即15秒。 |
| displayTags11+ | number | 否 | 是 | 媒体资源的金标类型，取值参考[DisplayTag](../Enums/arkts-apis-avsession-e.md#displaytag11)。 |
...
| fdSrc | [media.AVFileDescriptor](<../../../../Media Kit（媒体服务）/ArkTS API/@ohos.multimedia.media (媒体服务)/Interfaces (其他)/arkts-apis-media-i.md#avfiledescriptor9>) | 否 | 是 | 播放列表媒体本地文件的句柄。  **系统能力：** SystemCapability.Multimedia.AVSession.Core  **元服务API：** 从API version 12开始，该接口支持在元服务中使用。 |
| dataSrc12+ | [media.AVDataSrcDescriptor](<../../../../Media Kit（媒体服务）/ArkTS API/@ohos.multimedia.media (媒体服务)/Interfaces (其他)/arkts-apis-media-i.md#avdatasrcdescriptor10>) | 否 | 是 | 播放列表数据源描述。  **系统能力：** SystemCapability.Multimedia.AVSession.Core |
...
| displayTags11+ | number | 否 | 是 | 媒体资源的金标类型，取值参考[DisplayTag](../Enums/arkts-apis-avsession-e.md#displaytag11)。  在使用了cast+协议的音频投播场景下，不支持使用该属性。  **系统能力：** SystemCapability.Multimedia.AVSession.Core  **元服务API：** 从API version 12开始，该接口支持在元服务中使用。 |
...
| 名称 | 类型 | 只读 | 可选 | 说明 |
| --- | --- | --- | --- | --- |
| state | [PlaybackState](../Enums/arkts-apis-avsession-e.md#playbackstate10) | 否 | 是 | 播放状态。  **元服务API：** 从API version 12开始，该接口支持在元服务中使用。 |
...
| loopMode | [LoopMode](../Enums/arkts-apis-avsession-e.md#loopmode10) | 否 | 是 | 循环模式。  **元服务API：** 从API version 12开始，该接口支持在元服务中使用。 |
...
| avatar | [image.PixelMap](<../../../../Image Kit（图片处理服务）/ArkTS API/@ohos.multimedia.image (图片处理)/Interface (PixelMap)/arkts-apis-image-pixelmap.md>) | 否 | 是 | 来电人头像。 |
...
| 名称 | 类型 | 只读 | 可选 | 说明 |
| --- | --- | --- | --- | --- |
| state | [CallState](../Enums/arkts-apis-avsession-e.md#callstate11) | 否 | 否 | 当前通话状态。 |
...
| 名称 | 类型 | 只读 | 可选 | 说明 |
| --- | --- | --- | --- | --- |
| castCategory | [AVCastCategory](../Enums/arkts-apis-avsession-e.md#avcastcategory10) | 否 | 否 | 投播的类别。  **系统能力：** SystemCapability.Multimedia.AVSession.Core  **元服务API：** 从API version 12开始，该接口支持在元服务中使用。 |
...
| supportedProtocols11+ | number | 否 | 是 | 播放设备支持的协议。  默认为TYPE\_LOCAL,具体取值来自[ProtocolType](../Enums/arkts-apis-avsession-e.md#protocoltype11)，可以是protocolType中的某个协议或者多个协议的组合。  设备仅支持一种协议，返回对应枚举值；设备支持多种协议，返回对应枚举值之和。  **系统能力：** SystemCapability.Multimedia.AVSession.AVCast  **元服务API：** 从API version 12开始，该接口支持在元服务中使用。 |
...
| 名称 | 类型 | 只读 | 可选 | 说明 |
| --- | --- | --- | --- | --- |
| command | [AVControlCommandType](../Types/arkts-apis-avsession-t.md#avcontrolcommandtype10) | 否 | 否 | 命令（不同命令对应不同参数）。  **元服务API：** 从API version 12开始，该接口支持在元服务中使用。 |
| parameter | [LoopMode](../Enums/arkts-apis-avsession-e.md#loopmode10) | string | number | 否 | 是 | 命令对应的参数。  **元服务API：** 从API version 12开始，该接口支持在元服务中使用。 |
...
| 名称 | 类型 | 只读 | 可选 | 说明 |
| --- | --- | --- | --- | --- |
| sessionType | [AVSessionType](../Types/arkts-apis-avsession-t.md#avsessiontype10) | 否 | 是 | 会话类型，默认值为audio。  当前仅支持的会话类型有audio和video。如果传入voice\_call或video\_call，将默认按照传入audio处理。  **元服务API：** 从API version 14开始，该接口支持在元服务中使用。 |
| pickerStyle22+ | [AVCastPickerStyle](<../../@ohos.multimedia.avCastPickerParam (投播组件参数)/js-apis-avcastpickerparam.md#avcastpickerstyle12>) | 否 | 是 | 设置组件样式。 |
...
| 名称 | 类型 | 只读 | 可选 | 说明 |
| --- | --- | --- | --- | --- |
| streamInfos | Array<[audio.AudioStreamInfo](<../../../../Audio Kit（音频服务）/ArkTS API/@ohos.multimedia.audio (音频管理)/Interfaces (其他)/arkts-apis-audio-i.md#audiostreaminfo8>)> | 是 | 否 | 音频能力参数的列表。 |
...
| callerType | [CallerType](../Enums/arkts-apis-avsession-e.md#callertype22) | 否 | 是 | 调用方来源。 |
...
| type | [AVSessionType](../Types/arkts-apis-avsession-t.md#avsessiontype10) | 否 | 否 | 会话类型。 |
| sessionTag | string | 否 | 否 | 会话的自定义名称。 |
| elementName | [ElementName](<../../../../Ability Kit（程序框架服务）/ArkTS API/已停止维护的接口/bundle/ElementName/js-apis-bundle-elementname.md>) | 否 | 否 | 会话所属应用的信息（包含bundleName、abilityName等）。 |
