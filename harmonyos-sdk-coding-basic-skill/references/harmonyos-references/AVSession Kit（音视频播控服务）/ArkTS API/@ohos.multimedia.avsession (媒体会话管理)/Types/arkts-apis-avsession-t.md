...
scraped_at: 2026-06-01T16:19:10+08:00
doc_updated_at: 2026-05-07
content_hash: sha256:30a0c51aa345e7f5620ec4fd73b2bb6c1e0b0cce019f0b4c9b50e5eff4d7f703
---
说明
...
| 'setVolume' | 设置音量。对应参数使用number类型, 可通过[AVPlaybackState.maxVolume](<../Interfaces (其他)/arkts-apis-avsession-i.md#avplaybackstate10>)获取系统最大音量 |
| 'setSpeed' | 设置播放倍速。在音视频投播场景下，当远端设备使用DLNA协议进行连接时，不支持设置此项数据。对应参数使用[media.PlaybackSpeed](<../../../../Media Kit（媒体服务）/ArkTS API/@ohos.multimedia.media (媒体服务)/Enums/arkts-apis-media-e.md#playbackspeed8>)。 |
| 'setLoopMode' | 设置循环模式。对应参数使用[LoopMode](../Enums/arkts-apis-avsession-e.md#loopmode10)。 |
...
| 'fastForward' | 快进。对应参数请参考[SkipIntervals](../Enums/arkts-apis-avsession-e.md#skipintervals11)。  **元服务API：** 从API version 12开始，该接口支持在元服务中使用。 |
| 'rewind' | 快退。对应参数请参考[SkipIntervals](../Enums/arkts-apis-avsession-e.md#skipintervals11)。  **元服务API：** 从API version 12开始，该接口支持在元服务中使用。 |
...
| 'setLoopMode' | 设置循环模式。对应参数使用[LoopMode](../Enums/arkts-apis-avsession-e.md#loopmode10)。  **元服务API：** 从API version 12开始，该接口支持在元服务中使用。 |
| 'setTargetLoopMode' 18+ | 设置目标循环模式。对应参数推荐使用[LoopMode](../Enums/arkts-apis-avsession-e.md#loopmode10)。  **元服务API：** 从API version 18开始，该接口支持在元服务中使用。 |
| 'toggleFavorite' | 是否收藏。对应参数使用[AVMetadata.assetId](<../Interfaces (其他)/arkts-apis-avsession-i.md#avmetadata10>)。  **元服务API：** 从API version 12开始，该接口支持在元服务中使用。 |
| 'playFromAssetId' 11+ | 播放指定的assetId。  **元服务API：** 从API version 12开始，该接口支持在元服务中使用。 |
| 'playWithAssetId' 20+ | 播放指定的assetId。对应参数使用[AVMetadata.assetId](<../Interfaces (其他)/arkts-apis-avsession-i.md#avmetadata10>)，  字符串长度<40960字节。  **元服务API：** 从API version 20开始，该接口支持在元服务中使用。 |
