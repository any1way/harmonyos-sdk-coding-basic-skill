...
scraped_at: 2026-06-11T16:09:49+08:00
doc_updated_at: 2026-05-18
content_hash: sha256:5c0b36bbb39f57e8c0457bbe66dd74725134645b5092991d59cc30431552db38
---
...
**设备行为差异：** 对于API 24之前版本，该接口在Phone中可正常调用，在其他设备类型中统一返回业务错误码1019100016。对于API 24及之后版本，该接口在Phone、部分支持TUI能力的Tablet、PC/2in1中均可正常调用，在不支持TUI能力的Tablet、PC/2in1设备及其他设备类型中统一返回业务错误码1019100016。可使用[checkConfirmUITextFormat](devicesecurity-trusted-auth-api.md#checkconfirmuitextformat)查询能力是否支持。
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| challenge | Uint8Array | 是 | 在发起请求之前通过[Universal Keystore Kit初始化会话](<../../../Universal Keystore Kit（密钥管理服务）/ArkTS API/@ohos.security.huks (通用密钥库系统)/js-apis-huks.md#huksinitsession9>)获取的challenge值，参数规格为32字节随机数。 |
...
以下错误码的详细介绍请参见[ArkTS API错误码](<../ArkTS API错误码/TrustedAuthentication （数字盾服务）/devicesecurity-arktsapi-errcode-trusted-auth.md>) **。**
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| challenge | Uint8Array | 是 | 当needAuth为true时，在发起请求之前通过[Universal Keystore Kit初始化会话](<../../../Universal Keystore Kit（密钥管理服务）/ArkTS API/@ohos.security.huks (通用密钥库系统)/js-apis-huks.md#huksinitsession9>)获取的challenge值，参数规格为32字节随机数。  当needAuth为false时，该值可不从Universal Keystore Kit获取，challenge值为任意32字节数即可。 |
...
| **名称** | **类型** | 只读 | 可选 | **说明** |
| --- | --- | --- | --- | --- |
| result | number | 否 | 否 | 指定输入文本检查结果，如果可以正常显示，返回为0，否则返回[1019100011 不合法的TUI认证信息](<../ArkTS API错误码/TrustedAuthentication （数字盾服务）/devicesecurity-arktsapi-errcode-trusted-auth.md#section1019100011-不合法的tui认证信息>)。 |
...
| TRUSTED\_AUTH\_ERROR\_TUI\_OCCUPIED | 1019100025 | TUI界面被其他应用占用。  **起始版本：** 6.1.1(24)  **模型约束：** 此接口仅可在Stage模型下使用。 |
