...
scraped_at: 2026-06-01T16:31:12+08:00
doc_updated_at: 2026-05-26
content_hash: sha256:bb98d786b577218d308ec61b32ff9a36ef77838742f445cca28ff6a374f872f3
---
## 概述
...
**相关模块：** [SpatialRecon](../../模块/SpatialRecon/capi-spatialrecon.md)
...
| 名称 | typedef关键字 | 描述 |
| --- | --- | --- |
| [HMS\_SpatialRecon\_ModelWriteInfo](../../结构体/HMS_SpatialRecon_ModelWriteInfo/capi-spatialrecon-hms-spatialrecon-modelwriteinfo.md) | - | 空间重建模型写入的结构体。 |
| [HMS\_SpatialRecon\_DataFrame](../../结构体/HMS_SpatialRecon_DataFrame/capi-spatialrecon-hms-spatialrecon-dataframe.md) | HMS\_SpatialRecon\_DataFrame | 定义HMS（Huawei Mobile Services）空间重建数据帧的结构体，包含用于空间重建的相机内参、姿态信息、时间戳和图像数据。 |
| [HMS\_SpatialRecon\_Session](../../结构体/HMS_SpatialRecon_Session/capi-spatialrecon-hms-spatialrecon-session.md) | HMS\_SpatialRecon\_Session | 定义用于空间重建会话句柄的结构体，用于3D场景重建。 |
| [AREngine\_ARSession](../../结构体/AREngine_ARSession/capi-spatialrecon-arengine-arsession.md) | AREngine\_ARSession | 表示华为AR Engine中AR会话的不透明句柄。 |
| [AREngine\_ARFrame](../../结构体/AREngine_ARFrame/capi-spatialrecon-arengine-arframe.md) | AREngine\_ARFrame | 定义一个结构体，用于存储AR Engine中捕获的单帧AR图像数据，包含特定时间戳下的相机图像、追踪状态、锚点及AR相关信息。 |
...
| [HMS\_SpatialRecon\_Session](../../结构体/HMS_SpatialRecon_Session/capi-spatialrecon-hms-spatialrecon-session.md) \*\*outSpatialReconSession | 输出参数，接收指向新创建的空间重建会话对象的指针。调用方负责管理此对象的生命周期，并且最终必须使用适当的清理函数销毁它。 |
...
| 参数项 | 描述 |
| --- | --- |
| [HMS\_SpatialRecon\_Session](../../结构体/HMS_SpatialRecon_Session/capi-spatialrecon-hms-spatialrecon-session.md) \*spatialReconSession | 指向要销毁的空间重建会话的指针。该会话必须是先前创建的，如果为NULL，则函数无效果。 |
...
| 参数项 | 描述 |
| --- | --- |
| [HMS\_SpatialRecon\_Session](../../结构体/HMS_SpatialRecon_Session/capi-spatialrecon-hms-spatialrecon-session.md) \*spatialReconSession | 指向空间重建会话句柄的指针。 |
| [HMS\_SpatialRecon\_DataFrame](../../结构体/HMS_SpatialRecon_DataFrame/capi-spatialrecon-hms-spatialrecon-dataframe.md) \*inputFrame | 指向包含传感器数据的空间重建数据帧的指针。 |
...
| --- | --- |
| [HMS\_SpatialRecon\_Session](../../结构体/HMS_SpatialRecon_Session/capi-spatialrecon-hms-spatialrecon-session.md) \*spatialReconSession | 指向空间重建会话句柄的指针。 |
| [AREngine\_ARSession](../../结构体/AREngine_ARSession/capi-spatialrecon-arengine-arsession.md)\* arSession | 指向生成AREngine帧的AREngine会话的指针。 |
| [AREngine\_ARFrame](../../结构体/AREngine_ARFrame/capi-spatialrecon-arengine-arframe.md) \*arFrame | 指向包含AR跟踪数据和摄像头图像的AREngine帧的指针。 |
...
| 参数项 | 描述 |
| --- | --- |
| [HMS\_SpatialRecon\_Session](../../结构体/HMS_SpatialRecon_Session/capi-spatialrecon-hms-spatialrecon-session.md) \*spatialReconSession | 指向空间重建会话句柄的指针。如果spatialReconSession为空，则函数将返回错误码。 |
| [HMS\_SpatialRecon\_ModelWriteInfo](../../结构体/HMS_SpatialRecon_ModelWriteInfo/capi-spatialrecon-hms-spatialrecon-modelwriteinfo.md)\* writeInfo | 指向模型写入配置信息的指针。包含保存重建输出的参数。如果writeInfo指针为空，则重建完成后不会执行写入操作，之后可以单独调用HMS\_SpatialRecon\_SaveResultToFile接口。 |
...
| 参数项 | 描述 |
| --- | --- |
| [HMS\_SpatialRecon\_Session](../../结构体/HMS_SpatialRecon_Session/capi-spatialrecon-hms-spatialrecon-session.md) \*spatialReconSession | 指向空间重建会话实例的指针。 |
...
| [HMS\_SpatialRecon\_DataFrame](../../结构体/HMS_SpatialRecon_DataFrame/capi-spatialrecon-hms-spatialrecon-dataframe.md) \*outFrame | 输出参数，存储优化后的帧数据。 |
...
| 参数项 | 描述 |
| --- | --- |
| [HMS\_SpatialRecon\_Session](../../结构体/HMS_SpatialRecon_Session/capi-spatialrecon-hms-spatialrecon-session.md) \*spatialReconSession | 指向空间重建会话对象的指针。必须不为 NULL，并且应为有效且活跃的会话。 |
| [HMS\_SpatialRecon\_ModelWriteInfo](../../结构体/HMS_SpatialRecon_ModelWriteInfo/capi-spatialrecon-hms-spatialrecon-modelwriteinfo.md) \*writeInfo | 指向包含文件导出参数的结构体指针，如文件路径、格式和可选配置标志。必须不为NULL。 |
