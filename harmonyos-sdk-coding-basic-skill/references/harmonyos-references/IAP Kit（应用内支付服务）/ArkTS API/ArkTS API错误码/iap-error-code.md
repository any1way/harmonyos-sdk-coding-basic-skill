...
content_hash: sha256:63e7154ca81b6ca6f1a3f77befb8b2a7463d2f559a87d9dce4c6158a8ae34477
---
说明
...
* 若购买请求返回该错误码，建议通过[queryPurchases](../IAP/iap-iap.md#iapquerypurchases)接口确认用户是否存在已购但未发放权益的商品，及时发放权益。
...
1. 请参考[配置应用身份信息](<../../../../harmonyos-guides/应用服务/IAP Kit（应用内支付服务）/开发准备/配置应用身份信息/iap-config-app-identity-info.md>)、[添加公钥指纹](../../../../harmonyos-guides/应用开发准备/应用开发准备/application-dev-overview.md#条件必选添加公钥指纹)章节进行检查。
2. 请参考[配置签名信息](../../../../harmonyos-guides/应用开发准备/应用开发准备/application-dev-overview.md#配置签名信息)检查是否使用了自动签名方式，请使用手动签名方式。
3. 需要使用华为应用内支付功能的应用必须[开启和激活应用内购买服务](<../../../../harmonyos-guides/应用服务/IAP Kit（应用内支付服务）/开发准备/基本准备工作/开启和激活应用内购买服务/iap-enable-in-app-purchases.md>)。
...
请登录[AppGallery Connect](https://developer.huawei.com/consumer/cn/service/josp/agc/index.html)网站，选择“我的应用 > 运营 > 商品管理 > 商品列表”，查看对应商品是否存在、必填信息是否完整、商品信息已经提交审核并审核通过。如未审核通过，可使用沙盒账号来测试（参见[沙盒测试](<../../../../harmonyos-guides/应用服务/IAP Kit（应用内支付服务）/沙盒测试/iap-sandbox.md>)）。
...
可通过[queryPurchases](../IAP/iap-iap.md#iapquerypurchases)接口确认用户是否购买了该商品。

* 若商品为消耗型商品或非续期订阅商品，检查商品是否发货，确认发货成功之后调用[finishPurchase](../IAP/iap-iap.md#iapfinishpurchase)接口完成购买，下次可正常购买。
...
可通过[queryPurchases](../IAP/iap-iap.md#iapquerypurchases)接口查询是否有该商品的确认发货记录。
...
1. 手动签名：您需要在AGC中[申请调试证书](https://developer.huawei.com/consumer/cn/doc/app/agc-help-debug-cert-0000002283256797)、[注册调试设备](https://developer.huawei.com/consumer/cn/doc/app/agc-help-add-device-0000002283189937)、[申请调试Profile](https://developer.huawei.com/consumer/cn/doc/app/agc-help-debug-profile-0000002248181278)后，再[手动配置签名信息](../../../../harmonyos-guides/编写与调试应用/配置调试签名/ide-signing.md#section297715173233)。
...
需要在[AppGallery Connect](https://developer.huawei.com/consumer/cn/service/josp/agc/index.html)中的“用户与访问”中添加测试账号，具体请参见[沙盒测试中添加测试账号流程](<../../../../harmonyos-guides/应用服务/IAP Kit（应用内支付服务）/沙盒测试/iap-sandbox.md#配置沙盒测试>)。
...
[配置商品信息](<../../../../harmonyos-guides/应用服务/IAP Kit（应用内支付服务）/开发准备/配置商品信息/iap-config-product.md>)时没有为该商品配置自定义人群促销。

**处理步骤**

可通过[queryProducts](../IAP/iap-iap.md#iapqueryproducts-1)接口确认该商品是否配置了优惠活动，具体请参见[设置促销价格](https://developer.huawei.com/consumer/cn/doc/app/set-0000001931995712)。
...
可参考[生成优惠签名购买参数](<../../REST API/生成优惠签名购买参数/iap-server-subscribe-offer-sign.md>)进行签名过程处理。
...
1. 本应用未在用户账户所在[国家或地区](<../../../../harmonyos-guides/应用服务/IAP Kit（应用内支付服务）/附录/华为IAP范围覆盖/iap-appendix-coverage.md>)提供服务。
