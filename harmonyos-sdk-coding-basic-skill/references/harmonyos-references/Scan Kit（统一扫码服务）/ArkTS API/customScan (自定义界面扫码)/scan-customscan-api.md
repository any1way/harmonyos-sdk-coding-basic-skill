...
scraped_at: 2026-06-01T16:26:25+08:00
doc_updated_at: 2026-04-28
content_hash: sha256:07e7f6a6baf5f8b3f95f847c389b8b2edc13c28b83b5648b463a4d42dc041f93
---
本模块提供自定义界面扫码能力。
...
| **名称** | **类型** | 只读 | **可选** | **说明** |
| --- | --- | --- | --- | --- |
| width | number | 否 | 否 | [XComponent](../../../ArkUI（方舟UI框架）/ArkTS组件/渲染绘制/XComponent/ts-basic-components-xcomponent.md)组件的宽，默认使用单位为vp，支持px、lpx和vp。 |
| height | number | 否 | 否 | XComponent组件的高，默认使用单位为vp，支持px、lpx和vp。 |
| surfaceId | string | 否 | 否 | XComponent持有surface的ID。 |

说明

1. ViewControl的width和height需和XComponent的保持一致，start接口根据设置宽高值会匹配最接近的相机分辨率，如果宽高比例与相机的分辨率比例相差过大会影响预览流体验。XComponent组件为预览流提供的Surface，而XComponent的能力由UI提供，相关介绍可参见[XComponent](../../../ArkUI（方舟UI框架）/ArkTS组件/渲染绘制/XComponent/ts-basic-components-xcomponent.md)。
...
| scanCodeRects | Array<scanBarcode.[ScanCodeRect](<../scanBarcode (默认界面扫码)/scan-scanbarcode-api.md#scancoderect>)> | 否 | 是 | 相机预览流的码图检测位置信息。  **设备行为差异：** 该属性在带有Kirin NPU（Neural-network Processing Unit，神经网络处理器）的设备可正常返回，在不带有Kirin NPU的设备上返回undefined。 |
...
**设备行为差异：** 对于6.0.2(22)及之前版本，该接口在Phone、Tablet中可正常调用。对于6.1.0(23)及之后版本，该接口在Phone、Tablet、带后置相机的Wearable中可正常调用，申请相机权限成功后，在不带后置相机的Wearable中返回错误码1000500001。可以通过[cameraManager.getSupportedCameras](<../../../Camera Kit（相机服务）/ArkTS API/@ohos.multimedia.camera (相机管理)/Interface (CameraManager)/arkts-apis-camera-cameramanager.md#getsupportedcameras>)接口查询是否带后置相机。
...
| **参数名** | **类型** | **必填** | **说明** |
| --- | --- | --- | --- |
| options | scanBarcode.[ScanOptions](<../scanBarcode (默认界面扫码)/scan-scanbarcode-api.md#scanoptions>) | 否 | 自定义界面扫码参数。 |

**错误码：**

以下错误码的详细介绍请参见[ArkTS API错误码](<../ArkTS API错误码/scan-error-code.md>)。
...
| **类型** | **说明** |
| --- | --- |
| Promise<Array<scanBarcode.[ScanResult](<../scanBarcode (默认界面扫码)/scan-scanbarcode-api.md#scanresult>)>> | Promise对象，返回启动相机流扫码结果对象。 |
...
| --- | --- | --- | --- |
| viewControl | [ViewControl](scan-customscan-api.md#viewcontrol) | 是 | 相机控制参数。 |
| callback | AsyncCallback<Array<scanBarcode.[ScanResult](<../scanBarcode (默认界面扫码)/scan-scanbarcode-api.md#scanresult>)>> | 是 | 回调函数，当启动相机流扫码成功，err为undefined，data为获取到的Array<scanBarcode.[ScanResult](<../scanBarcode (默认界面扫码)/scan-scanbarcode-api.md#scanresult>)>；否则为错误对象。 |
...
**设备行为差异：** 对于6.0.2(22)及之前版本，该接口在Phone、Tablet中可正常调用。对于6.1.0(23)及之后版本，该接口在Phone、Tablet、带后置相机的Wearable中可正常调用，在不带后置相机的Wearable中返回错误码1000500001。可以通过[cameraManager.getSupportedCameras](<../../../Camera Kit（相机服务）/ArkTS API/@ohos.multimedia.camera (相机管理)/Interface (CameraManager)/arkts-apis-camera-cameramanager.md#getsupportedcameras>)接口查询是否带后置相机。
...
| **参数名** | **类型** | **必填** | **说明** |
| --- | --- | --- | --- |
| point | scanBarcode.[Point](<../scanBarcode (默认界面扫码)/scan-scanbarcode-api.md#point>) | 是 | 焦点。x、y设置范围应在[0，1]之内，超过范围，如果小于0设置0，大于1设置1。 |
