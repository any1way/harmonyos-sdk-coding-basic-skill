...
scraped_at: 2026-06-11T16:24:47+08:00
doc_updated_at: 2026-05-26
content_hash: sha256:97e4f93342bad1eb786f4353e4ba45da73c9cfef02cf4d20bef666d2baf2859d
---
...
由于/proc/{pid}/smaps\_rollup的读取耗时较长，建议不要在主线程中使用该接口，可通过[@ohos.taskpool](<../../../ArkTS（方舟编程语言）/ArkTS API/@ohos.taskpool (启动任务池)/js-apis-taskpool.md>)或[@ohos.worker](<../../../ArkTS（方舟编程语言）/ArkTS API/@ohos.worker (启动一个Worker)/js-apis-worker.md>)开启异步线程以避免应用出现卡顿。
...
以下错误码的详细介绍请参见[通用错误码](../../../通用错误码/errorcode-universal.md)与[HiDebug错误码](../../错误码/HiDebug错误码/errorcode-hiviewdfx-hidebug.md)。
...
4. try {
5. hidebug.dumpJsHeapData("heapData", true);
6. } catch (error) {
7. console.error(`error code: ${(error as BusinessError).code}, error msg: ${(error as BusinessError).message}`);
8. }
```
...
该接口补充了[hitrace](../../../../harmonyos-guides/系统/调测调优/调试命令/hitrace/hitrace.md)功能，开发者可通过该接口完成指定范围的trace自动化采集。由于该接口中trace采集过程中消耗的性能与需要采集的范围成正相关，建议开发者在使用该接口前，通过hitrace命令抓取应用的trace日志，从中筛选出所需trace采集的关键范围，以提高该接口性能。
...
| 类型 | 说明 |
| --- | --- |
| string | 返回trace文件名路径（接口返回真实物理路径，若应用内需要访问，请参考[应用沙箱路径和真实物理路径的对应关系](<../../../../harmonyos-guides/应用框架/Core File Kit（文件基础服务）/应用文件/应用沙箱目录/app-sandbox-directory.md#应用沙箱路径和真实物理路径的对应关系>)进行路径转换）。 |

**错误码**：

以下错误码的详细介绍请参见[通用错误码](../../../通用错误码/errorcode-universal.md)与[HiDebug Trace错误码](<../../错误码/HiDebug Trace错误码/errorcode-hiviewdfx-hidebug-trace.md>)。
...
以下错误码的详细介绍请参见[HiDebug Trace错误码](<../../错误码/HiDebug Trace错误码/errorcode-hiviewdfx-hidebug-trace.md>)。
...
接口不支持在[输入法应用](<../../../../harmonyos-guides/应用框架/IME Kit（输入法开发服务）/IME Kit简介/ime-kit-intro.md>)中使用。
...
4. try {
5. hidebug.requestTrace({
6. identifier: "trace_name",
7. bufferSizeKb: 1024,
8. durationMs: 1000,
9. reserved: 0,
10. }).then((tracePath: string) => {
11. hilog.info(0x0000, 'hidebug', `tracePath: ${tracePath}`)
12. }).catch((err: BusinessError) => {
13. hilog.error(0x0000, 'hidebug', `error code: ${err.code}, message: ${err.message}`)
14. })
15. } catch (error) {
16. hilog.error(0x0000, 'hidebug', `error code: ${(error as BusinessError).code}, message: ${(error as BusinessError).message}`)
17. }
```
...
以下错误码的详细介绍请参见[HiDebug CpuUsage错误码](<../../错误码/HiDebug CpuUsage错误码/errorcode-hiviewdfx-hidebug-cpuusage.md>)。
...
主要应用场景在于构造内存泄漏故障，参见[订阅资源泄漏事件（ArkTS）](<../../../../harmonyos-guides/系统/调测调优/Performance Analysis Kit（性能分析服务）/事件订阅/使用HiAppEvent订阅事件/系统事件/资源泄漏事件/订阅资源泄漏事件（ArkTS）/hiappevent-watcher-resourceleak-events-arkts.md>)、[订阅资源泄漏事件（C/C++）](<../../../../harmonyos-guides/系统/调测调优/Performance Analysis Kit（性能分析服务）/事件订阅/使用HiAppEvent订阅事件/系统事件/资源泄漏事件/订阅资源泄漏事件（CC++）/hiappevent-watcher-resourceleak-events-ndk.md>)。
...
由于读取 /proc/{pid}/smaps\_rollup 比较耗时，建议不在主线程中使用该接口。可以通过[@ohos.taskpool](<../../../ArkTS（方舟编程语言）/ArkTS API/@ohos.taskpool (启动任务池)/js-apis-taskpool.md>)或[@ohos.worker](<../../../ArkTS（方舟编程语言）/ArkTS API/@ohos.worker (启动一个Worker)/js-apis-worker.md>)开启异步线程，以避免应用卡顿。
...
获取系统[GC](../../../../harmonyos-guides/应用框架/ArkTS（方舟编程语言）/ArkTS运行时/GC垃圾回收/gc-introduction.md)统计信息。
...
根据参数获取指定的系统[GC](../../../../harmonyos-guides/应用框架/ArkTS（方舟编程语言）/ArkTS运行时/GC垃圾回收/gc-introduction.md)统计信息。
...
支持trace使用场景的标签，用户可通过[hitrace](../../../../harmonyos-guides/系统/调测调优/调试命令/hitrace/hitrace.md)抓取指定标签的trace内容。
...
以下错误码的详细介绍请参见[HiDebug错误码](../../错误码/HiDebug错误码/errorcode-hiviewdfx-hidebug.md)。
...
为当前线程转储虚拟机的原始堆快照，并生成的rawheap格式文件，使用Promise异步回调完成。该文件可通过[rawheap-translator工具](../../../../harmonyos-guides/系统/调测调优/调试命令/命令行工具/rawheap-translator工具/rawheap-translator.md)转化为heapsnapshot格式文件进行解析。
...
| 类型 | 说明 |
| --- | --- |
| Promise<string> | Promise对象，返回生成的快照文件路径（[应用沙箱内路径](<../../../../harmonyos-guides/应用框架/Core File Kit（文件基础服务）/应用文件/应用沙箱目录/app-sandbox-directory.md#应用沙箱路径和真实物理路径的对应关系>)）。 |
...
为当前线程转储虚拟机的原始堆快照，并支持清除nodeId缓存。生成的文件为rawheap格式，使用Promise异步回调完成。该文件可通过[rawheap-translator工具](../../../../harmonyos-guides/系统/调测调优/调试命令/命令行工具/rawheap-translator工具/rawheap-translator.md)转化为heapsnapshot格式文件进行解析。
...
4. hidebug.dumpJsRawHeapData(true, true).then((filePath: string) => {
5. console.info(`dumpJsRawHeapData success and generated file path is ${filePath}`);
6. }).catch((error: BusinessError) => {
7. console.error(`error code: ${error.code}, error msg: ${error.message}`);
8. })
```
...
更多关于GWP-ASan的说明，请参见[使用GWP-ASan检测内存错误](../../../../best-practices/稳定性/稳定性检测/开发态稳定性检测/地址越界类问题检测/使用GWP-ASan检测内存错误/bpta-stability-gwpasan-detection.md)。

说明

1. 若设备运行期间通过本接口设置的GWP-ASan应用数量超过配额限制，调用该接口将会失败并抛出错误码。请使用try-catch捕获异常，以避免应用异常退出。
2. 设备重启后，本接口设置的GWP-ASan参数将会失效。
3. 由于该接口涉及跨进程通信，耗时较长，为了避免引入性能问题，建议不要在主线程中直接调用该接口。可以通过[@ohos.taskpool](<../../../ArkTS（方舟编程语言）/ArkTS API/@ohos.taskpool (启动任务池)/js-apis-taskpool.md>)或[@ohos.worker](<../../../ArkTS（方舟编程语言）/ArkTS API/@ohos.worker (启动一个Worker)/js-apis-worker.md>)开启异步线程，以避免应用卡顿。
...
5. @Concurrent
6. function enableGwpAsanTask(): void {
7. let options: hidebug.GwpAsanOptions = {
8. alwaysEnabled: true,
9. sampleRate: 2500,
10. maxSimutaneousAllocations: 5000,
11. };
12. let duration: number = 4;
13. hidebug.enableGwpAsanGrayscale(options, duration);
14. }

16. taskpool.execute(enableGwpAsanTask).then(() => {
17. console.info(`Succeeded in enabling GWP-ASan.`);
18. }).catch((error: BusinessError) => {
19. const err: BusinessError = error as BusinessError;
20. console.error(`Failed to enable GWP-ASan. Code: ${err.code}, message: ${err.message}`);
21. })
```
...
由于该接口涉及跨进程通信，耗时较长，为了避免引入性能问题，建议不要在主线程中直接调用该接口。可以通过[@ohos.taskpool](<../../../ArkTS（方舟编程语言）/ArkTS API/@ohos.taskpool (启动任务池)/js-apis-taskpool.md>)或[@ohos.worker](<../../../ArkTS（方舟编程语言）/ArkTS API/@ohos.worker (启动一个Worker)/js-apis-worker.md>)开启异步线程，以避免应用卡顿。
...
4. @Concurrent
5. function disableGwpAsanTask(): void {
6. hidebug.disableGwpAsanGrayscale();
7. }
8. taskpool.execute(disableGwpAsanTask).then(() => {
9. console.info(`Disable GWP-ASan succeeded.`);
10. })
```
...
4. @Concurrent
5. function getGwpAsanStateTask(): number {
6. return hidebug.getGwpAsanGrayscaleState();
7. }
8. taskpool.execute(getGwpAsanStateTask).then((remainDays: Object) => {
9. console.info(`GWP-ASan remain days: ${remainDays as number}.`);
10. })
```
...
默认裁剪级别是TRIM\_LEVEL\_1。如果设置了TRIM\_LEVEL\_2裁剪，需使用API version 20之后的[rawheap-translator](../../../../harmonyos-guides/系统/调测调优/调试命令/命令行工具/rawheap-translator工具/rawheap-translator.md)工具才能将.rawheap文件转换为.heapsnapshot文件，否则可能导致转换失败。
...
3. hidebug.setProcDumpInSharedOOM(true);
```
...
3. let rssInfo: hidebug.RssInfo = hidebug.getRssInfo();
4. console.info(`rss: ${rssInfo.rss}, swapRss: ${rssInfo.swapRss}`);
```
