...
scraped_at: 2026-06-01T15:35:09+08:00
doc_updated_at: 2026-05-18
content_hash: sha256:620fca1f8986c10f23536ebc7f14b9e2d1ceb89e5d60b4a5cfdb3ba8baf09c51
---
说明

以下仅介绍本模块特有错误码，通用错误码请参考[通用错误码说明文档](../../../通用错误码/errorcode-universal.md)。

错误码排查请参考：[关系型数据库错误码常见场景及排查步骤](../../../../harmonyos-guides/应用框架/ArkData（方舟数据管理）/ArkData常见问题/data-faq.md#关系型数据库错误码常见场景及排查步骤)。
...
1. **问题定位**：优先排查使用场景，再查看日志。可在日志中搜索"fdsan"进行排查，详见[fdsan使用指导](../../../../harmonyos-guides/NDK开发/代码开发/CC++标准库/fdsan使用指导/fdsan.md)。
2. **数据库恢复**：如果可以接受数据库数据丢失，则可尝试删除数据库后重新创建。否则，需要备份数据库以便恢复。具体操作可参考[数据库备份与恢复](<../../../../harmonyos-guides/应用框架/ArkData（方舟数据管理）/数据可靠性与安全性/数据库备份与恢复 (ArkTS)/data-backup-and-restore.md>)。
...
1. 重新尝试。
2. 如果是[attach](<../../ArkTS API/@ohos.data.relationalStore (关系型数据库)/Interface (RdbStore)/arkts-apis-data-relationalstore-rdbstore.md#attach12>)或[detach](<../../ArkTS API/@ohos.data.relationalStore (关系型数据库)/Interface (RdbStore)/arkts-apis-data-relationalstore-rdbstore.md#detach12>)接口，增加waitTime值来增加等待时长。
...
对同一数据库表设置的分布式表类型前后不一致，分布式表类型可见[DistributedType](<../../ArkTS API/@ohos.data.relationalStore (关系型数据库)/Enums/arkts-apis-data-relationalstore-e.md#distributedtype10>)。
