...
scraped_at: 2026-06-11T15:38:14+08:00
doc_updated_at: 2026-06-05
content_hash: sha256:cc66df69c167ecf34d9824b64a2458c262070a32fde63c697b4d1c35396a19f1
---
...
在使用[taskpool](../../../../harmonyos-guides/应用框架/ArkTS（方舟编程语言）/ArkTS并发/多线程并发/TaskPool简介/taskpool-introduction.md)进行多线程计算时，因为对跨线程传递的数据类型限制，关系型数据库常规的ValuesBucket、Asset、Assets数据存储容器不能直接用于跨线程传递。
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| values | collections.Array<[ValueType](<../@ohos.data.relationalStore (关系型数据库)/Types/arkts-apis-data-relationalstore-t.md#valuetype>)> | 是 | 可跨线程传递的数组数据。 |
...
| 类型 | 说明 |
| --- | --- |
| collections.Array<[ValueType](<../@ohos.data.relationalStore (关系型数据库)/Types/arkts-apis-data-relationalstore-t.md#valuetype>)> | 可跨线程传递的数组数据。 |
...
记录资产附件（文件、图片、视频等类型文件）的相关信息。用于支持资产数据跨线程传递，继承自[lang.ISendable](<../../../ArkTS（方舟编程语言）/ArkTS API/@arkts.lang (ArkTS语言基础能力)/js-apis-arkts-lang.md#langisendable>)。资产类型的相关接口暂不支持Datashare。使用[sendableRelationalStore.toSendableAsset](js-apis-data-sendablerelationalstore.md#sendablerelationalstoretosendableasset)方法创建。
...
| status | number | 否 | 是 | 资产的状态，取值与[relationalStore.AssetStatus](<../@ohos.data.relationalStore (关系型数据库)/Enums/arkts-apis-data-relationalstore-e.md#assetstatus10>)枚举值保持一致，默认值为relationalStore.AssetStatus.ASSET\_NORMAL。 |
...
| 类型 | 说明 |
| --- | --- |
| [collections.Array](<../../../ArkTS（方舟编程语言）/ArkTS API/@arkts.collections (ArkTS容器集)/Class (Array)/arkts-apis-arkts-collections-array.md>)<[Asset](js-apis-data-sendablerelationalstore.md#asset)> | 用于并发场景的Asset附件数据集合。 |
...
| string | 表示值类型为字符串。 |
| boolean | 表示值类型为布尔值。 |
| [collections.Uint8Array](<../../../ArkTS（方舟编程语言）/ArkTS API/@arkts.collections (ArkTS容器集)/Class (Uint8Array)/arkts-apis-arkts-collections-uint8array.md>) | 表示值类型为Uint8类型的数组。 |
...
| [collections.Float32Array](<../../../ArkTS（方舟编程语言）/ArkTS API/@arkts.collections (ArkTS容器集)/Class (Float32Array)/arkts-apis-arkts-collections-float32array.md>) | 表示值类型为浮点数组。  当字段类型是collections.Float32Array时，在创建表的sql语句中，类型应当为：floatvector(128)。 |
| bigint | 表示值类型为任意长度的整数。  当字段类型是bigint时，在创建表的sql语句中，类型应当为：UNLIMITED INT，详见[通过关系型数据库实现数据持久化](<../../../../harmonyos-guides/应用框架/ArkData（方舟数据管理）/应用数据持久化/通过关系型数据库实现数据持久化 (ArkTS)/data-persistence-by-rdb-store.md>)。  **说明：**  bigint类型字段不能比较大小，不适用以下谓词操作：between、notBetween、greaterThan、lessThan、greaterThanOrEqualTo、lessThanOrEqualTo、orderByAsc、orderByDesc。  bigint类型字段的数据写入时，需通过BigInt()方法或在数据尾部添加'n'的方式明确为bigint类型，如'let data = BigInt(1234)'或'let data = 1234n'。  bigint字段如果写入number类型的数据，则查询该数据的返回类型为number，而非bigint。 |
...
| 类型 | 说明 |
| --- | --- |
| [collections.Map](<../../../ArkTS（方舟编程语言）/ArkTS API/@arkts.collections (ArkTS容器集)/Class (Map)/arkts-apis-arkts-collections-map.md>)<string, [ValueType](js-apis-data-sendablerelationalstore.md#valuetype)> | 并发场景的键值对数据存储，其中，键的类型为string，值的类型为ValueType。 |
...
| 类型 | 说明 |
| --- | --- |
| [relationalStore.ValuesBucket](<../@ohos.data.relationalStore (关系型数据库)/Types/arkts-apis-data-relationalstore-t.md#valuesbucket>) | 非并发场景的键值对数据存储。 |
...
表示[ValueType](<../@ohos.data.relationalStore (关系型数据库)/Types/arkts-apis-data-relationalstore-t.md#valuetype>)数据数组存储。不支持跨线程传递。

**系统能力：** SystemCapability.DistributedDataManager.RelationalStore.Core

| 类型 | 说明 |
| --- | --- |
| Array<[relationalStore.ValueType](<../@ohos.data.relationalStore (关系型数据库)/Types/arkts-apis-data-relationalstore-t.md#valuetype>)> | 非并发场景的数组数据存储，值的类型为ValueType。 |
...
| 类型 | 说明 |
| --- | --- |
| [relationalStore.Asset](<../@ohos.data.relationalStore (关系型数据库)/Interfaces (其他)/arkts-apis-data-relationalstore-i.md#asset10>) | 非并发场景的资产附件数据存储。 |
...
24. // 数据插入
25. const rowId = store.insertSync("test", dataItem);
