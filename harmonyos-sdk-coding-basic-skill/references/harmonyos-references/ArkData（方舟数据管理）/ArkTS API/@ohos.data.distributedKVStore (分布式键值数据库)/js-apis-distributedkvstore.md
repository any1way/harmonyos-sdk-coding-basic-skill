...
scraped_at: 2026-06-11T15:37:52+08:00
doc_updated_at: 2026-06-05
content_hash: sha256:999643665154b90257d9c5f358a14ae6ec391166f564d22abde717175b390398
---
...
| 名称 | 类型 | 只读 | 可选 | 说明 |
| --- | --- | --- | --- | --- |
| context | [BaseContext](<../../../Ability Kit（程序框架服务）/ArkTS API/接口依赖的元素及定义/application/BaseContext/js-apis-inner-application-basecontext.md>) | 否 | 否 | 应用的上下文。  FA模型的应用Context定义见[Context](<../../../Ability Kit（程序框架服务）/ArkTS API/FA模型能力的接口/app/Context (FA模型的上下文基类)/js-apis-inner-app-context.md>)。  Stage模型的应用Context定义见[Context](<../../../Ability Kit（程序框架服务）/ArkTS API/接口依赖的元素及定义/application/UIAbilityContext/js-apis-inner-application-uiabilitycontext.md>)。  从API version 10开始，context的参数类型为[BaseContext](<../../../Ability Kit（程序框架服务）/ArkTS API/接口依赖的元素及定义/application/BaseContext/js-apis-inner-application-basecontext.md>)。 |
...
| value | Uint8Array | string | number | boolean | 否 | 否 | 键值对中的值。Uint8Array、string类型的长度不大于[MAX\_VALUE\_LENGTH](js-apis-distributedkvstore.md#constants)。 |
...
| 名称 | 类型 | 只读 | 可选 | 说明 |
| --- | --- | --- | --- | --- |
| key | string | 否 | 否 | 表示键名。不能为空且长度不大于[MAX\_KEY\_LENGTH](js-apis-distributedkvstore.md#constants)。 |
...
在获取已有的分布式键值数据库时，如果数据库文件无法打开（例如文件头损坏），将触发自动重建逻辑，并返回新创建的分布式键值数据库实例。建议对重要且无法重新生成的数据使用备份恢复功能，以防止数据丢失。有关备份恢复的使用方法，请参阅[数据库备份与恢复](<../../../../harmonyos-guides/应用框架/ArkData（方舟数据管理）/数据可靠性与安全性/数据库备份与恢复 (ArkTS)/data-backup-and-restore.md>)。
...
以下错误码的详细介绍请参见[分布式键值数据库错误码](../../错误码/分布式键值数据库错误码/errorcode-distributedkvstore.md)和[通用错误码](../../../通用错误码/errorcode-universal.md)。
...
获取已有的分布式键值数据库时，如果数据库文件无法打开（如文件头损坏），将触发自动重建逻辑，并返回新创建的分布式键值数据库实例。建议对重要且无法重新生成的数据使用备份恢复功能，防止数据丢失。备份恢复的使用方法详见[数据库备份与恢复](<../../../../harmonyos-guides/应用框架/ArkData（方舟数据管理）/数据可靠性与安全性/数据库备份与恢复 (ArkTS)/data-backup-and-restore.md>)。
...
使用Schema创建数据库请参见[通过键值型数据库实现数据持久化](<../../../../harmonyos-guides/应用框架/ArkData（方舟数据管理）/应用数据持久化/通过键值型数据库实现数据持久化 (ArkTS)/data-persistence-by-kv-store.md#开发步骤>)中使用getKVStore()方法创建并获取键值数据库示例。
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| prefix | string | 是 | 表示指定的键前缀，长度不大于[MAX\_KEY\_LENGTH](js-apis-distributedkvstore.md#constants)，不能包含'^'。包含'^'将导致谓词失效，查询结果会返回数据库中的所有数据。 |
...
其中deviceId为[DeviceBasicInfo](<../../../Distributed Service Kit（分布式管理服务）/ArkTS API/@ohos.distributedDeviceManager (设备管理)/js-apis-distributeddevicemanager.md#devicebasicinfo>)中的networkId，通过调用[deviceManager.getAvailableDeviceListSync](<../../../Distributed Service Kit（分布式管理服务）/ArkTS API/@ohos.distributedDeviceManager (设备管理)/js-apis-distributeddevicemanager.md#getavailabledevicelistsync>)方法得到。
...
| value | Uint8Array | string | number | boolean | 是 | 要添加数据的value，支持Uint8Array、string、number、boolean，Uint8Array、string的长度不大于[MAX\_VALUE\_LENGTH](js-apis-distributedkvstore.md#constants)。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| keys | string[] | 是 | 表示要批量删除的键名列表，不能为空，数组中每个元素的长度不大于[MAX\_KEY\_LENGTH](js-apis-distributedkvstore.md#constants)。 |
...
以下错误码的详细介绍请参见[分布式键值数据库错误码](../../错误码/分布式键值数据库错误码/errorcode-distributedkvstore.md)。
...
在手动同步方式下，触发数据库端端同步。同步结果可通过订阅[on('syncComplete')](js-apis-distributedkvstore.md#onsynccomplete)事件获取。关于键值型数据库的端端同步方式说明，请见[键值型数据库跨设备数据同步](<../../../../harmonyos-guides/应用框架/ArkData（方舟数据管理）/同应用跨设备数据同步（分布式）/键值型数据库跨设备数据同步 (ArkTS)/data-sync-of-kv-store.md>)。

说明

其中deviceIds为[DeviceBasicInfo](<../../../Distributed Service Kit（分布式管理服务）/ArkTS API/@ohos.distributedDeviceManager (设备管理)/js-apis-distributeddevicemanager.md#devicebasicinfo>)中的networkId, 通过调用[deviceManager.getAvailableDeviceListSync](<../../../Distributed Service Kit（分布式管理服务）/ArkTS API/@ohos.distributedDeviceManager (设备管理)/js-apis-distributeddevicemanager.md#getavailabledevicelistsync>)方法得到。
...
在手动同步方式下，触发数据库端端同步，支持按查询条件过滤同步数据。同步结果可通过订阅[on('syncComplete')](js-apis-distributedkvstore.md#onsynccomplete)事件获取。关于键值型数据库的端端同步方式说明，请见[键值型数据库跨设备数据同步](<../../../../harmonyos-guides/应用框架/ArkData（方舟数据管理）/同应用跨设备数据同步（分布式）/键值型数据库跨设备数据同步 (ArkTS)/data-sync-of-kv-store.md>)。
...
其中deviceId通过调用[deviceManager.getAvailableDeviceListSync](<../../../Distributed Service Kit（分布式管理服务）/ArkTS API/@ohos.distributedDeviceManager (设备管理)/js-apis-distributeddevicemanager.md#getavailabledevicelistsync>)方法得到。
