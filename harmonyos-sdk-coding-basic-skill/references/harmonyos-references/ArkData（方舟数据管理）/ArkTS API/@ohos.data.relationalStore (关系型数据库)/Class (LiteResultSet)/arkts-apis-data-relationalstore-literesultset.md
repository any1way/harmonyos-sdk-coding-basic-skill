...
scraped_at: 2026-06-01T15:33:25+08:00
doc_updated_at: 2026-04-20
content_hash: sha256:a1ee56c367641acbc05f96626733ecbf9cd2191a393ccdc9ff4d8c0e425559cf
---
提供通过查询数据库生成的数据库结果集的访问方法。结果集是指用户调用关系型数据库查询接口之后返回的结果集合，提供了多种灵活的数据访问方式，以便用户获取各项数据。

LiteResultSet实例不会实时刷新。使用结果集后，如果数据库中的数据发生变化（如增删改操作），需要重新查询才能获取到最新的数据。

下列API示例中，都需先使用[queryWithoutRowCount](<../Interface (RdbStore)/arkts-apis-data-relationalstore-rdbstore.md#querywithoutrowcount23>)、[querySqlWithoutRowCount](<../Interface (RdbStore)/arkts-apis-data-relationalstore-rdbstore.md#querysqlwithoutrowcount23>)等query类方法中任一方法获取到LiteResultSet实例，再通过此实例调用对应方法。
...
| 类型 | 说明 |
| --- | --- |
| [ValueType](../Types/arkts-apis-data-relationalstore-t.md#valuetype) | 允许返回的数据字段类型。 |
...
以[Asset](<../Interfaces (其他)/arkts-apis-data-relationalstore-i.md#asset10>)形式获取当前行中指定列的值。
...
以[Assets](../Types/arkts-apis-data-relationalstore-t.md#assets10)形式获取当前行中指定列的值。
...
| 类型 | 说明 |
| --- | --- |
| [ValuesBucket](../Types/arkts-apis-data-relationalstore-t.md#valuesbucket) | 返回指定行的值。当结果集中包含重名列时，返回值会不符合预期，建议使用[getCurrentRowData](arkts-apis-data-relationalstore-literesultset.md#getcurrentrowdata23)接口获取。 |
...
| 类型 | 说明 |
| --- | --- |
| Promise<Array<[ValuesBucket](../Types/arkts-apis-data-relationalstore-t.md#valuesbucket)>> | 返回maxCount条数据，剩余数据不足maxCount条则返回剩余数据，返回空数组时代表已经遍历到结果集的末尾。当结果集中包含重名列时，返回值会不符合预期，建议使用[getRowsData](arkts-apis-data-relationalstore-literesultset.md#getrowsdata23)接口获取。 |
