...
scraped_at: 2026-06-01T15:33:13+08:00
doc_updated_at: 2026-04-24
content_hash: sha256:8b2f75ae1dcac785549e2056795f02da796e940139041c71959c18eee9de7563
---
...
共享用户首选项的持久化文件存储在[preferencesDir](<../../../../harmonyos-guides/应用框架/Ability Kit（程序框架服务）/Stage模型开发指导/Stage模型应用组件/应用上下文Context/application-context-stage.md#获取应用文件路径>)路径下，创建preferences对象前，需要保证preferencesDir路径可读写。持久化文件存储路径中的[加密等级](<../../../Ability Kit（程序框架服务）/ArkTS API/Stage模型能力的接口/@ohos.app.ability.contextConstant (Context相关常量)/js-apis-app-ability-contextconstant.md#areamode>)会影响文件的可读写状态，路径访问限制详见[应用文件目录与应用文件路径](<../../../../harmonyos-guides/应用框架/Core File Kit（文件基础服务）/应用文件/应用沙箱目录/app-sandbox-directory.md#应用文件目录与应用文件路径>)。

共享用户首选项可以在ArkTS并发实例间（包括主线程、TaskPool&Worker工作线程）传递，传递的行为是引用传递，性能优于普通的[用户首选项](<../@ohos.data.preferences (用户首选项)/js-apis-data-preferences.md>)，可参考[Sendable使用场景](../../../../harmonyos-guides/应用框架/ArkTS（方舟编程语言）/ArkTS并发/并发线程间通信/线程间通信对象/Sendable对象/Sendable使用场景/sendable-guide.md)。
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| context | [Context](<../../../Ability Kit（程序框架服务）/ArkTS API/接口依赖的元素及定义/application/Context (Stage模型的上下文基类)/js-apis-inner-application-context.md>) | 是 | 应用上下文。 |
...
| 类型 | 说明 |
| --- | --- |
| Promise<[Preferences](js-apis-data-sendablepreferences.md#preferences)> | Promise对象，返回Preferences实例。  该实例继承[ISendable](../../../../harmonyos-guides/应用框架/ArkTS（方舟编程语言）/ArkTS并发/并发线程间通信/线程间通信对象/Sendable对象/Sendable对象简介/arkts-sendable.md#isendable)，可以在ArkTS并发实例间（包括主线程、TaskPool&Worker工作线程）传递，传递的行为是引用传递，参考[Sendable使用场景](../../../../harmonyos-guides/应用框架/ArkTS（方舟编程语言）/ArkTS并发/并发线程间通信/线程间通信对象/Sendable对象/Sendable使用场景/sendable-guide.md)。 |

**错误码：**

以下错误码的详细介绍请参见[通用错误码](../../../通用错误码/errorcode-universal.md)和[用户首选项错误码](../../错误码/用户首选项错误码/errorcode-preferences.md)。
...
| 类型 | 说明 |
| --- | --- |
| [Preferences](js-apis-data-sendablepreferences.md#preferences) | 返回Preferences实例。  该实例继承[ISendable](../../../../harmonyos-guides/应用框架/ArkTS（方舟编程语言）/ArkTS并发/并发线程间通信/线程间通信对象/Sendable对象/Sendable对象简介/arkts-sendable.md#isendable)，可以在ArkTS并发实例间（包括主线程、TaskPool&Worker工作线程）传递，传递的行为是引用传递，参考[Sendable使用场景](../../../../harmonyos-guides/应用框架/ArkTS（方舟编程语言）/ArkTS并发/并发线程间通信/线程间通信对象/Sendable对象/Sendable使用场景/sendable-guide.md)。 |
...
| dataGroupId | string|null | 否 | 是 | 应用组ID，需要向应用市场获取，详见[dataGroupId申请流程](<../../../../harmonyos-guides/应用框架/IME Kit（输入法开发服务）/输入法安全模式介绍/ime-kit-security.md#共享沙箱介绍>)。基于dataGroupId的数据共享支持两种场景：1.同一应用的不同进程间共享，只支持三方应用中输入法和输入法的扩展场景使用；2.不同应用间的数据共享，只支持系统应用使用。  为可选参数。指定在此dataGroupId对应的沙箱路径下创建Preferences实例。当此参数不填时，默认在本应用沙箱目录下创建Preferences实例。  **模型约束：** 此属性仅在Stage模型下可用。 |

## Preferences

PhonePC/2in1TabletTVWearable

Preferences继承自[ISendable](../../../../harmonyos-guides/应用框架/ArkTS（方舟编程语言）/ArkTS并发/并发线程间通信/线程间通信对象/Sendable对象/Sendable对象简介/arkts-sendable.md#isendable)，可以在ArkTS并发实例间（包括主线程、TaskPool&Worker工作线程）传递，传递的行为是引用传递，提供获取和修改存储数据的接口。
...
| defValue | [lang.ISendable](../../../../harmonyos-guides/应用框架/ArkTS（方舟编程语言）/ArkTS并发/并发线程间通信/线程间通信对象/Sendable对象/Sendable对象简介/arkts-sendable.md#isendable) | 是 | 默认返回值。 |

**返回值：**

| 类型 | 说明 |
| --- | --- |
| Promise<[lang.ISendable](../../../../harmonyos-guides/应用框架/ArkTS（方舟编程语言）/ArkTS并发/并发线程间通信/线程间通信对象/Sendable对象/Sendable对象简介/arkts-sendable.md#isendable)> | Promise对象，返回键对应的值。  该实例继承[ISendable](../../../../harmonyos-guides/应用框架/ArkTS（方舟编程语言）/ArkTS并发/并发线程间通信/线程间通信对象/Sendable对象/Sendable对象简介/arkts-sendable.md#isendable)，可以在ArkTS并发实例间（包括主线程、TaskPool&Worker工作线程）传递，传递的行为是引用传递，参考[Sendable使用场景](../../../../harmonyos-guides/应用框架/ArkTS（方舟编程语言）/ArkTS并发/并发线程间通信/线程间通信对象/Sendable对象/Sendable使用场景/sendable-guide.md)。 |
...
| 类型 | 说明 |
| --- | --- |
| [lang.ISendable](../../../../harmonyos-guides/应用框架/ArkTS（方舟编程语言）/ArkTS并发/并发线程间通信/线程间通信对象/Sendable对象/Sendable对象简介/arkts-sendable.md#isendable) | 返回键对应的值。  该实例继承[ISendable](../../../../harmonyos-guides/应用框架/ArkTS（方舟编程语言）/ArkTS并发/并发线程间通信/线程间通信对象/Sendable对象/Sendable对象简介/arkts-sendable.md#isendable)，可以在ArkTS并发实例间（包括主线程、TaskPool&Worker工作线程）传递，传递的行为是引用传递，参考[Sendable使用场景](../../../../harmonyos-guides/应用框架/ArkTS（方舟编程语言）/ArkTS并发/并发线程间通信/线程间通信对象/Sendable对象/Sendable使用场景/sendable-guide.md)。 |
...
| 类型 | 说明 |
| --- | --- |
| Promise<[lang.ISendable](../../../../harmonyos-guides/应用框架/ArkTS（方舟编程语言）/ArkTS并发/并发线程间通信/线程间通信对象/Sendable对象/Sendable对象简介/arkts-sendable.md#isendable)> | Promise对象，返回所有包含的键值数据。  该对象继承[ISendable](../../../../harmonyos-guides/应用框架/ArkTS（方舟编程语言）/ArkTS并发/并发线程间通信/线程间通信对象/Sendable对象/Sendable对象简介/arkts-sendable.md#isendable)，可以在ArkTS并发实例间（包括主线程、TaskPool&Worker工作线程）传递，传递的行为是引用传递，参考[Sendable使用场景](../../../../harmonyos-guides/应用框架/ArkTS（方舟编程语言）/ArkTS并发/并发线程间通信/线程间通信对象/Sendable对象/Sendable使用场景/sendable-guide.md)。 |

**错误码：**

以下错误码的详细介绍请参见[用户首选项错误码](../../错误码/用户首选项错误码/errorcode-preferences.md)。
...
| 类型 | 说明 |
| --- | --- |
| [lang.ISendable](../../../../harmonyos-guides/应用框架/ArkTS（方舟编程语言）/ArkTS并发/并发线程间通信/线程间通信对象/Sendable对象/Sendable对象简介/arkts-sendable.md#isendable) | 返回所有包含的键值数据。  该对象继承[ISendable](../../../../harmonyos-guides/应用框架/ArkTS（方舟编程语言）/ArkTS并发/并发线程间通信/线程间通信对象/Sendable对象/Sendable对象简介/arkts-sendable.md#isendable)，可以在ArkTS并发实例间（包括主线程、TaskPool&Worker工作线程）传递，传递的行为是引用传递，参考[Sendable使用场景](../../../../harmonyos-guides/应用框架/ArkTS（方舟编程语言）/ArkTS并发/并发线程间通信/线程间通信对象/Sendable对象/Sendable使用场景/sendable-guide.md)。 |
...
| value | [lang.ISendable](../../../../harmonyos-guides/应用框架/ArkTS（方舟编程语言）/ArkTS并发/并发线程间通信/线程间通信对象/Sendable对象/Sendable对象简介/arkts-sendable.md#isendable) | 是 | 存储的新值。 |
...
| callback | Callback<[lang.ISendable](../../../../harmonyos-guides/应用框架/ArkTS（方舟编程语言）/ArkTS并发/并发线程间通信/线程间通信对象/Sendable对象/Sendable对象简介/arkts-sendable.md#isendable)> | 是 | 回调函数。回调支持返回多个键值对，其中键为发生变更的订阅key，值为变更后的数据：支持number、string、boolean、bigint以及可序列化的object。 |
...
| callback | Callback<[lang.ISendable](../../../../harmonyos-guides/应用框架/ArkTS（方舟编程语言）/ArkTS并发/并发线程间通信/线程间通信对象/Sendable对象/Sendable对象简介/arkts-sendable.md#isendable)> | 否 | 需要取消的回调函数，若callback不填写，表示所有的callback都需要处理；若callback填写，表示只处理该callback。 |
