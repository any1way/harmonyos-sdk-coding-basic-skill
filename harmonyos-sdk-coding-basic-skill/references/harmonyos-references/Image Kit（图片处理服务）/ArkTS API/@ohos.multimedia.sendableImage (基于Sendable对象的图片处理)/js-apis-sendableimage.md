...
scraped_at: 2026-06-11T16:31:41+08:00
doc_updated_at: 2026-06-05
content_hash: sha256:ce8f12bc242f8cfe5bfe4e57a34d07a07cc80602eaaf85613f8268d1987daf6a
---
本模块基于[Sendable](../../../../harmonyos-guides/应用框架/ArkTS（方舟编程语言）/ArkTS并发/并发线程间通信/线程间通信对象/Sendable对象/Sendable对象简介/arkts-sendable.md)对象，提供图片处理效果，包括通过属性创建PixelMap、读取图像像素数据、读取区域内的图片数据等。
...
| options | [image.InitializationOptions](<../@ohos.multimedia.image (图片处理)/Interfaces (其他)/arkts-apis-image-i.md#initializationoptions8>) | 是 | 创建像素的属性，包括透明度，尺寸，缩放值，像素格式和是否可编辑。 |
...
5. async function CreatePixelMap() {
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| sequence | [rpc.MessageSequence](<../../../IPC Kit（进程间通信服务）/ArkTS API/@ohos.rpc (RPC通信)/js-apis-rpc.md#messagesequence9>) | 是 | 保存有PixelMap信息的MessageSequence。 |
...
以下错误码的详细介绍请参见[Image错误码](../../错误码/Image错误码/errorcode-image.md)。
...
7. class MySequence implements rpc.Parcelable {
8. pixel_map: sendableImage.PixelMap;
9. constructor(conPixelmap: sendableImage.PixelMap) {
10. this.pixel_map = conPixelmap;
11. }
12. marshalling(messageSequence: rpc.MessageSequence) {
13. this.pixel_map.marshalling(messageSequence);
14. return true;
15. }
16. unmarshalling(messageSequence: rpc.MessageSequence) {
17. try {
18. this.pixel_map = sendableImage.createPixelMapFromParcel(messageSequence);
19. } catch(e) {
20. let error = e as BusinessError;
21. console.error(`createPixelMapFromParcel error. code is ${error.code}, message is ${error.message}`);
22. return false;
23. }
24. return true;
25. }
26. }
27. async function CreatePixelMapFromParcel() {
28. const color: ArrayBuffer = new ArrayBuffer(96);
29. let bufferArr: Uint8Array = new Uint8Array(color);
30. for (let i = 0; i < bufferArr.length; i++) {
31. bufferArr[i] = 0x80;
32. }
33. let opts: image.InitializationOptions = {
34. editable: true,
35. pixelFormat: 4,
36. size: { height: 4, width: 6 },
37. alphaType: 3
38. }
39. let pixelMap: sendableImage.PixelMap | undefined = undefined;
40. await sendableImage.createPixelMap(color, opts).then((srcPixelMap: sendableImage.PixelMap) => {
41. pixelMap = srcPixelMap;
42. })
43. if (pixelMap != undefined) {
44. // 序列化。
45. let parcelable: MySequence = new MySequence(pixelMap);
46. let data: rpc.MessageSequence = rpc.MessageSequence.create();
47. data.writeParcelable(parcelable);

49. // 反序列化rpc获取到data。
50. let ret: MySequence = new MySequence(pixelMap);
51. data.readParcelable(ret);

53. // 获取到PixelMap。
54. let newPixelMap = ret.pixel_map;
55. }
56. }
```
...
根据Surface ID和区域信息创建一个PixelMap对象。该区域的大小由[Region](<../@ohos.multimedia.image (图片处理)/Interfaces (其他)/arkts-apis-image-i.md#region8>).size指定。使用Promise异步回调。
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| surfaceId | string | 是 | 对应Surface的ID，可通过预览组件获取，如[XComponent](../../../ArkUI（方舟UI框架）/ArkTS组件/渲染绘制/XComponent/ts-basic-components-xcomponent.md)组件。 |
| region | [image.Region](<../@ohos.multimedia.image (图片处理)/Interfaces (其他)/arkts-apis-image-i.md#region8>) | 是 | 截取的画面区域。仅支持从画面左上角开始截取部分或整个画面，即Region中的x和y必须为0，Region.size中width和height的取值范围分别为[1, 预览流宽度]和[1, 预览流高度]。如需截取任意区域，可先获取整个画面，再使用[crop](js-apis-sendableimage.md#crop)截取所需区域。 |
...
5. async function CreatePixelMapFromSurface(surfaceId: string) {
...
5. async function CreatePixelMapSync() {
6. const color: ArrayBuffer = new ArrayBuffer(96); // 96为需要创建的像素buffer大小，取值为：height * width *4。
7. let opts: image.InitializationOptions = { editable: true, pixelFormat: 3, size: { height: 4, width: 6 } }
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| pixelmap | [image.PixelMap](<../@ohos.multimedia.image (图片处理)/Interface (PixelMap)/arkts-apis-image-pixelmap.md>) | 是 | image下的非sendable的PixelMap。 |
...
以下错误码的详细介绍请参见[通用错误码说明文档](../../../通用错误码/errorcode-universal.md)和[Image错误码](../../错误码/Image错误码/errorcode-image.md)。
...
4. async function ConvertFromPixelMap() {
...
由于图片占用内存较大，所以当PixelMap对象使用完成后，应主动调用[release](<../@ohos.multimedia.image (图片处理)/Interface (PixelMap)/arkts-apis-image-pixelmap.md#release7>)方法及时释放内存。释放时应确保该对象的所有异步方法均执行完成，且后续不再使用该对象。
...
| 类型 | 说明 |
| --- | --- |
| [image.PixelMap](<../@ohos.multimedia.image (图片处理)/Interface (PixelMap)/arkts-apis-image-pixelmap.md>) | 成功同步返回image下的非sendable的PixelMap对象，失败抛出异常。 |
...
4. async function ConvertToPixelMap() {
5. const color: ArrayBuffer = new ArrayBuffer(96); // 96为需要创建的像素buffer大小，取值为：height * width *4。
6. let opts: image.InitializationOptions = { editable: true, pixelFormat: 3, size: { height: 4, width: 6 } }
...
4. async function ReadPixelsToBuffer(pixelMap : sendableImage.PixelMap) {
...
3. async function ReadPixelsToBufferSync(pixelMap: sendableImage.PixelMap) {
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| area | [image.PositionArea](<../@ohos.multimedia.image (图片处理)/Interfaces (其他)/arkts-apis-image-i.md#positionarea7>) | 是 | 区域大小，根据区域读取。 |
...
5. async function ReadPixels(pixelMap : sendableImage.PixelMap) {
...
4. async function ReadPixelsSync(pixelMap : sendableImage.PixelMap) {
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| area | [image.PositionArea](<../@ohos.multimedia.image (图片处理)/Interfaces (其他)/arkts-apis-image-i.md#positionarea7>) | 是 | 区域，根据区域写入。 |
...
5. async function WritePixels(pixelMap : sendableImage.PixelMap) {
6. const area: image.PositionArea = {
7. pixels: new ArrayBuffer(8),
8. offset: 0,
9. stride: 8,
...
4. async function WritePixelsSync(pixelMap : sendableImage.PixelMap) {
5. const area: image.PositionArea = {
6. pixels: new ArrayBuffer(8),
7. offset: 0,
8. stride: 8,
...
4. async function WriteBufferToPixels(pixelMap : sendableImage.PixelMap) {
5. const color: ArrayBuffer = new ArrayBuffer(96); // 96为需要创建的像素buffer大小，取值为：height * width *4。
...
3. async function WriteBufferToPixelsSync(pixelMap: sendableImage.PixelMap) {
4. const bufferSize = pixelMap.getPixelBytesNumber();
...
| 类型 | 说明 |
| --- | --- |
| Promise<[image.ImageInfo](<../@ohos.multimedia.image (图片处理)/Interfaces (其他)/arkts-apis-image-i.md#imageinfo>)> | Promise实例，用于异步获取图像像素信息，失败时返回错误信息。 |
...
5. async function GetImageInfo(pixelMap : sendableImage.PixelMap) {
6. if (pixelMap != undefined) {
...
| 类型 | 说明 |
| --- | --- |
| [image.ImageInfo](<../@ohos.multimedia.image (图片处理)/Interfaces (其他)/arkts-apis-image-i.md#imageinfo>) | 图像像素信息。 |
...
4. async function GetImageInfoSync(pixelMap : sendableImage.PixelMap) {
...
3. async function GetBytesNumberPerRow(pixelMap : sendableImage.PixelMap) {
...
3. async function GetPixelBytesNumber(pixelMap : sendableImage.PixelMap) {
...
3. async function GetDensity(pixelMap : sendableImage.PixelMap) {
...
4. async function Opacity(pixelMap : sendableImage.PixelMap) {
...
3. async function OpacitySync(pixelMap : sendableImage.PixelMap) {
...
4. async function CreateAlphaPixelmap(pixelMap : sendableImage.PixelMap) {
5. if (pixelMap != undefined) {
...
3. async function CreateAlphaPixelmapSync(pixelMap : sendableImage.PixelMap) {
...
4. async function Scale(pixelMap : sendableImage.PixelMap) {
...
3. async function ScaleSync(pixelMap : sendableImage.PixelMap) {
...
4. async function Translate(pixelMap : sendableImage.PixelMap) {
...
3. async function TranslateSync(pixelMap : sendableImage.PixelMap) {
...
4. async function Rotate(pixelMap : sendableImage.PixelMap) {
...
3. async function RotateSync(pixelMap : sendableImage.PixelMap) {
...
4. async function Flip(pixelMap : sendableImage.PixelMap) {
...
3. async function FlipSync(pixelMap : sendableImage.PixelMap) {
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| region | [image.Region](<../@ohos.multimedia.image (图片处理)/Interfaces (其他)/arkts-apis-image-i.md#region8>) | 是 | 裁剪的尺寸。 |
...
5. async function Crop(pixelMap : sendableImage.PixelMap) {
6. let region: image.Region = { x: 0, y: 0, size: { height: 100, width: 100 } };
7. if (pixelMap != undefined) {
...
4. async function CropSync(pixelMap : sendableImage.PixelMap) {
...
| 类型 | 说明 |
| --- | --- |
| [colorSpaceManager.ColorSpaceManager](<../../../ArkGraphics 2D（方舟2D图形服务）/ArkTS API/@ohos.graphics.colorSpaceManager (色彩管理)/js-apis-colorspacemanager.md#colorspacemanager>) | 图像广色域信息。 |
...
3. async function GetColorSpace(pixelMap : sendableImage.PixelMap) {
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| colorSpace | [colorSpaceManager.ColorSpaceManager](<../../../ArkGraphics 2D（方舟2D图形服务）/ArkTS API/@ohos.graphics.colorSpaceManager (色彩管理)/js-apis-colorspacemanager.md#colorspacemanager>) | 是 | 图像广色域信息。 |
...
4. async function SetColorSpace(pixelMap : sendableImage.PixelMap) {
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| targetColorSpace | [colorSpaceManager.ColorSpaceManager](<../../../ArkGraphics 2D（方舟2D图形服务）/ArkTS API/@ohos.graphics.colorSpaceManager (色彩管理)/js-apis-colorspacemanager.md#colorspacemanager>) | 是 | 目标色彩空间，支持SRGB、DCI\_P3、DISPLAY\_P3、ADOBE\_RGB\_1998。 |
...
5. async function ApplyColorSpace(pixelMap : sendableImage.PixelMap) {
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| sequence | [rpc.MessageSequence](<../../../IPC Kit（进程间通信服务）/ArkTS API/@ohos.rpc (RPC通信)/js-apis-rpc.md#messagesequence9>) | 是 | 新创建的MessageSequence。 |
...
13. console.info('marshalling');
14. return true;
15. }
16. unmarshalling(messageSequence : rpc.MessageSequence) {
17. sendableImage.createPixelMap(new ArrayBuffer(96), {size: { height:4, width: 6}}).then((pixelParcel: sendableImage.PixelMap) => {
18. pixelParcel.unmarshalling(messageSequence).then(async (pixelMap: sendableImage.PixelMap) => {
19. this.pixel_map = pixelMap;
20. pixelMap.getImageInfo().then((imageInfo: image.ImageInfo) => {
21. console.info("unmarshalling information h:" + imageInfo.size.height + "w:" + imageInfo.size.width);
22. })
23. })
24. });
25. return true;
...
29. async function Marshalling() {
30. const color: ArrayBuffer = new ArrayBuffer(96);
31. let bufferArr: Uint8Array = new Uint8Array(color);
32. for (let i = 0; i < bufferArr.length; i++) {
33. bufferArr[i] = 0x80;
34. }
35. let opts: image.InitializationOptions = {
36. editable: true,
37. pixelFormat: 4,
38. size: { height: 4, width: 6 },
39. alphaType: 3
40. }
41. let pixelMap: sendableImage.PixelMap | undefined = undefined;
42. await sendableImage.createPixelMap(color, opts).then((srcPixelMap: sendableImage.PixelMap) => {
43. pixelMap = srcPixelMap;
44. })
45. if (pixelMap != undefined) {
46. // 序列化。
47. let parcelable: MySequence = new MySequence(pixelMap);
48. let data: rpc.MessageSequence = rpc.MessageSequence.create();
49. data.writeParcelable(parcelable);

51. // 反序列化rpc获取到data。
52. let ret: MySequence = new MySequence(pixelMap);
53. data.readParcelable(ret);
54. }
55. }
```
...
29. async function Unmarshalling() {
30. const color: ArrayBuffer = new ArrayBuffer(96);
31. let bufferArr: Uint8Array = new Uint8Array(color);
32. for (let i = 0; i < bufferArr.length; i++) {
33. bufferArr[i] = 0x80;
...
4. async function Release(pixelMap: sendableImage.PixelMap) {
5. if (pixelMap != undefined) {
...
继承自[lang.ISendable](../../../../harmonyos-guides/应用框架/ArkTS（方舟编程语言）/ArkTS并发/并发线程间通信/线程间通信对象/Sendable对象/Sendable对象简介/arkts-sendable.md#isendable)。
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| uri | string | 是 | 图片路径，当前仅支持应用沙箱路径。  当前支持格式有：.jpg .png .gif .bmp .webp .dng [SVG](<../@ohos.multimedia.image (图片处理)/Functions/arkts-apis-image-f.md#svg标签说明>) .ico。 |
...
3. async function CreateImageSource() {
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| size | [image.Size](<../@ohos.multimedia.image (图片处理)/Interfaces (其他)/arkts-apis-image-i.md#size>) | 是 | 图像的默认大小。 |
| format | [image.ImageFormat](<../@ohos.multimedia.image (图片处理)/Enums/arkts-apis-image-e.md#imageformat9>) | 是 | 图像格式，取值为image.ImageFormat常量，目前仅支持 ImageFormat:JPEG。 |
...
4. async function CreateImageReceiver() {
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| options | [image.DecodingOptions](<../@ohos.multimedia.image (图片处理)/Interfaces (其他)/arkts-apis-image-i.md#decodingoptions7>) | 否 | 解码参数。 |
...
4. async function CreatePixelMap(context : Context) {
5. const path: string = context.cacheDir + "/test.jpg";
...
4. async function Release(context : Context) {
5. const path: string = context.cacheDir + "/test.jpg";
6. const sendableImageSourceObj: sendableImage.ImageSource = sendableImage.createImageSource(path);
...
提供基本的图像操作，包括获取图像信息、读写图像数据。调用[readNextImage](js-apis-sendableimage.md#readnextimage)和[readLatestImage](js-apis-sendableimage.md#readlatestimage)接口时会返回Image。继承自[ISendable](../../../../harmonyos-guides/应用框架/ArkTS（方舟编程语言）/ArkTS并发/并发线程间通信/线程间通信对象/Sendable对象/Sendable对象简介/arkts-sendable.md#isendable)。
...
| size | [Size](js-apis-sendableimage.md#size) | 是 | 否 | 图像大小。  如果Image对象所存储的是相机预览流数据（YUV图像数据），那么获取到的size中的宽和高分别对应YUV图像的宽和高。  如果Image对象所存储的是相机拍照流数据（JPEG图像数据），由于已是编码后的文件，size中的宽等于JPEG文件大小，高等于1。  Image对象所存储的数据是预览流还是拍照流，取决于应用将receiver中的surfaceId传给相机的是previewOutput还是captureOutput。  相机预览与拍照最佳实践请参考[双路预览(ArkTS)](<../../../../harmonyos-guides/媒体/Camera Kit（相机服务）/开发相机应用基础能力(ArkTS)/双路预览(ArkTS)/camera-dual-channel-preview.md>)与[拍照实践(ArkTS)](<../../../../harmonyos-guides/媒体/Camera Kit（相机服务）/开发相机应用基础能力(ArkTS)/拍照实践(ArkTS)/camera-shooting-case.md>)。 |
| format | number | 是 | 否 | 图像格式，参考[OH\_NativeBuffer\_Format](<../../../ArkGraphics 2D（方舟2D图形服务）/C API/头文件/buffer_common.h/capi-buffer-common-h.md#oh_nativebuffer_format>)。 |
| timestamp | number | 是 | 否 | 图像时间戳。时间戳以纳秒为单位，通常是单调递增的。时间戳的具体含义和基准取决于图像的生产者，在相机预览/拍照场景，生产者就是相机。来自不同生产者的图像的时间戳可能有不同的含义和基准，因此可能无法进行比较。如果要获取某张照片的生成时间，可以通过[getImageProperty](<../@ohos.multimedia.image (图片处理)/Interface (ImageSource)/arkts-apis-image-imagesource.md#getimageproperty11>)接口读取相关的EXIF信息。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| componentType | [image.ComponentType](<../@ohos.multimedia.image (图片处理)/Enums/arkts-apis-image-e.md#componenttype9>) | 是 | 图像的组件类型。 |

**返回值：**

| 类型 | 说明 |
| --- | --- |
| Promise<[image.Component](<../@ohos.multimedia.image (图片处理)/Interfaces (其他)/arkts-apis-image-i.md#component9>)> | Promise实例，用于异步返回组件缓冲区。 |
...
| 名称 | 类型 | 只读 | 可选 | 说明 |
| --- | --- | --- | --- | --- |
| size | [image.Size](<../@ohos.multimedia.image (图片处理)/Interfaces (其他)/arkts-apis-image-i.md#size>) | 是 | 否 | 图片大小。 |
| capacity | number | 是 | 否 | 同时访问的图像数。该参数仅作为期望值，实际capacity由设备硬件决定。 |
| format | [image.ImageFormat](<../@ohos.multimedia.image (图片处理)/Enums/arkts-apis-image-e.md#imageformat9>) | 是 | 否 | 图像格式。 |
...
5. async function GetReceivingSurfaceId() {
6. let size: image.Size = {
7. height: 8192,
8. width: 8
9. }
...
5. async function ReadLatestImage() {
6. let size: image.Size = {
7. height: 8192,
8. width: 8
9. }
...
5. async function ReadNextImage() {
6. let size: image.Size = {
7. height: 8192,
8. width: 8
9. }
