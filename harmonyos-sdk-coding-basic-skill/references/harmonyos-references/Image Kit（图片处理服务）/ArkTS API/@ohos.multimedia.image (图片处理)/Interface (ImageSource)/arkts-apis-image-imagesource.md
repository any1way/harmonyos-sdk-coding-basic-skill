...
scraped_at: 2026-06-01T16:21:59+08:00
doc_updated_at: 2026-05-26
content_hash: sha256:e46ddbbbd1e6ae717f9ac2217d1cb2b88077c361a2a8be088adffc244d8582db
---
ImageSource类，用于获取图片相关信息。

在调用ImageSource的方法前，需要先通过[image.createImageSource](../Functions/arkts-apis-image-f.md#imagecreateimagesource)构建一个ImageSource实例。
...
| 名称 | 类型 | 只读 | 可选 | 说明 |
| --- | --- | --- | --- | --- |
| supportedFormats | Array<string> | 是 | 否 | 支持的图片格式，包括：png、jpeg、bmp、gif、webp、dng、heic12+、wbmp23+、heifs23+、tiff23+。部分格式的解码能力依赖于具体的设备硬件，建议在调用前使用[image.getImageSourceSupportedFormats20+](../Functions/arkts-apis-image-f.md#imagegetimagesourcesupportedformats20)接口，动态查询当前设备上的解码能力。 |
...
| callback | AsyncCallback<[ImageInfo](<../Interfaces (其他)/arkts-apis-image-i.md#imageinfo>)> | 是 | 回调函数。当获取图片信息成功，err为undefined，data为获取到的图片信息；否则为错误对象。 |
...
| 类型 | 说明 |
| --- | --- |
| Promise<[ImageInfo](<../Interfaces (其他)/arkts-apis-image-i.md#imageinfo>)> | Promise对象，返回获取到的图片信息。 |
...
该方法为同步方法，调用时会阻塞当前线程，不建议在主线程中调用，否则可能导致应用卡顿、掉帧或响应延迟。具体场景参考[耗时任务并发场景简介](../../../../../harmonyos-guides/应用框架/ArkTS（方舟编程语言）/ArkTS并发/应用多线程开发实践/耗时任务并发场景/耗时任务并发场景简介/time-consuming-task-overview.md)。
...
| 类型 | 说明 |
| --- | --- |
| [ImageInfo](<../Interfaces (其他)/arkts-apis-image-i.md#imageinfo>) | 同步返回获取到的图片信息。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| key | [PropertyKey](../Enums/arkts-apis-image-e.md#propertykey7) | 是 | 图片属性名。 |
| options | [ImagePropertyOptions](<../Interfaces (其他)/arkts-apis-image-i.md#imagepropertyoptions11>) | 否 | 图片属性，包括图片序号与默认属性值。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| key | Array<[PropertyKey](../Enums/arkts-apis-image-e.md#propertykey7)> | 是 | 图片属性名的数组。 |

**返回值：**

| 类型 | 说明 |
| --- | --- |
| Promise<Record<[PropertyKey](../Enums/arkts-apis-image-e.md#propertykey7), string | null>> | Promise对象，返回图片属性值，如获取失败则返回null。 |
...
8. console.error(`Failed to get the properties, error.code ${err.code}, error.message ${err.message}`);
9. });
10. }
```
...
| 类型 | 说明 |
| --- | --- |
| string | 返回图片Exif中指定属性键的值（如获取失败则返回属性默认值），各个数据值作用请参考[PropertyKey](../Enums/arkts-apis-image-e.md#propertykey7)。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| records | Record<[PropertyKey](../Enums/arkts-apis-image-e.md#propertykey7), string | null> | 是 | 包含图片属性名和属性值的数组。 |
...
* 调用该接口修改属性会改变属性字节长度，建议通过传入文件描述符来创建[image.createImageSource](../Functions/arkts-apis-image-f.md#imagecreateimagesource7)实例或通过传入的uri创建[image.createImageSource](../Functions/arkts-apis-image-f.md#imagecreateimagesource)实例。
...
读取DNG格式图片时，该接口对部分propertyKeys有特殊处理。以下字段的字符串取值请参考[PropertyKey](../Enums/arkts-apis-image-e.md#propertykey7)中的值：
...
* 从API版本24开始，支持读取DNG元数据。要查询的属性的具体信息请参考[DngPropertyKey](../Enums/arkts-apis-image-e.md#dngpropertykey24)。
...
| 类型 | 说明 |
| --- | --- |
| Promise<[ImageMetadata](<../Interfaces (其他)/arkts-apis-image-i.md#imagemetadata23>)> | Promise对象，返回ImageMetadata对象，其中含有图片属性名对应的metadata对象，通过ImageMetadata中的metadata对象可以获取图片属性值。 |
...
12. console.error(`Failed to read image metadata. error.code is ${error.code}, error.message is ${error.message}`);
13. })
14. }
```
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| imageMetadata | [ImageMetadata](<../Interfaces (其他)/arkts-apis-image-i.md#imagemetadata23>) | 是 | 图像的元数据集。若imageMetadata中的属性值都为空，则清空所有Exif元数据。 |
...
10. console.info(`Succeeded in writing image metadata.`);
11. }).catch((error: BusinessError) => {
12. console.error(`Failed to write image metadata. error.code is ${error.code}, error.message is ${error.message}`);
13. });
14. }
```
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| metadataTypes | [MetadataType](../Enums/arkts-apis-image-e.md#metadatatype13)[] | 否 | 元数据类型的数组。当该参数缺省时，获取全部支持的元数据。 |
...
| 类型 | 说明 |
| --- | --- |
| Promise<[ImageMetadata](<../Interfaces (其他)/arkts-apis-image-i.md#imagemetadata23>)> | Promise对象。返回的ImageMetadata对象中含有对应的metadata对象，通过ImageMetadata中的metadata对象可以获取图片属性值。 |
...
9. }
10. }).catch((error: BusinessError) => {
11. console.error(`Failed to read image metadata by type. error.code is ${error.code}, error.message is ${error.message}`);
12. })
13. }
```
...
| 类型 | 说明 |
| --- | --- |
| Promise<[ImageRawData](<../Interfaces (其他)/arkts-apis-image-i.md#imagerawdata24>)> | Promise对象，返回ImageRawData对象，其中含有数据缓冲区和像素位数。 |
...
5. console.info(`createImageRawData successfully. length: ${data.buffer.byteLength}, bitPerPixel:${data.bitsPerPixel}`);
...
16. console.error(`Failed to create image rawData. error.code is ${error.code}, error.message is ${error.message}`);
17. });
18. }
```
...
由于图片占用内存较大，所以当Picture对象使用完成后，应主动调用[release](<../Interface (Picture)/arkts-apis-image-picture.md#release13>)方法，及时释放内存。
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| options | [DecodingOptionsForPicture](<../Interfaces (其他)/arkts-apis-image-i.md#decodingoptionsforpicture13>) | 否 | 解码参数。 |

**返回值：**

| 类型 | 说明 |
| --- | --- |
| Promise<[Picture](<../Interface (Picture)/arkts-apis-image-picture.md>)> | Promise对象，返回Picture。 |
...
7. console.info('Succeeded in creating picture.');
8. } else {
9. console.error('Failed to create picture.');
10. }
11. }
```
...
6. console.info('Succeeded in creating picture for frame: ' + index);
7. } catch (e) {
8. console.error('Failed to create picture for frame: ' + index);
9. }
10. }
11. }
```
...
从API version 15开始，推荐使用[createPixelMapUsingAllocator](arkts-apis-image-imagesource.md#createpixelmapusingallocator15)，该接口可以指定输出pixelMap的内存类型[AllocatorType](../Enums/arkts-apis-image-e.md#allocatortype15)，详情请参考[图片解码内存优化(ArkTS)](<../../../../../harmonyos-guides/媒体/Image Kit（图片处理服务）/图片开发指导(ArkTS)/图片解码/图片解码内存优化(ArkTS)/image-allocator-type.md>)。

说明

* 该方法为非线程安全的方法，不支持在同一个ImageSource实例上并发调用。
* 由于图片占用内存较大，所以当PixelMap对象使用完成后，应主动调用[release](<../Interface (PixelMap)/arkts-apis-image-pixelmap.md#release7>)方法，及时释放内存。
* 释放时应确保该对象的所有异步方法均执行完成，且后续不再使用该对象。
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| options | [DecodingOptions](<../Interfaces (其他)/arkts-apis-image-i.md#decodingoptions7>) | 否 | 解码参数。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| callback | AsyncCallback<[PixelMap](<../Interface (PixelMap)/arkts-apis-image-pixelmap.md>)> | 是 | 回调函数，当创建PixelMap对象成功，err为undefined，data为获取到的PixelMap对象；否则为错误对象。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| options | [DecodingOptions](<../Interfaces (其他)/arkts-apis-image-i.md#decodingoptions7>) | 是 | 解码参数。 |
| callback | AsyncCallback<[PixelMap](<../Interface (PixelMap)/arkts-apis-image-pixelmap.md>)> | 是 | 回调函数，当创建PixelMap对象成功，err为undefined，data为获取到的PixelMap对象；否则为错误对象。 |
...
从API version 15开始，推荐使用[createPixelMapUsingAllocatorSync](arkts-apis-image-imagesource.md#createpixelmapusingallocatorsync15)，该接口可以指定输出pixelMap的内存类型[AllocatorType](../Enums/arkts-apis-image-e.md#allocatortype15)，详情请参考[图片解码内存优化(ArkTS)](<../../../../../harmonyos-guides/媒体/Image Kit（图片处理服务）/图片开发指导(ArkTS)/图片解码/图片解码内存优化(ArkTS)/image-allocator-type.md>)。
...
| 类型 | 说明 |
| --- | --- |
| [PixelMap](<../Interface (PixelMap)/arkts-apis-image-pixelmap.md>) | 用于同步返回创建结果。 |
...
| 类型 | 说明 |
| --- | --- |
| Promise<Array<[PixelMap](<../Interface (PixelMap)/arkts-apis-image-pixelmap.md>)>> | 异步返回PixelMap数组。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| callback | AsyncCallback<Array<[PixelMap](<../Interface (PixelMap)/arkts-apis-image-pixelmap.md>)>> | 是 | 回调函数，当创建PixelMap对象数组成功，err为undefined，data为获取到的PixelMap对象数组；否则为错误对象。 |
...
使用指定的分配器根据图像解码参数异步创建PixelMap对象。使用Promise异步回调。接口使用详情请参考[图片解码内存优化(ArkTS)](<../../../../../harmonyos-guides/媒体/Image Kit（图片处理服务）/图片开发指导(ArkTS)/图片解码/图片解码内存优化(ArkTS)/image-allocator-type.md>)。
...
| --- | --- | --- | --- |
| options | [DecodingOptions](<../Interfaces (其他)/arkts-apis-image-i.md#decodingoptions7>) | 否 | 解码参数。 |
| allocatorType | [AllocatorType](../Enums/arkts-apis-image-e.md#allocatortype15) | 否 | 用于图像解码的内存类型。默认值为AllocatorType.AUTO。 |
...
根据指定的分配器同步创建一个基于图像解码参数的PixelMap对象。接口使用详情请参考[图片解码内存优化(ArkTS)](<../../../../../harmonyos-guides/媒体/Image Kit（图片处理服务）/图片开发指导(ArkTS)/图片解码/图片解码内存优化(ArkTS)/image-allocator-type.md>)。
...
| --- | --- | --- | --- |
| key | string | 是 | 图片属性名。 |
| options | [GetImagePropertyOptions](<../Interfaces (其他)/arkts-apis-image-i.md#getimagepropertyoptionsdeprecated>) | 否 | 图片属性，包括图片序号与默认属性值。 |
...
| --- | --- | --- | --- |
| key | string | 是 | 图片属性名。 |
| options | [GetImagePropertyOptions](<../Interfaces (其他)/arkts-apis-image-i.md#getimagepropertyoptionsdeprecated>) | 是 | 图片属性，包括图片序号与默认属性值。 |
| callback | AsyncCallback<string> | 是 | 回调函数，当获取图片属性值成功，err为undefined，data为获取到的图片属性值；否则为错误对象。 |
