...
scraped_at: 2026-06-11T16:31:32+08:00
doc_updated_at: 2026-06-05
content_hash: sha256:79856f4910af951784d7cb979805610a952cec5da8c4462209ebe446450c5566
---
Picture类，一些包含特殊信息的图片可以解码为Picture（也可以称为多图对象）。多图对象一般包含主图、辅助图和元数据。其中主图包含图像的大部分信息，主要用于显示图像内容；辅助图用于存储与主图相关但不同的数据，展示图像更丰富的信息；元数据一般用来存储关于图像文件的信息。多图对象类用于读取或写入多图对象。在调用Picture的方法前，需要先通过[image.createPicture](../Functions/arkts-apis-image-f.md#imagecreatepicture13)创建一个Picture实例。
...
| 类型 | 说明 |
| --- | --- |
| [PixelMap](<../Interface (PixelMap)/arkts-apis-image-pixelmap.md>) | 同步返回PixelMap对象。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| options | [HdrComposeOptions](<../Interfaces (其他)/arkts-apis-image-i.md#hdrcomposeoptions23>) | 否 | 合成HDR的选项。 |

**返回值：**

| 类型 | 说明 |
| --- | --- |
| Promise<[PixelMap](<../Interface (PixelMap)/arkts-apis-image-pixelmap.md>) | undefined> | Promise对象，返回PixelMap或undefined。 |
...
5. async function GetHdrComposedPixelmapWithOptions(picture : image.Picture) {
6. if (picture == null) {
7. console.error('picture is null');
8. return;
9. }

11. let opt: image.HdrComposeOptions = {
12. desiredPixelFormat: image.PixelMapFormat.RGBA_1010102
13. };
14. let hdrComposedPixelmap: image.PixelMap | undefined = await picture.getHdrComposedPixelmapWithOptions(opt);
15. if (hdrComposedPixelmap == null || hdrComposedPixelmap == undefined) {
16. console.error(`GetHdrComposedPixelmapWithOptions failed`);
17. return;
18. }

20. hdrComposedPixelmap.getImageInfo().then((imageInfo: image.ImageInfo) => {
21. if (imageInfo !== null) {
22. console.info(`GetHdrComposedPixelmapWithOptions information height:${imageInfo.size.height} width:${imageInfo.size.width}`);
23. }
24. }).catch((error: BusinessError) => {
25. console.error(`Failed to getHdrComposedPixelmapWithOptions information. error.code: ${error.code} ,error.message: ${error.message}`);
26. });
27. }
```
...
| 类型 | 说明 |
| --- | --- |
| [PixelMap](<../Interface (PixelMap)/arkts-apis-image-pixelmap.md>) | null | 返回Pixelmap对象，如果没有则返回null。 |
...
| auxiliaryPicture | [AuxiliaryPicture](<../Interface (AuxiliaryPicture)/arkts-apis-image-auxiliarypicture.md>) | 是 | 辅助图对象。 |
...
| 类型 | 说明 |
| --- | --- |
| [AuxiliaryPicture](<../Interface (AuxiliaryPicture)/arkts-apis-image-auxiliarypicture.md>) | null | 返回AuxiliaryPicture对象，如果没有则返回null。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| metadataType | [MetadataType](../Enums/arkts-apis-image-e.md#metadatatype13) | 是 | 元数据类型。 |
| metadata | [Metadata](<../Interface (Metadata)/arkts-apis-image-metadata.md>) | 是 | 元数据对象。 |
...
13. console.info('Succeeded in creating picture.');
14. } else {
15. console.error('Failed to create picture.');
16. }
...
22. console.info('Succeeded in setting metadata.');
23. }).catch((error: BusinessError) => {
24. console.error(`Failed to set metadata. error.code: ${error.code} ,error.message: ${error.message}`);
25. });
26. } else {
...
| 类型 | 说明 |
| --- | --- |
| Promise<[Metadata](<../Interface (Metadata)/arkts-apis-image-metadata.md>)> | Promise对象。返回元数据。 |
...
6. console.info('Succeeded in getting picture metadata.');
7. } else {
8. console.error('Failed to get picture metadata.');
9. }
10. } else {
...
12. console.info('Succceed in marshalling.');
13. return true;
14. } else {
...
23. }).catch((error: BusinessError) => {
24. console.error(`Failed to unmarshall. error.code: ${error.code} ,error.message: ${error.message}`);
25. });
26. return true;
27. }
28. }
