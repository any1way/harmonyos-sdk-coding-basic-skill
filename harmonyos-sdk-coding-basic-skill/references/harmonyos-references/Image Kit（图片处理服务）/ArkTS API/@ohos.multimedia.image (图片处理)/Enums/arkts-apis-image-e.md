...
content_hash: sha256:a877ccdfda0de2c0f14cfcdf703a17b7cb40fc2c502109ba65986a66641e53e9
---
说明
...
| RGBA\_8888 | 3 | 颜色信息由R（Red），G（Green），B（Blue）与透明度（Alpha）四部分组成，每个部分占8位，总共占32位。对应[相机服务CameraFormat中的CAMERA\_FORMAT\_RGBA\_8888](<../../../../Camera Kit（相机服务）/ArkTS API/@ohos.multimedia.camera (相机管理)/Enums/arkts-apis-camera-e.md#cameraformat>)。  **元服务API：** 从API version 11开始，该接口支持在元服务中使用。  **卡片能力：** 从API version 12开始，该接口支持在ArkTS卡片中使用。 |
...
| NV219+ | 8 | YVU像素排列，V分量在U分量之前。颜色信息由亮度分量Y和交错排列的色度分量V和U组成，其中Y分量占8位，UV分量因4：2：0采样平均占4位，总共平均占12位。对应[相机服务CameraFormat中的CAMERA\_FORMAT\_YUV\_420\_SP](<../../../../Camera Kit（相机服务）/ArkTS API/@ohos.multimedia.camera (相机管理)/Enums/arkts-apis-camera-e.md#cameraformat>)。  **元服务API：** 从API version 11开始，该接口支持在元服务中使用。  **卡片能力：** 从API version 12开始，该接口支持在ArkTS卡片中使用。 |
...
| YCBCR\_P01012+ | 11 | 颜色信息由亮度分量Y和色度分量Cb与Cr组成，每个分量有效10位，实际存储时，Y平面每个像素占16位数据（10位有效），UV平面交错排列，每4个像素占32位数据（每色度分量10位有效），平均有效占15位。对应[相机服务CameraFormat中的CAMERA\_FORMAT\_YCBCR\_P010](<../../../../Camera Kit（相机服务）/ArkTS API/@ohos.multimedia.camera (相机管理)/Enums/arkts-apis-camera-e.md#cameraformat>)。 |
| YCRCB\_P01012+ | 12 | 颜色信息由亮度分量Y和色度分量Cr与Cb组成，每个分量有效10位，实际存储时，Y平面每个像素占16位数据（10位有效），UV平面交错排列，每4个像素占32位数据（每色度分量10位有效），平均有效占15位。对应[相机服务CameraFormat中的CAMERA\_FORMAT\_YCRCB\_P010](<../../../../Camera Kit（相机服务）/ArkTS API/@ohos.multimedia.camera (相机管理)/Enums/arkts-apis-camera-e.md#cameraformat>)。 |
...
在获取和使用特定辅助图前，应首先调用Picture的[getAuxiliaryPicture](<../Interface (Picture)/arkts-apis-image-picture.md#getauxiliarypicture13>)方法尝试获取该辅助图。

**系统能力：** SystemCapability.Multimedia.Image.Core

| 名称 | 值 | 说明 |
| --- | --- | --- |
| GAINMAP | 1 | 增益图（Gain Map）。  用于更准确地生成HDR图像。  HDR合成通常需要同时使用SDR主图、增益图和HDR元数据（参考[getMetadata](<../Interface (PixelMap)/arkts-apis-image-pixelmap.md#getmetadata12>)）共同计算亮度映射关系。 |
...
* 格式示例仅用于说明修改传值和读取结果的格式。具体接口使用方法请参考：[modifyImageProperty](<../Interface (ImageSource)/arkts-apis-image-imagesource.md#modifyimageproperty11>)（修改单个Exif字段）、[modifyImageProperties](<../Interface (ImageSource)/arkts-apis-image-imagesource.md#modifyimageproperties12>)（修改多个Exif字段）、[getImageProperty](<../Interface (ImageSource)/arkts-apis-image-imagesource.md#getimageproperty11>)（读取单个Exif字段）、[getImageProperties](<../Interface (ImageSource)/arkts-apis-image-imagesource.md#getimageproperties12>)（读取多个Exif字段）。
...
| ORIENTATION | "Orientation" | 图片方向。  1："Top-left"，图像未旋转。  2："Top-right"，镜像水平翻转。  3："Bottom-right"，图像旋转180°。  4："Bottom-left"，镜像垂直翻转。  5："Left-top"，镜像水平翻转再顺时针旋转270°。  6："Right-top"，顺时针旋转90°。  7："Right-bottom"，镜像水平翻转再顺时针旋转90°。  8："Left-bottom"，顺时针旋转270°。  如果读到未定义值x会返回"Unknown Value x"。获取该属性时会以字符串的形式返回。修改该属性时既可以以数字形式指定，也可以以字符串形式指定。  更多关于图片旋转角度的说明可参考：[如何获取图片的旋转角度信息](<../../../../../harmonyos-guides/媒体/Image Kit（图片处理服务）/Image Kit常见问题/如何获取图片的旋转角度信息/image-rotate-faq.md>)。  **读写能力：** 可读写。 | **修改传参格式说明：** 修改时传入相应的数字或者字符串。  **修改示例：**imageSource.modifyImageProperty(key,'1');  或imageSource.modifyImageProperty(key,'Top-left');  **读取结果示例：** "Top-left" |
...
| GPS\_H\_POSITIONING\_ERROR12+ | "GPSHPositioningError" | 此标签指示水平定位误差，单位为米（m）。  **读写能力：** 可读写。 | **修改传参格式说明：** 非负有理数字符串。  **修改示例：**imageSource.modifyImageProperty(key,'100');  **读取结果示例：** "100" |
...
| 名称 | 值 | 说明 |
| --- | --- | --- |
| HEIFS\_DELAY\_TIME | 'HeifsDelayTime' | HEIF序列图片的每帧延迟时长。  单位为毫秒（ms）。 |
...
| 名称 | 值 | 说明 |
| --- | --- | --- |
| CANVAS\_WIDTH | 'WebPCanvasWidth' | WebP图片的画布像素宽度。  单位为像素（px）。 |
| CANVAS\_HEIGHT | 'WebPCanvasHeight' | WebP图片的画布像素高度。  单位为像素（px）。 |
| DELAY\_TIME | 'WebPDelayTime' | WebP图片钳制后的帧延迟时长。钳制范围为[100, 65535]。  单位为毫秒（ms）。 |
| UNCLAMPED\_DELAY\_TIME | 'WebPUnclampedDelayTime' | WebP图片未钳制的帧延迟时长。  单位为毫秒（ms）。 |
...
* 格式示例中的key为：image.DngPropertyKey.XXX（XXX为枚举的名称，如：image.DngPropertyKey.DNG\_BACKWARD\_VERSION）。
* 格式示例仅用于读取结果的格式。具体接口使用方法请参考：[readImageMetadata](<../Interface (ImageSource)/arkts-apis-image-imagesource.md#readimagemetadata23>)。
* 返回字段类型具体参考[DngMetadata](<../Interfaces (其他)/arkts-apis-image-i.md#dngmetadata24>)。
...
| 名称 | 值 | 说明 |
| --- | --- | --- |
| AUTO | 0 | 自适应，根据图片信息处理。即如果图片本身为HDR图片，则会按照HDR内容解码；反之按照SDR内容解码。通过[CreateIncrementalSource](../Functions/arkts-apis-image-f.md#imagecreateincrementalsource9)创建的ImageSource会解码为SDR内容。 |
| SDR | 1 | 按照标准动态范围处理图片。 |
| HDR | 2 | 按照高动态范围处理图片。通过[CreateIncrementalSource](../Functions/arkts-apis-image-f.md#imagecreateincrementalsource9)创建的ImageSource会解码为SDR内容。 |
...
| 名称 | 值 | 说明 |
| --- | --- | --- |
| AUTO | 0 | 自适应，根据[pixelmap](<../Interface (PixelMap)/arkts-apis-image-pixelmap.md>)内容处理。即如果pixelmap本身为HDR，则会按照HDR内容进行编码；反之按照SDR内容编码。 |
| SDR | 1 | 按照标准动态范围处理图片。 |
...
如果在配置解码选项[DecodingOptions](<../Interfaces (其他)/arkts-apis-image-i.md#decodingoptions7>)时，未填入参数cropAndScaleStrategy，并且同时设置了参数desiredRegion和desiredSize，由于系统对于不同图片格式采用的解码算法不同，最终解码效果将略有差异。
...
枚举，[pixelmap](<../Interface (PixelMap)/arkts-apis-image-pixelmap.md>)使用的HDR相关元数据信息的关键字。

**系统能力：** SystemCapability.Multimedia.Image.Core

| 名称 | 值 | 说明 |
| --- | --- | --- |
| HDR\_METADATA\_TYPE | 0 | [pixelmap](<../Interface (PixelMap)/arkts-apis-image-pixelmap.md>)使用的元数据类型。 |
