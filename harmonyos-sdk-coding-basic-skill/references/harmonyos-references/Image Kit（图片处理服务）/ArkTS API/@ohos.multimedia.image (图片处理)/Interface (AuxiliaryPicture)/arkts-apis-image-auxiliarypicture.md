...
scraped_at: 2026-06-01T16:21:52+08:00
doc_updated_at: 2026-05-19
content_hash: sha256:95286b6e5cddadc6242ee3e5ee0d573cbacf644524bf582ffffaa32ca0339952
---
AuxiliaryPicture类，用于读取或写入图像的辅助图数据以及获取图像的辅助图信息。目前支持的辅助图类型可参考[AuxiliaryPictureType](../Enums/arkts-apis-image-e.md#auxiliarypicturetype13)。

在调用AuxiliaryPicture的方法前，需要通过[image.createAuxiliaryPicture](../Functions/arkts-apis-image-f.md#imagecreateauxiliarypicture13)或Picture的[getAuxiliaryPicture](<../Interface (Picture)/arkts-apis-image-picture.md#getauxiliarypicture13>)创建一个AuxiliaryPicture实例。
...
14. console.info('Succeeded in writing pixels from buffer.');
15. } else {
...
15. console.info('Succeeded in reading pixels to buffer.');
16. }).catch((error: BusinessError) => {
17. console.error(`Failed to read pixels to buffer. error.code: ${error.code}, error.message: ${error.message}`);
18. });
19. } else {
...
| 类型 | 说明 |
| --- | --- |
| [AuxiliaryPictureType](../Enums/arkts-apis-image-e.md#auxiliarypicturetype13) | 操作成功，返回辅助图的类型。 |
...
4. console.info('Succeeded in getting auxiliary picture type ' +  JSON.stringify(type));
5. } else {
6. console.error('Failed to get auxiliary picture type.');
7. }
8. }
```
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| metadataType | [MetadataType](../Enums/arkts-apis-image-e.md#metadatatype13) | 是 | 元数据的类型，用于设置对应的元数据。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| metadataType | [MetadataType](../Enums/arkts-apis-image-e.md#metadatatype13) | 是 | 元数据类型，用于获取对应类型的元数据。 |

**返回值：**

| 类型 | 说明 |
| --- | --- |
| Promise<[Metadata](<../Interface (Metadata)/arkts-apis-image-metadata.md>)> | Promise对象，返回元数据的Promise对象。 |
...
6. console.info('Succeeded in getting AuxPictureObj Metadata.' );
7. } else {
8. console.error('Failed to get AuxPictureObj Metadata.');
9. }
10. } else {
...
| 类型 | 说明 |
| --- | --- |
| [AuxiliaryPictureInfo](<../Interfaces (其他)/arkts-apis-image-i.md#auxiliarypictureinfo13>) | 返回辅助图图像信息。 |
...
8. } else {
9. console.error('Failed to get auxiliary picture information.');
10. }
11. }
```
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| info | [AuxiliaryPictureInfo](<../Interfaces (其他)/arkts-apis-image-i.md#auxiliarypictureinfo13>) | 是 | 辅助图的图像信息。 |
