...
scraped_at: 2026-06-11T16:31:34+08:00
doc_updated_at: 2026-06-05
content_hash: sha256:a70f4cc00689cf86bb719a0ed640017fd814d7085f5608c55962fa95d67d9058
---
图像像素类，用于读取或写入图像数据以及获取图像信息。在调用PixelMap的方法前，需要先通过[image.createPixelMap](../Functions/arkts-apis-image-f.md#imagecreatepixelmap8)创建一个PixelMap实例。目前PixelMap序列化大小最大128MB，超过会送显失败。大小计算方式为（宽\*高\*每像素占用字节数（参考[PixelMapFormat](../Enums/arkts-apis-image-e.md#pixelmapformat7)））。

从API version 11开始，PixelMap支持通过[Worker](<../../../../ArkTS（方舟编程语言）/ArkTS API/@ohos.worker (启动一个Worker)/js-apis-worker.md>)跨线程调用。当PixelMap通过[Worker](<../../../../ArkTS（方舟编程语言）/ArkTS API/@ohos.worker (启动一个Worker)/js-apis-worker.md>)跨线程后，原线程的PixelMap的所有接口均不能调用，否则将报错501 服务器不具备完成请求的功能。

在调用PixelMap的方法前，可以通过[image.createPixelMap](../Functions/arkts-apis-image-f.md#imagecreatepixelmap8)传入像素数据创建一个PixelMap对象，也可以通过[ImageSource](<../Interface (ImageSource)/arkts-apis-image-imagesource.md>)进行图片解码创建PixelMap对象。

开发元服务请通过[ImageSource](<../Interface (ImageSource)/arkts-apis-image-imagesource.md>)构建PixelMap对象。
...
固定按照BGRA\_8888格式，读取PixelMap指定区域内的图像像素数据，并写入[PositionArea](<../Interfaces (其他)/arkts-apis-image-i.md#positionarea7>).pixels缓冲区中，该区域由[PositionArea](<../Interfaces (其他)/arkts-apis-image-i.md#positionarea7>).region指定。使用Promise异步回调。
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| area | [PositionArea](<../Interfaces (其他)/arkts-apis-image-i.md#positionarea7>) | 是 | 区域大小，根据区域读取。 |
...
固定按照BGRA\_8888格式，读取PixelMap指定区域内的图像像素数据，并写入[PositionArea](<../Interfaces (其他)/arkts-apis-image-i.md#positionarea7>).pixels缓冲区中，该区域由[PositionArea](<../Interfaces (其他)/arkts-apis-image-i.md#positionarea7>).region指定，使用callback形式返回。
...
固定按照BGRA\_8888格式，读取PixelMap指定区域内的图像像素数据，并写入[PositionArea](<../Interfaces (其他)/arkts-apis-image-i.md#positionarea7>).pixels缓冲区中，该区域由[PositionArea](<../Interfaces (其他)/arkts-apis-image-i.md#positionarea7>).region指定，同步返回结果。
...
固定按照BGRA\_8888格式，读取[PositionArea](<../Interfaces (其他)/arkts-apis-image-i.md#positionarea7>).pixels缓冲区中的图像像素数据，并写入PixelMap指定区域内，该区域由[PositionArea](<../Interfaces (其他)/arkts-apis-image-i.md#positionarea7>).region指定。使用Promise异步回调。
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| area | [PositionArea](<../Interfaces (其他)/arkts-apis-image-i.md#positionarea7>) | 是 | 区域，根据区域写入。 |
...
固定按照BGRA\_8888格式，读取[PositionArea](<../Interfaces (其他)/arkts-apis-image-i.md#positionarea7>).pixels缓冲区中的图像像素数据，并写入PixelMap指定区域内，该区域由[PositionArea](<../Interfaces (其他)/arkts-apis-image-i.md#positionarea7>).region指定，使用callback形式返回。
...
固定按照BGRA\_8888格式，读取[PositionArea](<../Interfaces (其他)/arkts-apis-image-i.md#positionarea7>).pixels缓冲区中的图像像素数据，并写入PixelMap指定区域内，该区域由[PositionArea](<../Interfaces (其他)/arkts-apis-image-i.md#positionarea7>).region指定，同步返回结果。
...
| 类型 | 说明 |
| --- | --- |
| Promise<[ImageInfo](<../Interfaces (其他)/arkts-apis-image-i.md#imageinfo>)> | Promise对象，返回图像像素信息。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| callback | AsyncCallback<[ImageInfo](<../Interfaces (其他)/arkts-apis-image-i.md#imageinfo>)> | 是 | 回调函数。当获取图像像素信息成功，err为undefined，data为获取到的图像像素信息；否则为错误对象。 |
...
| 类型 | 说明 |
| --- | --- |
| [ImageInfo](<../Interfaces (其他)/arkts-apis-image-i.md#imageinfo>) | 图像像素信息。 |
...
| x | number | 是 | 宽度的缩放倍数。 |
| y | number | 是 | 高度的缩放倍数。 |
| level | [AntiAliasingLevel](../Enums/arkts-apis-image-e.md#antialiasinglevel12) | 是 | 采用的缩放算法。 |
...
| x | number | 是 | 宽度的缩放倍数。 |
| y | number | 是 | 高度的缩放倍数。 |
| level | [AntiAliasingLevel](../Enums/arkts-apis-image-e.md#antialiasinglevel12) | 否 | 采用的缩放算法，默认值为AntiAliasingLevel.NONE。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| region | [Region](<../Interfaces (其他)/arkts-apis-image-i.md#region8>) | 是 | 裁剪的区域。取值范围不能超过图片的宽和高（单位：像素（px））。 |
| x | number | 是 | 宽度的缩放倍数。不能为0。 |
| y | number | 是 | 高度的缩放倍数。不能为0。 |
| level | [AntiAliasingLevel](../Enums/arkts-apis-image-e.md#antialiasinglevel12) | 否 | 采用的缩放算法。默认值是AntiAliasingLevel.NONE。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| region | [Region](<../Interfaces (其他)/arkts-apis-image-i.md#region8>) | 是 | 裁剪的尺寸。取值范围不能超过图片的宽高。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| key | [HdrMetadataKey](../Enums/arkts-apis-image-e.md#hdrmetadatakey12) | 是 | HDR元数据的关键字，可用于查询对应值。 |

**返回值：**

| 类型 | 说明 |
| --- | --- |
| [HdrMetadataValue](../Types/arkts-apis-image-t.md#hdrmetadatavalue12) | 返回元数据的值。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| key | [HdrMetadataKey](../Enums/arkts-apis-image-e.md#hdrmetadatakey12) | 是 | HDR元数据的关键字，用于设置对应值。 |
| value | [HdrMetadataValue](../Types/arkts-apis-image-t.md#hdrmetadatavalue12) | 是 | 元数据的值。 |
...
创建DMA\_ALLOC内存的PixelMap方法请参考: [系统默认的内存分配方式](<../../../../../harmonyos-guides/媒体/Image Kit（图片处理服务）/图片开发指导(ArkTS)/图片解码/图片解码内存优化(ArkTS)/image-allocator-type.md#系统默认的内存分配方式>)。
...
5. @Concurrent
6. // 子线程方法。
7. async function loadPixelMap(rawFileDescriptor: number): Promise<PixelMap> {
8. // 创建imageSource。
9. const imageSource = image.createImageSource(rawFileDescriptor);
10. // 创建pixelMap。
11. const pixelMap = imageSource.createPixelMapSync();
12. // 释放imageSource。
13. imageSource.release();
14. // 使pixelMap在跨线程传输完成后，断开原线程的引用。
15. pixelMap.setTransferDetached(true);
16. // 返回pixelMap给主线程。
17. return pixelMap;
18. }
...
21. @Component
22. struct Demo {
23. @State pixelMap: PixelMap | undefined = undefined;
24. // 主线程方法。
25. private loadImageFromThread(): void {
26. let context = this.getUIContext().getHostContext() as common.UIAbilityContext;
27. const resourceMgr = context.resourceManager;
28. // 此处‘example.jpg’仅作示例，请开发者自行替换，否则imageSource创建失败会导致后续无法正常执行。
29. resourceMgr.getRawFd('example.jpg').then(rawFileDescriptor => {
30. taskpool.execute(loadPixelMap, rawFileDescriptor).then(pixelMap => {
31. if (pixelMap) {
32. this.pixelMap = pixelMap as PixelMap;
33. console.info('Succeeded in creating pixelMap.');
...
11. console.info('marshalling');
12. return true;
13. }
14. unmarshalling(messageSequence : rpc.MessageSequence) {
15. image.createPixelMap(new ArrayBuffer(96), {size: { height:4, width: 6}}).then((pixelParcel: image.PixelMap) => {
16. pixelParcel.unmarshalling(messageSequence).then(async (pixelMap: image.PixelMap) => {
17. this.pixel_map = pixelMap;
18. pixelMap.getImageInfo().then((imageInfo: image.ImageInfo) => {
19. console.info(`unmarshalling information h: ${imageInfo.size.height} w: ${imageInfo.size.width}`);
20. })
21. })
22. });
23. return true;
...
33. editable: true,
34. pixelFormat: image.PixelMapFormat.BGRA_8888,
35. size: { height: 4, width: 6 },
36. alphaType: image.AlphaType.UNPREMUL
37. }
38. let pixelMap: image.PixelMap | undefined = undefined;
39. await image.createPixelMap(color, opts).then((srcPixelMap: image.PixelMap) => {
...
从MessageSequence中获取PixelMap，如需使用同步方式创建PixelMap可使用：[createPixelMapFromParcel](../Functions/arkts-apis-image-f.md#imagecreatepixelmapfromparcel11)。
...
24. }
25. }
26. async function Unmarshalling() {
27. const color: ArrayBuffer = new ArrayBuffer(96);
28. let bufferArr: Uint8Array = new Uint8Array(color);
29. for (let i = 0; i < bufferArr.length; i++) {
30. bufferArr[i] = 0x80;
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| targetPixelFormat | [PixelMapFormat](../Enums/arkts-apis-image-e.md#pixelmapformat7) | 是 | 目标像素格式，用于YUV和RGB类型互转，或者ASTC\_4x4类型转为RGBA\_8888类型。目前仅支持NV12/NV21与RGB888/RGBA8888/RGB565/BGRA8888/RGBAF16互转，YCRCB\_P010/YCBCR\_P010与RGBA1010102互转，ASTC\_4x4转为RGBA\_8888。 |
...
```
1. async function IsReleased(pixelMap: PixelMap) { // 未释放的PixelMap。
