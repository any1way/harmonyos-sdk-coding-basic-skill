...
scraped_at: 2026-06-11T16:31:18+08:00
doc_updated_at: 2026-06-09
content_hash: sha256:867909f04f0d3dc2d9b17e55f4f661f968f069b3aae2d2f3e734885ad2c2ee88
---
说明
...
由于图片占用内存较大，所以当Picture对象使用完成后，应主动调用[release](<../Interface (Picture)/arkts-apis-image-picture.md#release13>)方法及时释放内存。释放时应确保该对象的所有异步方法均执行完成，且后续不再使用该对象。
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| mainPixelmap | [PixelMap](<../Interface (PixelMap)/arkts-apis-image-pixelmap.md>) | 是 | 主图的pixelmap。 |

**返回值：**

| 类型 | 说明 |
| --- | --- |
| [Picture](<../Interface (Picture)/arkts-apis-image-picture.md>) | 返回Picture对象。 |
...
11. console.info('Succeeded in creating picture');
12. } else {
13. console.error('Failed to create picture');
14. }
15. }
```
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| sequence | [rpc.MessageSequence](<../../../../IPC Kit（进程间通信服务）/ArkTS API/@ohos.rpc (RPC通信)/js-apis-rpc.md#messagesequence9>) | 是 | 保存有Picture信息的MessageSequence。 |
...
12. console.info('Succeeded in marshalling.');
13. return true;
14. } else {
15. console.error('Failed to marshall.');
16. return false;
17. }
18. }
...
由于图片占用内存较大，所以当PixelMap对象使用完成后，应主动调用[release](<../Interface (PixelMap)/arkts-apis-image-pixelmap.md#release7>)方法及时释放内存。释放时应确保该对象的所有异步方法均执行完成，且后续不再使用该对象。
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| colors | ArrayBuffer | 是 | 图像像素数据的缓冲区，用于初始化PixelMap的像素。初始化前，缓冲区中的像素格式需要由[InitializationOptions](<../Interfaces (其他)/arkts-apis-image-i.md#initializationoptions8>).srcPixelFormat指定，否则默认缓冲区的像素格式为BGRA\_8888。  **说明：** 图像像素数据的缓冲区长度：length = width \* height \* 单位像素字节数。 |
| options | [InitializationOptions](<../Interfaces (其他)/arkts-apis-image-i.md#initializationoptions8>) | 是 | 创建像素的属性，包括透明度，尺寸，缩略值，像素格式和是否可编辑。 |

**返回值：**

| 类型 | 说明 |
| --- | --- |
| Promise<[PixelMap](<../Interface (PixelMap)/arkts-apis-image-pixelmap.md>)> | Promise对象，返回PixelMap。  当创建的pixelMap大小超过原图大小时，返回原图pixelMap大小。 |
...
| colors | ArrayBuffer | 是 | 图像像素数据的缓冲区，用于初始化PixelMap的像素。初始化前，缓冲区中的像素格式需要由[InitializationOptions](<../Interfaces (其他)/arkts-apis-image-i.md#initializationoptions8>).srcPixelFormat指定，否则默认缓冲区的像素格式为BGRA\_8888。  **说明：** 图像像素数据的缓冲区长度：length = width \* height \* 单位像素字节数。 |
| options | [InitializationOptions](<../Interfaces (其他)/arkts-apis-image-i.md#initializationoptions8>) | 是 | 创建像素的属性，包括透明度、尺寸、缩略值、像素格式和是否可编辑。 |
| callback | AsyncCallback<[PixelMap](<../Interface (PixelMap)/arkts-apis-image-pixelmap.md>)> | 是 | 回调函数，当创建PixelMap成功，err为undefined，data为获取到的PixelMap对象；否则为错误对象。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| colors | ArrayBuffer | 是 | 图像像素数据的缓冲区，用于初始化PixelMap的像素。初始化前，必须通过[InitializationOptions](<../Interfaces (其他)/arkts-apis-image-i.md#initializationoptions8>).srcPixelFormat指定缓冲区中的像素格式。  **说明：** 图像像素数据的缓冲区长度：length = width \* height \* 单位像素字节数。 |
| param | [InitializationOptions](<../Interfaces (其他)/arkts-apis-image-i.md#initializationoptions8>) | 是 | 创建像素的属性，包括透明度、尺寸、缩略值、像素格式和是否可编辑。 |
| allocatorType | [AllocatorType](../Enums/arkts-apis-image-e.md#allocatortype15) | 否 | 指定创建pixelmap的内存类型，默认内存类型是AllocatorType.AUTO。  1. image.AllocatorType.AUTO：不支持该内存类型的格式有UNKNOWN、YCBCR\_P010、YCRCB\_P010和ASTC\_4x4。RGBA\_1010102默认申请DMA内存。其他格式（RGB\_565、RGBA\_8888、BGRA\_8888和RGBAF\_16）尺寸大于512\*512默认申请DMA内存，否则申请共享内存。  2. image.AllocatorType.DMA：RGBA\_1010102、RGB\_565、RGBA\_8888、BGRA\_8888和RGBAF\_16支持DMA内存类型，其余格式不支持。  3. image.AllocatorType.SHARED：UNKNOWN、RGBA\_1010102、YCBCR\_P010、YCRCB\_P010和ASTC\_4x4不支持共享内存，其余格式支持。 |

**返回值：**

| 类型 | 说明 |
| --- | --- |
| Promise<[PixelMap](<../Interface (PixelMap)/arkts-apis-image-pixelmap.md>)> | Promise对象，返回PixelMap。 |
...
| 类型 | 说明 |
| --- | --- |
| [PixelMap](<../Interface (PixelMap)/arkts-apis-image-pixelmap.md>) | 成功同步返回PixelMap对象，失败抛出异常。 |
...
52. if (newPixelmap != undefined ) {
53. console.info('Get PixelMap successfully.')
54. }
55. }
56. }
```
...
根据Surface ID和区域信息创建一个PixelMap对象。该区域的大小由[Region](<../Interfaces (其他)/arkts-apis-image-i.md#region8>).size指定。使用Promise异步回调。
...
| region | [Region](<../Interfaces (其他)/arkts-apis-image-i.md#region8>) | 是 | 截取的画面区域。仅支持从画面左上角开始截取部分或整个画面，即Region中的x和y必须为0，Region.size中width和height的取值范围分别为[1, 预览流宽度]和[1, 预览流高度]。如需截取任意区域，可先使用[image.createPixelMapFromSurface](arkts-apis-image-f.md#imagecreatepixelmapfromsurface15)获取整个画面，再使用[crop](<../Interface (PixelMap)/arkts-apis-image-pixelmap.md#crop9>)截取所需区域。 |
...
以同步方式，根据Surface ID和区域信息创建一个PixelMap对象。该区域的大小由[Region](<../Interfaces (其他)/arkts-apis-image-i.md#region8>).size指定。
...
| --- | --- | --- | --- |
| surfaceId | string | 是 | 对应Surface的ID，可通过预览组件获取，如[XComponent](../../../../ArkUI（方舟UI框架）/ArkTS组件/渲染绘制/XComponent/ts-basic-components-xcomponent.md)组件。 |
| region | [Region](<../Interfaces (其他)/arkts-apis-image-i.md#region8>) | 是 | 截取的画面区域。仅支持从画面左上角开始截取部分或整个画面，即Region中的x和y必须为0，Region.size中width和height的取值范围分别为[1, 预览流宽度]和[1, 预览流高度]。如需截取任意区域，可先使用[image.createPixelMapFromSurfaceSync](arkts-apis-image-f.md#imagecreatepixelmapfromsurfacesync15)获取整个画面，再使用[cropSync](<../Interface (PixelMap)/arkts-apis-image-pixelmap.md#cropsync12>)截取所需区域。 |
...
3. console.info('Succeeded in creating pixelMap.');
4. }).catch((e: Error) => {
...
| 类型 | 说明 |
| --- | --- |
| [PixelMap](<../Interface (PixelMap)/arkts-apis-image-pixelmap.md>) | 成功会同步返回PixelMap对象，失败则抛出异常。 |
...
6. console.info('Succeeded in creating pixelMap.');
7. } catch (e) {
...
```
1. function CreatePixelMapUsingAllocatorSync() {
2. const color: ArrayBuffer = new ArrayBuffer(96); // 96为需要创建的像素buffer大小，取值为：height * width *4。
3. let opts: image.InitializationOptions = {
4. size: { height: 4, width: 6 },
5. srcPixelFormat: image.PixelMapFormat.RGBA_8888, // 缓冲区中的源像素数据的像素格式。
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| src | [PixelMap](<../Interface (PixelMap)/arkts-apis-image-pixelmap.md>) | 是 | 源PixelMap对象。 |
| dst | [PixelMap](<../Interface (PixelMap)/arkts-apis-image-pixelmap.md>) | 是 | 目标PixelMap对象。 |
...
由于图片占用内存较大，所以当ImageSource实例使用完成后，应主动调用[release](<../Interface (ImageSource)/arkts-apis-image-imagesource.md#release>)方法及时释放内存。释放时应确保该实例的所有异步方法均执行完成，且后续不再使用该实例。
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| uri | string | 是 | 图片路径，当前仅支持应用沙箱路径。  当前支持格式有：.jpg .png .gif .bmp .webp .dng .heic12+ .wbmp23+ .heifs23+ .tiff23+ .svg10+（可参考[SVG标签说明](arkts-apis-image-f.md#svg标签说明)） .ico11+。部分格式的解码能力依赖于具体的设备硬件，建议在调用前使用[image.getImageSourceSupportedFormats20+](arkts-apis-image-f.md#imagegetimagesourcesupportedformats20)接口，动态查询当前设备上的解码能力。 |

**返回值：**

| 类型 | 说明 |
| --- | --- |
| [ImageSource](<../Interface (ImageSource)/arkts-apis-image-imagesource.md>) | 返回ImageSource类实例，失败时返回undefined。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| uri | string | 是 | 图片路径，当前仅支持应用沙箱路径（可参考[使用说明](<../../../../Core File Kit（文件基础服务）/ArkTS API/@ohos.file.fs (文件管理)/js-apis-file-fs.md#使用说明>)）。  当前支持格式有：JPEG、PNG、GIF、BMP、WebP、DNG、HEIC12+、WBMP23+、HEIFS23+、TIFF23+、SVG10+（可参考[SVG标签说明](arkts-apis-image-f.md#svg标签说明)）、ICO11+。部分格式的解码能力依赖于具体的设备硬件，建议在调用前使用[image.getImageSourceSupportedFormats20+](arkts-apis-image-f.md#imagegetimagesourcesupportedformats20)接口，动态查询当前设备上的解码能力。 |
| options | [SourceOptions](<../Interfaces (其他)/arkts-apis-image-i.md#sourceoptions9>) | 是 | 图片属性，包括图片像素密度、像素格式和图片尺寸。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| rawfile | [resourceManager.RawFileDescriptor](<../../../../Localization Kit（本地化开发服务）/ArkTS API/@ohos.resourceManager (资源管理)/js-apis-resource-manager.md#rawfiledescriptor9>) | 是 | 图像资源文件的RawFileDescriptor。 |
| options | [SourceOptions](<../Interfaces (其他)/arkts-apis-image-i.md#sourceoptions9>) | 否 | 图片属性，包括图片像素密度、像素格式和图片尺寸。 |
...
* 获取图片信息：指定序号-[getImageInfo](<../Interface (ImageSource)/arkts-apis-image-imagesource.md#getimageinfo>)、直接获取-[getImageInfo](<../Interface (ImageSource)/arkts-apis-image-imagesource.md#getimageinfo-1>)
* 获取图片中给定索引处图像的指定属性键的值：[getImageProperty](<../Interface (ImageSource)/arkts-apis-image-imagesource.md#getimageproperty11>)
* 批量获取图片中的指定属性键的值：[getImageProperties](<../Interface (ImageSource)/arkts-apis-image-imagesource.md#getimageproperties12>)
* 更新增量数据：[updateData](<../Interface (ImageSource)/arkts-apis-image-imagesource.md#updatedata9>)
* 创建PixelMap对象：通过图片解码参数创建-[createPixelMap](<../Interface (ImageSource)/arkts-apis-image-imagesource.md#createpixelmap7>)、通过默认参数创建-[createPixelMap](<../Interface (ImageSource)/arkts-apis-image-imagesource.md#createpixelmap7-1>) 、通过图片解码参数-[createPixelMap](<../Interface (ImageSource)/arkts-apis-image-imagesource.md#createpixelmap7-2>)
* 释放ImageSource实例：[release](<../Interface (ImageSource)/arkts-apis-image-imagesource.md#release>)
...
| 类型 | 说明 |
| --- | --- |
| [ImageSource](<../Interface (ImageSource)/arkts-apis-image-imagesource.md>) | 返回ImageSource，失败时返回undefined。 |
...
由于图片占用内存较大，所以当ImagePacker实例使用完成后，应主动调用[release](<../Interface (ImagePacker)/arkts-apis-image-imagepacker.md#release>)方法及时释放内存。释放时应确保该实例的所有异步方法均执行完成，且后续不再使用该实例。
...
| 类型 | 说明 |
| --- | --- |
| [ImagePacker](<../Interface (ImagePacker)/arkts-apis-image-imagepacker.md>) | 返回ImagePacker实例。 |
...
由于图片占用内存较大，所以当AuxiliaryPicture实例使用完成后，应主动调用[release](<../Interface (AuxiliaryPicture)/arkts-apis-image-auxiliarypicture.md#release13>)方法及时释放内存。释放时应确保该实例的所有异步方法均执行完成，且后续不再使用该实例。
...
| size | [Size](<../Interfaces (其他)/arkts-apis-image-i.md#size>) | 是 | 辅助图的尺寸。单位：px。 |
| type | [AuxiliaryPictureType](../Enums/arkts-apis-image-e.md#auxiliarypicturetype13) | 是 | 辅助图类型。 |

**返回值：**

| 类型 | 说明 |
| --- | --- |
| [AuxiliaryPicture](<../Interface (AuxiliaryPicture)/arkts-apis-image-auxiliarypicture.md>) | 如果操作成功，则返回AuxiliaryPicture实例。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| auxiliaryPictureInfo | [AuxiliaryPictureInfo](<../Interfaces (其他)/arkts-apis-image-i.md#auxiliarypictureinfo13>) | 是 | 辅助图图像信息。  - 输入的ArrayBuffer的pixelFormat和最终创建出的辅助图的实际pixelFormat需与auxiliaryPictureInfo中指定的pixelFormat保持一致。  - 当AuxiliaryPictureType为GAINMAP时，AllocatorType仅支持传入AUTO/DMA。  - 当传入SHARE\_MEMORY时，返回错误码7600205。 |
| allocatorType | [AllocatorType](../Enums/arkts-apis-image-e.md#allocatortype15) | 否 | 图像解码的内存类型，AUTO及默认情况下按照DMA处理。 |
...
通过图片大小、图片格式、容量创建ImageReceiver实例。ImageReceiver做为图片的接收方、消费者，它的参数属性实际上不会对接收到的图片产生影响。图片属性的配置应在发送方、生产者进行，如相机预览流[createPreviewOutput](<../../../../Camera Kit（相机服务）/ArkTS API/@ohos.multimedia.camera (相机管理)/Interface (CameraManager)/arkts-apis-camera-cameramanager.md#createpreviewoutput>)。

由于图片占用内存较大，所以当ImageReceiver实例使用完成后，应主动调用[release](<../Interface (ImageReceiver)/arkts-apis-image-imagereceiver.md#release9>)方法及时释放内存。释放时应确保该实例的所有异步方法均执行完成，且后续不再使用该实例。
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| size | [Size](<../Interfaces (其他)/arkts-apis-image-i.md#size>) | 是 | 图像的默认大小。该参数不会影响接收到的图片大小，实际返回大小由生产者决定，如相机。 |
| format | [ImageFormat](../Enums/arkts-apis-image-e.md#imageformat9) | 是 | 图像格式，取值为[ImageFormat](../Enums/arkts-apis-image-e.md#imageformat9)常量（目前仅支持 ImageFormat:JPEG，实际返回格式由生产者决定，如相机）。 |
...
| 类型 | 说明 |
| --- | --- |
| [ImageReceiver](<../Interface (ImageReceiver)/arkts-apis-image-imagereceiver.md>) | 如果操作成功，则返回ImageReceiver实例。 |
...
通过ImageReceiverOptions创建ImageReceiver实例。ImageReceiver做为图片的接收方、消费者，其参数属性实际上不会对接收到的图片产生影响。图片属性的配置应在发送方、生产者进行，如相机预览流[createPreviewOutput](<../../../../Camera Kit（相机服务）/ArkTS API/@ohos.multimedia.camera (相机管理)/Interface (CameraManager)/arkts-apis-camera-cameramanager.md#createpreviewoutput>)。
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| options | [ImageReceiverOptions](<../Interfaces (其他)/arkts-apis-image-i.md#imagereceiveroptions23>) | 否 | 创建ImageReceiver的属性，包括图片的默认大小和同时访问的最大图片数。  未传入options时，默认的size为1920\*1080，单位为像素（px），表示期望接收宽为1920px，高为1080px的图片。  未传入options时，默认的capacity为3，表示期望同时最多有3张图片等待读取。 |

**返回值：**

| 类型 | 说明 |
| --- | --- |
| [ImageReceiver](<../Interface (ImageReceiver)/arkts-apis-image-imagereceiver.md>) | undefined | 操作成功时返回ImageReceiver实例，否则返回undefined。 |
...
由于图片占用内存较大，所以当ImageCreator实例使用完成后，应主动调用[release](<../Interface (ImageCreator)/arkts-apis-image-imagecreator.md#release9>)方法及时释放内存。释放时应确保该实例的所有异步方法均执行完成，且后续不再使用该实例。
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| size | [Size](<../Interfaces (其他)/arkts-apis-image-i.md#size>) | 是 | 图像的默认大小。 |
| format | [ImageFormat](../Enums/arkts-apis-image-e.md#imageformat9) | 是 | 图像格式，如YCBCR\_422\_SP，JPEG。 |
| capacity | number | 是 | 同时访问的最大图像数。该参数仅作为期望值，实际capacity由设备硬件决定。 |
...
| 类型 | 说明 |
| --- | --- |
| [ImageCreator](<../Interface (ImageCreator)/arkts-apis-image-imagecreator.md>) | 如果操作成功，则返回ImageCreator实例。 |

**错误码：**

以下错误码的详细介绍请参见[通用错误码说明文档](../../../../通用错误码/errorcode-universal.md)和。
...
通过宽、高、图片格式、容量创建ImageReceiver实例。ImageReceiver做为图片的接收方、消费者，它的参数属性实际上不会对接收到的图片产生影响。图片属性的配置应在发送方、生产者进行，如相机预览流[createPreviewOutput](<../../../../Camera Kit（相机服务）/ArkTS API/@ohos.multimedia.camera (相机管理)/Interface (CameraManager)/arkts-apis-camera-cameramanager.md#createpreviewoutput>)。
...
| format | number | 是 | 图像格式，取值为[ImageFormat](../Enums/arkts-apis-image-e.md#imageformat9)常量（目前仅支持 ImageFormat:JPEG，实际返回格式由生产者决定，如相机）。 |
| capacity | number | 是 | 同时访问的最大图像数。该参数仅作为期望值，实际capacity由设备硬件决定。 |
