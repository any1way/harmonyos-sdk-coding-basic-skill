...
scraped_at: 2026-06-11T16:07:17+08:00
doc_updated_at: 2026-04-29
content_hash: sha256:2fdcf1933a443762de8018174cee09d5761174d4f98280e77cc86225284dcf12
---
...
| 参数名 | 类型 | 必填 | 装饰器类型 | 说明 |
| --- | --- | --- | --- | --- |
| customItemBuilder | [CustomBuilder](../../../ArkUI（方舟UI框架）/ArkTS组件/公共定义/基础类型定义/ts-types.md#custombuilder8) | 否 | @BuilderParam | 自定义列表卡片项内容。 |
| hdsListItemCard | [HdsListItemCardOptions](../HdsListItemCard/ui-design-hdslistitemcard.md#hdslistitemcardoptions) | 否 | - | 列表卡片项内容。 |
| swipeActionOptions | [HdsSwipeActionOptions](ui-design-hdslistitem.md#hdsswipeactionoptions) | [SwipeActionOptions](../../../ArkUI（方舟UI框架）/ArkTS组件/滚动与滑动/ListItem/ts-container-listitem.md#swipeactionoptions9对象说明) | 否 | - | 动效横滑内容展示。  HdsSwipeActionOptions是HdsListItem封装后的横滑动效类型，SwipeActionOptions支持用户自定义使用ListItem的横滑动效类型。 |
| listItemModifier | [ListItemModifier](../../../ArkUI（方舟UI框架）/ArkTS组件/通用属性/动态属性与自定义/动态属性设置/ts-universal-attributes-attribute-modifier.md#自定义modifier) | 否 | - | ListItem属性样式修改器。  **起始版本：** 6.0.1(21) |
| menuStyle | [MenuStyle](ui-design-hdslistitem.md#menustyle) | 否 | @Prop | ListItem预览菜单样式。  **起始版本：** 6.1.0(23) |
| menuBuilder | [CustomBuilder](../../../ArkUI（方舟UI框架）/ArkTS组件/公共定义/基础类型定义/ts-types.md#custombuilder8) | 否 | @BuilderParam | 自定义弹出菜单内容。  **起始版本：** 6.1.0(23) |
...
| accessibilityText | [ResourceStr](../../../ArkUI（方舟UI框架）/ArkTS组件/公共定义/基础类型定义/ts-types.md#resourcestr) | 否 | 是 | 图标的无障碍文本属性。  当组件不包含文本属性时，屏幕朗读选中此组件时不播报，使用者无法清楚地知道当前选中了什么组件。为了解决此场景，开发人员可为不包含文字信息的组件设置无障碍文本，当屏幕朗读选中此组件时播报无障碍文本的内容，帮助屏幕朗读的使用者清楚地知道自己选中了什么组件。  默认值：""。 |
| accessibilityDescription | [ResourceStr](../../../ArkUI（方舟UI框架）/ArkTS组件/公共定义/基础类型定义/ts-types.md#resourcestr) | 否 | 是 | 图标的无障碍描述。  此描述用于向用户详细解释当前组件，开发人员应为组件的这一属性提供较为详尽的文本说明，以协助用户理解即将执行的操作及其可能产生的后果。特别是当这些后果无法仅从组件的属性和无障碍文本中直接获知时。如果组件同时具备文本属性和无障碍说明属性，当组件被选中时，系统将首先播报组件的文本属性，随后播报无障碍说明属性的内容。  默认值：“单指双击即可执行”。 |
...
| accessibilityRole | [AccessibilityRoleType](../../../ArkUI（方舟UI框架）/ArkTS组件/通用属性/无障碍属性/ts-universal-attributes-accessibility.md#accessibilityroletype18枚举说明) | 否 | 是 | 图标的无障碍组件类型。  可以根据应用诉求，修改组件类型，用于控制无障碍模式下对组件的朗读方式和朗读内容。  默认值：AccessibilityRoleType.ROLE\_NONE。  **起始版本：** 6.1.0(23) |
...
| backgroundColor | [ResourceColor](../../../ArkUI（方舟UI框架）/ArkTS组件/公共定义/基础类型定义/ts-types.md#resourcecolor) | 否 | 是 | 图标背景色。 |
...
| 名称 | 类型 | 只读 | 可选 | 说明 |
| --- | --- | --- | --- | --- |
| backgroundColor | [ResourceColor](../../../ArkUI（方舟UI框架）/ArkTS组件/公共定义/基础类型定义/ts-types.md#resourcecolor) | 否 | 是 | 删除按钮图标背景色。 |
| iconColor | [ResourceColor](../../../ArkUI（方舟UI框架）/ArkTS组件/公共定义/基础类型定义/ts-types.md#resourcecolor) | 否 | 是 | 删除按钮图标颜色。 |
...
| [ImageOptions](../HdsListItemCard/ui-design-hdslistitemcard.md#accessibilitygroupoptions) | image资源类型。 |
...
| 名称 | 类型 | 只读 | 可选 | 说明 |
| --- | --- | --- | --- | --- |
| responseType | [ResponseType](../../../ArkUI（方舟UI框架）/ArkTS组件/公共定义/枚举说明/ts-appendix-enums.md#responsetype8) | 否 | 是 | 预览菜单响应类型。 |
| menuOptions | [ContextMenuOptions](../../../ArkUI（方舟UI框架）/ArkTS组件/通用属性/弹窗控制/菜单控制/ts-universal-attributes-menu.md#contextmenuoptions10) | 否 | 是 | 预览菜单选项信息。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| state | [SwipeActionState](../../../ArkUI（方舟UI框架）/ArkTS组件/滚动与滑动/ListItem/ts-container-listitem.md#swipeactionstate11枚举说明) | 是 | 列表项滑动的状态。  - state为SwipeActionState.COLLAPSED时，表示收起状态，此时操作项处于隐藏状态。  - state为SwipeActionState.EXPANDED时，表示展开状态，此时操作项处于显示状态。  - state为SwipeActionState.ACTIONING时，表示长距离状态，当HdsListItem进入长距删除区后，删除HdsListItem的状态。 |
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/80/v3/-rl2H5O_RSyuy4KzvTL_qQ/zh-cn_image_0000002622700563.gif?HW-CC-KV=V1&HW-CC-Date=20260611T080716Z&HW-CC-Expire=86400&HW-CC-Sign=D02C5655D028A26430525852E4B96C92B0D79B77AEFCF498D99CD9EB85ADA319)
