...
scraped_at: 2026-06-01T16:32:16+08:00
doc_updated_at: 2026-05-26
content_hash: sha256:245ca7c8fcad6e7e59dbe01c511895a6a615ba3e83158d7ec35178374e8e06fd
---
...
| PHONE\_NUMBER | 1 | 表示用PhoneNumber来关联华为账号。  用户在登录成功后，如之前未对快速验证手机号进行授权，则会拉起手机号授权页面；如已授权，则返回对应数据。  该登录类型不需要实时验证华为账号的手机号码，响应数据包含openID、unionID、authorizationCode、idToken字段。  应用使用Authorization Code调用[获取用户级凭证接口](<../../REST API/开放接口调用凭证/获取用户级凭证/account-api-obtain-user-token.md#接口原型>)向华为账号服务器请求获取Access Token，再使用Access Token调用[获取华为账号用户信息接口](<../../REST API/获取用户信息/获取华为账号用户信息-获取手机号/account-api-get-user-info-get-phone.md#接口原型>)获取用户信息，从用户信息中获取用户手机号。 |
| REAL\_TIME\_PHONE\_NUMBER | 2 | 表示用PhoneNumber来关联华为账号。  用户每次在登录成功后，都会拉起实时验证手机号授权页面。  该登录类型会实时验证华为账号的手机号码，响应数据包含openID、unionID、authorizationCode、idToken字段。  应用使用Authorization Code调用[获取用户级凭证接口](<../../REST API/开放接口调用凭证/获取用户级凭证/account-api-obtain-user-token.md#接口原型>)向华为账号服务器请求获取Access Token，再使用Access Token调用[获取华为账号用户信息接口](<../../REST API/获取用户信息/获取华为账号用户信息-获取手机号/account-api-get-user-info-get-phone.md#接口原型>)获取用户信息，从用户信息中获取用户手机号。  **说明：** REAL\_TIME\_PHONE\_NUMBER暂不支持使用。 |
| QUICK\_LOGIN | 3 | 表示用PhoneNumber来关联华为账号。  该类型不支持Icon类型和图文类型的LoginWithHuaweiIDButton组件。  该登录类型需要通过[AuthorizationWithHuaweiIDRequest](<../../ArkTS API/authentication (华为账号应用统一认证服务)/account-api-authentication.md#authorizationwithhuaweiidrequest>)接口获取华为账号绑定的匿名手机号，如果未获取到华为账号绑定的匿名手机号，请使用其他登录类型。  该登录类型响应数据包含openID、unionID、authorizationCode、idToken字段。  应用使用Authorization Code调用[/oauth2/v6/quickLogin/getPhoneNumber接口](<../../REST API/获取用户信息/一键登录获取华为账号绑定号码和UnionIDOpenID/account-api-get-user-info-quicklogin-by-code.md#接口原型>)获取用户信息，从用户信息中获取用户手机号。  **起始版本：** 5.0.0(12)  **设备行为差异：** 该接口在Phone、PC/2in1、Tablet、TV中可正常调用（TV设备从5.1.1(19)版本开始支持），在其他设备类型中返回1001500003错误码。 |
...
| 名称 | 类型 | 只读 | 可选 | 说明 |
| --- | --- | --- | --- | --- |
| appIcon | [PixelMap](<../../../Image Kit（图片处理服务）/ArkTS API/@ohos.multimedia.image (图片处理)/Interface (PixelMap)/arkts-apis-image-pixelmap.md>) | [ResourceStr](../../../ArkUI（方舟UI框架）/ArkTS组件/公共定义/基础类型定义/ts-types.md#resourcestr) | [DrawableDescriptor](<../../../ArkUI（方舟UI框架）/ArkTS API/UI界面/@ohos.arkui.drawableDescriptor (DrawableDescriptor)/js-apis-arkui-drawabledescriptor.md#drawabledescriptor>) | 否 | 否 | 应用的图标。 |
| appName | [ResourceStr](../../../ArkUI（方舟UI框架）/ArkTS组件/公共定义/基础类型定义/ts-types.md#resourcestr) | 否 | 否 | 应用的名称，长度限制1-19个字符，字符超长和大字体下展示不全会截断，使用省略号填充。 |
| appDescription | [ResourceStr](../../../ArkUI（方舟UI框架）/ArkTS组件/公共定义/基础类型定义/ts-types.md#resourcestr) | 否 | 是 | 应用的详细描述，长度限制1-44个字符。  **说明：**  - 在4.1.0(11)版本，为必填参数。  - 从5.0.0(12)版本开始，为非必填参数。仅当登录类型为[LoginType.QUICK\_LOGIN](account-api-component-manager.md#logintype)时不需要设置该值。 |
...
| text | [ResourceStr](../../../ArkUI（方舟UI框架）/ArkTS组件/公共定义/基础类型定义/ts-types.md#resourcestr) | 否 | 否 | 在登录面板上显示的隐私文本内容，一般用于展示应用隐私协议和《华为账号用户认证协议》跳转链接。 |
...
| 名称 | 类型 | 只读 | 可选 | 说明 |
| --- | --- | --- | --- | --- |
| icon | [PixelMap](<../../../Image Kit（图片处理服务）/ArkTS API/@ohos.multimedia.image (图片处理)/Interface (PixelMap)/arkts-apis-image-pixelmap.md>) | [ResourceStr](../../../ArkUI（方舟UI框架）/ArkTS组件/公共定义/基础类型定义/ts-types.md#resourcestr) | [DrawableDescriptor](<../../../ArkUI（方舟UI框架）/ArkTS API/UI界面/@ohos.arkui.drawableDescriptor (DrawableDescriptor)/js-apis-arkui-drawabledescriptor.md#drawabledescriptor>) | 否 | 否 | 可选登录区域展示其他登录方式的Icon。根据[视觉规范](https://developer.huawei.com/consumer/cn/doc/design-guides/id-0000001880001344#section61791745172816)，Icon建议为半径18vp的圆形图片。 |
...
| 名称 | 类型 | 只读 | 可选 | 说明 |
| --- | --- | --- | --- | --- |
| text | [ResourceStr](../../../ArkUI（方舟UI框架）/ArkTS组件/公共定义/基础类型定义/ts-types.md#resourcestr) | 否 | 否 | 可选登录按钮的文字描述。 |
...
| anonymousPhoneNumber | string | 否 | 是 | 华为账号绑定的匿名手机号。当登录类型为[LoginType.QUICK\_LOGIN](account-api-component-manager.md#logintype)时需要设置该参数。可参考[华为账号一键登录客户端开发](<../../../../harmonyos-guides/应用服务/Account Kit（华为账号服务）/登录/华为账号一键登录（获取手机号和UnionIDOpenID）/account-phone-unionid-login.md#客户端开发>)获取。  **起始版本：** 5.0.0(12) |
...
| riskLevel | boolean | 否 | 是 | 是否需要获取华为账号用户风险等级。  仅登录类型为[LoginType.QUICK\_LOGIN](account-api-component-manager.md#logintype)时需要设置该参数。  true：需要[获取用户风险等级](<../../../../harmonyos-guides/应用服务/Account Kit（华为账号服务）/获取华为账号用户信息/获取风险等级/通过华为账号一键登录获取用户风险等级/account-get-risklevel-byquicklogin.md>)。  false：不获取用户风险等级。  默认值：false。  **起始版本：** 5.1.0(18) |
...
[LoginPanel](<../LoginPanel (华为账号Panel登录组件)/account-api-loginpanel.md>)组件控制器，用来注册组件内部的点击事件。
...
| 类型 | 说明 |
| --- | --- |
| [LoginPanelController](account-api-component-manager.md#loginpanelcontroller) | [LoginPanel](<../LoginPanel (华为账号Panel登录组件)/account-api-loginpanel.md>)组件控制器可以用来注册组件内部的点击事件或者调用组件提供的方法。 |

**错误码：**

以下错误码的详细介绍请参见[ArkTS错误码](../../ArkTS错误码/account-api-error-code.md)和[账号管理错误码](<../../../基础功能/Basic Services Kit（基础服务）/错误码/账号管理错误码/errorcode-account.md>)。
...
| [1005300001](../../ArkTS错误码/account-api-error-code.md#section1005300001-用户未同意协议) | The user did not accept the agreement. |
...
| 名称 | 类型 | 只读 | 可选 | 说明 |
| --- | --- | --- | --- | --- |
| style | [Style](account-api-component-manager.md#style) | 否 | 否 | [LoginWithHuaweiIDButton](<../LoginWithHuaweiIDButton (华为账号Button登录组件)/account-api-huawei-id-button.md>)组件的样式。 |
...
| BUTTON\_CUSTOM | 10 | 支持自定义背景颜色、文字颜色及圆角等效果的按钮，字体大小会随着按钮的宽高自适应，最小为9号，最大为16号。开发者可以通过[CustomButtonParams](account-api-component-manager.md#custombuttonparams)设置按钮自定义的参数，且需要自行适配深浅色模式。  根据开发场景从以下链接中选择其中一个规范查看。  请参见[【华为账号一键登录】按钮](https://developer.huawei.com/consumer/cn/doc/design-guides/id-0000001880001344#section41792374210)视觉规范。  请参见[【华为账号登录】按钮](https://developer.huawei.com/consumer/cn/doc/design-guides/id-0000001880001344#section2624430102713)视觉规范。  **起始版本：** 5.0.0(12)  **说明：** 此类型仅支持纯文本按钮。 |
...
| 名称 | 类型 | 只读 | 可选 | 说明 |
| --- | --- | --- | --- | --- |
| fontColor | [FontColor](account-api-component-manager.md#fontcolor) | 否 | 是 | [LoginWithHuaweiIDButton](<../LoginWithHuaweiIDButton (华为账号Button登录组件)/account-api-huawei-id-button.md>)按钮文字颜色。  默认值：[FontColor](account-api-component-manager.md#fontcolor).WHITE |
| backgroundColor | [ResourceColor](../../../ArkUI（方舟UI框架）/ArkTS组件/公共定义/基础类型定义/ts-types.md#resourcecolor) | 否 | 是 | [LoginWithHuaweiIDButton](<../LoginWithHuaweiIDButton (华为账号Button登录组件)/account-api-huawei-id-button.md>)按钮背景颜色。  默认值：Red |
...
| 名称 | 类型 | 只读 | 可选 | 说明 |
| --- | --- | --- | --- | --- |
| center | [[Length](../../../ArkUI（方舟UI框架）/ArkTS组件/公共定义/基础类型定义/ts-types.md#length), [Length](../../../ArkUI（方舟UI框架）/ArkTS组件/公共定义/基础类型定义/ts-types.md#length)] | 否 | 否 | 径向渐变的中心点，即相对于当前组件左上角的坐标。 |
| radius | number | string | 否 | 否 | 径向渐变的半径。  取值范围：[0,+∞)。  值小于0时，按0处理。 |
| colors | Array<[[ResourceColor](../../../ArkUI（方舟UI框架）/ArkTS组件/公共定义/基础类型定义/ts-types.md#resourcecolor), number]> | 否 | 否 | 指定某百分比位置处的渐变色颜色，设置非法颜色直接跳过。  不设置则无效果。 |
...
| 名称 | 类型 | 只读 | 可选 | 说明 |
| --- | --- | --- | --- | --- |
| center | [[Length](../../../ArkUI（方舟UI框架）/ArkTS组件/公共定义/基础类型定义/ts-types.md#length), [Length](../../../ArkUI（方舟UI框架）/ArkTS组件/公共定义/基础类型定义/ts-types.md#length)] | 否 | 否 | 角度渐变的中心点，即相对于当前组件左上角的坐标。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| value | [Length](../../../ArkUI（方舟UI框架）/ArkTS组件/公共定义/基础类型定义/ts-types.md#length) | 是 | 当按钮为图文按钮或文本按钮时，设置按钮文字的字号。根据[华为账号开放登录视觉规范](https://developer.huawei.com/consumer/cn/doc/design-guides/id-0000001880001344#section61791745172816)，设置范围建议在按钮高度的30%~50%之间。  如果没传此值，字号大小在9~16vp之间随按钮宽度自适应。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| value | [ResourceColor](../../../ArkUI（方舟UI框架）/ArkTS组件/公共定义/基础类型定义/ts-types.md#resourcecolor) | [FontColor](account-api-component-manager.md#fontcolor) | 是 | 当按钮类型为[BUTTON\_CUSTOM](account-api-component-manager.md#style)时，此参数可设置按钮文字颜色。当已经设置[CustomButtonParams](account-api-component-manager.md#custombuttonparams)的fontColor属性时，优先取此参数值。  不设置则为黑色。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| value | [ResourceStr](../../../ArkUI（方舟UI框架）/ArkTS组件/公共定义/基础类型定义/ts-types.md#resourcestr) | 是 | 当按钮类型为[BUTTON\_CUSTOM](account-api-component-manager.md#style)时，此参数可设置按钮文字字体。  默认字体'HarmonyOS Sans'。  应用当前支持'HarmonyOS Sans'字体和[注册自定义字体](<../../../ArkUI（方舟UI框架）/ArkTS API/UI界面/@ohos.font (注册自定义字体)/js-apis-font.md>)。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| value | [ResourceColor](../../../ArkUI（方舟UI框架）/ArkTS组件/公共定义/基础类型定义/ts-types.md#resourcecolor) | 是 | 当按钮类型为[BUTTON\_CUSTOM](account-api-component-manager.md#style)时，此参数可设置按钮背景颜色。  当已经设置[CustomButtonParams](account-api-component-manager.md#custombuttonparams)的backgroundColor属性时，优先取此参数值。  不设置则为 Red 红色背景。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| value | [BackgroundEffectOptions](../../../ArkUI（方舟UI框架）/ArkTS组件/通用属性/基础属性/背景设置/ts-universal-attributes-background.md#backgroundeffectoptions11) | 是 | 当按钮类型为[BUTTON\_CUSTOM](account-api-component-manager.md#style)时，此参数可设置按钮背景属性，包括饱和度，亮度，颜色。  不设置则无效果。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| value | [BackgroundBrightnessOptions](../../../ArkUI（方舟UI框架）/ArkTS组件/通用属性/基础属性/背景设置/ts-universal-attributes-background.md#backgroundbrightnessoptions12对象说明) | 是 | 当按钮类型为[BUTTON\_CUSTOM](account-api-component-manager.md#style)时，此参数可设置按钮背景提亮效果，包括：亮度变化速率，提亮程度。  不设置则无效果。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| value | [LinearGradient](../../../ArkUI（方舟UI框架）/ArkTS组件/通用属性/布局与边框/图片边框设置/ts-universal-attributes-border-image.md#lineargradient) | 是 | 当按钮类型为[BUTTON\_CUSTOM](account-api-component-manager.md#style)时，此参数可设置按钮背景颜色线性渐变效果，不设置则无效果。  **说明：**  使用此属性时，按钮不能有背景色，否则按钮边缘会出现锯齿毛边，需把按钮背景色设置为透明色，如backgroundColor(Color.Transparent)。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| value | [ShadowOptions](../../../ArkUI（方舟UI框架）/ArkTS组件/通用属性/视效与模糊/图像效果/ts-universal-attributes-image-effect.md#shadowoptions对象说明) | [ShadowStyle](../../../ArkUI（方舟UI框架）/ArkTS组件/通用属性/视效与模糊/图像效果/ts-universal-attributes-image-effect.md#shadowstyle10枚举说明) | 是 | 当按钮类型为[BUTTON\_CUSTOM](account-api-component-manager.md#style)时，此参数可设置按钮阴影效果，不设置则无效果。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| value | number | [Resource](../../../ArkUI（方舟UI框架）/ArkTS组件/公共定义/基础类型定义/ts-types.md#resource) | 是 | 元素的不透明度，取值范围为0~1，1表示不透明，0表示完全透明，达到隐藏组件效果，但是在布局中占位。  默认值：1。  当按钮类型为[BUTTON\_CUSTOM](account-api-component-manager.md#style)时，此参数可设置按钮的不透明度。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| value | [BorderOptions](../../../ArkUI（方舟UI框架）/ArkTS组件/公共定义/基础类型定义/ts-types.md#borderoptions) | 是 | 当按钮类型为[BUTTON\_CUSTOM](account-api-component-manager.md#style)时，此参数可设置按钮的边框样式，不设置则无效果。  **说明：** 设置内边框需要预留充足的空间展示按钮，请确保按钮父组件宽高大于按钮本身。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| value | [BorderImageOption](../../../ArkUI（方舟UI框架）/ArkTS组件/通用属性/布局与边框/图片边框设置/ts-universal-attributes-border-image.md#borderimageoption对象说明) | 是 | 当按钮类型为[BUTTON\_CUSTOM](account-api-component-manager.md#style)时，此参数可设置按钮的图片边框样式，不设置则无效果。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| value | [OutlineOptions](../../../ArkUI（方舟UI框架）/ArkTS组件/公共定义/基础类型定义/ts-types.md#outlineoptions11对象说明) | 是 | 当按钮类型为[BUTTON\_CUSTOM](account-api-component-manager.md#style)时，此参数可设置按钮的外描边样式，不设置则无效果。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| value | [HoverEffect](../../../ArkUI（方舟UI框架）/ArkTS组件/公共定义/枚举说明/ts-appendix-enums.md#hovereffect8) | 是 | 当按钮类型为[BUTTON\_CUSTOM](account-api-component-manager.md#style)时，此参数可设置按钮的悬浮态效果，不设置则为默认效果。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| value | [ClickEffect](../../../ArkUI（方舟UI框架）/ArkTS组件/通用属性/视效与模糊/点击回弹效果/ts-universal-attributes-click-effect.md#clickeffect对象说明) | 是 | 当按钮类型为[BUTTON\_CUSTOM](account-api-component-manager.md#style)时，此参数可设置按钮的点击回弹效果，不设置则无效果。 |
...
| 名称 | 类型 | 只读 | 可选 | 说明 |
| --- | --- | --- | --- | --- |
| width | [Length](../../../ArkUI（方舟UI框架）/ArkTS组件/公共定义/基础类型定义/ts-types.md#length) | 否 | 是 | 设置华为账号登录按钮宽度。 |
| height | [Length](../../../ArkUI（方舟UI框架）/ArkTS组件/公共定义/基础类型定义/ts-types.md#length) | 否 | 是 | 设置华为账号登录按钮高度。 |
...
[LoginWithHuaweiIDButton](<../LoginWithHuaweiIDButton (华为账号Button登录组件)/account-api-huawei-id-button.md#loginwithhuaweiidbutton>)组件控制器，用来回调组件内部的点击事件。
...
| 类型 | 说明 |
| --- | --- |
| [LoginWithHuaweiIDButtonController](account-api-component-manager.md#loginwithhuaweiidbuttoncontroller) | [LoginWithHuaweiIDButton](<../LoginWithHuaweiIDButton (华为账号Button登录组件)/account-api-huawei-id-button.md>)组件控制器可以用来注册组件内部的点击事件或者调用组件提供的方法。 |
...
以下错误码的详细介绍请参见[ArkTS错误码](../../ArkTS错误码/account-api-error-code.md)。

| 错误码ID | 错误信息 |
| --- | --- |
| [1005300002](../../ArkTS错误码/account-api-error-code.md#section1005300002-用户未点击华为账号一键登录按钮) | The user did not click the HUAWEI ID login button. |
