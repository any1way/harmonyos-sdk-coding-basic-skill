...
scraped_at: 2026-06-01T16:32:29+08:00
doc_updated_at: 2026-04-20
content_hash: sha256:0d8e1b347ae6fac3c7a0b0651d8c081bc9189398191261d8188eb653a1364ae6
---
注意
...
[获取头像昵称](<../../../../../harmonyos-guides/应用服务/Account Kit（华为账号服务）/获取华为账号用户信息/获取头像昵称/account-get-avatar-nickname.md>)场景，应用服务端向华为账号服务器调用该接口获取头像昵称及其相关信息和UnionID/OpenID。
...
* 需确保调用端网络正常。
* 应用获取华为账号头像昵称之前需要在获取Authorization Code时携带profile（昵称和头像）scope，详见[业务流程](<../../../../../harmonyos-guides/应用服务/Account Kit（华为账号服务）/获取华为账号用户信息/获取头像昵称/account-get-avatar-nickname.md#业务流程>)。
...
| 参数 | 是否必选 | 参数类型 | 描述 |
| --- | --- | --- | --- |
| access\_token | 是 | String | 通过[获取用户级凭证](../../开放接口调用凭证/获取用户级凭证/account-api-obtain-user-token.md)获取的Access Token。获取凭证时Authorization Code需包含profile scope的授权。 |
...
| displayName | 否 | String | 用户昵称，该字段返回场景详见[获取头像昵称](<../../../../../harmonyos-guides/应用服务/Account Kit（华为账号服务）/获取华为账号用户信息/获取头像昵称/account-get-avatar-nickname.md>)。  - 请求参数“getNickName”为0或不传时，返回匿名化账号。  - 请求参数“getNickName”为1时，返回昵称，没有昵称时返回匿名化账号。  **说明：**  调用成功后响应中若不包含用户昵称displayName，请确认获取的Authorization Code是否携带了profile（昵称和头像）scope。参考[客户端开发](<../../../../../harmonyos-guides/应用服务/Account Kit（华为账号服务）/获取华为账号用户信息/获取头像昵称/account-get-avatar-nickname.md#客户端开发>) |
| displayNameFlag | 否 | int | 返回的昵称类型 。  0：昵称。  1：匿名账号。  **说明：**  调用成功后响应中若不包含昵称类型displayNameFlag，请确认获取的Authorization Code是否携带了profile（昵称和头像）scope。参考[客户端开发](<../../../../../harmonyos-guides/应用服务/Account Kit（华为账号服务）/获取华为账号用户信息/获取头像昵称/account-get-avatar-nickname.md#客户端开发>) |
| headPictureURL | 否 | String | 用户头像，该字段返回场景详见[获取头像昵称](<../../../../../harmonyos-guides/应用服务/Account Kit（华为账号服务）/获取华为账号用户信息/获取头像昵称/account-get-avatar-nickname.md>)。用户未设置头像时不返回。 |
