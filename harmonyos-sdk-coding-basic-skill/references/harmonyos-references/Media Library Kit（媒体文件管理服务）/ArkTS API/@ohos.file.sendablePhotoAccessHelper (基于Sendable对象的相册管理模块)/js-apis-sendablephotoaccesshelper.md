...
scraped_at: 2026-06-01T16:26:02+08:00
doc_updated_at: 2026-05-26
content_hash: sha256:e37b35d5ce9c8c21c78b6a37f64edb4c8075aa6bafecdb295e98ce7c41a1b7df
---
该模块基于[Sendable](../../../../harmonyos-guides/应用框架/ArkTS（方舟编程语言）/ArkTS并发/并发线程间通信/线程间通信对象/Sendable对象/Sendable对象简介/arkts-sendable.md)对象，提供相册管理功能，包括创建相册和访问、修改相册中的媒体数据。
...
对于未申请'ohos.permission.READ\_IMAGEVIDEO'权限的应用，可以通过picker的方式调用该接口来查询指定URI对应的图片或视频资源，详情请参考[指定URI获取图片或视频资源](<../../../../harmonyos-guides/媒体/Media Library Kit（媒体文件管理服务）/使用Picker选择媒体库资源/photoaccesshelper-photoviewpicker.md#指定uri获取图片或视频资源>)。

**参数：**

| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| options | [photoAccessHelper.FetchOptions](<../@ohos.file.photoAccessHelper (相册管理模块)/Interfaces (其他)/arkts-apis-photoaccesshelper-i.md#fetchoptions>) | 是 | 图片和视频检索选项。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| burstKey | string | 是 | 一组连拍照片的唯一标识：uuid（可传入[PhotoKeys](<../@ohos.file.photoAccessHelper (相册管理模块)/Enums/arkts-apis-photoaccesshelper-e.md#photokeys>)的BURST\_KEY）。字符串长度为36字节。 |
| options | [photoAccessHelper.FetchOptions](<../@ohos.file.photoAccessHelper (相册管理模块)/Interfaces (其他)/arkts-apis-photoaccesshelper-i.md#fetchoptions>) | 是 | 连拍照片检索选项。 |
...
此接口在未申请相册管理模块权限'ohos.permission.WRITE\_IMAGEVIDEO'时，可以使用安全控件创建媒体资源，详情请参考[保存媒体库资源](<../../../../harmonyos-guides/媒体/Media Library Kit（媒体文件管理服务）/保存媒体库资源/photoaccesshelper-savebutton.md>)。
...
| options | [photoAccessHelper.CreateOptions](<../@ohos.file.photoAccessHelper (相册管理模块)/Interfaces (其他)/arkts-apis-photoaccesshelper-i.md#createoptions>) | 否 | 创建选项，例如{title: 'testPhoto'}。  文件名中不允许出现非法英文字符。  API18开始，非法字符包括： \ / : \* ? " < > |  API10-17，非法字符包括：. .. \ / : \* ? " ' ` < > | { } [ ] |
...
| options | [photoAccessHelper.FetchOptions](<../@ohos.file.photoAccessHelper (相册管理模块)/Interfaces (其他)/arkts-apis-photoaccesshelper-i.md#fetchoptions>) | 否 | 检索选项，不填时默认根据相册类型检索。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| options | [photoAccessHelper.FetchOptions](<../@ohos.file.photoAccessHelper (相册管理模块)/Interfaces (其他)/arkts-apis-photoaccesshelper-i.md#fetchoptions>) | 是 | 检索选项。 |
...
| 名称 | 类型 | 只读 | 可选 | 说明 |
| --- | --- | --- | --- | --- |
| uri12+ | string | 是 | 否 | 媒体文件资源URI（如：file://media/Photo/1/IMG\_datetime\_0001/displayName.jpg），详情参见用户文件URI介绍中的[媒体文件URI](<../../../../harmonyos-guides/应用框架/Core File Kit（文件基础服务）/用户文件/用户文件URI介绍/user-file-uri-intro.md#媒体文件uri>)。  **元服务API：** 从API version 12开始，该接口支持在元服务中使用。 |
...
| 类型 | 说明 |
| --- | --- |
| photoAccessHelper.PhotoAsset | 返回非Sendable类型的[PhotoAsset](<../@ohos.file.photoAccessHelper (相册管理模块)/Interface (PhotoAsset)/arkts-apis-photoaccesshelper-photoasset.md>)。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| member | string | 是 | 成员参数名称，在get时，除了'uri'、'media\_type'、'subtype'和'display\_name'四个属性之外，其他的属性都需要在fetchColumns中填入需要get的[PhotoKeys](<../@ohos.file.photoAccessHelper (相册管理模块)/Enums/arkts-apis-photoaccesshelper-e.md#photokeys>)，例如：get title属性fetchColumns: ['title']。 |

**返回值：**

| 类型 | 说明 |
| --- | --- |
| [photoAccessHelper.MemberType](<../@ohos.file.photoAccessHelper (相册管理模块)/Types/arkts-apis-photoaccesshelper-t.md#membertype>) | 获取PhotoAsset成员参数的值。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| member | string | 是 | 成员参数名称例如：[PhotoKeys](<../@ohos.file.photoAccessHelper (相册管理模块)/Enums/arkts-apis-photoaccesshelper-e.md#photokeys>).TITLE。字符串长度的取值范围为[1, 255]。 |
| value | string | 是 | 设置成员参数名称，只能修改[PhotoKeys](<../@ohos.file.photoAccessHelper (相册管理模块)/Enums/arkts-apis-photoaccesshelper-e.md#photokeys>).TITLE的值。title的参数规格为：  - 不应包含扩展名。  - 文件名字符串长度的取值范围为[1, 255]（资产文件名为标题+扩展名）。  - 不允许出现的非法英文字符，包括：. \ / : \* ? " ' ` < > | { } [ ] |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| size | [image.Size](<../../../Image Kit（图片处理服务）/ArkTS API/@ohos.multimedia.image (图片处理)/Interfaces (其他)/arkts-apis-image-i.md#size>) | 否 | 缩略图尺寸。 |

**返回值：**

| 类型 | 说明 |
| --- | --- |
| Promise<[image.PixelMap](<../../../Image Kit（图片处理服务）/ArkTS API/@ohos.multimedia.image (图片处理)/Interface (PixelMap)/arkts-apis-image-pixelmap.md>)> | Promise对象，返回缩略图的PixelMap。 |
...
详细错误码请参见[通用错误码](../../../通用错误码/errorcode-universal.md)和[文件管理错误码](<../../../Core File Kit（文件基础服务）/错误码/文件管理错误码/errorcode-filemanagement.md>)。
...
| 类型 | 说明 |
| --- | --- |
| [photoAccessHelper.Album](<../@ohos.file.photoAccessHelper (相册管理模块)/Interface (Album)/arkts-apis-photoaccesshelper-album.md>) | 返回非Sendable类型的Album。 |
