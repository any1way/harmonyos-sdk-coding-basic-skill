...
scraped_at: 2026-06-01T16:37:14+08:00
doc_updated_at: 2026-04-20
content_hash: sha256:7e2180790fc6a9d71d238be982c281f037edb9e9b4263beb26e9eddc232e6e90
---
## HTTPS安全要求
...
* **认证(Authentication)**：认证操作确认调用者身份，防抵赖和数据篡改。所有对商户开放接口均需要商户在请求头中传递[PayMercAuth](../../数据模型说明/payment-model.md#paymercauth)字段，在开放网关认证通过后方可进行后续流程。
...
2. 排序拼接：按key值的ASCII码顺序排序，并使用&拼接，列表场景使用符号“=”连接。
   ...
   具体可参考[示例代码](https://gitcode.com/HarmonyOS_Samples/paymentkit-sample-code-serverdemo-java)引入华为支付提供[Maven依赖（pay-java）](<../../../../harmonyos-guides/应用服务/Payment Kit（鸿蒙支付服务）/开发准备/云侧服务准备/payment-server-connect.md#集成maven依赖>)中SignStringUtil.java类的signString方法，排序拼接结果示例如下：
...
3. 签名：调用给定算法对排序后的串完成签名。
   ...
   **SHA256WithRSA/PSS：** 具体可参考[示例代码](https://gitcode.com/HarmonyOS_Samples/paymentkit-sample-code-serverdemo-java)引入华为支付提供[Maven依赖（pay-java）](<../../../../harmonyos-guides/应用服务/Payment Kit（鸿蒙支付服务）/开发准备/云侧服务准备/payment-server-connect.md#集成maven依赖>)中RSAUtils.java类的signByRSAWithPSS方法。

   **SM2：** 具体可参考[Maven依赖（pay-java）](<../../../../harmonyos-guides/应用服务/Payment Kit（鸿蒙支付服务）/开发准备/云侧服务准备/payment-server-connect.md#集成maven依赖>)中Sm2Utils.java类中的sign方法。
...
1. 获取华为支付公钥，此处公钥证书为[华为支付商户平台](https://petalpay-merchant.cloud.huawei.com/)获取的证书，可参见[下载华为支付证书](<../../../../harmonyos-guides/应用服务/Payment Kit（鸿蒙支付服务）/开发准备/准备证书/payment-certificates-config.md#下载华为支付证书>)。
2. 获取参与签名的字段，转换为map并将map的“key-value”排序后以符号“&”拼接。
3. 使用SM2算法进行验签，具体参考[示例代码](https://gitcode.com/HarmonyOS_Samples/paymentkit-sample-code-serverdemo-java)引入华为支付提供[Maven依赖（pay-java）](<../../../../harmonyos-guides/应用服务/Payment Kit（鸿蒙支付服务）/开发准备/云侧服务准备/payment-server-connect.md>)中VerifyTools.java类的verifySign方法。
