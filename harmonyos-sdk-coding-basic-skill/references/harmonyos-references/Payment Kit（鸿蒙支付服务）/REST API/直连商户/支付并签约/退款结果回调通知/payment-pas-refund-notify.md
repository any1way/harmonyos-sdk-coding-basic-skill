...
scraped_at: 2026-06-11T16:48:11+08:00
doc_updated_at: 2026-04-20
content_hash: sha256:4b6f31c28023a9d4078a98834dc9bfe593d0756c67c551d73d6ddddcb8bf6144
---
## 功能介绍
...
2. 为保证信息合法性，开发者需要对返回的支付信息进行[SM2验签](../../../公共说明/payment-rest-overview.md#验签规则)，验签注意事项：

   * 需直接使用通知的完整内容进行验签。
   * 验签前需要对返回数据进行排序拼接，sign字段是签名值，排序拼接后的待验签内容需要排除sign字段。
   * 验签公钥使用[华为支付证书](<../../../../../../harmonyos-guides/应用服务/Payment Kit（鸿蒙支付服务）/开发准备/准备证书/payment-certificates-config.md#华为支付证书>)。
3. 退款状态需根据refundOrderStatus字段判断。

## 场景描述

一笔订单退款成功后，华为支付服务器会调用开发者的应用/元服务在[申请退款](../../基础支付/申请退款/payment-service--refund.md)接口中传递的回调接口（callbackUrl）传递给开发者退款订单具体信息。
