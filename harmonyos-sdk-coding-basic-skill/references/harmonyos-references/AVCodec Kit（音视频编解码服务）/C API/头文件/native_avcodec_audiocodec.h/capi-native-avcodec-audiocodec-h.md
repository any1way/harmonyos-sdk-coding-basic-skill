...
scraped_at: 2026-06-01T16:18:13+08:00
doc_updated_at: 2026-03-09
content_hash: sha256:b10324dd7f51c7c1fb05a3e2ecacb4939f3859ef683d870d09bfc8bf46fd6a1a
---
## 概述
...
**相关模块：** [AudioCodec](../../模块/AudioCodec/capi-audiocodec.md)
...
| 名称 | typedef关键字 | 描述 |
| --- | --- | --- |
| [MediaKeySession](<../../../../DRM Kit（数字版权保护服务）/C API/结构体/MediaKeySession/capi-drm-mediakeysession.md>) | MediaKeySession | MediaKeySession字段。 |
...
| [OH\_AVCodec \*OH\_AudioCodec\_CreateByName(const char \*name)](capi-native-avcodec-audiocodec-h.md#oh_audiocodec_createbyname) | 通过音频编解码器名称创建音频编解码器实例，使用此接口的前提是知道编解码器的确切名称，编解码器的名称可以通过[OH\_AVCapability\_GetName](../native_avcapability.h/capi-native-avcapability-h.md#oh_avcapability_getname)获取。 |
...
| [OH\_AVFormat \*OH\_AudioCodec\_GetOutputDescription(OH\_AVCodec \*codec)](capi-native-avcodec-audiocodec-h.md#oh_audiocodec_getoutputdescription) | 获取编解码器输出数据的OH\_AVFormat信息。  需要注意的是，返回值所指向的OH\_AVFormat实例需要开发者调用[OH\_AVFormat\_Destroy](../native_avformat.h/capi-native-avformat-h.md#oh_avformat_destroy)接口手动释放。 |
...
| 参数项 | 描述 |
| --- | --- |
| const char \*mime | MIME类型描述字符串，请参阅[AVCODEC\_MIMETYPE](../native_avcodec_base.h/capi-native-avcodec-base-h.md#变量)。 |
...
| 类型 | 说明 |
| --- | --- |
| [OH\_AVCodec](../../结构体/OH_AVCodec/capi-codecbase-oh-avcodec.md) \* | 返回OH\_AVCodec实例的指针。 |
...
通过音频编解码器名称创建音频编解码器实例，使用此接口的前提是知道编解码器的确切名称，编解码器的名称可以通过[OH\_AVCapability\_GetName](../native_avcapability.h/capi-native-avcapability-h.md#oh_avcapability_getname)获取。
...
| 参数项 | 描述 |
| --- | --- |
| [OH\_AVCodec](../../结构体/OH_AVCodec/capi-codecbase-oh-avcodec.md) \*codec | 指向OH\_AVCodec实例的指针。 |

**返回：**

| 类型 | 说明 |
| --- | --- |
| [OH\_AVErrCode](../native_averrors.h/capi-native-averrors-h.md#oh_averrcode) | AV\_ERR\_OK：执行成功。  AV\_ERR\_INVALID\_VAL：OH\_AVCodec实例为nullptr或无效。  AV\_ERR\_INVALID\_STATE：编解码器服务不可用。  AV\_ERR\_NO\_MEMORY：内部资源已经释放。  AV\_ERR\_UNKNOWN：发生内部错误，建议检查日志。 |
...
| 类型 | 说明 |
| --- | --- |
| [OH\_AVErrCode](../native_averrors.h/capi-native-averrors-h.md#oh_averrcode) | AV\_ERR\_OK：执行成功。  AV\_ERR\_INVALID\_VAL：输入参数为nullptr或无效。  AV\_ERR\_INVALID\_STATE：编解码器服务不可用。 |
...
| --- | --- |
| [OH\_AVCodec](../../结构体/OH_AVCodec/capi-codecbase-oh-avcodec.md) \*codec | 指向OH\_AVCodec实例的指针。 |
| [const OH\_AVFormat](../../结构体/OH_AVFormat/capi-core-oh-avformat.md) \*format | 指向OH\_AVFormat的指针，给出要编解码的音频轨道的描述。 |

**返回：**

| 类型 | 说明 |
| --- | --- |
| [OH\_AVErrCode](../native_averrors.h/capi-native-averrors-h.md#oh_averrcode) | AV\_ERR\_OK：执行成功。  AV\_ERR\_INVALID\_VAL：输入参数为nullptr或无效。  AV\_ERR\_INVALID\_STATE：编解码器服务不可用。  AV\_ERR\_OPERATE\_NOT\_PERMIT：不允许操作，这可能是由于状态不正确或不支持的操作。  AV\_ERR\_UNKNOWN：发生内部错误，建议检查日志。 |
...
| 类型 | 说明 |
| --- | --- |
| [OH\_AVErrCode](../native_averrors.h/capi-native-averrors-h.md#oh_averrcode) | AV\_ERR\_OK：执行成功。  AV\_ERR\_INVALID\_VAL：OH\_AVCodec实例为nullptr或无效。  AV\_ERR\_INVALID\_STATE：编解码器服务不可用。  AV\_ERR\_OPERATE\_NOT\_PERMIT：不允许操作，这可能是由于状态不正确或不支持的操作。  AV\_ERR\_UNKNOWN：发生内部错误，建议检查日志。 |
...
| 类型 | 说明 |
| --- | --- |
| [OH\_AVErrCode](../native_averrors.h/capi-native-averrors-h.md#oh_averrcode) | AV\_ERR\_OK：执行成功。  AV\_ERR\_INVALID\_VAL：OH\_AVCodec实例为nullptr或无效。  AV\_ERR\_INVALID\_STATE：编解码器服务不可用。 |
...
需要注意的是，返回值所指向的OH\_AVFormat实例需要开发者调用[OH\_AVFormat\_Destroy](../native_avformat.h/capi-native-avformat-h.md#oh_avformat_destroy)接口手动释放。
...
| --- | --- |
| [OH\_AVCodec](../../结构体/OH_AVCodec/capi-codecbase-oh-avcodec.md) \*codec | 指向OH\_AVCodec实例的指针。 |
| const [OH\_AVFormat](../../结构体/OH_AVFormat/capi-core-oh-avformat.md) \*format | OH\_AVFormat句柄指针。 |
...
[OH\_AVCodecOnNeedInputBuffer](../native_avcodec_base.h/capi-native-avcodec-base-h.md#oh_avcodeconneedinputbuffer)回调将报告可用的输入缓冲区和对应的索引值。

一旦具有指定索引的缓冲区被提交给音频编解码器，该缓冲区将无法再次访问，直到再次收到[OH\_AVCodecOnNeedInputBuffer](../native_avcodec_base.h/capi-native-avcodec-base-h.md#oh_avcodeconneedinputbuffer)回调，收到相同索引时此缓冲区才可使用。
...
| 类型 | 说明 |
| --- | --- |
| [OH\_AVErrCode](../native_averrors.h/capi-native-averrors-h.md#oh_averrcode) | AV\_ERR\_OK：执行成功。  AV\_ERR\_INVALID\_VAL：输入参数为nullptr或无效。 |
...
| --- | --- |
| [OH\_AVCodec](../../结构体/OH_AVCodec/capi-codecbase-oh-avcodec.md) \*codec | 指向OH\_AVCodec实例的指针。 |
| [MediaKeySession](<../../../../DRM Kit（数字版权保护服务）/C API/结构体/MediaKeySession/capi-drm-mediakeysession.md>) \*mediaKeySession | 带有解密功能的媒体秘钥会话实例。 |
...
| 类型 | 说明 |
| --- | --- |
| [OH\_AVErrCode](../native_averrors.h/capi-native-averrors-h.md#oh_averrcode) | AV\_ERR\_OK：执行成功。  AV\_ERR\_INVALID\_VAL：OH\_AVCodec实例为nullptr或无效，mediaKeySystemInfo实例为nullptr或无效。  AV\_ERR\_INVALID\_STATE：编解码器服务不可用。  AV\_ERR\_NO\_MEMORY：请求内存失败。 |
...
| 参数项 | 描述 |
| --- | --- |
| [struct OH\_AVCodec](../../结构体/OH_AVCodec/capi-codecbase-oh-avcodec.md) \*codec | 指向OH\_AVCodec实例的指针。 |
...
| 类型 | 说明 |
| --- | --- |
| [OH\_AVErrCode](../native_averrors.h/capi-native-averrors-h.md#oh_averrcode) | AV\_ERR\_OK：执行成功。  AV\_ERR\_INVALID\_VAL：执行失败，输入参数错误。  AV\_ERR\_INVALID\_STATE：执行失败，状态非法，没有启动编解码器等。  AV\_ERR\_OPERATE\_NOT\_PERMIT：执行失败，不允许非同步模式下调用。  AV\_ERR\_TRY\_AGAIN\_LATER：执行失败，超时时间内获取不到可用的缓冲区。 |
