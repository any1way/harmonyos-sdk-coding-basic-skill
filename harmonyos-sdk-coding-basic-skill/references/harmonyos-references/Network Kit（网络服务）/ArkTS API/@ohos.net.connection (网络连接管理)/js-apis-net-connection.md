...
scraped_at: 2026-06-01T16:06:29+08:00
doc_updated_at: 2026-05-26
content_hash: sha256:7f6cfb89473bb0d6ec33cf9c8e4ffc66de90ca11364e50a2c1a15189b27ec824
---
...
以下错误码的详细介绍请参见[网络连接管理错误码](../../错误码/网络连接管理错误码/errorcode-net-connection.md)和[通用错误码](../../../../通用错误码/errorcode-universal.md)。
...
若需使用本接口所配置的代理信息，则需在[HttpRequestOptions](<../@ohos.net.http (数据请求)/js-apis-http.md#httprequestoptions>)字段中将usingProxy设置为true以启用代理转发。本接口仅负责配置代理规则，不校验代理服务的有效性。
...
15. };
16. // 发起一个HTTP请求。
17. httpRequest.request("EXAMPLE_URL", options, (err: BusinessError, data: http.HttpResponse) => {
18. if (!err) {
...
以下错误码的详细介绍请参见[网络连接管理错误码](../../错误码/网络连接管理错误码/errorcode-net-connection.md)。
...
4. connection.getSystemNetPortStates().then((data: connection.NetPortStatesInfo) => {
5. console.info(`Succeeded to get data: ${JSON.stringify(data)}`);
6. if (data.tcpPortStatesInfo?.length) {
7. data.tcpPortStatesInfo?.forEach(item => {
8. console.info(`Succeeded to get Tcp data: ${JSON.stringify(item)}`);
9. })
10. } else {
11. console.info("TcpPortStatesInfo is undefined ");
12. }
13. if (data.udpPortStatesInfo?.length) {
14. data.udpPortStatesInfo?.forEach(item => {
15. console.info(`Succeeded to get Udp data: ${JSON.stringify(item)}`);
16. })
17. } else {
18. console.info("UdpPortStatesInfo is undefined ");
19. }
20. }).catch((error: BusinessError) => {
21. console.error(`Error fetching getSystemNetPortStates. Code:${error.code}, message:${error.message}`);
22. });
```
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| socketParam | [TCPSocket](<../@ohos.net.socket (Socket连接)/js-apis-socket.md#tcpsocket>) | [UDPSocket](<../@ohos.net.socket (Socket连接)/js-apis-socket.md#udpsocket>) | 是 | 待绑定的TCPSocket或UDPSocket对象。 |
...
定义一个HTTP请求，可以通过[http.createHttp](<../@ohos.net.http (数据请求)/js-apis-http.md#httpcreatehttp>)创建。
...
定义一个TCPSocket对象，可以通过[socket.constructTCPSocketInstance](<../@ohos.net.socket (Socket连接)/js-apis-socket.md#socketconstructtcpsocketinstance>)创建。
...
定义一个UDPSocket对象，可以通过[socket.constructUDPSocketInstance](<../@ohos.net.socket (Socket连接)/js-apis-socket.md#socketconstructudpsocketinstance>)创建。
