...
content_hash: sha256:b0b01221192e4873be9c3ac7db65d6dcf48a5f3b20f1b247becaef3c23123608
---
sensor模块提供了获取传感器数据的能力，包括获取传感器属性列表，订阅传感器数据，以及一些通用的传感器算法。

说明

本模块首批接口从API version 8开始支持。后续版本的新增接口，采用上角标单独标记接口的起始版本。订阅前可使用[getSingleSensor](js-apis-sensor.md#sensorgetsinglesensor9)接口获取该传感器的信息，获取该传感器信息成功时可正常订阅传感器，异常情况详见[getSingleSensor](js-apis-sensor.md#sensorgetsinglesensor9)错误码说明，具体使用方法可参考[指南开发步骤](<../../../../harmonyos-guides/系统/硬件/Sensor Service Kit（传感器服务）/传感器/传感器开发指导(ArkTS)/sensor-guidelines.md#开发步骤>)；订阅传感器数据时确保on订阅和off取消订阅成对出现。
...
以下错误码的详细介绍请参见[传感器错误码](../../错误码/传感器错误码/errorcode-sensor.md)和[通用错误码](../../../通用错误码/errorcode-universal.md)。错误码和错误信息会以异常的形式抛出，调用接口时需要使用try catch对可能出现的异常进行捕获操作。
...
以下错误码的详细介绍请参见[传感器错误码](../../错误码/传感器错误码/errorcode-sensor.md)。错误码和错误信息会以异常的形式抛出，调用接口时需要使用try catch对可能出现的异常进行捕获操作。
...
以下错误码的详细介绍请参见[通用错误码](../../../通用错误码/errorcode-universal.md)和[传感器错误码](../../错误码/传感器错误码/errorcode-sensor.md)。错误码和错误信息会以异常的形式抛出，调用接口时需要使用try catch对可能出现的异常进行捕获操作。。
