...
scraped_at: 2026-06-01T16:35:57+08:00
doc_updated_at: 2026-05-18
content_hash: sha256:cd465dc8b91696295db4a31bdda06b38289a11175766941b9285bf9762d28f19
---
## 导入模块
...
| callback | Callback<Array<[mapCommon.ClusterItem](../../mapCommon（地图属性模型）/map-common.md#clusteritem)>> | 是 | 回调函数，返回Array<[mapCommon.ClusterItem](../../mapCommon（地图属性模型）/map-common.md#clusteritem)>。 |
...
| callback | Callback<Array<[mapCommon.ClusterItem](../../mapCommon（地图属性模型）/map-common.md#clusteritem)>> | 是 | 回调函数，返回Array<[mapCommon.ClusterItem](../../mapCommon（地图属性模型）/map-common.md#clusteritem)>，监听聚合图层的聚合点点击事件。 |
...
| --- | --- | --- | --- |
| type | string | 是 | 'click'：监听聚合图层的聚合点点击事件。 |
| callback | Callback<Array<[mapCommon.ClusterItem](../../mapCommon（地图属性模型）/map-common.md#clusteritem)>> | 否 | 回调函数，返回Array<[mapCommon.ClusterItem](../../mapCommon（地图属性模型）/map-common.md#clusteritem)>，取消监听聚合图层的聚合点点击事件。  - callback为空：取消所有callback回调。  - callback非空：取消指定的callback回调。 |
...
| callback | Callback<[MarkerClusterInfo](<../Interface (MarkerClusterInfo)/map-map-markerclusterinfo.md>)> | 是 | 回调函数，返回Callback<[MarkerClusterInfo](<../Interface (MarkerClusterInfo)/map-map-markerclusterinfo.md>)>，监听聚合图层的标记点击事件。  MarkerClusterInfo包括：  - marker：聚合图层的标记。  - clusterItems：聚合节点数组。 |
...
| --- | --- | --- | --- |
| type | string | 是 | 'markerClusterClick'：聚合图层的标记点击监听事件。 |
| callback | Callback<[MarkerClusterInfo](<../Interface (MarkerClusterInfo)/map-map-markerclusterinfo.md>)> | 否 | 回调函数，返回Callback<[MarkerClusterInfo](<../Interface (MarkerClusterInfo)/map-map-markerclusterinfo.md>)>，取消监听聚合图层的标记点击事件。  - callback为空：取消所有callback回调。  - callback非空：取消指定的callback回调。  MarkerClusterInfo包括：  - marker：聚合图层的标记。  - clusterItems：聚合节点数组。 |
...
| **参数名** | **类型** | 必填 | **说明** |
| --- | --- | --- | --- |
| item | mapCommon.[ClusterItem](../../mapCommon（地图属性模型）/map-common.md#clusteritem) | 是 | 待聚合节点。 |
