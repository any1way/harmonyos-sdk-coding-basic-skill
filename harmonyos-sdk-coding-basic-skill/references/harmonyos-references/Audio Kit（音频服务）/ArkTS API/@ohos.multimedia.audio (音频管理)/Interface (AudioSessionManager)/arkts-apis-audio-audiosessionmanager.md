...
scraped_at: 2026-06-01T16:16:51+08:00
doc_updated_at: 2026-05-26
content_hash: sha256:dd7d4453221f8738f7db10f60f8f00db304d34b0531e16f6941e510b390d3f84
---
音频会话管理。

在使用AudioSessionManager的接口之前，需先通过[getSessionManager](<../Interface (AudioManager)/arkts-apis-audio-audiomanager.md#getsessionmanager12>)获取AudioSessionManager实例。
...
| callback | Callback<[AudioSessionDeactivatedEvent](<../Interfaces (其他)/arkts-apis-audio-i.md#audiosessiondeactivatedevent12>)> | 是 | 回调函数，返回音频会话停用原因。 |
...
| callback | Callback<[AudioSessionDeactivatedEvent](<../Interfaces (其他)/arkts-apis-audio-i.md#audiosessiondeactivatedevent12>)> | 否 | 回调函数，返回音频会话停用原因。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| scene | [AudioSessionScene](../Enums/arkts-apis-audio-e.md#audiosessionscene20) | 是 | 音频会话场景。 |
...
| callback | Callback<[AudioSessionStateChangedEvent](<../Interfaces (其他)/arkts-apis-audio-i.md#audiosessionstatechangedevent20>)> | 是 | 回调函数，返回音频会话变更提示信息。 |
...
| --- | --- | --- | --- |
| type | string | 是 | 事件回调类型，支持的事件为'audioSessionStateChanged'，当音频会话状态变更时，触发该事件。 |
| callback | Callback<[AudioSessionStateChangedEvent](<../Interfaces (其他)/arkts-apis-audio-i.md#audiosessionstatechangedevent20>)> | 否 | 回调函数，返回音频会话变更提示信息。 |
...
* 本接口适用于以下情况：当设置的[AudioSessionScene](../Enums/arkts-apis-audio-e.md#audiosessionscene20)为VoIP场景时，激活AudioSession后立即生效。若[AudioSessionScene](../Enums/arkts-apis-audio-e.md#audiosessionscene20)为非VoIP场景，激活AudioSession时不会生效，仅在启动播放的[StreamUsage](../Enums/arkts-apis-audio-e.md#streamusage)为语音消息、VoIP语音通话或VoIP视频通话时才生效。支持听筒、扬声器和系统默认设备。
...
| callback | Callback<[CurrentOutputDeviceChangedEvent](<../Interfaces (其他)/arkts-apis-audio-i.md#currentoutputdevicechangedevent20>)> | 是 | 回调函数，返回当前输出设备信息。 |
...
| callback | Callback<[CurrentOutputDeviceChangedEvent](<../Interfaces (其他)/arkts-apis-audio-i.md#currentoutputdevicechangedevent20>)> | 否 | 回调函数，用于返回当前输出设备变化的信息。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| deviceUsage | [DeviceUsage](../Enums/arkts-apis-audio-e.md#deviceusage12) | 是 | 音频设备类型（根据用途分类）。 |

**返回值：**

| 类型 | 说明 |
| --- | --- |
| [AudioDeviceDescriptors](../Types/arkts-apis-audio-t.md#audiodevicedescriptors) | 返回设备列表。 |
...
| callback | Callback<[DeviceChangeAction](<../Interfaces (其他)/arkts-apis-audio-i.md#devicechangeaction>)> | 否 | 回调函数，返回可选设备更新详情。 |
...
* 本接口不适用于VoIP通话录音，即[SourceType](../Enums/arkts-apis-audio-e.md#sourcetype8)为SOURCE\_TYPE\_VOICE\_COMMUNICATION的场景不适用。
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| inputAudioDevice | [AudioDeviceDescriptor](<../Interfaces (其他)/arkts-apis-audio-i.md#audiodevicedescriptor>) | 是 | 媒体输入设备。 |
...
3. try {
4. let data: audio.AudioDeviceDescriptors = audioSessionManager.getAvailableDevices(audio.DeviceUsage.MEDIA_INPUT_DEVICES);
5. console.info('Succeeded in doing getAvailableDevices.');
...
| 类型 | 说明 |
| --- | --- |
| [AudioDeviceDescriptor](<../Interfaces (其他)/arkts-apis-audio-i.md#audiodevicedescriptor>) | 媒体输入设备信息。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| category | [BluetoothAndNearlinkPreferredRecordCategory](../Enums/arkts-apis-audio-e.md#bluetoothandnearlinkpreferredrecordcategory21) | 是 | 在使用蓝牙或星闪进行录音时，应用程序的设备偏好分类。 |
...
| 类型 | 说明 |
| --- | --- |
| [BluetoothAndNearlinkPreferredRecordCategory](../Enums/arkts-apis-audio-e.md#bluetoothandnearlinkpreferredrecordcategory21) | 在使用蓝牙或星闪进行录音时，应用程序的设备偏好分类。 |
...
| callback | Callback<[CurrentInputDeviceChangedEvent](<../Interfaces (其他)/arkts-apis-audio-i.md#currentinputdevicechangedevent21>)> | 是 | 回调函数，返回当前输入设备信息。 |
...
| --- | --- | --- | --- |
| type | string | 是 | 事件回调类型，支持的事件为'currentInputDeviceChanged'，当前输入设备发生变化时，触发该事件。 |
| callback | Callback<[CurrentInputDeviceChangedEvent](<../Interfaces (其他)/arkts-apis-audio-i.md#currentinputdevicechangedevent21>)> | 否 | 回调函数，用于返回当前输入设备变化的信息。 |
...
如果启用此功能，当订阅音频会话状态更改事件后静音建议和取消静音建议提示将通过[AudioSessionStateChangedEvent](<../Interfaces (其他)/arkts-apis-audio-i.md#audiosessionstatechangedevent20>)回调发送。收到静音建议表示其他应用程序开始播放音频，且播放的音频和本应用的音频不能混音。

此功能仅支持已设置[AudioSessionScene](../Enums/arkts-apis-audio-e.md#audiosessionscene20)并激活模式模式为CONCURRENCY\_MIX\_WITH\_OTHERS的音频会话使用。并且仅在激活音频会话期间生效一次，每次激活音频会话前都必须重新启用。

详细说明请参考[启用混音播放下静音建议通知](<../../../../../harmonyos-guides/媒体/Audio Kit（音频服务）/音频焦点和音频会话管理/音频会话管理(ArkTS)/audio-session-management.md#启用混音播放下静音建议通知>)。
...
| 错误码ID | 错误信息 |
| --- | --- |
| 6800103 | Function is called without setting [AudioSessionScene](../Enums/arkts-apis-audio-e.md#audiosessionscene20) or called after audio session activation. |
...
* 若某条录音流同时调用了流级接口[AudioCapturer.setMuteHint](<../Interface (AudioCapturer)/arkts-apis-audio-audiocapturer.md#setmutehint24>)和本接口，流级接口设置优先级更高，以流级接口设置值为准。
