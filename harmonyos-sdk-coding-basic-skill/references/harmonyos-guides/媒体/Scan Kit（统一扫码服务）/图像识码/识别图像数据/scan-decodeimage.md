...
scraped_at: 2026-06-11T14:58:42+08:00
doc_updated_at: 2026-04-28
content_hash: sha256:9a6df5480992a1e10c6f648e50c535a83381fc7476604b7ddde88f34bd371d5f
---
## 基本概念
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/38/v3/VnuyrgRZTMuuqIlWqZaQGQ/zh-cn_image_0000002622698607.png?HW-CC-KV=V1&HW-CC-Date=20260611T065841Z&HW-CC-Expire=86400&HW-CC-Sign=1867F453F3224BA2E68EAC4C717BB7D9BF2C837D1777C1A1E6D9DE11FC204B6F)

1. 用户向应用发起识码请求。
2. 应用通过调用[Camera Kit](<../../../Camera Kit（相机服务）/Camera Kit简介/camera-overview.md>)启动相机，获取预览流数据。
...
识别图像数据中的码图，以Promise形式返回识别结果。具体API说明详见[接口文档](<../../../../../harmonyos-references/Scan Kit（统一扫码服务）/ArkTS API/detectBarcode (图像识码)/scan-imagedecode.md>)。

| 接口名 | 描述 |
| --- | --- |
| [decodeImage](<../../../../../harmonyos-references/Scan Kit（统一扫码服务）/ArkTS API/detectBarcode (图像识码)/scan-imagedecode.md#detectbarcodedecodeimage>)(image: [ByteImage](<../../../../../harmonyos-references/Scan Kit（统一扫码服务）/ArkTS API/detectBarcode (图像识码)/scan-imagedecode.md#byteimage>), options?: scanBarcode.[ScanOptions](<../../../../../harmonyos-references/Scan Kit（统一扫码服务）/ArkTS API/scanBarcode (默认界面扫码)/scan-scanbarcode-api.md#scanoptions>)): Promise<[DetectResult](<../../../../../harmonyos-references/Scan Kit（统一扫码服务）/ArkTS API/detectBarcode (图像识码)/scan-imagedecode.md#detectresult>)> | 启动图像识码，通过传入ByteImage类型的图像数据信息，使用Promise异步回调返回识码结果。 |
...
1. 导入图像识码接口和相关接口模块，该模块提供了图像识码参数和方法，导入方法如下。
   ...
   ```
2. 使用Camera Kit启动相机能力，实现双路预览功能，具体实现详见[双路预览](<../../../Camera Kit（相机服务）/开发相机应用基础能力(ArkTS)/双路预览(ArkTS)/camera-dual-channel-preview.md>)。
3. 通过ImageReceiver实时获取预览图像数据，详见[双路预览](<../../../Camera Kit（相机服务）/开发相机应用基础能力(ArkTS)/双路预览(ArkTS)/camera-dual-channel-preview.md>)，调用detectBarcode.decodeImage接口解析图像数据。请在识别完成后再释放图像数据。
   ...
   26. }
   ```
4. detectBarcode.[DetectResult](<../../../../../harmonyos-references/Scan Kit（统一扫码服务）/ArkTS API/detectBarcode (图像识码)/scan-imagedecode.md#detectresult>)中返回的cornerPoints可参考以下说明使用。
