...
scraped_at: 2026-06-11T14:55:18+08:00
doc_updated_at: 2026-06-03
content_hash: sha256:09c6c8bc9cf77217e293f63f8b492877c52a295c346f0d48acdef376a79326c1
---
...
* [AVCastPicker](<../../../../../../harmonyos-references/AVSession Kit（音视频播控服务）/ArkTS组件/@ohos.multimedia.avCastPicker (投播组件)/ohos-multimedia-avcastpicker.md>)：投播组件，提供设备发现认证连接的统一入口。
* [AVCastController](<../../../../../../harmonyos-references/AVSession Kit（音视频播控服务）/ArkTS API/@ohos.multimedia.avsession (媒体会话管理)/Interface (AVCastController)/arkts-apis-avsession-avcastcontroller.md>)：投播控制器，用于投播场景下，完成播放控制、远端播放状态监听等操作。
...
1. 创建播放器，并创建AVSession。
   ...
   示例中的context的获取方式请参见[获取UIAbility的上下文信息](<../../../../../应用框架/Ability Kit（程序框架服务）/Stage模型开发指导/Stage模型应用组件/UIAbility组件/UIAbility组件基本用法/uiability-usage.md#获取uiability的上下文信息>)。
...
2. 设置媒体资源信息，注册基础播控回调，接入系统播控中心的基础播控。
   ...
   * 接入投播组件前需要先适配媒体播控中心的基础播控业务，具体需要接入的内容请按照应用类型参考[应用接入自检表](../../../应用接入播控自检/应用接入播控自检表/playback-control-access-checklist.md)。
...
3. 在需要投播的播放界面创建投播组件AVCastPicker。
   ...
   * 系统AVCastPicker提供CustomerPicker参数，应用可以通过该参数自定义Picker按钮的显示，具体开发请参考[自定义样式](../../使用组件切换通话设备/using-switch-call-devices.md#自定义样式实现)。
...
4. 注册AVSession投播控制回调。用于感知投播设备连接。
   ...
   * 可以在on('outputDeviceChange')回调中使用[castCategory](<../../../../../../harmonyos-references/AVSession Kit（音视频播控服务）/ArkTS API/@ohos.multimedia.avsession (媒体会话管理)/Enums/arkts-apis-avsession-e.md#avcastcategory10>)来判断是否连接到远端设备。
   * on('outputDeviceChange')回调具体取值来自[ProtocolType](<../../../../../../harmonyos-references/AVSession Kit（音视频播控服务）/ArkTS API/@ohos.multimedia.avsession (媒体会话管理)/Enums/arkts-apis-avsession-e.md#protocoltype11>)，可以是protocoltype中的某个协议或多个协议的组合，设备仅支持一种协议，返回对应枚举值；设备支持多种协议，返回对应枚举值之和。
...
5. 使用AVCastController进行资源播放。
   ...
   * 如果通过FilePicker使用用户文件，请参考[选择用户文件](<../../../../../应用框架/Core File Kit（文件基础服务）/用户文件/选择与保存用户文件/选择用户文件/select-user-file.md>)。
   * 如果使用网络播放路径，需[申请相关权限](../../../../../系统/安全/程序访问控制/应用权限管控/申请应用权限/声明权限/declare-permissions.md)：ohos.permission.INTERNET。
...
6. 使用AVCastController，监听控制命令和进行播放控制。
   ...
   * 当前系统暂不支持清晰度调节，应用如需向用户提供清晰度调节入口，需要在用户选定不同清晰度时按照新的清晰度重新下发资源。
   ...
   36. this.castController?.on('playbackStateChange', 'all', (playbackState: avSession.AVPlaybackState) => {
...
8. 投播后资源切换。
   ...
   * 应用已开始投播，再次进入播放详情页时可以通过getOutputDevice()接口获取当前播放设备，来判断此时是否正在投播中。
     ...
     3. import { session } from './xxx'; // session声明的文件
     4. session?.getOutputDevice().then((outputDeviceInfo: avSession.OutputDeviceInfo) => {
...
9. 处理音频焦点。请参考[多音频并发处理](<../../../../Audio Kit（音频服务）/音频焦点和音频会话管理/音频焦点介绍/audio-playback-concurrency.md>)。
...
1. 应用进入播放页后，需要调用[getOutputDevicesync](<../../../../../../harmonyos-references/AVSession Kit（音视频播控服务）/ArkTS API/@ohos.multimedia.avsession (媒体会话管理)/Interface (AVSession)/arkts-apis-avsession-avsession.md#getoutputdevicesync10>)接口来判断当前是否存在投屏的设备，避免和其他应用冲突。
...
具体可参考[音频投播](../../../../../../best-practices/自由流转/多端协同/音频投播/bpta-audio-cast.md#section122641843162812) 与[视频投播](../../../../../../best-practices/自由流转/多端协同/音频投播/bpta-audio-cast.md#section122641843162812)。
