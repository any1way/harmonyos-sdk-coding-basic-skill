...
scraped_at: 2026-06-11T14:57:02+08:00
doc_updated_at: 2026-06-03
content_hash: sha256:6ba6ba8c71d84883122fed3de005f4fb987d4483e4edb3da52f27d9eb8e7e458
---
...
| 对象类型 | 适用场景 | 特性 |
| --- | --- | --- |
| [PixelMap](<../../../../../../harmonyos-references/Image Kit（图片处理服务）/C API/结构体/OH_PixelmapNative/capi-image-nativemodule-oh-pixelmapnative.md>) | 单图显示、基础图片处理 | 单一像素数据，支持图像变换（裁剪、缩放、旋转等）、位图操作。 |
| [Picture](<../../../../../../harmonyos-references/Image Kit（图片处理服务）/C API/结构体/OH_PictureNative/capi-image-nativemodule-oh-picturenative.md>) | HDR图片、HEIF专业格式、辅助图处理 | 包含主图+辅助图+元数据，可提取主图/增益图/合成HDR图为PixelMap后显示或处理，支持辅助图和元数据操作。 |

**选择建议：**

* 需要直接显示单张图片或进行裁剪、缩放、旋转等图像处理时，使用PixelMap。
* 需要处理HDR图片、获取辅助图（如GAINMAP）、操作图片元数据时，使用Picture。如需对Picture的内容进行裁剪缩放，可通过[OH\_PictureNative\_GetMainPixelmap()](<../../../../../../harmonyos-references/Image Kit（图片处理服务）/C API/头文件/picture_native.h/capi-picture-native-h.md#oh_picturenative_getmainpixelmap>)等接口提取PixelMap后再处理。
...
1. 导入相关头文件。
   ...
   [loadPicture.cpp](https://gitcode.com/HarmonyOS_Samples/guide-snippets/blob/HarmonyOS-feature-20260402/Media/Image/ImageNativeSample/entry/src/main/cpp/loadPicture.cpp#L15-L22)
...
3. 定义ImagePictureNative类。
   ...
   [imageKits.h](https://gitcode.com/HarmonyOS_Samples/guide-snippets/blob/HarmonyOS-feature-20260402/Media/Image/ImageNativeSample/entry/src/main/cpp/imageKits.h#L74-L86)
4. 创建一个ImagePictureNative实例。
   ...
   [loadPicture.cpp](https://gitcode.com/HarmonyOS_Samples/guide-snippets/blob/HarmonyOS-feature-20260402/Media/Image/ImageNativeSample/entry/src/main/cpp/loadPicture.cpp#L30-L32)
5. 定义ImageAuxiliaryPictureNative类。
   ...
   [imageKits.h](https://gitcode.com/HarmonyOS_Samples/guide-snippets/blob/HarmonyOS-feature-20260402/Media/Image/ImageNativeSample/entry/src/main/cpp/imageKits.h#L88-L98)
6. 创建一个ImageAuxiliaryPictureNative实例。
   ...
   [loadPicture.cpp](https://gitcode.com/HarmonyOS_Samples/guide-snippets/blob/HarmonyOS-feature-20260402/Media/Image/ImageNativeSample/entry/src/main/cpp/loadPicture.cpp#L33-L35)
...
8. 创建解码参数，配置解码参数，调用解码接口进行解码并获取辅助图。
   ...
   并非所有图片都包含辅助图。在获取辅助图前，应先调用OH\_PictureNative\_GetAuxiliaryPicture接口尝试获取。其他辅助图类型请参考[Image\_AuxiliaryPictureType](<../../../../../../harmonyos-references/Image Kit（图片处理服务）/C API/头文件/picture_native.h/capi-picture-native-h.md#image_auxiliarypicturetype>)。
   ...
   48. if (length <= 0) {
   49. OH_LOG_INFO(LOG_APP, "Desired auxiliary picture type list is empty.");
   50. return GetJsResult(env, IMAGE_BAD_PARAMETER);
   51. }
   ...
   113. Image_ErrorCode readCode = OH_AuxiliaryPictureNative_ReadPixels(g_thisAuxiliaryPicture ->auxiliaryPicture, buff,
   114. &g_thisAuxiliaryPicture ->buffSize);
   115. if (readCode != IMAGE_SUCCESS) {
   116. OH_LOG_ERROR(LOG_APP, "OH_AuxiliaryPictureNative_ReadPixels failed, errCode: %{public}d.", readCode);
   117. }
   118. OH_AuxiliaryPictureNative_Release(g_thisAuxiliaryPicture ->auxiliaryPicture);
   119. g_thisAuxiliaryPicture ->auxiliaryPicture = nullptr;
   120. delete []buff;
   121. }

   123. if (g_thisPicture->errorCode != IMAGE_SUCCESS) {
   124. OH_LOG_ERROR(LOG_APP, "ImageSourceNative_CreatePicture failed, errCode: %{public}d.",
   125. g_thisPicture->errorCode);
   126. return GetJsResult(env, g_thisPicture->errorCode);
   127. } else {
   128. OH_LOG_DEBUG(LOG_APP, "ImageSourceNative_CreatePicture success !");
   129. }

   131. return GetJsResult(env, g_thisPicture->errorCode);
   132. }
   ```

   [loadPicture.cpp](https://gitcode.com/HarmonyOS_Samples/guide-snippets/blob/HarmonyOS-feature-20260402/Media/Image/ImageNativeSample/entry/src/main/cpp/loadPicture.cpp#L37-L170)
