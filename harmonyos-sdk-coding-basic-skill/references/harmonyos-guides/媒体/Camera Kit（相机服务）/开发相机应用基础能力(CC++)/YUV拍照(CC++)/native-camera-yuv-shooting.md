...
scraped_at: 2026-06-01T11:29:24+08:00
doc_updated_at: 2026-04-20
content_hash: sha256:f4b4c6d2da046c2f1beccc1ddaf2344b05c1e16997980a498a7e2bb5ba4b215e
---
...
详细的相机功能API说明请参考Camera模块描述[OH\_Camera](<../../../../../harmonyos-references/Camera Kit（相机服务）/C API/模块/OH_Camera/capi-oh-camera.md>)。
...
2. 在CMake脚本中链接相关动态库。
   ...
   13. )
   ```
3. 创建并打开相机设备，参考[设备输入(C/C++)](../../开发相机应用必选能力(CC++)/设备输入(CC++)/native-camera-device-input.md)中的步骤3-4。
4. 获取相机设备完整输出能力。

   通过[OH\_CameraManager\_GetSupportedFullCameraOutputCapabilityWithSceneMode()](<../../../../../harmonyos-references/Camera Kit（相机服务）/C API/头文件/camera_manager.h/capi-camera-manager-h.md#oh_cameramanager_getsupportedfullcameraoutputcapabilitywithscenemode>)方法，获取当前设备支持的所有输出流的能力，包含预览流、拍照流、录像流等。输出流在CameraOutputCapability中的各个profile字段中，其中拍照流支持YUV格式。根据相机设备指定模式[Camera\_SceneMode](<../../../../../harmonyos-references/Camera Kit（相机服务）/C API/头文件/camera.h/capi-camera-h.md#camera_scenemode>)的不同，需要添加不同类型的输出流。
...
5. 选择设备支持的输出流能力，创建拍照输出流。
   ...
   可以通过[OH\_CameraManager\_GetSupportedFullCameraOutputCapabilityWithSceneMode()](<../../../../../harmonyos-references/Camera Kit（相机服务）/C API/头文件/camera_manager.h/capi-camera-manager-h.md#oh_cameramanager_getsupportedfullcameraoutputcapabilitywithscenemode>)获取相机在指定模式下支持的完整输出能力cameraOutputCapability，参考步骤2。在cameraOutputCapability的photoProfiles中选择支持YUV格式的profile，作为创建拍照输出流的参数photoProfile。
   ...
   10. }
   ```
6. 注册单段式(PhotoAvailable)或分段式(PhotoAssetAvailable)拍照回调，若应用希望快速得到回图，推荐使用[分段式拍照(PhotoAssetAvailable)](../分段式拍照(CC++)/native-camera-deferred-capture.md)回调。
   ...
   * **单段式拍照（PhotoAvailable）开发流程**：
     ...
     + 在会话[OH\_CaptureSession\_CommitConfig](<../../../../../harmonyos-references/Camera Kit（相机服务）/C API/头文件/capture_session.h/capi-capture-session-h.md#oh_capturesession_commitconfig>)前注册单段式拍照回调。
   ...
   * **分段式拍照（PhotoAvailable）开发流程**：
     ...
     + 在会话[OH\_CaptureSession\_CommitConfig](<../../../../../harmonyos-references/Camera Kit（相机服务）/C API/头文件/capture_session.h/capi-capture-session-h.md#oh_capturesession_commitconfig>)前注册分段式拍照回调。
     + 在分段式拍照回调函数中获取图片信息，解析出pixelMap数据，做自定义业务处理。
     + 将处理完的pixelMap通过回调传给ArkTS侧，做图片显示或通过安全控件写文件保存图片。
     + 调用[OH\_PhotoOutput\_Capture](<../../../../../harmonyos-references/Camera Kit（相机服务）/C API/头文件/photo_output.h/capi-photo-output-h.md#oh_photooutput_capture>)拍照后，需要及时调用[OH\_MediaAssetChangeRequest\_SaveCameraPhoto](<../../../../../harmonyos-references/Media Library Kit（媒体文件管理服务）/C API/头文件/media_asset_change_request_capi.h/capi-media-asset-change-request-capi-h.md#oh_mediaassetchangerequest_savecameraphoto>)保存图片或[OH\_MediaAssetChangeRequest\_DiscardCameraPhoto](<../../../../../harmonyos-references/Media Library Kit（媒体文件管理服务）/C API/头文件/media_asset_change_request_capi.h/capi-media-asset-change-request-capi-h.md#oh_mediaassetchangerequest_discardcameraphoto>)取消保存图片，否则会影响后续图片的拍摄。
     + 使用完后解注册分段式拍照回调函数。
     ...
     115. }
     ```
7. 创建拍照类型会话（参考[会话管理(C/C++)](../../开发相机应用必选能力(CC++)/会话管理(CC++)/native-camera-session-management.md)），开启会话，准备拍照。
8. 触发拍照。

   通过[OH\_PhotoOutput\_Capture()](<../../../../../harmonyos-references/Camera Kit（相机服务）/C API/头文件/photo_output.h/capi-photo-output-h.md#oh_photooutput_capture>)方法，执行拍照任务。
