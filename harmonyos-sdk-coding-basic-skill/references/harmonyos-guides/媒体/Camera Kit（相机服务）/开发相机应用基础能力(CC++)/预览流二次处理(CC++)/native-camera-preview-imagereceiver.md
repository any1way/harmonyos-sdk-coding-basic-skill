...
content_hash: sha256:ed97ef040c45d7266c817b24dbe8f3c96b0d7d9150d22884ad30f75e27b04978
---
...
1. 导入NDK接口，接口中提供了相机相关的属性和方法，导入方法如下。
   ...
   [camera\_manager.h](https://gitcode.com/HarmonyOS_Samples/guide-snippets/blob/HarmonyOS-feature-20260402/Media/Camera/NDKPreviewImageSample/entry/src/main/cpp/camera_manager.h#L19-L40)
2. 在CMake脚本中链接相关动态库。
   ...
   9. )
   ```
3. 初始化图片接收器[ImageReceiver](<../../../Image Kit（图片处理服务）/图片开发指导(CC++)/图片接收/使用Image_NativeModule完成图片接收/image-receiver-c.md>)实例，获取SurfaceId。
   ...
   [napi\_init.cpp](https://gitcode.com/HarmonyOS_Samples/guide-snippets/blob/HarmonyOS-feature-20260402/Media/Camera/NDKPreviewImageSample/entry/src/main/cpp/napi_init.cpp#L177-L223)
4. 通过上一步获取到的SurfaceId创建预览流（在创建预览流之前需要将SurfaceId类型转成char \*），参考[预览(C/C++)](../预览(CC++)/native-camera-preview.md)步骤4。
5. 创建会话，使能会话，参考[会话管理(C/C++)](../../开发相机应用必选能力(CC++)/会话管理(CC++)/native-camera-session-management.md)。
6. 注册ImageReceiver图片接收器的回调，监听获取每帧上报图像内容。
   ...
   [napi\_init.cpp](https://gitcode.com/HarmonyOS_Samples/guide-snippets/blob/HarmonyOS-feature-20260402/Media/Camera/NDKPreviewImageSample/entry/src/main/cpp/napi_init.cpp#L73-L175)
