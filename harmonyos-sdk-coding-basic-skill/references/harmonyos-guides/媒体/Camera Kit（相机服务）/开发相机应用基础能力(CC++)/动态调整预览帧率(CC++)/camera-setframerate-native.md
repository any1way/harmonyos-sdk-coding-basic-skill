...
scraped_at: 2026-06-11T14:56:34+08:00
doc_updated_at: 2026-05-26
content_hash: sha256:232212b3d92713e3fa9584d5c8123ba0706d8657e018428ca04decc68a4f2d34
---
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/3a/v3/b3NzUAz3TX2g79ZnBoI5aQ/zh-cn_image_0000002622858455.png?HW-CC-KV=V1&HW-CC-Date=20260611T065633Z&HW-CC-Expire=86400&HW-CC-Sign=C93C7941C0AC601AAF48EAD53270B289211104838CCFB0048026C1C74A88B18C)

与普通的[预览](../预览(CC++)/native-camera-preview.md)流程相比，动态调整预览帧率的注意点如图上标识：

1. 调用[OH\_CameraManager\_CreateCaptureSession](<../../../../../harmonyos-references/Camera Kit（相机服务）/C API/头文件/camera_manager.h/capi-camera-manager-h.md#oh_cameramanager_createcapturesession>)创建会话（Session）时，需要指定模式为NORMAL\_PHOTO或NORMAL\_VIDEO。
...
如何配置会话（Session）、释放资源，请参考[会话管理](../../开发相机应用必选能力(CC++)/会话管理(CC++)/native-camera-session-management.md) > [预览](../预览(CC++)/native-camera-preview.md)。
...
相机使用预览等功能前，均需创建相机会话，调用[OH\_CameraManager\_CreateCaptureSession](<../../../../../harmonyos-references/Camera Kit（相机服务）/C API/头文件/camera_manager.h/capi-camera-manager-h.md#oh_cameramanager_createcapturesession>)创建一个会话。

创建会话时调用[OH\_CaptureSession\_SetSessionMode](<../../../../../harmonyos-references/Camera Kit（相机服务）/C API/头文件/capture_session.h/capi-capture-session-h.md#oh_capturesession_setsessionmode>)指定[Camera\_SceneMode](<../../../../../harmonyos-references/Camera Kit（相机服务）/C API/头文件/camera.h/capi-camera-h.md#camera_scenemode>)为NORMAL\_PHOTO或NORMAL\_VIDEO，创建出的Session处于拍照或录像模式。
...
1. 调用[OH\_PreviewOutput\_GetSupportedFrameRates](<../../../../../harmonyos-references/Camera Kit（相机服务）/C API/头文件/preview_output.h/capi-preview-output-h.md#oh_previewoutput_getsupportedframerates>)，查询当前previewOutput支持的帧率范围。

   说明

   需要在Session调用[OH\_CaptureSession\_CommitConfig](<../../../../../harmonyos-references/Camera Kit（相机服务）/C API/头文件/capture_session.h/capi-capture-session-h.md#oh_capturesession_commitconfig>)完成配流之后调用。
   ...
   14. }
   ```
2. 根据实际开发需求，调用[OH\_PreviewOutput\_SetFrameRate](<../../../../../harmonyos-references/Camera Kit（相机服务）/C API/头文件/preview_output.h/capi-preview-output-h.md#oh_previewoutput_setframerate>)接口对帧率进行动态调整。
   ...
   * 需要在Session调用[OH\_CaptureSession\_CommitConfig](<../../../../../harmonyos-references/Camera Kit（相机服务）/C API/头文件/capture_session.h/capi-capture-session-h.md#oh_capturesession_commitconfig>)完成配流之后调用。
   * 可在Session调用[OH\_CaptureSession\_Start](<../../../../../harmonyos-references/Camera Kit（相机服务）/C API/头文件/capture_session.h/capi-capture-session-h.md#oh_capturesession_start>)启动预览前后任意时刻调用。
   ...
   * 在调用OH\_PreviewOutput\_SetFrameRate接口设置固定帧率后，支持重新设置固定帧率，但必须保证新设置的帧率可以整除之前设置的帧率或者被之前设置的帧率整除。
   ...
   8. }
   ```
3. （可选）通过[OH\_PreviewOutput\_GetActiveFrameRate](<../../../../../harmonyos-references/Camera Kit（相机服务）/C API/头文件/preview_output.h/capi-preview-output-h.md#oh_previewoutput_getactiveframerate>)接口查询已设置过并生效的帧率。

   仅通过[OH\_PreviewOutput\_SetFrameRate](<../../../../../harmonyos-references/Camera Kit（相机服务）/C API/头文件/preview_output.h/capi-preview-output-h.md#oh_previewoutput_setframerate>)接口显式设置过帧率才可查询当前生效帧率信息。
