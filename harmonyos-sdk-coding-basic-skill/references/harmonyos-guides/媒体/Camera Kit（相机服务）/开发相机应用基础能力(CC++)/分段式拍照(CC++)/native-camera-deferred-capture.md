...
content_hash: sha256:9a587fa295eb9b14d21643a954cdb6570de7fb70ffe93df9a3933bd984b597f0
---
...
1. 导入NDK接口，接口中提供了相机相关的属性和方法，导入方法如下。
   ...
   [camera\_manager.h](https://gitcode.com/HarmonyOS_Samples/guide-snippets/blob/HarmonyOS-feature-20260402/Media/Camera/NDKDeferredCaptureSample/entry/src/main/cpp/camera_manager.h#L18-L55)
2. 在CMake脚本中链接相关动态库。
   ...
   8. )
   ```
3. 相机初始化及拍照触发参考[拍照(C/C++)](../拍照(CC++)/native-camera-shooting.md)。
4. 注册分段式（PhotoAssetAvailable）拍照回调，对比单段式拍照，仅注册的拍照回调接口不同。
   ...
   * 通过mediaAsset直接落盘图片或者通过mediaAsset配置策略模式请求图像资源，业务处理后通过buffer保存图片，或显示图片(参考[拍照(C/C++)](../拍照(CC++)/native-camera-shooting.md)步骤5)。
   * 使用完后解注册分段式拍照回调函数。
   ...
   [camera\_manager.cpp](https://gitcode.com/HarmonyOS_Samples/guide-snippets/blob/HarmonyOS-feature-20260402/Media/Camera/NDKDeferredCaptureSample/entry/src/main/cpp/camera_manager.cpp#L1654-L1847)
