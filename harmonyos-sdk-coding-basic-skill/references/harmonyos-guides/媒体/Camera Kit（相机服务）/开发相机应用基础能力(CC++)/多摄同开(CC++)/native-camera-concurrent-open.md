...
content_hash: sha256:06b8a9c128348768fce97df467eda6112636c17f7fc03a9310084f7ab062d80a
---
...
详细的API说明请参考[Camera](<../../../../../harmonyos-references/Camera Kit（相机服务）/C API/模块/OH_Camera/capi-oh-camera.md>)。
...
2. 在CMake脚本中链接相关动态库。
   ...
   6. librawfile.z.so)
   ```
3. 通过[OH\_CameraManager\_GetCameraDevice](<../../../../../harmonyos-references/Camera Kit（相机服务）/C API/头文件/camera_manager.h/capi-camera-manager-h.md#oh_cameramanager_getcameradevice>)方法获取对应的前置相机和后置相机。如果接口返回值为CAMERA\_SERVICE\_FATAL\_ERROR，基于示例中的配置信息，表示当前设备不支持指定位置（前置/后置）的默认类型相机，无法实现多摄同开功能。
   ...
   15. }
   ```
4. 获取对应的并发能力集。通过[OH\_CameraManager\_GetCameraConcurrentInfos](<../../../../../harmonyos-references/Camera Kit（相机服务）/C API/头文件/camera_manager.h/capi-camera-manager-h.md#oh_cameramanager_getcameraconcurrentinfos>)方法，获取[Camera\_ConcurrentInfo](<../../../../../harmonyos-references/Camera Kit（相机服务）/C API/结构体/Camera_ConcurrentInfo/capi-oh-camera-camera-concurrentinfo.md>)相机并发能力集对象数组，数组包含了前置和后置同时开启的相机在指定多摄同开模式下支持的相机模式和相机输出能力，**在多摄同开场景下设置的模式和输出能力必须在并发能力集的范围之内**。
...
6. 打开相机。通过[OH\_CameraInput\_OpenConcurrentCameras](<../../../../../harmonyos-references/Camera Kit（相机服务）/C API/头文件/camera_input.h/capi-camera-input-h.md#oh_camerainput_openconcurrentcameras>)以并发状态打开指定相机，在使用该接口前，请先查询接口是否支持并发能力集，并优先调用[OH\_CameraManager\_GetCameraConcurrentInfos](<../../../../../harmonyos-references/Camera Kit（相机服务）/C API/头文件/camera_manager.h/capi-camera-manager-h.md#oh_cameramanager_getcameraconcurrentinfos>)方法，获取多摄同开状态下的相机并发能力集。请勿在未查询并发能力集的情况下使用[OH\_CameraInput\_OpenConcurrentCameras](<../../../../../harmonyos-references/Camera Kit（相机服务）/C API/头文件/camera_input.h/capi-camera-input-h.md#oh_camerainput_openconcurrentcameras>)，否则会导致打开相机失败。
   ...
   13. }
   ```
7. 会话流程。配置对应的相机输入流[Camera\_Input](<../../../../../harmonyos-references/Camera Kit（相机服务）/C API/结构体/Camera_Input/capi-oh-camera-camera-input.md>)和需要的相机输出流，此处以预览输出流[Camera\_PreviewOutput](<../../../../../harmonyos-references/Camera Kit（相机服务）/C API/结构体/Camera_PreviewOutput/capi-oh-camera-camera-previewoutput.md>)为例 。
