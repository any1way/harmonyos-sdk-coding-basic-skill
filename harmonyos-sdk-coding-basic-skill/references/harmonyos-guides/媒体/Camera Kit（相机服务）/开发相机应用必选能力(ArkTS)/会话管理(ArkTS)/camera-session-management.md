...
scraped_at: 2026-06-01T11:28:32+08:00
doc_updated_at: 2026-03-23
content_hash: sha256:591fc742d675dcba1a3fda3f00dc94b925dc4d997bb77c17780dc6d4abd969b6
---
...
* 配置相机的输入流和输出流。相机在拍摄前，必须完成输入输出流的配置。
  ...
  配置输入流即添加设备输入，对用户而言，相当于选择设备的某一相机拍摄；配置输出流，即选择数据将以什么形式输出。当应用需要实现拍照时，输出流应配置为预览流和拍照流，预览流的数据将显示在[XComponent](../../../../../harmonyos-references/ArkUI（方舟UI框架）/ArkTS组件/渲染绘制/XComponent/ts-basic-components-xcomponent.md)组件上，拍照流的数据将通过[ImageReceiver](<../../../../../harmonyos-references/Image Kit（图片处理服务）/ArkTS API/@ohos.multimedia.image (图片处理)/Interface (ImageReceiver)/arkts-apis-image-imagereceiver.md>)接口的能力保存到相册中。
* 添加闪光灯、调整焦距等配置。具体支持的配置及接口说明请参考[@ohos.multimedia.camera (相机管理)](<../../../../../harmonyos-references/Camera Kit（相机服务）/ArkTS API/@ohos.multimedia.camera (相机管理)/模块描述/arkts-apis-camera.md>)。
...
2. 调用[cameraManager](<../../../../../harmonyos-references/Camera Kit（相机服务）/ArkTS API/@ohos.multimedia.camera (相机管理)/Interface (CameraManager)/arkts-apis-camera-cameramanager.md>)中的[createSession](<../../../../../harmonyos-references/Camera Kit（相机服务）/ArkTS API/@ohos.multimedia.camera (相机管理)/Interface (CameraManager)/arkts-apis-camera-cameramanager.md#createsession11>)方法创建一个会话。
   ...
   11. }
   ```
3. 调用[VideoSession](<../../../../../harmonyos-references/Camera Kit（相机服务）/ArkTS API/@ohos.multimedia.camera (相机管理)/Interface (VideoSession)/arkts-apis-camera-videosession.md>)中的[beginConfig](<../../../../../harmonyos-references/Camera Kit（相机服务）/ArkTS API/@ohos.multimedia.camera (相机管理)/Interface (Session)/arkts-apis-camera-session.md#beginconfig11>)方法配置会话。
   ...
   8. }
   ```
4. 使能。向会话中添加相机的输入流和输出流，调用[addInput](<../../../../../harmonyos-references/Camera Kit（相机服务）/ArkTS API/@ohos.multimedia.camera (相机管理)/Interface (Session)/arkts-apis-camera-session.md#addinput11>)添加相机的输入流；调用[addOutput](<../../../../../harmonyos-references/Camera Kit（相机服务）/ArkTS API/@ohos.multimedia.camera (相机管理)/Interface (Session)/arkts-apis-camera-session.md#addoutput11>)添加相机的输出流。以下示例代码以添加预览流previewOutput和拍照流photoOutput为例，即当前模式支持拍照和预览。

   调用VideoSession中的[commitConfig](<../../../../../harmonyos-references/Camera Kit（相机服务）/ArkTS API/@ohos.multimedia.camera (相机管理)/Interface (Session)/arkts-apis-camera-session.md#commitconfig11>)和[start](<../../../../../harmonyos-references/Camera Kit（相机服务）/ArkTS API/@ohos.multimedia.camera (相机管理)/Interface (Session)/arkts-apis-camera-session.md#start11>)方法提交相关配置，并启动会话。

   说明

   在调用[addOutput](<../../../../../harmonyos-references/Camera Kit（相机服务）/ArkTS API/@ohos.multimedia.camera (相机管理)/Interface (Session)/arkts-apis-camera-session.md#addoutput11>)添加相机的输出流前，可通过[canAddOutput](<../../../../../harmonyos-references/Camera Kit（相机服务）/ArkTS API/@ohos.multimedia.camera (相机管理)/Interface (Session)/arkts-apis-camera-session.md#canaddoutput11>)判断当前相机输出流是否可以添加到session中。

   相机输入流cameraInput创建流程请参考[设备输入](../设备输入(ArkTS)/camera-device-input.md)，相机预览输出流previewOutput和拍照输出流photoOutput创建流程请分别参考[预览](../../开发相机应用基础能力(ArkTS)/预览(ArkTS)/camera-preview.md)和[拍照](../../开发相机应用基础能力(ArkTS)/拍照(ArkTS)/camera-shooting.md)。
   ...
   56. }
   ```
5. 会话控制。调用VideoSession中的[stop](<../../../../../harmonyos-references/Camera Kit（相机服务）/ArkTS API/@ohos.multimedia.camera (相机管理)/Interface (Session)/arkts-apis-camera-session.md#stop11>)方法可以停止当前会话。调用[removeOutput](<../../../../../harmonyos-references/Camera Kit（相机服务）/ArkTS API/@ohos.multimedia.camera (相机管理)/Interface (Session)/arkts-apis-camera-session.md#removeoutput11>)和[addOutput](<../../../../../harmonyos-references/Camera Kit（相机服务）/ArkTS API/@ohos.multimedia.camera (相机管理)/Interface (Session)/arkts-apis-camera-session.md#addoutput11>)方法可以完成会话切换控制。以下示例代码以移除拍照流photoOutput，添加视频流videoOutput为例，完成了拍照到录像的切换。
