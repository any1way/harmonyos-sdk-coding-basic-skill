...
content_hash: sha256:ddf2657a70c1dee25e99402fe8185e476181c8738f1d7587c2ddb1b8f87ea60a
---
...
2. 在CMake脚本中链接相关动态库。
   ...
   5. )
   ```
3. 通过[OH\_Camera\_GetCameraManager()](<../../../../../harmonyos-references/Camera Kit（相机服务）/C API/头文件/camera.h/capi-camera-h.md#oh_camera_getcameramanager>)方法，获取cameraManager对象。
...
4. 通过[OH\_CameraManager\_GetSupportedCameras()](<../../../../../harmonyos-references/Camera Kit（相机服务）/C API/头文件/camera_manager.h/capi-camera-manager-h.md#oh_cameramanager_getsupportedcameras>)方法，获取当前设备支持的相机列表，列表中存储了设备支持的所有相机ID。若列表不为空，则说明列表中的每个ID都支持独立创建相机对象；否则，说明当前设备无可用相机，无法进行后续操作。
   ...
   7. }
   8. // 在不使用cameras时，需要调用OH_CameraManager_DeleteSupportedCameras()方法释放相机列表并置空指针防止内存泄漏。
...
通过[OH\_CameraManager\_RegisterCallback()](<../../../../../harmonyos-references/Camera Kit（相机服务）/C API/头文件/camera_manager.h/capi-camera-manager-h.md#oh_cameramanager_registercallback>)注册cameraStatus事件，通过回调返回监听结果，callback返回Camera\_StatusInfo参数，参数的具体内容可参考相机管理器回调接口实例[Camera\_StatusInfo](<../../../../../harmonyos-references/Camera Kit（相机服务）/C API/结构体/Camera_StatusInfo/capi-oh-camera-camera-statusinfo.md>)。
