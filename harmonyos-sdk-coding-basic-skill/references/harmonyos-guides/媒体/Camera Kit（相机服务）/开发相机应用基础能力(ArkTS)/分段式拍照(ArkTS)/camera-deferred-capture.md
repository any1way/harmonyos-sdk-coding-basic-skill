...
scraped_at: 2026-06-11T14:55:47+08:00
doc_updated_at: 2026-06-03
content_hash: sha256:376074597d9891e69ea1cd669cb76187fcf7871c76192211b810271a727916cb
---
分段式拍照是相机的重要功能之一，即应用下发拍照任务后，系统将分多阶段上报不同质量的图片。

* 在第一阶段，系统快速上报轻量处理的图片，轻量处理的图片比全质量图低，出图速度快。应用通过回调会收到一个[PhotoAsset](<../../../../../harmonyos-references/Media Library Kit（媒体文件管理服务）/ArkTS API/@ohos.file.photoAccessHelper (相册管理模块)/Interface (PhotoAsset)/arkts-apis-photoaccesshelper-photoasset.md>)对象，通过该对象可调用媒体库接口，读取图片或落盘图片。
...
* 通过PhotoOutput，监听photoAssetAvailable回调，获取[photoAccessHelper](<../../../../../harmonyos-references/Media Library Kit（媒体文件管理服务）/ArkTS API/@ohos.file.photoAccessHelper (相册管理模块)/模块描述/arkts-apis-photoaccesshelper.md>)的PhotoAsset对象。
...
3. 设置拍照photoAssetAvailable的回调。
   ...
   不建议开发者同时注册[photoAvailable](<../../../../../harmonyos-references/Camera Kit（相机服务）/ArkTS API/@ohos.multimedia.camera (相机管理)/Interface (PhotoOutput)/arkts-apis-camera-photooutput.md#onphotoavailable11>)和[photoAssetAvailable](<../../../../../harmonyos-references/Camera Kit（相机服务）/ArkTS API/@ohos.multimedia.camera (相机管理)/Interface (PhotoOutput)/arkts-apis-camera-photooutput.md#onphotoassetavailable12>)。
   ...
   请求图片参考媒体库接口：[requestImageData](<../../../../../harmonyos-references/Media Library Kit（媒体文件管理服务）/ArkTS API/@ohos.file.photoAccessHelper (相册管理模块)/Class (MediaAssetManager)/arkts-apis-photoacce_03248920.md#requestimagedata11>) 和 [onDataPrepared](<../../../../../harmonyos-references/Media Library Kit（媒体文件管理服务）/ArkTS API/@ohos.file.photoAccessHelper (相册管理模块)/Interface (MediaAssetDataHandler)/arkts-apis-p_59162830.md#ondataprepared11>)
4. 拍照时的会话配置及触发拍照的方式，与普通拍照相同，请参考[拍照](../拍照(ArkTS)/camera-shooting.md)的步骤4-5。
...
* 通过注册固定的captureReady回调函数获取监听可拍下一张结果，photoOutput创建成功时即可监听，当下一张可拍时触发，该事件返回结果为下一张可拍的相关信息。
  ...
  8. }
  ```
* 通过注册固定的error回调函数获取监听拍照输出流的错误结果。callback返回拍照输出接口使用错误时的对应错误码，错误码类型参见[CameraErrorCode](<../../../../../harmonyos-references/Camera Kit（相机服务）/ArkTS API/@ohos.multimedia.camera (相机管理)/Enums/arkts-apis-camera-e.md#cameraerrorcode>)。
