...
scraped_at: 2026-06-11T14:56:04+08:00
doc_updated_at: 2026-06-05
content_hash: sha256:620cc0990f7b42ac52176da6b519a554f1ed633012ad4ff0b1e85512bbb77317
---
屏幕处于不同的屏幕状态时，原始图像需旋转不同的角度，以确保图像在合适的方向显示，效果如图所示。

![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/38/v3/rpYjQKIjRIOsrBL3Sj-rww/zh-cn_image_0000002622858443.png?HW-CC-KV=V1&HW-CC-Date=20260611T065602Z&HW-CC-Expire=86400&HW-CC-Sign=13687D71DE329EB4DA98BFD2963AC9750E26AF5C9CBACF62FE26071B58B0E951)

本开发指导将指导开发者在预览、拍照、录像等不同场景下，如何适配相机的旋转角度。

* 在预览时，图像旋转角度与屏幕显示旋转角度（[Display](<../../../../../../harmonyos-references/ArkUI（方舟UI框架）/ArkTS API/屏幕管理/@ohos.display (屏幕属性)/js-apis-display.md#属性>).rotation）相关。具体开发指导：[创建会话](camera-rotation-angle-adaptation.md#创建会话) > [预览](camera-rotation-angle-adaptation.md#预览)。
...
详细的API参考说明，请参考[Camera API文档](<../../../../../../harmonyos-references/Camera Kit（相机服务）/ArkTS API/@ohos.multimedia.camera (相机管理)/js-apis-camera.md>)。
...
1. 导入相机等相关模块。
   ...
   ```
2. 创建[Session](<../../../../../../harmonyos-references/Camera Kit（相机服务）/ArkTS API/@ohos.multimedia.camera (相机管理)/Interface (Session)/arkts-apis-camera-session.md>)会话。

   相机使用预览等功能前，均需创建相机会话，调用[CameraManager](<../../../../../../harmonyos-references/Camera Kit（相机服务）/ArkTS API/@ohos.multimedia.camera (相机管理)/Interface (CameraManager)/arkts-apis-camera-cameramanager.md>)中的[createSession](<../../../../../../harmonyos-references/Camera Kit（相机服务）/ArkTS API/@ohos.multimedia.camera (相机管理)/Interface (CameraManager)/arkts-apis-camera-cameramanager.md#createsession11>)方法创建一个会话，创建会话时需指定创建[SceneMode](<../../../../../../harmonyos-references/Camera Kit（相机服务）/ArkTS API/@ohos.multimedia.camera (相机管理)/Enums/arkts-apis-camera-e.md#scenemode11>)为NORMAL\_PHOTO或NORMAL\_VIDEO，创建的session处于拍照或者录像模式。

   ```
   1. createPhotoSession(cameraManager: camera.CameraManager): camera.Session | undefined {
   ...
   12. createVideoSession(cameraManager: camera.CameraManager): camera.Session | undefined {
...
1. 调用[PreviewOutput](<../../../../../../harmonyos-references/Camera Kit（相机服务）/ArkTS API/@ohos.multimedia.camera (相机管理)/Interface (PreviewOutput)/arkts-apis-camera-previewoutput.md>)中的[getPreviewRotation](<../../../../../../harmonyos-references/Camera Kit（相机服务）/ArkTS API/@ohos.multimedia.camera (相机管理)/Interface (PreviewOutput)/arkts-apis-camera-previewoutput.md#getpreviewrotation12>)接口，获取[预览旋转角度](../相机旋转角度的术语/camera-rotation-term.md#预览旋转角度)。

   displayRotation：[显示设备的屏幕旋转角度](../相机旋转角度的术语/camera-rotation-term.md#屏幕旋转角度)，可通过[display.getDefaultDisplaySync](<../../../../../../harmonyos-references/ArkUI（方舟UI框架）/ArkTS API/屏幕管理/@ohos.display (屏幕属性)/js-apis-display.md#displaygetdefaultdisplaysync9>)获取[Display](<../../../../../../harmonyos-references/ArkUI（方舟UI框架）/ArkTS API/屏幕管理/@ohos.display (屏幕属性)/js-apis-display.md#属性>)对象并读取其rotation属性值，并将对应角度填入。
   ...
   该接口需要在session调用[commitConfig](<../../../../../../harmonyos-references/Camera Kit（相机服务）/ArkTS API/@ohos.multimedia.camera (相机管理)/Interface (Session)/arkts-apis-camera-session.md#commitconfig11>)完成配流后调用，如果存在异步执行的情况，previewOutput未添加到session里或者已调用的session.release，导致两者关系未绑定，此时调用getPreviewRotation，则会调用失败，并抛出错误码[CameraErrorCode.SERVICE\_FATAL\_ERROR](<../../../../../../harmonyos-references/Camera Kit（相机服务）/错误码/Camera错误码/errorcode-camera.md#section7400201-相机服务异常>)。
   ...
   12. }
   ```
2. 调用[PreviewOutput](<../../../../../../harmonyos-references/Camera Kit（相机服务）/ArkTS API/@ohos.multimedia.camera (相机管理)/Interface (PreviewOutput)/arkts-apis-camera-previewoutput.md>)中的[setPreviewRotation](<../../../../../../harmonyos-references/Camera Kit（相机服务）/ArkTS API/@ohos.multimedia.camera (相机管理)/Interface (PreviewOutput)/arkts-apis-camera-previewoutput.md#setpreviewrotation12>)，设置图像的预览旋转角度。

   该接口需要在session调用[commitConfig](<../../../../../../harmonyos-references/Camera Kit（相机服务）/ArkTS API/@ohos.multimedia.camera (相机管理)/Interface (Session)/arkts-apis-camera-session.md#commitconfig11>)完成配流后调用，如果多次调用，以最新调用设置的图像预览旋转角度为准。

   * previewRotation：预览旋转角度，取上一步[getPreviewRotation](<../../../../../../harmonyos-references/Camera Kit（相机服务）/ArkTS API/@ohos.multimedia.camera (相机管理)/Interface (PreviewOutput)/arkts-apis-camera-previewoutput.md#getpreviewrotation12>)的返回值。
   * isDisplayLocked：可选入参，默认为false。当设置为false，即屏幕方向未锁定，[预览旋转角度](../相机旋转角度的术语/camera-rotation-term.md#预览旋转角度)将根据[相机镜头角度](../相机旋转角度的术语/camera-rotation-term.md#相机镜头安装角度)+[屏幕显示旋转角度](../相机旋转角度的术语/camera-rotation-term.md#屏幕旋转角度)的值计算；当设置为true，Surface旋转锁定，不跟随窗口变化，旋转角度仅取相机镜头角度计算。
...
1. 在[会话配置](../../../开发相机应用必选能力(ArkTS)/会话管理(ArkTS)/camera-session-management.md)过程中调用预览旋转接口，即：使用[commitConfig](<../../../../../../harmonyos-references/Camera Kit（相机服务）/ArkTS API/@ohos.multimedia.camera (相机管理)/Interface (Session)/arkts-apis-camera-session.md#commitconfig11>)接口提交相关配置后调用，建议在[Start](<../../../../../../harmonyos-references/Camera Kit（相机服务）/ArkTS API/@ohos.multimedia.camera (相机管理)/Interface (Session)/arkts-apis-camera-session.md#start11>)起流前调用。

   ```
   1. // previewOutput是创建的预览输出
   2. try {
   3. let initDisplayRotation = display.getDefaultDisplaySync().rotation;
   4. let initPreviewRotation = this.output?.getPreviewRotation(initDisplayRotation * camera.ImageRotation.ROTATION_90);
   5. let isDisplayLocked: boolean = false; // 建议与setXComponentSurfaceRotation入参的lock属性保持一致
   6. this.output?.setPreviewRotation(initPreviewRotation, isDisplayLocked);
   7. } catch (error) {
   8. // 失败返回错误码error.code并处理
   9. let err = error as BusinessError;
   10. console.error(`initPreviewRotation call failed. error code: ${err.code}`);
   11. }
   ```
2. 应用使用相机时，通过监听[Display对象变化](<../../../../../../harmonyos-references/ArkUI（方舟UI框架）/ArkTS API/屏幕管理/@ohos.display (屏幕属性)/js-apis-display.md#displayonaddremovechange>)，感知窗口当前状态，如当前相机窗口发生旋转时，需对预览流进行角度修正。推荐在[会话配置](../../../开发相机应用必选能力(ArkTS)/会话管理(ArkTS)/camera-session-management.md)中完成调用预览旋转接口后，直接创建监听。
   ...
   10. let isDisplayLocked: boolean = false; // 建议与setXComponentSurfaceRotation入参的lock属性保持一致
   11. previewOutput.setPreviewRotation(previewRotation, isDisplayLocked);
   12. } catch (error) {
   13. // 失败返回错误码error.code并处理
   14. let err = error as BusinessError;
   15. console.error(`display change PreviewRotation call failed. error code: ${err.code}`);
   16. }
   17. });
   ```
...
1. 调用[PhotoOutput](<../../../../../../harmonyos-references/Camera Kit（相机服务）/ArkTS API/@ohos.multimedia.camera (相机管理)/Interface (PhotoOutput)/arkts-apis-camera-photooutput.md>)中的[getPhotoRotation](<../../../../../../harmonyos-references/Camera Kit（相机服务）/ArkTS API/@ohos.multimedia.camera (相机管理)/Interface (PhotoOutput)/arkts-apis-camera-photooutput.md#getphotorotation12>)可以获取到拍照旋转角度。

   该接口需要在session调用[commitConfig](<../../../../../../harmonyos-references/Camera Kit（相机服务）/ArkTS API/@ohos.multimedia.camera (相机管理)/Interface (Session)/arkts-apis-camera-session.md#commitconfig11>)完成配流后调用。

   deviceDegree：设备旋转角度。拍照的旋转角度与重力方向（即设备旋转角度）相关，获取方式请见[计算设备旋转角度](camera-rotation-angle-adaptation.md#计算设备旋转角度)。

   ```
   1. getPhotoRotation(photoOutput: camera.PhotoOutput, deviceDegree: number): camera.ImageRotation {
   ...
   [PhotoManager.ets](https://gitcode.com/HarmonyOS_Samples/guide-snippets/blob/HarmonyOS-feature-20260402/Media/Camera/RotationDisplayCamera/camera/src/main/ets/cameramanagers/PhotoManager.ets#L319-L330)
2. 应用将拍照角度写入[PhotoCaptureSetting](<../../../../../../harmonyos-references/Camera Kit（相机服务）/ArkTS API/@ohos.multimedia.camera (相机管理)/Interfaces (其他)/arkts-apis-camera-i.md#photocapturesetting>).rotation。
3. 其余参数的配置及拍照，可参考[拍照开发指导](../../拍照(ArkTS)/camera-shooting.md)。

## 录像

完成[会话创建](camera-rotation-angle-adaptation.md#创建会话)后，开发者可根据实际需求，配置输出流。

1. 调用[VideoOutput](<../../../../../../harmonyos-references/Camera Kit（相机服务）/ArkTS API/@ohos.multimedia.camera (相机管理)/Interface (VideoOutput)/arkts-apis-camera-videooutput.md>)中的[getVideoRotation](<../../../../../../harmonyos-references/Camera Kit（相机服务）/ArkTS API/@ohos.multimedia.camera (相机管理)/Interface (VideoOutput)/arkts-apis-camera-videooutput.md#getvideorotation12>)可以获取到录像的旋转角度。
   ...
   ```
   1. getVideoRotation(videoOutput: camera.VideoOutput, deviceDegree: number): camera.ImageRotation {
   ...
   [VideoManager.ets](https://gitcode.com/HarmonyOS_Samples/guide-snippets/blob/HarmonyOS-feature-20260402/Media/Camera/RotationDisplayCamera/camera/src/main/ets/cameramanagers/VideoManager.ets#L347-L358)
2. 在[AVRecorder.prepare](<../../../../../../harmonyos-references/Media Kit（媒体服务）/ArkTS API/@ohos.multimedia.media (媒体服务)/Interface (AVRecorder)/arkts-apis-media-avrecorder.md#prepare9>)后使用[updateRotation](<../../../../../../harmonyos-references/Media Kit（媒体服务）/ArkTS API/@ohos.multimedia.media (媒体服务)/Interface (AVRecorder)/arkts-apis-media-avrecorder.md#updaterotation12>)设置录像角度。
3. 其余参数的配置及启动录像，可参考[录像开发指导](../../录像(ArkTS)/camera-recording.md)。

**录像流旋转接口适配示例代码：**

```
1. async getVideoRotationAndUpdate(videoOutput: camera.VideoOutput, deviceDegree: number, avRecorder: media.AVRecorder) {
2. let videoRotation: camera.ImageRotation = camera.ImageRotation.ROTATION_0;
3. try {
4. videoRotation = videoOutput.getVideoRotation(deviceDegree);
5. console.info(`Video rotation is: ${videoRotation}`);
6. if (avRecorder.state === 'prepared') {
7. await avRecorder.updateRotation(videoRotation);
8. }
9. } catch (error) {
10. // 失败返回错误码error.code并处理
11. let err = error as BusinessError;
12. console.error(`getVideoRotationAndUpdate call failed. error code: ${err.code}`);
13. }
14. }
```
...
当前可通过调用[once(type: SensorId.GRAVITY, callback: Callback<GravityResponse>)](<../../../../../../harmonyos-references/Sensor Service Kit（传感器服务）/ArkTS API/@ohos.sensor (传感器)/js-apis-sensor.md#gravity9-1>)获取一次重力传感器在x、y、z三个方向上的数据，计算得出设备旋转角度deviceDegree，示例如下所示。

如果无法获得重力传感器数据，需要申请重力传感器权限ohos.permission.ACCELEROMETER。权限申请请参考[声明权限](../../../../../系统/安全/程序访问控制/应用权限管控/申请应用权限/声明权限/declare-permissions.md)，如何获取传感器数据请参考[传感器开发指导](<../../../../../系统/硬件/Sensor Service Kit（传感器服务）/传感器/传感器开发指导(ArkTS)/sensor-guidelines.md>)。

```
1. getRealData(data: sensor.GravityResponse): number {
2. let getDeviceDegree: number = 0;
3. let x = data.x;
4. let y = data.y;
5. let z = data.z;
6. if ((x * x + y * y) * 3 < z * z) {
7. return getDeviceDegree;
8. } else {
9. try {
10. let sd: Decimal = Decimal.atan2(y, -x);
11. let sc: Decimal = Decimal.round(Number(sd) / 3.141592653589 * 180);
12. getDeviceDegree = 90 - Number(sc);
13. getDeviceDegree = getDeviceDegree >= 0 ? getDeviceDegree % 360 : getDeviceDegree % 360 + 360;
14. } catch (error) {
15. let err = error as BusinessError;
16. console.error(`decimal failed, error: ${err.code}`);
17. }
18. }
19. return getDeviceDegree;
20. }

22. async getGravity() : Promise<number> {
23. let data: sensor.Sensor[];
24. let isSupported: boolean = false;
25. try {
26. data = await sensor.getSensorList();
27. } catch (error) {
28. let err = error as BusinessError;
29. console.error(`getSensorList failed, error: ${err.code}`);
30. return -1; // 异常场景下返回默认值
31. }

33. for (let i = 0; i < data.length; i++) {
34. if (data[i].sensorId === sensor.SensorId.GRAVITY) {
35. isSupported = true;
36. break;
37. }
38. }
...
41. const promise: Promise<number> = new Promise((resolve) => {
42. sensor.once(sensor.SensorId.GRAVITY, (data: sensor.GravityResponse) => {
43. resolve(this.getRealData(data));
44. });
45. })
46. return promise;
47. } else {
48. const promise: Promise<number> = new Promise((resolve) => {
49. sensor.once(sensor.SensorId.ACCELEROMETER, (data: sensor.AccelerometerResponse) => {
50. resolve(this.getRealData(data as sensor.GravityResponse));
51. });
52. })
53. return promise;
54. }
...
57. console.error(`gePromise failed, error: ${err.code}`);
58. return -1; // 异常场景下返回默认值
59. }
60. }

62. // 获取当前设备旋转角度
63. async getCurrentDeviceDegree() : Promise<number> {
64. getDeviceDegree = await getGravity(); // 调用使用await
65. return getDeviceDegree;
66. }
```
...
在部分折叠屏设备上，[不同折叠状态](<../../../../../../harmonyos-references/ArkUI（方舟UI框架）/ArkTS API/屏幕管理/@ohos.display (屏幕属性)/js-apis-display.md#foldstatus10>)下的[设备自然方向](../相机旋转角度的术语/camera-rotation-term.md#设备自然方向)会发生改变，导致不同折叠状态下真实的[相机镜头安装角度](../相机旋转角度的术语/camera-rotation-term.md#相机镜头安装角度)不同。为了屏蔽不同设备间的差异，使得不同折叠状态下的相机镜头安装角度一致，系统会自动调整部分折叠状态下的相机采集图像方向（通过旋转裁切的方式）和相机镜头安装角度，因此会存在视场角（Field of View, FOV）损失，可能会导致相机预览、拍照、录像可见范围降低，因此如果需要实现相机无损出图，可以通过[usePhysicalCameraOrientation](<../../../../../../harmonyos-references/Camera Kit（相机服务）/ArkTS API/@ohos.multimedia.camera (相机管理)/Interface (CameraInput)/arkts-apis-camera-camerainput.md#usephysicalcameraorientation22>)接口来实现相机无损出图。具体方式如下：

设备是否支持无损出图，首先需要确认设备的相机镜头安装角度是否可变，可以通过[isPhysicalCameraOrientationVariable](<../../../../../../harmonyos-references/Camera Kit（相机服务）/ArkTS API/@ohos.multimedia.camera (相机管理)/Interface (CameraInput)/arkts-apis-camera-camerainput.md#isphysicalcameraorientationvariable22>)接口查询。

1. 当相机镜头安装角度不可变时，不同折叠状态下的相机出图均为无损出图。
2. 当相机镜头安装角度可变时：

   * 如应用需要实现相机无损出图，由于相机镜头安装角度与相机旋转相关，需要应用完成[相机旋转的适配](camera-rotation-angle-adaptation.md#top)后，通过[getPhysicalCameraOrientation](<../../../../../../harmonyos-references/Camera Kit（相机服务）/ArkTS API/@ohos.multimedia.camera (相机管理)/Interface (CameraInput)/arkts-apis-camera-camerainput.md#getphysicalcameraorientation22>)接口获取设备当前折叠状态下真实的相机镜头安装角度，并通过[usePhysicalCameraOrientation](<../../../../../../harmonyos-references/Camera Kit（相机服务）/ArkTS API/@ohos.multimedia.camera (相机管理)/Interface (CameraInput)/arkts-apis-camera-camerainput.md#usephysicalcameraorientation22>)接口实现相机无损出图（相机镜头安装角度不可变时使用[usePhysicalCameraOrientation](<../../../../../../harmonyos-references/Camera Kit（相机服务）/ArkTS API/@ohos.multimedia.camera (相机管理)/Interface (CameraInput)/arkts-apis-camera-camerainput.md#usephysicalcameraorientation22>)将会返回[7400102](<../../../../../../harmonyos-references/Camera Kit（相机服务）/错误码/Camera错误码/errorcode-camera.md#section7400102-非法操作>)错误码，未适配相机旋转时使用相机无损出图会导致预览、拍照、录像旋转异常），推荐在[createCameraInput](<../../../../../../harmonyos-references/Camera Kit（相机服务）/ArkTS API/@ohos.multimedia.camera (相机管理)/Interface (CameraManager)/arkts-apis-camera-cameramanager.md#createcamerainput>)后直接使用[usePhysicalCameraOrientation](<../../../../../../harmonyos-references/Camera Kit（相机服务）/ArkTS API/@ohos.multimedia.camera (相机管理)/Interface (CameraInput)/arkts-apis-camera-camerainput.md#usephysicalcameraorientation22>)接口实现相机无损出图。
...
4. // 查询设备的相机镜头安装角度是否可变
5. let isVariable: boolean = cameraInput.isPhysicalCameraOrientationVariable();
...
图像显示出现拉伸或压缩等变形，是因为图像分辨率与[XComponent](../../../../../../harmonyos-references/ArkUI（方舟UI框架）/ArkTS组件/渲染绘制/XComponent/ts-basic-components-xcomponent.md)的宽高比不匹配。以应用层下发的1920\*1080(16:9)竖屏和横屏为例，器件出图均是按照4:3比例出一张RAW图，在此基础上，根据应用层下发的16:9比例进行裁切，提供数据给应用层。因此，无论手机持握方向如何变化，应用层接收的数据始终是16:9比例的图片。具体图示如下：

| 设备和镜头方向 | 处理过程示意图 | XComponent布局 |
| --- | --- | --- |
| **设备条件：**  手机竖屏、充电口向下。  使用后置相机拍摄。  **可得：**  - 后置相机镜头角度 = 90°  - 屏幕旋转角度 = 0°，[Display.rotation](<../../../../../../harmonyos-references/ArkUI（方舟UI框架）/ArkTS API/屏幕管理/@ohos.display (屏幕属性)/js-apis-display.md#属性>) = 0  - **图像预览旋转角度 = 0°+90° = 90°** |  | 出图与最终成像有90度夹角，布局宽高与图像宽高交换。 |
| **设备条件：**  手机横屏、充电口向右。  使用后置相机拍摄。  **可得：**  - 后置相机镜头角度 = 90°  - 屏幕旋转角度 = 270°，[Display.rotation](<../../../../../../harmonyos-references/ArkUI（方舟UI框架）/ArkTS API/屏幕管理/@ohos.display (屏幕属性)/js-apis-display.md#属性>) = 3  - **图像预览旋转角度 = 270°+90° = 360° = 0°** |  | 出图与最终成像有0度夹角，布局与图像宽高比一致。 |
...
首先，将XComponent的宽度和高度作为状态变量进行监听，通过[Window.on('windowSizeChange')](<../../../../../../harmonyos-references/ArkUI（方舟UI框架）/ArkTS API/窗口管理/@ohos.window (窗口)/Interface (Window)/arkts-apis-window-window.md#onwindowsizechange7>)监听窗口的变化，根据屏幕旋转角度（[Display.rotation](<../../../../../../harmonyos-references/ArkUI（方舟UI框架）/ArkTS API/屏幕管理/@ohos.display (屏幕属性)/js-apis-display.md#属性>) ）与相机镜头角度（[CameraDevice.cameraOrientation](<../../../../../../harmonyos-references/ArkUI（方舟UI框架）/ArkTS API/屏幕管理/@ohos.display (屏幕属性)/js-apis-display.md#orientation10>)）之间的角度来确定布局的宽高比，以确保布局能跟随窗口实时调整。
...
除了指定XComponent的宽高外，还可以通过设置XComponent的renderFit来实现图片的自适应大小显示、居中裁剪显示等效果。具体详情请参考[RenderFit介绍](../../../../../../harmonyos-references/ArkUI（方舟UI框架）/ArkTS组件/通用属性/视效与模糊/组件内容填充方式/ts-universal-attributes-renderfit.md#renderfit)。

### 自绘制场景预览角度的归一化处理

在自绘制场景中，对于后置摄像头，可以通过调用[getPreviewRotation](<../../../../../../harmonyos-references/Camera Kit（相机服务）/ArkTS API/@ohos.multimedia.camera (相机管理)/Interface (PreviewOutput)/arkts-apis-camera-previewoutput.md#getpreviewrotation12>)获取旋转角度，将图像转正；对于前置摄像头，由于存在水平镜像和垂直镜像的差异，为了简化操作，需先对前置摄像头的图像角度进行归一化处理后，再将图像转正，并根据业务需求决定是否进行镜像处理。
...
通过设置[PhotoCaptureSetting](<../../../../../../harmonyos-references/Camera Kit（相机服务）/ArkTS API/@ohos.multimedia.camera (相机管理)/Interfaces (其他)/arkts-apis-camera-i.md#photocapturesetting>)中的mirror属性改变拍照镜像。
