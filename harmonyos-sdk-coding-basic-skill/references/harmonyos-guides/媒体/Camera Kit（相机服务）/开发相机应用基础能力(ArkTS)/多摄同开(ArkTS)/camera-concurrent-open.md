...
scraped_at: 2026-06-01T11:28:55+08:00
doc_updated_at: 2026-03-09
content_hash: sha256:dbbc7b98fe12410eb229dec6aff0294580b0e9a4b5a7efd9932e0b6185cce891
---
...
1. 需要导入相机框架、媒体库、图片等相关领域依赖。
   ...
   ```
2. 通过[getCameraDevice](<../../../../../harmonyos-references/Camera Kit（相机服务）/ArkTS API/@ohos.multimedia.camera (相机管理)/Interface (CameraManager)/arkts-apis-camera-cameramanager.md#getcameradevice18>)获取对应的前置和后置相机。如果接口返回undefined，基于示例中的配置信息，表示当前设备不支持指定位置（前置/后置）的默认类型相机，无法实现多摄同开功能。
   ...
   13. }
   ```
3. 获取对应的并发能力集。通过[getCameraConcurrentInfos](<../../../../../harmonyos-references/Camera Kit（相机服务）/ArkTS API/@ohos.multimedia.camera (相机管理)/Interface (CameraManager)/arkts-apis-camera-cameramanager.md#getcameraconcurrentinfos18>)获取相机的输出并发能力信息数组[CameraConcurrentInfo](<../../../../../harmonyos-references/Camera Kit（相机服务）/ArkTS API/@ohos.multimedia.camera (相机管理)/Interfaces (其他)/arkts-apis-camera-i.md#cameraconcurrentinfo18>)，数组内部包含相机在对应并发模式下支持的模式和输出能力，**在多摄同开场景下设置的模式和输出能力必须在并发能力集的范围之内**。若[getCameraConcurrentInfos](<../../../../../harmonyos-references/Camera Kit（相机服务）/ArkTS API/@ohos.multimedia.camera (相机管理)/Interface (CameraManager)/arkts-apis-camera-cameramanager.md#getcameraconcurrentinfos18>)接口返回空数组，则表明当前设备不支持并发功能。
...
6. 确定录像输出流。
   ...
   91. }
   ```
7. 打开相机。通过[open](<../../../../../harmonyos-references/Camera Kit（相机服务）/ArkTS API/@ohos.multimedia.camera (相机管理)/Interface (CameraInput)/arkts-apis-camera-camerainput.md#open18>)以多摄同开状态打开指定相机。在使用[open](<../../../../../harmonyos-references/Camera Kit（相机服务）/ArkTS API/@ohos.multimedia.camera (相机管理)/Interface (CameraInput)/arkts-apis-camera-camerainput.md#open18>)接口前，请先查询接口是否支持并发能力集，并优先调用[getCameraConcurrentInfos](<../../../../../harmonyos-references/Camera Kit（相机服务）/ArkTS API/@ohos.multimedia.camera (相机管理)/Interface (CameraManager)/arkts-apis-camera-cameramanager.md#getcameraconcurrentinfos18>)方法，获取多摄同开状态下的相机并发能力集。请勿在未查询并发能力集的情况下使用[open](<../../../../../harmonyos-references/Camera Kit（相机服务）/ArkTS API/@ohos.multimedia.camera (相机管理)/Interface (CameraInput)/arkts-apis-camera-camerainput.md#open18>)，否则会导致打开相机失败。
