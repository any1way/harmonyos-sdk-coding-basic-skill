...
content_hash: sha256:1e790ed39015bda2207c2cc102a75ca2d9c96fe25eac47d4294c216f6aa610a7
---
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/63/v3/Qujot1vWSK624M1eOsDIew/zh-cn_image_0000002622858453.png?HW-CC-KV=V1&HW-CC-Date=20260611T065607Z&HW-CC-Expire=86400&HW-CC-Sign=1FCB706157D79A578EB10F26C8923C5EA87D20855D76C8040ADB8F57CB31FDAE)

与普通的[预览](../预览(ArkTS)/camera-preview.md)流程相比，动态调整预览帧率的注意点如图上标识：

1. 调用[createSession](<../../../../../harmonyos-references/Camera Kit（相机服务）/ArkTS API/@ohos.multimedia.camera (相机管理)/Interface (CameraManager)/arkts-apis-camera-cameramanager.md#createsession11>)创建会话（Session）时，需要指定模式为NORMAL\_PHOTO或NORMAL\_VIDEO。
...
如何配置会话（Session）、释放资源，请参考[会话管理](../../开发相机应用必选能力(ArkTS)/会话管理(ArkTS)/camera-session-management.md) > [预览](../预览(ArkTS)/camera-preview.md)，或是[完整流程](camera-framerate.md#完整流程)示例。
...
相机使用预览等功能前，均需完成[相机管理](../../开发相机应用必选能力(ArkTS)/相机管理(ArkTS)/camera-device-management.md)和创建相机会话，调用[CameraManager](<../../../../../harmonyos-references/Camera Kit（相机服务）/ArkTS API/@ohos.multimedia.camera (相机管理)/Interface (CameraManager)/arkts-apis-camera-cameramanager.md>)的[createSession](<../../../../../harmonyos-references/Camera Kit（相机服务）/ArkTS API/@ohos.multimedia.camera (相机管理)/Interface (CameraManager)/arkts-apis-camera-cameramanager.md#createsession11>)创建一个会话。

创建会话时需指定[SceneMode](<../../../../../harmonyos-references/Camera Kit（相机服务）/ArkTS API/@ohos.multimedia.camera (相机管理)/Enums/arkts-apis-camera-e.md#scenemode11>)为NORMAL\_PHOTO或NORMAL\_VIDEO，创建出的Session处于拍照或录像模式。
...
1. 调用[PreviewOutput](<../../../../../harmonyos-references/Camera Kit（相机服务）/ArkTS API/@ohos.multimedia.camera (相机管理)/Interface (PreviewOutput)/arkts-apis-camera-previewoutput.md>)的[getSupportedFrameRates](<../../../../../harmonyos-references/Camera Kit（相机服务）/ArkTS API/@ohos.multimedia.camera (相机管理)/Interface (PreviewOutput)/arkts-apis-camera-previewoutput.md#getsupportedframerates12>)，查询当前支持的帧率范围。

   说明

   需要在Session调用[commitConfig](<../../../../../harmonyos-references/Camera Kit（相机服务）/ArkTS API/@ohos.multimedia.camera (相机管理)/Interface (Session)/arkts-apis-camera-session.md#commitconfig11>)完成配流之后调用。
   ...
   4. }
   ```
2. 根据实际开发需求，调用[PreviewOutput](<../../../../../harmonyos-references/Camera Kit（相机服务）/ArkTS API/@ohos.multimedia.camera (相机管理)/Interface (PreviewOutput)/arkts-apis-camera-previewoutput.md>)类提供的[setFrameRate](<../../../../../harmonyos-references/Camera Kit（相机服务）/ArkTS API/@ohos.multimedia.camera (相机管理)/Interface (PreviewOutput)/arkts-apis-camera-previewoutput.md#setframerate12>)接口对帧率进行动态调整。
   ...
   * 需要在Session调用[commitConfig](<../../../../../harmonyos-references/Camera Kit（相机服务）/ArkTS API/@ohos.multimedia.camera (相机管理)/Interface (Session)/arkts-apis-camera-session.md#commitconfig11>)完成配流之后调用。
   * 可在Session调用[start](<../../../../../harmonyos-references/Camera Kit（相机服务）/ArkTS API/@ohos.multimedia.camera (相机管理)/Interface (Session)/arkts-apis-camera-session.md#start11>)启动预览前后任意时刻调用。
   ...
   * 在调用setFrameRate接口设置固定帧率后，支持重新设置固定帧率，但必须保证新设置的帧率可以整除之前设置的帧率或者被之前设置的帧率整除。
   ...
   8. }
   ```
3. （可选）通过[PreviewOutput](<../../../../../harmonyos-references/Camera Kit（相机服务）/ArkTS API/@ohos.multimedia.camera (相机管理)/Interface (PreviewOutput)/arkts-apis-camera-previewoutput.md>)提供的[getActiveFrameRate](<../../../../../harmonyos-references/Camera Kit（相机服务）/ArkTS API/@ohos.multimedia.camera (相机管理)/Interface (PreviewOutput)/arkts-apis-camera-previewoutput.md#getactiveframerate12>)接口查询已设置过并生效的帧率。

   仅通过[setFrameRate](<../../../../../harmonyos-references/Camera Kit（相机服务）/ArkTS API/@ohos.multimedia.camera (相机管理)/Interface (PreviewOutput)/arkts-apis-camera-previewoutput.md#setframerate12>)接口显式设置过帧率才可查询当前生效帧率信息。
