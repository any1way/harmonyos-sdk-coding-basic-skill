...
scraped_at: 2026-06-11T14:54:38+08:00
doc_updated_at: 2026-04-30
content_hash: sha256:390c2970e57cfc93a01e03fecde9b0c42b0506dd049c2138e57749c35a85b5c5
---
...
当前支持的编码能力，请参考[AVCodec支持的格式](../../AVCodec支持的格式/avcodec-support-formats.md#视频编码)。

视频编码的限制约束、支持的能力、状态机调用关系请参考[视频编码](../视频编码/video-encoding.md)。
...
详细的API说明请参考[VideoEncoder](<../../../../../harmonyos-references/AVCodec Kit（音视频编解码服务）/C API/头文件/native_avcodec_videoencoder.h/capi-native-avcodec-videoencoder-h.md>)。

![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/ad/v3/3QpZVentSGSFfOtLHkSrKQ/zh-cn_image_0000002592378910.png?HW-CC-KV=V1&HW-CC-Date=20260611T065437Z&HW-CC-Expire=86400&HW-CC-Sign=6E59CAD849E8BC1D919FC0867804E568C9DE35D2F754C89058643B18271E1D5A)
...
1. 创建编码器实例。
   ...
   * [OH\_AVCODEC\_MIMETYPE\_VIDEO\_AVC](<../../../../../harmonyos-references/AVCodec Kit（音视频编解码服务）/C API/头文件/native_avcodec_base.h/capi-native-avcodec-base-h.md#变量>)：AVC格式视频编解码器。
...
2. 调用OH\_VideoEncoder\_Configure()配置编码器。
   ...
   * 参数校验规则请参考[OH\_VideoEncoder\_Configure()](<../../../../../harmonyos-references/AVCodec Kit（音视频编解码服务）/C API/头文件/native_avcodec_videoencoder.h/capi-native-avcodec-videoencoder-h.md#oh_videoencoder_configure>)。
...
3. 设置surface。
   ...
   nativeWindow：获取方式请参考[视频编码Surface模式](../视频编码/video-encoding.md#surface模式)的“步骤-6：设置surface”。
...
6. 获取可用buffer并释放编码帧。
   ...
   * 调用[OH\_VideoEncoder\_QueryOutputBuffer](<../../../../../harmonyos-references/AVCodec Kit（音视频编解码服务）/C API/头文件/native_avcodec_videoencoder.h/capi-native-avcodec-videoencoder-h.md#oh_videoencoder_queryoutputbuffer>)接口获取下一个可用的输出缓冲区（buffer）的索引（index）。
   * 根据获取的索引（index），调用[OH\_VideoEncoder\_GetOutputBuffer](<../../../../../harmonyos-references/AVCodec Kit（音视频编解码服务）/C API/头文件/native_avcodec_videoencoder.h/capi-native-avcodec-videoencoder-h.md#oh_videoencoder_getoutputbuffer>)接口获取对应的缓冲区（buffer）实例。
   * 调用[OH\_VideoEncoder\_FreeOutputBuffer](<../../../../../harmonyos-references/AVCodec Kit（音视频编解码服务）/C API/头文件/native_avcodec_videoencoder.h/capi-native-avcodec-videoencoder-h.md#oh_videoencoder_freeoutputbuffer>)接口释放编码帧。
...
10. （可选）调用OH\_VideoEncoder\_Reset()重置编码器。
    ...
    调用OH\_VideoEncoder\_Reset接口后，编码器回到初始化的状态，需要调用接口[OH\_VideoEncoder\_Configure](<../../../../../harmonyos-references/AVCodec Kit（音视频编解码服务）/C API/头文件/native_avcodec_videoencoder.h/capi-native-avcodec-videoencoder-h.md#oh_videoencoder_configure>)和[OH\_VideoEncoder\_Prepare](<../../../../../harmonyos-references/AVCodec Kit（音视频编解码服务）/C API/头文件/native_avcodec_videoencoder.h/capi-native-avcodec-videoencoder-h.md#oh_videoencoder_prepare>)重新配置。
...
5. 获取可用buffer并写入码流至编码器
   ...
   * 调用[OH\_VideoEncoder\_QueryInputBuffer](<../../../../../harmonyos-references/AVCodec Kit（音视频编解码服务）/C API/头文件/native_avcodec_videoencoder.h/capi-native-avcodec-videoencoder-h.md#oh_videoencoder_queryinputbuffer>)接口获取下一个可用的输入缓冲区（buffer）的索引（index）。
   * 根据获取的索引（index），调用[OH\_VideoEncoder\_GetInputBuffer](<../../../../../harmonyos-references/AVCodec Kit（音视频编解码服务）/C API/头文件/native_avcodec_videoencoder.h/capi-native-avcodec-videoencoder-h.md#oh_videoencoder_getinputbuffer>)接口获取对应的缓冲区（buffer）实例。
   * 将需要编码的数据写入该缓冲区（buffer）后，调用[OH\_VideoEncoder\_PushInputBuffer](<../../../../../harmonyos-references/AVCodec Kit（音视频编解码服务）/C API/头文件/native_avcodec_videoencoder.h/capi-native-avcodec-videoencoder-h.md#oh_videoencoder_pushinputbuffer>)接口将其送入编码输入队列进行编码。当最后一帧数据被送入编码输入队列时，需要将flag标识成[AVCODEC\_BUFFER\_FLAGS\_EOS](<../../../../../harmonyos-references/AVCodec Kit（音视频编解码服务）/C API/头文件/native_avbuffer_info.h/capi-native-avbuffer-info-h.md#oh_avcodecbufferflags>)，通知编码器输入结束。
