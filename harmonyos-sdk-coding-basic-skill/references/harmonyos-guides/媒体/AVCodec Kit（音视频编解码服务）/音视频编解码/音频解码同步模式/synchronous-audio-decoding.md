...
scraped_at: 2026-06-11T14:54:36+08:00
doc_updated_at: 2026-06-03
content_hash: sha256:e06cdfa0aa6f36838157d24f550688e3ee89d056ec4668accaaa885e75fbc3dc
---
...
支持的解码能力请参考[AVCodec支持的格式](../../AVCodec支持的格式/avcodec-support-formats.md#音频解码)。

**适用场景**

通常推荐使用异步模式，详细内容请参考[音频解码](../音频解码/audio-decoding.md)。若需要主动请求buffer去送帧，则可以使用同步模式。

将音视频文件解码为PCM码流，通常需要以下步骤：[媒体数据解析](../../媒体数据封装与解析/媒体数据解析/audio-video-demuxer.md) -> 音频解码。

本指南描述音频解码过程：输入音频帧和解码出PCM码流。

## 开发指导

详细的API说明请参考[AudioCodec](<../../../../../harmonyos-references/AVCodec Kit（音视频编解码服务）/C API/头文件/native_avcodec_audiocodec.h/capi-native-avcodec-audiocodec-h.md>)。
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/f8/v3/1bhthYubQN-lDLRHlZ3kdA/zh-cn_image_0000002622698535.png?HW-CC-KV=V1&HW-CC-Date=20260611T065434Z&HW-CC-Expire=86400&HW-CC-Sign=BC95A783E2C2029CAFF8A0B0A53F05B7207E72793134DC05D42926BA31149E74)
...
3. （可选）OH\_AudioCodec\_SetDecryptionConfig设置解密配置。
   ...
   DRM相关接口详见[DRM](<../../../../../harmonyos-references/DRM Kit（数字版权保护服务）/C API/模块/Drm/capi-drm.md>)。
...
4. 调用OH\_AudioCodec\_Configure()配置解码器。
   ...
   ![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/a/v3/ThibiYV3SomxnBafSNp6RA/zh-cn_image_0000002592218974.png?HW-CC-KV=V1&HW-CC-Date=20260611T065434Z&HW-CC-Expire=86400&HW-CC-Sign=92756C0203A5D8E2DFBB24EFFE3288AE456B5F257893F29B1959CF85E7254F85)

   各音频解码类型参数范围说明：

   ![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/32/v3/JFXioq7LTluBa1V7M3gLdQ/zh-cn_image_0000002592378908.png?HW-CC-KV=V1&HW-CC-Date=20260611T065434Z&HW-CC-Expire=86400&HW-CC-Sign=B66F02FA694703893A73135072492532E815A635AE9204CC692E46025DAF0E19)
   ...
   4. // 配置音频声道数（必须）。
   5. constexpr uint32_t DEFAULT_CHANNEL_COUNT = 2;
   6. // 配置是否为ADTS解码（aac解码时可选）。
   7. constexpr uint32_t DEFAULT_AAC_TYPE = 1;
   8. OH_AVFormat *format = OH_AVFormat_Create();
   9. // 写入format。
   10. OH_AVFormat_SetIntValue(format, OH_MD_KEY_AUD_SAMPLE_RATE, DEFAULT_SAMPLERATE);
   11. OH_AVFormat_SetIntValue(format, OH_MD_KEY_AUD_CHANNEL_COUNT, DEFAULT_CHANNEL_COUNT);
   12. OH_AVFormat_SetIntValue(format, OH_MD_KEY_AAC_IS_ADTS, DEFAULT_AAC_TYPE);
   13. OH_AVFormat_SetIntValue(format, OH_MD_KEY_ENABLE_SYNC_MODE, 1); // 同步模式配置。
   14. // 配置解码器。
   15. OH_AVErrCode ret = OH_AudioCodec_Configure(audioDec_, format);
   16. if (ret != AV_ERR_OK) {
   17. // 异常处理。
   18. }
   ```
...
7. （可选）调用OH\_AVCencInfo\_SetAVBuffer()，设置cencInfo。
   ...
   如果当前播放的节目是DRM加密节目，并且由上层应用进行[媒体数据解析](../../媒体数据封装与解析/媒体数据解析/audio-video-demuxer.md)，则需要调用OH\_AVCencInfo\_SetAVBuffer()将cencInfo设置给AVBuffer，以实现媒体数据的解密。
