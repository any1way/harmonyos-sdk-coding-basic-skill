...
scraped_at: 2026-06-11T14:54:37+08:00
doc_updated_at: 2026-05-26
content_hash: sha256:5e220645616de3d26f7ce410ead703d8ed024e40a52ff6f8781a790d807830f7
---
...
本文档主要介绍异步模式视频编码的实现流程，同步模式视频编码请参考[视频编码同步模式](../视频编码同步模式/synchronous-video-encoding.md)。根据编码前数据输入方式的不同，编码器支持Surface模式和Buffer模式两种输入模式，适用于不同的应用场景。

* Surface模式。

  编码器通过[NativeWindow](<../../../../../harmonyos-references/ArkGraphics 2D（方舟2D图形服务）/C API/结构体/NativeWindow/capi-nativewindow-nativewindow.md>)来获取输入数据，可以与其他模块对接（如相机模块）。适用于与相机、屏幕录制等数据源直接对接的编码场景。
* Buffer模式。
...
AVCodec支持的视频编码格式请参考[视频编码](../../AVCodec支持的格式/avcodec-support-formats.md#视频编码)。
...
4. 运行状态（Executing）。
   ...
   * Executing状态具有三个子状态：Running、Flushed和End-of-Stream。
     ...
     + End-of-Stream：编码器接收到输入buffer的flags为[OH\_AVCodecBufferFlags](<../../../../../harmonyos-references/AVCodec Kit（音视频编解码服务）/C API/头文件/native_avbuffer_info.h/capi-native-avbuffer-info-h.md#oh_avcodecbufferflags>)中的AVCODEC\_BUFFER\_FLAGS\_EOS，或者调用OH\_VideoEncoder\_NotifyEndOfStream接口时，进入End-of-Stream子状态。在此状态下，编码器不再接受新的输入，但是仍然会继续生成输出，直到输出到达尾帧。
5. 错误状态（Error）。
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/83/v3/V-mxLkS5TIe6xWkGg_tz6w/zh-cn_image_0000002622858415.png?HW-CC-KV=V1&HW-CC-Date=20260611T065435Z&HW-CC-Expire=86400&HW-CC-Sign=7179B88C1A584AD40A0840FB223A00138ABDD5717DA3177016AED988B0B3291E)

## 开发指导

详细的API说明请参考[native\_avcodec\_videoencoder.h](<../../../../../harmonyos-references/AVCodec Kit（音视频编解码服务）/C API/头文件/native_avcodec_videoencoder.h/capi-native-avcodec-videoencoder-h.md>)。
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/f5/v3/8nijFyi5Qla5jzFCPhGvUQ/zh-cn_image_0000002622698537.png?HW-CC-KV=V1&HW-CC-Date=20260611T065435Z&HW-CC-Expire=86400&HW-CC-Sign=25CA2FB6C203F7CB11719256DE9DB4E6E3226AEA1F8FC07BAEFAB031FDF7BD18)
...
3. 调用OH\_VideoEncoder\_RegisterCallback()设置回调函数。
   ...
   * OH\_AVCodecOnError 编码器运行错误，返回的错误码详情请参见[OH\_AVCodecOnError](<../../../../../harmonyos-references/AVCodec Kit（音视频编解码服务）/C API/头文件/native_avcodec_base.h/capi-native-avcodec-base-h.md#oh_avcodeconerror>)；
...
4. （可选）调用OH\_VideoEncoder\_RegisterParameterCallback()在Configure接口之前注册随帧通路回调。
   ...
   详情请参考[时域可分层视频编码](../时域可分层视频编码/video-encoding-temporal-scalability.md)。
...
5. 调用OH\_VideoEncoder\_Configure()配置编码器。
   ...
   参数校验规则请参考[OH\_VideoEncoder\_Configure()参考文档](<../../../../../harmonyos-references/AVCodec Kit（音视频编解码服务）/C API/头文件/native_avcodec_videoencoder.h/capi-native-avcodec-videoencoder-h.md#oh_videoencoder_configure>)。

   参数取值范围可以通过能力查询接口获取，具体示例请参考[获取支持的编解码能力文档](../获取支持的编解码能力/obtain-supported-codecs.md)。
   ...
   50. rateMode == static_cast<int32_t>(OH_BitrateMode::BITRATE_MODE_CBR_HIGH_QUALITY)){
   51. OH_AVFormat_SetLongValue(format.get(), OH_MD_KEY_BITRATE, bitRate);
   52. }
   53. OH_AVFormat_SetIntValue(format.get(), OH_MD_KEY_VIDEO_ENCODE_BITRATE_MODE, rateMode);
   54. OH_AVErrCode ret = OH_VideoEncoder_Configure(videoEnc, format.get());
   55. if (ret != AV_ERR_OK) {
   56. // 异常处理。
   57. }
   ```
...
6. 获取surface。
   ...
   OHNativeWindow\*变量类型的使用方法请参考图形子系统 [OHNativeWindow](<../../../../../harmonyos-references/ArkGraphics 2D（方舟2D图形服务）/C API/模块/NativeWindow/capi-nativewindow.md>)。
...
13. 调用OH\_VideoEncoder\_FreeOutputBuffer()释放编码帧。
    ...
    * buffer：回调函数OnNewOutputBuffer传入的参数，可以通过[OH\_AVBuffer\_GetAddr](<../../../../../harmonyos-references/AVCodec Kit（音视频编解码服务）/C API/头文件/native_avbuffer.h/capi-native-avbuffer-h.md#oh_avbuffer_getaddr>)接口得到共享内存地址的指针；
...
8. 调用OH\_VideoEncoder\_PushInputBuffer()写入编码图像。
   ...
   * buffer：回调函数OnNeedInputBuffer传入的参数，可以通过[OH\_AVBuffer\_GetAddr](<../../../../../harmonyos-references/AVCodec Kit（音视频编解码服务）/C API/头文件/native_avbuffer.h/capi-native-avbuffer-h.md#oh_avbuffer_getaddr>)接口得到共享内存地址的指针；
   ...
   * OH\_MD\_KEY\_VIDEO\_SLICE\_HEIGHT表示hStride。
   ...
   ![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/1f/v3/_NipRDfeSe-hh-tsM4tvig/zh-cn_image_0000002592218976.png?HW-CC-KV=V1&HW-CC-Date=20260611T065435Z&HW-CC-Expire=86400&HW-CC-Sign=C175655EDA73F1F9F288D1A541ADD3BEB772A87B07018A275F0825C0EE9BFFF0)
...
| 支持的能力 | 使用简述 |
| --- | --- |
| 分层编码、设置LTR帧、参考帧 | 具体可参考：[时域可分层视频编码](../时域可分层视频编码/video-encoding-temporal-scalability.md)。 |
| 支持历史帧repeat编码 | 具体可参考：native\_avcodec\_base.h[变量][OH\_MD\_KEY\_VIDEO\_ENCODER\_REPEAT\_PREVIOUS\_FRAME\_AFTER](<../../../../../harmonyos-references/AVCodec Kit（音视频编解码服务）/C API/头文件/native_avcodec_base.h/capi-native-avcodec-base-h.md#变量>)中的OH\_MD\_KEY\_VIDEO\_ENCODER\_REPEAT\_PREVIOUS\_FRAME\_AFTER和OH\_MD\_KEY\_VIDEO\_ENCODER\_REPEAT\_PREVIOUS\_MAX\_COUNT。 |
| 支持的能力 | 使用简述 |
| -------------------------- | ------------------------------------------------- |
...
| 分层编码，LTR设置 | 具体可参考：[时域可分层视频编码](../时域可分层视频编码/video-encoding-temporal-scalability.md) |
