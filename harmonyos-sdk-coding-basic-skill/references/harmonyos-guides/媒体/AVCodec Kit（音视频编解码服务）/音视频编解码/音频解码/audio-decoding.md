...
scraped_at: 2026-06-11T14:54:34+08:00
doc_updated_at: 2026-06-03
content_hash: sha256:d23404ca44ba9dd9a7d047a8dbab8defada9f71eea98d2cb3f4faff365839e43
---
开发者可以调用本模块的Native API接口，完成音频解码，即将媒体数据解码为PCM码流。

当前支持的解码能力请参考[AVCodec支持的格式](../../AVCodec支持的格式/avcodec-support-formats.md#音频解码)。
...
详细的API说明请参考[native\_avcodec\_audiocodec.h](<../../../../../harmonyos-references/AVCodec Kit（音视频编解码服务）/C API/头文件/native_avcodec_audiocodec.h/capi-native-avcodec-audiocodec-h.md>)。
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/8f/v3/HLiZ7VMCQya2y0cI0RDw7Q/zh-cn_image_0000002592218972.png?HW-CC-KV=V1&HW-CC-Date=20260611T065320Z&HW-CC-Expire=86400&HW-CC-Sign=2CB241F31219BAB32D95FA119CB242E4703966303977DBF4C0E1F1CDD102879F)
...
4. （可选）OH\_AudioCodec\_SetDecryptionConfig设置解密配置。
   ...
   当获取到DRM信息(参考[音视频解封装](../../媒体数据封装与解析/媒体数据解析/audio-video-demuxer.md)开发步骤第4步)后，通过此接口进行解密配置。

   DRM相关接口详见[DRM API文档](<../../../../../harmonyos-references/DRM Kit（数字版权保护服务）/C API/模块/Drm/capi-drm.md>)。
...
5. 调用OH\_AudioCodec\_Configure()配置解码器。
   ...
   ![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/2c/v3/1ADIVvd9Quu1QXPpamvRTg/zh-cn_image_0000002592218974.png?HW-CC-KV=V1&HW-CC-Date=20260611T065320Z&HW-CC-Expire=86400&HW-CC-Sign=F34447A6DB8ADBE97FB508592E1AAE2CB786EFC3023166750A71FEA32504BA59)

   各音频解码类型参数范围说明：

   ![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/ab/v3/g88u8H3jQEebv5hjfD7ZNQ/zh-cn_image_0000002592378908.png?HW-CC-KV=V1&HW-CC-Date=20260611T065320Z&HW-CC-Expire=86400&HW-CC-Sign=8BAE05C1F730FD12905AE9E33F99EA167EB4E16236D6F9C3F375686982330481)

   从API version 20开始，支持通过[OH\_AVCapability\_GetAudioSupportedSampleRateRanges](<../../../../../harmonyos-references/AVCodec Kit（音视频编解码服务）/C API/头文件/native_avcapability.h/capi-native-avcapability-h.md#oh_avcapability_getaudiosupportedsamplerateranges>)接口进行采样率范围能力查询，以下几种音频解码类型支持对范围内的任意采样率进行解码：
   ...
   ```
   1. // 以下配置的值仅为示例值，开发者需根据实际解码能力动态设置。
   2. // 配置音频采样率（必须）。
   3. constexpr uint32_t DEFAULT_SAMPLERATE = 44100;
   4. // 配置音频码率（可选）。
   5. constexpr uint32_t DEFAULT_BITRATE = 32000;
   6. // 配置音频声道数（必须）。
   7. constexpr uint32_t DEFAULT_CHANNEL_COUNT = 2;
   8. // 配置最大输入长度（可选）。
   9. constexpr uint32_t DEFAULT_MAX_INPUT_SIZE = 1152;
   10. // 配置是否为ADTS解码（aac解码时可选）。
   11. constexpr uint32_t DEFAULT_AAC_TYPE = 1;
   12. // 配置划分音频数据块字节数，从API version 22开始支持，仅WMAV1、WMAV2、WMA PRO解码时必须配置。
   13. constexpr int32_t DEFAULT_BLOCK_ALIGN = 1;
   14. OH_AVFormat *format = OH_AVFormat_Create();
   15. // 写入format。
   16. OH_AVFormat_SetIntValue(format, OH_MD_KEY_AUD_SAMPLE_RATE, DEFAULT_SAMPLERATE);
   17. OH_AVFormat_SetIntValue(format, OH_MD_KEY_BITRATE, DEFAULT_BITRATE);
   18. OH_AVFormat_SetIntValue(format, OH_MD_KEY_AUD_CHANNEL_COUNT, DEFAULT_CHANNEL_COUNT);
   19. OH_AVFormat_SetIntValue(format, OH_MD_KEY_MAX_INPUT_SIZE, DEFAULT_MAX_INPUT_SIZE);
   20. OH_AVFormat_SetIntValue(format, OH_MD_KEY_AAC_IS_ADTS, DEFAULT_AAC_TYPE);
   21. OH_AVFormat_SetIntValue(format, OH_MD_KEY_BLOCK_ALIGN, DEFAULT_BLOCK_ALIGN);
   22. // 配置解码器。
   23. int32_t ret = OH_AudioCodec_Configure(audioDec_, format);
   24. if (ret != AV_ERR_OK) {
   25. // 异常处理。
   26. }
   ```
...
8. （可选）调用OH\_AVCencInfo\_SetAVBuffer()，设置cencInfo。
   ...
   若当前播放的节目是DRM加密节目，且由上层应用做[媒体解封装](../../媒体数据封装与解析/媒体数据解析/audio-video-demuxer.md)，则须调用OH\_AVCencInfo\_SetAVBuffer()将cencInfo设置给AVBuffer，以实现AVBuffer中媒体数据的解密。
