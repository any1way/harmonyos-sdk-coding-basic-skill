...
scraped_at: 2026-06-11T14:54:31+08:00
doc_updated_at: 2026-05-26
content_hash: sha256:2f4b89769b2703997b40163c7cf40aa26a6c92050bb161f56740efa8a000c76f
---
...
3. 获得音视频编解码能力实例。
   ...
   ```
   1. // 获取系统中所有视频解码器的能力实例列表。
   2. uint32_t count = 0;
   3. OH_AVCapability **capabilityList = OH_AVCodec_GetCapabilityList(OH_AVCODEC_TYPE_VIDEO_DECODER, &count);
   ```
4. 按需调用相应的查询接口。详细的API说明请参考[native\_avcapability.h](<../../../../../harmonyos-references/AVCodec Kit（音视频编解码服务）/C API/头文件/native_avcapability.h/capi-native-avcapability-h.md>)。
...
编解码标准包含多种编码工具，适用于不同的编码场景。对于特定应用场景，编解码标准按档次确定所需编码工具的开启与关闭情况（例如，H.264有基本档次、主档次和高档次）。详情参见 [OH\_AVCProfile](<../../../../../harmonyos-references/AVCodec Kit（音视频编解码服务）/C API/头文件/native_avcodec_base.h/capi-native-avcodec-base-h.md#oh_avcprofile>)。

级别划分了编解码器所需的处理能力和存储空间。H.264有1到6.2的20个级别，参考[OH\_AVCLevel](<../../../../../harmonyos-references/AVCodec Kit（音视频编解码服务）/C API/头文件/native_avcodec_base.h/capi-native-avcodec-base-h.md#oh_avclevel>)。
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/fc/v3/Apa-IxzfQ26_DvqQbtp2KA/zh-cn_image_0000002622858411.png?HW-CC-KV=V1&HW-CC-Date=20260611T065429Z&HW-CC-Expire=86400&HW-CC-Sign=78EC467FF7178531506845C356F952DE87B32D9B59D418A2F6A3D750CDA665A9)
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/aa/v3/Ea8qLSt2TF2nB-Kpw9mGaw/zh-cn_image_0000002622698533.png?HW-CC-KV=V1&HW-CC-Date=20260611T065429Z&HW-CC-Expire=86400&HW-CC-Sign=F676C0302FC35E19714FF349D7EDF3302038DC73BB886DA9F44F0753F8940696)
...
视频像素格式指示的编码输入图像或解码输出图像的像素排布方式，参考[OH\_AVPixelFormat](<../../../../../harmonyos-references/AVCodec Kit（音视频编解码服务）/C API/头文件/native_avformat.h/capi-native-avformat-h.md#oh_avpixelformat>)。
...
编解码特性是指在特定编解码场景中使用的可选特性，例如视频编码场景的时域可分级编码、 低时延编解码等。具体请参考[OH\_AVCapabilityFeature](<../../../../../harmonyos-references/AVCodec Kit（音视频编解码服务）/C API/头文件/native_avcapability.h/capi-native-avcapability-h.md#oh_avcapabilityfeature>)。
...
```
1. // 1. 定义期望的MIME类型。
2. const char *targetMime = OH_AVCODEC_MIMETYPE_VIDEO_AVC;
3. uint32_t count = 0;

5. // 2. 获取所有视频解码器的能力列表。
6. OH_AVCapability **capabilityList = OH_AVCodec_GetCapabilityList(OH_AVCODEC_TYPE_VIDEO_DECODER, &count);

8. if (capabilityList != nullptr && count > 0) {
9. for (uint32_t i = 0; i < count; i++) {
10. OH_AVCapability *cap = capabilityList[i];

12. // 3. 检查是否为目标的MIME类型，且必须是安全解码器。
13. if (OH_AVCapability_CheckMimeType(cap, targetMime) && OH_AVCapability_IsSecure(cap)) {
14. // 4. 找到符合条件的解码器，获取其名称用于创建实例。
15. const char *codecName = OH_AVCapability_GetName(cap);
16. OH_AVCodec *secureVideoDec = OH_VideoDecoder_CreateByName(codecName);

18. if (secureVideoDec != nullptr) {
19. // 5. 成功创建安全解码器，跳出循环执行后续业务。
20. break;
21. }
22. }
23. }
