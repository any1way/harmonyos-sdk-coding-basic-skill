...
scraped_at: 2026-06-11T14:57:54+08:00
doc_updated_at: 2026-06-03
content_hash: sha256:7b1a5457f4f754ddf5fda9edd49406c9161706fd8827521545ce3d95fcd37316
---
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/22/v3/oHPqJ6wnQLeRKQY8YyBDkA/zh-cn_image_0000002622858471.png?HW-CC-KV=V1&HW-CC-Date=20260611T065752Z&HW-CC-Expire=86400&HW-CC-Sign=25A318A0C9C97D488BB13AB109E1C17D203ECE2D9A050E0C5B1A2F11D2B8CB9B)
...
AVRecorder只负责视频数据的处理，需要与视频数据采集模块配合才能完成视频录制。视频数据采集模块需要通过Surface将视频数据传递给AVRecorder进行数据处理。当前常用的数据采集模块为相机模块，具体请参考[相机-录像](<../../../../Camera Kit（相机服务）/开发相机应用基础能力(CC++)/录像(CC++)/native-camera-recording.md>)。

文件的创建与存储，请参考[应用文件访问与管理](<../../../../../应用框架/Core File Kit（文件基础服务）/应用文件/应用文件访问与管理/应用文件访问(ArkTS)/app-file-access.md>)，默认存储在应用的沙箱路径之下，如需存储至图库，请使用[安全控件保存媒体资源](<../../../../Media Library Kit（媒体文件管理服务）/保存媒体库资源/photoaccesshelper-savebutton.md>)对沙箱内文件进行存储。

开发者通过引入[avrecorder.h](<../../../../../../harmonyos-references/Media Kit（媒体服务）/C API/头文件/avrecorder.h/capi-avrecorder-h.md>)、[avrecorder\_base.h](<../../../../../../harmonyos-references/Media Kit（媒体服务）/C API/头文件/avrecorder_base.h/capi-avrecorder-base-h.md>)和[native\_averrors.h](<../../../../../../harmonyos-references/AVCodec Kit（音视频编解码服务）/C API/头文件/native_averrors.h/capi-native-averrors-h.md>)头文件，使用视频录制相关API。

AVRecorder详细的API说明请参考[AVRecorder API参考](<../../../../../../harmonyos-references/Media Kit（媒体服务）/C API/模块/AVRecorder/capi-avrecorder.md>)。
...
使用[native\_avformat.h](<../../../../../../harmonyos-references/AVCodec Kit（音视频编解码服务）/C API/头文件/native_avformat.h/capi-native-avformat-h.md>)相关接口时，需引入如下头文件。
...
2. 设置业务需要的监听事件，监听状态变化及错误上报。
   ...
   36. OH_LOG_ERROR(LOG_APP, "==NDKDemo== Recorder OnStateChange ERROR, reason: %{public}s", reasonStr);
   37. // 处理状态变更。
   38. }
   39. }
...
3. 配置视频录制参数，调用OH\_AVRecorder\_Prepare()接口，此时进入prepared状态。
   ...
   * 录制输出的url地址（即示例里avConfig中的url），形式为fd://xx (fd number)。需要调用基础文件操作接口实现应用文件访问能力，获取方式参考[应用文件访问与管理](<../../../../../应用框架/Core File Kit（文件基础服务）/应用文件/应用文件访问与管理/应用文件访问(CC++)/native-fileio-guidelines.md>)。
   ...
   83. }
   ```
4. 获取视频录制需要的SurfaceID，初始化视频数据输入源。该步骤需要在输入源模块完成，以相机为例，需要创建录像输出流，包括创建Camera对象、获取相机列表、创建相机输入流等，相机详细步骤请参考[相机-录像方案](<../../../../Camera Kit（相机服务）/开发相机应用基础能力(CC++)/录像(CC++)/native-camera-recording.md>)。
   ...
   7. }
   ```
5. 初始化视频数据输入源。该步骤需要在输入源模块完成，以相机为例，需要创建录像输出流，包括创建Camera对象、获取相机列表、创建相机输入流等，相机详细步骤请参考[相机-录像方案](<../../../../Camera Kit（相机服务）/开发相机应用基础能力(CC++)/录像(CC++)/native-camera-recording.md>)。
...
244. }
245. if (g_outputFd > 0) {
246. close(g_outputFd);
247. }

249. napi_value res;
250. napi_create_int32(env, result, &res);
251. return res;
252. }

254. // 7.重置录制状态。
255. static napi_value ResetAVRecorder(napi_env env, napi_callback_info info)
256. {
257. (void)info;
258. // 检查g_avRecorder是否有效。
259. if (g_avRecorder == nullptr) {
260. OH_LOG_ERROR(LOG_APP, "==NDKDemo== g_avRecorder is nullptr!");
261. napi_value res;
262. napi_create_int32(env, AV_ERR_INVALID_VAL, &res);
263. return res;
264. }

266. int result = OH_AVRecorder_Reset(g_avRecorder);
267. if (result != AV_ERR_OK) {
268. OH_LOG_ERROR(LOG_APP, "==NDKDemo== AVRecorder Reset failed %{public}d", result);
269. }
270. napi_value res;
271. napi_create_int32(env, result, &res);
272. return res;
273. }

275. // 8.释放录制资源。
276. static napi_value ReleaseAVRecorder(napi_env env, napi_callback_info info)
277. {
278. (void)info;
279. // 检查g_avRecorder是否有效。
280. if (g_avRecorder == nullptr) {
281. OH_LOG_ERROR(LOG_APP, "==NDKDemo== g_avRecorder is nullptr!");
282. napi_value res;
283. napi_create_int32(env, AV_ERR_INVALID_VAL, &res);
284. return res;
285. }
286. if (g_outputFd > 0) {
287. close(g_outputFd);
288. g_outputFd = -1;
289. }
290. int result = OH_AVRecorder_Release(g_avRecorder);
291. g_avRecorder = nullptr;   // 释放录制资源后，需要显式地将g_avRecorder指针置空。

293. if (result != AV_ERR_OK) {
294. OH_LOG_ERROR(LOG_APP, "==NDKDemo== AVRecorder Release failed %{public}d", result);
295. }
296. napi_value res;
297. napi_create_int32(env, result, &res);
298. return res;
299. }
```
