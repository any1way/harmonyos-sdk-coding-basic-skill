...
scraped_at: 2026-06-01T10:56:32+08:00
doc_updated_at: 2026-03-23
content_hash: sha256:db7c0b2a307c834ab2c48f332e9d5a8e552047971c1653d6c79227bc9cee2209
---
...
* 多HAP场景下，在安装或更新时，存在一致性校验，详情参考[应用安装与更新一致性校验](../../应用程序包安装卸载与更新/应用安装与更新一致性校验/install-and-update-consistency-verification.md)。使用打包工具进行打包成APP时，也会进行合法性校验，详情请参考[打包工具](../../../../../系统/调测调优/调试命令/打包拆包工具/打包工具/packing-tool.md#app打包指令)。
* 多HAP场景下，同一应用的所有HAP、HSP的签名证书要保持一致。上架应用市场是以App Pack形式上架，应用市场分发时会将所有HAP从App Pack中拆分出来，同时对所有HAP进行重签名，以保证签名证书的一致性。在调试阶段，开发者通过命令行或DevEco Studio将HAP安装到设备上时，要保证所有HAP签名证书一致，否则会出现安装失败的问题，签名操作请参考[应用/元服务签名](../../../../../编写与调试应用/配置调试签名/ide-signing.md)。
...
* HAP中支持添加UIAbility组件或ExtensionAbility组件，以及pages页面。具体操作可参考[在模块中添加Ability](../../../../../开发环境搭建/工程创建/模块管理/添加Ability/ide-add-new-ability.md)和[添加Page](../../../../../开发环境搭建/工程创建/模块管理/添加Page/ide-add-page.md)。
* HAP中支持引用HAR或HSP共享包，详见[HAR的使用](../HAR/har-package.md#使用)、[HSP的使用](../HSP/in-app-hsp.md#使用)。

## 调试

通过DevEco Studio编译打包，生成单个或者多个HAP，即可基于HAP进行调试。如需根据不同的部署环境、目标人群、运行环境等，将同一个HAP定制编译为不同版本，请参见[定制编译指导](../../../../../构建应用/配置构建流程/配置多目标产物/能力说明/ide-customized-multi-targets-and-products-guides.md#section1011341611469)。

开发者可以使用DevEco Studio或者hdc工具进行调试：

* **方法一：** 使用DevEco Studio进行调试，详见[应用程序包调试方法](../../../../../编写与调试应用/应用调试/自定义运行调试配置/ide-run-debug-configurations.md)。
* **方法二：** 使用[hdc工具](../../../../../系统/调测调优/调试命令/hdc/hdc.md)进行调试。
  ...
  完成HAP安装或更新后，即可参考相关调试命令进行[调试](../../../../../系统/调测调优/调试命令/aa工具/aa-tool.md#进入调试模式命令attach)。
