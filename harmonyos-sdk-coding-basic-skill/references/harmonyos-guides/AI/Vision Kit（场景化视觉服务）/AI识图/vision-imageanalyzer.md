...
scraped_at: 2026-06-11T15:19:39+08:00
doc_updated_at: 2026-04-29
content_hash: sha256:c4cddb5884e862352e014e5b8c9730f441b0c0f217824b58ea29a8cb8650d595
---
## 场景介绍

AI识图是通过聚合OCR（Optical Character Recognition）、主体分割、实体识别、多目标识别等AI能力，提供场景化的文本识别、主体分割、识图搜索功能。AI识图功能主开关入口在基础控件API列表中，如果您接受AI识图默认的交互和功能，仅需使用基础控件提供的相关使能接口打开功能开关即可。该文档配套的API配合基础控件使用，主要满足您的定制诉求，帮助您完成AI识图功能交互上的细粒度控制，获取文本识别、图像分割等分析结果以便您进行扩展业务的开发，目前支持的基础控件范围包括[Image](../../../../harmonyos-references/ArkUI（方舟UI框架）/ArkTS组件/图片与视频/Image/ts-basic-components-image.md#enableanalyzer11)、[Video](../../../../harmonyos-references/ArkUI（方舟UI框架）/ArkTS组件/图片与视频/Video/ts-media-components-video.md#enableanalyzer12)、[XComponent](../../../../harmonyos-references/ArkUI（方舟UI框架）/ArkTS组件/渲染绘制/XComponent/ts-basic-components-xcomponent.md#enableanalyzer12)。其中，配合Image控件可完成静态图片上的识图功能，配合Video控件可完成视频播放暂停帧的识图功能，配合XComponent可完成自定义渲染等场景下的图像的识图功能。
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/8d/v3/E88iX-lVRf6mNUKMcgw8pA/zh-cn_image_0000002622699359.png?HW-CC-KV=V1&HW-CC-Date=20260611T071938Z&HW-CC-Expire=86400&HW-CC-Sign=0B7DB6DBD1BE9CB9DD2FF2A54B0A86B44A82DE0856352C289D03CF5936C93AAC)

## 约束与限制

* 支持的文本语种类型：简体中文、繁体中文、英文、维吾尔文、藏文。
* 支持图片最小规格100\*100分辨率。
* 分析图像要求是静态非矢量图，即svg、gif等图像类型不支持分析，支持传入[PixelMap](<../../../../harmonyos-references/Image Kit（图片处理服务）/ArkTS API/@ohos.multimedia.image (图片处理)/Interface (PixelMap)/arkts-apis-image-pixelmap.md>)进行分析，目前仅支持[RGBA\_8888](<../../../../harmonyos-references/Image Kit（图片处理服务）/ArkTS API/@ohos.multimedia.image (图片处理)/Enums/arkts-apis-image-e.md#pixelmapformat7>)类型。
* 支持翻译的图片宽高最小比例为1:3（高度小于宽度的3倍），支持文本识别的图片宽高最小比例为1:7（高度小于宽度的7倍）。
* 当前设备支持本能力可以通过[getImageAnalyzerSupportTypes](../../../../harmonyos-references/ArkUI（方舟UI框架）/ArkTS组件/图片与视频/图像类型定义/ts-image-common.md#getimageanalyzersupporttypes12)进行判断。

  返回格式为“SupportTypes: [主体识别功能枚举值,文字识别功能枚举值,对象查找功能枚举值]”，具体枚举值可参见[ImageAnalyzerType](../../../../harmonyos-references/ArkUI（方舟UI框架）/ArkTS组件/图片与视频/图像类型定义/ts-image-common.md#imageanalyzertype12)。
...
1. 将AI识图控件相关的类添加。
   ...
   ```
2. 初始化[VisionImageAnalyzerController](<../../../../harmonyos-references/Vision Kit（场景化视觉服务）/ArkTS API/visionImageAnalyzer（AI识图控件）/vision-image-analyzer.md#visionimageanalyzercontroller>)对象。
...
3. 判断设备是否支持AI识图相关功能，若支持，添加订阅事件和取消事件监听方法。
   ...
   31. this.visionImageAnalyzerController.off('imageAnalyzerVisibilityChange');
   32. this.visionImageAnalyzerController.off('textAnalysis');
   33. this.visionImageAnalyzerController.off('selectedTextChange');
   34. this.visionImageAnalyzerController.off('subjectAnalysis');
   35. this.visionImageAnalyzerController.off('selectedSubjectsChange');
   36. this.visionImageAnalyzerController.off('analyzerFailed');
   37. }
   ```
4. 绑定[VisionImageAnalyzerController](<../../../../harmonyos-references/Vision Kit（场景化视觉服务）/ArkTS API/visionImageAnalyzer（AI识图控件）/vision-image-analyzer.md#visionimageanalyzercontroller>)对象，可以控制识图相关的交互。
...
38. this.visionImageAnalyzerController.off('imageAnalyzerVisibilityChange');
39. this.visionImageAnalyzerController.off('textAnalysis');
40. this.visionImageAnalyzerController.off('selectedTextChange');
41. this.visionImageAnalyzerController.off('subjectAnalysis');
42. this.visionImageAnalyzerController.off('selectedSubjectsChange');
43. this.visionImageAnalyzerController.off('analyzerFailed');
44. }

46. build() {
47. Stack() {
48. Image($r('app.media.img'), {
49. types: [ImageAnalyzerType.TEXT, ImageAnalyzerType.SUBJECT, ImageAnalyzerType.OBJECT_LOOKUP],
50. aiController: this.visionImageAnalyzerController
51. })
52. .width('100%')
53. .height('100%')
54. .enableAnalyzer(this.isSupportImageAnalyzer ? true : false)
