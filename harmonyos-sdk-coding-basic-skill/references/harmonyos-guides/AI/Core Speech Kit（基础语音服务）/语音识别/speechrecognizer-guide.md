...
scraped_at: 2026-06-01T15:13:23+08:00
doc_updated_at: 2026-04-29
content_hash: sha256:6e5cfe700b7122a65aad755d1ea19873de32112d558ab0916568c357ddd85378
---
...
1. 在使用语音识别时，将实现语音识别相关的类添加至工程。
   ...
   ```
2. 调用[createEngine](<../../../../harmonyos-references/Core Speech Kit（基础语音服务）/ArkTS API/speechRecognizer（语音识别）/hms-ai-speechrecognizer.md#speechrecognizercreateengine>)方法，对引擎进行初始化，并创建[SpeechRecognitionEngine](<../../../../harmonyos-references/Core Speech Kit（基础语音服务）/ArkTS API/speechRecognizer（语音识别）/hms-ai-speechrecognizer.md#speechrecognitionengine>)实例。

   createEngine方法提供了两种调用形式，当前以其中一种作为示例，其他方式可参见[speechRecognizer.createEngine](<../../../../harmonyos-references/Core Speech Kit（基础语音服务）/ArkTS API/speechRecognizer（语音识别）/hms-ai-speechrecognizer.md#speechrecognizercreateengine>)。
   ...
   21. });
   ```
3. 得到[SpeechRecognitionEngine](<../../../../harmonyos-references/Core Speech Kit（基础语音服务）/ArkTS API/speechRecognizer（语音识别）/hms-ai-speechrecognizer.md#speechrecognitionengine>)实例对象后，实例化[RecognitionListener](<../../../../harmonyos-references/Core Speech Kit（基础语音服务）/ArkTS API/speechRecognizer（语音识别）/hms-ai-speechrecognizer.md#recognitionlistener>)对象，然后调用[setListener](<../../../../harmonyos-references/Core Speech Kit（基础语音服务）/ArkTS API/speechRecognizer（语音识别）/hms-ai-speechrecognizer.md#setlistener>)方法设置回调，用来接收语音识别相关的回调信息。
   ...
   ```
4. 分别为音频文件转文字和麦克风转文字功能设置开始识别的相关参数，调用[startListening](<../../../../harmonyos-references/Core Speech Kit（基础语音服务）/ArkTS API/speechRecognizer（语音识别）/hms-ai-speechrecognizer.md#startlistening>)方法，开始识别。
   ...
   16. }
   ```
5. 传入音频流，调用[writeAudio](<../../../../harmonyos-references/Core Speech Kit（基础语音服务）/ArkTS API/speechRecognizer（语音识别）/hms-ai-speechrecognizer.md#writeaudio>)方法，开始写入音频流。读取音频文件时，开发者需预先准备一个pcm格式音频文件。
...
6. （可选）当需要查询语音识别服务支持的语种信息，可调用[listLanguages](<../../../../harmonyos-references/Core Speech Kit（基础语音服务）/ArkTS API/speechRecognizer（语音识别）/hms-ai-speechrecognizer.md#listlanguages>)方法。

   listLanguages方法提供了两种调用形式，当前以其中一种作为示例，其他方式可参见[listlanguages](<../../../../harmonyos-references/Core Speech Kit（基础语音服务）/ArkTS API/speechRecognizer（语音识别）/hms-ai-speechrecognizer.md#listlanguages>)。
   ...
   10. });
   ```
7. （可选）当需要结束识别时，可调用[finish](<../../../../harmonyos-references/Core Speech Kit（基础语音服务）/ArkTS API/speechRecognizer（语音识别）/hms-ai-speechrecognizer.md#finish>)方法。

   ```
   1. // 结束识别
   2. asrEngine?.finish(sessionId);
   ```
8. （可选）当需要取消识别时，可调用[cancel](<../../../../harmonyos-references/Core Speech Kit（基础语音服务）/ArkTS API/speechRecognizer（语音识别）/hms-ai-speechrecognizer.md#cancel>)方法。

   ```
   1. // 取消识别
   2. asrEngine?.cancel(sessionId);
   ```
9. （可选）当需要释放语音识别引擎资源时，可调用[shutdown](<../../../../harmonyos-references/Core Speech Kit（基础语音服务）/ArkTS API/speechRecognizer（语音识别）/hms-ai-speechrecognizer.md#shutdown>)方法。

   ```
   1. // 释放识别引擎资源
   2. asrEngine?.shutdown();
   ```
10. 需要在[module.json5配置文件](../../../基础入门/开发基础知识/应用配置文件（Stage模型）/module.json5配置文件/module-configuration-file.md)中添加ohos.permission.MICROPHONE权限，确保麦克风使用正常。详细步骤可查看[声明权限](../../../系统/安全/程序访问控制/应用权限管控/申请应用权限/声明权限/declare-permissions.md)章节。
...
54. }
55. console.info(TAG, 'init start ');
56. }
