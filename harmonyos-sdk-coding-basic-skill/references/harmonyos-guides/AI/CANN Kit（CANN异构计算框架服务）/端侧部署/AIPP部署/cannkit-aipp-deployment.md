...
scraped_at: 2026-06-11T15:16:35+08:00
doc_updated_at: 2026-04-20
content_hash: sha256:8e7e0d470f2062b8a475d3ab5e2dc650095812083cee10e87abe3f053a0f652d
---
## 基本概念
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/bb/v3/E55nlNOpSRikIw6IU82xCg/zh-cn_image_0000002622859097.png?HW-CC-KV=V1&HW-CC-Date=20260611T071634Z&HW-CC-Expire=86400&HW-CC-Sign=4E93E83502713AD1E392C78D103FAD170D8CD612951FB61A4A77C62B78DA0662)

## 接口说明

以下接口为AIPP参数设置接口，如要使用更丰富的设置和查询接口，请参见[API参考](<../../../../../harmonyos-references/CANN Kit（CANN异构计算框架服务）/C API/模块/CANN/cannkit.md>)。
...
1. 调用[HMS\_HiAIAippParam\_Create](<../../../../../harmonyos-references/CANN Kit（CANN异构计算框架服务）/C API/模块/CANN/cannkit.md#hms_hiaiaippparam_create>)创建动态AIPP配置实例。
2. 设置与计算图关联的配置。

   * 调用[HMS\_HiAIAippParam\_SetInputIndex](<../../../../../harmonyos-references/CANN Kit（CANN异构计算框架服务）/C API/模块/CANN/cannkit.md#hms_hiaiaippparam_setinputindex>)设置此动态AIPP配置所在输入的索引。
   * 调用[HMS\_HiAIAippParam\_SetInputAippIndex](<../../../../../harmonyos-references/CANN Kit（CANN异构计算框架服务）/C API/模块/CANN/cannkit.md#hms_hiaiaippparam_setinputaippindex>)设置此动态AIPP配置所在某个输入的输出分支索引。
3. 设置动态AIPP输入图片相关配置。

   * 调用[HMS\_HiAIAippParam\_SetInputFormat](<../../../../../harmonyos-references/CANN Kit（CANN异构计算框架服务）/C API/模块/CANN/cannkit.md#hms_hiaiaippparam_setinputformat>)设置输入图片的格式。
   * 调用[HMS\_HiAIAippParam\_SetInputShape](<../../../../../harmonyos-references/CANN Kit（CANN异构计算框架服务）/C API/模块/CANN/cannkit.md#hms_hiaiaippparam_setinputshape>)设置输入图片原始宽高。
4. 开发者按需设置以下动态AIPP功能参数。

   * 调用[HMS\_HiAIAippParam\_SetChannelSwapConfig](<../../../../../harmonyos-references/CANN Kit（CANN异构计算框架服务）/C API/模块/CANN/cannkit.md#hms_hiaiaippparam_setchannelswapconfig>)设置通道交换参数。
   * 调用[HMS\_HiAIAippParam\_SetCscConfig](<../../../../../harmonyos-references/CANN Kit（CANN异构计算框架服务）/C API/模块/CANN/cannkit.md#hms_hiaiaippparam_setcscconfig>)设置图片色域转换参数。
   * 调用[HMS\_HiAIAippParam\_SetCropConfig](<../../../../../harmonyos-references/CANN Kit（CANN异构计算框架服务）/C API/模块/CANN/cannkit.md#hms_hiaiaippparam_setcropconfig>)设置图片裁剪参数。
   * 调用[HMS\_HiAIAippParam\_SetResizeConfig](<../../../../../harmonyos-references/CANN Kit（CANN异构计算框架服务）/C API/模块/CANN/cannkit.md#hms_hiaiaippparam_setresizeconfig>)设置图片缩放大小参数。
   * 调用[HMS\_HiAIAippParam\_SetPadConfig](<../../../../../harmonyos-references/CANN Kit（CANN异构计算框架服务）/C API/模块/CANN/cannkit.md#hms_hiaiaippparam_setpadconfig>)设置图片填充大小参数。
   * 调用[HMS\_HiAIAippParam\_SetChannelPadding](<../../../../../harmonyos-references/CANN Kit（CANN异构计算框架服务）/C API/模块/CANN/cannkit.md#hms_hiaiaippparam_setchannelpadding>)设置各通道上的填充值参数。
   * 调用[HMS\_HiAIAippParam\_SetRotationAngle](<../../../../../harmonyos-references/CANN Kit（CANN异构计算框架服务）/C API/模块/CANN/cannkit.md#hms_hiaiaippparam_setrotationangle>)设置旋转角度。
   * 调用[HMS\_HiAIAippParam\_SetDtcMeanPixel](<../../../../../harmonyos-references/CANN Kit（CANN异构计算框架服务）/C API/模块/CANN/cannkit.md#hms_hiaiaippparam_setdtcmeanpixel>)设置数据类型转换通道像素平均值。
   * 调用[HMS\_HiAIAippParam\_SetDtcMinPixel](<../../../../../harmonyos-references/CANN Kit（CANN异构计算框架服务）/C API/模块/CANN/cannkit.md#hms_hiaiaippparam_setdtcminpixel>)设置数据类型转换通道像素最小值。
   * 调用[HMS\_HiAIAippParam\_SetDtcVarReciPixel](<../../../../../harmonyos-references/CANN Kit（CANN异构计算框架服务）/C API/模块/CANN/cannkit.md#hms_hiaiaippparam_setdtcvarrecipixel>)设置数据类型转换通道像素方差。
5. 将AIPP配置设置到[NN\_Tensor](<../../../../../harmonyos-references/Neural Network Runtime Kit（Neural Network运行时服务）/C API/结构体/NN_Tensor/capi-neuralnetworkruntime-nn-tensor.md>)。

   通过构造输入输出Tensor后，调用[HMS\_HiAITensor\_SetAippParams](<../../../../../harmonyos-references/CANN Kit（CANN异构计算框架服务）/C API/模块/CANN/cannkit.md#hms_hiaitensor_setaippparams>)给输入Tensor设置AIPP参数。
6. 执行模型推理。
7. 调用[HMS\_HiAIAippParam\_Destroy](<../../../../../harmonyos-references/CANN Kit（CANN异构计算框架服务）/C API/模块/CANN/cannkit.md#hms_hiaiaippparam_destroy>)销毁动态AIPP配置实例。
