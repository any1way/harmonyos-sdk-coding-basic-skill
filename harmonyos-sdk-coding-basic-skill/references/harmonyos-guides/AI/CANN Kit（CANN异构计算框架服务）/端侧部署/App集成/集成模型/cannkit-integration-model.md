...
scraped_at: 2026-06-11T15:16:42+08:00
doc_updated_at: 2026-04-20
content_hash: sha256:6d77bbe622dcef1a3600d8b8e44d996476ba0ffa5cdba11f8a1f09993e32069d
---
...
本节阐述同步模式下单模型的使用，从流程上分别阐述每个步骤在应用层和native层的实现和调用。接口请参见[API参考](<../../../../../../harmonyos-references/CANN Kit（CANN异构计算框架服务）/C API/模块/CANN/cannkit.md>)，示例请参见[SampleCode](https://gitcode.com/HarmonyOS_Samples/cannkit-samplecode-clientdemo-cpp)，本示例支持加载离线模型对图片中的物体进行分类，App运行效果图如下所示。

![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/8a/v3/h8_Y6hdpTjShjNEvgpUVYQ/zh-cn_image_0000002622699219.png?HW-CC-KV=V1&HW-CC-Date=20260611T071641Z&HW-CC-Expire=86400&HW-CC-Sign=5C9FB583B47A4D1976E989DC34E50D2EDE0DF38F2106989E1E304841150FCF88)

## 预置模型

为了让App运行时能够读取到模型文件和处理推理结果，需要先把离线模型和模型对应的结果标签文件预置到工程的“entry/src/main/resources/rawfile”目录中。

本示例所使用的离线模型的转换和生成请参考[Caffe模型转换](../../../模型转换/离线模型转换/模型转换示例/cannkit-model-conversion-example.md#caffe模型转换)。
...
2. （可选）根据需要调用[HMS\_HiAIOptions\_SetOmOptions](<../../../../../../harmonyos-references/CANN Kit（CANN异构计算框架服务）/C API/模块/CANN/cannkit.md#hms_hiaioptions_setomoptions>)接口，打开维测功能（如Profiling）。
3. 把模型buffer传递给HIAIModelManager类的HIAIModelManager::LoadModelFromBuffer接口，该接口调用[OH\_NNCompilation\_ConstructWithOfflineModelBuffer](<../../../../../../harmonyos-references/Neural Network Runtime Kit（Neural Network运行时服务）/C API/头文件/neural_network_core.h/capi-neural-network-core-h.md#oh_nncompilation_constructwithofflinemodelbuffer>)创建模型的编译实例。
4. 设置模型的deviceID。
   ...
   32. }
   ```
5. 调用[OH\_NNCompilation\_Build](<../../../../../../harmonyos-references/Neural Network Runtime Kit（Neural Network运行时服务）/C API/头文件/neural_network_core.h/capi-neural-network-core-h.md#oh_nncompilation_build>)，执行模型编译。
6. 调用[OH\_NNExecutor\_Construct](<../../../../../../harmonyos-references/Neural Network Runtime Kit（Neural Network运行时服务）/C API/头文件/neural_network_core.h/capi-neural-network-core-h.md#oh_nnexecutor_construct>)，创建模型执行器。
7. 调用[OH\_NNCompilation\_Destroy](<../../../../../../harmonyos-references/Neural Network Runtime Kit（Neural Network运行时服务）/C API/头文件/neural_network_core.h/capi-neural-network-core-h.md#oh_nncompilation_destroy>)，释放模型编译实例。
...
调用[OH\_NNExecutor\_RunSync](<../../../../../../harmonyos-references/Neural Network Runtime Kit（Neural Network运行时服务）/C API/头文件/neural_network_core.h/capi-neural-network-core-h.md#oh_nnexecutor_runsync>)，完成模型的同步推理。
...
1. 调用[OH\_NNTensor\_GetDataBuffer](<../../../../../../harmonyos-references/Neural Network Runtime Kit（Neural Network运行时服务）/C API/头文件/neural_network_core.h/capi-neural-network-core-h.md#oh_nntensor_getdatabuffer>)，获取输出的Tensor，在输出Tensor中会得到模型的输出数据。
...
开发者可根据需要自行设置模型推理优先级。使用[OH\_NNCompilation\_SetPriority](<../../../../../../harmonyos-references/Neural Network Runtime Kit（Neural Network运行时服务）/C API/头文件/neural_network_core.h/capi-neural-network-core-h.md#oh_nncompilation_setpriority>)接口，默认值为OH\_NN\_PRIORITY\_NONE，本接口应在模型推理前调用。
