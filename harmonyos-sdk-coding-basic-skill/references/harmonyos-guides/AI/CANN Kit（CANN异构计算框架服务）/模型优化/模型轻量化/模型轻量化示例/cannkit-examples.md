...
scraped_at: 2026-06-11T15:16:23+08:00
doc_updated_at: 2026-04-20
content_hash: sha256:45379b043a13d3069ca478c7fb0342b82750b7941595ab69ef00e910e7e51a4d
---
## TensorFlow Quant\_INT8-8无训练量化Demo

### 环境准备

请参见[环境准备](../无训练量化/cannkit-no-training-and-quantization.md#环境准备)，安装TensorFlow及依赖。
...
* 准备量化输入数据
  ...
  参见[模型量化](../无训练量化/cannkit-no-training-and-quantization.md#模型量化)，将图片或二进制形式的校准集放入"dopt\_tf\_py3/demo/quant8-8/notrain/tensorflow\_mnist/mnist\_test/"中。该路径下已经放入了图片校准集。
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/6b/v3/rNHPnb7fQOOliE9fetbrVw/zh-cn_image_0000002622699209.png?HW-CC-KV=V1&HW-CC-Date=20260611T071622Z&HW-CC-Expire=86400&HW-CC-Sign=680D1E8F0F46CE014CAA120C47B8457DD8FBEA50C9F2ABA1778AEA45511744AC)

## PyTorch Quant\_INT8-8无训练量化Demo

### 环境准备

请参见[环境准备](../无训练量化/cannkit-no-training-and-quantization.md#环境准备-1)，安装PyTorch及依赖。
...
* 准备量化输入数据
  ...
  参见[模型量化](../无训练量化/cannkit-no-training-and-quantization.md#模型量化-1)，将图片或二进制形式的校准集放入"dopt\_pytorch\_py3/demo/quant8-8/notrain/pytorch\_mnist/"中。
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/49/v3/IaJNeS3FSf6ce6CLtO6u9Q/zh-cn_image_0000002592219648.png?HW-CC-KV=V1&HW-CC-Date=20260611T071622Z&HW-CC-Expire=86400&HW-CC-Sign=533AB2043D116835756E8878D2852EDB591D737DBDE9BDE0D1976FDE309B15FD)

## ONNX Quant\_INT8-8无训练量化Demo

### 环境准备

环境准备请参见[环境准备](../无训练量化/cannkit-no-training-and-quantization.md#环境准备-2)，安装ONNX及依赖。
...
其中，./config.prototxt配置内容为（[配置文件使用方法](../无训练量化/cannkit-no-training-and-quantization.md#onnx模型无训练量化)）：
...
请参见[准备TensorFlow环境](../插件式量化/cannkit-plugin-based-quantization.md#准备tensorflow环境)。安装TensorFlow-gpu 2.8.0版本以及其必要的依赖。
...
请参见[准备PyTorch环境](../插件式量化/cannkit-plugin-based-quantization.md#准备pytorch环境)。安装PyTorch-gpu 1.11版本以及其必要的依赖。
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/8/v3/VXAJh3h8TwOwVKjnbt-Tbg/zh-cn_image_0000002592379582.png?HW-CC-KV=V1&HW-CC-Date=20260611T071622Z&HW-CC-Expire=86400&HW-CC-Sign=6186AEEF4075CDD77ECBC485A10B703EA59F602554617B2D0ECE7B43193062F0)
...
2. 环境准备请参见[环境准备](../网络结构搜索训练/cannkit-network-structure-search-training.md#环境准备)。
3. 加载依赖的开源代码：
...
4. 配置demo下的scen.yaml文件，请参见[搜索参数配置](../网络结构搜索训练/cannkit-network-structure-search-training.md#搜索参数配置)。scen.yaml中提供了建议参数，开发者可根据实际需求修改。
5. 修改demo下的user\_module.py文件，模型接口定义请参见[TensorFlow用户自定义接口](../网络结构搜索训练/cannkit-network-structure-search-training.md#tensorflow开发者自定义接口)。user\_module.py中提供了建议配置，开发者可根据实际需求进行修改。
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/7d/v3/z4BXPZNjSWq7nq7EL5m7XQ/zh-cn_image_0000002622859091.png?HW-CC-KV=V1&HW-CC-Date=20260611T071622Z&HW-CC-Expire=86400&HW-CC-Sign=47E289EE22B95CD12407C238BE07AD091106E4287D8167359B0672D6C10DB9C3)
...
1. 准备数据集，包括用于预训的ImageNet数据集（tfrecord格式）和用于训练的COCO数据集（原始格式）。若有完成预训练的ckpt文件，则不需再准备ImageNet数据集。请参见[搜索参数配置](../网络结构搜索训练/cannkit-network-structure-search-training.md#搜索参数配置)，修改scen.yaml文件中的数据集路径。
2. 环境准备请参见[环境准备](../网络结构搜索训练/cannkit-network-structure-search-training.md#环境准备)。
3. 加载依赖的开源代码。
...
4. 配置demo的scen.yaml文件和pre\_train.yaml，请参见[搜索参数配置](../网络结构搜索训练/cannkit-network-structure-search-training.md#搜索参数配置)。scen.yaml中提供了建议参数，开发者可根据实际需求修改。
5. 修改demo的user\_module.py文件，模型接口定义请参见[TensorFlow用户自定义接口](../网络结构搜索训练/cannkit-network-structure-search-training.md#tensorflow开发者自定义接口)。user\_module.py中提供了建议配置，开发者可根据实际需求进行修改。
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/8d/v3/uVl9dvXoSrK69pt36wVS9Q/zh-cn_image_0000002622699211.png?HW-CC-KV=V1&HW-CC-Date=20260611T071622Z&HW-CC-Expire=86400&HW-CC-Sign=640095E4F55189AE5CB3A80A3077EAB4C925CACE26F2B355372D4762162E9E18)
...
1. 准备数据集，包括用于预训练的ImageNet数据集（tfrecord格式）和用于训练的VOC数据集（tfrecord格式）。若有完成预训练的ckpt文件，则不需再准备ImageNet数据集。请参见[搜索参数配置](../网络结构搜索训练/cannkit-network-structure-search-training.md#搜索参数配置)，修改scen.yaml文件中的数据集路径。
2. 环境准备请参见[环境准备](../网络结构搜索训练/cannkit-network-structure-search-training.md#环境准备)。
3. 加载依赖的开源代码。
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/ab/v3/q3NaQEJ7TfKKDseQgps1PQ/zh-cn_image_0000002592219650.png?HW-CC-KV=V1&HW-CC-Date=20260611T071622Z&HW-CC-Expire=86400&HW-CC-Sign=65E1E975B0D7A88386E582061D652714F7F17A6BCA782F923F61CA99D7BF718E)
...
2. 环境准备请参见[环境准备](../网络结构搜索训练/cannkit-network-structure-search-training.md#环境准备)。
3. 配置demo下的scen.yaml文件，请参见[搜索参数配置](../网络结构搜索训练/cannkit-network-structure-search-training.md#搜索参数配置)。scen.yaml中提供了建议参数，开发者可根据实际需求修改。
4. 修改demo下的user\_module.py文件，模型接口定义请参见[PyTorch开发者自定义接口](../网络结构搜索训练/cannkit-network-structure-search-training.md#pytorch开发者自定义接口)。user\_module.py中提供了建议配置，开发者可根据实际需求进行修改。
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/2f/v3/ixV-pPZ5TSyn7yUARBCasQ/zh-cn_image_0000002592379584.png?HW-CC-KV=V1&HW-CC-Date=20260611T071622Z&HW-CC-Expire=86400&HW-CC-Sign=B4D94A378460EC99CDE8AEF554797C4836E2660F4D38214462488F8EC44320A1)
...
1. 准备数据集，包括用于预训练的ImageNet数据集（原始格式）和用于训练VOC数据集（原始格式）。若有完成预训练的ckpt文件，则不需再准备ImageNet数据集。请参见[搜索参数配置](../网络结构搜索训练/cannkit-network-structure-search-training.md#搜索参数配置)，修改scen.yaml文件中的数据集路径。
2. 环境准备请参见[环境准备](../网络结构搜索训练/cannkit-network-structure-search-training.md#环境准备)。
3. 加载依赖的开源代码：参考tools\_dopt/dopt\_pytorch\_py3/demo/nas\_ea/ea\_seg\_voc\_pytorch/readme.md
4. 配置demo下的scen.yaml文件和pre\_train.yaml文件，请参见[搜索参数配置](../网络结构搜索训练/cannkit-network-structure-search-training.md#搜索参数配置)。scen.yaml中提供了建议参数，开发者可根据实际需求修改。
5. 修改demo下的user\_module.py文件，模型接口定义请参见[PyTorch开发者自定义接口](../网络结构搜索训练/cannkit-network-structure-search-training.md#pytorch开发者自定义接口)。user\_module.py中提供了建议配置，开发者可根据实际需求进行修改。
6. 执行脚本run\_release.sh，在results下，生成多个model\_arch\_result\_\*.py文件。开发者可根据log\_segmentation中提供的信息选择合适的网络结构进行训练。后续训练可参考readme.md中的指导。
