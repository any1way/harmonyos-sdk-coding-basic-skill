...
content_hash: sha256:70a8d6b0b1f55b9d538a3b6dab8aa087096ff97c697310ec29464e8d88fb88d3
---
## 适配插件开发
...
3. 实现回调函数ParseParamByOpFunc。其函数声明如下所示：
   ...
   * op\_dest：DDK算子数据结构，保存算子信息，Operator类的详细描述请参见[Operator](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/cannkit-operator-construction-and-destructor)。
   ...
   ```
   1. Status ParseParamAddCustom(const ge::Operator& op_src, ge::Operator& op_dest) {
...
2. 在%{DDK\_INSTALL\_PATH}/tools/tools\_omg执行如下命令生成离线模型。（如下命令中使用的目录以及文件均为样例，请以实际为准）
   ...
   ```
   1. ./omg --model ./add_custom.onnx --framework 5 --output out/custom_graph --target=omc --platform kirin9020
   ```
...
4. 参考[生成输入数据](../../算子入图（GE图）开发/图编译和图执行/cannkit-graph-compilation-and-execution.md#生成输入数据)准备符合模型输入要求的\*.bin格式的输入数据。
5. 参考[APP集成代码](../../算子入图（GE图）开发/图编译和图执行/cannkit-graph-compilation-and-execution.md#app集成代码)，完成模型集成。
