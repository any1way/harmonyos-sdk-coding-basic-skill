...
scraped_at: 2026-06-01T15:26:25+08:00
doc_updated_at: 2026-04-30
content_hash: sha256:c463569a2a33ab12cfe29c26df676820312960d42e00b905f5573eab39a39c80
---
## Hvigor预置对象
...
获取根项目的节点对象。在[nodesInitialized和之后的阶段](../../../概述/构建系统生命周期/ide-hvigor-life-cycle.md#section746253616316)才能使用，否则会报错。
...
此API写在[hvigorconfig.ts文件](../../../概述/构建系统生命周期/ide-hvigor-life-cycle.md#section810245135914)中才会生效，在构建生命周期的初始化阶段被执行。
...
从DevEco Studio 6.0.0 Beta3版本开始，支持排除工程中不存在的模块，具体请参考[通过Hvigor执行ohpm install](../../../提升构建效率/性能优化实验特性/ide-hvigor-experimental-properties.md#section11663628141317)。
...
HarmonyOS应用工程级配置中的Product信息接口。在调用Product的接口前，可以先通过OhosAppContext的[getCurrentProduct](../插件上下文/ide-build-expanding-context.md#section962214395515)方法来获取本对象。
...
依赖其他模块的任务时，建议将任务注册操作放在[hook-nodesEvaluated](../../../概述/构建系统生命周期/ide-hvigor-life-cycle.md#section746253616316)中，否则注册时其他模块的任务可能还未被初始化，导致当前任务无法注册。
...
应用工程使用的SDK相关的信息。在调用SdkDetails的接口前，可以先通过OhosAppContext的[getSdkDetails](../插件上下文/ide-build-expanding-context.md#section085712212299)方法来获取本对象。
