...
scraped_at: 2026-06-01T14:55:43+08:00
doc_updated_at: 2026-05-14
content_hash: sha256:4866ac4b32c2c4e7250436b50dc642b8fea30c3c27b8cdb96d0ac5882cfe9854
---
## 约束与限制

从5.1.0(18)开始，识别平面语义能力支持部分Phone、部分Tablet设备。请参考[硬件要求](../../开发准备/arengine-preparations.md#硬件要求)判断设备是否支持平面及物体语义特性（[ARENGINE\_FEATURE\_TYPE\_SEMANTIC](<../../../../../harmonyos-references/AR Engine（AR引擎服务）/ArkTS API/arEngine（AR增强现实能力）/arengine-api-arengine.md#arfeaturetype>)）。

## 接口说明

获取平面语义信息可以通过[ARPlane](<../../../../../harmonyos-references/AR Engine（AR引擎服务）/ArkTS API/arEngine（AR增强现实能力）/arengine-api-arengine.md#arplane>)平面对象获取，以下接口为平面相关接口。详细接口和说明，请参考[AR Engine API参考](<../../../../../harmonyos-references/AR Engine（AR引擎服务）/ArkTS API/arEngine（AR增强现实能力）/arengine-api-arengine.md>)。

| 接口名 | 描述 |
| --- | --- |
| [ARTrackable.getPose](<../../../../../harmonyos-references/AR Engine（AR引擎服务）/ArkTS API/arEngine（AR增强现实能力）/arengine-api-arengine.md#artrackablegetpose>) | 获取追踪目标的位姿信息。 |
| [ARTrackable.getAnchors](<../../../../../harmonyos-references/AR Engine（AR引擎服务）/ArkTS API/arEngine（AR增强现实能力）/arengine-api-arengine.md#artrackablegetanchors>) | 获取绑定到输入可跟踪对象的锚点对象。 |
...
| [ARPlane.getPolygonXZ](<../../../../../harmonyos-references/AR Engine（AR引擎服务）/ArkTS API/arEngine（AR增强现实能力）/arengine-api-arengine.md#arplanegetpolygonxz>) | 获取检测到的平面2D顶点数组。 |
| [ARPlane.getSubsumedBy](<../../../../../harmonyos-references/AR Engine（AR引擎服务）/ArkTS API/arEngine（AR增强现实能力）/arengine-api-arengine.md#arplanegetsubsumedby>) | 获取平面的父平面（当平面与另一个平面合并时会生成父平面）。 |
| [ARPlane.isPoseInExtents](<../../../../../harmonyos-references/AR Engine（AR引擎服务）/ArkTS API/arEngine（AR增强现实能力）/arengine-api-arengine.md#arplaneisposeinextents>) | 检查给定位姿是否在平面的边界矩形内。 |
| [ARPlane.isPoseInPolygon](<../../../../../harmonyos-references/AR Engine（AR引擎服务）/ArkTS API/arEngine（AR增强现实能力）/arengine-api-arengine.md#arplaneisposeinpolygon>) | 检查给定位姿是否在平面的边界多边形内。 |
...
识别平面语义之前需要先检测识别环境中的平面，如何检测识别环境中的平面请参考[检测环境中的平面](../../平面识别/检测环境中的平面（ArkTS）/arengine-get-plane.md)。
...
更改semanticMode为[ARSemanticMode](<../../../../../harmonyos-references/AR Engine（AR引擎服务）/ArkTS API/arEngine（AR增强现实能力）/arengine-api-arengine.md#arsemanticmode>).PLANE，启用平面语义识别能力。
...
自定义方法获取顶点数据getVertices、创建索引generateMeshIndex、创建mesh数据generateMeshInput，可参考[检测平面的自定义方法](../../平面识别/检测环境中的平面（ArkTS）/arengine-get-plane.md#检测平面的自定义方法)。

arrayBufferFloat32ToNumber可以参考[数据类型转换说明](<../../AR Engine常见问题/数据类型转换说明/arengine-arraybuffer-info.md>)。
