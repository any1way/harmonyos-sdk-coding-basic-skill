...
scraped_at: 2026-06-11T15:01:01+08:00
doc_updated_at: 2026-05-26
content_hash: sha256:b815a67c180c37f9d5825924a6508b2b9c942710c92d10df419dbcf8dd9a0dee
---
...
模型加载后，可以通过ArkUI的[Component3D](../../../../harmonyos-references/ArkUI（方舟UI框架）/ArkTS组件/渲染绘制/Component3D/ts-basic-components-component3d.md)渲染组件呈现给用户，Component3D负责将ArkGraphics 3D场景渲染到界面中。在自定义场景模式下，开发者可以使用ArkTS API创建并管理相机和光源节点，从而设置合适的观察角度和光照效果；在自动场景模式下，框架会根据模型自动创建基础相机和光照。ArkTS API可通过napi调用AGP中由C++实现的相应能力。

![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/c/v3/q1HlBJRXTjqXbaZDPo_uHg/zh-cn_image_0000002622858575.png?HW-CC-KV=V1&HW-CC-Date=20260611T070100Z&HW-CC-Expire=86400&HW-CC-Sign=9CDCA5867A7C85516C8EDF7E6B5F194A7890E02946245FCF129233DEF7A28CDB)
...
1. 导入相关模块。
   ...
   [init.ets](https://gitcode.com/HarmonyOS_Samples/guide-snippets/blob/HarmonyOS-feature-20260402/ArkGraphics3D/entry/src/main/ets/scene/init.ets#L16-L18)
2. 加载场景资源。

   ArkGraphics 3D提供Scene.load()接口用于加载glTF模型文件，模型加载成功后会返回一个Scene实例，可通过它继续创建相机或光照等组件。模型文件支持.gltf与.glb格式，其中.glb为二进制封装形式，与.gltf内容等价但更便于加载与使用。模型加载支持相对路径和绝对路径两种方式：相对路径加载使用$rawfile()，从应用的resources/rawfile/目录读取内置资源文件；绝对路径加载需从应用沙箱目录读取文件，Scene.load()仅支持访问应用自身创建或写入的沙箱文件，具体示例可参见[load()](<../../../../harmonyos-references/ArkGraphics 3D（方舟3D图形）/ArkTS API/graphics3d/Scene/js-apis-inner-scene.md#load>)。
   ...
   [init.ets](https://gitcode.com/HarmonyOS_Samples/guide-snippets/blob/HarmonyOS-feature-20260402/ArkGraphics3D/entry/src/main/ets/scene/init.ets#L40-L65)
3. 创建相机并设置场景渲染参数。
   ...
   [init.ets](https://gitcode.com/HarmonyOS_Samples/guide-snippets/blob/HarmonyOS-feature-20260402/ArkGraphics3D/entry/src/main/ets/scene/init.ets#L49-L57)
4. 渲染3D模型。
   ...
   [init.ets](https://gitcode.com/HarmonyOS_Samples/guide-snippets/blob/HarmonyOS-feature-20260402/ArkGraphics3D/entry/src/main/ets/scene/init.ets#L72-L79)
...
1. 导入相关模块。
   ...
   [camera.ets](https://gitcode.com/HarmonyOS_Samples/guide-snippets/blob/HarmonyOS-feature-20260402/ArkGraphics3D/entry/src/main/ets/scene/camera.ets#L16-L18)
2. 加载场景资源。
   ...
   [camera.ets](https://gitcode.com/HarmonyOS_Samples/guide-snippets/blob/HarmonyOS-feature-20260402/ArkGraphics3D/entry/src/main/ets/scene/camera.ets#L25-L65)
3. 创建相机并配置相机参数。
   ...
   [camera.ets](https://gitcode.com/HarmonyOS_Samples/guide-snippets/blob/HarmonyOS-feature-20260402/ArkGraphics3D/entry/src/main/ets/scene/camera.ets#L36-L59)
4. 初始化与渲染绑定。
   ...
   [camera.ets](https://gitcode.com/HarmonyOS_Samples/guide-snippets/blob/HarmonyOS-feature-20260402/ArkGraphics3D/entry/src/main/ets/scene/camera.ets#L94-L103)
5. 相机交互。
   ...
   [camera.ets](https://gitcode.com/HarmonyOS_Samples/guide-snippets/blob/HarmonyOS-feature-20260402/ArkGraphics3D/entry/src/main/ets/scene/camera.ets#L168-L187)
...
1. 导入相关模块。
   ...
   [light.ets](https://gitcode.com/HarmonyOS_Samples/guide-snippets/blob/HarmonyOS-feature-20260402/ArkGraphics3D/entry/src/main/ets/scene/light.ets#L16-L18)
2. 加载场景资源。
   ...
   [light.ets](https://gitcode.com/HarmonyOS_Samples/guide-snippets/blob/HarmonyOS-feature-20260402/ArkGraphics3D/entry/src/main/ets/scene/light.ets#L25-L61)
3. 创建灯光并配置灯光参数。
   ...
   [light.ets](https://gitcode.com/HarmonyOS_Samples/guide-snippets/blob/HarmonyOS-feature-20260402/ArkGraphics3D/entry/src/main/ets/scene/light.ets#L36-L53)
4. 初始化与渲染绑定。
   ...
   [light.ets](https://gitcode.com/HarmonyOS_Samples/guide-snippets/blob/HarmonyOS-feature-20260402/ArkGraphics3D/entry/src/main/ets/scene/light.ets#L94-L108)
5. 灯光交互。
   ...
   [light.ets](https://gitcode.com/HarmonyOS_Samples/guide-snippets/blob/HarmonyOS-feature-20260402/ArkGraphics3D/entry/src/main/ets/scene/light.ets#L123-L147)
