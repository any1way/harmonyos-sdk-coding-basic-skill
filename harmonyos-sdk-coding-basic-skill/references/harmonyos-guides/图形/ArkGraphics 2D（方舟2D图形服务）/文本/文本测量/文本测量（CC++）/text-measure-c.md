...
scraped_at: 2026-06-01T14:57:00+08:00
doc_updated_at: 2026-05-26
content_hash: sha256:c83bb742a6f0df2fed01add3ec8fb5b8dac024062fe0fabb846fd3e8a99feedb
---
## 场景介绍
...
文本测量中常用接口如下表所示，详细接口说明参考[drawing\_text\_typography.h](<../../../../../../harmonyos-references/ArkGraphics 2D（方舟2D图形服务）/C API/头文件/drawing_text_typography.h/capi-drawing-text-typography-h.md>)。
...
2. 导入依赖的相关头文件。
   ...
   [sample\_bitmap.cpp](https://gitcode.com/HarmonyOS_Samples/guide-snippets/blob/HarmonyOS-feature-20260402/ArkGraphics2D/TextEngine/NDKTextMeasurement/entry/src/main/cpp/samples/sample_bitmap.cpp#L17-L21)
3. 创建段落生成器ParagraphBuilder，并设置段落样式。

   ```
   1. // 创建文本样式，设置文本颜色为黑色并设置字体大小为50
   2. OH_Drawing_TextStyle *myTextStyle = OH_Drawing_CreateTextStyle();
   3. OH_Drawing_SetTextStyleColor(myTextStyle, OH_Drawing_ColorSetArgb(0xFF, 0x00, 0x00, 0x00));
   4. OH_Drawing_SetTextStyleFontSize(myTextStyle, 50.0);
   5. // 创建一个段落样式对象，以设置排版风格
   6. OH_Drawing_TypographyStyle *typographyStyle = OH_Drawing_CreateTypographyStyle();
   7. // 设置段落样式的对齐方式为左对齐
   8. OH_Drawing_SetTypographyTextAlign(typographyStyle, TEXT_ALIGN_LEFT);
   9. // 创建一个段落生成器
   10. OH_Drawing_TypographyCreate *handler = OH_Drawing_CreateTypographyHandler(typographyStyle, fontCollection);
   11. // 在段落生成器中设置文本样式
   12. OH_Drawing_TypographyHandlerPushTextStyle(handler, myTextStyle);
   13. // 在段落生成器中添加文本内容
   14. const char *text = "排版测量的文字度量信息";
   ...
   [sample\_bitmap.cpp](https://gitcode.com/HarmonyOS_Samples/guide-snippets/blob/HarmonyOS-feature-20260402/ArkGraphics2D/TextEngine/NDKTextMeasurement/entry/src/main/cpp/samples/sample_bitmap.cpp#L171-L189)
4. 调用排版接口并设置段落排版宽度，对段落进行塑型排版。
   ...
   [sample\_bitmap.cpp](https://gitcode.com/HarmonyOS_Samples/guide-snippets/blob/HarmonyOS-feature-20260402/ArkGraphics2D/TextEngine/NDKTextMeasurement/entry/src/main/cpp/samples/sample_bitmap.cpp#L193-L196)
5. 调用段落测量信息获取接口，获取指定数据。
   ...
   [sample\_bitmap.cpp](https://gitcode.com/HarmonyOS_Samples/guide-snippets/blob/HarmonyOS-feature-20260402/ArkGraphics2D/TextEngine/NDKTextMeasurement/entry/src/main/cpp/samples/sample_bitmap.cpp#L199-L231)
