...
scraped_at: 2026-06-11T15:02:22+08:00
doc_updated_at: 2026-05-26
content_hash: sha256:85b898f828ab9ac03c42be1908d914ac3bd026c8607bedec5285d94f5a674191
---
...
* 可通过以下方式查询相关扩展特性是否支持：
  ...
  对于Vulkan，使用[HMS\_XEG\_EnumerateDeviceExtensionProperties](<../../../../harmonyos-references/XEngine Kit（GPU加速引擎服务）/C API/模块/XEngine/xengine-kit-xengine.md#hms_xeg_enumeratedeviceextensionproperties>)扩展特性查询接口进行查询，如查询结果包含XEG\_RT\_SHADOW\_AO\_EXTENSION\_NAME，则表示支持该特性，若查询结果未包含，则表示不支持该特性。

## 接口说明

以下为光线追踪阴影和环境光遮蔽特性需要使用的接口，详细说明请参考[接口文档](<../../../../harmonyos-references/XEngine Kit（GPU加速引擎服务）/C API/模块/XEngine/xengine-kit-xengine.md>)。
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/5f/v3/hLsae6J8R-iWlqxQPmi5cA/zh-cn_image_0000002592219172.jpg?HW-CC-KV=V1&HW-CC-Date=20260611T070221Z&HW-CC-Expire=86400&HW-CC-Sign=08CE10FE5D0890382C2EE58FC3ED184D160EABBEE62F5B46FD12B43F0E49D43C)
...
XEngine VisibleMask特性的光线追踪阴影（Ray-Traced Shadow，简称RTShadow）和环境光遮蔽（Ray-Traced AO，简称RTAO）效果API需要与Vulkan API延迟渲染管线配合使用。相关代码在Native层实现，渲染结果通过[XComponent](../../../../harmonyos-references/ArkUI（方舟UI框架）/ArkTS组件/渲染绘制/XComponent/ts-basic-components-xcomponent.md)组件显示到屏幕。

在调用XEngine Kit特性接口前，需要先通过[Syscap](../../../../harmonyos-references/系统能力SystemCapability使用指南/syscap.md#什么是systemcapabilitysyscap)查询确认您的目标设备支持SystemCapability.Graphic.XEngine系统能力。

1. 调用[HMS\_XEG\_EnumerateDeviceExtensionProperties](<../../../../harmonyos-references/XEngine Kit（GPU加速引擎服务）/C API/模块/XEngine/xengine-kit-xengine.md#hms_xeg_enumeratedeviceextensionproperties>)接口，获取XEngine支持的扩展信息，只有在支持XEG\_RT\_SHADOW\_AO\_EXTENSION\_NAME扩展时才可以使用光线追踪阴影和环境光遮蔽特性的接口。
   ...
   20. }
   ```
2. 调用[HMS\_XEG\_CreateRTVisibleMask](<../../../../harmonyos-references/XEngine Kit（GPU加速引擎服务）/C API/模块/XEngine/xengine-kit-xengine.md#hms_xeg_creatertvisiblemask>)接口，创建实例句柄。
   ...
   23. }
   ```
3. 调用[HMS\_XEG\_CmdRenderRTVisibleMask](<../../../../harmonyos-references/XEngine Kit（GPU加速引擎服务）/C API/模块/XEngine/xengine-kit-xengine.md#hms_xeg_cmdrenderrtvisiblemask>)接口执行渲染命令，每帧都需要调用。
   ...
   40. // 以相机实际位置的x坐标为准
   41. description.worldCameraOrigin[0] = 0.0;
   42. // 以相机实际位置的y坐标为准
   43. description.worldCameraOrigin[1] = 0.0;
   44. // 以相机实际位置的z坐标为准
   45. description.worldCameraOrigin[2] = 0.0;
   46. // gBufferDepth是GBuffer深度图像的VkImageView，需要用户进行初始化
   47. VkImageView gBufferDepth;
   48. description.inputDepthImage = gBufferDepth;
   49. // gBufferNormal是GBuffer法线图像的VkImageView，需要用户进行初始化，关于法线的格式和编码详见API参考
   50. VkImageView gBufferNormal;
   51. description.inputNormalImage = gBufferNormal;
   52. // outputShadowAOView是保存XEngine RTShadow和RTAO渲染输出的VkImageView，需要用户进行初始化
   53. VkImageView outputShadowAOView;
   54. description.outputShadowAOImage = outputShadowAOView;
   55. // sceneTlas是场景的Top Level光线追踪加速结构，需要用户进行初始化
   56. VkAccelerationStructureKHR sceneTlas;
   57. description.accelerationStructure = sceneTlas;
   58. // 相机观察矩阵，需要用户进行初始化
   59. float viewMatrix[16];
   60. // 相机投影矩阵，需要用户进行初始化
   61. float projectionMatrix[16];
   62. memcpy(description.viewMatrix, viewMatrix, sizeof(viewMatrix));
   63. memcpy(description.projectionMatrix, projectionMatrix, sizeof(projectionMatrix));
   64. // Vulkan命令缓冲区，需要用户进行初始化
   65. VkCommandBuffer vkCommandBuffer;
   66. VkResult ret = HMS_XEG_CmdRenderRTVisibleMask(vkCommandBuffer, rtVisibleMask, &description);
   67. if (ret != VK_SUCCESS) {
   68. // 录制命令错误，进行错误处理
   69. }
   70. // 设置Pipeline Barrier以同步对RTShadow和RTAO渲染输出的读取
   71. VkImageMemoryBarrier imageMemoryBarrier;
   72. imageMemoryBarrier.sType = VK_STRUCTURE_TYPE_IMAGE_MEMORY_BARRIER;
   73. imageMemoryBarrier.pNext = nullptr;
   74. imageMemoryBarrier.srcAccessMask = VK_ACCESS_COLOR_ATTACHMENT_WRITE_BIT;
   75. // 根据实际访问方式设置
   76. imageMemoryBarrier.dstAccessMask = VK_ACCESS_COLOR_ATTACHMENT_READ_BIT;
   77. imageMemoryBarrier.oldLayout = VK_IMAGE_LAYOUT_COLOR_ATTACHMENT_OPTIMAL;
   78. imageMemoryBarrier.newLayout = VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL;
   79. imageMemoryBarrier.srcQueueFamilyIndex = VK_QUEUE_FAMILY_IGNORED;
   80. imageMemoryBarrier.dstQueueFamilyIndex = VK_QUEUE_FAMILY_IGNORED;
   81. // outputShadowAOImage是保存RTShadow和RTAO渲染输出的VkImage，需要用户进行初始化
   82. VkImage outputShadowAOImage;
   83. imageMemoryBarrier.image = outputShadowAOImage;
   84. imageMemoryBarrier.subresourceRange = { VK_IMAGE_ASPECT_COLOR_BIT, 0, 1, 0, 1 };
   85. vkCmdPipelineBarrier(vkCommandBuffer,
   86. VK_PIPELINE_STAGE_COLOR_ATTACHMENT_OUTPUT_BIT,
   87. // 根据实际访问stage设置
   88. VK_PIPELINE_STAGE_FRAGMENT_SHADER_BIT,
   89. 0, 0, nullptr, 0, nullptr, 1, &imageMemoryBarrier);
   ```
   ...
   ```
   1. // 光照片段着色器代码示例
   2. // textureRayTracingOutputShadowAO需要绑定为RTShadow和RTAO的输出图像
   3. layout (binding = 0) uniform sampler2D textureRayTracingOutputShadowAO;

   5. // color为当前着色点不考虑阴影和环境光遮蔽时的颜色值
   6. vec3 color;
   7. // 用户的着色点颜色计算过程
   8. // ...
   9. // 采样阴影和环境光遮蔽纹理，TexCoords为当前采样点的UV坐标。
   10. vec2 shadowAO = texture(textureRayTracingOutputShadowAO, TexCoords).xy;
   11. float shadow = shadowAO.x;
   12. float ao = shadowAO.y;
   13. // finalColor为最终颜色值
   14. vec3 finalColor = color * pow(ao, 2.0) * shadow;
   ```
4. 调用[HMS\_XEG\_DestroyRTVisibleMask](<../../../../harmonyos-references/XEngine Kit（GPU加速引擎服务）/C API/模块/XEngine/xengine-kit-xengine.md#hms_xeg_destroyrtvisiblemask>)接口销毁特性实例句柄以释放资源，在不需要再使用特性或应用退出时需要调用。
