...
content_hash: sha256:9deaa509a76cdc1ade27d63804caaa1434136b12a2fedb118a50c8ebe4fcead7
---
...
* 可通过以下方式查询相关扩展特性是否支持：
  ...
  对于OpenGL ES，使用[HMS\_XEG\_GetString](<../../../../harmonyos-references/XEngine Kit（GPU加速引擎服务）/C API/模块/XEngine/xengine-kit-xengine.md#hms_xeg_getstring>)扩展特性查询接口进行查询，如查询结果包含[XEG\_NEURAL\_UPSCALE\_EXTENSION\_NAME](<../../../../harmonyos-references/XEngine Kit（GPU加速引擎服务）/C API/模块/XEngine/xengine-kit-xengine.md#xeg_neural_upscale_extension_name>)，则表示支持该特性，若查询结果未包含，则表示不支持该特性。

## 接口说明

以下接口为OpenGL ES空域AI超分设置接口，如需使用更丰富的设置和查询接口，具体API说明详见[接口文档](<../../../../harmonyos-references/XEngine Kit（GPU加速引擎服务）/C API/模块/XEngine/xengine-kit-xengine.md>)。
...
* 下面是基于OpenGL ES图形API平台集成空域AI超分的主要业务流程
  ...
  ![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/ea/v3/91AryLf0TM6h9JIJPHs7Lw/zh-cn_image_0000002622858609.jpg?HW-CC-KV=V1&HW-CC-Date=20260611T070217Z&HW-CC-Expire=86400&HW-CC-Sign=BB99F475F87A08FB01DB7ADDF650D2C7829F0C202DC11E58182DCEACB9A9EB49)
...
Native层实现使用OpenGL ES和XEngine图形API搭建图像渲染管线并集成空域AI超分，渲染结果通过[XComponent](../../../../harmonyos-references/ArkUI（方舟UI框架）/ArkTS组件/渲染绘制/XComponent/ts-basic-components-xcomponent.md)组件显示到屏幕。
...
1. 调用[HMS\_XEG\_GetString](<../../../../harmonyos-references/XEngine Kit（GPU加速引擎服务）/C API/模块/XEngine/xengine-kit-xengine.md#hms_xeg_getstring>)接口，获取XEngine支持的扩展信息，只有在支持[XEG\_NEURAL\_UPSCALE\_EXTENSION\_NAME](<../../../../harmonyos-references/XEngine Kit（GPU加速引擎服务）/C API/模块/XEngine/xengine-kit-xengine.md#xeg_neural_upscale_extension_name>)扩展时才可以使用空域AI超分的相关接口。
...
3. 在超分输入纹理上进行渲染。
   ...
   ```
4. 调用[HMS\_XEG\_NeuralUpscaleParameter](<../../../../harmonyos-references/XEngine Kit（GPU加速引擎服务）/C API/模块/XEngine/xengine-kit-xengine.md#hms_xeg_neuralupscaleparameter>)接口，设置空域AI超分的输入参数。
   ...
   ```
5. 调用[HMS\_XEG\_RenderNeuralUpscale](<../../../../harmonyos-references/XEngine Kit（GPU加速引擎服务）/C API/模块/XEngine/xengine-kit-xengine.md#hms_xeg_renderneuralupscale>)接口执行空域AI超分。
