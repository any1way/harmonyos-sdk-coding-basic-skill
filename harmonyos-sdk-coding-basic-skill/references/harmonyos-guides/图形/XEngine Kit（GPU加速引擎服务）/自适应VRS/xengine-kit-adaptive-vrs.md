...
scraped_at: 2026-06-11T15:02:18+08:00
doc_updated_at: 2026-05-26
content_hash: sha256:02f7d53a50ab5f2c749c5b4689096beecd0c9d28de2da3148008338ee0f6666d
---
...
* 可通过以下方式查询相关扩展特性是否支持：
  ...
  对于OpenGL ES，使用[HMS\_XEG\_GetString](<../../../../harmonyos-references/XEngine Kit（GPU加速引擎服务）/C API/模块/XEngine/xengine-kit-xengine.md#hms_xeg_getstring>)扩展特性查询接口进行查询，如查询结果包含[XEG\_ADAPTIVE\_VRS\_EXTENSION\_NAME](<../../../../harmonyos-references/XEngine Kit（GPU加速引擎服务）/C API/模块/XEngine/xengine-kit-xengine.md#xeg_adaptive_vrs_extension_name>)，则表示支持该特性，若查询结果未包含，则表示不支持该特性。

## 接口说明

以下接口为自适应VRS设置接口，如需使用更丰富的设置和查询接口，具体API说明详见[接口文档](<../../../../harmonyos-references/XEngine Kit（GPU加速引擎服务）/C API/模块/XEngine/xengine-kit-xengine.md>)。
...
* 下面是基于GLES图形API平台集成自适应VRS的主要业务流程
  ...
  ![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/3a/v3/EpkPHQCDRiOsf-UGCFnKKw/zh-cn_image_0000002622698731.jpg?HW-CC-KV=V1&HW-CC-Date=20260611T070217Z&HW-CC-Expire=86400&HW-CC-Sign=630CC3F824A3B3EB4BA99F52DE114317B24D4AC1C6408E86EA700421112D4540)
...
* 下面是基于Vulkan图形API平台集成自适应VRS的主要业务流程
  ...
  ![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/6e/v3/mMBFSgMiQMO877MKk4F5BA/zh-cn_image_0000002592219170.jpg?HW-CC-KV=V1&HW-CC-Date=20260611T070217Z&HW-CC-Expire=86400&HW-CC-Sign=8A9864644C7E013F6C562BBE9C3429120B2A85F65B34DE8E3A9CD79F2A259F4B)
...
1. 调用[HMS\_XEG\_GetString](<../../../../harmonyos-references/XEngine Kit（GPU加速引擎服务）/C API/模块/XEngine/xengine-kit-xengine.md#hms_xeg_getstring>)接口，获取XEngine支持的扩展信息，只有在支持[XEG\_ADAPTIVE\_VRS\_EXTENSION\_NAME](<../../../../harmonyos-references/XEngine Kit（GPU加速引擎服务）/C API/模块/XEngine/xengine-kit-xengine.md#xeg_adaptive_vrs_extension_name>)扩展时才可以使用自适应VRS的相关接口。
   ...
   6. }
   ```
2. 调用[HMS\_XEG\_AdaptiveVRSParameter](<../../../../harmonyos-references/XEngine Kit（GPU加速引擎服务）/C API/模块/XEngine/xengine-kit-xengine.md#hms_xeg_adaptivevrsparameter>)接口，对自适应VRS的参数赋值。
   ...
   13. // sensitivity为控制生成着色率图像的阈值，用户可自定义，建议取值范围为[0.0, 1.0]
   14. GLfloat sensitivity = 0.15;
   15. HMS_XEG_AdaptiveVRSParameter(XEG_ADAPTIVE_VRS_ERROR_SENSITIVITY, &sensitivity);
   16. // flip为判断是否执行图像上下翻转，为true表示进行图像上下翻转，false则表示不进行图像上下翻转，此处以false为例
   17. GLboolean flip = false;
   18. HMS_XEG_AdaptiveVRSParameter(XEG_ADAPTIVE_VRS_FLIP, &flip);
   ```
3. 调用[HMS\_XEG\_DispatchAdaptiveVRS](<../../../../harmonyos-references/XEngine Kit（GPU加速引擎服务）/C API/模块/XEngine/xengine-kit-xengine.md#hms_xeg_dispatchadaptivevrs>)接口计算着色率图像。
   ...
   ```
4. 调用[HMS\_XEG\_ApplyAdaptiveVRS](<../../../../harmonyos-references/XEngine Kit（GPU加速引擎服务）/C API/模块/XEngine/xengine-kit-xengine.md#hms_xeg_applyadaptivevrs>)接口，将着色率图像应用到渲染目标中。
...
1. 调用[HMS\_XEG\_EnumerateDeviceExtensionProperties](<../../../../harmonyos-references/XEngine Kit（GPU加速引擎服务）/C API/模块/XEngine/xengine-kit-xengine.md#hms_xeg_enumeratedeviceextensionproperties>)接口，获取XEngine支持的扩展信息，只有在支持[XEG\_ADAPTIVE\_VRS\_EXTENSION\_NAME](<../../../../harmonyos-references/XEngine Kit（GPU加速引擎服务）/C API/模块/XEngine/xengine-kit-xengine.md#xeg_adaptive_vrs_extension_name>)扩展时才可以使用自适应VRS的相关接口。
...
2. 声明实例句柄。
   ...
   ```
3. 调用[HMS\_XEG\_CreateAdaptiveVRS](<../../../../harmonyos-references/XEngine Kit（GPU加速引擎服务）/C API/模块/XEngine/xengine-kit-xengine.md#hms_xeg_createadaptivevrs>)接口，定义并创建实例。
   ...
   ```
4. 调用[HMS\_XEG\_CmdDispatchAdaptiveVRS](<../../../../harmonyos-references/XEngine Kit（GPU加速引擎服务）/C API/模块/XEngine/xengine-kit-xengine.md#hms_xeg_cmddispatchadaptivevrs>)接口，下发自适应VRS命令，生成perImage着色率纹理。
   ...
   7. // commandBuffer为命令缓冲区，用户需进行初始化
   ...
   ```
5. 调用[HMS\_XEG\_DestroyAdaptiveVRS](<../../../../harmonyos-references/XEngine Kit（GPU加速引擎服务）/C API/模块/XEngine/xengine-kit-xengine.md#hms_xeg_destroyadaptivevrs>)接口，卸载VRS实例，清理VRS相关资源。
