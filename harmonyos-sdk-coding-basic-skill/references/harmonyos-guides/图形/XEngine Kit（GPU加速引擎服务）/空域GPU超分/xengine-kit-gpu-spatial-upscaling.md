...
content_hash: sha256:ce528c33bb94db4a1d4836db1e81244081b2bd5be6921f13c3436dd5598c9f0d
---
...
* 可通过以下方式查询相关扩展特性是否支持：
  ...
  如查询结果包含[XEG\_SPATIAL\_UPSCALE\_EXTENSION\_NAME](<../../../../harmonyos-references/XEngine Kit（GPU加速引擎服务）/C API/模块/XEngine/xengine-kit-xengine.md#xeg_spatial_upscale_extension_name>)，则表示支持该特性，若查询结果未包含，则表示不支持该特性。

## 接口说明

以下接口为OpenGL ES和Vulkan空域GPU超分设置接口，如需使用更丰富的设置和查询接口，具体API说明详见[接口文档](<../../../../harmonyos-references/XEngine Kit（GPU加速引擎服务）/C API/模块/XEngine/xengine-kit-xengine.md>)。
...
* 下面是基于GLES图形API平台集成空域GPU超分的主要业务流程
  ...
  ![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/9a/v3/5kehuC4mSjmJS8uJmlzDDg/zh-cn_image_0000002622858607.jpg?HW-CC-KV=V1&HW-CC-Date=20260611T070215Z&HW-CC-Expire=86400&HW-CC-Sign=241A909A43B3E2F7D7268F5F89C1AA4D33AE82F213AE9A161DA1CD344219D108)
...
* 下面是基于Vulkan图形API平台集成空域GPU超分的主要业务流程
  ...
  ![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/45/v3/4ik0K5z8Rc2Es62FHNno6w/zh-cn_image_0000002622698729.jpg?HW-CC-KV=V1&HW-CC-Date=20260611T070215Z&HW-CC-Expire=86400&HW-CC-Sign=96418B307939AB2CBB5A5FA7C12419CA92F31AE54A2E3DD721606343EF1B320B)
...
使用EGL和OpenGL ES图形API搭建图像渲染管线并集成空域GPU超分在Native层实现，渲染结果通过[XComponent](../../../../harmonyos-references/ArkUI（方舟UI框架）/ArkTS组件/渲染绘制/XComponent/ts-basic-components-xcomponent.md)组件显示到屏幕。
...
1. 调用[HMS\_XEG\_GetString](<../../../../harmonyos-references/XEngine Kit（GPU加速引擎服务）/C API/模块/XEngine/xengine-kit-xengine.md#hms_xeg_getstring>)接口，获取XEngine支持的扩展信息，只有在支持XEG\_SPATIAL\_UPSCALE\_EXTENSION\_NAME扩展时才可以使用空域GPU超分的相关接口。
   ...
   6. }
   ```
2. 调用[HMS\_XEG\_SpatialUpscaleParameter](<../../../../harmonyos-references/XEngine Kit（GPU加速引擎服务）/C API/模块/XEngine/xengine-kit-xengine.md#hms_xeg_spatialupscaleparameter>)接口，对空域GPU超分的参数赋值。
   ...
   ```
3. 调用[HMS\_XEG\_RenderSpatialUpscale](<../../../../harmonyos-references/XEngine Kit（GPU加速引擎服务）/C API/模块/XEngine/xengine-kit-xengine.md#hms_xeg_renderspatialupscale>)接口进行超分。
...
使用Vulkan图形API搭建图像渲染管线并集成空域GPU超分在Native层实现，渲染结果通过[XComponent](../../../../harmonyos-references/ArkUI（方舟UI框架）/ArkTS组件/渲染绘制/XComponent/ts-basic-components-xcomponent.md)组件显示到屏幕。
...
1. 调用[HMS\_XEG\_EnumerateDeviceExtensionProperties](<../../../../harmonyos-references/XEngine Kit（GPU加速引擎服务）/C API/模块/XEngine/xengine-kit-xengine.md#hms_xeg_enumeratedeviceextensionproperties>)接口，获取XEngine支持的扩展信息，只有在支持XEG\_SPATIAL\_UPSCALE\_EXTENSION\_NAME扩展时才可以使用空域GPU超分的相关接口。
...
2. 声明实例句柄。
   ...
   ```
3. 调用[HMS\_XEG\_CreateSpatialUpscale](<../../../../harmonyos-references/XEngine Kit（GPU加速引擎服务）/C API/模块/XEngine/xengine-kit-xengine.md#hms_xeg_createspatialupscale>)接口，创建超分实例。
   ...
   ```
4. 调用[HMS\_XEG\_CmdRenderSpatialUpscale](<../../../../harmonyos-references/XEngine Kit（GPU加速引擎服务）/C API/模块/XEngine/xengine-kit-xengine.md#hms_xeg_cmdrenderspatialupscale>)接口下发超分，每帧都需要调用。
   ...
   ```
5. 调用[HMS\_XEG\_DestroySpatialUpscale](<../../../../harmonyos-references/XEngine Kit（GPU加速引擎服务）/C API/模块/XEngine/xengine-kit-xengine.md#hms_xeg_destroyspatialupscale>)接口销毁实例。
