...
scraped_at: 2026-06-11T15:02:16+08:00
doc_updated_at: 2026-05-26
content_hash: sha256:cd5e70202ca2f15e1ded5750fa1c7040dc471993e4824d1965a597036a610510
---
...
* 可通过以下方式查询相关扩展特性是否支持：
  ...
  + 对于OpenGL ES，使用[HMS\_XEG\_GetString](<../../../../harmonyos-references/XEngine Kit（GPU加速引擎服务）/C API/模块/XEngine/xengine-kit-xengine.md#hms_xeg_getstring>)扩展特性查询接口进行查询。
  + 对于Vulkan，使用[HMS\_XEG\_EnumerateDeviceExtensionProperties](<../../../../harmonyos-references/XEngine Kit（GPU加速引擎服务）/C API/模块/XEngine/xengine-kit-xengine.md#hms_xeg_enumeratedeviceextensionproperties>)扩展特性查询接口进行查询。

  如查询结果包含[XEG\_TEMPORAL\_UPSCALE\_EXTENSION\_NAME](<../../../../harmonyos-references/XEngine Kit（GPU加速引擎服务）/C API/模块/XEngine/xengine-kit-xengine.md#xeg_temporal_upscale_extension_name>)，则表示支持该特性，若查询结果未包含，则表示不支持该特性。

## 接口说明

以下接口为OpenGL ES和Vulkan时域AI超分设置接口，如需使用更丰富的设置和查询接口，具体API说明详见[接口文档](<../../../../harmonyos-references/XEngine Kit（GPU加速引擎服务）/C API/模块/XEngine/xengine-kit-xengine.md>)。
...
* 下面是基于GLES图形API平台集成时域AI超分的主要业务流程
  ...
  ![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/72/v3/Uv5HCZ9DRoS4_zA2dKn8vw/zh-cn_image_0000002592219168.jpg?HW-CC-KV=V1&HW-CC-Date=20260611T070215Z&HW-CC-Expire=86400&HW-CC-Sign=E4FCF8E2554F82BC00393115FCA5A703FBCBEF1948A30DFBB5361B1812A157EC)

1. 用户在进入游戏初始化场景时调用HMS\_XEG\_GetString接口查询XEngine支持的特性，当查询接口返回支持的特性列表中包含[XEG\_TEMPORAL\_UPSCALE\_EXTENSION\_NAME](<../../../../harmonyos-references/XEngine Kit（GPU加速引擎服务）/C API/模块/XEngine/xengine-kit-xengine.md#xeg_temporal_upscale_extension_name>)时代表可以使用此特性。
...
* 下面是基于Vulkan图形API平台集成时域AI超分的主要业务流程
  ...
  ![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/b0/v3/6qWyJynFRkSo8uxNxW0bNA/zh-cn_image_0000002592379102.jpg?HW-CC-KV=V1&HW-CC-Date=20260611T070215Z&HW-CC-Expire=86400&HW-CC-Sign=E1101ADA37FF4195398480BF0DAB3B76ACD560DCE656CDDFD4265325947DF2CF)
...
使用EGL和OpenGL ES图形API搭建图像渲染管线并集成时域AI超分在Native层实现，渲染结果通过[XComponent](../../../../harmonyos-references/ArkUI（方舟UI框架）/ArkTS组件/渲染绘制/XComponent/ts-basic-components-xcomponent.md)组件显示到屏幕。

本节阐述OpenGL ES图形API的时域AI超分的使用。

在调用XEngine Kit能力前，需要先通过[Syscap](../../../../harmonyos-references/系统能力SystemCapability使用指南/syscap.md#什么是systemcapabilitysyscap)查询您的目标设备是否支持SystemCapability.Graphic.XEngine系统能力。

1. 调用[HMS\_XEG\_GetString](<../../../../harmonyos-references/XEngine Kit（GPU加速引擎服务）/C API/模块/XEngine/xengine-kit-xengine.md#hms_xeg_getstring>)接口，获取XEngine支持的扩展信息，只有在支持[XEG\_TEMPORAL\_UPSCALE\_EXTENSION\_NAME](<../../../../harmonyos-references/XEngine Kit（GPU加速引擎服务）/C API/模块/XEngine/xengine-kit-xengine.md#xeg_temporal_upscale_extension_name>)扩展时才可以使用时域AI超分的相关接口。
   ...
   6. }
   ```
2. 调用[HMS\_XEG\_TemporalUpscaleParameter](<../../../../harmonyos-references/XEngine Kit（GPU加速引擎服务）/C API/模块/XEngine/xengine-kit-xengine.md#hms_xeg_temporalupscaleparameter>)接口，对时域AI超分的参数赋值。
   ...
   ```
3. 调用[HMS\_XEG\_RenderTemporalUpscale](<../../../../harmonyos-references/XEngine Kit（GPU加速引擎服务）/C API/模块/XEngine/xengine-kit-xengine.md#hms_xeg_rendertemporalupscale>)接口进行超分，每帧都需要调用。
...
使用Vulkan图形API搭建图像渲染管线，并集成时域AI超分在Native层实现，渲染结果通过[XComponent](../../../../harmonyos-references/ArkUI（方舟UI框架）/ArkTS组件/渲染绘制/XComponent/ts-basic-components-xcomponent.md)组件显示到屏幕。
...
1. 调用[HMS\_XEG\_EnumerateDeviceExtensionProperties](<../../../../harmonyos-references/XEngine Kit（GPU加速引擎服务）/C API/模块/XEngine/xengine-kit-xengine.md#hms_xeg_enumeratedeviceextensionproperties>)接口，获取XEngine支持的扩展信息，只有在支持[XEG\_TEMPORAL\_UPSCALE\_EXTENSION\_NAME](<../../../../harmonyos-references/XEngine Kit（GPU加速引擎服务）/C API/模块/XEngine/xengine-kit-xengine.md#xeg_temporal_upscale_extension_name>)扩展时才可以使用时域AI超分的相关接口。
...
2. 声明实例句柄。
   ...
   ```
3. 调用[HMS\_XEG\_CreateTemporalUpscale](<../../../../harmonyos-references/XEngine Kit（GPU加速引擎服务）/C API/模块/XEngine/xengine-kit-xengine.md#hms_xeg_createtemporalupscale>)接口，创建时域AI超分实例。
   ...
   6. VkRect2D dstRect2D;
   7. dstRect2D.offset.x = 0;
   8. dstRect2D.offset.y = 0;
   9. dstRect2D.extent.width = highResWidth;
   10. dstRect2D.extent.height = highResHeight;
   11. const uint32_t jitterNum = 8;
   12. // Vulkan逻辑设备，用户需进行初始化
   13. VkDevice device;
   14. // XEG_TemporalUpscaleCreateInfo为创建XEG_TemporalUpscale对象所需信息
   15. XEG_TemporalUpscaleCreateInfo createInfo;
   16. // 指定输入图像的大小，即低分辨率图像的尺寸
   17. createInfo.inputSize = {lowResWidth, lowResHeight};
   18. // 指定输出图像的大小，即高分辨率图像的尺寸
   19. createInfo.outputSize = {highResWidth, highResHeight};
   20. createInfo.outputRegion = dstRect2D;
   21. // 指定输出图像的颜色格式
   22. createInfo.outputFormat = VK_FORMAT_R8G8B8A8_UNORM;
   23. // jitterNum为相机抖动的周期数
   24. createInfo.jitterNum = jitterNum;
   25. // 指定了深度值是否反转
   26. createInfo.isDepthReversed = true;
   27. VkResult res = HMS_XEG_CreateTemporalUpscale(device, &createInfo, &xegTemporalUpscale);
   28. if (res != VK_SUCCESS) {
   29. exit(1); // return error;
   30. }
   ```
4. 调用[HMS\_XEG\_CmdRenderTemporalUpscale](<../../../../harmonyos-references/XEngine Kit（GPU加速引擎服务）/C API/模块/XEngine/xengine-kit-xengine.md#hms_xeg_cmdrendertemporalupscale>)接口下发超分，每帧都需要调用。
   ...
   2. 调用时域AI超分渲染接口。
      ...
      10. // motionVectorImageView为用户创建的运动矢量图像的VkImageView
      11. VkImageView motionVectorImageView = VK_NULL_HANDLE;
      12. // depthImageView为用户创建的深度图像的VkImageView
      ...
      ```
5. 调用[HMS\_XEG\_DestroyTemporalUpscale](<../../../../harmonyos-references/XEngine Kit（GPU加速引擎服务）/C API/模块/XEngine/xengine-kit-xengine.md#hms_xeg_destroytemporalupscale>)接口销毁实例。
