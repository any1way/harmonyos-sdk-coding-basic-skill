...
content_hash: sha256:266a118e990b1b0439b54425b8d7647411be733ab482c11e8913d96519a1b233
---
...
* 可通过以下方式查询相关扩展特性是否支持：
  ...
  对于Vulkan，使用[HMS\_XEG\_EnumerateDeviceExtensionProperties](<../../../../harmonyos-references/XEngine Kit（GPU加速引擎服务）/C API/模块/XEngine/xengine-kit-xengine.md#hms_xeg_enumeratedeviceextensionproperties>)扩展特性查询接口进行查询，如查询结果包含XEG\_RT\_REFLECTION\_EXTENSION\_NAME，则表示支持该特性，若查询结果未包含，则表示不支持该特性。

## 接口说明

以下接口为使用光线追踪反射特性需要使用的接口，关于这些接口的详细说明见[接口文档](<../../../../harmonyos-references/XEngine Kit（GPU加速引擎服务）/C API/模块/XEngine/xengine-kit-xengine.md>)。
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/2f/v3/spZUaQSZTK-1Ynh9H8GitA/zh-cn_image_0000002622698733.png?HW-CC-KV=V1&HW-CC-Date=20260611T070219Z&HW-CC-Expire=86400&HW-CC-Sign=39B25D0C2EB6D8091B84FA3926D71A9C571EECBBBC10F4C465A6294BFB457CF6)
...
7. 用户退出光线追踪反射适用的游戏场景。
8. 游戏应用调用[HMS\_XEG\_DestroyRTReflection](<../../../../harmonyos-references/XEngine Kit（GPU加速引擎服务）/C API/模块/XEngine/xengine-kit-xengine.md#hms_xeg_destroyrtreflection>)接口销毁光线追踪反射实例。
...
使用Vulkan图形API搭建图像渲染管线，并集成光线追踪反射效果的代码需要在Native层实现，渲染结果通过[XComponent](../../../../harmonyos-references/ArkUI（方舟UI框架）/ArkTS组件/渲染绘制/XComponent/ts-basic-components-xcomponent.md)组件显示到屏幕。
...
1. 调用[HMS\_XEG\_EnumerateDeviceExtensionProperties](<../../../../harmonyos-references/XEngine Kit（GPU加速引擎服务）/C API/模块/XEngine/xengine-kit-xengine.md#hms_xeg_enumeratedeviceextensionproperties>)接口，获取XEngine支持的扩展信息，只有在支持XEG\_RT\_REFLECTION\_EXTENSION\_NAME扩展时才可以使用光线追踪反射的相关接口。
...
2. 声明实例句柄。
   ...
   ```
3. 调用[HMS\_XEG\_CreateRTReflection](<../../../../harmonyos-references/XEngine Kit（GPU加速引擎服务）/C API/模块/XEngine/xengine-kit-xengine.md#hms_xeg_creatertreflection>)接口，创建光线追踪反射实例。
   ...
   17. }
   ```
4. 调用[HMS\_XEG\_CmdRenderRTReflection](<../../../../harmonyos-references/XEngine Kit（GPU加速引擎服务）/C API/模块/XEngine/xengine-kit-xengine.md#hms_xeg_cmdrenderrtreflection>)接口下发渲染命令，每帧都需要调用。
   ...
   24. }
   ```
5. 调用[HMS\_XEG\_DestroyRTReflection](<../../../../harmonyos-references/XEngine Kit（GPU加速引擎服务）/C API/模块/XEngine/xengine-kit-xengine.md#hms_xeg_destroyrtreflection>)接口销毁实例，释放资源，当特性不再使用或应用退出时需要调用。
