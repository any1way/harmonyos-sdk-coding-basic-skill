...
content_hash: sha256:3904adb2d04f4677e8f7bfff4d5c453e3d08a5dd2493ad8e12321704b705c2d6
---
...
* 可通过以下方式查询相关扩展特性是否支持：
  ...
  对于Vulkan，使用[HMS\_XEG\_EnumerateDeviceExtensionProperties](<../../../../harmonyos-references/XEngine Kit（GPU加速引擎服务）/C API/模块/XEngine/xengine-kit-xengine.md#hms_xeg_enumeratedeviceextensionproperties>)扩展特性查询接口进行查询，如查询结果包含XEG\_RTGI\_EXTENSION\_NAME，则表示支持该特性，若查询结果未包含，则表示不支持该特性。
...
以下接口为RTGI设置接口，如需使用更丰富的设置和查询接口，具体API说明详见[接口文档](<../../../../harmonyos-references/XEngine Kit（GPU加速引擎服务）/C API/模块/XEngine/xengine-kit-xengine.md>)。
...
* 编写CMakeLists.txt
  ...
  8. // 其他库文件
  9. // ...
  10. ${xengine-lib} RenderBehavior SceneLoader VulkanBase
  11. )
  ```
...
* 下面是基于Vulkan图形API平台集成动态漫反射全局光照的主要业务流程
  ...
  ![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/fc/v3/lmlEDMTkT3WnxMRZ_hse3g/zh-cn_image_0000002592379106.jpg?HW-CC-KV=V1&HW-CC-Date=20260611T070221Z&HW-CC-Expire=86400&HW-CC-Sign=44451A4A5B691E094039D8659038217F5CFA8082C02B8B703B2BB153A5190B9C)
...
3. 创建动态漫反射全局光照使用的创建信息，调用[HMS\_XEG\_CreateRTGI](<../../../../harmonyos-references/XEngine Kit（GPU加速引擎服务）/C API/模块/XEngine/xengine-kit-xengine.md#hms_xeg_creatertgi>)接口创建动态漫反射全局光照实例。
...
使用Vulkan图形API搭建图像渲染管线，并集成RT DDGI在Native层实现，渲染结果通过[XComponent](../../../../harmonyos-references/ArkUI（方舟UI框架）/ArkTS组件/渲染绘制/XComponent/ts-basic-components-xcomponent.md)组件显示到屏幕。
...
1. 调用[HMS\_XEG\_EnumerateDeviceExtensionProperties](<../../../../harmonyos-references/XEngine Kit（GPU加速引擎服务）/C API/模块/XEngine/xengine-kit-xengine.md#hms_xeg_enumeratedeviceextensionproperties>)接口，获取XEngine支持的扩展信息，只有在支持XEG\_RTGI\_EXTENSION\_NAME扩展时才可以使用RT DDGI的相关接口。
...
2. 声明实例句柄。
   ...
   1. XEG_RTGI xegRTGI;
   ```
3. 调用[HMS\_XEG\_CreateRTGI](<../../../../harmonyos-references/XEngine Kit（GPU加速引擎服务）/C API/模块/XEngine/xengine-kit-xengine.md#hms_xeg_creatertgi>)接口，创建RT DDGI实例。
   ...
   29. }
   ```
4. 调用[HMS\_XEG\_CmdRenderRTGI](<../../../../harmonyos-references/XEngine Kit（GPU加速引擎服务）/C API/模块/XEngine/xengine-kit-xengine.md#hms_xeg_cmdrenderrtgi>)接口执行渲染命令，每帧都需要调用。
   ...
   ```
5. 若使用延迟渲染管线，则可以在调用[HMS\_XEG\_CmdRenderRTGI](<../../../../harmonyos-references/XEngine Kit（GPU加速引擎服务）/C API/模块/XEngine/xengine-kit-xengine.md#hms_xeg_cmdrenderrtgi>)接口之后，调用[HMS\_XEG\_CmdSetSynchronization](<../../../../harmonyos-references/XEngine Kit（GPU加速引擎服务）/C API/模块/XEngine/xengine-kit-xengine.md#hms_xeg_cmdsetsynchronization>)接口，设置同步信号，等待GI渲染结果写入指定图像，[HMS\_XEG\_CmdSetSynchronization](<../../../../harmonyos-references/XEngine Kit（GPU加速引擎服务）/C API/模块/XEngine/xengine-kit-xengine.md#hms_xeg_cmdsetsynchronization>)接口需要每帧调用。

   ```
   1. // GI渲染结果会写入到XEG_DDGIDescription中的outputGIImage图像中
   2. HMS_XEG_CmdSetSynchronization(commandBuffer, &xegRTGI);
   ```
6. 调用[HMS\_XEG\_DestroyRTGI](<../../../../harmonyos-references/XEngine Kit（GPU加速引擎服务）/C API/模块/XEngine/xengine-kit-xengine.md#hms_xeg_destroyrtgi>)接口销毁实例。
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/46/v3/_IUDUgoPQ4-Eozz_Wyly7w/zh-cn_image_0000002622858613.jpg?HW-CC-KV=V1&HW-CC-Date=20260611T070221Z&HW-CC-Expire=86400&HW-CC-Sign=2328037DA709569A6DBFF0118C7D17889C86378D660A75BD8309F4E6423F2FFF)
...
3. 创建神经网络全局光照使用的创建信息，调用[HMS\_XEG\_CreateRTGI](<../../../../harmonyos-references/XEngine Kit（GPU加速引擎服务）/C API/模块/XEngine/xengine-kit-xengine.md#hms_xeg_creatertgi>)接口创建神经网络全局光照实例。
...
使用Vulkan图形API搭建图像渲染管线，并集成RT NNGI在Native层实现，渲染结果通过[XComponent](../../../../harmonyos-references/ArkUI（方舟UI框架）/ArkTS组件/渲染绘制/XComponent/ts-basic-components-xcomponent.md)组件显示到屏幕。

本节阐述Vulkan图形API的RT NNGI使用。

在调用XEngine Kit能力前，需要先通过[Syscap](../../../../harmonyos-references/系统能力SystemCapability使用指南/syscap.md#什么是systemcapabilitysyscap)查询您的目标设备是否支持SystemCapability.Graphic.XEngine系统能力。

1. 调用[HMS\_XEG\_EnumerateDeviceExtensionProperties](<../../../../harmonyos-references/XEngine Kit（GPU加速引擎服务）/C API/模块/XEngine/xengine-kit-xengine.md#hms_xeg_enumeratedeviceextensionproperties>)接口，获取XEngine支持的扩展信息，只有在支持XEG\_RTGI\_EXTENSION\_NAME扩展时才可以使用RT NNGI的相关接口。
...
2. 声明实例句柄。
   ...
   1. XEG_RTGI xegRTGI;
   ```
3. 调用[HMS\_XEG\_CreateRTGI](<../../../../harmonyos-references/XEngine Kit（GPU加速引擎服务）/C API/模块/XEngine/xengine-kit-xengine.md#hms_xeg_creatertgi>)接口，创建RT NNGI实例。

   ```
   1. // Vulkan逻辑设备，用户需进行初始化
   2. VkDevice device;
   3. // XEG_NNGICreateInfo为创建XEG_NNGI对象所需信息
...
4. 调用[HMS\_XEG\_CmdRenderRTGI](<../../../../harmonyos-references/XEngine Kit（GPU加速引擎服务）/C API/模块/XEngine/xengine-kit-xengine.md#hms_xeg_cmdrenderrtgi>)接口执行渲染命令，每帧都需要调用。
   ...
   ```
5. 在调用[HMS\_XEG\_CmdRenderRTGI](<../../../../harmonyos-references/XEngine Kit（GPU加速引擎服务）/C API/模块/XEngine/xengine-kit-xengine.md#hms_xeg_cmdrenderrtgi>)接口之后，调用[HMS\_XEG\_CmdSetSynchronization](<../../../../harmonyos-references/XEngine Kit（GPU加速引擎服务）/C API/模块/XEngine/xengine-kit-xengine.md#hms_xeg_cmdsetsynchronization>)接口，执行训练步骤，[HMS\_XEG\_CmdSetSynchronization](<../../../../harmonyos-references/XEngine Kit（GPU加速引擎服务）/C API/模块/XEngine/xengine-kit-xengine.md#hms_xeg_cmdsetsynchronization>)接口需要每帧调用。
