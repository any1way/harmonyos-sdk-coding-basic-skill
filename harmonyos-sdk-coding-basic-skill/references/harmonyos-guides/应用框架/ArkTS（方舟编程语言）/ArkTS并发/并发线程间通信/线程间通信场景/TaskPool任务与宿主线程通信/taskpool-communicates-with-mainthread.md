...
content_hash: sha256:773f62cd6ef9411a38e31106c251640f2a5cb9759b324db0ec9cea7b17a45dbf
---
...
1. 实现接收Task消息的方法。
   ...
   [TaskSendDataUsage.ets](https://gitcode.com/HarmonyOS_Samples/guide-snippets/blob/HarmonyOS-feature-20260402/ArkTS/ArkTsConcurrent/ConcurrentThreadCommunication/InterThreadCommunicationScenario/entry/src/main/ets/managers/TaskSendDataUsage.ets#L17-L24)
2. 在需要执行的Task中，添加sendData()接口将消息发送给宿主线程。在宿主线程通过onReceiveData()接口接收消息。这样宿主线程就可以通过notice()接口接收到Task发送的数据。
   ...
   41. let loadPictureTask: taskpool.Task = new taskpool.Task(loadPictureSendData, 30);
   42. // 设置notice方法接收Task发送的消息
   43. loadPictureTask.onReceiveData(notice);
   44. taskpool.execute(loadPictureTask).then((res: object) => {
   ...
   [TaskSendDataUsage.ets](https://gitcode.com/HarmonyOS_Samples/guide-snippets/blob/HarmonyOS-feature-20260402/ArkTS/ArkTsConcurrent/ConcurrentThreadCommunication/InterThreadCommunicationScenario/entry/src/main/ets/managers/TaskSendDataUsage.ets#L16-L75)
