...
scraped_at: 2026-06-01T11:11:29+08:00
doc_updated_at: 2026-05-26
content_hash: sha256:88fbc1ac4e992da9b9bf467cb6ea2deee46eb832fbbac53b217c9ff7efcf18eb
---
## 概述
...
端侧问答模型关键接口如下表所示，具体API说明详见[API参考](<../../../../harmonyos-references/Data Augmentation Kit（数据增强服务）/ArkTS API/localChatModel（端侧问答模型）/dataaugmentation-localchatmodel-api.md>)。

| 接口名 | 描述 |
| --- | --- |
| [init](<../../../../harmonyos-references/Data Augmentation Kit（数据增强服务）/ArkTS API/localChatModel（端侧问答模型）/dataaugmentation-localchatmodel-api.md#init>)(): Promise<boolean> | 初始化端侧问答模型，负责拉起模型管理应用。 |
| [chat](<../../../../harmonyos-references/Data Augmentation Kit（数据增强服务）/ArkTS API/localChatModel（端侧问答模型）/dataaugmentation-localchatmodel-api.md#chat>)(info: [QuestionInfo](<../../../../harmonyos-references/Data Augmentation Kit（数据增强服务）/ArkTS API/localChatModel（端侧问答模型）/dataaugmentation-localchatmodel-api.md#questioninfo>), config: [Config](<../../../../harmonyos-references/Data Augmentation Kit（数据增强服务）/ArkTS API/localChatModel（端侧问答模型）/dataaugmentation-localchatmodel-api.md#config>), callback: AsyncCallback<[Answer](<../../../../harmonyos-references/Data Augmentation Kit（数据增强服务）/ArkTS API/localChatModel（端侧问答模型）/dataaugmentation-localchatmodel-api.md#answer>)>): Promise<void> | 与端侧模型进行交互，实现端侧模型的问答功能。 |

注意

模型资源来自[Matrix](https://matrix.openharmony.cn/#/model/main)模型库，[chat](<../../../../harmonyos-references/Data Augmentation Kit（数据增强服务）/ArkTS API/localChatModel（端侧问答模型）/dataaugmentation-localchatmodel-api.md#chat>)接口默认调用模型为Qwen25-7B-Instruct。

## 开发步骤

### 申请权限

申请ohos.permission.INTERNET权限，配置方式请参阅声明权限[声明权限](../../../系统/安全/程序访问控制/应用权限管控/申请应用权限/声明权限/declare-permissions.md)。

### 功能开发

1. 调用[init](<../../../../harmonyos-references/Data Augmentation Kit（数据增强服务）/ArkTS API/localChatModel（端侧问答模型）/dataaugmentation-localchatmodel-api.md#init>)接口，拉起本地AI模型管理。
2. 本地AI模型管理首次拉起，弹出隐私声明界面，同意后下载默认模型。
3. 本地AI模型管理非首次拉起，打开设置>系统>本地AI模型管理，下载默认模型。
4. 等待模型下载完成后，调用[chat](<../../../../harmonyos-references/Data Augmentation Kit（数据增强服务）/ArkTS API/localChatModel（端侧问答模型）/dataaugmentation-localchatmodel-api.md#chat>)接口，开始进行端侧问答。
...
100. console.info('modelChat finished');
...
105. };
106. try {
107. console.info('modelChat Starting chat...');
