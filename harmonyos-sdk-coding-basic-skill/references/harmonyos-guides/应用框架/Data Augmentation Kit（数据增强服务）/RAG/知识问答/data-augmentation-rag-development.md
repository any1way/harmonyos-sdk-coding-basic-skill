...
content_hash: sha256:a2d8372756d89e0ee671cc47fd75b249a3f5f18f5847bb39c27a60d3fc4ef84c
---
知识问答是通过检索增强生成（RAG）技术，从数据源中精准提取信息并生成答案的智能交互方式。可用于企业客服、医疗辅助、IT支持等领域。

## 约束限制

1. 知识问答需要先对数据源库进行[知识加工](../../知识加工/data-augmentation-knowledge-processing.md)生成知识库，否则无法问答。
...
RAG关键接口如下表所示，具体API说明详见[API参考](<../../../../../harmonyos-references/Data Augmentation Kit（数据增强服务）/ArkTS API/rag（检索增强生成）/dataaugmentation-rag-api.md#导入模块>)。除接口外，还可以通过配置文件进行深度定制RAG，详见[RAG配置](../RAG配置/配置提示词模板/data-augmentation-config-prompt-template.md)。

| 接口名 | 描述 |
| --- | --- |
| abstract [streamChat](<../../../../../harmonyos-references/Data Augmentation Kit（数据增强服务）/ArkTS API/rag（检索增强生成）/dataaugmentation-rag-api.md#streamchat>)(query: string, callback: Callback<[LLMStreamAnswer](<../../../../../harmonyos-references/Data Augmentation Kit（数据增强服务）/ArkTS API/rag（检索增强生成）/dataaugmentation-rag-api.md#llmstreamanswer>)>): Promise<[LLMRequestInfo](<../../../../../harmonyos-references/Data Augmentation Kit（数据增强服务）/ArkTS API/rag（检索增强生成）/dataaugmentation-rag-api.md#llmrequestinfo>)> | 继承ChatLLM类实现大模型客户端时需要实现的函数。RAG在检索前的问题预处理、检索后的回答生成时，会调用这个函数与大语言模型交互。 |
| [createRagSession](<../../../../../harmonyos-references/Data Augmentation Kit（数据增强服务）/ArkTS API/rag（检索增强生成）/dataaugmentation-rag-api.md#createragsession>)(context: [common.Context](<../../../../../harmonyos-references/Ability Kit（程序框架服务）/ArkTS API/接口依赖的元素及定义/application/Context (Stage模型的上下文基类)/js-apis-inner-application-context.md>), config: [Config](<../../../../../harmonyos-references/Data Augmentation Kit（数据增强服务）/ArkTS API/rag（检索增强生成）/dataaugmentation-rag-api.md#config>)): Promise<[RagSession](<../../../../../harmonyos-references/Data Augmentation Kit（数据增强服务）/ArkTS API/rag（检索增强生成）/dataaugmentation-rag-api.md#ragsession>)> | 获得一个会话用于进行知识问答。不支持多线程调用。 |
| [streamRun](<../../../../../harmonyos-references/Data Augmentation Kit（数据增强服务）/ArkTS API/rag（检索增强生成）/dataaugmentation-rag-api.md#streamrun>)(question: string, config: [RunConfig](<../../../../../harmonyos-references/Data Augmentation Kit（数据增强服务）/ArkTS API/rag（检索增强生成）/dataaugmentation-rag-api.md#runconfig>), callback: AsyncCallback<[Stream](<../../../../../harmonyos-references/Data Augmentation Kit（数据增强服务）/ArkTS API/rag（检索增强生成）/dataaugmentation-rag-api.md#stream>)>): Promise<number> | 知识问答接口，传入问题以及问答配置项。当RAG生成问题结果时，触发callback回调函数来流式传递数据。支持的长度为1000个字符内（UTF-8下一个汉字占3个字符）。不支持多线程调用。 |

## 开发准备

1. 申请网络权限。[streamChat](<../../../../../harmonyos-references/Data Augmentation Kit（数据增强服务）/ArkTS API/rag（检索增强生成）/dataaugmentation-rag-api.md#streamchat>)中需要开发者实现与LLM交互的功能，因此需要为应用申请网络权限。
   ...
   7. ],
   ```
2. 完成知识加工配置。请参考[知识加工](../../知识加工/data-augmentation-knowledge-processing.md)。

## 开发步骤

下面仅对关键步骤关键代码进行片段式说明，省略了很多非核心代码，如果需要查看完整功能示例代码，请参考[示例代码](../完整示例代码/data-augmentation-rag-demo.md)。应用的一次流式问答过程，和RagSession、ChatLLM、知识库的交互流程，可参考[流式问答调用流程图](data-augmentation-rag-development.md#流式问答调用流程图)。
...
1. 导入@kit.DataAugmentationKit模块，其余依赖需要开发者按需添加。
   ...
   ```
2. 创建http工具类，用以和大模型交互，用户也可选择webSocket（可参考[Network Kit](<../../../../../harmonyos-references/Network Kit（网络服务）/ArkTS API/network-api-arkts.md>)）或者其他方式与大模型交互。本示例选用了[ModelArts平台](https://console.huaweicloud.com/modelarts)的qwen3-235b-a22b模型作为示例，开发者使用时需根据实际情况选择合适大模型。示例代码包括如下三个环节：
   ...
   * 初始化大模型以及向大模型发送请求。
   ...
   ```
3. 继承实现[ChatLLM](<../../../../../harmonyos-references/Data Augmentation Kit（数据增强服务）/ArkTS API/rag（检索增强生成）/dataaugmentation-rag-api.md#chatllm>)类，在此函数中与大模型进行交互，并将大模型返回结果通过callback函数返回给RagSession。
   ...
   43. }
   ```
4. 创建[Config](<../../../../../harmonyos-references/Data Augmentation Kit（数据增强服务）/ArkTS API/rag（检索增强生成）/dataaugmentation-rag-api.md#config>)配置中的属性。下面简要介绍几个主要属性，有关全量配置字段的详细含义，请参见[智慧化数据检索](../../智慧化数据检索-ArkTS/dataaugmentation-retrieval.md)中的说明。开发者可以根据自身需求进行选择性配置。

   * [RetrievalConfig](<../../../../../harmonyos-references/Data Augmentation Kit（数据增强服务）/ArkTS API/retrieval（智慧化数据平台）/dataaugmentation-retrieval-api.md#retrievalconfig>)主要配置知识库的数据库配置。[知识加工](../../知识加工/data-augmentation-knowledge-processing.md)将会生成向量及倒排两种知识库表。
     ...
     29. };
     ```
   * [RetrievalCondition](<../../../../../harmonyos-references/Data Augmentation Kit（数据增强服务）/ArkTS API/retrieval（智慧化数据平台）/dataaugmentation-retrieval-api.md#retrievalcondition>)主要配置检索条件及多路召回之后的排序配置。其中fromClause为查询目标索引名，可按照如下示例代码配置为业务数据库表及知识加工产生的数据库表联合形成的虚拟表；responseColumns为召回的字段集合，范围为fromClause配置的数据库表中的列。关于知识库的数据库表结构可参见：[知识加工](../../知识加工/data-augmentation-knowledge-processing.md)。
     ...
     37. };
     ```
   * 完成Config数据的构造。[ChatLLM](<../../../../../harmonyos-references/Data Augmentation Kit（数据增强服务）/ArkTS API/rag（检索增强生成）/dataaugmentation-rag-api.md#chatllm>)参数则使用步骤3继承实现的ChatLLM的自定义的类的实例。
...
6. 使用步骤5创建的RagSession的streamRun()函数进行问答。
   ...
   * answerTypes属性用来指定流式输出的数据类型（[StreamType](<../../../../../harmonyos-references/Data Augmentation Kit（数据增强服务）/ArkTS API/rag（检索增强生成）/dataaugmentation-rag-api.md#streamtype>)），当前示例代码配置了三种数据类型，所以最终streamRun()函数的callback回调函数将会输出这三种类型的数据。
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/34/v3/b2WNshSOQW2MioIMXIL0Kg/zh-cn_image_0000002592378628.png?HW-CC-KV=V1&HW-CC-Date=20260611T063718Z&HW-CC-Expire=86400&HW-CC-Sign=6D376F04899E3AC21A33C54D7FB5862A591F18EEF5690E2F552462F69D05CF4C)
