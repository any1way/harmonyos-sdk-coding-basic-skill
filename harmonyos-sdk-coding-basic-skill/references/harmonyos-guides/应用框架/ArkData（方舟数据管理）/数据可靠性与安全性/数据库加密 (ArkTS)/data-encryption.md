...
scraped_at: 2026-06-01T11:00:20+08:00
doc_updated_at: 2026-05-26
content_hash: sha256:05eb0e833e90906d5c4f9aaaeb0c6671fcf3f9d3378098a8e685518f078144e6
---
## 场景介绍

为了增强数据库的安全性，数据库提供了一个安全适用的数据库加密能力，从而对数据库存储的内容实施有效保护。通过数据库加密等安全方法实现了数据库数据存储的保密性和完整性要求，使得数据库以密文方式存储并在密态方式下工作，确保了数据安全。

键值型数据库的加密属性在创建时确定，不可修改；关系型数据库从API version 22开始，支持通过[rekeyEx](<../../../../../harmonyos-references/ArkData（方舟数据管理）/ArkTS API/@ohos.data.relationalStore (关系型数据库)/Interface (RdbStore)/arkts-apis-data-relationalstore-rdbstore.md#rekeyex22>)接口调整其加密属性。
...
具体接口及功能，可见[分布式键值数据库](<../../../../../harmonyos-references/ArkData（方舟数据管理）/ArkTS API/@ohos.data.distributedKVStore (分布式键值数据库)/js-apis-distributedkvstore.md>)。
...
关系型数据库，通过[StoreConfig](<../../../../../harmonyos-references/ArkData（方舟数据管理）/ArkTS API/@ohos.data.relationalStore (关系型数据库)/Interfaces (其他)/arkts-apis-data-relationalstore-i.md#storeconfig>)中encrypt属性来设置是否加密。encrypt参数为true时表示加密；为false时表示不加密；默认值为false。
...
[Encryption.ets](https://gitcode.com/HarmonyOS_Samples/guide-snippets/blob/HarmonyOS-feature-20260402/ArkData/RelationalStore/NativeDataEncryption/entry/src/main/ets/pages/encryption/Encryption.ets#L17-L23)
...
[Encryption.ets](https://gitcode.com/HarmonyOS_Samples/guide-snippets/blob/HarmonyOS-feature-20260402/ArkData/RelationalStore/NativeDataEncryption/entry/src/main/ets/pages/encryption/Encryption.ets#L30-L47)
...
[Encryption.ets](https://gitcode.com/HarmonyOS_Samples/guide-snippets/blob/HarmonyOS-feature-20260402/ArkData/RelationalStore/NativeDataEncryption/entry/src/main/ets/pages/encryption/Encryption.ets#L54-L95)
