...
content_hash: sha256:394776c6043d431cf58168fec5486a774eb67be8ef894a1a98c2223234234df7
---
## 场景介绍
...
加密后的数据库只能通过接口进行访问，无法通过其它方式打开数据库文件。数据库的加密属性在创建数据库时确认，无法变更；关系型数据库从API version 22开始，支持通过[OH\_Rdb\_RekeyEx](<../../../../../harmonyos-references/ArkData（方舟数据管理）/C API/头文件/relational_store.h/capi-relational-store-h.md#oh_rdb_rekeyex>)接口调整其加密属性。
...
2. 导入头文件。
   ...
   [napi\_init.cpp](https://gitcode.com/HarmonyOS_Samples/guide-snippets/blob/HarmonyOS-feature-20260402/ArkData/RelationalStore/NativeDataEncryption/entry/src/main/cpp/napi_init.cpp#L18-L20)
3. 针对是否配置自定义加密/解密参数，有如下两种场景：
   ...
   * 场景1：不配置自定义加密/解密参数，此时会使用默认的配置进行数据库的加密/解密。
     ...
     [napi\_init.cpp](https://gitcode.com/HarmonyOS_Samples/guide-snippets/blob/HarmonyOS-feature-20260402/ArkData/RelationalStore/NativeDataEncryption/entry/src/main/cpp/napi_init.cpp#L25-L41)
   * 场景2：使用OH\_Rdb\_SetCryptoParam接口配置加密参数，此时会使用开发者自定义的密钥和算法参数进行数据库的加密/解密。
     ...
     [napi\_init.cpp](https://gitcode.com/HarmonyOS_Samples/guide-snippets/blob/HarmonyOS-feature-20260402/ArkData/RelationalStore/NativeDataEncryption/entry/src/main/cpp/napi_init.cpp#L52-L97)
