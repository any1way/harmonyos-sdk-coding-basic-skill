...
scraped_at: 2026-06-01T10:59:51+08:00
doc_updated_at: 2026-05-26
content_hash: sha256:bc821db2f2af5a15f549a039a096c1073ca8f8f66897022dac732b140327f55c
---
## 场景介绍

针对[UTD标准化数据类型](<../../../../../harmonyos-references/ArkData（方舟数据管理）/ArkTS API/@ohos.data.uniformTypeDescriptor (标准化数据定义与描述)/js-apis-data-uniformtypedescriptor.md#uniformdatatype>)中的部分常见类型，为了方便业务使用，我们按照不同的数据类型提供了标准化数据结构，例如系统定义的桌面图标类型（对应的标准化数据类型标识为'openharmony.app-item'），我们明确定义了该数据结构对应的相关描述信息。

某些业务场景下应用可以直接使用我们具体定义的UTD标准化数据结构，例如跨应用拖拽场景。拖出方应用可以按照标准化数据结构将拖拽数据写入[拖拽事件](../../../../../harmonyos-references/ArkUI（方舟UI框架）/ArkTS组件/通用事件/交互响应事件/拖拽事件/ts-universal-events-drag-drop.md#dragevent7)，拖入方应用从拖拽事件中读取拖拽数据并按照标准化数据结构进行数据的解析。这使得不同应用间的数据交互遵从相同的标准定义，有效减少了跨应用数据交互的开发工作量。
...
| 数据结构 | 数据类型 | 说明 |
| --- | --- | --- |
| [PlainText](<../../../../../harmonyos-references/ArkData（方舟数据管理）/ArkTS API/@ohos.data.uniformDataStruct (标准化数据结构)/js-apis-data-uniformdatastruct.md#plaintext>) | 'general.plain-text' | 纯文本。 |
| [Hyperlink](<../../../../../harmonyos-references/ArkData（方舟数据管理）/ArkTS API/@ohos.data.uniformDataStruct (标准化数据结构)/js-apis-data-uniformdatastruct.md#hyperlink>) | 'general.hyperlink' | 超链接。 |
| [HTML](<../../../../../harmonyos-references/ArkData（方舟数据管理）/ArkTS API/@ohos.data.uniformDataStruct (标准化数据结构)/js-apis-data-uniformdatastruct.md#html>) | 'general.html' | 富文本。 |
| [OpenHarmonyAppItem](<../../../../../harmonyos-references/ArkData（方舟数据管理）/ArkTS API/@ohos.data.uniformDataStruct (标准化数据结构)/js-apis-data-uniformdatastruct.md#openharmonyappitem>) | 'openharmony.app-item' | 图标。 |
| [ContentForm](<../../../../../harmonyos-references/ArkData（方舟数据管理）/ArkTS API/@ohos.data.uniformDataStruct (标准化数据结构)/js-apis-data-uniformdatastruct.md#contentform14>) | 'general.content-form' | 内容卡片。 |
...
1. 导入对应模块。
   ...
   [UdmfInterface.ets](https://gitcode.com/HarmonyOS_Samples/guide-snippets/blob/HarmonyOS-feature-20260402/ArkData/Udmf/UniformDataStructure/entry/src/main/ets/pages/UdmfInterface.ets#L16-L20)
2. 创建超链接数据记录。
3. 创建数据记录并添加到统一数据对象中。
4. 创建统一数据对象实例。
5. 添加plainText数据记录。
...
7. 遍历每条记录，判断该记录的数据类型，转换为子类对象并得到原数据记录。
   ...
   [UdmfInterface.ets](https://gitcode.com/HarmonyOS_Samples/guide-snippets/blob/HarmonyOS-feature-20260402/ArkData/Udmf/UniformDataStructure/entry/src/main/ets/pages/UdmfInterface.ets#L23-L90)
