...
content_hash: sha256:8b3d7efef6c6b1b8682abe2d353183db29a82c9c2106bab91afa13d8aad517be
---
## 场景介绍
...
* API version 18之前：ArkTS API仅支持[XML存储模式](<../通过用户首选项实现数据持久化 (ArkTS)/data-persistence-by-preferences.md#xml存储>)；C API仅支持[GSKV存储模式](<../通过用户首选项实现数据持久化 (ArkTS)/data-persistence-by-preferences.md#gskv存储>)；存储模式互不兼容，不支持ArkTS和C API操作同一个Preferences实例。
...
详细的接口说明请参考[Preferences接口文档](<../../../../../harmonyos-references/ArkData（方舟数据管理）/C API/模块/Preferences/capi-preferences.md>)。
