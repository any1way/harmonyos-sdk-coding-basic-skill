...
scraped_at: 2026-06-01T11:00:04+08:00
doc_updated_at: 2026-05-26
content_hash: sha256:6d034c8af8844f0178fd1d91a2cbc6d88a79bb1af8037fb485f0c4b1dd0aa871
---
## 场景介绍
...
以下是向量数据库持久化功能的相关接口，更多接口及使用方式请见[@ohos.data.relationalStore (关系型数据库)](<../../../../../harmonyos-references/ArkData（方舟数据管理）/ArkTS API/@ohos.data.relationalStore (关系型数据库)/模块描述/arkts-apis-data-relationalstore.md>)。
...
1. 判断当前系统是否支持向量数据库，若不支持，则表示当前系统不具备向量数据库能力。示例代码如下：
   ...
   [vectorStoreCTUD.ets](https://gitcode.com/HarmonyOS_Samples/guide-snippets/blob/HarmonyOS-feature-20260402/ArkData/VectorStore/entry/src/main/ets/pages/crud/vectorStoreCTUD.ets#L16-L42)
2. 若支持向量数据库则需要获取一个RdbStore。通过getRdbStore接口创建数据库，并执行建表操作。
   ...
   * 错误码的详细介绍请参见[通用错误码](../../../../../harmonyos-references/通用错误码/errorcode-universal.md)和[关系型数据库错误码](../../../../../harmonyos-references/ArkData（方舟数据管理）/错误码/关系型数据库错误码/errorcode-data-rdb.md)。
   ...
   [vectorStoreCTUD.ets](https://gitcode.com/HarmonyOS_Samples/guide-snippets/blob/HarmonyOS-feature-20260402/ArkData/VectorStore/entry/src/main/ets/pages/crud/vectorStoreCTUD.ets#L23-L54)
3. 获取到RdbStore后，调用execute接口插入数据。
   ...
   [vectorStoreCTUD.ets](https://gitcode.com/HarmonyOS_Samples/guide-snippets/blob/HarmonyOS-feature-20260402/ArkData/VectorStore/entry/src/main/ets/pages/crud/vectorStoreCTUD.ets#L56-L66)
4. 获取到RdbStore后，调用execute接口修改或删除数据。示例代码如下：
   ...
   [vectorStoreCTUD.ets](https://gitcode.com/HarmonyOS_Samples/guide-snippets/blob/HarmonyOS-feature-20260402/ArkData/VectorStore/entry/src/main/ets/pages/crud/vectorStoreCTUD.ets#L68-L89)
5. 获取到RdbStore后，调用querySql方法查找数据，返回一个ResultSet结果集。
   ...
   [vectorStoreCTUD.ets](https://gitcode.com/HarmonyOS_Samples/guide-snippets/blob/HarmonyOS-feature-20260402/ArkData/VectorStore/entry/src/main/ets/pages/crud/vectorStoreCTUD.ets#L91-L139)
6. 创建视图并执行查询。示例代码如下：
   ...
   [vectorStoreCTUD.ets](https://gitcode.com/HarmonyOS_Samples/guide-snippets/blob/HarmonyOS-feature-20260402/ArkData/VectorStore/entry/src/main/ets/pages/crud/vectorStoreCTUD.ets#L141-L151)
7. ‌使用向量索引进行查询，提升查询效率。
   ...
   * 向量索引的命中条件。必须是ORDER BY + LIMIT类型的查询，ORDER BY只有一个排序条件，这个条件是向量距离条件；ORDER BY与DESC连用，不会使用向量索引；查询距离度量与创建索引时的度量需要保持一致，例如创建向量索引时使用L2，在查询时使用<->进行度量才可以命中索引。
   ...
   [vectorStoreCTUD.ets](https://gitcode.com/HarmonyOS_Samples/guide-snippets/blob/HarmonyOS-feature-20260402/ArkData/VectorStore/entry/src/main/ets/pages/crud/vectorStoreCTUD.ets#L153-L171)
8. 手动回收索引删除产生的磁盘碎片。从API version 20 开始支持此功能。
   ...
   * 磁盘碎片回收任务由后台基于负载自动调度，通常仅在低负载场景下执行，成功所需时间依赖负载情况。
   ...
   [vectorStoreCTUD.ets](https://gitcode.com/HarmonyOS_Samples/guide-snippets/blob/HarmonyOS-feature-20260402/ArkData/VectorStore/entry/src/main/ets/pages/crud/vectorStoreCTUD.ets#L173-L180)
9. 配置数据老化功能。当应用的数据需要经常清理时，可以按时间或空间配置数据老化策略，从而实现数据的自动化清理。
   ...
   **表4** 数据老化策略参数(parameter)
   ...
   [vectorStoreCTUD.ets](https://gitcode.com/HarmonyOS_Samples/guide-snippets/blob/HarmonyOS-feature-20260402/ArkData/VectorStore/entry/src/main/ets/pages/crud/vectorStoreCTUD.ets#L182-L189)
10. 配置数据压缩功能。该功能在建表时配置，可以压缩数据类型为text的列数据。
    ...
    [vectorStoreCTUD.ets](https://gitcode.com/HarmonyOS_Samples/guide-snippets/blob/HarmonyOS-feature-20260402/ArkData/VectorStore/entry/src/main/ets/pages/crud/vectorStoreCTUD.ets#L191-L198)
11. 删除数据库。
    ...
    [vectorStoreCTUD.ets](https://gitcode.com/HarmonyOS_Samples/guide-snippets/blob/HarmonyOS-feature-20260402/ArkData/VectorStore/entry/src/main/ets/pages/crud/vectorStoreCTUD.ets#L200-L208)
