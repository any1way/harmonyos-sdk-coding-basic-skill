...
scraped_at: 2026-06-11T14:26:05+08:00
doc_updated_at: 2026-06-09
content_hash: sha256:eb089d6dbb807c1713aa05dfac2c568ba9a72a443e009dc0c5200e82047b5a87
---
## 场景介绍
...
* **极简集成**
  ...
  简化开发：开发者通过使用[端云同步](<../端云数据同步关系型数据库端侧开发指导 (ArkTS)/data-cloud-sync-of-rdb-store.md>)为应用快速添加云同步功能。
...
1. 应用声明。
   ...
   通过配置[app.json5](../../../../基础入门/开发基础知识/应用配置文件（Stage模型）/app.json5配置文件/app-configuration-file.md)文件中app标签的cloudStructuredDataSyncEnabled字段来控制是否接入云同步能力，字段为true时，表示接入，在“设置-云空间”内即可看到应用开关。
...
3. 开发流程具体请见[端云数据同步关系型数据库端侧开发指导](<../端云数据同步关系型数据库端侧开发指导 (ArkTS)/data-cloud-sync-of-rdb-store.md>)。
4. 体验设计建议。
   ...
   同步受功能开启状态、网络条件、云存储空间等方面的影响，请参考[端云同步状态码](<../../../../../harmonyos-references/ArkData（方舟数据管理）/ArkTS API/@ohos.data.relationalStore (关系型数据库)/Enums/arkts-apis-data-relationalstore-e.md#progresscode10>)设计同步状态提醒。例如：
   ...
   * 同步已完成，有明确完成状态码时，参考如下表格增加提醒：
     ...
     前往“设置-云空间”管理，也可添加[Deeplink](<../../../Ability Kit（程序框架服务）/Stage模型开发指导/应用间跳转/拉起指定应用/使用Deep Linking实现应用间跳转/deep-linking-startup.md>)文字链，用户点击直接跳转至云空间首页，跳转uri为：hicloud://cloudDrive/getInfo?path=MainActivity。
