...
content_hash: sha256:689d58080fe3c78cb70ca25eaae0ed74f36e80d7df7d22c725fcf2860ea83f46
---
Web组件能够实现在不同窗口的组件树上进行挂载或移除操作，这一能力使得开发者可以预先创建Web组件，从而实现性能优化。例如，Tab页为Web组件时，页面预先渲染，便于即时显示。

离线Web组件基于自定义占位组件[NodeContainer](../../../../harmonyos-references/ArkUI（方舟UI框架）/ArkTS组件/自定义占位组件/NodeContainer/ts-basic-components-nodecontainer.md)实现。基本原理是构建支持命令式创建的Web组件，此类组件创建后不会立即挂载到组件树中，状态为Hidden和Inactive，因此不会立即对用户呈现。开发者可以在后续使用中按需动态挂载这些组件，以实现更灵活的使用方式。
...
如下图所示，在需要离屏创建Web组件时，定义一个自定义组件以封装Web组件，此Web组件在离线状态下被创建，封装于无状态的[NodeContainer](../../../../harmonyos-references/ArkUI（方舟UI框架）/ArkTS组件/自定义占位组件/NodeContainer/ts-basic-components-nodecontainer.md)节点中，并与相应的[NodeController](<../../../../harmonyos-references/ArkUI（方舟UI框架）/ArkTS API/UI界面/arkui/NodeController/js-apis-arkui-nodecontroller.md>)组件绑定。Web组件在后台预渲染完毕后，当需要展示时，通过[NodeController](<../../../../harmonyos-references/ArkUI（方舟UI框架）/ArkTS API/UI界面/arkui/NodeController/js-apis-arkui-nodecontroller.md>)将其挂载到ViewTree的[NodeContainer](../../../../harmonyos-references/ArkUI（方舟UI框架）/ArkTS组件/自定义占位组件/NodeContainer/ts-basic-components-nodecontainer.md)中，即与对应的[NodeContainer](../../../../harmonyos-references/ArkUI（方舟UI框架）/ArkTS组件/自定义占位组件/NodeContainer/ts-basic-components-nodecontainer.md)组件绑定，即可挂载上树并显示。

![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/63/v3/M4zgqSPtTRiWfRvnAG2Ceg/zh-cn_image_0000002622858091.png?HW-CC-KV=V1&HW-CC-Date=20260611T063607Z&HW-CC-Expire=86400&HW-CC-Sign=957C618F293FC7B6B57344D1E29161A9043E05F68FF51766761B87654BF4B483)
...
[EntryAbility.ets](https://gitcode.com/HarmonyOS_Samples/guide-snippets/blob/HarmonyOS-feature-20260402/ArkWeb/UseOfflineWebComp/entry/src/main/ets/entryability/EntryAbility.ets#L16-L47)
...
[Common.ets](https://gitcode.com/HarmonyOS_Samples/guide-snippets/blob/HarmonyOS-feature-20260402/ArkWeb/UseOfflineWebComp/entry/src/main/ets/pages/Common.ets#L17-L106)
...
[Index.ets](https://gitcode.com/HarmonyOS_Samples/guide-snippets/blob/HarmonyOS-feature-20260402/ArkWeb/UseOfflineWebComp/entry/src/main/ets/pages/Index.ets#L17-L37)
...
[Entry1Ability.ets](https://gitcode.com/HarmonyOS_Samples/guide-snippets/blob/HarmonyOS-feature-20260402/ArkWeb/UseOfflineWebComp/entry1/src/main/ets/entry1ability/Entry1Ability.ets#L16-L47)
...
[Index.ets](https://gitcode.com/HarmonyOS_Samples/guide-snippets/blob/HarmonyOS-feature-20260402/ArkWeb/UseOfflineWebComp/entry1/src/main/ets/pages/Index.ets#L16-L38)
...
[Index2.ets](https://gitcode.com/HarmonyOS_Samples/guide-snippets/blob/HarmonyOS-feature-20260402/ArkWeb/UseOfflineWebComp/entry1/src/main/ets/pages/Index2.ets#L15-L35)
...
3. 预渲染的网页会在后台不断进行渲染，建议在预渲染完成后立即停止渲染，以防止发热和功耗问题。可以参考以下示例，使用 [onFirstMeaningfulPaint](<../../../../harmonyos-references/ArkWeb（方舟Web）/ArkTS 组件/Web/事件/arkts-basic-components-web-events.md#onfirstmeaningfulpaint12>) 来确定停止时机，该接口适用于http和https网页。
...
[Common.ets](https://gitcode.com/HarmonyOS_Samples/guide-snippets/blob/HarmonyOS-feature-20260402/ArkWeb/UseOfflineWebComp/entry2/src/main/ets/pages/Common.ets#L16-L127)
...
[Common.ets](https://gitcode.com/HarmonyOS_Samples/guide-snippets/blob/HarmonyOS-feature-20260402/ArkWeb/UseOfflineWebComp/entry3/src/main/ets/pages/Common.ets#L152-L221)
...
示例演示了如何让应用退后台释放离线Web组件以及切前台恢复离线Web组件，在[UIAbility](<../../../../harmonyos-references/Ability Kit（程序框架服务）/ArkTS API/Stage模型能力的接口/@ohos.app.ability.UIAbility (带界面的应用组件)/js-apis-app-ability-uiability.md>)的onBackground和onForeground回调中分别进行了离线Web组件的释放和恢复。
...
[Entry3Ability.ets](https://gitcode.com/HarmonyOS_Samples/guide-snippets/blob/HarmonyOS-feature-20260402/ArkWeb/UseOfflineWebComp/entry3/src/main/ets/entry3ability/Entry3Ability.ets#L48-L60)
...
* Page1页面同时显示了两个Web页面，每个页面使用了一个离线Web组件，加载并显示相同URL的内容。该页面用于演示预渲染与不预渲染的效果对比，以及如何复用离线组件。
  ...
  ![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/cd/v3/3bIydypYT1mGevpRgQEggA/zh-cn_image_0000002622698213.gif?HW-CC-KV=V1&HW-CC-Date=20260611T063607Z&HW-CC-Expire=86400&HW-CC-Sign=73ECD450FC655F54D12805A2DC600A4A0978E86984B6B71F23E83D25422A9F92)
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/65/v3/eAoqdfe-S0uQJqG8trA24Q/zh-cn_image_0000002592218652.gif?HW-CC-KV=V1&HW-CC-Date=20260611T063607Z&HW-CC-Expire=86400&HW-CC-Sign=A2DF20C5F0B123279168CD319BA8FA5BC3B24606FDC4F44226BCC72E5DDF2A1B)
...
检查是否已在module.json5中添加网络权限，添加方法请参考[在配置文件中声明权限](../../../系统/安全/程序访问控制/应用权限管控/申请应用权限/声明权限/declare-permissions.md#在配置文件中声明权限)。
...
[module.json5](https://gitcode.com/HarmonyOS_Samples/guide-snippets/blob/HarmonyOS-feature-20260402/ArkWeb/UseOfflineWebComp/entry2/src/main/module.json5#L41-L47)

2.排查[NodeContainer](../../../../harmonyos-references/ArkUI（方舟UI框架）/ArkTS组件/自定义占位组件/NodeContainer/ts-basic-components-nodecontainer.md)与节点绑定的逻辑。

检查节点是否已上组件树，建议在已有的Web组件上方加上Text（请参考以下例子），如果白屏的时候没有出现Text，建议检查[NodeContainer](../../../../harmonyos-references/ArkUI（方舟UI框架）/ArkTS组件/自定义占位组件/NodeContainer/ts-basic-components-nodecontainer.md)与节点绑定的情况。
...
如果整个节点已上树，可通过日志[WebPattern::OnVisibleAreaChange](../../../../harmonyos-references/ArkUI（方舟UI框架）/ArkTS组件/通用事件/组件变化事件/组件可见区域变化事件/ts-universal-component-visible-area-change-event.md#onvisibleareachange)查看Web组件可见性状态是否正确，不可见的Web组件可能会造成白屏。
