...
scraped_at: 2026-06-01T11:11:04+08:00
doc_updated_at: 2026-05-26
content_hash: sha256:5b95c26eeca56df95ece9b5ddd3a4958da1b0d0c31aab42e47920eb3ab9a1f89
---
...
* 如果设置[autoCreateEmptyFile](<../../../../../../harmonyos-references/Core File Kit（文件基础服务）/ArkTS API/@ohos.file.picker (选择器)/js-apis-file-picker.md#documentsaveoptions>)参数为false，获取的URI除了具备**临时读写权限**外，还具备**临时创建和删除权限**。
* 获取持久化权限需要通过[FilePicker设置永久授权](../授权持久化/file-persistpermission.md#通过picker获取临时授权并进行授权持久化)方式获取。
* 使用Picker对音频、图片、视频、文档类文件的保存操作**无需申请权限**。

**系统隔离说明**

* 通过Picker保存的文件存储在用户指定的目录。此类文件与图库管理的资源隔离，无法在图库中看到。
* 若开发者需要保存图片、视频资源到图库，可使用用户无感的[安全控件进行保存](<../../../../../媒体/Media Library Kit（媒体文件管理服务）/保存媒体库资源/photoaccesshelper-savebutton.md#使用安全控件保存媒体库资源>)。

## 保存图片或视频类文件

[PhotoViewPicker](<../../../../../../harmonyos-references/Core File Kit（文件基础服务）/ArkTS API/@ohos.file.picker (选择器)/js-apis-file-picker.md#photoviewpickerdeprecated>)在后续版本不再演进，建议使用[Media Library Kit（媒体文件管理服务）中能力来保存媒体库资源](<../../../../../媒体/Media Library Kit（媒体文件管理服务）/保存媒体库资源/photoaccesshelper-savebutton.md>)。

如果开发场景无法调用安全控件进行图片、视频保存，可使用相册管理模块[PhotoAccessHelper.showAssetsCreationDialog](<../../../../../../harmonyos-references/Media Library Kit（媒体文件管理服务）/ArkTS API/@ohos.file.photoAccessHelper (相册管理模块)/Interface (PhotoAccessHelper)/arkts-apis-photo_93807367.md#showassetscreationdialog12>)接口进行保存操作。
...
1. 模块导入。
   ...
   ```
2. 根据实际业务需求配置[文件保存选项](<../../../../../../harmonyos-references/Core File Kit（文件基础服务）/ArkTS API/@ohos.file.picker (选择器)/js-apis-file-picker.md#documentsaveoptions>)。以下代码仅例举各选项的配置参考。
   ...
   ```
3. 创建文件选择器[DocumentViewPicker](<../../../../../../harmonyos-references/Core File Kit（文件基础服务）/ArkTS API/@ohos.file.picker (选择器)/js-apis-file-picker.md#constructor12>)实例。调用[save()](<../../../../../../harmonyos-references/Core File Kit（文件基础服务）/ArkTS API/@ohos.file.picker (选择器)/js-apis-file-picker.md#save>)接口拉起FilePicker界面进行文件保存。
   ...
   [Index.ets](https://gitcode.com/HarmonyOS_Samples/guide-snippets/blob/HarmonyOS-feature-20260402/CoreFile/UserFile/SavingUserFiles/entry/src/main/ets/pages/Index.ets#L136-L149)

   注意

   * Picker会默认[预置空文件](<../../../../../../harmonyos-references/Core File Kit（文件基础服务）/ArkTS API/@ohos.file.picker (选择器)/js-apis-file-picker.md#documentsaveoptions>)并返回保存文件的URI数组，应用拿到URI后可使用[文件管理](<../../../../../../harmonyos-references/Core File Kit（文件基础服务）/ArkTS API/@ohos.file.fs (文件管理)/js-apis-file-fs.md>)模块的接口进行文件读写操作。
   * 避免在Picker回调中直接操作URI。
   * 建议使用全局变量保存URI以供后续使用。
   * 可以通过[DOWNLOAD模式](save-user-file.md#download模式保存文件)直达下载目录。
4. 待界面从FilePicker返回后，使用[fileIo.openSync](<../../../../../../harmonyos-references/Core File Kit（文件基础服务）/ArkTS API/@ohos.file.fs (文件管理)/js-apis-file-fs.md#fileioopensync>)接口，通过URI打开这个文件得到文件描述符（fd）。
   ...
   6. }
   ```
5. 通过（fd）使用[fileIo.writeSync](<../../../../../../harmonyos-references/Core File Kit（文件基础服务）/ArkTS API/@ohos.file.fs (文件管理)/js-apis-file-fs.md#fileiowritesync>)接口对这个文件进行编辑修改，编辑修改完成后关闭（fd）。
...
2. 配置保存选项。
   ...
   ```
3. 创建[音频选择器AudioViewPicker](<../../../../../../harmonyos-references/Core File Kit（文件基础服务）/ArkTS API/@ohos.file.picker (选择器)/js-apis-file-picker.md#audioviewpicker>)实例。调用[save()](<../../../../../../harmonyos-references/Core File Kit（文件基础服务）/ArkTS API/@ohos.file.picker (选择器)/js-apis-file-picker.md#save-5>)接口拉起FilePicker界面进行文件保存。
   ...
   [Index.ets](https://gitcode.com/HarmonyOS_Samples/guide-snippets/blob/HarmonyOS-feature-20260402/CoreFile/UserFile/SavingUserFiles/entry/src/main/ets/pages/Index.ets#L218-L231)
   ...
   * 建议使用全局变量保存URI以供后续使用。
   * 可以通过[DOWNLOAD模式](save-user-file.md#download模式保存文件)直达下载目录。
4. 待界面从FilePicker返回后，可以使用[fileIo.openSync](<../../../../../../harmonyos-references/Core File Kit（文件基础服务）/ArkTS API/@ohos.file.fs (文件管理)/js-apis-file-fs.md#fileioopensync>)接口，通过URI打开这个文件得到文件描述符（fd）。
