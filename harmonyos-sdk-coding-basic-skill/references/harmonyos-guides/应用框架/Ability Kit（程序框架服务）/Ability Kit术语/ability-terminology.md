...
content_hash: sha256:5c9ee63b36f2178badaa381c880fa12f0c834bded8434d3dd47285a68080b000
---
## A

### AbilityStage

[AbilityStage](<../../../../harmonyos-references/Ability Kit（程序框架服务）/ArkTS API/Stage模型能力的接口/@ohos.app.ability.AbilityStage (AbilityStage组件管理器)/js-apis-app-ability-abilitystage.md>)是一个[Module](../../../基础入门/开发基础知识/应用程序包基础知识/应用程序包概述/application-package-overview.md#应用的多module设计机制)级别的组件管理器。
...
[意图提供方管理能力](<../../../../harmonyos-references/Ability Kit（程序框架服务）/ArkTS API/Stage模型能力的接口/@ohos.app.ability.insightIntentProvider (意图提供方管理能力)/js-apis-app-ability-insightintentprovider.md>)指通过声明标准意图或自定义意图，将业务功能接入意图框架的应用主体。
...
开发者可以设置[onNewProcessRequest](<../../../../harmonyos-references/Ability Kit（程序框架服务）/ArkTS API/Stage模型能力的接口/@ohos.app.ability.AbilityStage (AbilityStage组件管理器)/js-apis-app-ability-abilitystage.md#onnewprocessrequest11>)的返回值，让主控进程通过接收该接口的回调信息，决定新的Ability实例运行在哪个进程中。
...
显式Want启动是指在启动目标应用组件时，调用方传入的[want](<../../../../harmonyos-references/Ability Kit（程序框架服务）/ArkTS API/通用能力的接口(推荐)/@ohos.app.ability.Want (Want)/js-apis-app-ability-want.md>)参数中指定了abilityName和bundleName，称为显式Want启动。
...
隐式Want启动是指在启动目标应用组件时，调用方传入的[want](<../../../../harmonyos-references/Ability Kit（程序框架服务）/ArkTS API/通用能力的接口(推荐)/@ohos.app.ability.Want (Want)/js-apis-app-ability-want.md>)参数中未指定abilityName，称为隐式Want启动。

当目标处理对象不明确时，可以使用隐式Want启动。它允许当前应用直接调用其他应用提供的某项能力，而无需关心该能力由哪个具体应用提供。

### 应用级so

应用开发者通过[Node-API](../../../NDK开发/代码开发/使用Node-API实现ArkTSJS与CC++语言交互/使用Node-API实现跨语言交互开发流程/use-napi-process.md)方式自行开发的so，属于应用的一部分，随应用发布。
