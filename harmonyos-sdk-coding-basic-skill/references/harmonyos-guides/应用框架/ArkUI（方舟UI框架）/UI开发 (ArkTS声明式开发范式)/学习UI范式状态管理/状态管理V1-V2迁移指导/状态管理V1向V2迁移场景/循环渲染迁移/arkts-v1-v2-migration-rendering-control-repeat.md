...
scraped_at: 2026-06-01T11:03:30+08:00
doc_updated_at: 2026-04-20
content_hash: sha256:1f614a0a2097f9156d4560d0c3656a8ed1fedc1db573f027f5d7758fe8b4a66d
---
本文档主要介绍组件循环渲染从V1向V2的迁移，涉及如下渲染控制组件。

| 迁移前渲染控制组件名称 | 迁移后渲染控制组件名称 |
| --- | --- |
| [ForEach](../../../../学习UI范式渲染控制/ForEach：循环渲染/arkts-rendering-control-foreach.md) | [Repeat](../../../../学习UI范式渲染控制/Repeat：可复用的循环渲染/arkts-new-rendering-control-repeat.md) |
| [LazyForEach](../../../../学习UI范式渲染控制/LazyForEach：数据懒加载/arkts-rendering-control-lazyforeach.md) | [Repeat](../../../../学习UI范式渲染控制/Repeat：可复用的循环渲染/arkts-new-rendering-control-repeat.md) |

## ForEach迁移Repeat

状态管理V2推荐使用Repeat替代ForEach。

ForEach使用示例请参考[数据源数组项子属性变化](../../../../学习UI范式渲染控制/ForEach：循环渲染/arkts-rendering-control-foreach.md#数据源数组项子属性变化)。对应在V2中，ForEach迁移Repeat示例代码参考如下。
...
示例1中，在容器组件[List](<../../../../列表与网格/创建列表 (List)/arkts-layout-development-create-list.md>)中使用LazyForEach，并基于数据源循环渲染出了一系列[Text](<../../../../使用文本/文本显示 (TextSpan)/arkts-common-components-text-display.md>)子组件。
...
1. 使用状态管理V2装饰器。
   ...
   Repeat推荐和状态管理V2装饰器配合使用（[懒加载](../../../../学习UI范式渲染控制/Repeat：可复用的循环渲染/arkts-new-rendering-control-repeat.md#懒加载能力说明)模式下只支持和状态管理V2装饰器配合使用）。如果之前使用的是状态管理V1装饰器，需要修改为状态管理V2装饰器。
...
2. 迁移数据源。
   ...
   LazyForEach使用专用的数据结构[IDataSource](../../../../../../../../harmonyos-references/ArkUI（方舟UI框架）/ArkTS组件/状态管理与渲染控制/LazyForEach/ts-rendering-control-lazyforeach.md#idatasource)作为数据源。迁移至Repeat后，不再使用IDataSource作为数据源，而是使用状态管理V2装饰的数组作为数据源。
...
3. 迁移组件生成函数和键值生成函数。
   ...
   从LazyForEach迁移至Repeat时，两者的语法存在差异。Repeat需要在[.each()](../../../../../../../../harmonyos-references/ArkUI（方舟UI框架）/ArkTS组件/状态管理与渲染控制/Repeat/ts-rendering-control-repeat.md#each)或[.template()](../../../../../../../../harmonyos-references/ArkUI（方舟UI框架）/ArkTS组件/状态管理与渲染控制/Repeat/ts-rendering-control-repeat.md#template)中设置组件生成函数，在[.key()](../../../../../../../../harmonyos-references/ArkUI（方舟UI框架）/ArkTS组件/状态管理与渲染控制/Repeat/ts-rendering-control-repeat.md#key)中设置键值生成函数。
...
4. 配置懒加载功能。
   ...
   Repeat具有[懒加载和全量加载](../../../../学习UI范式渲染控制/Repeat：可复用的循环渲染/arkts-new-rendering-control-repeat.md#懒加载能力说明)两种模式。

   * 全量加载模式渲染所有子节点（对标[ForEach](../../../../学习UI范式渲染控制/ForEach：循环渲染/arkts-rendering-control-foreach.md)）。
   * 懒加载模式动态渲染屏幕区域和预加载区域内的子节点（需要与容器组件配合使用，对标LazyForEach）。

   从LazyForEach迁移至Repeat时，需要调用[virtualScroll](../../../../../../../../harmonyos-references/ArkUI（方舟UI框架）/ArkTS组件/状态管理与渲染控制/Repeat/ts-rendering-control-repeat.md#virtualscroll)属性，使能懒加载。
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/31/v3/o2-h4W1MSE20nGw8fCDzJA/zh-cn_image_0000002583118138.gif?HW-CC-KV=V1&HW-CC-Date=20260601T030329Z&HW-CC-Expire=86400&HW-CC-Sign=A7A1521709E362C24A8EA3CF95A444E11C89EEC1DB0271E5160197F588654A03)

### 数据更新操作

**LazyForEach示例**

当LazyForEach的数据源发生变化时，开发者需要根据数据源的变化情况调用[DataChangeListener](../../../../../../../../harmonyos-references/ArkUI（方舟UI框架）/ArkTS组件/状态管理与渲染控制/LazyForEach/ts-rendering-control-lazyforeach.md#datachangelistener)对应的接口，通知LazyForEach做相应的更新。主要的数据操作包括：添加数据、删除数据、交换数据、修改单个数据、修改多个数据、精准批量修改数据。
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/52/v3/FHoe5hu3RKGWSmQVA-Umjg/zh-cn_image_0000002613637903.gif?HW-CC-KV=V1&HW-CC-Date=20260601T030329Z&HW-CC-Expire=86400&HW-CC-Sign=E5603AC0E7DCD59B6ED84E09FC7BA2EEA984F75C9BBCC12B167594930D273AF5)

### 修改数据子属性

**LazyForEach示例**

LazyForEach可以使用[@Observed与@ObjectLink](../../../状态管理（V1）/管理组件拥有的状态/@Observed装饰器和@ObjectLink装饰器：嵌套类对象属性变化/arkts-observed-and-objectlink.md)装饰器实现对数据子属性的观测。当有数据子属性发生变化时，仅更新使用了该子属性的组件，从而提高性能。
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/36/v3/f0-lpnTaTqOqkty7X7gj-g/zh-cn_image_0000002613518011.gif?HW-CC-KV=V1&HW-CC-Date=20260601T030329Z&HW-CC-Expire=86400&HW-CC-Sign=86209F2CFCA920A348D69A405DCEAEB87C890641709447CB03D5C5757BF63BEB)

### 状态管理V2观测组件内部状态

**LazyForEach示例**

状态管理V2的[@Local](../../../状态管理（V2）/管理组件拥有的状态/@Local装饰器：组件内部状态/arkts-new-local.md)装饰器提供了观测自定义组件内部变量的能力。被@Local装饰的变量发生变化时，会通知LazyForEach更新对应的组件。
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/5d/v3/XC_lGpH4SIGmlpGm0CQTcw/zh-cn_image_0000002582958214.gif?HW-CC-KV=V1&HW-CC-Date=20260601T030329Z&HW-CC-Expire=86400&HW-CC-Sign=01D0388C61BDFAA042534832D8A5331E8EF7FD5EB1B7C805BAEBFCB708A54043)

### 状态管理V2观测组件外部输入

**LazyForEach示例**

状态管理V2的[@Param](../../../状态管理（V2）/管理组件拥有的状态/@Param：组件外部输入/arkts-new-param.md)装饰器提供了观测自定义组件外部输入变量的能力，可以实现父子组件间的数据同步。将父组件的变量传递给子组件，并用@Param装饰，当父组件变量发生变化时，会通知对应的组件更新。
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/e7/v3/SIWTsH8yStSUoNW-C62VHw/zh-cn_image_0000002583118142.gif?HW-CC-KV=V1&HW-CC-Date=20260601T030329Z&HW-CC-Expire=86400&HW-CC-Sign=A951D532A99CB5805E3AB5479A839AFE9A39A148B11E1AF7957DCFFCB99E16B3)

### 拖拽排序

**LazyForEach示例**

LazyForEach的[onMove](../../../../../../../../harmonyos-references/ArkUI（方舟UI框架）/ArkTS组件/通用属性/交互属性/拖拽排序/ts-universal-attributes-drag-sorting.md#onmove)属性提供了拖拽排序能力。
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/5e/v3/QGc4I0NrQBu0Ab2siKcPbQ/zh-cn_image_0000002613637905.gif?HW-CC-KV=V1&HW-CC-Date=20260601T030329Z&HW-CC-Expire=86400&HW-CC-Sign=5D3D7B8A5AC5D82B25C74D223EFCAA1979A7BB9FCB9C83635BB4DB475DEDD7B5)

### 组件复用

**LazyForEach示例**

LazyForEach自身并不具备组件复用能力，为实现组件复用，需要与[@Reusable](../../../../学习UI范式基本语法/自定义组件/自定义组件复用/@Reusable装饰器：V1组件复用/arkts-reusable.md)装饰器配合使用（被@Reusable装饰的自定义组件具有复用能力）。
...
Repeat本身具备组件复用能力，同时也支持与状态管理V2的[@ReusableV2](../../../../学习UI范式基本语法/自定义组件/自定义组件复用/@ReusableV2装饰器：V2组件复用/arkts-new-reusablev2.md)装饰器联合使用。因此，迁移至Repeat后，其组件复用具有两种实现方案。

1. 直接使用Repeat自身的复用能力。
2. 使用@ReusableV2装饰器提供的复用能力。

需要注意的是，Repeat默认启用自身的复用能力，且优先级高于@ReusableV2装饰器。若要使用@ReusableV2装饰器，必须先手动关闭Repeat自身的复用能力，即Repeat中[VirtualScrollOptions](../../../../../../../../harmonyos-references/ArkUI（方舟UI框架）/ArkTS组件/状态管理与渲染控制/Repeat/ts-rendering-control-repeat.md#virtualscrolloptions)的reusable属性设置为false（@ReusableV2装饰器从API version 18开始支持，Repeat从API version 18开始支持关闭自身复用能力）。
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/dc/v3/VvO94IUFSFWt3VqMic17DA/zh-cn_image_0000002613518015.gif?HW-CC-KV=V1&HW-CC-Date=20260601T030329Z&HW-CC-Expire=86400&HW-CC-Sign=2F64E9E117B92D98ED269AB48FA7CB0555B49F2A1D7BC0390F73EC2B0AB2EDBE)
...
Repeat本身具备模板渲染能力，开发者可以通过[templateId](../../../../../../../../harmonyos-references/ArkUI（方舟UI框架）/ArkTS组件/状态管理与渲染控制/Repeat/ts-rendering-control-repeat.md#templateid)方法为不同的数据项选择不同的模板，再通过[template](../../../../../../../../harmonyos-references/ArkUI（方舟UI框架）/ArkTS组件/状态管理与渲染控制/Repeat/ts-rendering-control-repeat.md#template)方法为不同的模板配置不同的组件生成函数。同时，开发者仍然可以选择自己实现逻辑判断，为不同的数据项分配不同的模板。
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/ad/v3/l3xID36tSbK9pJjNswvHJA/zh-cn_image_0000002582958216.gif?HW-CC-KV=V1&HW-CC-Date=20260601T030329Z&HW-CC-Expire=86400&HW-CC-Sign=0AECB00CAF25A2109955907867478F2D7F60B1135E0158C2614DFF0FFB0C5992)
