...
scraped_at: 2026-06-11T14:29:21+08:00
doc_updated_at: 2026-05-26
content_hash: sha256:3f1c95ca1be9101809df8e0cf0e3c69630048f8d7a8cb05e802000134bf1e432
---
PersistentStorage是应用程序中的可选单例对象。此对象的作用是持久化存储选定的AppStorage属性，以确保这些属性在应用程序重新启动时的值与应用程序关闭时的值相同。

PersistentStorage提供状态变量持久化的能力，但是需要注意，其持久化和读回UI的能力都需要依赖AppStorage。在阅读本文档前，建议提前阅读：[AppStorage](../AppStorage：应用全局的UI状态存储/arkts-appstorage.md)，[PersistentStorage API文档](../../../../../../../../harmonyos-references/ArkUI（方舟UI框架）/ArkTS组件/状态管理与渲染控制/应用级变量的状态管理/ts-state-management.md#persistentstorage)。
...
PersistentStorage功能上耦合了AppStorage，并且数据在不同module中使用也会有问题，因此推荐开发者使用[PersistenceV2](<../../../状态管理（V2）/管理应用拥有的状态/PersistenceV2 持久化存储UI状态/arkts-new-persistencev2.md>)的globalConnect接口替换掉PersistentStorage的persistProp接口。PersistentStorage向PersistenceV2迁移的方案见[PersistentStorage->PersistenceV2](../../../状态管理V1-V2迁移指导/状态管理V1向V2迁移场景/应用内状态变量迁移/arkts-v1-v2-migration-application.md#persistentstorage-persistencev2)。
...
PersistentStorage的持久化变量最好是小于2kb的数据，不要大量的数据持久化，因为PersistentStorage写入磁盘是在UI线程同步执行的，大量数据本地读写会影响UI渲染性能。如果开发者需要存储大量的数据，建议使用[@ohos.data.relationalStore (关系型数据库)](<../../../../../../../../harmonyos-references/ArkData（方舟数据管理）/ArkTS API/@ohos.data.relationalStore (关系型数据库)/模块描述/arkts-apis-data-relationalstore.md>)相关接口。

PersistentStorage和UI实例相关联，持久化操作需要在UI实例初始化成功后（即[loadContent](<../../../../../../../../harmonyos-references/ArkUI（方舟UI框架）/ArkTS API/窗口管理/@ohos.window (窗口)/Interface (WindowStage)/arkts-apis-window-windowstage.md#loadcontent9>)传入的回调被调用时）才可以被调用，早于该时机调用会导致持久化失败。
...
2. 在AppStorage获取对应属性：
   ...
   [PageOneMessageStorage.ets](https://gitcode.com/HarmonyOS_Samples/guide-snippets/blob/HarmonyOS-feature-20260402/ArkUISample/PersistentStorage/entry/src/main/ets/pages/PageOneMessageStorage.ets#L15-L39)
...
* 新应用安装后首次启动运行：
  ...
  **图1** PersistProp初始化流程
  ...
  ![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/f6/v3/cuBM_jmDTLWllESotw6Kzw/zh-cn_image_0000002622857443.png?HW-CC-KV=V1&HW-CC-Date=20260611T062920Z&HW-CC-Expire=86400&HW-CC-Sign=1D05231FE5261BA9B9C14AA9BB38408B57BFA63B0B933713C1662C38E48CD560)
* 触发点击事件后：
...
[PageThreeAppStorage.ets](https://gitcode.com/HarmonyOS_Samples/guide-snippets/blob/HarmonyOS-feature-20260402/ArkUISample/PersistentStorage/entry/src/main/ets/pages/PageThreeAppStorage.ets#L15-L18)
...
[PageThreeAppStorage.ets](https://gitcode.com/HarmonyOS_Samples/guide-snippets/blob/HarmonyOS-feature-20260402/ArkUISample/PersistentStorage/entry/src/main/ets/pages/PageThreeAppStorage.ets#L27-L33)
...
[PageFourMessageChange.ets](https://gitcode.com/HarmonyOS_Samples/guide-snippets/blob/HarmonyOS-feature-20260402/ArkUISample/PersistentStorage/entry/src/main/ets/pages/PageFourMessageChange.ets#L15-L52)
...
[PageFivePersistedDate.ets](https://gitcode.com/HarmonyOS_Samples/guide-snippets/blob/HarmonyOS-feature-20260402/ArkUISample/PersistentStorage/entry/src/main/ets/pages/PageFivePersistedDate.ets#L15-L69)
...
[PageSixPersistedMap.ets](https://gitcode.com/HarmonyOS_Samples/guide-snippets/blob/HarmonyOS-feature-20260402/ArkUISample/PersistentStorage/entry/src/main/ets/pages/PageSixPersistedMap.ets#L15-L60)
...
[PageSevenPersistedSet.ets](https://gitcode.com/HarmonyOS_Samples/guide-snippets/blob/HarmonyOS-feature-20260402/ArkUISample/PersistentStorage/entry/src/main/ets/pages/PageSevenPersistedSet.ets#L15-L82)
