...
scraped_at: 2026-06-11T14:29:14+08:00
doc_updated_at: 2026-05-26
content_hash: sha256:624a6b8b426a13af1e93bc717bbb5b65b4bf83ad82ee10d17394bdbd0e44cd8b
---
@Watch应用于对状态变量的监听。如果开发者需要关注某个状态变量的值是否改变，可以使用@Watch为状态变量设置回调函数。

@Watch提供了状态变量的监听能力，@Watch仅能监听到可以观察到的变化。

在阅读本文档前，建议开发者对状态管理基本观察能力有基本的了解。建议提前阅读：[@State](../@State装饰器：组件内状态/arkts-state.md)。
...
| 装饰器的顺序 | 装饰器顺序不影响实际功能，开发者可以根据自己的需要决定装饰器顺序的先后。建议[@State](../@State装饰器：组件内状态/arkts-state.md)、[@Prop](../@Prop装饰器：父子单向同步/arkts-prop.md)、[@Link](../@Link装饰器：父子双向同步/arkts-link.md)等装饰器在@Watch装饰器之前，以保持整体风格的一致。 |
...
1. 当观察到状态变量的变化（包括双向绑定的[AppStorage](../../管理应用拥有的状态/AppStorage：应用全局的UI状态存储/arkts-appstorage.md)和[LocalStorage](../../管理应用拥有的状态/LocalStorage：页面级UI状态存储/arkts-localstorage.md)中对应的key发生的变化）的时候，对应的@Watch的回调方法将被触发；
...
[CountModifier.ets](https://gitcode.com/HarmonyOS_Samples/guide-snippets/blob/HarmonyOS-feature-20260402/ArkUISample/Watch/entry/src/main/ets/pages/CountModifier.ets#L15-L47)
...
[BasketModifier.ets](https://gitcode.com/HarmonyOS_Samples/guide-snippets/blob/HarmonyOS-feature-20260402/ArkUISample/Watch/entry/src/main/ets/pages/BasketModifier.ets#L15-L75)
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/45/v3/KI_bVeGMSAahE1iu-EUeMg/zh-cn_image_0000002622697561.gif?HW-CC-KV=V1&HW-CC-Date=20260611T062913Z&HW-CC-Expire=86400&HW-CC-Sign=3BFFF65ED97596E5EADE3AA98B675720ECC8C361F075AE96FEE907F590EDC1E0)

### @Watch的触发时机

为了展示@Watch回调触发时间是根据状态变量真正变化的时间，本示例在子组件中同时使用@Link和[@ObjectLink](../@Observed装饰器和@ObjectLink装饰器：嵌套类对象属性变化/arkts-observed-and-objectlink.md)装饰器，分别观察不同的状态对象。通过在父组件中更改状态变量并观察@Watch回调的先后顺序，来表明@Watch触发的时机与赋值、同步的关系。
...
[ParentComponent.ets](https://gitcode.com/HarmonyOS_Samples/guide-snippets/blob/HarmonyOS-feature-20260402/ArkUISample/Watch/entry/src/main/ets/pages/ParentComponent.ets#L16-L97)
...
[UsePropertyName.ets](https://gitcode.com/HarmonyOS_Samples/guide-snippets/blob/HarmonyOS-feature-20260402/ArkUISample/Watch/entry/src/main/ets/pages/UsePropertyName.ets#L15-L47)
