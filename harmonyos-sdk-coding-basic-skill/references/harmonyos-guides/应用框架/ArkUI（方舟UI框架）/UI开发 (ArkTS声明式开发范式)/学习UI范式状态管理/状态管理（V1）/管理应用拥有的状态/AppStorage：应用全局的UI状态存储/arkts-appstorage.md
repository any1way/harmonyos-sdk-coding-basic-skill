...
scraped_at: 2026-06-11T14:29:20+08:00
doc_updated_at: 2026-05-26
content_hash: sha256:d174b8c315ff9aa0c5f3c4e4bc1da1a5e1c712a325fffcdfe2830d9b18f18bf5
---
在阅读本文档前，建议提前阅读：[状态管理概述](../../../状态管理概述/arkts-state-management-overview.md)，从而对状态管理框架中AppStorage的定位有一个宏观了解。

AppStorage是与应用进程绑定的全局UI状态存储中心，由UI框架在应用启动时创建，将UI状态数据存储于运行内存，实现应用级全局状态共享。

作为应用的“中枢”，AppStorage是[持久化数据PersistentStorage](../PersistentStorage：持久化存储UI状态/arkts-persiststorage.md)和[环境变量Environment](../Environment：设备环境查询/arkts-environment.md)与UI交互的中转桥梁。其核心价值在于为开发者提供跨ability的大范围UI状态数据共享能力。

AppStorage提供了API接口，允许开发者在自定义组件外手动触发AppStorage对应属性的增、删、改、查操作。建议配合[AppStorage API文档](../../../../../../../../harmonyos-references/ArkUI（方舟UI框架）/ArkTS组件/状态管理与渲染控制/应用级变量的状态管理/ts-state-management.md#appstorage)阅读。最佳实践请参考[状态管理最佳实践](../../../../../../../../best-practices/声明式语法/状态管理最佳实践/bpta-status-management.md)。

说明

多组件间状态共享和同步、状态管理和UI解耦，可以参考解决方案[基于StateStore的全局状态管理开发实践](../../../../../../../../best-practices/声明式语法/基于StateStore的全局状态管理/bpta-global-state-management-state-store.md)。

不涉及UI组件同步的数据处理工作，建议[通过用户首选项实现数据持久化](<../../../../../../ArkData（方舟数据管理）/应用数据持久化/通过用户首选项实现数据持久化 (ArkTS)/data-persistence-by-preferences.md>)。

## 概述

AppStorage是在应用启动时创建的单例，用于提供应用状态数据的中心存储。这些状态数据在应用级别可访问。AppStorage在应用运行过程中保留其属性。

AppStorage中的属性通过唯一的字符串类型属性名（key）访问，支持与UI组件同步，并可在应用业务逻辑中被访问。其支持应用的[主线程](<../../../../../../Ability Kit（程序框架服务）/Stage模型开发指导/线程模型/thread-model-stage.md>)内多个[UIAbility](<../../../../../../../../harmonyos-references/Ability Kit（程序框架服务）/ArkTS API/Stage模型能力的接口/@ohos.app.ability.UIAbility (带界面的应用组件)/js-apis-app-ability-uiability.md>)实例间的UI状态数据共享。

AppStorage中的属性可以被双向同步，并具有不同的功能，比如数据持久化（详见[PersistentStorage](../PersistentStorage：持久化存储UI状态/arkts-persiststorage.md)）。这些UI状态是通过业务逻辑实现，与UI解耦，如果希望这些UI状态在UI中使用，需要用到[@StorageProp](arkts-appstorage.md#storageprop)和[@StorageLink](arkts-appstorage.md#storagelink)。
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/85/v3/mWiP92-HQFefK5VOz38qLA/zh-cn_image_0000002592218002.png?HW-CC-KV=V1&HW-CC-Date=20260611T062917Z&HW-CC-Expire=86400&HW-CC-Sign=C8F268D513161094B5F75FAB872A6E9545080B44B125FBBBD33DB6927250FD46)
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/56/v3/25ebUFRBROaZgOcpirSqNQ/zh-cn_image_0000002592377936.png?HW-CC-KV=V1&HW-CC-Date=20260611T062917Z&HW-CC-Expire=86400&HW-CC-Sign=402693686457BB6A6001BC6726CC5BC83A7A4C75B7B15F8ACA5E7A125E0B374A)
...
3. AppStorage与[PersistentStorage](../PersistentStorage：持久化存储UI状态/arkts-persiststorage.md)以及[Environment](../Environment：设备环境查询/arkts-environment.md)配合使用时，需要注意以下几点：

   (1) 在AppStorage中创建属性后，调用PersistentStorage.[persistProp](../../../../../../../../harmonyos-references/ArkUI（方舟UI框架）/ArkTS组件/状态管理与渲染控制/应用级变量的状态管理/ts-state-management.md#persistprop10)接口时，会使用AppStorage中已存在的值，并覆盖PersistentStorage中的同名属性。因此，建议使用相反的调用顺序。反例可见[在PersistentStorage之前访问AppStorage中的属性](../PersistentStorage：持久化存储UI状态/arkts-persiststorage.md#在persistentstorage之前访问appstorage中的属性)。

   (2) 如果在AppStorage中已创建属性，再调用Environment.[envProp](../../../../../../../../harmonyos-references/ArkUI（方舟UI框架）/ArkTS组件/状态管理与渲染控制/应用级变量的状态管理/ts-state-management.md#envprop10)创建同名属性，会调用失败。因为AppStorage已有同名属性，Environment环境变量不会再写入AppStorage中，所以建议不要在AppStorage中使用Environment预置环境变量名。
   ...
   ```
4. 状态装饰器装饰的变量，改变会引起UI的渲染更新。如果改变的变量仅用于消息传递，不用于UI更新，推荐使用[emitter](<../../../../../../../../harmonyos-references/基础功能/Basic Services Kit（基础服务）/ArkTS API/进程线程通信/@ohos.events.emitter (Emitter)/js-apis-emitter.md>)方式。具体示例可见[不建议借助@StorageLink的双向同步机制实现事件通知](arkts-appstorage.md#不建议借助storagelink的双向同步机制实现事件通知)。
...
AppStorage是单例，其所有API均为静态方法，使用方法类似于[LocalStorage](../LocalStorage：页面级UI状态存储/arkts-localstorage.md)中对应的非静态方法。
...
[PageTwo.ets](https://gitcode.com/HarmonyOS_Samples/guide-snippets/blob/HarmonyOS-feature-20260402/ArkUISample/AppStorage/entry/src/main/ets/pages/PageTwo.ets#L15-L79)

### AppStorage支持联合类型

在下面的示例中，变量linkA的类型为number | null，变量linkB的类型为number | undefined。[Text](<../../../../使用文本/文本显示 (TextSpan)/arkts-common-components-text-display.md>)组件初始化分别显示为null和undefined，点击切换为数字，再次点击切换回null和undefined。
...
[PageThree.ets](https://gitcode.com/HarmonyOS_Samples/guide-snippets/blob/HarmonyOS-feature-20260402/ArkUISample/AppStorage/entry/src/main/ets/pages/PageThree.ets#L15-L69)
...
[PageOne.ets](https://gitcode.com/HarmonyOS_Samples/guide-snippets/blob/HarmonyOS-feature-20260402/ArkUISample/AppStorage/entry/src/main/ets/pages/PageOne.ets#L15-L59)
...
[PageFour.ets](https://gitcode.com/HarmonyOS_Samples/guide-snippets/blob/HarmonyOS-feature-20260402/ArkUISample/AppStorage/entry/src/main/ets/pages/PageFour.ets#L15-L52)
...
[PageFive.ets](https://gitcode.com/HarmonyOS_Samples/guide-snippets/blob/HarmonyOS-feature-20260402/ArkUISample/AppStorage/entry/src/main/ets/pages/PageFive.ets#L15-L51)
...
[PageSix.ets](https://gitcode.com/HarmonyOS_Samples/guide-snippets/blob/HarmonyOS-feature-20260402/ArkUISample/AppStorage/entry/src/main/ets/pages/PageSix.ets#L15-L52)
...
[Index.ets](https://gitcode.com/HarmonyOS_Samples/guide-snippets/blob/HarmonyOS-feature-20260402/ArkUISample/AppStorage/entry/src/main/ets/pages/Index.ets#L16-L59)
...
[Page.ets](https://gitcode.com/HarmonyOS_Samples/guide-snippets/blob/HarmonyOS-feature-20260402/ArkUISample/AppStorage/entry/src/main/ets/pages/Page.ets#L16-L63)
...
示例代码中，TapImage中的点击事件会触发AppStorage中tapIndex对应属性的改变。由于@StorageLink是双向同步的，修改会同步回AppStorage中，因此所有绑定AppStorage的tapIndex自定义组件都能感知到tapIndex的变化。使用[@Watch](../../管理组件拥有的状态/@Watch装饰器：状态变量更改通知/arkts-watch.md)监听到tapIndex的变化后，修改状态变量tapColor，从而触发UI刷新（此处tapIndex未直接绑定在UI上，因此tapIndex的变化不会直接触发UI刷新）。
...
[ViewData.ets](https://gitcode.com/HarmonyOS_Samples/guide-snippets/blob/HarmonyOS-feature-20260402/ArkUISample/AppStorage/entry/src/main/ets/pages/ViewData.ets#L15-L97)

相比借助@StorageLink的双向同步机制实现事件通知，开发者可以使用[emit](<../../../../../../../../harmonyos-references/基础功能/Basic Services Kit（基础服务）/ArkTS API/进程线程通信/@ohos.events.emitter (Emitter)/js-apis-emitter.md#emitteremit>)订阅某个事件并接收事件回调的方式来减少开销，增强代码的可读性。
...
[PageEight.ets](https://gitcode.com/HarmonyOS_Samples/guide-snippets/blob/HarmonyOS-feature-20260402/ArkUISample/AppStorage/entry/src/main/ets/pages/PageEight.ets#L15-L124)
...
[Gallery.ets](https://gitcode.com/HarmonyOS_Samples/guide-snippets/blob/HarmonyOS-feature-20260402/ArkUISample/AppStorage/entry/src/main/ets/pages/Gallery.ets#L15-L84)
...
[PageTen.ets](https://gitcode.com/HarmonyOS_Samples/guide-snippets/blob/HarmonyOS-feature-20260402/ArkUISample/AppStorage/entry/src/main/ets/pages/PageTen.ets#L15-L47)
