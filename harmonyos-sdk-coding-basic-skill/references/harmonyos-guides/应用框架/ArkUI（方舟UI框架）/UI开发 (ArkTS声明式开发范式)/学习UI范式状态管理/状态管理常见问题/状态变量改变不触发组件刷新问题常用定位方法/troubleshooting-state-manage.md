...
scraped_at: 2026-06-11T14:29:43+08:00
doc_updated_at: 2026-04-28
content_hash: sha256:9f933d6d60e817701073b8450ff10f0678e8da654c09bfc3b246304546c0f110
---
...
这部分的原理在本篇文档中仅做简要说明，详情见[状态管理原理介绍](../../状态管理原理介绍/arkts-state-management-introduce.md)。基于上面的状态变量触发UI刷新的流程，在定位不刷新问题时，可以分为以下五步来定位。
...
* 使用DevEco Studio的ArkUI Inspector。具体使用方法见[状态管理Inspector调试能力](../../../../UI开发调试调优/UI调优/ui-inspector-profiler.md#状态管理inspector调试能力)。
* 使用[hidumper](../../../../../../系统/调测调优/调试命令/hidumper/hidumper/hidumper.md)工具。具体使用方法见[状态管理hidumper能力](../../../../UI开发调试调优/UI调优/ui-inspector-profiler.md#状态管理hidumper能力)。
...
在状态管理V1中，若开发者确认赋值前后值已发生变化却未能触发UI刷新，应检查当前赋值操作是否可被观察（从API version 23开始，开发者可以使用[canBeObserved](../../辅助接口/canBeObserved接口：判断对象是否可被观察/arkts-new-canbeobserved.md)接口来判断对象是否可被观察）。示例如下。

在下面的示例中，开发者对this.inner.value的赋值无法触发Text(`Child: inner value: ${this.inner.value}`)组件的刷新，在遇到这个问题时，应该从以下方面排查当前赋值操作是否是可被观察。

* [@Watch](../../状态管理（V1）/管理组件拥有的状态/@Watch装饰器：状态变量更改通知/arkts-watch.md)的监听函数是否执行。
* 如果状态变量为复杂类型且需要观察其属性的赋值变化，开发者还可以通过[getTarget](../../辅助接口/getTarget接口：获取状态管理框架代理前的原始对象/arkts-new-gettarget.md)来判断当前变量是否可观察。
* 使用DevEco Studio的Profiler工具观察此次赋值是否有状态变量变化的上报，具体使用方法见[状态管理profiler调优能力](../../../../UI开发调试调优/UI调优/ui-inspector-profiler.md#状态管理profiler调优能力)。
...
在上面的示例中，Inner没有被[@Observed](../../状态管理（V1）/管理组件拥有的状态/@Observed装饰器和@ObjectLink装饰器：嵌套类对象属性变化/arkts-observed-and-objectlink.md)装饰，所以其属性value的赋值无法被观察：

* @Watch('onChange')函数没有执行。
* 日志提示inner is not observed object。
* ArkUI State泳道没有状态变量变化的上报信息。

  ![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/56/v3/uEuZmEGDSeyVSLfFp7zztQ/zh-cn_image_0000002592377964.png?HW-CC-KV=V1&HW-CC-Date=20260611T062942Z&HW-CC-Expire=86400&HW-CC-Sign=2F9B3E53E63D874A0924C3BB3010BD42F7051959E889BC1D794321A2B1CADA73)

需要注意，并非所有的类对象都需要被@Observed装饰。[@State](../../状态管理（V1）/管理组件拥有的状态/@State装饰器：组件内状态/arkts-state.md)装饰器会默认对复杂对象包装第一层代理，而对嵌套对象，则需要在内层对象的类声明上增加@Observed装饰。
...
* ArkUI State泳道有状态变量变化的上报信息。
  ...
  ![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/83/v3/-e53jRVAS9ehTVd8HgK7aQ/zh-cn_image_0000002622857471.png?HW-CC-KV=V1&HW-CC-Date=20260611T062942Z&HW-CC-Expire=86400&HW-CC-Sign=CD6BDD12884AEE237B6C4AA091433F40E1D4B197657B7281C3F22AFB02D91581)
...
* 普通类：
  ...
  + 观察ArkUI State泳道是否有状态变量变化信息上报，具体使用方法见[状态管理profiler调优能力](../../../../UI开发调试调优/UI调优/ui-inspector-profiler.md#状态管理profiler调优能力)。
* 内置类型（Built-in Types）：
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/eb/v3/b8beHLkcSmOss8mgRSgIkA/zh-cn_image_0000002622697593.png?HW-CC-KV=V1&HW-CC-Date=20260611T062942Z&HW-CC-Expire=86400&HW-CC-Sign=C94CE13FF122AF73A32BDA71E10B8938BFC9CC0C34EECD71F163F0E611CF12A3)
...
* 同步对象(sync peer)：如@State和[@Link](../../状态管理（V1）/管理组件拥有的状态/@Link装饰器：父子双向同步/arkts-link.md)、[@Provide](../../状态管理（V1）/管理组件拥有的状态/@Provide装饰器和@Consume装饰器：与后代组件双向同步/arkts-provide-and-consume.md)和[@Consume](../../状态管理（V1）/管理组件拥有的状态/@Provide装饰器和@Consume装饰器：与后代组件双向同步/arkts-provide-and-consume.md)。开发者可以通过DevEco Studio的ArkUI Inspector来查看数据源和同步对象之间是否存在同步关系，具体见[状态管理Inspector调试能力](../../../../UI开发调试调优/UI调优/ui-inspector-profiler.md#状态管理inspector调试能力)。
* 依赖其所属组件的更新函数：如@State通知@Prop变化、@State通知@ObjectLink变化。开发者可以使用断点调试工具，或者[getHash接口](<../../../../../../../harmonyos-references/ArkTS（方舟编程语言）/ArkTS API/@ohos.util (util工具函数)/js-apis-util.md#utilgethash12>)来判断数据源和同步对象是否为同一个对象的引用(hashcode并不固定，以开发者自己打印的为准)。

**状态管理V2**

状态管理V2没有同步对象(sync peer)的概念。[@Local](../../状态管理（V2）/管理组件拥有的状态/@Local装饰器：组件内部状态/arkts-new-local.md)和[@Param](../../状态管理（V2）/管理组件拥有的状态/@Param：组件外部输入/arkts-new-param.md)的同步方式是依赖@Param组件所属组件的更新函数。

这类问题中，常见的场景是和[ForEach](../../../学习UI范式渲染控制/ForEach：循环渲染/arkts-rendering-control-foreach.md)和[LazyForEach](../../../学习UI范式渲染控制/LazyForEach：数据懒加载/arkts-rendering-control-lazyforeach.md)联用导致数据源和其同步对象断链。如以下示例。
...
这类问题常发生于，开发者在组件的同步回调里改变了状态变量，导致当前正在刷新的组件再次被加入到了待刷新的组件列表里，从而使状态管理框架忽略了这个组件的刷新。以[Image组件](../../../../../../../harmonyos-references/ArkUI（方舟UI框架）/ArkTS组件/图片与视频/Image/ts-basic-components-image.md)的[onComplete](../../../../../../../harmonyos-references/ArkUI（方舟UI框架）/ArkTS组件/图片与视频/Image/ts-basic-components-image.md#oncomplete)接口为例。
