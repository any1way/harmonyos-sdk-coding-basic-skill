...
scraped_at: 2026-06-01T11:05:40+08:00
doc_updated_at: 2026-05-26
content_hash: sha256:ef48619d3d5da39a10190169718c1de292d40b406526da0a04ee1fd245a2a1ec
---
手势冲突是指多个手势识别器在同一组件或重叠区域同时识别时产生竞争，导致识别结果不符合预期。常见冲突场景包括：

* 同一组件上的多手势（如按钮同时添加点击与长按手势）。
* 父子组件的同类型手势识别器。
* 系统默认手势与自定义手势（如[Scroll](../../../../../../../harmonyos-references/ArkUI（方舟UI框架）/ArkTS组件/滚动与滑动/Scroll/ts-container-scroll.md)滑动手势与子组件点击手势冲突）。
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/a0/v3/GzwVsBO_T2mB02pK83W06A/zh-cn_image_0000002613518413.png?HW-CC-KV=V1&HW-CC-Date=20260601T030540Z&HW-CC-Expire=86400&HW-CC-Sign=D72B5DF0B189E007CB159DD57BEA80C9ED7A914927A51FF29B54E2149BA51FD1)

自定义手势判定涉及以下接口。

| **接口** | **说明** |
| --- | --- |
| [onGestureJudgeBegin](../../../../../../../harmonyos-references/ArkUI（方舟UI框架）/ArkTS组件/手势处理/手势控制/自定义手势判定/ts-gesture-customize-judge.md#ongesturejudgebegin) | 用于手势拦截，是通用事件。在手势满足系统触发阈值场景下，回调给应用判断是否拦截手势。 |
| [onGestureRecognizerJudgeBegin](../../../../../../../harmonyos-references/ArkUI（方舟UI框架）/ArkTS组件/手势处理/手势控制/手势拦截增强/ts-gesture-blocking-enhancement.md#ongesturerecognizerjudgebegin) | 用于手势拦截、获取手势识别器和设置手势识别器开闭状态。是onGestureJudgeBegin接口的能力扩展，可以代替onGestureJudgeBegin接口。  获取手势识别器时，会获取一次交互中手势响应链上的所有手势识别器，以及当前即将触发成功的手势识别器，此时可以设置手势的激活状态。 |

以下示例中，Image和Stack两个组件位于同一区域。长按Stack组件的上半部分可触发挂载在Stack组件上的长按手势，长按Stack组件的下半部分则会响应Image组件的拖拽操作。

**图2** 示例图

![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/96/v3/NnmofbOPR6-7gyOlz5p9aw/zh-cn_image_0000002582958616.png?HW-CC-KV=V1&HW-CC-Date=20260601T030540Z&HW-CC-Expire=86400&HW-CC-Sign=3C606BD79DD54F7DDB1757A0C0789E2402072EF25640EEDBC8F76C64B89C729E)
...
1. Image组件设置拖拽。
   ...
   [CustomGestures.ets](https://gitcode.com/HarmonyOS_Samples/guide-snippets/blob/HarmonyOS-feature-20260402/ArkUISample/GestureConflict/entry/src/main/ets/Component/CustomGestures/CustomGestures.ets#L34-L46)
2. Stack组件设置手势。
   ...
   11. 该资源文件的value值为"LongPressGesture 长按上半区 灰色区域，灰色区域响应"
   12. */
   ...
   [CustomGestures.ets](https://gitcode.com/HarmonyOS_Samples/guide-snippets/blob/HarmonyOS-feature-20260402/ArkUISample/GestureConflict/entry/src/main/ets/Component/CustomGestures/CustomGestures.ets#L50-L69)
3. Stack组件设置拦截。
   ...
   [CustomGestures.ets](https://gitcode.com/HarmonyOS_Samples/guide-snippets/blob/HarmonyOS-feature-20260402/ArkUISample/GestureConflict/entry/src/main/ets/Component/CustomGestures/CustomGestures.ets#L71-L83)
4. 代码完整示例。
   ...
   30. try {
   31. this.promptAction.showToast({ message: $r('app.string.Allow_dragging_prompt') });
   32. } catch (error) {
   33. console.error('ShowToast failed!')
   34. }
   35. })
   36. .width('200vp').height('200vp')
   37. // Stack的上半区是绑定了长按手势的浮动区域。
   38. Stack() {
   39. }
   40. .width('200vp')
   41. .height('200vp')
   42. .hitTestBehavior(HitTestMode.Transparent)
   43. .gesture(GestureGroup(GestureMode.Parallel,
   44. LongPressGesture()
   45. .onAction(() => {
   46. /*
   47. 请将$r('app.string.Stop_dragging_prompt')替换为实际资源文件，在本示例中
   48. 该资源文件的value值为"LongPressGesture 长按上半区 灰色区域，灰色区域响应"
   49. */
   50. try {
   51. this.promptAction.showToast({ message: $r('app.string.Stop_dragging_prompt') });
   52. } catch (error) {
   53. console.error('ShowToast failed!')
   54. }
   55. })
   56. .tag('longPress')
   57. ))
   58. .onGestureJudgeBegin((gestureInfo: GestureInfo, event: BaseGestureEvent) => {
   59. // 如果是长按类型手势，判断点击的位置是否在上半区
   60. if (gestureInfo.type == GestureControl.GestureType.LONG_PRESS_GESTURE) {
   61. if (event.fingerList.length > 0 && event.fingerList[0].localY < 100) {
   62. return GestureJudgeResult.CONTINUE;
   63. } else {
   ...
   [CustomGestureDetermination.ets](https://gitcode.com/HarmonyOS_Samples/guide-snippets/blob/HarmonyOS-feature-20260402/ArkUISample/GestureConflict/entry/src/main/ets/Component/CustomGestures/CustomGestureDetermination.ets#L15-L82)

   ![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/eb/v3/G5TT3mBfSOaylS911TFoAA/zh-cn_image_0000002583118542.png?HW-CC-KV=V1&HW-CC-Date=20260601T030540Z&HW-CC-Expire=86400&HW-CC-Sign=8883E2BF753D0FACB7F6DFF8ABE39E16212458F107688E77CFB6A831AB514A79)
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/d7/v3/zrK2_K-DQFiOV7o_ER4f-w/zh-cn_image_0000002613638309.png?HW-CC-KV=V1&HW-CC-Date=20260601T030540Z&HW-CC-Expire=86400&HW-CC-Sign=2177C8804E32C177F08CB1BBD2EE5705BD9C9E728CCBDC913876A2E2478A5B3A)

手势并行动态控制的前提是手势识别成功，如果手势不成功则不会产生手势回调响应。

1. 业务手势作业流：指真正触发UI变化的业务手势，比如使页面滚动的PanGesture，触发点击的TapGesture等。
2. 监听手势作业流：指在监听手势运行的过程中，应根据上下文的业务状态变化动态控制手势识别器的开闭，例如判断组件嵌套滚动过程中是否已滑至边缘。这一监听事件可借助一个使用[并行手势绑定方式](../绑定手势方法/arkts-gesture-events-binding.md#parallelgesture并行手势绑定方法)的PanGesture实现，或者采用Touch事件来完成。
...
| **接口** | **说明** |
| --- | --- |
| [shouldBuiltInRecognizerParallelWith](../../../../../../../harmonyos-references/ArkUI（方舟UI框架）/ArkTS组件/手势处理/手势控制/手势拦截增强/ts-gesture-blocking-enhancement.md#shouldbuiltinrecognizerparallelwith) | 用于设置系统组件内置手势与其他手势并行。 |
| [onGestureRecognizerJudgeBegin](../../../../../../../harmonyos-references/ArkUI（方舟UI框架）/ArkTS组件/手势处理/手势控制/手势拦截增强/ts-gesture-blocking-enhancement.md#ongesturerecognizerjudgebegin) | 用于手势拦截，获取手势识别器，初始化手势识别器开闭状态。 |
| [parallelGesture](../绑定手势方法/arkts-gesture-events-binding.md#parallelgesture并行手势绑定方法) | 可使开发者定义的手势，与比他优先级高的手势并行。 |
...
1. 使用shouldBuiltInRecognizerParallelWith接口设置外部Scroll组件的PanGesture手势与内部Scroll组件的PanGesture手势并行。
   ...
   [GestureControl.ets](https://gitcode.com/HarmonyOS_Samples/guide-snippets/blob/HarmonyOS-feature-20260402/ArkUISample/GestureConflict/entry/src/main/ets/Component/GestureAndMotionControl/GestureControl.ets#L75-L87)
2. 使用onGestureRecognizerJudgeBegin接口获取到Scroll组件的PanGesture手势识别器，同时根据内外Scroll组件的边界条件，设置内外手势的开闭状态。
   ...
   [GestureControl.ets](https://gitcode.com/HarmonyOS_Samples/guide-snippets/blob/HarmonyOS-feature-20260402/ArkUISample/GestureConflict/entry/src/main/ets/Component/GestureAndMotionControl/GestureControl.ets#L90-L117)
3. 设置监听手势，监听Scroll组件状态，动态调整手势开闭状态，控制手势回调是否触发，从而控制Scroll是否滚动。
   ...
   [GestureControl.ets](https://gitcode.com/HarmonyOS_Samples/guide-snippets/blob/HarmonyOS-feature-20260402/ArkUISample/GestureConflict/entry/src/main/ets/Component/GestureAndMotionControl/GestureControl.ets#L119-L155)
4. 代码完整示例。
   ...
   [GestureAndMotionControl.ets](https://gitcode.com/HarmonyOS_Samples/guide-snippets/blob/HarmonyOS-feature-20260402/ArkUISample/GestureConflict/entry/src/main/ets/Component/GestureAndMotionControl/GestureAndMotionControl.ets#L16-L153)

## 阻止手势参与识别

手势识别基于[触摸测试](../../交互基础机制说明/arkts-interaction-basic-principles.md#触摸测试)的响应链结果进行，因此在用户按下时，通过控制响应链中手势识别器的参与状态，实现高效的动态干预手势处理。

从API version 20开始，可以结合[onTouchTestDone](../../../../../../../harmonyos-references/ArkUI（方舟UI框架）/ArkTS组件/手势处理/手势控制/手势拦截增强/ts-gesture-blocking-enhancement.md#ontouchtestdone20)接口来阻止手势参与识别。完成触摸测试后，系统通过该接口回调返回所有手势识别器对象。应用可根据类型、组件标识或关联组件信息筛选识别器，并通过调用[preventBegin](../../../../../../../harmonyos-references/ArkUI（方舟UI框架）/ArkTS组件/手势处理/手势公共接口/ts-gesture-common.md#preventbegin20)接口主动禁用特定识别器。
...
[PreventIdentification.ets](https://gitcode.com/HarmonyOS_Samples/guide-snippets/blob/HarmonyOS-feature-20260402/ArkUISample/GestureConflict/entry/src/main/ets/Component/PreventGestureRecognition/PreventIdentification.ets#L182-L192)

根据手势所归属的组件禁用：

组件需要提前通过通用属性[id](../../../../../../../harmonyos-references/ArkUI（方舟UI框架）/ArkTS组件/通用属性/基础属性/组件标识/ts-universal-attributes-component-id.md#id)配置组件标识。
...
[PreventIdentification.ets](https://gitcode.com/HarmonyOS_Samples/guide-snippets/blob/HarmonyOS-feature-20260402/ArkUISample/GestureConflict/entry/src/main/ets/Component/PreventGestureRecognition/PreventIdentification.ets#L194-L204)
...
[PreventIdentification.ets](https://gitcode.com/HarmonyOS_Samples/guide-snippets/blob/HarmonyOS-feature-20260402/ArkUISample/GestureConflict/entry/src/main/ets/Component/PreventGestureRecognition/PreventIdentification.ets#L206-L216)
...
在NDK中onTouchTestDone与preventBegin对应的接口分别为[OH\_ArkUI\_SetTouchTestDoneCallback](<../../../../../../../harmonyos-references/ArkUI（方舟UI框架）/C API/头文件/native_gesture.h/capi-native-gesture-h.md#oh_arkui_settouchtestdonecallback>)和[OH\_ArkUI\_PreventGestureRecognizerBegin](<../../../../../../../harmonyos-references/ArkUI（方舟UI框架）/C API/头文件/native_gesture.h/capi-native-gesture-h.md#oh_arkui_preventgesturerecognizerbegin>)，它们的使用方式及功能与ArkTS接口一致。
...
[PreventGestureRecognition.ets](https://gitcode.com/HarmonyOS_Samples/guide-snippets/blob/HarmonyOS-feature-20260402/ArkUISample/GestureConflict/entry/src/main/ets/Component/PreventGestureRecognition/PreventGestureRecognition.ets#L16-L256)

![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/9e/v3/Js5fqcabQSeuPDdaspu4ew/zh-cn_image_0000002613518415.gif?HW-CC-KV=V1&HW-CC-Date=20260601T030540Z&HW-CC-Expire=86400&HW-CC-Sign=58494EA8392C783411C7982415AC08D2792FC5B0F5D8EDBD0EFFCACDEF7933C1)
