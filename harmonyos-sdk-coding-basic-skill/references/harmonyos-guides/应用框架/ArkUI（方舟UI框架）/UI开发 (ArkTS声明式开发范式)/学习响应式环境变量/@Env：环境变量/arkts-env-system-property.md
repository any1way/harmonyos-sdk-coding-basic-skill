...
scraped_at: 2026-06-11T14:29:56+08:00
doc_updated_at: 2026-06-05
content_hash: sha256:ec323b0de55548b33c758b13faa77812bc4beb10479e079a3c3886c1b8a7e17c
---
在多设备开发的场景中，开发者可以使用[@Env](../../../../../../harmonyos-references/ArkUI（方舟UI框架）/ArkTS组件/响应式环境变量/@Env：环境变量/ts-env-system-property.md)装饰器监听系统环境变量的改变，并根据系统环境变量来进行相应的场景判断，以减少不同设备间的适配逻辑和重复开发。

说明

从API version 22开始，@Env支持在[@Component](../../学习UI范式基本语法/自定义组件/创建自定义组件/arkts-create-custom-components.md#component)和[@ComponentV2](../../学习UI范式基本语法/自定义组件/创建自定义组件/arkts-create-custom-components.md#componentv2)中使用。
...
* 根据入参读取相应的环境变量信息，详情见[@Env支持参数](arkts-env-system-property.md#env支持参数)。目前支持以下几种环境变量：
  + [SystemProperties.BREAK\_POINT](../../../../../../harmonyos-references/ArkUI（方舟UI框架）/ArkTS组件/响应式环境变量/@Env：环境变量/ts-env-system-property.md#systemproperties)，用于获取窗口不同宽高阈值下对应的断点值信息。
  + [SystemProperties.WINDOW\_SIZE23+](../../../../../../harmonyos-references/ArkUI（方舟UI框架）/ArkTS组件/响应式环境变量/@Env：环境变量/ts-env-system-property.md#systemproperties)，用于获取窗口的大小信息，单位为vp。
  + [SystemProperties.WINDOW\_SIZE\_PX23+](../../../../../../harmonyos-references/ArkUI（方舟UI框架）/ArkTS组件/响应式环境变量/@Env：环境变量/ts-env-system-property.md#systemproperties)，用于获取窗口的大小信息，单位为px。
  + [SystemProperties.WINDOW\_AVOID\_AREA23+](../../../../../../harmonyos-references/ArkUI（方舟UI框架）/ArkTS组件/响应式环境变量/@Env：环境变量/ts-env-system-property.md#systemproperties)，用于获取窗口的避让区域信息，单位为vp。
  + [SystemProperties.WINDOW\_AVOID\_AREA\_PX23+](../../../../../../harmonyos-references/ArkUI（方舟UI框架）/ArkTS组件/响应式环境变量/@Env：环境变量/ts-env-system-property.md#systemproperties)，用于获取窗口的避让区域信息，单位为px。
* 系统环境变量改变时，通知@Env装饰的变量更新，并触发@Env关联组件刷新，以实现界面内容的同步更新。
* @Env装饰的变量不允许开发者初始化。@Env会返回给开发者可观察的环境变量类（由[@ObservedV2](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/arkts-new-observedv2-and-trace)装饰，且其由属性[@Trace](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/arkts-new-observedv2-and-trace)装饰）的实例。开发者如果想监听环境变量的变化，可以使用[addMonitor](../../学习UI范式状态管理/辅助接口/addMonitorclearMonitor接口：动态添加取消监听/arkts-new-addmonitor-clearmonitor.md)，具体示例见[在@ComponentV2中使用@Env](arkts-env-system-property.md#在componentv2中使用env)。

## @Env支持参数

@Env支持的参数请参考[SystemProperties枚举类型说明](../../../../../../harmonyos-references/ArkUI（方舟UI框架）/ArkTS组件/响应式环境变量/@Env：环境变量/ts-env-system-property.md#systemproperties)。

## @Env和Environment能力对比

@Env和[Environment](../../学习UI范式状态管理/状态管理（V1）/管理应用拥有的状态/Environment：设备环境查询/arkts-environment.md)都是系统环境变量相关，但两者能力有较大的不同，具体能力对比见下表。

| 能力 | @Env | Environment |
| --- | --- | --- |
| 起始API version | 从API version 22开始支持。 | 从API version 7开始支持。 |
| 支持参数 | [SystemProperties的枚举值](../../../../../../harmonyos-references/ArkUI（方舟UI框架）/ArkTS组件/响应式环境变量/@Env：环境变量/ts-env-system-property.md#systemproperties) | 支持languageCode等参数，详情见[Environment内置参数](../../学习UI范式状态管理/状态管理（V1）/管理应用拥有的状态/Environment：设备环境查询/arkts-environment.md#environment内置参数)。 |
| 使用形式 | @Env为装饰器，可声明在@Component或@ComponentV2中，获取对应参数的环境变量信息。 | 通过[envProp](../../../../../../harmonyos-references/ArkUI（方舟UI框架）/ArkTS组件/状态管理与渲染控制/应用级变量的状态管理/ts-state-management.md#envprop10)等接口获取当前应用的环境变量，并存入[AppStorage](../../学习UI范式状态管理/状态管理（V1）/管理应用拥有的状态/AppStorage：应用全局的UI状态存储/arkts-appstorage.md)中，开发者可通过AppStorage的接口访问系统环境变量的值，具体例子见[从ui中访问environment参数](../../学习UI范式状态管理/状态管理（V1）/管理应用拥有的状态/Environment：设备环境查询/arkts-environment.md#从ui中访问environment参数)。 |
...
* @Env装饰的变量为只读属性，不允许开发者进行初始化或赋值操作，否则会有编译时报错。如果开发者绕过编译时检查，则会有运行时报错。
  ...
  18. }
  ```
* @Env当前支持[SystemProperties的枚举值](../../../../../../harmonyos-references/ArkUI（方舟UI框架）/ArkTS组件/响应式环境变量/@Env：环境变量/ts-env-system-property.md#systemproperties)。若使用不支持的参数，将触发编译时报错。
...
* @Env装饰的变量在@Component和@ComponentV2传递遵循以下规则：
  ...
  + @Env装饰的变量仅能用于初始化@Component中常规变量，否则会有编译时报错。需要注意，通过[BuilderNode](<../../../../../../harmonyos-references/ArkUI（方舟UI框架）/ArkTS API/UI界面/arkui/BuilderNode/js-apis-arkui-buildernode.md>)切换窗口，会导致@Env依据新的窗口更新环境变量实例。在切换窗口的场景中，不建议开发者使用@Env变量来初始化子组件的常规变量，否则会造成该常规变量无法被@Env通知触发其关联UI组件刷新。具体示例可见[通过BuilderNode切换窗口](arkts-env-system-property.md#通过buildernode切换窗口)。
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/34/v3/7qgdYhBqT9m_pGP-HHwOuQ/zh-cn_image_0000002622697627.png?HW-CC-KV=V1&HW-CC-Date=20260611T062955Z&HW-CC-Expire=86400&HW-CC-Sign=7F63FADC4E196A284ECEF78568A9D39C7AE2073AE0BD167341E74374FE8A26B6)

基于上面流程，下面的示例中以@Env使用SystemProperties.BREAK\_POINT为例，各个组件中的初始化如下图。

![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/31/v3/hA_aq-n9TTC7bUSorIOqNA/zh-cn_image_0000002592218066.png?HW-CC-KV=V1&HW-CC-Date=20260611T062955Z&HW-CC-Expire=86400&HW-CC-Sign=BA31050374F7429C1AA7E6B059D870C422E5E09998D46AAF37CD1C4304CC62DC)
...
* 在@ComponentV2中声明@Env，获取当前@ComponentV2组件创建时所在窗口尺寸的布局断点信息，并用[addMonitor](../../学习UI范式状态管理/辅助接口/addMonitorclearMonitor接口：动态添加取消监听/arkts-new-addmonitor-clearmonitor.md)监听this.breakpoint的属性的变化。
* 将@Env装饰的变量传递给CompV2中[@Param](../../学习UI范式状态管理/状态管理（V2）/管理组件拥有的状态/@Param：组件外部输入/arkts-new-param.md)装饰的变量和Comp中的常规变量。
...
@Env用于展示@Component/@ComponentV2所在[窗口](<../../../../../../harmonyos-references/ArkUI（方舟UI框架）/ArkTS API/窗口管理/@ohos.window (窗口)/模块描述/arkts-apis-window.md>)的环境变量信息。开发者通过BuilderNode切换@Component@ComponentV2所在的窗口实例时，@Env会根据新的窗口获取对应的环境变量信息，并触发关联的UI组件刷新。以SystemProperties.BREAK\_POINT为例。
...
下面的示例包含了创建子窗的流程，具体可参考[管理应用窗口（Stage模型）](../../../窗口管理/管理应用窗口（Stage模型）/application-window-stage.md)。
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/6c/v3/joJCr0QLQUWqiOzczpEmMw/zh-cn_image_0000002592378000.gif?HW-CC-KV=V1&HW-CC-Date=20260611T062955Z&HW-CC-Expire=86400&HW-CC-Sign=29FFD80CEDF2833D65A66AF485635CE0C02D0E761DFABFE5A109BE9D812A7A0D)
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/bd/v3/WLDwSD9RTRmJaD0k0QcsFg/zh-cn_image_0000002622857507.gif?HW-CC-KV=V1&HW-CC-Date=20260611T062955Z&HW-CC-Expire=86400&HW-CC-Sign=C276D9DAE674851A5867F723BDF0CC66136A907D726F055058E0BDB10B938962)
