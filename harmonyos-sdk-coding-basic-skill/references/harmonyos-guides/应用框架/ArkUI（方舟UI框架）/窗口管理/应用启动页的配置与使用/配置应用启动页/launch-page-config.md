...
scraped_at: 2026-06-11T14:34:46+08:00
doc_updated_at: 2026-06-05
content_hash: sha256:6160dc10858f1c2e37c3bba2a996309d7199239fda7b4d745037268778b58974
---
## 启动页的分类和实现方式

启动页分为简易启动页和增强启动页，开发者通过在module.json5配置文件中的[abilities标签](../../../../../基础入门/开发基础知识/应用配置文件（Stage模型）/module.json5配置文件/module-configuration-file.md#abilities标签)配置启动页资源，涉及的对应字段及含义如下表所示。
...
简易启动页是每个UIAbility都必须配置的能力，开发者通过配置module.json5文件中的[abilities标签](../../../../../基础入门/开发基础知识/应用配置文件（Stage模型）/module.json5配置文件/module-configuration-file.md#abilities标签)的startWindowIcon和startWindowBackground字段，实现简易启动页的配置。
...
[module.json5](https://gitcode.com/HarmonyOS_Samples/guide-snippets/blob/HarmonyOS-feature-20260402/ArkUISample/StartWindow/sampleForStartWindow/entry/src/main/module.json5#L37-L40)

此时，默认启动页呈现示意图如下：

**图1** 默认启动页示意图

![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/33/v3/AHM_OVH6RzeZM_wfqfdRCg/zh-cn_image_0000002592218606.png?HW-CC-KV=V1&HW-CC-Date=20260611T063445Z&HW-CC-Expire=86400&HW-CC-Sign=C3E1BFFDC558D05A863476C43BF1B6B2D93E4358D93DBE9C4D6F76F2352BDB60)
...
* 启动页中的图片资源支持的文件格式同[Image组件](../../../../../../harmonyos-references/ArkUI（方舟UI框架）/ArkTS组件/图片与视频/Image/ts-basic-components-image.md)，考虑到解码性能和显示效果，建议启动页中使用jpg或png格式的图片资源。
...
1. 在创建的UIAbility模板中，开发者可以新增startWindow字段指向二级配置json文件，以启用启动页增强配置。
   ...
   [module.json5](https://gitcode.com/HarmonyOS_Samples/guide-snippets/blob/HarmonyOS-feature-20260402/ArkUISample/StartWindow/EnhancedStartingWindow/entry/src/main/module.json5#L39-L19)
2. 配置二级json文件的具体字段。启动页资源主要在上、下两个区域进行展示，如果对应区域的资源未配置，则留空，其他区域的位置和尺寸不受影响。
   ...
   | 字段 | 类型 | 是否可缺省 | 含义 |
   | --- | --- | --- | --- |
   | startWindowType | string | 是 | 标识当前UIAbility组件是否隐藏启动页。  当前仅支持在2in1设备或平板设备的自由多窗模式下使用。  不同取值含义如下：  - "REQUIRED\_SHOW"：强制显示启动页。不受[Ability管理服务（即StartOptions中hideStartWindow字段）](<../../../../../../harmonyos-references/Ability Kit（程序框架服务）/ArkTS API/Stage模型能力的接口/@ohos.app.ability.StartOptions (startAbility的可选参数)/js-apis-app-ability-startoptions.md#startoptions>)的影响。  - "REQUIRED\_HIDE"：强制隐藏启动页。不受[Ability管理服务（即StartOptions中hideStartWindow字段）](<../../../../../../harmonyos-references/Ability Kit（程序框架服务）/ArkTS API/Stage模型能力的接口/@ohos.app.ability.StartOptions (startAbility的可选参数)/js-apis-app-ability-startoptions.md#startoptions>)的影响。  - "OPTIONAL\_SHOW"：可选显示，默认行为为显示启动页，如果[Ability管理服务（即StartOptions中hideStartWindow字段）](<../../../../../../harmonyos-references/Ability Kit（程序框架服务）/ArkTS API/Stage模型能力的接口/@ohos.app.ability.StartOptions (startAbility的可选参数)/js-apis-app-ability-startoptions.md#startoptions>)设置隐藏启动页，则隐藏启动页。  - 如未配置该字段，默认取值为"REQUIRED\_SHOW"，即强制显示启动页。  从API version 20开始支持该字段。 |
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/fa/v3/xOnpsOIjTDOqbFDdwZG-Rw/zh-cn_image_0000002592378540.png?HW-CC-KV=V1&HW-CC-Date=20260611T063445Z&HW-CC-Expire=86400&HW-CC-Sign=CCC9B494C826B9C7FEB764DDAB34E5BB7D5A0CD7539B89C5AFE28B60626F5747)
