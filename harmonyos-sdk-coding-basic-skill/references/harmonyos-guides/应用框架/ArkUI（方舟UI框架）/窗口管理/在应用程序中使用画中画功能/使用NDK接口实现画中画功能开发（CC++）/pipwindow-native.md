...
content_hash: sha256:79784e37afb3f3cfddd64089714faf8745b84b520ded07c1d79d6fbc437d3e45
---
...
Node-API模块注册，具体使用请参考[Native API在应用工程中的使用指导](../../../../../NDK开发/代码开发/使用Node-API实现ArkTSJS与CC++语言交互/Node-API开发规范/napi-guidelines.md)。
...
[napi\_init.cpp](https://gitcode.com/HarmonyOS_Samples/guide-snippets/blob/HarmonyOS-feature-20260402/ArkUIWindowPipSamples/WindowPip/entry/src/main/cpp/napi_init.cpp#L16-L350)
...
[Index.d.ts](https://gitcode.com/HarmonyOS_Samples/guide-snippets/blob/HarmonyOS-feature-20260402/ArkUIWindowPipSamples/WindowPip/entry/src/main/cpp/types/libentry/Index.d.ts#L16-L45)
...
[CMakeLists.txt](https://gitcode.com/HarmonyOS_Samples/guide-snippets/blob/HarmonyOS-feature-20260402/ArkUIWindowPipSamples/WindowPip/entry/src/main/cpp/CMakeLists.txt#L1-L16)
...
13. } catch (err) {
14. Logger.error('testTag', 'Failed to set color mode: %{public}s', JSON.stringify(err));
15. }
16. }

18. onDestroy(): void {
19. Logger.info('testTag', '%{public}s', 'Ability onDestroy');
20. }

22. onWindowStageCreate(windowStage: window.WindowStage): void {
23. // Main window is created, set main page for this ability
24. Logger.info('testTag', '%{public}s', 'Ability onWindowStageCreate');
25. let windowClass: window.Window | undefined = undefined;
26. let windowClassId: number = -1;

28. windowStage.getMainWindow().then((window) => {
29. if (window == null) {
30. Logger.error('Failed to obtaining the window. Cause: The data is empty');
31. return;
32. }
33. windowClass = window;
34. windowClass.setUIContent('pages/Index');
35. windowClassId = windowClass.getWindowProperties().id;
36. AppStorage.setOrCreate('windowId', windowClassId);
37. Logger.info('Succeeded in obtaining the window')

39. let ctx = window.getUIContext();
40. AppStorage.setOrCreate('UIContext', ctx);
41. // 通过主窗口UIContext创建typeNode节点
42. PipManager.getInstance().makeTypeNode(ctx);
43. }).catch((err: BusinessError) => {
44. Logger.error(`Failed to obtaining the window. Cause code: ${err.code}, message: ${err.message}`);
45. });
46. windowStage.loadContent('pages/Index', (err) => {
47. if (err.code) {
48. Logger.error('testTag', 'Failed to load the content. Cause: %{public}s', JSON.stringify(err));
49. return;
50. }
51. Logger.info('testTag', 'Succeeded in loading the content.');
52. });
53. }
54. onWindowStageDestroy(): void {
55. // Main window is destroyed, release UI related resources
56. Logger.info('testTag', '%{public}s', 'Ability onWindowStageDestroy');
57. }

59. onForeground(): void {
60. // Ability has brought to foreground
61. Logger.info('testTag', '%{public}s', 'Ability onForeground');
62. }

64. onBackground(): void {
65. // Ability has back to background
66. Logger.info('testTag', '%{public}s', 'Ability onBackground');
67. }
68. }
```

[EntryAbility.ets](https://gitcode.com/HarmonyOS_Samples/guide-snippets/blob/HarmonyOS-feature-20260402/ArkUIWindowPipSamples/WindowPip/entry/src/main/ets/entryability/EntryAbility.ets#L17-L101)

示例中的视频播放需要使用AVPlayer，具体实现可参考[视频播放](<../../../../../媒体/Media Kit（媒体服务）/媒体开发指导(ArkTS)/播放/使用AVPlayer播放视频(ArkTS)/video-playback.md>)。
...
111. Logger.info('avPlayerFdSrc');
112. try {
113. // 创建avPlayer实例对象
114. this.avPlayer = await media.createAVPlayer();
115. } catch (err) {
116. let error = err as BusinessError;
117. Logger.error(`createAVPlayer failed, code: ${error.code}, msg: ${error.message}`);
118. return;
119. }

121. // 创建状态机变化回调函数
122. this.setAVPlayerCallback();
123. // 通过UIAbilityContext的resourceManager成员的getRawFd接口获取媒体资源播放地址
124. // 返回类型为{fd,offset,length},fd为HAP包fd地址，offset为媒体资源偏移量，length为播放长度

126. try {
127. let context = AppStorage.get('UIAbilityContext') as common.UIAbilityContext;
128. let fileDescriptor = await context.resourceManager.getRawFd('640x360.mp4');
129. Logger.info('getRawFd');
130. // 为fdSrc赋值触发initialized状态机上报
131. this.avPlayer.fdSrc = fileDescriptor;
132. } catch (err) {
133. let error = err as BusinessError;
134. Logger.error(`getRawFd failed, code=${error.code}, msg=${error.message}`);
135. }
136. }

138. async playNext() {
139. if (this.avPlayer === null) {
140. return;
141. }
142. this.jumpNext = true;
143. this.avPlayer?.stop()
144. .then(() => {
145. Logger.info(`Succeeded to stop avPlayer`);
146. }).catch((err: BusinessError) => {
147. Logger.error(`Failed to stop avPlayer, Cause:${err.code}, message:${err.message}`);
148. });
149. }

151. play() {
152. if (this.state_ === 'paused') {
153. this.avPlayer?.play()
154. .then(() => {
155. Logger.info(`Succeeded to play avPlayer`);
156. }).catch((err: BusinessError) => {
157. Logger.error(`Failed to play avPlayer, Cause:${err.code}, message:${err.message}`);
158. });
159. }
160. }

162. pause() {
163. if (this.state_ === 'playing') {
164. this.avPlayer?.pause()
165. .then(() => {
166. Logger.info(`Succeeded to pause avPlayer`);
167. }).catch((err: BusinessError) => {
168. Logger.error(`Failed to pause avPlayer, Cause:${err.code}, message:${err.message}`);
169. });
170. }
171. }

173. stopAvPlayer() {
174. Logger.info('stopAvPlayer>>>')
175. if (!this.avPlayer) {
176. return;
177. }
178. this.avPlayer.stop()
179. .then(() => {
180. Logger.info(`Succeeded to stop avPlayer`);
181. }).catch((err: BusinessError) => {
182. Logger.error(`Failed to stop avPlayer, Cause:${err.code}, message:${err.message}`);
183. });
184. Logger.info('stopping>>>');
185. this.avPlayer.reset()
186. .then(() => {
187. Logger.info(`Succeeded to reset avPlayer`);
188. }).catch((err: BusinessError) => {
189. Logger.error(`Failed to reset avPlayer, Cause:${err.code}, message:${err.message}`);
190. });
191. }
192. }
```

[NDKAVPlayer.ets](https://gitcode.com/HarmonyOS_Samples/guide-snippets/blob/HarmonyOS-feature-20260402/ArkUIWindowPipSamples/WindowPip/entry/src/main/ets/model/NDKAVPlayer.ets#L16-L209)
...
[Index.ets](https://gitcode.com/HarmonyOS_Samples/guide-snippets/blob/HarmonyOS-feature-20260402/ArkUIWindowPipSamples/WindowPip/entry/src/main/ets/pages/Index.ets#L18-L113)
...
[NDKImplementPage.ets](https://gitcode.com/HarmonyOS_Samples/guide-snippets/blob/HarmonyOS-feature-20260402/ArkUIWindowPipSamples/WindowPip/entry/src/main/ets/pages/NDKImplementPage.ets#L16-L207)

以上示例代码对应的示意图如下所示：

![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/a5/v3/ZoECIUH-TdejHV_iC19-ug/zh-cn_image_0000002622698147.gif?HW-CC-KV=V1&HW-CC-Date=20260611T063432Z&HW-CC-Expire=86400&HW-CC-Sign=F0EDF51C29DF2AA7E8578FC92BE120340D01A8199F2C6B469970606686D488BF)
