...
scraped_at: 2026-06-01T11:07:08+08:00
doc_updated_at: 2026-05-26
content_hash: sha256:6d7f7df4dbbd370f82f4a514ae7feaee0042e2e1c6fb9e5e1acc851433e1ba51
---
ArkUI在Native侧提供的能力作为ArkTS的子集，部分能力不会在Native侧提供，如声明式UI语法，自定义struct组件，UI高级组件。

针对需要使用ArkTS侧独立能力的场景，ArkUI开发框架提供了Native侧嵌入ArkTS组件的能力，该能力依赖[ComponentContent](<../../../../../harmonyos-references/ArkUI（方舟UI框架）/ArkTS API/UI界面/arkui/ComponentContent/js-apis-arkui-componentcontent.md>)机制，通过ComponentContent完成对ArkTS组件的封装，然后将封装对象传递到Native侧，通过Native侧的[OH\_ArkUI\_GetNodeHandleFromNapiValue](<../../../../../harmonyos-references/ArkUI（方舟UI框架）/C API/头文件/native_node_napi.h/capi-native-node-napi-h.md#oh_arkui_getnodehandlefromnapivalue>)接口转化为ArkUI\_NodeHandle对象用于Native侧组件挂载使用。

说明

* 通过OH\_ArkUI\_GetNodeHandleFromNapiValue接口获得的ArkUI\_NodeHandle对象只能作为子组件参数使用，如[addChild](<../../../../../harmonyos-references/ArkUI（方舟UI框架）/C API/结构体/ArkUI_NativeNodeAPI_1/capi-arkui-nativemodule-arkui-nativenodeapi-1.md#addchild>)接口的第二个参数，将该对象使用在其他场景下，如[setAttribute](<../../../../../harmonyos-references/ArkUI（方舟UI框架）/C API/结构体/ArkUI_NativeNodeAPI_1/capi-arkui-nativemodule-arkui-nativenodeapi-1.md#setattribute>)设置属性将不生效并返回错误码。
* 针对Native侧修改ArkTS组件的场景，需要在Native侧通过Node-API方式构建ArkTS侧的更新数据，再通过ComponentContent的[update](<../../../../../harmonyos-references/ArkUI（方舟UI框架）/ArkTS API/UI界面/arkui/ComponentContent/js-apis-arkui-componentcontent.md#update>)接口更新。
* [构建自定义组件](../构建自定义组件/ndk-build-custom-components.md)时，相关函数如measureNode等无法对ArkTS模块内部的组件进行调用。

以下示例代码在[接入ArkTS页面](../接入ArkTS页面/ndk-access-the-arkts-page.md)章节基础上引入ArkTS的Refresh组件。

**图1** Refresh组件挂载文本列表

![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/79/v3/9g4gSDIoR1qVG78YonBhJQ/zh-cn_image_0000002583118652.gif?HW-CC-KV=V1&HW-CC-Date=20260601T030708Z&HW-CC-Expire=86400&HW-CC-Sign=19878E03B74B25D8094A51288AAD7E58E22694816BA1193756F25F0409840BF9)
...
1. 注册ArkTS组件创建函数给Native侧，以便Native侧调用，创建函数使用ComponentContent能力进行封装。
   ...
   [MixedModule.ets](https://gitcode.com/HarmonyOS_Samples/guide-snippets/blob/HarmonyOS-feature-20260402/ArkUISample/NativeType/NdkEmbedArktsComponents/entry/src/main/ets/pages/MixedModule.ets#L15-L144)
2. 将创建和更新函数注册给Native侧。
   ...
   [Index.ets](https://gitcode.com/HarmonyOS_Samples/guide-snippets/blob/HarmonyOS-feature-20260402/ArkUISample/NativeType/NdkEmbedArktsComponents/entry/src/main/ets/pages/Index.ets#L15-L61)
   ...
   [NapiInit.cpp](https://gitcode.com/HarmonyOS_Samples/guide-snippets/blob/HarmonyOS-feature-20260402/ArkUISample/NativeType/NdkEmbedArktsComponents/entry/src/main/cpp/NapiInit.cpp#L15-L48)
3. Native侧通过Node-API保存创建和更新函数，用于后续调用。
   ...
   [ArkUIMixedRefreshTemplate.h](https://gitcode.com/HarmonyOS_Samples/guide-snippets/blob/HarmonyOS-feature-20260402/ArkUISample/NativeType/NdkEmbedArktsComponents/entry/src/main/cpp/ArkUIMixedRefreshTemplate.h#L15-L39)
   ...
   [ArkUIMixedRefreshTemplate.cpp](https://gitcode.com/HarmonyOS_Samples/guide-snippets/blob/HarmonyOS-feature-20260402/ArkUISample/NativeType/NdkEmbedArktsComponents/entry/src/main/cpp/ArkUIMixedRefreshTemplate.cpp#L15-L45)
...
4. 抽象混合模式下组件的基类，用于通用逻辑管理。
   ...
   [ArkUIMixedNode.h](https://gitcode.com/HarmonyOS_Samples/guide-snippets/blob/HarmonyOS-feature-20260402/ArkUISample/NativeType/NdkEmbedArktsComponents/entry/src/main/cpp/ArkUIMixedNode.h#L15-L47)
5. 实现Refresh组件的混合模式封装对象。
   ...
   [ArkUIMixedRefresh.h](https://gitcode.com/HarmonyOS_Samples/guide-snippets/blob/HarmonyOS-feature-20260402/ArkUISample/NativeType/NdkEmbedArktsComponents/entry/src/main/cpp/ArkUIMixedRefresh.h#L15-L112)
   ...
   [ArkUIMixedRefresh.cpp](https://gitcode.com/HarmonyOS_Samples/guide-snippets/blob/HarmonyOS-feature-20260402/ArkUISample/NativeType/NdkEmbedArktsComponents/entry/src/main/cpp/ArkUIMixedRefresh.cpp#L15-L223)
6. 定时器模块相关简单实现。
   ...
   [UITimer.h](https://gitcode.com/HarmonyOS_Samples/guide-snippets/blob/HarmonyOS-feature-20260402/ArkUISample/NativeType/NdkEmbedArktsComponents/entry/src/main/cpp/UITimer.h#L15-L90)
7. 使用[接入ArkTS页面](../接入ArkTS页面/ndk-access-the-arkts-page.md)章节的页面结构，将Refresh组件作为文本列表的父组件。
   ...
   [MixedRefreshExample.h](https://gitcode.com/HarmonyOS_Samples/guide-snippets/blob/HarmonyOS-feature-20260402/ArkUISample/NativeType/NdkEmbedArktsComponents/entry/src/main/cpp/MixedRefreshExample.h#L15-L62)
   ...
   [NativeEntry.cpp](https://gitcode.com/HarmonyOS_Samples/guide-snippets/blob/HarmonyOS-feature-20260402/ArkUISample/NativeType/NdkEmbedArktsComponents/entry/src/main/cpp/NativeEntry.cpp#L15-L59)
8. 在Native侧提供Node-API的桥接方法，实现ArkTS侧的NativeNode模块接口。
   ...
   [Index.d.ts](https://gitcode.com/HarmonyOS_Samples/guide-snippets/blob/HarmonyOS-feature-20260402/ArkUISample/NativeType/NdkEmbedArktsComponents/entry/src/main/cpp/types/libentry/Index.d.ts#L15-L21)
