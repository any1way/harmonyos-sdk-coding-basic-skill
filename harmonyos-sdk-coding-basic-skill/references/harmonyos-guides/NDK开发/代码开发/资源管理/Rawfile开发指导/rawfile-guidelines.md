...
scraped_at: 2026-06-11T15:21:35+08:00
doc_updated_at: 2026-05-26
content_hash: sha256:095152a04d5d4d35973859e147b5bf14d7d2c9301d2c64bcf04fd0e9f8db46d5
---
## 场景介绍

开发者可以通过本指导了解在HarmonyOS应用中，如何使用Native Rawfile接口操作Rawfile目录和文件。功能包括文件列表遍历、文件打开、搜索、读取和关闭Rawfile。

64后缀相关接口属于新增接口，新接口支持打开更大的rawfile文件(超过2G建议使用)，具体请参考：[Rawfile接口介绍](<../../../../../harmonyos-references/Localization Kit（本地化开发服务）/C API/模块/rawfile/capi-rawfile.md>)。64相关的开发步骤和非64一致，将非64接口替换为64接口即可，例如：OH\_ResourceManager\_OpenRawFile替换为OH\_ResourceManager\_OpenRawFile64。
...
详细的接口说明请参考[rawfile](<../../../../../harmonyos-references/Localization Kit（本地化开发服务）/C API/模块/rawfile/capi-rawfile.md>)。
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/f9/v3/Rmkm4WVWRIOk4uZuas-fYw/zh-cn_image_0000002622859273.png?HW-CC-KV=V1&HW-CC-Date=20260611T072134Z&HW-CC-Expire=86400&HW-CC-Sign=9EEEEC8985A0EA6C4415D5E9D6779995BBB0BF41F996E35FAA34F74FC86B3F53)
...
2. 打开src/main/cpp/types/libentry/index.d.ts文件，在此文件中声明ArkTS侧接口getFileList、getRawFileContent、getRawFileDescriptor、isRawDir。
   ...
   [Index.d.ts](https://gitcode.com/HarmonyOS_Samples/guide-snippets/blob/HarmonyOS-feature-20260402/ResourceManagement/RawFile/entry/src/main/cpp/types/libentry/Index.d.ts#L16-L22)
...
1. 打开src/main/cpp/hello.cpp文件，在Init方法中添加ArkTS接口与C++接口的映射。ArkTS侧接口getFileList、getRawFileContent、getRawFileDescriptor、isRawDir，映射C++接口分别为GetFileList、GetRawFileContent、GetRawFileDescriptor、IsRawDir。
   ...
   [hello.cpp](https://gitcode.com/HarmonyOS_Samples/guide-snippets/blob/HarmonyOS-feature-20260402/ResourceManagement/RawFile/entry/src/main/cpp/hello.cpp#L255-L270)
2. 在src/main/cpp/目录下创建hello.h文件，在hello.h文件中增加对应的四个方法，如下所示：
   ...
   [hello.h](https://gitcode.com/HarmonyOS_Samples/guide-snippets/blob/HarmonyOS-feature-20260402/ResourceManagement/RawFile/entry/src/main/cpp/hello.h#L16-L33)
3. 在hello.cpp文件中实现上述四个方法。通过env和info获取Js的资源管理对象，并转换为Native的资源管理对象，即可调用Native资源管理对象的接口，示例代码如下：
   ...
   [hello.cpp](https://gitcode.com/HarmonyOS_Samples/guide-snippets/blob/HarmonyOS-feature-20260402/ResourceManagement/RawFile/entry/src/main/cpp/hello.cpp#L16-L22)
   ...
   [hello.cpp](https://gitcode.com/HarmonyOS_Samples/guide-snippets/blob/HarmonyOS-feature-20260402/ResourceManagement/RawFile/entry/src/main/cpp/hello.cpp#L24-L27)
   ...
   [hello.cpp](https://gitcode.com/HarmonyOS_Samples/guide-snippets/blob/HarmonyOS-feature-20260402/ResourceManagement/RawFile/entry/src/main/cpp/hello.cpp#L29-L75)
   ...
   [hello.cpp](https://gitcode.com/HarmonyOS_Samples/guide-snippets/blob/HarmonyOS-feature-20260402/ResourceManagement/RawFile/entry/src/main/cpp/hello.cpp#L77-L136)
   ...
   [hello.cpp](https://gitcode.com/HarmonyOS_Samples/guide-snippets/blob/HarmonyOS-feature-20260402/ResourceManagement/RawFile/entry/src/main/cpp/hello.cpp#L138-L213)
   ...
   [hello.cpp](https://gitcode.com/HarmonyOS_Samples/guide-snippets/blob/HarmonyOS-feature-20260402/ResourceManagement/RawFile/entry/src/main/cpp/hello.cpp#L215-L253)
...
2. 资源获取包括获取本应用包资源、应用内跨包资源、跨应用包资源。
   ...
   Context的更多使用信息请参考[应用上下文Context](<../../../../应用框架/Ability Kit（程序框架服务）/Stage模型开发指导/Stage模型应用组件/应用上下文Context/application-context-stage.md>)。
3. 调用src/main/cpp/types/libentry/index.d.ts中声明的接口，如getFileList，传入js的资源管理对象以及rawfile文件夹的相对路径。
   ...
   [Index.ets](https://gitcode.com/HarmonyOS_Samples/guide-snippets/blob/HarmonyOS-feature-20260402/ResourceManagement/RawFile/entry/src/main/ets/pages/Index.ets#L15-L79)
