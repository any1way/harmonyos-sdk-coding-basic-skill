...
scraped_at: 2026-06-11T15:19:58+08:00
doc_updated_at: 2026-05-28
content_hash: sha256:506a30f71fbc43668b4ddcf8231db817dc55821d4e673aaf65ce1167cf88f80d
---
## 获取JS传入参数及其数量
...
本规则旨在强调napi\_value生命周期情况，若只想往JS线程抛任务，**不推荐**使用uv\_queue\_work方法。如有抛任务的需要，请使用[napi\_threadsafe\_function系列](../Node-API典型使用场景/使用Node-API接口进行线程安全开发/use-napi-thread-safety.md)接口。
...
8. static napi_value InitOther(napi_env env, napi_value exports)
9. {
10. // ...
11. return exports;
12. }
...
15. static napi_module nativeModule = {
16. .nm_version = 1,
17. .nm_flags = 0,
18. .nm_filename = nullptr,
19. // 没有在nm_register_func对应的函数加上static
20. .nm_register_func = Init,
21. // 模块实现中.nm_modname字段没有与模块名完全匹配，会导致多线程场景模块加载失败
22. .nm_modname = "entry",
23. .nm_priv = nullptr,
24. .reserved = { 0 },
25. };

27. // 同一个so中定义了第二个不同的napi_module
28. static napi_module otherModule = {
29. .nm_version = 1,
30. .nm_flags = 0,
31. .nm_filename = nullptr,
32. .nm_register_func = InitOther,
33. .nm_modname = "other",
34. .nm_priv = nullptr,
35. .reserved = { 0 },
36. };

38. // 模块注册的入口函数名为RegisterModule，容易与其他模块重复
39. extern "C" __attribute__((constructor)) void RegisterModule()
40. {
41. napi_module_register(&nativeModule);
42. // 同一个so中注册了两个不同的模块，框架加载时可能匹配到错误的模块，
43. // 无论通过何种方式（如多个构造函数、全局对象构造等）多次调用napi_module_register，
44. // 框架加载so时都可能匹配到错误的模块，导致非预期行为
45. napi_module_register(&otherModule);
46. }
```

图一

![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/58/v3/f5K4mwKnSYm69i3nMTe93Q/zh-cn_image_0000002622699375.png?HW-CC-KV=V1&HW-CC-Date=20260611T071957Z&HW-CC-Expire=86400&HW-CC-Sign=E9E086A11A9461EC572EC2B7743FFBC89D0BB8B26BDC85D2462EF20A19B0FB9E)

图二

![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/1e/v3/64K1fdK4SJSF0-g4GCBhjg/zh-cn_image_0000002592219814.png?HW-CC-KV=V1&HW-CC-Date=20260611T071957Z&HW-CC-Expire=86400&HW-CC-Sign=E3209C7159116B341D9D21DFDF83CB751CE05A826FD386F18DF78A1EE4CDC559)
...
**【规则】** napi\_create\_external系列接口创建出来的JS对象仅允许在当前线程传递和使用，跨线程传递（如使用worker的post\_message）将会导致应用crash。若需跨线程传递绑定有Native对象的JS对象，请使用napi\_coerce\_to\_native\_binding\_object接口绑定JS对象和Native对象。具体API说明详见[API参考](../Node-API使用指导/使用Node-API接口进行object相关开发/use-napi-about-object.md#napi_create_external)。
