...
scraped_at: 2026-06-11T15:20:52+08:00
doc_updated_at: 2026-06-09
content_hash: sha256:4a52f5027016a64da1dc498854e7730793bab2ebbef04f474f52a5df0883c4e3
---
## JSVM-API 的数据类型
...
12. /** background deserialize code cache result.
13. * @since 24
14. */
15. JSVM_COMPILE_BACKGROUND_DESERIALIZE_RESULT,
16. /** whether the code cache is rejected.
17. * @since 24
18. */
19. JSVM_COMPILE_CODE_CACHE_REJECTED,
20. } JSVM_CompileOptionId;
```
...
JSVM 提供了多种配置选项，允许开发者在执行 OH\_JSVM\_Init 时灵活配置其行为。可以通过 OH\_JSVM\_GetVMInfo 接口获取当前 JSVM 版本所对应的 V8 引擎版本。JSVM 中可支持的选项范围与对应的 V8 引擎版本可支持的选项范围保持一致。OH\_JSVM\_GetVMInfo 接口的使用参考[使用JSVM-API接口获取JSVM API的版本号](<../JSVM-API使用指导/使用JSVM-API接口获取JSVM API的版本号/use-jsvm-about-version.md>)。
...
35. auto compileMode = data ? JSVM_COMPILE_MODE_CONSUME_CODE_CACHE :  JSVM_COMPILE_MODE_DEFAULT;
...
OH\_JSVM\_CreateCodeCache接口用法可参考[使用code cache加速编译](<../JSVM-API典型使用场景指导/JSVM-API调优&高性能使用示例/使用code cache加速编译/use-jsvm-about-code-cache.md>)。
...
详见[使用 JSVM-API WebAssembly 接口](../JSVM-API使用指导/使用JSVM-API接口进行WebAssembly模块相关开发/use-jsvm-about-wasm.md)。
...
具体示例参考[使用JSVM-API接口进行class相关开发](../JSVM-API使用指导/使用JSVM-API接口进行class相关开发/use-jsvm-about-class.md)。
...
[创建和使用虚拟机的启动快照。](../JSVM-API使用指导/使用JSVM-API接口进行虚拟机快照相关开发/use-jsvm-create-snapshot.md)
...
[使用JSVM-API接口进行任务队列相关开发](../JSVM-API使用指导/使用JSVM-API接口进行任务队列相关开发/use-jsvm-execute_tasks.md)
