...
scraped_at: 2026-06-01T15:16:07+08:00
doc_updated_at: 2026-04-29
content_hash: sha256:e1e39969e4aa27cf3fccc8e41f677fa7a16d1b48979dc50a5caf43d4e1013bad
---
## 简介
...
仅需替换[使用JSVM-API实现JS与C/C++语言交互开发流程](../../使用JSVM-API实现JS与CC++语言交互开发流程/use-jsvm-process.md)示例代码中的“TestJSVM()”函数即可运行。
...
17. CHECK(OH_JSVM_OpenVMScope(vm, &vmScope));
18. CHECK(OH_JSVM_CreateEnv(vm, sizeof(descriptor) / sizeof(descriptor[0]), descriptor, &env));
19. // 打开JSVM_SCOPE_CHECK校验选项
20. CHECK(OH_JSVM_SetDebugOption(env, JSVM_SCOPE_CHECK, true));
...
45. // 关闭JSVM_SCOPE_CHECK校验选项
46. CHECK(OH_JSVM_SetDebugOption(env, JSVM_SCOPE_CHECK, false));
47. CHECK(OH_JSVM_DestroyEnv(env));
48. CHECK(OH_JSVM_CloseVMScope(vm, vmScope));
...
40. // 关闭JSVM_SCOPE_CHECK校验选项
41. CHECK(OH_JSVM_SetDebugOption(env, JSVM_SCOPE_CHECK, false));
42. CHECK(OH_JSVM_DestroyEnv(env));
43. CHECK(OH_JSVM_CloseVMScope(vm, vmScope));
