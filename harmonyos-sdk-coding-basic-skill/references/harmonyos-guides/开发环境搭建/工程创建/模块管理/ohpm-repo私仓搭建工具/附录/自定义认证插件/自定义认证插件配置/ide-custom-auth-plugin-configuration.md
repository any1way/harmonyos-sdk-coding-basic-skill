...
scraped_at: 2026-06-01T15:17:54+08:00
doc_updated_at: 2026-03-17
content_hash: sha256:f9100878534963f0f2ede305718fe0f5478ef6080473af7f76a90d4790d09ea9
---
...
2. 进入ohpm-repo解压根目录，把模板文件中[tsconfig.json 文件](../模板文件/ide-custom-auth-plugin-template.md#section14188258114612)移动到ohpm-repo解压根目录。
3. 建议将模板文件中[CustomAuth.ts](../模板文件/ide-custom-auth-plugin-template.md#section14072085013)文件存入ohpm-repo解压根目录的plugins文件夹内。
...
2. 编译插件文件。
   ...
   * 命令成功执行后会在ohpm-repo解压根目录的plugins/outDir文件夹中生成编译后的文件CustomAuth.js。
     ...
     如果CustomAuth.ts没有存放在ohpm-repo解压根目录的plugins内，请先修改[tsconfig.json 文件](../模板文件/ide-custom-auth-plugin-template.md#section14188258114612)中include和outDir参数，前者指定待编译插件代码的存储目录，后者指定编译完成后文件的输出位置，然后再在ohpm-repo解压根目录执行编译命令tsc。
...
5. ohpm-repo服务部署。在完成上述操作之后，按照ohpm-repo[部署指导](../../../部署指导/ide-ohpm-deploy-guide.md)，完成服务部署。
