...
scraped_at: 2026-06-01T15:17:32+08:00
doc_updated_at: 2026-04-20
content_hash: sha256:a684890f14dab21dab6c8b43c36ffd6a8fa0058bedd04c39d9487441f4798392
---
批量下载ohpm-repo或OpenHarmony三方库中心仓的包文件。

## 前提条件

已成功执行[export\_pkginfo 命令](<../ohpm-repo export_pkginfo/ide-ohpm-repo-export-pkginfo.md>)，生成pkgInfo\_xxx.json文件。
...
说明：执行[export\_pkginfo 命令](<../ohpm-repo export_pkginfo/ide-ohpm-repo-export-pkginfo.md>)生成的pkgInfo\_xxx.json文件中记录着ohpm-repo或OpenHarmony三方库中心仓中所有已上架的包，若仅需要批量下载部分包文件，可以手动修改pkgInfo\_xxx.json文件，命令只会批量下载pkgInfo\_xxx.json文件中指定的包，包如果有其他依赖，所依赖的包也会一并下载。
...
必须在batch\_download命令后面配置<pkg\_list>参数，指定执行[export\_pkginfo 命令](<../ohpm-repo export_pkginfo/ide-ohpm-repo-export-pkginfo.md>)导出的json文件。
...
3、ohpm-repo从5.3.0开始支持多仓库配置，当从OpenHarmony三方库中心仓下载包，生成的包zip文件，目录名为ohpm，在后续执行[batch\_publish](<../ohpm-repo batch_publish/ide-ohpm-repo-batch-publish.md>)命令时，默认导入ohpm-repo仓库名为ohpm的仓库中。
