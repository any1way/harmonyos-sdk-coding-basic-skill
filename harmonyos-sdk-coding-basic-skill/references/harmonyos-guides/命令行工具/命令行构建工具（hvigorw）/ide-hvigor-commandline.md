...
scraped_at: 2026-06-01T15:27:17+08:00
doc_updated_at: 2026-04-20
content_hash: sha256:048bede89ca295356f103ac27f1741110fa5d329167699670cf035346a2b3352
---
hvigorw作为Hvigor的wrapper包装工具，支持自动安装Hvigor构建工具和相关插件依赖，以及执行Hvigor构建命令。

执行命令前，需要先配置JDK，配置Node.js、hvigor等环境变量，具体请参考[搭建流水线](../搭建流水线/ide-command-line-building-app.md)。
...
常见的任务和参数如下，更多任务请参考[任务详细说明](../../构建应用/概述/构建任务说明/ide-hvigor-task-process.md#section196919100414)。
...
| -p debuggable=true/false | 该配置会覆盖构建模式中对应的buildOption中的debuggable配置。  关于debuggable的合并优先级，请参考[合并编译选项规则](../../构建应用/定制构建/灵活定制编译选项/能力说明/ide-hvigor-compilation-options-customizing-guide.md#section1727865610255)。 |
...
| -p parameterFile=param.json/json5 | 设置oh-package.json5文件的参数配置文件，其中"param"可自行修改为对应配置文件名称。详细使用请参考[parameterFile](../三方依赖管理工具（ohpm）/oh-package.json5/ide-oh-package-json5.md#section122411462820)。 |
| -p buildVersion=1 | 设置构建版本号为1，详细使用请参考[app.json5的buildVersion](../../基础入门/开发基础知识/应用配置文件（Stage模型）/app.json5配置文件/app-configuration-file.md#配置文件标签)。  该参数从hvigorw 6.23.3版本开始支持。 |
...
| 参数 | 说明 |
| --- | --- |
| --daemon | 启用[守护进程](../../构建应用/提升构建效率/守护进程/ide-hvigor-daemon.md)。 |
...
| 参数 | 说明 |
| --- | --- |
| --parallel, --no-parallel | Hvigor默认开启[并行构建](../../构建应用/提升构建效率/并行构建/ide-hvigor-improve-performance.md)能力，如需关闭在命令行后添加--no-parallel。 |
| --incremental, --no-incremental | Hvigor默认开启[增量构建](../../构建应用/提升构建效率/增量构建/ide-hvigor-incremental-build.md)能力，如需关闭在命令行后添加--no-incremental。 |
...
| -p buildOption | 命令包含此参数时会打印buildOption配置，不含此参数时将不会展示buildOption配置，输出的buildOption优先级请参考[合并编译选项规则](../../构建应用/定制构建/灵活定制编译选项/能力说明/ide-hvigor-compilation-options-customizing-guide.md#section1727865610255)。 |
...
| --config, -c | 指定hvigor-config.json5配置文件中的参数。  当前仅支持设置properties里的参数，具体支持的参数请查看[hvigor-config.json5文件](../../构建应用/配置文件/hvigor-config.json5文件/ide-hvigor-set-options.md)中properties支持的参数。  --config properties.key=value 同 -c properties.key=value |
