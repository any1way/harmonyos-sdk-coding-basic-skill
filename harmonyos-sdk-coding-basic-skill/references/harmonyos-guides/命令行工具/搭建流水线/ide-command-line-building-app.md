...
scraped_at: 2026-06-11T15:32:39+08:00
doc_updated_at: 2026-06-10
content_hash: sha256:c770ad0a896b6d2a3154ff196b0fa76187dc33ee8ee198dcd5d2f15cd034bb34
---
...
4. 执行如下命令，检查JDK安装结果。
   ...
   ![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/92/v3/ZpBMdV9qQyizCl0cR3Kz_A/zh-cn_image_0000002602186141.png?HW-CC-KV=V1&HW-CC-Date=20260611T073238Z&HW-CC-Expire=86400&HW-CC-Sign=63902030828FCBE0872F2B1DFE2DC7D9962D9B3A8EB587AA7058CB4B954D1ECC)

### 获取命令行工具

1. [命令行工具获取](../获取命令行工具/ide-commandline-get.md#section21298572437)。其他系统(Windows/macOS)请根据实际情况下载对应版本。
2. 执行如下命令，解压命令行工具commandline-tools-linux-xxx.zip，工具名称请根据实际情况进行修改。
   ...
   ![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/2a/v3/8c95lxpURu62KDJE6okfVA/zh-cn_image_0000002602066085.png?HW-CC-KV=V1&HW-CC-Date=20260611T073238Z&HW-CC-Expire=86400&HW-CC-Sign=640F226E03DFD4B9D50A10E7F36F0A33316F18370F690EDD0F3A71D00619B701)
...
2. 执行如下命令，查询Node.js版本信息，确认配置成功。
   ...
   ![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/e9/v3/Dn-M77jsRHqUScsYDkxBrg/zh-cn_image_0000002571546604.png?HW-CC-KV=V1&HW-CC-Date=20260611T073238Z&HW-CC-Expire=86400&HW-CC-Sign=9392561DCF91B0E6BBE9B9751CD1F5D5AEDAE4BE21D7E4A7F0A219703561DF02)
...
在linux系统的构建场景下，使用[纹理压缩](../../构建应用/配置文件/工程级build-profile.json5文件/ide-hvigor-build-profile-app.md#section2095319147103)功能需要安装libGL1库。例如Ubuntu/Debian上安装libgl1-mesa-dev，CentOS/RHEL上安装mesa-libGL-devel。
...
* 本文使用Linux作为流水线构建环境，Linux环境会对大小写敏感，如果您的代码引用中有大小写错误（例如代码中import funcA from './aaa'，而实际文件为AAA.ets），而且开发环境是Windows或者Mac，那么有可能出现Windows或者Mac环境下编译通过，而Linux环境下编译不通过的现象。通过在项目级的build-profile.json5文件中配置caseSensitiveCheck为true来打开大小写敏感，保持Windows或者Mac环境编译与Linux环境编译结果一致。
  ...
  2. {
  3. "name": "default",
  4. "compatibleSdkVersion": "6.1.1(24)",
...
* 如果在非daemon模式下，需要修改node内存配置，可在command-line-tools的hvigor/bin/hvigorw文件中取消第15行的注释，并配置对应的数值。如将node内存配置为10240，示例如下：
  ...
  ```
* 如果是在daemon模式下，请参考[设置守护进程内存](../../构建应用/提升构建效率/守护进程/ide-hvigor-daemon.md#section327617383145)。

编译构建常见的任务和扩展参数如下，更多关于Hvigor命令行参数详见：[常用命令](../命令行构建工具（hvigorw）/ide-hvigor-commandline.md#section16300629103)。

**表1** HarmonyOS应用构建常用扩展参数

| 选项 | 说明 |
| --- | --- |
| -p buildMode={debug | release} | 采用debug/release模式进行编译构建。  缺省时：构建Hap/Hsp/Har时为debug模式，构建App时为release模式。  关于构建模式的详细说明，请参考[指定构建模式](../../构建应用/定制构建/灵活定制编译选项/能力说明/ide-hvigor-compilation-options-customizing-guide.md#section192461528194916)。针对HAR构建，请参考[构建HAR](../../构建应用/配置构建流程/构建HAR/ide-hvigor-build-har.md)。 |
...
生成证书请求文件后，在AppGallery Connect中申请、下载调试数字证书和Profile文件，具体请参考[申请调试证书](../../编写与调试应用/配置调试签名/ide-signing.md#section081822416419)和[申请Profile文件和添加权限信息](../../编写与调试应用/配置调试签名/ide-signing.md#section89479413571)。
...
通过[hdc工具](../../系统/调测调优/调试命令/hdc/hdc.md)将HAP推送到真机设备上进行安装，需要注意的是，推送的HAP必须是携带签名信息的，否则会导致HAP安装失败。
...
1. 请在可访问网络的电脑上创建一个空文件夹，在文件夹中创建一个package.json文件，在文件中填写如下内容：
   ...
   5. }
   ```
2. 先配置[环境变量](ide-command-line-building-app.md#section159168531288)，再打开[命令行工具](../获取命令行工具/ide-commandline-get.md)，在文件夹下执行 npm install 命令，会生成node\_modules文件夹。

   ![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/34/v3/dCXz6LGVREeJfq-yRPBAEg/zh-cn_image_0000002571386966.png?HW-CC-KV=V1&HW-CC-Date=20260611T073238Z&HW-CC-Expire=86400&HW-CC-Sign=B9A3B00E7B6BB24A092567AAF471C1674141F380AF38B6B449B2CE6DA612B9B9)
...
1. 请在可访问网络的电脑上创建一个空文件夹，在文件夹中创建一个package.json文件，配置npm依赖，示例如下：
   ...
   5. }
   ```
2. 打开[命令行工具](../获取命令行工具/ide-commandline-get.md)，在文件夹下执行 npm install 命令，会生成node\_modules文件夹。

   ![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/a4/v3/50zwrHjOSdWO6JT4BODHNQ/zh-cn_image_0000002571546606.png?HW-CC-KV=V1&HW-CC-Date=20260611T073238Z&HW-CC-Expire=86400&HW-CC-Sign=CFF6C21DAD15FDBFD9D35395A4BEC8E63722068C0CD1623CE105FA1D56DD3CFC)
3. 将node\_modules文件夹拷贝到无网络电脑的工程根目录下。

### 安装ohpm依赖插件

请参考[安装三方库](../../开发环境搭建/离线环境配置指导/ide-no-network.md#section208435578175)。

### 安装libGL1库

在linux系统的构建场景下，使用[纹理压缩](../../构建应用/配置文件/工程级build-profile.json5文件/ide-hvigor-build-profile-app.md#section2095319147103)功能需要安装libGL1库。
