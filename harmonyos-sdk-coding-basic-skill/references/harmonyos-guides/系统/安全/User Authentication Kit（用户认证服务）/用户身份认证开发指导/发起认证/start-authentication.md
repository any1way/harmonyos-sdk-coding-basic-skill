...
scraped_at: 2026-06-11T14:47:05+08:00
doc_updated_at: 2026-05-26
content_hash: sha256:815e99e5a418def6c9e686e9e866bc7558f638de169e6810bdeb96cb5cdede93
---
应用发起身份认证请求，获取身份认证结果，以访问受保护的系统、服务或应用的功能和数据，包括用户个人数据。

## 接口说明

具体参数、返回值、错误码等描述，请参考对应的[userAuth.getUserAuthInstance](<../../../../../../harmonyos-references/User Authentication Kit（用户认证服务）/ArkTS API/@ohos.userIAM.userAuth (用户认证)/js-apis-useriam-userauth.md#userauthgetuserauthinstance10>)。
...
认证控件的样式如图所示，通过[WidgetParam](<../../../../../../harmonyos-references/User Authentication Kit（用户认证服务）/ArkTS API/@ohos.userIAM.userAuth (用户认证)/js-apis-useriam-userauth.md#widgetparam10>)配置对应参数。

![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/72/v3/SIc5OuB3QaqaT0MfSI3q-w/zh-cn_image_0000002592218838.png?HW-CC-KV=V1&HW-CC-Date=20260611T064704Z&HW-CC-Expire=86400&HW-CC-Sign=193550430F7D2200F1C55A34527B09DA92E00E7317A2297F679CBDB673ED028E)
...
* 标注2：当生物认证失败后，将显示一个按钮。点击该按钮，可以从生物认证切换到其他口令认证类型（AuthParam.authType）。
  ...
  开发者如需[切换自定义认证](../切换自定义认证/apply-custom-authentication.md)，需要配置导航按键上显示的文本（WidgetParam.navigationButtonText），最大长度为60字符。API 10-17仅在单指纹、单人脸场景下支持配置。从API 18开始，增加支持人脸+指纹场景。
...
1. [申请权限](../开发准备/prerequisites.md#申请权限)：ohos.permission.ACCESS\_BIOMETRIC。
2. 指定用户认证相关参数[AuthParam](<../../../../../../harmonyos-references/User Authentication Kit（用户认证服务）/ArkTS API/@ohos.userIAM.userAuth (用户认证)/js-apis-useriam-userauth.md#authparam10>)（包括挑战值、认证类型[UserAuthType](<../../../../../../harmonyos-references/User Authentication Kit（用户认证服务）/ArkTS API/@ohos.userIAM.userAuth (用户认证)/js-apis-useriam-userauth.md#userauthtype8>)列表和认证等级[AuthTrustLevel](<../../../../../../harmonyos-references/User Authentication Kit（用户认证服务）/ArkTS API/@ohos.userIAM.userAuth (用户认证)/js-apis-useriam-userauth.md#authtrustlevel8>)）、配置认证控件界面[WidgetParam](<../../../../../../harmonyos-references/User Authentication Kit（用户认证服务）/ArkTS API/@ohos.userIAM.userAuth (用户认证)/js-apis-useriam-userauth.md#widgetparam10>)，调用[getUserAuthInstance](<../../../../../../harmonyos-references/User Authentication Kit（用户认证服务）/ArkTS API/@ohos.userIAM.userAuth (用户认证)/js-apis-useriam-userauth.md#userauthgetuserauthinstance10>)获取认证对象。
3. 调用[UserAuthInstance.on('result')](<../../../../../../harmonyos-references/User Authentication Kit（用户认证服务）/ArkTS API/@ohos.userIAM.userAuth (用户认证)/js-apis-useriam-userauth.md#onresult10-1>)接口订阅认证结果。
4. 调用[UserAuthInstance.start](<../../../../../../harmonyos-references/User Authentication Kit（用户认证服务）/ArkTS API/@ohos.userIAM.userAuth (用户认证)/js-apis-useriam-userauth.md#start10>)接口发起认证，通过[IAuthCallback](<../../../../../../harmonyos-references/User Authentication Kit（用户认证服务）/ArkTS API/@ohos.userIAM.userAuth (用户认证)/js-apis-useriam-userauth.md#iauthcallback10>)回调返回认证结果[UserAuthResult](<../../../../../../harmonyos-references/User Authentication Kit（用户认证服务）/ArkTS API/@ohos.userIAM.userAuth (用户认证)/js-apis-useriam-userauth.md#userauthresult10>)。当认证成功时返回认证通过类型（[UserAuthType](<../../../../../../harmonyos-references/User Authentication Kit（用户认证服务）/ArkTS API/@ohos.userIAM.userAuth (用户认证)/js-apis-useriam-userauth.md#userauthtype8>)）和令牌信息（AuthToken）。
...
[Index.ets](https://gitcode.com/HarmonyOS_Samples/guide-snippets/blob/HarmonyOS-feature-20260402/UserAuthentication/entry/src/main/ets/pages/Index.ets#L126-L169)
...
[Index.ets](https://gitcode.com/HarmonyOS_Samples/guide-snippets/blob/HarmonyOS-feature-20260402/UserAuthentication/entry/src/main/ets/pages/Index.ets#L176-L224)
...
[Index.ets](https://gitcode.com/HarmonyOS_Samples/guide-snippets/blob/HarmonyOS-feature-20260402/UserAuthentication/entry/src/main/ets/pages/Index.ets#L231-L279)
...
在PC/2in1设备上，应用如果使用模应用弹窗方式发起认证（即配置用户界面参数[widgetParam](<../../../../../../harmonyos-references/User Authentication Kit（用户认证服务）/ArkTS API/@ohos.userIAM.userAuth (用户认证)/js-apis-useriam-userauth.md#widgetparam10>)时传入了有效的uiContext），收到认证结果后，若需弹出其他窗口，应先获取控件弹窗释放的标志消息，通过[on('authTip')](<../../../../../../harmonyos-references/User Authentication Kit（用户认证服务）/ArkTS API/@ohos.userIAM.userAuth (用户认证)/js-apis-useriam-userauth.md#onauthtip20>)接口订阅控件释放消息（authTipInfo.tipCode = UserAuthTipCode.WIDGET\_RELEASED）。
...
[Index.ets](https://gitcode.com/HarmonyOS_Samples/guide-snippets/blob/HarmonyOS-feature-20260402/UserAuthentication/entry/src/main/ets/pages/Index.ets#L286-L330)
