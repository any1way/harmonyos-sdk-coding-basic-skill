...
content_hash: sha256:5c2428fb61451e3c2e00edcd79b6b34ef0a36e6a7842f7a386815cf265f5aeba
---
Enterprise Data Guard Kit（企业数据保护服务）为企业安全管控类[MDM](<../../../基础功能/MDM Kit（企业设备管理服务）/mdm-kit.md>)应用提供关键信息资产（KIA）文件的识别、外发管控以及企业恢复密钥的管理能力，支撑企业构建完整的数据防泄漏解决方案，实现企业数据资产可知、可控、可追溯。
...
当前文件分级管控服务仅支持对用户数据进行文件扫描和分级标识。文件扫描仅限于默认路径范围内的子目录，且[获取文件URI](../文件分级管控/获取文件URI/fileguard-get-file-url.md)、[删除指定路径下的文件](../文件分级管控/删除指定路径下的文件/fileguard-delete-file.md)以及以只写模式[打开文件](../文件分级管控/打开文件/fileguard-openfile.md)的功能仅适用于用户个人数据目录下的绝对路径。

| 文件路径 | 说明 |
| --- | --- |
| /data/service/el2/  /data/app/el1/bundle/public/  /mnt/hmdfs/  /data/app/el1/  /data/app/el2/  /data/app/el3/  /data/app/el4/  /data/app/el5/ | 默认路径范围内的子目录。文件路径与物理路径对应的关系及不同加密分区的差异，请参考[应用沙箱路径和真实物理路径的对应关系](<../../../../应用框架/Core File Kit（文件基础服务）/应用文件/应用沙箱目录/app-sandbox-directory.md#应用沙箱路径和真实物理路径的对应关系>)和[获取和修改加密分区](<../../../../应用框架/Ability Kit（程序框架服务）/Stage模型开发指导/Stage模型应用组件/应用上下文Context/application-context-stage.md#获取和修改加密分区>)。 |
