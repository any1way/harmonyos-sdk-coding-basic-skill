...
scraped_at: 2026-06-01T11:13:02+08:00
doc_updated_at: 2026-05-26
content_hash: sha256:c71489207cf13c216f564da11d6731b5998087e5ae0654fced5507ee56234b3e
---
应用在申请权限时，需在项目的配置文件中逐个声明所需权限，否则无法获取授权，并可能导致应用上架申请被驳回。

## 在配置文件中声明权限

应用必须在[module.json5配置文件](../../../../../../基础入门/开发基础知识/应用配置文件（Stage模型）/module.json5配置文件/module-configuration-file.md)的requestPermissions标签中声明权限。

| 属性 | 含义 | 数据类型 | 取值范围 |
| --- | --- | --- | --- |
| name | 需要使用的权限名称。 | 字符串 | **必填**，需为系统已定义的权限，取值范围请参考[应用权限列表](../../应用权限列表/app-permissions.md)。 |
| reason | 申请权限的原因。 | 字符串 | **可选填写**，该字段用于应用上架校验，申请user\_grant/manual\_settings权限时必填并需多语种适配。  格式为$string: \*\*\*。string资源引用需要在string.json文件配置标签"name": "reason"，配置样例可参考[资源文件示例](../../../../../../基础入门/资源分类与访问/resource-categories-and-access.md#资源文件示例)。  reason填写内容可参考[权限使用理由的文案内容规范](declare-permissions.md#权限使用理由的文案内容规范)。 |
...
在多HAP场景下，已在[entry](../../../../../../基础入门/开发基础知识/应用程序包基础知识/应用程序包开发与使用/HAP/hap-package.md)模块中声明的权限，无需在[feature](../../../../../../基础入门/开发基础知识/应用程序包基础知识/应用程序包开发与使用/HAP/hap-package.md)模块中重复添加，权限将在整个应用中生效。

同理，在[feature](../../../../../../基础入门/开发基础知识/应用程序包基础知识/应用程序包开发与使用/HAP/hap-package.md)模块中已声明的权限，在[entry](../../../../../../基础入门/开发基础知识/应用程序包基础知识/应用程序包开发与使用/HAP/hap-package.md)模块也无需重复添加。

## 声明样例

在[module.json5配置文件](../../../../../../基础入门/开发基础知识/应用配置文件（Stage模型）/module.json5配置文件/module-configuration-file.md)的requestPermissions标签中声明权限。
...
[module.json5](https://gitcode.com/HarmonyOS_Samples/guide-snippets/blob/HarmonyOS-feature-20260402/Security/RequestUserAuthorization/entry/src/main/module.json5#L16-L106)

## 权限使用理由的文案内容规范

当申请user\_grant/manual\_settings权限时，字段reason（申请权限的原因）必填。开发者需在应用配置文件中配置每个需要的权限。

但在实际向用户弹窗申请授权时，user\_grant权限将会以[权限组](../../应用权限管控概述/app-permission-mgmt-overview.md#权限组和子权限)的形式向用户申请。当前支持的权限组请查看[应用权限组列表](../../应用权限组列表/app-permission-group-list.md)。
...
3. 如果应用内多包申请的权限名称相同，但权限使用理由不一致，系统返回的权限申请详细信息[ReqPermissionDetail](<../../../../../../../harmonyos-references/Ability Kit（程序框架服务）/ArkTS API/接口依赖的元素及定义/bundleManager/BundleInfo/js-apis-bundlemanager-bundleinfo.md#reqpermissiondetail>)中只会显示一个权限申请理由。优先级从高到低为：entry类型HAP、feature类型HAP、应用内HSP。
