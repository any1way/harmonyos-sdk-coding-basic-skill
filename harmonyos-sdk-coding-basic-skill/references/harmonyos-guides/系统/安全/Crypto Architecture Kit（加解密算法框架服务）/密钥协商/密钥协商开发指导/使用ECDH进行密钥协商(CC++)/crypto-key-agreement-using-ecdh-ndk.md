...
content_hash: sha256:c4d9a84b6cba110ec196d59969301de1ffefa1aaab43be812dc7397ac7357573
---
...
1. 调用[OH\_CryptoAsymKeyGenerator\_Create](<../../../../../../../harmonyos-references/Crypto Architecture Kit（加解密算法框架服务）/C API/头文件/crypto_asym_key.h/capi-crypto-asym-key-h.md#oh_cryptoasymkeygenerator_create>)、[OH\_CryptoAsymKeyGenerator\_Generate](<../../../../../../../harmonyos-references/Crypto Architecture Kit（加解密算法框架服务）/C API/头文件/crypto_asym_key.h/capi-crypto-asym-key-h.md#oh_cryptoasymkeygenerator_generate>)、[OH\_CryptoAsymKeyGenerator\_Convert](<../../../../../../../harmonyos-references/Crypto Architecture Kit（加解密算法框架服务）/C API/头文件/crypto_asym_key.h/capi-crypto-asym-key-h.md#oh_cryptoasymkeygenerator_convert>)生成密钥算法为ECC、密钥长度为256位的非对称密钥（keyPair）。
   ...
   如何生成ECC非对称密钥，开发者可参考下文示例，并结合[非对称密钥生成和转换规格：ECC](../../../密钥生成和转换/密钥生成和转换规格/非对称密钥生成和转换规格/crypto-asym-key-generation-conversion-spec.md#ecc)和[随机生成非对称密钥对](../../../密钥生成和转换/密钥生成和转换开发指导/随机生成非对称密钥对(CC++)/crypto-generate-asym-key-pair-randomly-ndk.md)理解。参考文档与当前示例可能存在入参差异，请在阅读时注意区分。
2. 调用[OH\_CryptoKeyAgreement\_Create](<../../../../../../../harmonyos-references/Crypto Architecture Kit（加解密算法框架服务）/C API/头文件/crypto_key_agreement.h/capi-crypto-key-agreement-h.md#oh_cryptokeyagreement_create>)，指定字符串参数'ECC256'，创建密钥算法为ECC、密钥长度为256位的密钥协议生成器。
...
24. static OH_Crypto_ErrCode ConvertKeyPairByBlob(OH_CryptoAsymKeyGenerator *eccGen, OH_CryptoKeyPair **keyPair)
25. {
...
53. ret = ConvertKeyPairByBlob(eccGen, &keyPairA);
...
[ECDH.cpp](https://gitcode.com/HarmonyOS_Samples/guide-snippets/blob/HarmonyOS-feature-20260402/Security/CryptoArchitectureKit/KeyNegotiationCpp/entry/src/main/cpp/types/project/ECDH.cpp#L16-L114)
