...
scraped_at: 2026-06-11T14:42:16+08:00
doc_updated_at: 2026-06-05
content_hash: sha256:fbce3b7abb566089e033c3307dd94c3039ffaa2ca121f49182589ed7184c62f5
---
...
1. 调用[OH\_CryptoSign\_Create](<../../../../../../../harmonyos-references/Crypto Architecture Kit（加解密算法框架服务）/C API/头文件/crypto_signature.h/capi-crypto-signature-h.md#oh_cryptosign_create>)，指定字符串参数'ECC256|SHA256'，创建非对称密钥类型为ECC256、摘要算法为SHA256的Sign实例，用于完成签名操作。
2. 调用[OH\_CryptoSign\_Init](<../../../../../../../harmonyos-references/Crypto Architecture Kit（加解密算法框架服务）/C API/头文件/crypto_signature.h/capi-crypto-signature-h.md#oh_cryptosign_init>)，使用私钥[OH\_CryptoPrivKey](<../../../../../../../harmonyos-references/Crypto Architecture Kit（加解密算法框架服务）/C API/结构体/OH_CryptoPrivKey/capi-cryptoasymkeyapi-oh-cryptoprivkey.md>)初始化Sign实例。
...
4. 调用[OH\_CryptoSign\_Final](<../../../../../../../harmonyos-references/Crypto Architecture Kit（加解密算法框架服务）/C API/头文件/crypto_signature.h/capi-crypto-signature-h.md#oh_cryptosign_final>)，对数据进行签名。
...
5. static OH_Crypto_ErrCode doTestEcdsaSign() {
...
1. 调用[OH\_CryptoVerify\_Create](<../../../../../../../harmonyos-references/Crypto Architecture Kit（加解密算法框架服务）/C API/头文件/crypto_signature.h/capi-crypto-signature-h.md#oh_cryptoverify_create>)，指定字符串参数'ECC256|SHA256'，创建非对称密钥类型为ECC256、摘要算法为SHA256的Verify实例，用于完成验签操作。
...
3. 调用[OH\_CryptoVerify\_Update](<../../../../../../../harmonyos-references/Crypto Architecture Kit（加解密算法框架服务）/C API/头文件/crypto_signature.h/capi-crypto-signature-h.md#oh_cryptoverify_update>)，传入待验证的数据。当前单次update长度没有限制，开发者可以根据数据量判断如何调用update，如果数据量较小，可以直接调用OH\_CryptoVerify\_Final接口一次性传入。
...
[ecdsa\_signature\_verification.cpp](https://gitcode.com/HarmonyOS_Samples/guide-snippets/blob/HarmonyOS-feature-20260402/Security/CryptoArchitectureKit/SignatureVerification/SigningSignatureVerification/entry/src/main/cpp/types/project/ecdsa_signature_verification.cpp#L15-L80)
