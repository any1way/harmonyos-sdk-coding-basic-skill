...
scraped_at: 2026-06-11T14:41:32+08:00
doc_updated_at: 2026-06-09
content_hash: sha256:63b80e7a67788f823b29ad3f44d90b8982f198e740ebf5875f53220da3892a36
---
...
如何生成AES对称密钥，开发者可参考下文示例，并结合[对称密钥生成和转换规格：AES](../../../密钥生成和转换/密钥生成和转换规格/对称密钥生成和转换规格/crypto-sym-key-generation-conversion-spec.md#aes)和[随机生成对称密钥](../../../密钥生成和转换/密钥生成和转换开发指导/随机生成对称密钥(CC++)/crypto-generate-sym-key-randomly-ndk.md)文档进行理解。参考文档与当前示例可能存在入参差异，请注意区分。

**加密**

1. 调用[OH\_CryptoSymCipher\_Create](<../../../../../../../harmonyos-references/Crypto Architecture Kit（加解密算法框架服务）/C API/头文件/crypto_sym_cipher.h/capi-crypto-sym-cipher-h.md#oh_cryptosymcipher_create>)，指定字符串参数'AES128|GCM|PKCS7'，创建对称密钥算法为AES128、分组模式为GCM、填充模式为PKCS7的Cipher实例，用于完成加密操作。
...
3. 调用[OH\_CryptoSymCipher\_Init](<../../../../../../../harmonyos-references/Crypto Architecture Kit（加解密算法框架服务）/C API/头文件/crypto_sym_cipher.h/capi-crypto-sym-cipher-h.md#oh_cryptosymcipher_init>)，设置模式为加密（CRYPTO\_ENCRYPT\_MODE），指定对称密钥（OH\_CryptoSymKey）和GCM模式的加密参数（OH\_CryptoSymCipherParams），以初始化加密 Cipher 实例。
4. 将一次传入数据量设置为20字节，多次调用[OH\_CryptoSymCipher\_Update](<../../../../../../../harmonyos-references/Crypto Architecture Kit（加解密算法框架服务）/C API/头文件/crypto_sym_cipher.h/capi-crypto-sym-cipher-h.md#oh_cryptosymcipher_update>)，更新数据（明文）。
...
5. 调用[OH\_CryptoSymCipher\_Final](<../../../../../../../harmonyos-references/Crypto Architecture Kit（加解密算法框架服务）/C API/头文件/crypto_sym_cipher.h/capi-crypto-sym-cipher-h.md#oh_cryptosymcipher_final>)，获取密文。
...
1. 调用[OH\_CryptoSymCipher\_Create](<../../../../../../../harmonyos-references/Crypto Architecture Kit（加解密算法框架服务）/C API/头文件/crypto_sym_cipher.h/capi-crypto-sym-cipher-h.md#oh_cryptosymcipher_create>)，指定参数'AES128|GCM|PKCS7'，创建对称密钥类型为AES128、分组模式为GCM、填充模式为PKCS7的Cipher实例，完成解密操作。
2. 调用[OH\_CryptoSymCipherParams\_SetParam](<../../../../../../../harmonyos-references/Crypto Architecture Kit（加解密算法框架服务）/C API/头文件/crypto_sym_cipher.h/capi-crypto-sym-cipher-h.md#oh_cryptosymcipherparams_setparam>)设置authTag作为解密的认证信息。
...
4. 将一次传入数据量设置为20字节，多次调用[OH\_CryptoSymCipher\_Update](<../../../../../../../harmonyos-references/Crypto Architecture Kit（加解密算法框架服务）/C API/头文件/crypto_sym_cipher.h/capi-crypto-sym-cipher-h.md#oh_cryptosymcipher_update>)，更新数据（密文）。
...
[aes\_gcm\_segment\_encryption\_decryption.cpp](https://gitcode.com/HarmonyOS_Samples/guide-snippets/blob/HarmonyOS-feature-20260402/Security/CryptoArchitectureKit/EncryptionDecryption/EncryptionDecryptionGuidanceAes/entry/src/main/cpp/types/project/aes_gcm_segment_encryption_decryption.cpp#L16-L148)
