...
content_hash: sha256:f7ddcdb2d9a989bb8987575dfc95679843c910fd22cd03922b9b26f0f556395d
---
## 简介

地址越界问题是指访问了不合法的地址，导致程序运行出现异常，通常表现为应用崩溃（Crash），其故障原因为释放后使用（use after free）、重复释放（double-free）、栈溢出（stack-overflow）、堆溢出（heap-overflow）等。由于应用崩溃日志信息有限且非崩溃第一现场，地址越界问题定位较为困难，一般依赖ASan、HWASan、GWP-ASan等检测工具以获取更多内存操作信息。从API13开始推荐[使用HWASan检测工具](../../../../../../best-practices/稳定性/稳定性检测/开发态稳定性检测/地址越界类问题检测/使用HWASan检测内存错误/bpta-stability-hwasan-detection.md)进行地址越界问题的分析。

## 常见越界类型与影响

常见地址越界类型和影响可参看[地址越界经典问题类型](../../../../../../best-practices/稳定性/稳定性检测/开发态稳定性检测/地址越界类问题检测/地址越界经典问题类型/bpta-stability-address-sanitizer-catagory.md)。

## 地址越界检测原理

检测原理和使用方法可参看[地址越界类问题检测](../../../../../../best-practices/稳定性/稳定性检测/开发态稳定性检测/地址越界类问题检测/bpta-stability-ram-detection.md)。
...
DevEco Studio会收集设备/data/log/faultlog/faultlogger/路径下的进程崩溃故障日志到FaultLog下，根据进程名和故障和时间分类显示。获取日志的方法参见：[DevEco Studio使用指南-FaultLog](../../../../../编写与调试应用/日志与故障分析/故障分析/FaultLog/ide-fault-log.md)。

**方式二：通过HiAppEvent接口订阅**

HiAppEvent给开发者提供了故障订阅接口，详见[HiAppEvent介绍](../../事件订阅/HiAppEvent介绍/hiappevent-intro.md)。参考[订阅地址越界事件（ArkTS）](../../事件订阅/使用HiAppEvent订阅事件/系统事件/地址越界事件/订阅地址越界事件（ArkTS）/hiappevent-watcher-address-sanitizer-events-arkts.md)或[订阅地址越界事件（C/C++）](../../事件订阅/使用HiAppEvent订阅事件/系统事件/地址越界事件/订阅地址越界事件（CC++）/hiappevent-watcher-address-sanitizer-events-ndk.md)完成地址越界事件订阅，并通过事件的[external\_log](../../事件订阅/使用HiAppEvent订阅事件/系统事件/地址越界事件/地址越界事件介绍/hiappevent-watcher-address-sanitizer-events.md#params字段说明)字段读取故障日志文件内容。
...
1. **提取故障类型**
   ...
   * GWP-ASan
     ...
     在[GWP-ASan日志](address-sanitizer-guidelines.md#gwp-asan日志规格)中，故障类型根据原始日志中包含"at"的行提取。可能的故障类型包括Use After Free、Double Free、Invalid (Wild) Free等，详细类型说明可参考[GWP-ASan异常检测类型](../../../../../../best-practices/稳定性/稳定性检测/开发态稳定性检测/地址越界类问题检测/使用GWP-ASan检测内存错误/bpta-stability-gwpasan-detection.md#section73731529454)。
   * ASan/HWASan/MemDebug

     在[ASan](address-sanitizer-guidelines.md#asan日志规格)、[HWASan](address-sanitizer-guidelines.md#hwasan日志规格)和[MemDebug](address-sanitizer-guidelines.md#memdebug日志规格)日志中，故障原因通过日志中的"Reason"字段提取，提取结果将作为后续聚类依据。详细字段说明可参考[AddrSanitizer日志type字段说明](../../事件订阅/使用HiAppEvent订阅事件/系统事件/地址越界事件/地址越界事件介绍/hiappevent-watcher-address-sanitizer-events.md)。
2. **标准化栈信息**
