...
scraped_at: 2026-06-11T14:52:20+08:00
doc_updated_at: 2026-04-30
content_hash: sha256:91b93b882b6fab040e56ffcca0f3734d274d431b93d9f662db877544d7e5a20a
---
## 简介
...
资源泄漏主要分为三类：内存泄漏、句柄泄漏和线程泄漏。对于每种泄漏，系统会通过周期采样的方式对进程的资源使用情况进行检测，如果资源使用超过阈值，会抓取对应维测并上报泄漏事件。通过Hiappevent资源泄漏事件进行订阅，订阅方法详见[资源泄漏事件](../../事件订阅/使用HiAppEvent订阅事件/系统事件/资源泄漏事件/资源泄漏事件介绍/hiappevent-watcher-resourceleak-events.md)。
...
1. 表格中所述阈值/基线均为系统默认，如果生态在开发过程中需要自行设定基线，可以使用[hidebug.setAppResourceLimit接口](<../../../../../../harmonyos-references/Performance Analysis Kit（性能分析服务）/ArkTS API/@ohos.hidebug (Debug调试)/js-apis-hidebug.md#hidebugsetappresourcelimit12>)进行设置，该接口建议在开发阶段调用，不要在正式发布阶段使用。
2. 虚拟机内存使用率计算公式 = heapUsed / totalHeap。

   heapUsed：当前虚拟机使用的堆大小，单位：KB。可通过[hidebug.getAppVMMemoryInfo()](<../../../../../../harmonyos-references/Performance Analysis Kit（性能分析服务）/ArkTS API/@ohos.hidebug (Debug调试)/js-apis-hidebug.md#hidebuggetappvmmemoryinfo12>)接口获取。

   totalHeap：当前虚拟机的堆总大小，单位：KB。可通过[hidebug.getAppVMMemoryInfo()](<../../../../../../harmonyos-references/Performance Analysis Kit（性能分析服务）/ArkTS API/@ohos.hidebug (Debug调试)/js-apis-hidebug.md#hidebuggetappvmmemoryinfo12>)接口获取。
3. 当应用上报JS\_ERROR/CPP\_CRASH故障，Error message包含“OutOfMemory”时，可参考[内存泄漏分析方法](../../../../../../best-practices/稳定性/稳定性分析/资源泄漏类问题分析方法/bpta-stability-leak-way.md#section728319329442)辅助定位。
...
1. 句柄泄漏调用栈、native内存泄漏调用栈、js泄漏内存快照等维测因为开销较大，所以在[nolog版本](<../../Performance Analysis Kit术语/performance-analysis-kit-terminology.md#nolog版本>)默认不开启。
2. 如果开发者希望获取到nolog版本的js泄漏内存快照，可参考[资源泄漏事件订阅（ArkTS）](../../事件订阅/使用HiAppEvent订阅事件/系统事件/资源泄漏事件/订阅资源泄漏事件（ArkTS）/hiappevent-watcher-resourceleak-events-arkts.md)增加对nolog版本js内存快照的订阅。

## 日志获取

资源泄漏日志由LeakDetector模块进行管理，可通过以下方式获取：

* 方式一：通过[DevEco Testing进行探索测试](<../../../../../应用测试/专项测试/DevEco Testing/探索测试/exploratory-testing.md>)并获取日志。
  ...
  1. native内存泄漏的调用栈（memleak-native-[process\_name]-[pid]-[timestamp].txt）无法直接在DevEco Studio打开，需要修改后缀名为.nas，然后使用DevEco Studio-Profiler-打开并分析，详情见[内存分析及优化](../../../../../优化应用性能/基础内存：Allocation分析/内存分析介绍/ide-insight-session-allocations-memory.md)。
  2. js泄漏的维测日志 memleak-js-[process\_name]-[pid]-[tid]-[timestamp].rawheap 为二进制内存快照文件，需要通过[translator工具](../../../调试命令/命令行工具/rawheap-translator工具/rawheap-translator.md)转换为.heapsnapshot文件，通过DevEco Studio或浏览器打开展示，详情见[Snapshot离线导入](../../../../../优化应用性能/内存泄露：Snapshot分析/Snapshot模板基本操作/ide-snapshot-basic-operations.md#section6760173514388)。
* 方式二：通过DevEco Studio主动采集日志。

  DevEco Studio的profiler模块提供[Allocation](../../../../../优化应用性能/基础内存：Allocation分析/内存分析介绍/ide-insight-session-allocations-memory.md)（获取native调用栈profiler）和 **[Snapshot](../../../../../优化应用性能/内存泄露：Snapshot分析/案例：ArkTS内存泄漏分析/ide-arkts-memory-leak-analysis.md)** （获取JS层heapdump）两种采集方式：

  ![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/98/v3/rfzHYp7VQSuZOkFWcZyHKQ/zh-cn_image_0000002592218920.png?HW-CC-KV=V1&HW-CC-Date=20260611T065219Z&HW-CC-Expire=86400&HW-CC-Sign=7E1AA3ADF9D4E76EB57BEC4796FF087FA4403506025E5CE8F931508B0A7C24F0)
* 方式三：通过HiAppEvent接口订阅。

  HiAppEvent对外提供故障订阅接口，可以订阅各类故障打点，详见[HiAppEvent介绍](../../事件订阅/HiAppEvent介绍/hiappevent-intro.md)，其中资源泄漏的订阅方式详见[资源泄漏事件介绍](../../事件订阅/使用HiAppEvent订阅事件/系统事件/资源泄漏事件/资源泄漏事件介绍/hiappevent-watcher-resourceleak-events.md)。资源泄漏故障日志存于/data/storage/el2/log/resourcelimit/路径，日志名统一为RESOURCE\_OVERLIMIT\_[TIMESTAMP]\_[PID].log，可根据日志内容区分文件类型。
...
* **ashmem类型句柄**
  ...
  JS层API：[setMemoryNameSync](<../../../../../../harmonyos-references/Image Kit（图片处理服务）/ArkTS API/@ohos.multimedia.image (图片处理)/Interface (PixelMap)/arkts-apis-image-pixelmap.md#setmemorynamesync13>)。

  NATIVE层API：[OH\_PixelmapNative\_SetMemoryName](<../../../../../../harmonyos-references/Image Kit（图片处理服务）/C API/头文件/pixelmap_native.h/capi-pixelmap-native-h.md#oh_pixelmapnative_setmemoryname>)。
...
* 该文件记录了对象堆内存的详细信息。
* 日志文件需要将后缀名修改为.rawheap文件，再通过[translator工具](../../../调试命令/命令行工具/rawheap-translator工具/rawheap-translator.md)转换为.heapsnapshot文件，通过DevEco Studio或浏览器打开展示，详情见[Snapshot离线导入](../../../../../优化应用性能/内存泄露：Snapshot分析/Snapshot模板基本操作/ide-snapshot-basic-operations.md#section6760173514388)。
* API14后，需要将日志文件后缀名修改为.rawheap后，将其导入DevEco Studio并展示，详情见[Raw Heap离线导入](../../../../../优化应用性能/内存泄露：Snapshot分析/Snapshot模板基本操作/ide-snapshot-basic-operations.md#section6760173514388)。
...
* 检测到泄漏后抓取**15min内的进程内存trace**，可将日志如下图通过Open File加载到DevEco Studio进行解析。
  ...
  ![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/3a/v3/SpPT4-RrTo-V_6Ic2t8sSA/zh-cn_image_0000002592378854.png?HW-CC-KV=V1&HW-CC-Date=20260611T065219Z&HW-CC-Expire=86400&HW-CC-Sign=1807D60B3BE1CE40FC757FC8065342F28BDC4EA1E64493AAD265F91E0824550D)
...
* **All Heap：** 选择后展示抓取15分钟内的内存情况，记录了hook malloc等系统调用的堆栈。Native日志是以so+偏移的形式展示调用栈（每一行表示一次内存分配行为调用栈），需要结合符号表进一步分析。
  ...
  ![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/1/v3/XHWyJvlkRYWD073-LYDR3g/zh-cn_image_0000002622858361.png?HW-CC-KV=V1&HW-CC-Date=20260611T065219Z&HW-CC-Expire=86400&HW-CC-Sign=0165B9371CACCB555F9399D99F8F246B23AFC5DCEB70718AD5D4583473CD0C9E)
...
* **All Anonymous VM：** 选择后记录了当前hook mmap系统调用的堆栈信息。
  ...
  ![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/dd/v3/vvWAPyqCQMSkNknBFS0teg/zh-cn_image_0000002622698483.png?HW-CC-KV=V1&HW-CC-Date=20260611T065219Z&HW-CC-Expire=86400&HW-CC-Sign=6E00FED3DEFD71B4A0A55B9D2FCA2FD462F48F9320D78E29FFE717D155743F18)
...
2. 筛选出占比最高的内存泄漏类型后，可以根据以下规则进一步提取故障特征：
   ...
   * 堆内存：筛选内存申请最大的内存标签作为故障特征，如：从以下日志中提取的故障标签为“[anon:native\_heap:jemalloc]”。
     ...
     如果存在内存调用栈，可以根据NMD维测找到占用最高的内存区间，并结合抓取的调用栈维测聚类到具体代码段或者so作为怀疑点，具体分析方法可参考[Native泄漏分析方法](../../../../../../best-practices/稳定性/稳定性分析/资源泄漏类问题分析方法/bpta-stability-leak-way.md#section1658571616574)。
   ...
   * anon内存：筛选内存申请最大的内存标签作为故障特征，如：从以下日志中提取的故障标签为“[anon]”。
     ...
     如果存在内存调用栈，可通过分析调用栈中占比较高的聚类到具体代码段或者so作为怀疑点，具体分析方法可参考[Native泄漏分析方法](../../../../../../best-practices/稳定性/稳定性分析/资源泄漏类问题分析方法/bpta-stability-leak-way.md#section1658571616574)。
...
1. 提取故障日志中的ion维测信息来进行聚类，如下：
   ...
   18. ************ endl ************
   ```
2. 根据“leak\_type”和“buf\_name”对buffer进行分类，提取出“size\_bytes”之和占比最高buffer的“buf\_name”作为本次故障的故障特征。分析细节可参考[ION泄漏分析方法](../../../../../../best-practices/稳定性/稳定性分析/资源泄漏类问题分析方法/bpta-stability-leak-way.md#section5493141412410)。
...
3. pidNumber: 2017
4. processName: process1
5. killTime: 20260512090030
6. killReason: RssThresholdKiller
7. RssMemory: 8350500
```
...
```
1. LOGGER_MEMCHECK_SAMPLIFY_SMAPS_INFO
2. Size         Rss        Swap    Name
3. 256           4           0    [anon:ark-Object Space]
4. 256         240           0    [anon:ark-Non Movable Space]
5. 256          48           0    [anon:ark-Object Space]
6. 523520           4           0    [anon]
7. 783104           0           0    [anon:ArkTS Heap]
8. 256         144          16    [anon:ArkTS Heapnon movable space]
9. 256           8           0    [anon:ArkTS Heapread only space]
10. 256          32           0    [anon:ArkTS Heapshared non movable space]
11. 256          48           0    [anon:ArkTS Heapshared read only space]
12. 256         136          60    [anon:ArkTS Heapnon movable space]
13. 524288           0           0    [anon:ArkTS Heap]
14. 256          32           0    [anon:ArkTS Heapappspawn space]
15. 256         240          16    [anon:ArkTS Heapsemi space]
16. ......
```
...
gpu内核管控日志内容与[gpu内存泄漏日志规格](resource-leak-guidelines.md#ashmemiongpugpu_rs内存泄漏日志规格)中的“LOGGER\_MEMCHECK\_PROC\_INFO”字段下的维测内容保持一致。
...
ashmem内核管控日志内容与[ashmem内存泄漏日志规格](resource-leak-guidelines.md#ashmemiongpugpu_rs内存泄漏日志规格)中的“LOGGER\_MEMCHECK\_PROC\_INFO”字段下的维测内容保持一致。
...
ion内核管控日志内容与[ion内存泄漏日志规格](resource-leak-guidelines.md#ashmemiongpugpu_rs内存泄漏日志规格)中的“LOGGER\_MEMCHECK\_PROC\_INFO”字段下的维测内容保持一致。
...
5. killTime : 20260512090030
6. foreGround : no
7. killReason : ThreadKiller
8. threadCount : 3001

10. the process is killed by runnable thread threshold 3001 / 4124.
11. process1:2099
12. Thread_HI:9
13. Thread_GI:9
14. Thread_FI:9
