...
scraped_at: 2026-06-01T11:25:07+08:00
doc_updated_at: 2026-05-26
content_hash: sha256:d03d0f001f16b7c7c24324ac968b5923eca7f843893a2cd0681efd8e39f01559
---
## 接口说明

本文介绍如何使用HiAppEvent提供的ArkTS接口订阅资源泄漏事件。接口的具体使用说明（参数使用限制、具体取值范围等）请参考[@ohos.hiviewdfx.hiAppEvent](<../../../../../../../../../harmonyos-references/调测调优/Performance Analysis Kit（性能分析服务）/ArkTS API/@ohos.hiviewdfx.hiAppEvent (应用事件打点)/js-apis-hiviewdfx-hiappevent.md>)。
...
2. 编辑工程中的“entry > src > main > ets > entryability > EntryAbility.ets”文件，在onCreate函数中添加系统事件的订阅，示例代码如下：
   ...
   17. if (deviceInfo.sdkApiVersion >= 24) {  // API Version 24及以后版本，支持设置页面切换日志
   18. // 配置页面切换日志
   19. let switchLogPolicy : hiAppEvent.EventPolicy = {
   20. "resourceOverlimitPolicy": {
   21. "pageSwitchLogEnable": true
   22. }
   23. };
   24. // 开发者可以设置资源泄漏日志配置参数
   25. hiAppEvent.configEventPolicy(switchLogPolicy).then(() => {
   26. hilog.info(0x0000, 'testTag', `HiAppEvent success to config event policy.`);
   27. }).catch((err: BusinessError) => {
   28. hilog.error(0x0000, 'testTag', `HiAppEvent code: ${err.code}, message: ${err.message}`);
   29. });
   30. }

   32. hiAppEvent.addWatcher({
   33. // 自定义观察者名称，系统会使用名称来标识不同的观察者
   34. name: "watcher",
   35. // 订阅感兴趣的系统事件，此处是订阅了资源泄漏事件
   36. appEventFilters: [
   37. {
   38. domain: hiAppEvent.domain.OS,
   39. names: [hiAppEvent.event.RESOURCE_OVERLIMIT]
   40. }
   41. ],
   42. // 自行实现订阅实时回调函数，以便对订阅获取到的事件数据进行自定义处理
   43. onReceive: (domain: string, appEventGroups: Array<hiAppEvent.AppEventGroup>) => {
   44. hilog.info(0x0000, 'testTag', `HiAppEvent onReceive: domain=${domain}`);
   45. for (const eventGroup of appEventGroups) {
   46. // 根据事件集合中的事件名称区分不同的系统事件
   47. hilog.info(0x0000, 'testTag', `HiAppEvent eventName=${eventGroup.name}`);
   48. for (const eventInfo of eventGroup.appEventInfos) {
   49. // 获取到资源泄漏事件发生时内存信息
   50. hilog.info(0x0000, 'testTag', `HiAppEvent eventInfo=${JSON.stringify(eventInfo)}`);
   51. // 开发者可以获取到资源泄漏事件的页面切换日志
   52. hilog.info(0x0000, 'testTag', `HiAppEvent eventInfo.params.page_switch_log=${JSON.stringify(eventInfo.params['page_switch_log'])}`);
   53. }
   54. }
   55. }
   56. });
...
1. 编辑工程中的“entry > src > main > ets > pages > Index.ets”文件，添加按钮并在其onClick函数构造资源泄漏场景，以触发资源泄漏事件。
   ...
   此处需要使用[hidebug.setAppResourceLimit](<../../../../../../../../../harmonyos-references/调测调优/Performance Analysis Kit（性能分析服务）/ArkTS API/@ohos.hidebug (Debug调试)/js-apis-hidebug.md#hidebugsetappresourcelimit12>)设置内存限制，造成内存泄漏，同步在“开发者选项”中打开“系统资源泄漏日志”（开关状态变更后需重启设备）。

   资源泄漏问题定位可参考[Resource Leak（资源泄漏）检测](<../../../../../故障检测/Resource Leak（资源泄漏）检测/resource-leak-guidelines.md>)。
   ...
   [Index.ets](https://gitcode.com/HarmonyOS_Samples/guide-snippets/blob/HarmonyOS-feature-20260402/PerformanceAnalysisKit/HiAppEvent/EventSub/entry/src/main/ets/pages/Index.ets#L135-L164)
2. 添加 pss leak 相关内容：
...
4. pss内存泄漏事件上报后，系统会回调应用的onReceive函数，可以在Log窗口看到对系统事件数据的处理日志：
   ...
   4. HiAppEvent eventInfo.params.page_switch_log="[\"/data/storage/el2/log/page_switch/snapshot/page_switch-com.example.myapplication-1-1-20260427162423841.log\"]"
   ```

   如上，eventInfo中包含资源泄漏事件的[params字段](../资源泄漏事件介绍/hiappevent-watcher-resourceleak-events.md#params字段说明)，可以根据eventInfo中的resource\_type字段来判断当前的泄漏类型。
...
订阅后生成的.log日志文件需要将后缀名修改为.rawheap文件，再通过[translator工具](../../../../../../调试命令/命令行工具/rawheap-translator工具/rawheap-translator.md)转换为.heapsnapshot文件，通过DevEco Studio或浏览器打开展示，详情见[离线导入内存快照](../../../../../../../../优化应用性能/内存泄露：Snapshot分析/Snapshot模板基本操作/ide-snapshot-basic-operations.md#section6760173514388)。

API version 14后，开发者可以将日志文件后缀名修改为.rawheap后，将其导入DevEco Studio并展示，详情见[离线导入内存快照](../../../../../../../../优化应用性能/内存泄露：Snapshot分析/Snapshot模板基本操作/ide-snapshot-basic-operations.md#section6760173514388)。
