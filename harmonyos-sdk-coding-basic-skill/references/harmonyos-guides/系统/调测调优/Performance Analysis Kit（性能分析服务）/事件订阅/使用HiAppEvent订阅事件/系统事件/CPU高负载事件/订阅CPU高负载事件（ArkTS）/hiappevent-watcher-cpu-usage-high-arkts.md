...
scraped_at: 2026-06-01T11:25:30+08:00
doc_updated_at: 2026-05-14
content_hash: sha256:ffe21a5471bc50959e0773b9382caa59ec99aa020ef7206830dc389163545eb0
---
## 接口说明

API接口的具体使用说明（参数使用限制、具体取值范围等）请参考[@ohos.hiviewdfx.hiAppEvent (应用事件打点)ArkTS API文档](<../../../../../../../../../harmonyos-references/调测调优/Performance Analysis Kit（性能分析服务）/ArkTS API/@ohos.hiviewdfx.hiAppEvent (应用事件打点)/js-apis-hiviewdfx-hiappevent.md>)。
...
3. （可选）以下示例用于模拟自定义CPU高负载事件的配置策略
   ...
   编辑工程中的“entry > src > main > ets > entryability > EntryAbility.ets”文件，自定义CPU高负载事件的配置策略，用法详见[CpuUsageHighPolicy](<../../../../../../../../../harmonyos-references/调测调优/Performance Analysis Kit（性能分析服务）/ArkTS API/@ohos.hiviewdfx.hiAppEvent (应用事件打点)/js-apis-hiviewdfx-hiappevent.md#cpuusagehighpolicy22>)章节，示例代码如下：
   ...
   此外，Debug版本应用，采样栈每日采集次数阈值范围为：[-1, 100]； Release版本应用，采样栈每日采集次数阈值范围为阈值范围：[0, 20]，详见[CpuUsageHighPolicy](<../../../../../../../../../harmonyos-references/调测调优/Performance Analysis Kit（性能分析服务）/ArkTS API/@ohos.hiviewdfx.hiAppEvent (应用事件打点)/js-apis-hiviewdfx-hiappevent.md#cpuusagehighpolicy22>)。
...
7. 点击DevEco Studio界面中的运行按钮，运行应用工程，然后在应用界面中点击“CPU加压”按钮，触发CPU高负载事件。应用保持在前台且屏幕处于亮屏状态，五到十分钟之后可以在Log窗口看到对系统事件数据的处理日志：
   ...
   默认状态下，CPU高负载事件[抓取次数存在限制](../../../../../功耗检测/power-detection.md#约束和限制)。
