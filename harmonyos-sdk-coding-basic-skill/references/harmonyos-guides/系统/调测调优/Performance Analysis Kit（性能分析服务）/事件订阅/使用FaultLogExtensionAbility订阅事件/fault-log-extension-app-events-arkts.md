...
scraped_at: 2026-06-11T14:52:36+08:00
doc_updated_at: 2026-04-30
content_hash: sha256:4c6d186b82324e16ace280d4d0e21a3e811c2a1e0a009178b38078f75ddf8a26
---
从API version 21开始，可以在FaultLogExtensionAbility中使用HiAppEvent事件订阅接口，实现应用故障事件（仅包括[崩溃事件](../使用HiAppEvent订阅事件/系统事件/崩溃事件/崩溃事件介绍/hiappevent-watcher-crash-events.md)和[应用冻屏事件](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/hiappevent-watcher-freeze-events)）的延迟通知。应用因崩溃或冻屏退出后，无法启动或长时间未启动的场景下，可以不依赖应用启动实现故障事件信息的订阅回调。FaultLogExtensionAbility仅用于补充处理故障事件，不能替代[主进程](<../../../../../应用框架/Ability Kit（程序框架服务）/Stage模型开发指导/进程模型/process-model-stage.md#基本进程类型>)正常启动时进行故障事件处理。
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/cd/v3/eBbcNWT8SJqPnXnVpMd2hw/zh-cn_image_0000002622698489.png?HW-CC-KV=V1&HW-CC-Date=20260611T065235Z&HW-CC-Expire=86400&HW-CC-Sign=1784126C26CC170C0DA36DD08B49629F1C2C941D1166C1F38B48EA5FC6F14006)

1. 主进程启动后，在主进程中添加事件观察者A和事件观察者B，其中A包含正常实现的回调处理函数以及事件订阅过滤条件[appEventFilter](<../../../../../../harmonyos-references/Performance Analysis Kit（性能分析服务）/ArkTS API/@ohos.hiviewdfx.hiAppEvent (应用事件打点)/js-apis-hiviewdfx-hiappevent.md#appeventfilter>)，应用发生故障后正常重启会由A的回调处理HiAppEvent事件；B的回调处理函数为空实现，仅用于生成需要保存的事件订阅过滤条件。
...
* FaultLogExtensionAbility被拉起后只有10s的时间用以完成故障处理。超时没有处理完成可以在[onDisconnect](<../../../../../../harmonyos-references/Performance Analysis Kit（性能分析服务）/ArkTS API/@ohos.hiviewdfx.FaultLogExtensionAbility (故障延迟通知)/js-apis-hiviewdfx-faultlogextensionability.md#ondisconnect>)中保存状态。
...
* FaultLogExtensionAbility调用限制的API名单见[附录](<../../../../../../harmonyos-references/Performance Analysis Kit（性能分析服务）/ArkTS API/@ohos.hiviewdfx.FaultLogExtensionAbility (故障延迟通知)/js-apis-hiviewdfx-faultlogextensionability.md#附录>)。
...
API接口使用说明，包括参数使用限制和具体取值范围。请参考[@ohos.hiviewdfx.FaultLogExtensionAbility (故障延迟通知)](<../../../../../../harmonyos-references/Performance Analysis Kit（性能分析服务）/ArkTS API/@ohos.hiviewdfx.FaultLogExtensionAbility (故障延迟通知)/js-apis-hiviewdfx-faultlogextensionability.md>)。
...
| 接口名 | 描述 |
| --- | --- |
| [onConnect(): void](<../../../../../../harmonyos-references/Performance Analysis Kit（性能分析服务）/ArkTS API/@ohos.hiviewdfx.FaultLogExtensionAbility (故障延迟通知)/js-apis-hiviewdfx-faultlogextensionability.md#onconnect>) | 生命周期回调函数，系统连接FaultLogExtensionAbility时触发。 |
| [onDisconnect(): void](<../../../../../../harmonyos-references/Performance Analysis Kit（性能分析服务）/ArkTS API/@ohos.hiviewdfx.FaultLogExtensionAbility (故障延迟通知)/js-apis-hiviewdfx-faultlogextensionability.md#ondisconnect>) | 生命周期回调函数，系统断开FaultLogExtensionAbility时触发。 |
| [onFaultReportReady(): void](<../../../../../../harmonyos-references/Performance Analysis Kit（性能分析服务）/ArkTS API/@ohos.hiviewdfx.FaultLogExtensionAbility (故障延迟通知)/js-apis-hiviewdfx-faultlogextensionability.md#onfaultreportready>) | 生命周期回调函数，系统准备好故障信息后，回调该函数通知ability进行处理。回调函数中的业务逻辑建议不超过10s。 |
...
3. 在“entry > src > main > ets” 路径下，新建faultlogextension/MyFaultLogExtensionAbility.ets文件。新建类MyFaultLogExtensionAbility继承FaultLogExtensionAbility，重写订阅功能相关的三个接口函数，代码示例如下：
   ...
   45. // 开发者可以获取到应用冻屏事件发生时应用的唯一关联id
   46. hilog.info(0x0000, 'testTag', `HiAppEvent eventInfo.params.app_running_unique_id=${eventInfo.params['app_running_unique_id']}`);
   47. // 开发者可以获取到应用冻屏事件发生时应用的包名
   48. hilog.info(0x0000, 'testTag', `HiAppEvent eventInfo.params.bundle_name=${eventInfo.params['bundle_name']}`);
   49. // 开发者可以获取到应用冻屏事件发生时应用的进程名称
   50. hilog.info(0x0000, 'testTag', `HiAppEvent eventInfo.params.process_name=${eventInfo.params['process_name']}`);
   51. // 开发者可以获取到应用冻屏事件发生时应用的进程id
   52. hilog.info(0x0000, 'testTag', `HiAppEvent eventInfo.params.pid=${eventInfo.params['pid']}`);
   53. hilog.info(0x0000, 'testTag', `HiAppEvent eventInfo.params.uid=${eventInfo.params['uid']}`);
   54. hilog.info(0x0000, 'testTag', `HiAppEvent eventInfo.params.uuid=${eventInfo.params['uuid']}`);
   55. // 开发者可以获取到应用冻屏事件发生的异常类型、异常原因
   56. hilog.info(0x0000, 'testTag', `HiAppEvent eventInfo.params.exception=${JSON.stringify(eventInfo.params['exception'])}`);
   57. // 开发者可以获取到应用冻屏事件发生时日志信息
   58. hilog.info(0x0000, 'testTag', `HiAppEvent eventInfo.params.hilog.size=${eventInfo.params['hilog'].length}`);
   59. // 开发者可以获取到应用冻屏事件发生时主线程未处理消息
   60. hilog.info(0x0000, 'testTag', `HiAppEvent eventInfo.params.event_handler=${eventInfo.params['event_handler']}`);
   61. hilog.info(0x0000, 'testTag', `HiAppEvent eventInfo.params.event_handler_size_3s=${eventInfo.params['event_handler_size_3s']}`);
   62. hilog.info(0x0000, 'testTag', `HiAppEvent eventInfo.params.event_handler_size_6s=${eventInfo.params['event_handler_size_6s']}`);
   63. // 开发者可以获取到应用冻屏事件发生时同步binder调用信息
   64. hilog.info(0x0000, 'testTag', `HiAppEvent eventInfo.params.peer_binder=${eventInfo.params['peer_binder']}`);
   65. // 开发者可以获取到应用冻屏事件发生时全量线程调用栈
   66. hilog.info(0x0000, 'testTag', `HiAppEvent eventInfo.params.threads.size=${eventInfo.params['threads'].length}`);
   67. // 开发者可以获取到应用冻屏事件发生时内存信息
   68. hilog.info(0x0000, 'testTag', `HiAppEvent eventInfo.params.memory=${JSON.stringify(eventInfo.params['memory'])}`);
   69. // 开发者可以获取到应用冻屏事件发生时的故障日志文件
   70. hilog.info(0x0000, 'testTag', `HiAppEvent eventInfo.params.external_log=${JSON.stringify(eventInfo.params['external_log'])}`);
   71. hilog.info(0x0000, 'testTag', `HiAppEvent eventInfo.params.log_over_limit=${eventInfo.params['log_over_limit']}`);
   72. }
   73. }
   74. }
   75. });
