...
scraped_at: 2026-06-11T14:53:04+08:00
doc_updated_at: 2026-06-05
content_hash: sha256:62d272510d2a6ae04ba1485172ff155caa1331962f158bdd6a821107e166fadc
---
Bundle Manager（包管理工具，简称bm）是实现应用安装、卸载、更新、查询等功能的工具，bm为开发者提供基本的应用安装包的调试能力。

## 环境要求（hdc工具）

在使用本工具前，开发者需要先获取[hdc工具](../hdc/hdc.md#环境准备)，执行hdc shell。
...
表示当前系统账号的编号，系统账号的相关接口请参考[@ohos.account.osAccount (系统账号管理)](<../../../../../harmonyos-references/基础功能/Basic Services Kit（基础服务）/ArkTS API/账号管理/@ohos.account.osAccount (系统账号管理)/js-apis-osaccount.md>)，下面给出几种常见的系统账号。
...
```
1. bm install [-h] [-p filePath] [-r] [-w waitingTime] [-s hspDirPath] [-u userId] [-d] [-g]
```
...
| -g | 可选参数，安装签名证书类型为debug的应用时自动授予[user\_grant](../../../安全/程序访问控制/应用权限管控/应用权限管控概述/app-permission-mgmt-overview.md#user_grant用户授权)和[manual\_settings](../../../安全/程序访问控制/应用权限管控/应用权限管控概述/app-permission-mgmt-overview.md#manual_settings手动设置授权)权限。  仅对[开发者模式](../../../../编写与调试应用/应用调试/开发者模式/开发者选项/ide-developer-mode.md#section530763213432)下的签名证书类型为debug的应用生效。可以通过[Profile签名文件](https://developer.huawei.com/consumer/cn/doc/app/agc-help-profile-overview-0000002283260125)中的type字段查看签名证书类型。  签名证书类型为debug的应用更新为签名证书类型为release的应用时取消已授予的[user\_grant](../../../安全/程序访问控制/应用权限管控/应用权限管控概述/app-permission-mgmt-overview.md#user_grant用户授权)和[manual\_settings](../../../安全/程序访问控制/应用权限管控/应用权限管控概述/app-permission-mgmt-overview.md#manual_settings手动设置授权)权限。从API version 24开始支持。 |
...
17. # 安装签名证书类型为debug的应用时自动授予user_grant权限和manual_settings权限
18. bm install -p /data/local/tmp/ohos.app.hap -g
```
...
注：hqf文件制作方式可参考[HQF打包指令](../打包拆包工具/打包工具/packing-tool.md#hqf打包指令)。
...
方法一. 使用[自动签名](../../../../编写与调试应用/配置调试签名/ide-signing.md#section18815157237)。在连接设备后，重新为应用进行签名。

方法二. 使用手动签名，请参考[手动签名](../../../../编写与调试应用/配置调试签名/ide-signing.md#section297715173233)。
...
请开发者根据实际场景选择自动签名或者手动签名，例如无法连接互联网的情况下推荐使用手动签名方式，详情参考[使用场景说明](../../../../编写与调试应用/配置调试签名/ide-signing.md#section54361623194519)。

说明

在工程级build-profile.json5文件下的products标签中，signingConfig字段为非必填字段，若该字段缺失，将导致签名失效。详情请参考[products](../../../../构建应用/配置文件/工程级build-profile.json5文件/ide-hvigor-build-profile-app.md#section45865492619)标签下的字段说明。

方法一. 使用[自动签名](../../../../编写与调试应用/配置调试签名/ide-signing.md#section18815157237)。在连接设备后，重新为应用进行签名。

方法二. 使用手动签名，请参考[手动签名](../../../../编写与调试应用/配置调试签名/ide-signing.md#section297715173233)。

方法三. 如果安装APP时报这个错误码，需要在[工程级build-profile.json5文件](../../../../构建应用/配置文件/工程级build-profile.json5文件/ide-hvigor-build-profile-app.md)里配置[packOptions](../../../../构建应用/配置文件/工程级build-profile.json5文件/ide-hvigor-build-profile-app.md#section03812484215)的appWithSignedPkg属性为true，保证APP里的HAP/HSP有签名。
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/77/v3/NqBSYVbcRX2EoGUpBAEdvQ/zh-cn_image_0000002592218934.png?HW-CC-KV=V1&HW-CC-Date=20260611T065125Z&HW-CC-Expire=86400&HW-CC-Sign=C48095CB0CFF4512B3084C281280139D6BB67D5BDD819F5DA30CE1BAD390BC03)
...
1. [app.json5配置文件](../../../../基础入门/开发基础知识/应用配置文件（Stage模型）/app.json5配置文件/app-configuration-file.md#配置文件标签)中的bundleName、[module.json5配置文件](../../../../基础入门/开发基础知识/应用配置文件（Stage模型）/module.json5配置文件/module-configuration-file.md#配置文件标签)中name不符合命名规则。
...
* 方法一：先通过[bm install -p](bm-tool.md#安装命令install)命令安装依赖的动态共享包（HSP）模块，再在应用运行配置页勾选Keep Application Data，点击OK保存配置，再运行/调试。
  ...
  ![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/11/v3/XRDcWRu4SySq57SPWtMvBw/zh-cn_image_0000002592378868.png?HW-CC-KV=V1&HW-CC-Date=20260611T065125Z&HW-CC-Expire=86400&HW-CC-Sign=441367F49B311203C200299AE34307FFAF9C6F50E88460B322254D0EFF540311)
* 方法二：在运行配置页，选择Deploy Multi Hap标签页，勾选Deploy Multi Hap Packages，选择依赖的模块，点击OK保存配置，再进行运行/调试。

  ![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/9a/v3/tfW1EDAPRBi0KYtgLTrw6Q/zh-cn_image_0000002622858375.png?HW-CC-KV=V1&HW-CC-Date=20260611T065125Z&HW-CC-Expire=86400&HW-CC-Sign=20D4FA46B8C5A81DBEE7B11F25ABDF197D8D87FBB5ADBD0B66EBC1FAB4462EA4)
* 方法三：单击Run > Edit Configurations，在General中，勾选Auto Dependencies。点击OK保存配置，再运行/调试。

  ![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/4e/v3/162JXZyNSCiAry_dKNl8JA/zh-cn_image_0000002622698497.png?HW-CC-KV=V1&HW-CC-Date=20260611T065125Z&HW-CC-Expire=86400&HW-CC-Sign=7EF783D5F866022344CA2B47EEFCA6B040BFD08EC55D939E91C899ACAB34480F)
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/5d/v3/JLPKlJQLTu2f9pPAzltuzQ/zh-cn_image_0000002592218936.png?HW-CC-KV=V1&HW-CC-Date=20260611T065125Z&HW-CC-Expire=86400&HW-CC-Sign=E88964CA2DDD575453103D08487C281AFA81D72EEE2095D3CACAA6659BEBC62A)
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/2f/v3/FTAqWfkNSAm9DQNnWd_-Zw/zh-cn_image_0000002592378870.png?HW-CC-KV=V1&HW-CC-Date=20260611T065125Z&HW-CC-Expire=86400&HW-CC-Sign=FD5CABD5F211DA393AB07A2ACC7CF0F59994C055177577951FDBB5EE4116B3AC)
...
* 方法1：请参考[app.json5配置文件](../../../../基础入门/开发基础知识/应用配置文件（Stage模型）/app.json5配置文件/app-configuration-file.md)和[module.json5配置文件](../../../../基础入门/开发基础知识/应用配置文件（Stage模型）/module.json5配置文件/module-configuration-file.md)查看并补充必填字段。
* 方法2：通过hilog日志判断缺失字段。
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/30/v3/tgvuDe5wRueu9RQGULVkzg/zh-cn_image_0000002622858377.png?HW-CC-KV=V1&HW-CC-Date=20260611T065125Z&HW-CC-Expire=86400&HW-CC-Sign=278956481A03F1BAF8444A0E0F33C8587D36FEE67948CBF7069A36A2094B2091)
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/99/v3/kUc9RlJgQf-eqUx3C8reSA/zh-cn_image_0000002622698499.png?HW-CC-KV=V1&HW-CC-Date=20260611T065125Z&HW-CC-Expire=86400&HW-CC-Sign=1E4AF39B9AF3B9CD00284138B3E97989D76C8DAB19E738A8F9988A76ACDA4E78)
...
* 场景二：在[build-profile.json5](../../../../构建应用/配置文件/工程级build-profile.json5文件/ide-hvigor-build-profile-app.md#section153288223224)文件中，配置的profile文件与.cer文件不匹配。
...
1. 使用[自动签名](../../../../编写与调试应用/配置调试签名/ide-signing.md#section18815157237)。在连接设备后，重新为应用进行签名。
...
查看应用安装的设备存储空间并清理，保证满足安装所需空间，再重试安装应用。模拟器上存储空间清理之后仍然失败，请尝试创建新的模拟器设备，并设置更大的存储空间，详情请参考[创建新的模拟器](../../../../编写与调试应用/使用模拟器运行应用/管理模拟器/创建模拟器/ide-emulator-create.md#section1764055173710)。
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/f5/v3/OQI_tFeaSzibTsnDSlw3rw/zh-cn_image_0000002592218938.png?HW-CC-KV=V1&HW-CC-Date=20260611T065125Z&HW-CC-Expire=86400&HW-CC-Sign=ABD5676B789E62E6B7F8B8E2889A63AAD3795147741F78C58AB4262615481AE8)
...
应用APL等级为normal，只能使用APL等级为normal的权限，如果使用了system\_basic或system\_core等级的权限，将导致报错，具体请参考[权限机制中的基本概念](../../../安全/程序访问控制/应用权限管控/应用权限管控概述/app-permission-mgmt-overview.md#权限机制中的基本概念)。

**处理步骤**

根据报错描述中的权限名称在[应用权限列表](../../../安全/程序访问控制/应用权限管控/应用权限列表/app-permissions.md)中排查权限是否存在。

* 如果不存在，需要查看需要该权限的API接口的文档说明，以确保权限支持申请。例如：[setDevicePairingConfirmation](<../../../../../harmonyos-references/网络/Connectivity Kit（短距通信服务）/ArkTS API/@ohos.bluetooth.connection (蓝牙connection模块)/js-apis-bluetooth-connection.md#connectionsetdevicepairingconfirmation>)接口所需权限中，ohos.permission.MANAGE\_BLUETOOTH仅系统应用支持申请，此时请更换为ohos.permission.ACCESS\_BLUETOOTH权限。
* 如果存在，请查看对应的权限文档的要求，申请权限，同时请关注权限是否支持申请。例如：[受限开放权限](../../../安全/程序访问控制/应用权限管控/应用权限列表/受限开放权限/restricted-permissions.md)请参考文档[申请受限权限](../../../安全/程序访问控制/应用权限管控/申请应用权限/申请受限权限/declare-permissions-in-acl.md)。[企业类应用可用权限](../../../安全/程序访问控制/应用权限管控/应用权限列表/企业类应用可用权限/permissions-for-enterprise-apps.md)仅企业类应用支持申请，三方应用不支持。
...
多个HAP/HSP中配置的[SysCap](../../../../../harmonyos-references/系统能力SystemCapability使用指南/syscap.md#syscap的用途)不一致。
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/4f/v3/dZ_rojniTrWQOKO_n5EXug/zh-cn_image_0000002592378872.png?HW-CC-KV=V1&HW-CC-Date=20260611T065125Z&HW-CC-Expire=86400&HW-CC-Sign=E14ECF58C70143F9086A0FC589A440970D78F049F284419DB75781A86012F662)
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/44/v3/drJPrg0FT-SJ_6Z2NHhZwQ/zh-cn_image_0000002622858379.png?HW-CC-KV=V1&HW-CC-Date=20260611T065125Z&HW-CC-Expire=86400&HW-CC-Sign=9AF4D808BDDE76F6C43A6EA1D86BD74F36474B9C49F26B4429DCFA6606099215)
...
1. 设备上已安装的应用与新安装的应用中签名不一致或者多个包（HAP和HSP）之间的签名存在差异。若两个应用的签名[密钥](../../../../编写与调试应用/配置调试签名/ide-signing.md#section462703710326)或[APP ID](https://developer.huawei.com/consumer/cn/doc/app/agc-help-create-app-0000002247955506#section16423184171915)中至少有一项相同，则认为它们的签名一致。如果在DevEco Studio的“Edit Configurations”中勾选了“Keep Application Data”（即不卸载应用，直接覆盖安装），并且重新进行了签名，将导致该报错。
2. 如果某个应用被卸载但是保留了数据，那么后面安装相同包名的应用时，需要校验其签名信息的一致性。如果两者签名信息中的[密钥](../../../../编写与调试应用/配置调试签名/ide-signing.md#section462703710326)和[APP ID](https://developer.huawei.com/consumer/cn/doc/app/agc-help-create-app-0000002247955506#section16423184171915)都不一致，则会导致该报错。

**处理步骤**

1. 请卸载设备上已安装的应用（PC/2in1设备需要确保所有用户下都卸载完成，手机/平板侧需要关注隐私空间和主用户下是否卸载完成），或取消勾选“Keep Application Data”后，重新安装新的应用。
2. 如果是因不同团队提供的HSP导致签名不一致问题，可以采用[集成态HSP](../../../../基础入门/开发基础知识/典型场景的开发指导/集成态HSP/integrated-hsp.md)的方式统一提供HSP；在多HAP包的情况下，必须确保所有HAP包的签名一致。
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/84/v3/kzl3Z_8wT1KZgE3PIlFwBQ/zh-cn_image_0000002622698501.png?HW-CC-KV=V1&HW-CC-Date=20260611T065125Z&HW-CC-Expire=86400&HW-CC-Sign=9BED58E9C15E771FD02743C0E52D14B27B9E0AAE4439104D5655FBC686E1A1F6)
...
* 场景一：用户导入了三方提供的HSP模块，且该HSP既非[集成态HSP](../../../../基础入门/开发基础知识/典型场景的开发指导/集成态HSP/integrated-hsp.md)，又非同包名的HSP，造成包名不一致。
* 场景二：用户使用了错误的签名文件（后缀为.p7b）进行签名，造成包名不一致。

**处理步骤**

* 场景一：HSP只能给同包名的应用使用，只有集成态HSP可以给不同包名的应用使用。需要用户与三方开发者确认，三方开发者应提供集成态HSP、或同包名的HSP给用户使用。
* 场景二：检查签名流程和签名证书，参考[应用/元服务签名](../../../../编写与调试应用/配置调试签名/ide-signing.md)。
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/d4/v3/abqSS9RrR7C7wVnjMNpnOg/zh-cn_image_0000002592218940.png?HW-CC-KV=V1&HW-CC-Date=20260611T065125Z&HW-CC-Expire=86400&HW-CC-Sign=9AC2C57D3F82E19F2027FB45F4B4C0635A6ABDC3693907D450B5C38315342AC5)
...
1. 检查应用定义的proxyData内容是否符合要求，参考[proxyData标签](../../../../基础入门/开发基础知识/应用配置文件（Stage模型）/module.json5配置文件/module-configuration-file.md#proxydata标签)。
...
使用[自动签名](../../../../编写与调试应用/配置调试签名/ide-signing.md#section18815157237)或者[手动签名](../../../../编写与调试应用/配置调试签名/ide-signing.md#section297715173233)重新签名后安装调试。
...
使用[自动签名](../../../../编写与调试应用/配置调试签名/ide-signing.md#section18815157237)或者重新申请签名证书，在[手动签名](../../../../编写与调试应用/配置调试签名/ide-signing.md#section297715173233)重新签名后安装调试。
...
校验唯一标识[appIdentifier](../../../../基础入门/开发基础知识/应用程序包常见问题/common-problem-of-application.md#什么是appidentifier)不一样，导致安装失败。
...
1. 确认签名文件p7b中apl字段是否有误。
   ...
   ![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/9f/v3/SCNuKxDNTAybFSute5_Tmw/zh-cn_image_0000002592378874.png?HW-CC-KV=V1&HW-CC-Date=20260611T065125Z&HW-CC-Expire=86400&HW-CC-Sign=FAF2B40EB03D03687B2CC96D9623C649427AE43B8EBED19D077BE731C6D87F6B)
2. 若apl字段有误，修改UnsgnedReleasedProfileTemplate.json文件中apl字段，并重新签名。

   ![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/e5/v3/fZAOiyoUR4OYn5yL7VuBfA/zh-cn_image_0000002622858381.png?HW-CC-KV=V1&HW-CC-Date=20260611T065125Z&HW-CC-Expire=86400&HW-CC-Sign=4588036DB7B484D50D5D34156703256758F375ADD00F7ED70896136389714EDD)
...
检查overlay应用中的[app.json5配置文件](../../../../基础入门/开发基础知识/应用配置文件（Stage模型）/app.json5配置文件/app-configuration-file.md)的targetBundleName字段是否配置。
...
检查overlay应用中的[module.json5配置文件](../../../../基础入门/开发基础知识/应用配置文件（Stage模型）/module.json5配置文件/module-configuration-file.md)的targetModuleName字段是否配置。
...
当前设备不允许安装[Profile签名文件](https://developer.huawei.com/consumer/cn/doc/app/agc-help-profile-overview-0000002283260125)中如下两种类型的应用：enterprise\_mdm（企业MDM应用）、enterprise\_normal（普通企业应用）。Profile签名文件类型的取值及含义请参考[ApplicationInfo.appDistributionType](<../../../../../harmonyos-references/Ability Kit（程序框架服务）/ArkTS API/接口依赖的元素及定义/bundleManager/ApplicationInfo/js-apis-bundlemanager-applicationinfo.md#applicationinfo-1>)。
...
应用配置的[设备类型](../../../../基础入门/开发基础知识/应用配置文件（Stage模型）/module.json5配置文件/module-configuration-file.md#devicetypes标签)或者[requiredDeviceFeatures标签](../../../../基础入门/开发基础知识/应用配置文件（Stage模型）/module.json5配置文件/module-configuration-file.md#requireddevicefeatures标签)配置的设备特性不支持安装。
...
1. 使用[自动签名](../../../../编写与调试应用/配置调试签名/ide-signing.md#section18815157237)或者[手动签名](../../../../编写与调试应用/配置调试签名/ide-signing.md#section297715173233)重新签名后安装调试。
...
通过重新签名，确保应用签名信息中的[密钥](../../../../编写与调试应用/配置调试签名/ide-signing.md#section462703710326)和[APP ID](https://developer.huawei.com/consumer/cn/doc/app/agc-help-create-app-0000002247955506#section16423184171915)至少有一项与预置应用保持一致。
...
修改安装应用的[bundleName](../../../../基础入门/开发基础知识/应用配置文件（Stage模型）/app.json5配置文件/app-configuration-file.md#配置文件标签)，确保与预置应用的bundleName不一致。
...
方式二：重新[自动签名](../../../../编写与调试应用/配置调试签名/ide-signing.md#section18815157237)。
...
1. 参考[权限申请指导](../../../安全/程序访问控制/应用权限管控/申请应用权限/声明权限/declare-permissions.md)申请[ohos.permission.kernel.SUPPORT\_PLUGIN权限](../../../安全/程序访问控制/应用权限管控/应用权限列表/受限开放权限/restricted-permissions.md#ohospermissionkernelsupport_plugin)。
...
检查[module.json5](../../../../基础入门/开发基础知识/应用配置文件（Stage模型）/module.json5配置文件/module-configuration-file.md)的name字段是否为空。
...
重新签名，使多个HAP包签名信息一致。参考[应用/元服务签名](../../../../编写与调试应用/配置调试签名/ide-signing.md)。
...
[module.json](../../../../基础入门/开发基础知识/应用配置文件（Stage模型）/module.json5配置文件/module-configuration-file.md)、[pack.info](../../../../构建应用/概述/构建产物说明/ide-compile-build.md#section43931054115513)等配置文件缺失。
...
[module.json](../../../../基础入门/开发基础知识/应用配置文件（Stage模型）/module.json5配置文件/module-configuration-file.md)、[pack.info](../../../../构建应用/概述/构建产物说明/ide-compile-build.md#section43931054115513)等配置文件格式异常。
...
[module.json](../../../../基础入门/开发基础知识/应用配置文件（Stage模型）/module.json5配置文件/module-configuration-file.md)、[pack.info](../../../../构建应用/概述/构建产物说明/ide-compile-build.md#section43931054115513)等配置文件存在数据类型错误的字段。
...
[module.json](../../../../基础入门/开发基础知识/应用配置文件（Stage模型）/module.json5配置文件/module-configuration-file.md)、[pack.info](../../../../构建应用/概述/构建产物说明/ide-compile-build.md#section43931054115513)等配置文件存在字符串长度或者数组大小过大的字段。
...
安装过程中，解析安装包获取[SysCap](../../../../../harmonyos-references/系统能力SystemCapability使用指南/syscap.md#syscap的用途)信息失败。
...
1. 将设备或模拟器与DevEco Studio进行连接，具体指导及要求可查看[运行应用/元服务](../../../../编写与调试应用/使用本地真机运行应用/ide-run-device.md)。
2. 在命令行执行如下[hdc命令](bm-tool.md#环境要求hdc工具)，查询设备支持的Abi列表。
   ...
   ```
3. 根据查询返回结果，检查[模块级build-profile.json5](../../../../构建应用/配置文件/模块级build-profile.json5文件/ide-hvigor-build-profile.md)文件中的[“abiFilters”参数](<../../../../NDK开发/硬件兼容性/HarmonyOS ABI/ohos-abi.md#在编译架构中指定abi>)中的配置，规则如下：
...
使用[param工具](../param工具/param-tool.md)设置const.bms.support\_plugin的值为true，即执行hdc shell param set const.bms.support\_plugin true。
...
检查多HSP之间的包信息是否一致，包括[app.json5配置文件](../../../../基础入门/开发基础知识/应用配置文件（Stage模型）/app.json5配置文件/app-configuration-file.md#配置文件标签)中bundleName、bundleType、versionCode、apiReleaseType字段。
...
方案一：重新签名，签名过程中，请参考[自动签名](../../../../编写与调试应用/配置调试签名/ide-signing.md#section18815157237)的支持ACL权限、或者[手动签名](../../../../编写与调试应用/配置调试签名/ide-signing.md#section297715173233)的使用ACL的签名配置指导进行配置，确保待安装应用与已安装应用配置一致。
...
重新签名，签名过程中，请参考[自动签名](../../../../编写与调试应用/配置调试签名/ide-signing.md#section18815157237)的支持ACL权限、或者[手动签名](../../../../编写与调试应用/配置调试签名/ide-signing.md#section297715173233)的使用ACL的签名配置指导进行配置，使多个HAP包签名信息中allowed-acls的U1Enabled信息一致。
...
重新[打包APP](../打包拆包工具/打包工具/packing-tool.md#app打包指令)。
...
如需要适配当前设备，请在应用设备类型配置中增加当前[设备类型](../../../../基础入门/开发基础知识/应用配置文件（Stage模型）/module.json5配置文件/module-configuration-file.md#devicetypes标签)，然后重新[打包APP](../打包拆包工具/打包工具/packing-tool.md#app打包指令)。
...
1. 在应用的module.json5配置文件中配置了[executableBinaryPaths标签](../../../../基础入门/开发基础知识/应用配置文件（Stage模型）/module.json5配置文件/module-configuration-file.md#executablebinarypaths标签)，但是应用未配置解压模式。
2. 该设备不支持安装配置了[executableBinaryPaths标签](../../../../基础入门/开发基础知识/应用配置文件（Stage模型）/module.json5配置文件/module-configuration-file.md#executablebinarypaths标签)的应用。

**处理步骤**

1. 配置应用为解压模式，即在应用的[module.json5配置文件](../../../../基础入门/开发基础知识/应用配置文件（Stage模型）/module.json5配置文件/module-configuration-file.md#配置文件标签)中设置compressNativeLibs标签为true。
2. 更换为PC/2in1设备。
...
[开发者模式](../../../../编写与调试应用/应用调试/开发者模式/开发者选项/ide-developer-mode.md#section530763213432)下使用[-g参数](bm-tool.md#安装命令install)授权签名证书类型为非debug的应用。可以通过[Profile签名文件](https://developer.huawei.com/consumer/cn/doc/app/agc-help-profile-overview-0000002283260125)中的type字段来查看签名证书类型。
