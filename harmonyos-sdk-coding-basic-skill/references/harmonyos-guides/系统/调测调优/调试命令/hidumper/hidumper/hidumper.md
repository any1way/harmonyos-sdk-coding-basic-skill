...
scraped_at: 2026-06-01T11:26:21+08:00
doc_updated_at: 2026-04-30
content_hash: sha256:10b36abd915e89823ca2b7a63578304b8ff134b39bb66ce6ef21d92a5168f681
---
...
| [-e](hidumper.md#获取系统故障日志) | 获取记录的故障日志。输出日志的规格可参考：[CppCrash](<../../../Performance Analysis Kit（性能分析服务）/故障检测/崩溃检测/Cpp Crash（进程崩溃）检测/cppcrash-guidelines.md#日志规格>)、[JSCrash](<../../../Performance Analysis Kit（性能分析服务）/故障检测/崩溃检测/JS Crash（进程崩溃）检测/jscrash-guidelines.md#日志规格>)、[AppFreeze](<../../../Performance Analysis Kit（性能分析服务）/故障检测/AppFreeze（应用冻屏）检测/appfreeze-guidelines.md#日志规格>)。 |
...
| [-e --print [process\_name] [-n num] [--since timestamp] [--until timestamp]](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/hidumper#获取异常退出故障日志) | 获取异常退出故障日志。输出日志的规格可参考：[CppCrash](<../../../Performance Analysis Kit（性能分析服务）/故障检测/崩溃检测/Cpp Crash（进程崩溃）检测/cppcrash-guidelines.md#日志规格>)、[JSCrash](<../../../Performance Analysis Kit（性能分析服务）/故障检测/崩溃检测/JS Crash（进程崩溃）检测/jscrash-guidelines.md#日志规格>)、[AppFreeze](<../../../Performance Analysis Kit（性能分析服务）/故障检测/AppFreeze（应用冻屏）检测/appfreeze-guidelines.md#日志规格>)。  **说明**：从API version 22开始，支持该参数。 |
| [-e --print record\_id](hidumper.md#获取异常退出故障日志) | 获取指定异常退出记录id的故障日志。输出日志的规格可参考：[CppCrash](<../../../Performance Analysis Kit（性能分析服务）/故障检测/崩溃检测/Cpp Crash（进程崩溃）检测/cppcrash-guidelines.md#日志规格>)、[JSCrash](<../../../Performance Analysis Kit（性能分析服务）/故障检测/崩溃检测/JS Crash（进程崩溃）检测/jscrash-guidelines.md#日志规格>)、[AppFreeze](<../../../Performance Analysis Kit（性能分析服务）/故障检测/AppFreeze（应用冻屏）检测/appfreeze-guidelines.md#日志规格>)。  **说明**：从API version 22开始，支持该参数。 |
...
| [--mem-smaps pid [-v]](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/hidumper#查询进程内存) | 获取pid内存统计信息，数据来源于/proc/pid/smaps，使用-v指定更多详细信息。（仅支持导出[debug版本应用](<../../../Performance Analysis Kit（性能分析服务）/Performance Analysis Kit术语/performance-analysis-kit-terminology.md#debug版本应用>)）  **说明**：从API version 20开始，支持该参数。 |
| [--mem-jsheap pid [-T tid] [--gc] [--leakobj] [--raw] [--clean]](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/hidumper#查询虚拟机堆内存) | 必选参数pid。触发ArkTS应用JS线程的gc和堆内存快照导出。文件命名格式为：hidumper-jsheap-进程号-JS线程号-时间戳，文件内容为JSON结构的JS堆快照。  指定线程tid时，仅触发该线程的gc和堆内存快照导出。  指定--gc时，仅触发gc，不导出快照。  指定--leakobj时，应用开启泄露检测可获取泄露对象列表。  指定--raw时，堆快照以rawheap格式导出。  指定--clean时，快照导出后触发清理nodeId节点信息。  **说明**：  从API version 19开始，支持--raw参数。  从API version 24开始，支持--clean参数。 |

## 查询内存信息

HiDumper内存信息查询模块主要用于查看系统整机内存使用情况和单个进程内存使用情况。开发者需要了解相关的[内存基础知识](../../../../../../best-practices/性能/性能分析/分析内存占用问题/内存基础知识/bpta-memory-basic-knowledge.md)。
...
hidumper --mem-smaps [pid] [-v]命令调试的进程应为“使用调试证书签名的应用”，同[debug版本应用](<../../../Performance Analysis Kit（性能分析服务）/Performance Analysis Kit术语/performance-analysis-kit-terminology.md#debug版本应用>)。
...
hidumper --mem-jsheap pid [-T tid] [--gc] [--leakobj] [--raw] [--clean]命令调试的进程应为“使用调试证书签名的应用”，同[debug版本应用](<../../../Performance Analysis Kit（性能分析服务）/Performance Analysis Kit术语/performance-analysis-kit-terminology.md#debug版本应用>)。
...
* 可使用hidumper --mem-jsheap pid [-T tid] --raw获取指定进程或指定JS线程的虚拟机堆内存，生成的堆内存文件为rawheap格式，文件命名为hidumper-jsheap-进程号-JS线程号-时间戳.rawheap。rawheap的解析转换可参考使用：[rawheap-translator工具](../../命令行工具/rawheap-translator工具/rawheap-translator.md)。
...
* 可使用hidumper --mem-jsheap pid --leakobj获取指定进程的虚拟机堆内存和泄漏对象信息，文件命名为：hidumper-leaklist-进程号-时间戳。
  ...
  获取指定进程的虚拟机堆内存和泄露对象信息的前提是应用已通过[@ohos.hiviewdfx.jsLeakWatcher](<../../../../../../harmonyos-references/调测调优/Performance Analysis Kit（性能分析服务）/ArkTS API/@ohos.hiviewdfx.jsLeakWatcher (ArkTS泄漏检测)/js-apis-jsleakwatcher.md>)接口开启了泄漏检测功能。

  具体使用步骤为：

  1. 应用调用[jsLeakWatcher.enable](<../../../../../../harmonyos-references/调测调优/Performance Analysis Kit（性能分析服务）/ArkTS API/@ohos.hiviewdfx.jsLeakWatcher (ArkTS泄漏检测)/js-apis-jsleakwatcher.md#jsleakwatcherenable>)接口。
  2. 应用调用[jsLeakWatcher.watch](<../../../../../../harmonyos-references/调测调优/Performance Analysis Kit（性能分析服务）/ArkTS API/@ohos.hiviewdfx.jsLeakWatcher (ArkTS泄漏检测)/js-apis-jsleakwatcher.md#jsleakwatcherwatch>)接口。
...
* 可使用hidumper --mem-jsheap pid --clean命令获取指定进程虚拟机堆内存并触发清理nodeId节点信息。该命令不会生成任何文件，执行成功不会有命令回显。
  ...
  ```
  1. $ hidumper --mem-jsheap 64949 --clean  -> 64949 为目标应用进程号
  ```

上述生成的文件，可以通过hdc[文件传输](../../hdc/hdc.md#文件传输)命令从设备中获取。
...
以下异常退出原因，可参考[应用终止分析思路和分析步骤](<../../../Performance Analysis Kit（性能分析服务）/故障检测/App Killed（应用终止）检测/appkilled-guidelines.md#分析思路和分析步骤>)进行问题排查处理。
...
仅支持通过[Navigation](<../../../../../应用框架/ArkUI（方舟UI框架）/UI开发 (ArkTS声明式开发范式)/设置组件导航和页面路由/组件导航(Navigation) (推荐)/Navigation基础架构介绍/arkts-navigation-architecture.md>)组件实现页面路由的应用。
...
hidumper --mem 系列命令与[HiDebug](<../../../../../../harmonyos-references/调测调优/Performance Analysis Kit（性能分析服务）/ArkTS API/@ohos.hidebug (Debug调试)/js-apis-hidebug.md>)接口获取到的内存信息存在差异。
...
| [hidebug.getAppNativeMemInfo](<../../../../../../harmonyos-references/调测调优/Performance Analysis Kit（性能分析服务）/ArkTS API/@ohos.hidebug (Debug调试)/js-apis-hidebug.md#hidebuggetappnativememinfo12>) | 获取**应用自身**的内存使用情况 | 系统节点：/proc/pid/smaps\_rollup | 否 |

若想通过Hidebug获取图形内存，请参考[HiDebug能力概述](<../../../Performance Analysis Kit（性能分析服务）/系统调试信息获取/HiDebug能力概述/hidebug-guidelines.md>)中的相关说明。
...
hidumper --mem-jsheap [pid] --leakobj 命令依赖[JsLeakWatcher](<../../../../../../harmonyos-references/调测调优/Performance Analysis Kit（性能分析服务）/ArkTS API/@ohos.hiviewdfx.jsLeakWatcher (ArkTS泄漏检测)/js-apis-jsleakwatcher.md>)。应用需通过JsLeakWatcher接口开启泄漏检测功能。具体步骤参考：[查询虚拟机堆内存](hidumper.md#查询虚拟机堆内存)。
