...
scraped_at: 2026-06-11T14:48:37+08:00
doc_updated_at: 2026-05-26
content_hash: sha256:1a78eb71ce52650e880be52809787d257ebf95d76b8d990159df5443f83248da
---
## 概述
...
1. 开发前准备。
   ...
   多网并发能力在设备类型、网络状态及开发方案的选择上存在一定限制，具体接入要求可参考[多网发起和释放](../多网发起和释放/networkboost-netmultipath-request-release.md)，开发者也可以在调试过程中通过[多网请求错误码](<../../../../../../harmonyos-references/网络/Network Boost Kit（网络加速服务）/ArkTS API/netHandover（连接迁移）/networkboost-nethandover.md#nethandoverrequestmultipath>)分析。
2. 发起多网并发。

   (1) 权限配置：应用使用多网并发等网络加速能力，连接迁移能力部分接口需要ohos.permission.LINKTURBO权限，权限配置方法参见[申请权限步骤](../../开发准备/networkboost-preparations.md#申请权限步骤)。同时此权限为受限ACL权限需要特别配置和申请，具体操作步骤参考[配置签名](../../开发准备/networkboost-preparations.md#配置签名)和[受限ACL权限申请](../../开发准备/networkboost-preparations.md#受限acl权限申请)。

   (2) 发起流程：多网并发能力发起的核心步骤包括[netBoost.setSceneDesc()](<../../../../../../harmonyos-references/网络/Network Boost Kit（网络加速服务）/ArkTS API/netBoost（网络加速）/networkboost-netboost.md#netboostsetscenedesc>)设置多网场景、[netHandover.getMultiPathQuotaStats()](<../../../../../../harmonyos-references/网络/Network Boost Kit（网络加速服务）/ArkTS API/netHandover（连接迁移）/networkboost-nethandover.md#nethandovergetmultipathquotastats>)查询多网配额、[netHandover.requestMultiPath()](<../../../../../../harmonyos-references/网络/Network Boost Kit（网络加速服务）/ArkTS API/netHandover（连接迁移）/networkboost-nethandover.md#nethandoverrequestmultipath>)请求发起多网以及[netHandover.on('multiPathStateChange')](<../../../../../../harmonyos-references/网络/Network Boost Kit（网络加速服务）/ArkTS API/netHandover（连接迁移）/networkboost-nethandover.md#nethandoveronmultipathstatechange>)多网状态监听，详细实现可参考[代码实现](networkboost-netmultipath-network-turbo.md#代码实现)中的多网发起与接入。
3. 获取多网NetHandle并绑定Socket。

   (1) 成功发起多网请求后，可以在[netHandover.on('multiPathStateChange')](<../../../../../../harmonyos-references/网络/Network Boost Kit（网络加速服务）/ArkTS API/netHandover（连接迁移）/networkboost-nethandover.md#nethandoveronmultipathstatechange>)回调中监听并获取到可用的[NetHandle](<../../../../../../harmonyos-references/网络/Network Kit（网络服务）/ArkTS API/@ohos.net.connection (网络连接管理)/js-apis-net-connection.md#nethandle>)数据网络句柄对象。

   (2) 针对需要在多网路径上传输的数据，创建对应的TCP Socket（详见[TCPSocket](<../../../../../../harmonyos-references/网络/Network Kit（网络服务）/ArkTS API/@ohos.net.socket (Socket连接)/js-apis-socket.md#tcpsocket>)）/UDP Socket（详见[UDPSocket](<../../../../../../harmonyos-references/网络/Network Kit（网络服务）/ArkTS API/@ohos.net.socket (Socket连接)/js-apis-socket.md#udpsocket>)），通过[bindSocket()](<../../../../../../harmonyos-references/网络/Network Kit（网络服务）/ArkTS API/@ohos.net.connection (网络连接管理)/js-apis-net-connection.md#bindsocket9>)方法将Socket对象绑定到NetHandle对应的网络上。
4. Socket发起网络传输任务。

   绑定网络后，TCP Socket/UDP Socket进行[数据传输](<../../../Network Kit（网络服务）/访问网络/使用Socket访问网络/socket-connection.md#应用通过tcp-socket-server进行数据传输>)时将会使用对应的网络通路，多个Socket使用不同的网络通路同时发起网络传输任务就可以起到多网并发的效果。
...
2. [@ohos.net.connection](<../../../../../../harmonyos-references/网络/Network Kit（网络服务）/ArkTS API/@ohos.net.connection (网络连接管理)/js-apis-net-connection.md>)：网络连接管理，网络通路获取。
3. [Socket数据传输](<../../../Network Kit（网络服务）/访问网络/使用Socket访问网络/socket-connection.md>)：创建网络连接，执行网络传输任务。
4. [TaskPool](../../../../../应用框架/ArkTS（方舟编程语言）/ArkTS并发/多线程并发/TaskPool简介/taskpool-introduction.md)：管理多线程网络并发任务。
5. [@ohos.file.fs](<../../../../../../harmonyos-references/Core File Kit（文件基础服务）/ArkTS API/@ohos.file.fs (文件管理)/js-apis-file-fs.md>)：负责文件管理，包括文件分片、大小查询等。

### 开发步骤

![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/30/v3/wvzhYm0WTsSyuvQ_P_9NGw/zh-cn_image_0000002622858297.png?HW-CC-KV=V1&HW-CC-Date=20260611T064835Z&HW-CC-Expire=86400&HW-CC-Sign=364A2153042E123B9C89DB0F70D02A5619428D162BFD543FDD149A8FC488BFCA)

### 代码实现

1. 多网发起与接入。

   根据当前网络传输任务通过[netBoost.setSceneDesc()](<../../../../../../harmonyos-references/网络/Network Boost Kit（网络加速服务）/ArkTS API/netBoost（网络加速）/networkboost-netboost.md#netboostsetscenedesc>)设置多网场景，帮助系统进行多网并发管控和业务时长分析，当前对于文件分片上传任务选择'upload'场景。
   ...
   通过[netHandover.getMultiPathQuotaStats()](<../../../../../../harmonyos-references/网络/Network Boost Kit（网络加速服务）/ArkTS API/netHandover（连接迁移）/networkboost-nethandover.md#nethandovergetmultipathquotastats>)获取当前多网配额，包括次数和时长等，应用可按需添加业务逻辑如提示用户剩余配额等。
   ...
   通过[netHandover.requestMultiPath()](<../../../../../../harmonyos-references/网络/Network Boost Kit（网络加速服务）/ArkTS API/netHandover（连接迁移）/networkboost-nethandover.md#nethandoverrequestmultipath>)请求开启多网，支持Wi-Fi和蜂窝并发以及主卡和副卡并发，不支持开发者指定并发组合，并发组合由系统决定。
   ...
   多网请求成功后还需要使用[netHandover.on('multiPathStateChange')](<../../../../../../harmonyos-references/网络/Network Boost Kit（网络加速服务）/ArkTS API/netHandover（连接迁移）/networkboost-nethandover.md#nethandoveronmultipathstatechange>)监听多网状态，确认多网络通路的使能与可用情况，记录可用的NetHandle信息。
...
2. 多网通路绑定Socket。
   ...
   封装网络查询函数，通过[connection.getAllNetsSync()](<../../../../../../harmonyos-references/网络/Network Kit（网络服务）/ArkTS API/@ohos.net.connection (网络连接管理)/js-apis-net-connection.md#connectiongetallnetssync10>)获取当前所有网络信息。由于网络传输任务一般放在子线程实现，而当前无法跨线程传递Socket对象和NetHandle对象，为了在子线程实现网络与Socket绑定需要获取所有网络信息，再通过之前请求多网后记录的NetHandle对象的ID找到指定网络在执行绑定操作；如果应用业务不涉及多线程问题可直接绑定上一步多网状态监听中获取到的NetHandle。
...
3. 文件分片处理。
   ...
   通过[fileIo.statSync()](<../../../../../../harmonyos-references/Core File Kit（文件基础服务）/ArkTS API/@ohos.file.fs (文件管理)/js-apis-file-fs.md#fileiostatsync>)获取待上传文件的大小等信息，按照计算出的分片规格对文件进行分片处理。
...
4. 多Socket并发上传不同文件片段。
   ...
   封装网络上传接口，包括创建TCP Socket实例并绑定到指定网络NetHandle、设置网络请求头等配置、通过[TCPSocket.send()](<../../../../../../harmonyos-references/网络/Network Kit（网络服务）/ArkTS API/@ohos.net.socket (Socket连接)/js-apis-socket.md#send-2>)方法发送请求至服务端并通过[TCPSocket.on('message')](<../../../../../../harmonyos-references/网络/Network Kit（网络服务）/ArkTS API/@ohos.net.socket (Socket连接)/js-apis-socket.md#onmessage-1>)监听响应。
...
6. 释放资源。
   ...
   通过[netHandover.releaseMultiPath()](<../../../../../../harmonyos-references/网络/Network Boost Kit（网络加速服务）/ArkTS API/netHandover（连接迁移）/networkboost-nethandover.md#nethandoverreleasemultipath>)释放多网，注销相关的监听方法。
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/98/v3/GxPo_JxRS5ucSjPb74rb4Q/zh-cn_image_0000002622698419.png?HW-CC-KV=V1&HW-CC-Date=20260611T064835Z&HW-CC-Expire=86400&HW-CC-Sign=06DF4F1EAD3C257328547659F12BFD38B3F3DD20289947530370145FE6386473)
