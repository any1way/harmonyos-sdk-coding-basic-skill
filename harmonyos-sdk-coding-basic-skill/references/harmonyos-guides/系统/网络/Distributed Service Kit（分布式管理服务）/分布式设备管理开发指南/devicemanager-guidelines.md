...
scraped_at: 2026-06-01T11:19:44+08:00
doc_updated_at: 2026-05-28
content_hash: sha256:fc73d074ab81ab44fe41f9298776a0c12087c738f9f690639e82757152454e0b
---
## 分布式设备管理简介
...
2. 导入abilityAccessCtrl模块，用于获取权限申请的能力。
   ...
   ```
3. 分布式数据同步权限的授权方式为user\_grant，因此需要调用[requestPermissionsFromUser()](<../../../../../harmonyos-references/Ability Kit（程序框架服务）/ArkTS API/通用能力的接口(推荐)/@ohos.abilityAccessCtrl (程序访问控制管理)/js-apis-abilityaccessctrl.md#requestpermissionsfromuser9>)接口，以动态弹窗的方式向用户申请授权。
   ...
   [Index.ets](https://gitcode.com/HarmonyOS_Samples/guide-snippets/blob/HarmonyOS-feature-20260402/DistributedAppDev/DistributedAuthentication/entry/src/main/ets/pages/Index.ets#L71-L105)
...
发现周边同局域网或者开启蓝牙的设备。详细信息参见：[startDiscovering](<../../../../../harmonyos-references/网络/Distributed Service Kit（分布式管理服务）/ArkTS API/@ohos.distributedDeviceManager (设备管理)/js-apis-distributeddevicemanager.md#startdiscovering>)。
...
4. 创建设备管理实例，设备管理实例是分布式设备管理方法的调用入口。
   ...
   [RemoteDeviceModel.ets](https://gitcode.com/HarmonyOS_Samples/guide-snippets/blob/HarmonyOS-feature-20260402/DistributedAppDev/DistributedAuthentication/entry/src/main/ets/model/RemoteDeviceModel.ets#L66-L89)
5. 注册发现设备的回调，调用发现接口发现周边设备。发现状态持续两分钟，超过两分钟，会停止发现，最大发现数量99个。
   ...
   [RemoteDeviceModel.ets](https://gitcode.com/HarmonyOS_Samples/guide-snippets/blob/HarmonyOS-feature-20260402/DistributedAppDev/DistributedAuthentication/entry/src/main/ets/model/RemoteDeviceModel.ets#L260-L301)
6. 发现结束或页面退出时，调用停止发现接口释放发现监听。

   ```
   1. stopDeviceDiscovery(): void {
   2. if (typeof (this.deviceManager) == 'undefined') {
   3. logger.error('[DeviceManager.RemoteDeviceModel] deviceManager has not initialized');
   4. return;
   5. }
   ...
   7. try {
   8. this.deviceManager.stopDiscovering();
   9. this.deviceManager.off('discoverSuccess');
   10. this.deviceManager.off('discoverFailure');
   11. } catch (err) {
   12. let error: BusinessError = err as BusinessError;
   13. logger.error('[DeviceManager.RemoteDeviceModel] stopDeviceDiscovery failed err: ' + error.toString());
   14. }
   15. }
   ```

   [RemoteDeviceModel.ets](https://gitcode.com/HarmonyOS_Samples/guide-snippets/blob/HarmonyOS-feature-20260402/DistributedAppDev/DistributedAuthentication/entry/src/main/ets/model/RemoteDeviceModel.ets#L303-L319)
...
设备绑定。详细信息参见：[bindTarget](<../../../../../harmonyos-references/网络/Distributed Service Kit（分布式管理服务）/ArkTS API/@ohos.distributedDeviceManager (设备管理)/js-apis-distributeddevicemanager.md#bindtarget>)。
...
3. 选择不可信设备id，发起设备绑定。
   ...
   [RemoteDeviceModel.ets](https://gitcode.com/HarmonyOS_Samples/guide-snippets/blob/HarmonyOS-feature-20260402/DistributedAppDev/DistributedAuthentication/entry/src/main/ets/model/RemoteDeviceModel.ets#L338-L371)
...
设备信息查询。详细信息参见：[getAvailableDeviceListSync](<../../../../../harmonyos-references/网络/Distributed Service Kit（分布式管理服务）/ArkTS API/@ohos.distributedDeviceManager (设备管理)/js-apis-distributeddevicemanager.md#getavailabledevicelistsync>)。
...
4. 查询周围上线并且可信的设备。
   ...
   [RemoteDeviceModel.ets](https://gitcode.com/HarmonyOS_Samples/guide-snippets/blob/HarmonyOS-feature-20260402/DistributedAppDev/DistributedAuthentication/entry/src/main/ets/model/RemoteDeviceModel.ets#L114-L133)
...
设备上下线监听。详细信息参见：[on('deviceStateChange')](<../../../../../harmonyos-references/网络/Distributed Service Kit（分布式管理服务）/ArkTS API/@ohos.distributedDeviceManager (设备管理)/js-apis-distributeddevicemanager.md#ondevicestatechange>)。
...
4. 创建设备管理实例，设备管理实例是分布式设备管理方法的调用入口，并注册设备上下线回调。
   ...
   [RemoteDeviceModel.ets](https://gitcode.com/HarmonyOS_Samples/guide-snippets/blob/HarmonyOS-feature-20260402/DistributedAppDev/DistributedAuthentication/entry/src/main/ets/model/RemoteDeviceModel.ets#L168-L203)
