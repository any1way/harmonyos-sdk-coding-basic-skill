...
scraped_at: 2026-06-11T14:47:46+08:00
doc_updated_at: 2026-05-26
content_hash: sha256:c06e8e17f0c188cf070dde4bf2ce315d846c6f940142f5ade27a0fc92cc40b6a
---
## 简介
...
* **UIAbility**
  ...
  [UIAbility](<../../../../../应用框架/Ability Kit（程序框架服务）/Stage模型开发指导/Stage模型应用组件/UIAbility组件/UIAbility组件概述/uiability-overview.md>)描述应用程序的界面交互能力，负责管理应用界面的生命周期、用户交互以及界面渲染等任务。
* **字节流**

  字节流是数据类型为[ArrayBuffer](../../../../../应用框架/ArkTS（方舟编程语言）/ArkTS并发/并发线程间通信/线程间通信对象/ArrayBuffer对象/arraybuffer-object.md)类型的数据。可以被用于存储二进制数据，例如图像或音频数据。
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/9c/v3/3bOfBAqMQvqT1PgaoIO8JA/zh-cn_image_0000002622698407.png?HW-CC-KV=V1&HW-CC-Date=20260611T064745Z&HW-CC-Expire=86400&HW-CC-Sign=D03BCF2A99A308DB59B8D9D1E0A3CED22E87ACD6BFAB462DA8E4594B64F23322)
...
可登录华为账号的设备A和设备B，设备间需要组网成功（设备组网通过调用[Device Manager](../../分布式设备管理开发指南/devicemanager-guidelines.md)的接口实现）。
...
4. 打开设备A和设备B的Wi-Fi和蓝牙。如果登录同一个华为账号，则设备间会进行自组网；非同账号环境下，需先通过[设备发现](../../分布式设备管理开发指南/devicemanager-guidelines.md#设备发现开发指导)和[设备绑定](../../分布式设备管理开发指南/devicemanager-guidelines.md#设备绑定开发指导)建立可信关系以完成组网。
...
应用跨设备连接管理接口如下表所示。具体API说明详见API参考：[abilityConnectionManager](<../../../../../../harmonyos-references/Distributed Service Kit（分布式管理服务）/ArkTS API/@ohos.distributedsched.abilityConnectionManager (应用多端协同管理)/js-apis-distributed-_62841863.md>)。
...
设备A上的应用，需要发现并选择设备B的[networkId](<../../../../../../harmonyos-references/Distributed Service Kit（分布式管理服务）/ArkTS API/@ohos.distributedDeviceManager (设备管理)/js-apis-distributeddevicemanager.md#devicebasicinfo>)来作为协同接口的入参。可调用分布式设备管理模块接口，进行对端设备的发现和选择，详情可参考[设备信息查询开发指导](../../分布式设备管理开发指南/devicemanager-guidelines.md#设备信息查询开发指导)。
...
应用申请[长时任务(ArkTS)](<../../../../../应用框架/Background Tasks Kit（后台任务开发服务）/长时任务(ArkTS)/continuous-task.md>)，消除此限制。
