...
content_hash: sha256:e01aa3ce1d13b3849f69c27be12be45b2c58f0f30d5c98ef343a8a056629b775
---
## 简介
...
两个设备的交互实现如下，在使用[linkEnhance](<../../../../../../harmonyos-references/Distributed Service Kit（分布式管理服务）/ArkTS API/@ohos.distributedsched.linkEnhance (增强连接)/js-apis-link-enhance.md>)能力后，当两端同时发起连接时，会自动识别合并底层多余物理链路，减少实际物理链路的个数，减少蓝牙链路资源的消耗，增加可用连接数量。

![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/77/v3/RPlqJCgdRRa-K3LImHDRcg/zh-cn_image_0000002592218846.png?HW-CC-KV=V1&HW-CC-Date=20260611T064745Z&HW-CC-Expire=86400&HW-CC-Sign=DEDFC21401F7D0ABD0041B4DCE9DB310AB255C811D06E01FAEAB030A3C6555E8)

### 约束与限制

* 设备互联时需要开启蓝牙功能。
* 通过蓝牙广播/扫描接口获取对端设备BLE MAC。蓝牙BLE接口参见[查找设备](<../../../Connectivity Kit（短距通信服务）/蓝牙/低功耗蓝牙/查找设备/ble-development-guide.md>)。
...
常用接口说明如下表。更多接口的详细介绍参考@ohos.distributedsched.linkEnhance[增强连接](<../../../../../../harmonyos-references/Distributed Service Kit（分布式管理服务）/ArkTS API/@ohos.distributedsched.linkEnhance (增强连接)/js-apis-link-enhance.md>)。
...
* 服务端开启蓝牙后，创建Server对象，并调用[start()接口](<../../../../../../harmonyos-references/Distributed Service Kit（分布式管理服务）/ArkTS API/@ohos.distributedsched.linkEnhance (增强连接)/js-apis-link-enhance.md#start>)开启服务，让服务端处于可连接状态，通过注册的事件监听，监听事件的变化通知。
* 客户端开启蓝牙后，创建Connection对象，并调用[connect()接口](<../../../../../../harmonyos-references/Distributed Service Kit（分布式管理服务）/ArkTS API/@ohos.distributedsched.linkEnhance (增强连接)/js-apis-link-enhance.md#connect>)发起连接，通过注册的事件监听，监听事件的变化通知。
* 连接成功后，可以使用[sendData接口](<../../../../../../harmonyos-references/Distributed Service Kit（分布式管理服务）/ArkTS API/@ohos.distributedsched.linkEnhance (增强连接)/js-apis-link-enhance.md#senddata>)发送数据。
