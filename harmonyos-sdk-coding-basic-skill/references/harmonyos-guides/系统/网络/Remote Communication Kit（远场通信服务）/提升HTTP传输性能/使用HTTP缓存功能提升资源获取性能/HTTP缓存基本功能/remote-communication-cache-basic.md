...
scraped_at: 2026-06-01T11:21:13+08:00
doc_updated_at: 2026-04-29
content_hash: sha256:aa2defa82d0f6b86cc87d762068eb1214da4b47b6361b743a7ba54da1f6382b7
---
...
在需要频繁获取网络热点数据的场景下，网络请求耗时在总时长中占比较大。开发者可以通过创建[ResponseCache](<../../../../../../../harmonyos-references/网络/Remote Communication Kit（远场通信服务）/ArkTS API/rcp（数据请求）/remote-communication-rcp.md#responsecache>)实例，并将其配置到Session中，即可使用HTTP缓存功能，提升资源加载性能。
...
1. 导入模块。
   ...
   ```
2. 创建[ResponseCache](<../../../../../../../harmonyos-references/网络/Remote Communication Kit（远场通信服务）/ArkTS API/rcp（数据请求）/remote-communication-rcp.md#responsecache>)实例。其中，pathToFolder即HTTP缓存响应记录文件路径，'/path/dir'请根据实际情况替换为需要存储HTTP缓存的沙箱路径。
...
Remote Communication Kit提供了丰富的缓存过期策略配置项供开发者使用，包括永不过期、绝对时间、相对时间以及滑动时间过期策略，具体API说明详见[接口文档](<../../../../../../../harmonyos-references/网络/Remote Communication Kit（远场通信服务）/ArkTS API/rcp（数据请求）/remote-communication-rcp.md#expirationpolicy>)。开发者可以根据业务特性选择不同的缓存过期策略，灵活调整缓存生命周期。下面以相对时间过期策略配置为例，讲述如何配置缓存过期策略，并体会对应的缓存逻辑。
...
1. 导入模块。
   ...
   1. import { rcp } from '@kit.RemoteCommunicationKit';
   ```
2. 创建[ResponseCache](<../../../../../../../harmonyos-references/网络/Remote Communication Kit（远场通信服务）/ArkTS API/rcp（数据请求）/remote-communication-rcp.md#responsecache>)实例。其中，pathToFolder即HTTP缓存响应记录文件路径，'/path/dir'请根据实际情况替换为需要存储HTTP缓存的沙箱路径。defaultExpirationPolicy即默认过期策略，示例代码中配置了时间间隔3秒的相对时间过期策略，具体API说明详见[RelativeTimeExpirationPolicy](<../../../../../../../harmonyos-references/网络/Remote Communication Kit（远场通信服务）/ArkTS API/rcp（数据请求）/remote-communication-rcp.md#relativetimeexpirationpolicy>)。
