...
scraped_at: 2026-06-01T11:21:01+08:00
doc_updated_at: 2026-04-20
content_hash: sha256:b4739c20bd140c67c662ab54bb007c9940f758fc6150a6af17421da81dae6811
---
## 场景介绍
...
* dnsRules（配置DNS规则）
  ...
  自定义DNS服务器（[DnsServers](<../../../../../../../../harmonyos-references/网络/Remote Communication Kit（远场通信服务）/ArkTS API/rcp（数据请求）/remote-communication-rcp.md#dnsservers>)）：可指定自定义的DNS服务器提供解析服务。

  自定义静态DNS（[StaticDnsRules](<../../../../../../../../harmonyos-references/网络/Remote Communication Kit（远场通信服务）/ArkTS API/rcp（数据请求）/remote-communication-rcp.md#staticdnsrules>)）：当默认的DNS不能正常解析部分域名，就需要手动添加静态DNS。添加静态DNS后，如果hostname匹配，则优先使用指定的地址。

  自定义动态DNS（[DynamicDnsRule](<../../../../../../../../harmonyos-references/网络/Remote Communication Kit（远场通信服务）/ArkTS API/rcp（数据请求）/remote-communication-rcp.md#dynamicdnsrule>)）：除了添加静态DNS外，还可以添加动态DNS。动态DNS可看作一个可以根据hostname和port直接返回IP地址的函数，如果设置，则优先使用函数中返回的地址。
...
* dnsOverHttps
  ...
  DNS over HTTPS配置（[DnsOverHttpsConfiguration](<../../../../../../../../harmonyos-references/网络/Remote Communication Kit（远场通信服务）/ArkTS API/rcp（数据请求）/remote-communication-rcp.md#dnsoverhttpsconfiguration>)）：配置HTTPS上的DNS（DOH）设置，以加密的HTTPS协议进行DNS解析请求，避免原始DNS协议中用户的DNS解析请求被窃听或者修改的问题，实现保护用户隐私的目的。如果设置，则优先使用DNS服务器解析的地址。
