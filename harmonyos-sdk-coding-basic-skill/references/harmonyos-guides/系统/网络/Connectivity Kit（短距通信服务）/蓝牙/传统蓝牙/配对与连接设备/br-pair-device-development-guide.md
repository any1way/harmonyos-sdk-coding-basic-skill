...
scraped_at: 2026-06-11T14:47:20+08:00
doc_updated_at: 2026-03-09
content_hash: sha256:a6a9d6a4722f53423476a6f337d19a1d3c923c26348e04f30295ebc9d8863c4c
---
## 简介
...
需要申请权限ohos.permission.ACCESS\_BLUETOOTH。如何配置和申请权限，具体操作请参考[声明权限](../../../../../安全/程序访问控制/应用权限管控/申请应用权限/声明权限/declare-permissions.md)和[向用户申请授权](../../../../../安全/程序访问控制/应用权限管控/申请应用权限/向用户申请授权/request-user-authorization.md)。
...
通过订阅配对状态变化事件，可以获取实时的配对状态。在整个配对过程中，涉及多种状态的跃迁，其中[BOND\_STATE\_BONDED](<../../../../../../../harmonyos-references/网络/Connectivity Kit（短距通信服务）/ArkTS API/@ohos.bluetooth.connection (蓝牙connection模块)/js-apis-bluetooth-connection.md#bondstate>)表示已配对。
...
若目标设备的配对状态是[BOND\_STATE\_INVALID](<../../../../../../../harmonyos-references/网络/Connectivity Kit（短距通信服务）/ArkTS API/@ohos.bluetooth.connection (蓝牙connection模块)/js-apis-bluetooth-connection.md#bondstate>)，则可以主动配对目标设备。

* 目标设备可以通过发现设备流程获取，详情请参考：[传统蓝牙查找设备](../查找设备/br-discovery-development-guide.md)或者[低功耗蓝牙查找设备](../../低功耗蓝牙/查找设备/ble-development-guide.md)。

配对过程中，系统会弹出对话框。不同配对类型，对话框样式可能不一样，其中“确认配对密钥（Confirm Passkey）”模式如下图1。若用户同意授权，才能配对成功。

![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/db/v3/BtiySNiWTT-7-wlp4eWGJA/zh-cn_image_0000002592218842.png?HW-CC-KV=V1&HW-CC-Date=20260611T064719Z&HW-CC-Expire=86400&HW-CC-Sign=131B815796E594E5D58182F325FBF689985F2408E56EE8D009A34F22EEF6C2DB)

**图1** 蓝牙配对请求对话框

蓝牙设备的实际MAC地址属于用户的隐私信息，在发现设备过程中，蓝牙子系统会给每个蓝牙外设分配一个虚拟MAC地址，并保存该虚拟MAC地址与外设实际MAC地址的映射关系。

若开发者不知道目标设备的[地址类型](<../../../../../../../harmonyos-references/网络/Connectivity Kit（短距通信服务）/ArkTS API/@ohos.bluetooth.common (蓝牙common模块)/js-apis-bluetooth-common.md#bluetoothaddresstype>)，推荐使用API version 20及以前的配对方式发起配对，详情请见[connection.pairDevice](<../../../../../../../harmonyos-references/网络/Connectivity Kit（短距通信服务）/ArkTS API/@ohos.bluetooth.connection (蓝牙connection模块)/js-apis-bluetooth-connection.md#connectionpairdevice>)。
...
若开发者已知目标设备的[地址类型](<../../../../../../../harmonyos-references/网络/Connectivity Kit（短距通信服务）/ArkTS API/@ohos.bluetooth.common (蓝牙common模块)/js-apis-bluetooth-common.md#bluetoothaddresstype>)，推荐使用API version 21开始支持的配对方式发起配对，详情请见[connection.pairDevice](<../../../../../../../harmonyos-references/网络/Connectivity Kit（短距通信服务）/ArkTS API/@ohos.bluetooth.connection (蓝牙connection模块)/js-apis-bluetooth-connection.md#connectionpairdevice21>)。
...
若应用配对完目标设备后，可以调用[connectAllowedProfiles](<../../../../../../../harmonyos-references/网络/Connectivity Kit（短距通信服务）/ArkTS API/@ohos.bluetooth.connection (蓝牙connection模块)/js-apis-bluetooth-connection.md#connectionconnectallowedprofiles16>)，发起连接该设备支持的profile能力（只包括A2DP、HFP和HID）。若应用需要使用SPP连接，请参考[连接和传输数据](../连接和传输数据/spp-development-guide.md)。

* 蓝牙子系统会在配对过程中查询和保存目标设备支持的所有profile能力。
* 配对完成后，应用可以主动查询目标设备的profile能力，需调用[getRemoteProfileUuids](<../../../../../../../harmonyos-references/网络/Connectivity Kit（短距通信服务）/ArkTS API/@ohos.bluetooth.connection (蓝牙connection模块)/js-apis-bluetooth-connection.md#connectiongetremoteprofileuuids12>)。若存在应用需要的能力，则可以在配对完成后30s内，发起连接目标设备的profile。
