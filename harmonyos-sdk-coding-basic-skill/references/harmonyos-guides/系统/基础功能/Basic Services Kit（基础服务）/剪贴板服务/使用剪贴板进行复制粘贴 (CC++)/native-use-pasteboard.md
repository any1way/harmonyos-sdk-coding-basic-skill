...
content_hash: sha256:a8c0efcac7d7fae97b7e23881c347a2c5289fd1528b83bbfed60636a6b682907
---
## 场景介绍
...
* [**OH\_PasteboardObserver**](<../../../../../../harmonyos-references/基础功能/Basic Services Kit（基础服务）/C API/结构体/OH_PasteboardObserver/capi-pasteboard-oh-pasteboardobserver.md>)：剪贴板数据变更观察者对象，用以监听剪贴板数据变更事件。
* [**OH\_Pasteboard**](<../../../../../../harmonyos-references/基础功能/Basic Services Kit（基础服务）/C API/结构体/OH_Pasteboard/capi-pasteboard-oh-pasteboard.md>)：剪贴板对象，用来进行查询、写入等操作。
* [**OH\_UdmfData**](<../../../../../../harmonyos-references/ArkData（方舟数据管理）/C API/结构体/OH_UdmfData/capi-udmf-oh-udmfdata.md>)：统一数据对象。

## 约束限制

* 剪贴板内容包含剪贴板系统服务元数据和应用设置的数据，总大小上限默认为128MB，PC/2in1设备可通过系统配置修改上限，有效范围为1MB~2GB。
* 为保证剪贴板数据的准确性，同一时间只能支持一个复制操作。
* 当前支持的数据类型：纯文本类型(OH\_UdsPlainText)、超文本标记语言类型(OH\_UdsHtml)、文件Uri类型(OH\_UdsFileUri)、像素图片类型(OH\_UdsPixelMap)、超链接类型(OH\_UdsHyperlink)、桌面图标类型(OH\_UdsAppItem)、自定义类型。ArkTS接口与NDK接口支持数据类型不完全一致，使用时须匹配接口支持类型，详情见[ArkTS接口与NDK接口数据类型对应关系](../使用剪贴板进行复制粘贴/use-pasteboard-to-copy-and-paste.md#arkts接口与ndk接口数据类型对应关系)。
...
* API version 12中新增的复制、粘贴接口[setUnifiedData](<../../../../../../harmonyos-references/基础功能/Basic Services Kit（基础服务）/ArkTS API/数据文件处理/@ohos.pasteboard (剪贴板)/js-apis-pasteboard.md#setunifieddata12>)/[getUnifiedData](<../../../../../../harmonyos-references/基础功能/Basic Services Kit（基础服务）/ArkTS API/数据文件处理/@ohos.pasteboard (剪贴板)/js-apis-pasteboard.md#getunifieddata12>)与本文档中的复制、粘贴接口[OH\_Pasteboard\_SetData](<../../../../../../harmonyos-references/基础功能/Basic Services Kit（基础服务）/C API/头文件/oh_pasteboard.h/capi-oh-pasteboard-h.md#oh_pasteboard_setdata>)/[OH\_Pasteboard\_GetData](<../../../../../../harmonyos-references/基础功能/Basic Services Kit（基础服务）/C API/头文件/oh_pasteboard.h/capi-oh-pasteboard-h.md#oh_pasteboard_getdata>)当前相互独立，进行写入、读取操作时请使用对应配套接口。

## 接口说明

详细接口见[Pasteboard文档](<../../../../../../harmonyos-references/基础功能/Basic Services Kit（基础服务）/C API/模块/Pasteboard/capi-pasteboard.md>)。
...
2. 引用头文件。
   ...
   [napi\_init.cpp](https://gitcode.com/HarmonyOS_Samples/guide-snippets/blob/HarmonyOS-feature-20260402/pasteboard/pasteboard_NDK_sample/entry/src/main/cpp/napi_init.cpp#L18-L26)
3. 定义剪贴板变化监听的回调函数。
   ...
   [napi\_init.cpp](https://gitcode.com/HarmonyOS_Samples/guide-snippets/blob/HarmonyOS-feature-20260402/pasteboard/pasteboard_NDK_sample/entry/src/main/cpp/napi_init.cpp#L32-L43)
4. 订阅剪贴板变化。
   ...
   [napi\_init.cpp](https://gitcode.com/HarmonyOS_Samples/guide-snippets/blob/HarmonyOS-feature-20260402/pasteboard/pasteboard_NDK_sample/entry/src/main/cpp/napi_init.cpp#L44-L56)
5. 向剪贴板写入数据。
   ...
   [napi\_init.cpp](https://gitcode.com/HarmonyOS_Samples/guide-snippets/blob/HarmonyOS-feature-20260402/pasteboard/pasteboard_NDK_sample/entry/src/main/cpp/napi_init.cpp#L57-L97)
6. 从剪贴板读取数据。
   ...
   [napi\_init.cpp](https://gitcode.com/HarmonyOS_Samples/guide-snippets/blob/HarmonyOS-feature-20260402/pasteboard/pasteboard_NDK_sample/entry/src/main/cpp/napi_init.cpp#L98-L141)
