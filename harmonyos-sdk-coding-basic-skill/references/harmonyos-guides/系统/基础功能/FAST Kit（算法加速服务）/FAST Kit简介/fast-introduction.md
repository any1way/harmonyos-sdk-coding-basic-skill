...
scraped_at: 2026-06-01T11:22:20+08:00
doc_updated_at: 2026-04-30
content_hash: sha256:06b3377d6a918ff981b0511bd0e7424df8c4f4f121be8f55c3a490a88dd75987
---
FAST Kit（算法加速服务）以理论计算机为基础、面向开发者提供算法加速能力。当前开放线段表、矩形划分求解器与数字信号处理（DSP）接口，支撑应用的开发体验和用户体验提升。

当前FAST Kit包括以下功能域：

* 高阶数据结构（Advanced Data Structure）加速功能域旨在融合理论计算机科学中具有理论保证的高级数据结构与现代硬件特性，当前提供[线段表数据结构](../使用SegmentMap查询维护区间信息/fast-segment-map.md)，[并发哈希表数据结构](../使用ConcurrentHashmap在多线程下完成键值信息的查找维护/fast-concurrent-hashmap.md)。
* 规划求解（Solver）加速功能域旨在将理论计算机科学与运筹学中的优化求解能力运用到鸿蒙生态中的各类场景，当前提供[矩形划分求解器](../使用RectPartition求解矩形划分/fast-rect-partition.md)。
* 数字信号处理（Digital Signal Processing）加速功能域旨在提供高性能的向量运算及二阶IIR滤波器能力，当前提供[向量运算](../使用DSP进行向量计算/fast-dsp-vector-calculation.md)，[二阶IIR滤波](../使用DSP进行二阶IIR滤波/fast-dsp-iir-filter.md)。
