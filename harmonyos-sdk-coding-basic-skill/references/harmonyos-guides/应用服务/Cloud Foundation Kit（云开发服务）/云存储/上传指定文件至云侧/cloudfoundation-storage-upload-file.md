...
scraped_at: 2026-06-01T15:02:25+08:00
doc_updated_at: 2026-04-20
content_hash: sha256:0eedbfbfdb9825d0586df7dbd4731155036860997864a916c03905d0d18cc452
---
...
1. 选择待上传的文件，下方示例代码中使用[photoAccessHelper.PhotoViewPicker](<../../../../../harmonyos-references/Media Library Kit（媒体文件管理服务）/ArkTS API/@ohos.file.photoAccessHelper (相册管理模块)/Class (PhotoViewPicker)/arkts-apis-photoaccess_90850778.md>)指定需要上传的文件。
2. 将待上传的文件复制到[context.cacheDir](<../../../../../harmonyos-references/Ability Kit（程序框架服务）/ArkTS API/接口依赖的元素及定义/application/Context (Stage模型的上下文基类)/js-apis-inner-application-context.md#属性>)目录下。

   说明

   由于StorageBucket.uploadFile接口传入参数localPath只能设置为context.cacheDir目录下的文件路径，所以上传前需要先将文件复制到context.cacheDir目录下。
3. 调用[StorageBucket.uploadFile](<../../../../../harmonyos-references/Cloud Foundation Kit（云开发服务）/ArkTS API/cloudStorage (云存储模块)/cloudfoundation-cloudstorage.md#uploadfile>)接口创建上传任务，监听上传任务的progress、completed、failed等事件。
4. 启动上传任务。
