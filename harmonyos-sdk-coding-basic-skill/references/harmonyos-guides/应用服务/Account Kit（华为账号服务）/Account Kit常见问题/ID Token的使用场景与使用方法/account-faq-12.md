...
scraped_at: 2026-06-11T15:03:16+08:00
doc_updated_at: 2026-06-03
content_hash: sha256:3a15d99649d831fbcb22722f4725b7c1c5e9bc79131dad7dc0ee613a2800091e
---
...
| sub | string | 是 | 即用户的UnionID。同一个开发者下的所有应用，此参数均相同。具体格式要求请参考[OpenID和UnionID的格式说明](../OpenID和UnionID的格式说明/account-faq-9.md)。 |
...
| openid | string | 是 | 用户OpenID。具体格式要求请参考[OpenID和UnionID的格式说明](../OpenID和UnionID的格式说明/account-faq-9.md)。 |
| nonce | string | 否 | 防重放攻击随机值。详情请参考[LoginWithHuaweiIDRequest](<../../../../../harmonyos-references/Account Kit（华为账号服务）/ArkTS API/authentication (华为账号应用统一认证服务)/account-api-authentication.md#loginwithhuaweiidrequest>)或[AuthorizationWithHuaweiIDRequest](<../../../../../harmonyos-references/Account Kit（华为账号服务）/ArkTS API/authentication (华为账号应用统一认证服务)/account-api-authentication.md#authorizationwithhuaweiidrequest>)的nonce字段说明。 |
| picture | string | 否 | 用户头像图片链接。该字段返回场景：[AuthorizationWithHuaweiIDRequest](<../../../../../harmonyos-references/Account Kit（华为账号服务）/ArkTS API/authentication (华为账号应用统一认证服务)/account-api-authentication.md#authorizationwithhuaweiidrequest>)中的scopes包含profile。 |
| display\_name | string | 否 | 华为账号对应的昵称，没有昵称则取匿名化的邮箱或手机号。该字段返回场景：  [AuthorizationWithHuaweiIDRequest](<../../../../../harmonyos-references/Account Kit（华为账号服务）/ArkTS API/authentication (华为账号应用统一认证服务)/account-api-authentication.md#authorizationwithhuaweiidrequest>)中的scopes包含profile。 |
| nickname | string | 否 | 华为账号对应的昵称。该字段返回场景：  [AuthorizationWithHuaweiIDRequest](<../../../../../harmonyos-references/Account Kit（华为账号服务）/ArkTS API/authentication (华为账号应用统一认证服务)/account-api-authentication.md#authorizationwithhuaweiidrequest>)中的scopes包含profile。 |
...
10. // 解析ID Token并验证
11. decodeIdToken(idToken: string): void {
12. const parts = idToken.split('.');
13. if (parts.length !== 3) {
14. return;
15. }
16. const idTokenObj: Record<string, Object> = {};
17. // ID Token头部
18. idTokenObj['header'] = JSON.parse(this.decodeBase64(parts[0]));
19. // ID Token负载
20. idTokenObj['payload'] = JSON.parse(this.decodeBase64(parts[1]));
21. // ID Token签名
22. idTokenObj['signature'] = parts[2];
23. const header: Record<string, string> = idTokenObj['header'] as Record<string, string>;
24. // 从负载中解析出nonce等数据
25. const payLoad: Record<string, string> = idTokenObj['payload'] as Record<string, string>;
26. const nonce: string = payLoad['nonce'];
27. // nonce可用于与请求体属性中的nonce作一致性比对，防止重放攻击
28. // 应用Client ID，使用前请替换
29. const CLIENT_ID: string = '<应用Client ID>';
30. const ID_TOKEN_ISSUE: string = 'https://accounts.huawei.com';
31. const iss: string = payLoad['iss'];
32. const aud: string = payLoad['aud'];
33. if(iss !== ID_TOKEN_ISSUE){
34. // 验证失败，开发者处理失败场景
35. hilog.error(0x0000, 'testTag', 'Failed to check iss');
36. return;
37. }
38. if(aud !== CLIENT_ID){
39. // 验证失败，开发者处理失败场景
40. hilog.error(0x0000, 'testTag', 'Failed to check aud');
41. return;
42. }
43. // 验证签名
44. this.checkSignature(idToken, header['kid'], header['alg']);
45. }

47. private stringToUint8Array(str: string): Uint8Array {
48. const arr: number[] = [];
49. for (let i = 0, j = str.length; i < j; ++i) {
50. arr.push(str.charCodeAt(i));
51. }
52. const tmpUint8Array: Uint8Array = new Uint8Array(arr);
53. return tmpUint8Array;
54. }

56. // 验签方法
57. private checkSignature(idToken: string, kid: string, alg: string) {
58. if (!idToken) {
59. return;
60. }
61. const parts = idToken.split('.');
62. if (parts.length !== 3) {
63. return;
64. }
65. const url = 'https://oauth-login.cloud.huawei.com/oauth2/v3/certs';
66. // 创建http请求，应用需在module.json5文件中先申请“ohos.permission.INTERNET”网络权限，请求才能发送成功
67. const httpRequest = http.createHttp();
68. httpRequest.request(url, (err, data) => {
69. if (err) {
70. hilog.error(0x0000, 'testTag', `Failed to httpRequest. Code: ${err.code}, message: ${err.message}`);
71. httpRequest.destroy();
72. return;
73. }
74. let nStr = '';
75. let eStr = '';
76. const keys: object[] = JSON.parse(data.result as string)['keys'];
77. for (let item of keys) {
78. if (kid === item['kid']) {
79. nStr = item['n'];
80. eStr = item['e'];
81. break;
82. }
83. }
84. const nBigInt = '0x' + buffer.from(nStr, 'base64url').toString('hex');
85. const eBigInt = '0x' + buffer.from(eStr, 'base64url').toString('hex');
86. const dsaCommonSpec: cryptoFramework.RSACommonParamsSpec = {
87. algName: 'RSA',
88. specType: cryptoFramework.AsyKeySpecType.COMMON_PARAMS_SPEC,
89. n: BigInt(nBigInt),
90. }
91. const rsaKeyPairSpec: cryptoFramework.RSAPubKeySpec = {
92. algName: 'RSA',
93. specType: cryptoFramework.AsyKeySpecType.PUBLIC_KEY_SPEC,
94. params: dsaCommonSpec,
95. pk: BigInt(eBigInt),
96. }
97. try {
98. const asyKeyGeneratorBySpec = cryptoFramework.createAsyKeyGeneratorBySpec(rsaKeyPairSpec);
99. asyKeyGeneratorBySpec.generatePubKey((error, publicKey) => {
100. if (error) {
101. return;
102. }
...
104. return;
105. }
106. const idTokenSign = parts[2];
107. const idTokenSignArr: cryptoFramework.DataBlob =
108. { data: new Uint8Array(buffer.from(idTokenSign, 'base64url').buffer) };
109. const idToken = parts[0] + '.' + parts[1];
110. const idTokenArr: cryptoFramework.DataBlob = { data: this.stringToUint8Array(idToken) };
111. const verifier = alg === 'PS256' ? cryptoFramework.createVerify('RSA2048|PSS|SHA256|MGF1_SHA256') :
112. cryptoFramework.createVerify('RSA2048|PKCS1|SHA256');
113. verifier.init(publicKey, () => {
114. verifier.verify(idTokenArr, idTokenSignArr, (data) => {
115. // 打印验签结果，结果为true则验签通过
116. hilog.info(0x0000, 'testTag', 'verify result is: %{public}s', data);
117. });
118. });
119. })
120. } catch (err) {
121. hilog.error(0x0000, 'testTag', `Failed to verify. Code: ${err.code}, message: ${err.message}`);
122. }
