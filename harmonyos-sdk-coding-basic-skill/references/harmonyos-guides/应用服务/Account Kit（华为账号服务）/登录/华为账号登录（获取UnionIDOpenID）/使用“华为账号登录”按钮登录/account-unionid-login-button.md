...
content_hash: sha256:e9f8f3069e9390e5f20a2a99d539c8bed405aa6eeb975d059cfb445eedd967d9
---
## 场景介绍
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/c4/v3/oe4KT-Y-Qu2wHsGpxRwJWw/zh-cn_image_0000002592379116.png?HW-CC-KV=V1&HW-CC-Date=20260611T070237Z&HW-CC-Expire=86400&HW-CC-Sign=01F6E04CD24BBD9774D925D16DBA57CE50BB796B14F8B7D75107426B3E33992A)
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/52/v3/Q2dAYCRjTiOmngwkZi6GEw/zh-cn_image_0000002622858623.png?HW-CC-KV=V1&HW-CC-Date=20260611T070237Z&HW-CC-Expire=86400&HW-CC-Sign=3478F1817884E1EE2FA92166D64FE4C2AA347E35DE0F2D6AB80537734A2933E6)
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/61/v3/4jpkMnc1Rd6MUPXUIUxVaw/zh-cn_image_0000002622698745.png?HW-CC-KV=V1&HW-CC-Date=20260611T070237Z&HW-CC-Expire=86400&HW-CC-Sign=E6BB6C5D51D03D416468EDA54D414E245527E93E010A865830A909DF7B045F3D)

流程说明：

1. 调用登录按钮展示登录页阶段（序号1-3）：

   1. 用户打开应用进行登录，应用设置[LoginType](<../../../../../../harmonyos-references/Account Kit（华为账号服务）/ArkTS组件/loginComponentManager (华为账号登录组件管理)/account-api-component-manager.md#logintype>)类型为LoginType.ID后拉起应用自己的登录页并展示“华为账号登录”按钮，用户点击按钮，请求华为账号授权信息。
2. 用户点击登录阶段（序号4-6）：
...
3. 用户关联应用账号阶段（序号7-16）：
   ...
   1. 应用服务端通过Authorization Code获取到Access Token，再使用Access Token调用[解析凭证接口](<../../../../../../harmonyos-references/Account Kit（华为账号服务）/REST API/开放接口调用凭证/解析凭证/account-api-get-token-info.md#接口原型>)获取用户相关信息。通过Authorization Code凭证获取用户信息可以有效避免黑客通过数据遍历、身份伪造、重放攻击等手段导致的安全风险。
...
| 接口名 | 描述 |
| --- | --- |
| [LoginWithHuaweiIDButton](<../../../../../../harmonyos-references/Account Kit（华为账号服务）/ArkTS组件/LoginWithHuaweiIDButton (华为账号Button登录组件)/account-api-huawei-id-button.md#loginwithhuaweiidbutton>) | 华为账号Button登录组件。  当前该组件支持Icon类型按钮、纯文本按钮、Icon和文本混合按钮，如果仍然不能满足开发者的诉求，可以使用[Style](<../../../../../../harmonyos-references/Account Kit（华为账号服务）/ArkTS组件/loginComponentManager (华为账号登录组件管理)/account-api-component-manager.md#style>)的BUTTON\_CUSTOM值定义按钮的文字颜色和背景色。 |
| [onClickLoginWithHuaweiIDButton](<../../../../../../harmonyos-references/Account Kit（华为账号服务）/ArkTS组件/loginComponentManager (华为账号登录组件管理)/account-api-component-manager.md#onclickloginwithhuaweiidbutton-1>)(callback: AsyncCallback<[HuaweiIDCredential](<../../../../../../harmonyos-references/Account Kit（华为账号服务）/ArkTS组件/loginComponentManager (华为账号登录组件管理)/account-api-component-manager.md#huaweiidcredential>)>): [LoginWithHuaweiIDButtonController](<../../../../../../harmonyos-references/Account Kit（华为账号服务）/ArkTS组件/loginComponentManager (华为账号登录组件管理)/account-api-component-manager.md#loginwithhuaweiidbuttoncontroller>) | 注册华为账号登录按钮的登录事件结果回调。使用callback异步回调。 |
| [setAgreementStatus](<../../../../../../harmonyos-references/Account Kit（华为账号服务）/ArkTS组件/loginComponentManager (华为账号登录组件管理)/account-api-component-manager.md#setagreementstatus-1>)(agreementStatus: [AgreementStatus](<../../../../../../harmonyos-references/Account Kit（华为账号服务）/ArkTS组件/loginComponentManager (华为账号登录组件管理)/account-api-component-manager.md#agreementstatus>)): [LoginWithHuaweiIDButtonController](<../../../../../../harmonyos-references/Account Kit（华为账号服务）/ArkTS组件/loginComponentManager (华为账号登录组件管理)/account-api-component-manager.md#loginwithhuaweiidbuttoncontroller>) | 设置协议状态方法。如果需要用户同意协议才能完成华为账号登录，请先设置协议状态为NOT\_ACCEPTED，当用户同意协议后设置协议状态为ACCEPTED，才可以完成华为账号登录。 |
...
1. 导入[LoginWithHuaweiIDButton](<../../../../../../harmonyos-references/Account Kit（华为账号服务）/ArkTS组件/LoginWithHuaweiIDButton (华为账号Button登录组件)/account-api-huawei-id-button.md>)模块及相关公共模块。
   ...
   ```
2. 调用[LoginWithHuaweiIDButton](<../../../../../../harmonyos-references/Account Kit（华为账号服务）/ArkTS组件/LoginWithHuaweiIDButton (华为账号Button登录组件)/account-api-huawei-id-button.md#loginwithhuaweiidbutton>)组件，展示华为账号登录按钮，用户点击华为账号登录按钮后，应用获取到Authorization Code、ID Token，将数据传给应用服务端，可参考[客户端与服务端交互开发](../../华为账号一键登录（获取手机号和UnionIDOpenID）/account-phone-unionid-login.md#客户端与服务端交互开发)的开发步骤a和b，完成服务端开发。通过Authorization Code凭证获取用户信息可以有效避免黑客通过数据遍历、身份伪造、重放攻击等手段导致的安全风险。应用可以通过公开的网址获取到华为账号服务器发布的公钥，对签名和ID Token中的必要信息进行验证，以证明其没有被篡改过。解析ID Token可参考[ID Token解析与验证](<../../../Account Kit常见问题/ID Token的使用场景与使用方法/account-faq-12.md#解析与验证>)。
...
2. 使用Access Token调用[解析凭证接口](<../../../../../../harmonyos-references/Account Kit（华为账号服务）/REST API/开放接口调用凭证/解析凭证/account-api-get-token-info.md#接口原型>)获取用户的UnionID。

   **Access Token过期处理**

   由于Access Token的有效期仅为60分钟，当Access Token失效或者即将失效时（可通过[REST API错误码](<../../../../../../harmonyos-references/Account Kit（华为账号服务）/REST API/开放接口调用凭证/解析凭证/account-api-get-token-info.md#错误码>)判断），可以使用Refresh Token（有效期180天）通过[刷新用户级凭证接口](<../../../../../../harmonyos-references/Account Kit（华为账号服务）/REST API/开放接口调用凭证/刷新用户级凭证/account-api-obtain-refresh-token.md#接口原型>)向华为账号服务器请求获取新的Access Token。
